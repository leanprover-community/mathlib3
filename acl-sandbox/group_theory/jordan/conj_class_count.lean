...
begin
  ...
  suffices hlc  : ∀ (n : fin (fintype.card α + 1)),
    fintype.card {a : (g.cycle_factors_finset) | sc g a = n} = m.count ↑n,
...
def ψ := ψ_aux g g.cycle_factors_finset (finset.subset.refl g.cycle_factors_finset)
...
begin
  ...
  ψ_aux g s hs uvs x = dite (c ∈ s) (λ h, ((uvs.snd c h) : equiv.perm α) x) (λ h, x),
...
begin
  ...
  { ext ⟨x, hx⟩, rw ← hψ_1 g uv x hx, rw ← hψ_1 g uv' x hx, rw h, },
  { ext c hc x,
    by_cases hx : c = g.cycle_of x,
    { rw ← hψ_2 g uv x c hc hx, rw ← hψ_2 g uv' x c hc hx, rw h, },
  ...
  { intro Hz,
    ...
    { intro x,
      ...
      { intro hx,
        ...
        rw ← Hz', rw equiv.perm.mem_support_cycle_of_iff,
        split, apply equiv.perm.same_cycle.refl, exact hx, },
      { intro hzx,
        let hzx' := id hzx,
        rw ← equiv.perm.cycle_of_mem_cycle_factors_finset_iff at hzx',
        obtain ⟨Hz'⟩ := Hz (g.cycle_of (z x)) hzx',
        specialize Hz' x,
        simp only [equiv.perm.mem_support_cycle_of_iff,
        equiv.perm.same_cycle.refl, true_and, and_true, hzx, iff_true] at Hz',
        rw ← equiv.perm.same_cycle.mem_support_iff Hz',
        exact hzx, }, },
    let u := equiv.perm.subtype_perm z hu,
    let v : Π (c : equiv.perm α), c ∈ g.cycle_factors_finset → ↥(subgroup.zpowers c) := λ c hc, ⟨
        (equiv.perm.of_subtype (z.subtype_perm (classical.some (Hz c hc)))), classical.some_spec (Hz c hc) ⟩,
    use ⟨u,v⟩,
    ext x,
    by_cases hx : x ∈ g.support,
      { rw hψ_2 g ⟨u, v⟩ x (g.cycle_of x) _ rfl,
...
lemma equiv.perm.conj_class_card_mul (g : equiv.perm α) (m : multiset ℕ)
  (hg : g.cycle_type = m) :
...
theorem equiv.perm.conj_class_card (g : equiv.perm α) (m : multiset ℕ)
  (hg : g.cycle_type = m) :
  fintype.card ({ h : equiv.perm α | is_conj g h}) =
  (fintype.card α).factorial / ((fintype.card α - m.sum).factorial * (m.prod *
    (m.dedup.map (λ (n : ℕ), (m.count n).factorial)).prod)) :=
begin
  rw ← equiv.perm.conj_class_card_mul g m hg,
  ...
  { intros a ha,
    have ha' : a ∈ g.cycle_type, simp only [hg, ha],
    apply lt_of_lt_of_le _ (equiv.perm.two_le_of_mem_cycle_type ha'),
    norm_num, },
  apply multiset.prod_pos,
  { intros a, rw multiset.mem_map,
    rintro ⟨b, hb, rfl⟩, apply nat.factorial_pos, },
end
...
begin
  ...
  { -- nonempty case
    ...
    rw ← equiv.perm.conj_class_card g m hg.2,
...
(fintype.card (mul_action.stabilizer (conj_act (alternating_group α)) g) =
  ((fintype.card α - m.sum).factorial / (ite ((∀ i ∈ m, odd i) ∧ m.sum + 1 ≥ fintype.card α) 2 1)) * (fintype.card α - m.sum).factorial * (m.prod *
    (m.dedup.map (λ (n : ℕ), (m.count n).factorial)).prod)) :=
begin
sorry
end
