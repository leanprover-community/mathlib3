...
lemma to_enat_lift (c : cardinal.{v}) : (cardinal.lift.{u v} c).to_enat = c.to_enat :=
begin
  cases lt_or_ge c ℵ₀ with hc hc,
  { rw [to_enat_apply_of_lt_aleph_0 hc, cardinal.to_enat_apply_of_lt_aleph_0 _],
    simp only [to_nat_lift],
    rw [← lift_aleph_0, lift_lt], exact hc },
  { rw [to_enat_apply_of_aleph_0_le hc, cardinal.to_enat_apply_of_aleph_0_le _],
  rw [← lift_aleph_0, lift_le], exact hc }
end

lemma to_enat_congr {β : Type v} (e : α ≃ β) : (#α).to_enat = (#β).to_enat :=
by rw [←to_enat_lift, lift_mk_eq.mpr ⟨e⟩, to_enat_lift]
...
lemma card_congr {α : Type*} {β : Type*} (f : α ≃ β) :
  enat.card α = enat.card β :=
cardinal.to_enat_congr f
...
begin
  cases lt_or_ge c ℵ₀,
  { rw [to_enat_apply_of_lt_aleph_0 h, coe_le_coe, ← to_nat_cast n],
    rw to_nat_le_iff_le_of_lt_aleph_0 (nat_lt_aleph_0 n) h,
    simp only [to_nat_cast] },
  { apply iff_of_true,
    { rw to_enat_apply_of_aleph_0_le h,
      exact le_top },
...
begin
  ...
  { rw [to_enat_apply_of_lt_aleph_0 h, coe_inj],
    rw ← to_nat_cast n,
...
begin
  cases lt_or_ge c ℵ₀,
  { rw [to_enat_apply_of_lt_aleph_0 h, coe_lt_coe, ← to_nat_cast n],
    rw to_nat_lt_iff_lt_of_lt_aleph_0 (nat_lt_aleph_0 n) h,
    simp only [to_nat_cast] },
  { apply iff_of_true,
    { rw to_enat_apply_of_aleph_0_le h, exact coe_lt_top n },
