...
variables {γ : Type*} {δ : Type*} [add_group γ] [add_group δ]
class is_add_group_hom  (f : γ → δ) : Prop :=
(add : ∀ a b, f (a + b) = f a + f b)

namespace is_add_group_hom

variables (f : γ → δ) [is_add_group_hom f]
...
instance comp {ε} [add_group ε] (g : δ → ε) [is_add_group_hom g] :
