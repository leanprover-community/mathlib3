...
-/
import algebra.lattice.bounded_lattice_experiment data.set.basic
...
/- Note: we can't really experiment with sets, because of the cyclic imports.
Let's fake it here by declaring some facts with the same attributes they have
in data.set.basic.
-/
section
open set
-- TODO: I had to add ematch here
@[simp, ematch] lemma mem_set_of {a : α} {p : α → Prop} : a ∈ {a | p a} = p a := rfl
theorem set_eq_def (s t : set α) : s = t ↔ ∀ x, x ∈ s ↔ x ∈ t :=
  ⟨begin intros h x, rw h end, set.ext⟩
theorem subset_def {s t : set α} : (s ⊆ t) = ∀ x, x ∈ s → x ∈ t := rfl
theorem union_def {s₁ s₂ : set α} : s₁ ∪ s₂ = {a | a ∈ s₁ ∨ a ∈ s₂} := rfl
theorem inter_def {s₁ s₂ : set α} : s₁ ∩ s₂ = {a | a ∈ s₁ ∧ a ∈ s₂} := rfl
@[simp] lemma mem_set_of_eq (a : α) (P : α → Prop) : a ∈ {a : α | P a} = P a :=  rfl
@[simp] lemma nmem_set_of_eq (a : α) (P : α → Prop) : a ∉ {a : α | P a} = ¬ P a := rfl
@[simp] lemma set_of_false : {a : α | false} = ∅ := set.ext $ assume a, by simp [mem_empty_eq]
...
theorem union_left_comm (s₁ s₂ s₃ : set α) : s₁ ∪ (s₂ ∪ s₃) = s₂ ∪ (s₁ ∪ s₃) := by finish [set_eq_def]
...
theorem inter_left_comm (s₁ s₂ s₃ : set α) : s₁ ∩ (s₂ ∩ s₃) = s₂ ∩ (s₁ ∩ s₃) := by finish [set_eq_def, iff_def]
...
@[simp] theorem subset_insert (x : α) (s : set α) : s ⊆ insert x s := assume y, assume ys, or.inr ys
@[simp] theorem mem_insert_iff (x a : α) (s : set α) : x ∈ insert a s ↔ (x = a ∨ x ∈ s) := iff.refl _
@[simp] theorem insert_eq_of_mem {a : α} {s : set α} (h : a ∈ s) : insert a s = s := by finish [set_eq_def, iff_def]
theorem singleton_def (a : α) : ({a} : set α) = insert a ∅ := rfl
@[simp] theorem mem_singleton_iff (a b : α) : a ∈ ({b} : set α) ↔ a = b := by finish [singleton_def]
