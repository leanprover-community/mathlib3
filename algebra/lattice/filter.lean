...
theorem pure_seq_eq_map : ∀ {α β : Type u} (g : α → β) (x : f α), pure g <*> x = g <$> x :=
...
theorem Inf_eq_finite_sets {s : set α} :
...
theorem image_swap_prod {s : set α} {t : set β} :
...
theorem prod_image_image_eq {α₁ : Type u} {α₂ : Type v} {β₁ : Type w} {β₂ : Type x}
...
theorem Union_subset_Union2 {ι₂ : Sort x} {s : ι → set α} {t : ι₂ → set α} (h : ∀i, ∃j, s i ⊆ t j) : (⋃i, s i) ⊆ (⋃i, t i) :=
@supr_le_supr2 (set α) ι ι₂ _ s t h

theorem Union_subset_Union_const {ι₂ : Sort x} {s : set α} (h : ι → ι₂) : (⋃ i:ι, s) ⊆ (⋃ j:ι₂, s) :=
...
theorem eq_of_sup_eq_inf_eq {α : Type u} [distrib_lattice α] {a b c : α}
...
theorem neg_subset_neg_iff_subset {α : Type u} {x y : set α} : - y ⊆ - x ↔ x ⊆ y :=
@neg_le_neg_iff_le (set α) _ _ _

theorem sUnion_eq_Union {α : Type u} {s : set (set α)} :
  (⋃₀ s) = (⋃ (i : set α) (h : i ∈ s), i) :=
set.ext $ by simp
...
theorem directed_on_Union {r} {ι : Sort v} {f : ι → set α} (hd : directed (⊇) f)
...
theorem univ_mem_sets' {f : filter α} {s : set α} (h : ∀ a, a ∈ s): s ∈ f.sets :=
let ⟨x, x_in_s⟩ := f.inhabited in
f.upwards_sets x_in_s (assume x _, h x)

theorem univ_mem_sets {f : filter α} : univ ∈ f.sets :=
univ_mem_sets' mem_univ

theorem inter_mem_sets {f : filter α} {x y : set α} (hx : x ∈ f.sets) (hy : y ∈ f.sets) :
  x ∩ y ∈ f.sets :=
let ⟨z, ⟨z_in_s, z_le_x, z_le_y⟩⟩ := f.directed_sets _ hx _ hy in
f.upwards_sets z_in_s (subset_inter z_le_x z_le_y)

theorem Inter_mem_sets {f : filter α} {s : β → set α}
...
theorem exists_sets_subset_iff {f : filter α} {x : set α} :
  (∃y∈f.sets, y ⊆ x) ↔ x ∈ f.sets :=
⟨assume ⟨y, hy, yx⟩, f.upwards_sets hy yx,
  assume hx, ⟨x, hx, subset.refl _⟩⟩

theorem monotone_mem_sets {f : filter α} : monotone (λs, s ∈ f.sets) :=
...
protected theorem le_Sup {s : set (filter α)} {f : filter α} : f ∈ s → f ≤ Sup s :=
...
theorem principal_mono {s t : set α} : principal s ≤ principal t ↔ s ⊆ t :=
by simp
...
theorem mem_inf_sets_of_left {f g : filter α} {s : set α} :
  s ∈ f.sets → s ∈ (f ⊓ g).sets :=
have f ⊓ g ≤ f, from inf_le_left,
assume hs, this hs

theorem mem_inf_sets_of_right {f g : filter α} {s : set α} :
  s ∈ g.sets → s ∈ (f ⊓ g).sets :=
...
theorem infi_sets_eq' {f : β → filter α} {s : set β} (h : directed_on (λx y, f x ≤ f y) s) (ne : ∃i, i ∈ s) :
...
theorem Inf_sets_eq_finite {s : set (filter α)} :
...
theorem binfi_sup_eq { f : filter α } {s : set (filter α)} :
...
theorem infi_sup_eq { f : filter α } {g : ι → filter α} :
  (⨅ x, f ⊔ g x) = f ⊔ infi g :=
...
theorem map_mono {f g : filter α} {m : α → β} (h : f ≤ g) : map m f ≤ map m g :=
le_of_sup_eq $ calc
  map m f ⊔ map m g = map m (f ⊔ g) : map_sup
                ... = map m g : congr_arg (map m) $ sup_of_le_right h

theorem monotone_map {m : α → β} : monotone (map m : filter α → filter β) :=
...
theorem map_infi_eq {f : ι → filter α} {m : α → β} (hf : directed (≤) f) (hι : nonempty ι) :
  map m (infi f) = (⨅ i, map m (f i)) :=
le_antisymm
  map_infi_le
...
theorem map_binfi_eq {ι : Type w} {f : ι → filter α} {m : α → β} {s : set ι}
...
theorem mem_bind_sets {β : Type u} {s : set β} {f : filter α} {m : α → filter β} :
...
theorem bind_mono {β : Type u} {f : filter α} {g h : α → filter β} (h₁ : {a | g a ≤ h a} ∈ f.sets) :
  f >>= g ≤ f >>= h :=
assume x h₂, f.upwards_sets (inter_mem_sets h₁ h₂) $ assume s ⟨gh', h'⟩, gh' h'

theorem bind_sup {β : Type u} {f g : filter α} {h : α → filter β} :
  (f ⊔ g) >>= h = (f >>= h) ⊔ (g >>= h) :=
by simp

theorem bind_mono2 {β : Type u} {f g : filter α} {h : α → filter β} (h₁ : f ≤ g) :
  f >>= h ≤ g >>= h :=
assume s h', h₁ h'

theorem principal_bind {β : Type u} {s : set α} {f : α → filter β} :
  (principal s >>= f) = (⨆x ∈ s, f x) :=
show join (map f (principal s)) = (⨆x ∈ s, f x),
  by simp [Sup_image]

theorem seq_mono {β : Type u} {f₁ f₂ : filter (α → β)} {g₁ g₂ : filter α}
...
theorem infi_neq_bot_of_directed {f : ι → filter α}
...
theorem infi_neq_bot_iff_of_directed {f : ι → filter α}
...
@[simp] theorem return_neq_bot {α : Type u} {a : α} : return a ≠ (⊥ : filter α) :=
by simp [return]
...
theorem map_vmap_le {f : filter β} {m : α → β} : map m (vmap m f) ≤ f :=
assume s hs, ⟨s, hs, subset.refl _⟩

theorem le_vmap_map {f : filter α} {m : α → β} : f ≤ vmap m (map m f) :=
...
theorem lift_mono' (hg : ∀s∈f.sets, g₁ s ≤ g₂ s) : f.lift g₁ ≤ f.lift g₂ :=
infi_le_infi $ assume s, infi_le_infi $ assume hs, hg s hs

theorem map_lift_eq {m : β → γ} (hg : monotone g) :
...
theorem vmap_lift_eq {m : γ → β} (hg : monotone g) :
...
theorem lift_lift_same_eq_lift {g : set α → set α → filter β}
...
theorem lift_principal {s : set α} (hg : monotone g) :
...
theorem lift_neq_bot_iff (hm : monotone g) : (f.lift g ≠ ⊥) ↔ (∀s∈f.sets, g s ≠ ⊥) :=
classical.by_cases
...
theorem mem_lift' {t : set α} (ht : t ∈ f.sets) : h t ∈ (f.lift' h).sets :=
le_principal_iff.mp $ show f.lift' h ≤ principal (h t),
  from infi_le_of_le t $ infi_le_of_le ht $ le_refl _

theorem mem_lift'_iff (hh : monotone h) {s : set β} : s ∈ (f.lift' h).sets ↔ (∃t∈f.sets, h t ⊆ s) :=
...
theorem lift'_mono' (hh : ∀s∈f.sets, h₁ s ⊆ h₂ s) : f.lift' h₁ ≤ f.lift' h₂ :=
infi_le_infi $ assume s, infi_le_infi $ assume hs, principal_mono.mpr $ hh s hs

theorem lift'_cong (hh : ∀s∈f.sets, h₁ s = h₂ s) : f.lift' h₁ = f.lift' h₂ :=
...
theorem principal_le_lift' {t : set β} (hh : ∀s∈f.sets, t ⊆ h s) :
...
theorem lift_lift'_assoc {g : set α → set β} {h : set β → filter γ}
...
theorem lift'_lift'_assoc {g : set α → set β} {h : set β → set γ}
  (hg : monotone g) (hh : monotone h) :
  (f.lift' g).lift' h = f.lift' (λs, h (g s)) :=
lift_lift'_assoc hg (monotone_comp hh monotone_principal)

theorem lift'_lift_assoc {g : set α → filter β} {h : set β → set γ}
...
theorem lift_lift'_same_eq_lift' {g : set α → set α → set β}
  (hg₁ : ∀s, monotone (λt, g s t)) (hg₂ : ∀t, monotone (λs, g s t)):
  f.lift (λs, f.lift' (g s)) = f.lift' (λs, g s s) :=
...
theorem lift'_neq_bot_iff (hh : monotone h) : (f.lift' h ≠ ⊥) ↔ (∀s∈f.sets, h s ≠ ∅) :=
...
theorem prod_mem_prod {s : set α} {t : set β} {f : filter α} {g : filter β}
...
theorem prod_same_eq {f : filter α} : filter.prod f f = f.lift' (λt, set.prod t t) :=
...
theorem mem_prod_same_iff {s : set (α×α)} {f : filter α} :
...
theorem prod_comm {f : filter α} {g : filter β} :
...
theorem prod_lift_lift {α₁ : Type u} {α₂ : Type v} {β₁ : Type w} {β₂ : Type x}
...
theorem prod_lift'_lift' {α₁ : Type u} {α₂ : Type v} {β₁ : Type w} {β₂ : Type x}
...
theorem prod_map_map_eq {α₁ : Type u} {α₂ : Type v} {β₁ : Type w} {β₂ : Type x}
...
theorem prod_neq_bot {f : filter α} {g : filter β} :
  filter.prod f g ≠ ⊥ ↔ (f ≠ ⊥ ∧ g ≠ ⊥) :=
...
theorem lift_infi {f : ι → filter α} {g : set α → filter β}
...
theorem lift'_infi {f : ι → filter α} {g : set α → set β}
  (hι : nonempty ι) (hg : ∀{s t}, g s ∩ g t = g (s ∩ t)) : (infi f).lift' g = (⨅i, (f i).lift' g) :=
...
theorem ultrafilter_pure {a : α} : ultrafilter (pure a) :=
⟨return_neq_bot,
  assume g hg ha,
  have {a} ∈ g.sets, by simp at ha; assumption,
  show ∀s∈g.sets, {a} ⊆ s, from by_contradiction $
  begin
    simp [not_forall_iff, not_implies_iff],
...
theorem ultrafilter_unique (hg : ultrafilter g) (hf : f ≠ ⊥) (h : f ≤ g) : f = g :=
le_antisymm h (hg.right _ hf h)

theorem exists_ultrafilter (h : f ≠ ⊥) : ∃u, u ≤ f ∧ ultrafilter u :=
let
  τ                := {f' // f' ≠ ⊥ ∧ f' ≤ f},
...
theorem le_of_ultrafilter {g : filter α} (hf : ultrafilter f) (h : f ⊓ g ≠ ⊥) :
  f ≤ g :=
le_of_inf_eq $ ultrafilter_unique hf h inf_le_left

theorem mem_or_compl_mem_of_ultrafilter (hf : ultrafilter f) (s : set α) :
  s ∈ f.sets ∨ - s ∈ f.sets :=
...
theorem mem_or_mem_of_ultrafilter {s t : set α} (hf : ultrafilter f) (h : s ∪ t ∈ f.sets) :
  s ∈ f.sets ∨ t ∈ f.sets :=
...
theorem mem_of_finite_sUnion_ultrafilter {s : set (set α)} (hf : ultrafilter f) (hs : finite s)
...
theorem mem_of_finite_Union_ultrafilter {is : set β} {s : β → set α}
...
theorem ultrafilter_of_split {f : filter α} (hf : f ≠ ⊥) (h : ∀s, s ∈ f.sets ∨ - s ∈ f.sets) :
  ultrafilter f :=
...
theorem ultrafilter_of_spec (h : f ≠ ⊥) : ultrafilter_of f ≤ f ∧ ultrafilter (ultrafilter_of f) :=
begin
...
theorem ultrafilter_of_le : ultrafilter_of f ≤ f :=
if h : f = ⊥ then by simp [ultrafilter_of, dif_pos, h]; exact le_refl _
  else (ultrafilter_of_spec h).left

theorem ultrafilter_ultrafilter_of (h : f ≠ ⊥) : ultrafilter (ultrafilter_of f) :=
(ultrafilter_of_spec h).right

theorem ultrafilter_of_ultrafilter (h : ultrafilter f) : ultrafilter_of f = f :=
