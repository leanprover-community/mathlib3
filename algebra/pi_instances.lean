...
lemma mul_apply [∀ i, has_mul $ f i] : (x * y) i = x i * y i := rfl
...
lemma one_apply [∀ i, has_one $ f i] : (1 : Π i, f i) i = 1 := rfl

instance has_inv [∀ i, has_inv $ f i] : has_inv (Π i : I, f i) :=
⟨λ x, λ i, (x i)⁻¹⟩

lemma inv_apply [∀ i, has_inv $ f i] : x⁻¹ i = (x i)⁻¹ := rfl
...
lemma add_apply [∀ i, has_add $ f i] : (x + y) i = x i + y i := rfl
...
lemma zero_apply [∀ i, has_zero $ f i] : (0 : Π i, f i) i = 0 := rfl

instance has_neg [∀ i, has_neg $ f i] : has_neg (Π i : I, f i) :=
⟨λ x, λ i, -(x i)⟩

lemma neg_apply [∀ i, has_neg $ f i] : (-x) i = -x i := rfl
...
lemma smul_apply {α : Type*} [∀ i, has_scalar α $ f i] (s : α) : (s • x) i = s • x i := rfl
