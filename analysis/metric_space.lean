...
iff.trans (mem_uniformity_dist) Main.symm

theorem uniformity_edist' : uniformity = (⨅ε:{ε:ennreal // ε>0}, principal {p:α×α | edist p.1 p.2 < ε.val}) :=
have A : ∀s, s ∈ (⨅ε:{ε:ennreal // ε>0}, principal {p:α×α | edist p.1 p.2 < ε.val}).sets ↔ (∃ε>0, ∀{a b:α}, edist a b < ε → (a, b) ∈ s) :=
begin
  assume s,
  rw [infi_sets_eq],
  simp [subset_def],
...
filter.ext $ assume s, iff.trans (mem_uniformity_dist_edist) ((A s).symm)
  ...
  begin
    rw [edist_dist, edist_dist, edist_dist, ← ennreal.coe_add, ennreal.coe_le_coe,
        ...
        nnreal.of_real_le_of_real_iff (@dist_nonneg _ _ x z) $
          add_le_add (@dist_nonneg _ _ x y) (@dist_nonneg _ _ y z)],
