...
variables [measurable_space s] (p : prob s)
...
begin
  ...
  apply lt_of_le_of_lt (prob_le_1 s p a) h,
end
...
p(a) + p(b) ≤ p(a ∩ b) + 1 :=
begin
  rewrite [← prob_union_inter s p a b h_1 h_2, ← add_comm],
...
variables (a b : ennreal) [measurable_space s] (p : prob s)
...
begin
  ...
  have g₂ : ∀ a, (p(a) ≠ 0) → (p(a))⁻¹ * p(a) = 1,
  {
    ...
    apply lt_of_le_of_lt (prob_le_1 s p a) g₁ 
  },
  rw g₂ b h₁, simp,
end
...
begin
    intros i,
    rw ← total_prob _ p f h₁ b h₅ h₃ h₄ i,
    rw cond_prob_swap _ p (f i) b (h₄ i) ,
    rw [div_def,mul_assoc],
    rw ennreal.mul_inv_cancel h₂ (prob_ne_top _ p b),
    simp,
    exact h₂,
end
end cond_prob