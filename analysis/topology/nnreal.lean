...
protected def add : ℝ≥0 → ℝ≥0 → ℝ≥0 := λ r s, ⟨r.val + s.val,begin
have h : (r.val ≤ r.val + s.val) := by rw le_add_iff_nonneg_right; exact s.property,
exact le_trans r.property h end⟩

protected def mul : ℝ≥0 → ℝ≥0 → ℝ≥0 := λ r s, ⟨r.val * s.val,begin
have h := mul_le_mul_of_nonneg_right r.2 s.2, simp at h, exact h end⟩
...
{ one  := 1,
  ...
  left_distrib := λ r s t, begin apply subtype.eq; simp,
  show r.val * (s+t).val = (r*s + r*t).val, simp, rw left_distrib, end,
  right_distrib := λ r s t, begin apply subtype.eq; simp,
  show (r+s).val * t.val = (r*t + s*t).val, simp, rw right_distrib, end,
...
{ le           := (≤),
  ...
  le_trans     := begin intros r s t, show r.val ≤ s.val → s.val ≤ t.val → r.val ≤ t.val, apply le_trans end,
  le_antisymm  := begin intros r s, show r.val ≤ s.val → s.val ≤ r.val → r = s, intros h1 h2,
  apply subtype.eq, apply le_antisymm h1 h2 end,
  le_total     := begin intros r s, show r.val ≤ s.val ∨ s.val ≤ r.val, apply le_total end,
  decidable_le := begin unfold decidable_rel, intros r s, show decidable (preorder.le r.val s.val),
  apply_instance, end }
...
{ add_le_add_left       := λ r s h t, begin show t.val + r.val ≤ t.val + s.val, apply add_le_add_left, exact h, end,
  lt_of_add_lt_add_left := λ r s t, begin intro h, have H : r.val + s.val < r.val + t.val,
  { apply lt_of_not_ge, show ¬ r + s ≥ r + t, simp, exact h},
  apply lt_of_not_ge, show ¬s.val ≥ t.val, simp, apply lt_of_add_lt_add_left H, end,
  ..nnreal.add_comm_monoid, ..nnreal.decidable_linear_order }
...
:= continuous.prod_mk (continuous.comp continuous_fst continuous_induced_dom) (continuous.comp continuous_snd continuous_induced_dom)
...
simp,
rw ←continuous_iff_induced_le,
have comp : (λ (x : ℝ≥0 × ℝ≥0), (x.fst).val + (x.snd).val) = (λ (x : ℝ × ℝ), x.fst + x.snd) ∘ (λ (x : ℝ≥0 × ℝ≥0), (x.fst.val, x.snd.val))
:= begin
apply funext, intro x, unfold function.comp
end,
rw comp,
