/-
Copyright (c) 2021 Joeff Lee. All rights reserved.
...
begin
  ...
  suffices : x ^ 3 + 3 * p * x - 2 * q
    ...
    { rw this, },
  obtain ⟨hr, hs3, ht, hsp⟩ := h_eq,
  have hc : s^3 - t^3 = 2 * q,
    { by_cases hp : p = 0,

      { simp [hp] at hs3, rw [hsp hp, hs3], norm_num },
      { simp [hp] at hs3, have : s ≠ 0 := λ h, by { apply hp, rw [h, mul_zero] at ht, exact ht.symm },
  ...
  calc (x - (s - t)) * (x - (s * ω - t * ω^2)) * (x - (s * ω^2 - t * ω))
      = x^3 - (s-t) * (1+ω+ω^2) * x^2
        ...
        - (s^3-t^3)*ω^3 + s*t*(s-t)*ω^2*(1+ω+ω^2)
        : by ring
  ... = x^3 + 3*(t*s)*x - (s^3-t^3)
        : by { rw [omega_sum, omega_cube_eq_one], ring }
...
begin
  ...
  { dsimp [y],
    rcases h_eq with ⟨hp, hq, -⟩,
    rw [hp, hq],
    field_simp, ring, },
  have h₂ := @cubic_basic_eq_zero_iff p q r s t _ (by simp [h_eq] at h_eq ⊢; cc) y,
  rw [h₁, h₂],
  dsimp [y],
...
begin
  ...
  have h₃ := @cubic_monic_eq_zero_iff (b/a) (c/a) (d/a) p q r s t _ (by {
    have : decidable (p = 0) := by assumption,
    have hsp : p ≠ 0 → s ≠ 0, {
      have : p = t * s := by simp [h_eq],
      apply mt,
      intro h,
      rwa[h, mul_zero] at this },
    simp [h_eq, ha] at h_eq hsp ⊢,
    split,
    repeat { field_simp, ring_nf, },
    assumption'
  }) x,
  rw [h₁, h₂, h₃],
  have h₄ :=
    calc b / a / 3 = b / (a * 3) : by { field_simp [ha] }
