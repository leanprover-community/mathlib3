...
lemma cubic_basic_eq_zero_iff
  (hr : r^2 = q^2 + p^3)
  (hs3 : s^3 = (if p = 0 then 2 * q else q + r))
  (ht : t * s = p)
  (hpt: p = 0 → t = 0)
  (x : ℂ) :
   x^3 + 3 * p * x - 2 * q = 0 ↔
...
begin
  ...
  have hc : s^3 - t^3 = 2 * q,
  { by_cases hp : p = 0,
    { simp [hp] at hs3, rw [hpt hp, hs3], norm_num },
...
begin
  ...
  { dsimp [y],
    rw [hp, hq],
    field_simp, ring, },
  have h₂ := @cubic_basic_eq_zero_iff p q r s t _ hr hs3 ht hpt y,
  rw [h₁, h₂],
  dsimp [y],
...
begin
  ...
  have h₃ := @cubic_monic_eq_zero_iff (b/a) (c/a) (d/a) p q r s t _ hp' hq' hr hs3 ht hpt x,
  rw [h₁, h₂, h₃],
  have h₄ :=
