...
begin
  ...
  { simp only [to_set, forall_fin_succ, hp0, side_unit_cube, mem_set_of, mem_Ico,
...
begin
  refine ⟨_, _, _⟩,
  { intro v,
    simp only [bottom, and_imp, mem_Union, mem_set_of],
    intros h0 hv,
    have : v ∈ (unit_cube : cube (n+1)).to_set,
    { dsimp only [to_set, unit_cube, mem_set_of],
...
begin
  ...
  { rintro ⟨⟩,
    ...
    { simp only [to_set, forall_fin_succ, hi.1, bottom_mem_side h2i', true_and, mem_set_of],
...
begin
  ...
  { rintros i' hi' ⟨p2, hp2, h2p2⟩, simp only [head_shift_up] at hi', classical, by_contra h2i',
    ...
    { simp only [to_set, not_forall, mem_set_of] at h2p1, cases h2p1 with j hj,
      ...
      { simp only [to_set, not_forall, mem_set_of], use j, rw [if_pos rfl], convert h2w },
      { intro j', by_cases h : j' = j,
    ...
    have h2p' : p' ∈ (cs i'').to_set,
    { simp only [to_set, forall_fin_succ, p', cons_succ, cons_zero, mem_set_of],
