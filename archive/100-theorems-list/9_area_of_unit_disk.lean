...
begin
  ...
  { have x_in : ∀ x : α, x ∈ s → {a | x ∈ s ∧ a ∈ Ioo (u x) (v x)} = Ioo (u x) (v x) :=
    ...
    { rw eventually_eq_iff_exists_mem,
      use s, split,
      { simp only [measure.ae, mem_set_of_eq, filter.mem_mk,
                   measure.restrict_apply hs.compl, measure_empty, compl_inter_self], },
      { intros x hx,
        simp only [g],
        split_ifs,
        norm_cast, }, },
    { rw [integral_congr_ae h₂, lintegral_coe_eq_integral],
      exact (integrable_congr h₂).mp (v_int.sub u_int), },
    { exact hs } },
  { apply_instance },
end
