...
not_prime_of_int_mul (int_large ha) (int_large hb) hc
...
/--
We define $a(b) := 4*(2+b)^4$, so that we can take $m = 2+b$ in `polynomial_not_prime`.
-/
def a (b : ℕ) : ℕ := 4*(2+b)^4
...
/-- `a` is a strictly monotone function; this is easily proven by chaining together lemmas
...
/-- We conclude by using the fact that `a` is an injective function from the natural numbers to
the set `good_nats`. -/
theorem imo1969_q1 : set.infinite good_nats :=
set.infinite_of_injective_forall_mem a_strict_mono.injective a_good
