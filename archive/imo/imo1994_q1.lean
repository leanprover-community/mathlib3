...
begin
  ...
  rw foo,
  rw (show k + c + (c + 1) = c + (k + c + 1), by ring),
  simp only [nat.add_mod_right],
  rw nat.mod_eq_of_lt (show c < k + c + 1, by linarith),
  simp,
end
...
begin
  ...
  { rintros k -,
    ...
    { intros x hx,
      ...
      { suffices hik : a i ≤ a k, linarith,
        exact a.monotone hi.2 },
      have h2 : a i + a (fin.last m - k) ∈ A :=
        hadd _ _ (order_emb_of_fin_mem A hm _) (order_emb_of_fin_mem A hm _) h1,
      rw [←mem_coe, ←range_order_emb_of_fin A hm, set.mem_range] at h2,
      cases h2 with j hj,
      use j,
      split, split, rotate 1,
      { exact fin.le_last j },
      { exact hj },
      rw [←strict_mono.lt_iff_lt a.strict_mono, hj],
      simp,
      exact (hrange (a i) (order_emb_of_fin_mem A hm i)).1 },

    -- A set of size k+1 embed in one of size k, which yeilds a contradiction
