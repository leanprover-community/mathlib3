...
theorem subst_wlog {x y z s r2 : ℝ} (hxy : 0 ≤ x * y) (hxyz : x + y + z = 0)
  (r20 : 0 ≤ r2) (hr2 : r2 ^2 = 2) :
  32 * abs (x * y * z * s) ≤ r2 * (x^2 + y^2 + z^2 + s^2)^2 :=
...
le_of_pow_le_pow _ (mul_nonneg r20 (sq_nonneg _)) nat.succ_pos' $
  ...
  ... = (r2 * (x^2 + y^2 + z^2 + s^2)^2)^2 :
          by rw [← div_mul_eq_mul_div_comm, mul_pow, hr2,
...
theorem proof₁ {a b c r2 : ℝ} (r20 : 0 ≤ r2) (hr2 : r2 ^2 = 2) :
  abs (a * b * (a^2 - b^2) + b * c * (b^2 - c^2) + c * a * (c^2 - a^2)) ≤
  9 * r2 / 32 * (a^2 + b^2 + c^2)^2 :=
calc _ = _ : congr_arg _ $ lhs_identity a b c
   ... ≤ _ : subst_proof₁ (a - b) (b - c) (c - a) (-(a + b + c)) (by ring) r20 hr2
   ... = _ : by ring
...
begin
  ...
  { intro, ring },
  have := h (2 - 3 * r2) 2 (2 + 3 * r2),
  rw [lhs_identity, h₁, h₂, hr2,
    abs_neg, abs_eq_self.mpr, ← div_le_iff] at this,
  { convert this using 1, ring },
  { apply pow_pos, norm_num },
  { exact mul_nonneg (mul_nonneg (sq_nonneg _) zero_le_two) r20 }
end

theorem imo2006_q3 (M : ℝ) {r2 : ℝ} (r20 : 0 ≤ r2) (hr2 : r2 ^2 = 2) :
  (∀ a b c : ℝ,
    abs (a * b * (a^2 - b^2) + b * c * (b^2 - c^2) + c * a * (c^2 - a^2)) ≤
    M * (a^2 + b^2 + c^2)^2) ↔
  9 * r2 / 32 ≤ M :=
⟨ λ h, proof₂ M h r20 hr2,
  λ h _ _ _, trans (proof₁ r20 hr2) (mul_le_mul_of_nonneg_right h $ sq_nonneg _)⟩
