...
by simp [equiv.traverse] with functor_norm;
   rw ← map_traverse; simp [equiv.map] with functor_norm;
...
by simp [equiv.traverse] with functor_norm; rw ← traverse_map g f;
   [ simp [equiv.map,function.comp] with functor_norm,
     apply_instance ]
