...
@[simp,ematch] lemma trans_coe (α : X ≅ Y) (β : Y ≅ Z) : ((α ≪≫ β) : X ⟶ Z) = (α : X ⟶ Y) ≫ (β : Y ⟶ Z) := rfl
...
@[simp,ematch] lemma trans_symm_coe (α : X ≅ Y) (β : Y ≅ Z) : ((α ≪≫ β).symm : Z ⟶ X) = (β.symm : Z ⟶ Y) ≫ (α.symm : Y ⟶ X) := rfl
...
@[simp,ematch] lemma hom_inv_id_coe (α : X ≅ Y) : (α : X ⟶ Y) ≫ (α.symm : Y ⟶ X) = 𝟙 X := begin unfold_coes, unfold symm, rw iso.hom_inv_id_lemma, end
@[simp,ematch] lemma inv_hom_id_coe (α : X ≅ Y) : (α.symm : Y ⟶ X) ≫ (α : X ⟶ Y) = 𝟙 Y := begin unfold_coes, unfold symm, rw iso.inv_hom_id_lemma, end
...
{ hom := F.map i.hom,
  inv := F.map i.inv,
  hom_inv_id := begin /- `obviously'` says: -/ dsimp at *, erw [←map_comp_lemma, iso.hom_inv_id_coe, ←map_id_lemma] end,
  inv_hom_id := begin /- `obviously'` says: -/ dsimp at *, erw [←map_comp_lemma, iso.inv_hom_id_coe, ←map_id_lemma] end }
