...
namespace op_hom
@[simp] lemma map {F G : (C ⥤ D)ᵒᵖ} (α : F ⟶ G) (X : C) : (op_hom C D).map α X = α X := rfl
end op_hom
...
namespace op_inv
@[simp] lemma map {F G : Cᵒᵖ ⥤ Dᵒᵖ} (α : F ⟶ G) (X : C) : (op_inv C D).map α X = α X := rfl
end op_inv
...
protected definition op (F : C ⥤ D) : Cᵒᵖ ⥤ Dᵒᵖ := (@op_hom C _ D _) (F : (C ⥤ D)ᵒᵖ)
