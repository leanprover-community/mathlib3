...
{ to_fun := λ s, f (of_normed_add_group_discrete (indicator s 1) 1 (norm_indicator_le_one s)),
  additive' := λ s t hst,
    begin
      have : of_normed_add_group_discrete (indicator (s ∪ t) 1) 1 (norm_indicator_le_one (s ∪ t))
              = of_normed_add_group_discrete (indicator s 1) 1 (norm_indicator_le_one s)
              + of_normed_add_group_discrete (indicator t 1) 1 (norm_indicator_le_one t),
        by { ext x, simp [indicator_union_of_disjoint hst], },
      rw [this, f.map_add],
    end,
  exists_bound := ⟨∥f∥, λ s, begin
    have I : ∥of_normed_add_group_discrete (indicator s 1) 1 (norm_indicator_le_one s)∥ ≤ 1,
      by apply norm_of_normed_add_group_le _ zero_le_one,
...
begin
  change μ.extension_to_bounded_functions
    (of_normed_add_group_discrete (indicator s (λ x, 1)) 1 (norm_indicator_le_one s)) = (μ s).to_real,
...
of_normed_add_group_discrete (indicator (spf Hcont x) 1) 1 (norm_indicator_le_one _)
...
norm_of_normed_add_group_le _ zero_le_one _
