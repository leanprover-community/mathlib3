...
instance decidable_zero_symm {γ : Type w} [has_zero γ] [decidable_pred (eq (0 : γ))] : decidable_pred (λ x, x = (0:γ)) :=
...
def mk (s : finset ι) (x : dfinsupp.finset ι β s) : dfinsupp ι β :=
⟦⟨s, x⟩⟧

instance : has_zero (ι →ₚ β) := ⟨mk ∅ $ λ _, 0⟩

instance : inhabited (ι →ₚ β) := ⟨0⟩

protected def lift_on {γ : Sort l} (f : ι →ₚ β)
...
protected def induction_on {C : ι →ₚ β → Prop} (f : ι →ₚ β)
  (H : ∀ s x, C (mk s x)) : C f :=
...
def single (i : ι) (b : β i) : ι →ₚ β :=
mk (finset.singleton i) $ λ j, eq.rec_on (finset.mem_singleton.1 j.2).symm b

@[simp] lemma single_zero {i} : (single i 0 : ι →ₚ β) = 0 :=
begin
...
def on_finset (s : finset ι) (f : Π i, β i) : ι →ₚ β :=
mk s $ λ i, f i.1

/-- The composition of `f : β₁ → β₂` and `g : ι →ₚ β₁` is
  `map_range f hf g : ι →ₚ β₂`, well defined when `f 0 = 0`. -/
def map_range (f : Π i, β₁ i → β₂ i) (hf : ∀ i, f i 0 = 0) (g : ι →ₚ β₁) : ι →ₚ β₂ :=
...
def filter (p : ι → Prop) [decidable_pred p] (f : ι →ₚ β) : ι →ₚ β :=
...
def subtype_domain (p : ι → Prop) [decidable_pred p]
  (f : ι →ₚ β) : (subtype p →ₚ (β ∘ subtype.val)) :=
...
instance [Π i, add_group (β i)] : has_neg (ι →ₚ β) :=
⟨λ f, f.map_range (λ _, has_neg.neg) (λ _, neg_zero)⟩

def to_has_scalar' {γ : Type w} [ring γ] [Π i, module γ (β i)] : has_scalar γ (ι →ₚ β) :=
...
@[simp] lemma zero_apply {i : ι} : (0 : ι →ₚ β) i = 0 := rfl

@[extensionality]
lemma ext {f g : ι →ₚ β} : (∀ i, f i = g i) → f = g :=
begin
...
@[simp] lemma single_apply {i i' b} : (single i b : ι →ₚ β) i' = (if h : i = i' then eq.rec_on h b else 0) :=
begin
  dsimp [single],
  by_cases h : i = i',
...
@[simp] lemma single_eq_same {i b} : (single i b : ι →ₚ β) i = b :=
by simp

@[simp] lemma single_eq_of_ne {i i' b} (h : i ≠ i') : (single i b : ι →ₚ β) i' = 0 :=
by simp [h]

@[simp] lemma on_finset_apply {s : finset ι} {f : Π i, β i} {i} :
  (on_finset s f : ι →ₚ β) i = if i ∈ s then f i else 0 :=
...
@[simp] lemma map_range_apply
  {f : Π i, β₁ i → β₂ i} {hf : ∀ i, f i 0 = 0} {g : ι →ₚ β₁} {i : ι} :
...
def zip_with (f : Π i, β₁ i → β₂ i → β i) (hf : ∀ i, f i 0 0 = 0) (g₁ : ι →ₚ β₁) (g₂ : ι →ₚ β₂) : (ι →ₚ β) :=
begin
...
@[simp] lemma zip_with_apply
  {f : Π i, β₁ i → β₂ i → β i} {hf : ∀ i, f i 0 0 = 0} {g₁ : ι →ₚ β₁} {g₂ : ι →ₚ β₂} {i : ι} :
...
@[simp] lemma erase_apply {i j : ι} {f : ι →ₚ β} :
...
@[simp] lemma erase_same {i : ι} {f : ι →ₚ β} : (f.erase i) i = 0 :=
by simp

@[simp] lemma erase_ne {i i' : ι} {f : ι →ₚ β} (h : i' ≠ i) : (f.erase i) i' = f i' :=
by simp [h]
...
instance : has_add (ι →ₚ β) :=
⟨zip_with (λ i, (+)) (λ i, add_zero 0)⟩

@[simp] lemma add_apply [Π i, add_monoid (β i)] {g₁ g₂ : ι →ₚ β} {i : ι} : (g₁ + g₂) i = g₁ i + g₂ i :=
zip_with_apply
...
lemma single_add_erase {i : ι} {f : ι →ₚ β} : single i (f i) + f.erase i = f :=
ext $ λ i',
if h : i = i' then by subst h; simp
else by simp [ne.symm h, h]

lemma erase_add_single {i : ι} {f : ι →ₚ β} : f.erase i + single i (f i) = f :=
ext $ λ i',
if h : i = i' then by subst h; simp
else by simp [ne.symm h, h]

protected theorem induction {p : (ι →ₚ β) → Prop} (f : ι →ₚ β)
  (h0 : p 0) (ha : ∀i b (f : ι →ₚ β), f i = 0 → b ≠ 0 → p f → p (single i b + f)) :
  p f :=
begin
...
lemma induction₂ {p : (ι →ₚ β) → Prop} (f : ι →ₚ β)
  (h0 : p 0) (ha : ∀i b (f : ι →ₚ β), f i = 0 → b ≠ 0 → p f → p (f + single i b)) :
  p f :=
...
instance [Π i, add_comm_monoid (β i)] : add_comm_monoid (ι →ₚ β) :=
{ add_comm := λ x y, ext $ by simp,
  .. dfinsupp.add_monoid }

@[simp] lemma neg_apply [Π i, add_group (β i)] {g : ι →ₚ β} {i : ι} : (- g) i = - g i :=
map_range_apply

instance [Π i, add_group (β i)] : add_group (ι →ₚ β) :=
{ add_left_neg := λ f, ext $ by simp,
  .. dfinsupp.add_monoid,
  .. (infer_instance : has_neg (ι →ₚ β)) }

@[simp] lemma sub_apply [Π i, add_group (β i)] {g₁ g₂ : ι →ₚ β} {i : ι} : (g₁ - g₂) i = g₁ i - g₂ i :=
by rw [sub_eq_add_neg]; simp

instance [Π i, add_comm_group (β i)] : add_comm_group (ι →ₚ β) :=
{ add_comm := λ _ _, ext $ by simp, ..dfinsupp.add_group }

@[simp] lemma filter_apply [Π i, has_zero (β i)]
  {p : ι → Prop} [decidable_pred p] {i : ι} {f : ι →ₚ β} :
...
@[simp] lemma filter_apply_pos [Π i, has_zero (β i)]
  {p : ι → Prop} [decidable_pred p] {f : ι →ₚ β} {i : ι} (h : p i) :
  f.filter p i = f i :=
by simp [h]
...
lemma filter_pos_add_filter_neg [Π i, add_monoid (β i)] {f : ι →ₚ β}
...
@[simp] lemma subtype_domain_add [Π i, add_monoid (β i)] {p : ι → Prop} [decidable_pred p] {v v' : ι →ₚ β} :
  (v + v').subtype_domain p = v.subtype_domain p + v'.subtype_domain p :=
ext $ by simp

@[simp] lemma subtype_domain_neg [Π i, add_group (β i)] {p : ι → Prop} [decidable_pred p] {v : ι →ₚ β} :
  (- v).subtype_domain p = - v.subtype_domain p :=
ext $ by simp

@[simp] lemma subtype_domain_sub [Π i, add_group (β i)] {p : ι → Prop} [decidable_pred p] {v v' : ι →ₚ β} :
  (v - v').subtype_domain p = v.subtype_domain p - v'.subtype_domain p :=
ext $ by simp

local attribute [instance] to_has_scalar'

@[simp] lemma smul_apply' {γ : Type w} [ring γ] [Π i, module γ (β i)] {i : ι} {b : γ} {v : ι →ₚ β} :
  (b • v) i = b • (v i) :=
map_range_apply

def to_module {γ : Type w} [ring γ] [Π i, module γ (β i)] : module γ (ι →ₚ β) :=
{ smul_add := assume i x y, dfinsupp.ext $ by simp [smul_add],
  ...
  .. (infer_instance : has_scalar γ (ι →ₚ β)) }

lemma finite_supp [Π i, has_zero (β i)] (f : ι →ₚ β) : set.finite {i | f i ≠ 0} :=
begin
  refine dfinsupp.induction_on f (λ s x, _),
...
@[simp] theorem mem_support_to_fun (f : ι →ₚ β) (i) : i ∈ f.support ↔ f i ≠ 0 :=
begin
  refine dfinsupp.induction_on f (λ s x, _),
  by_cases h1 : i ∈ s; simp [h1, support]
end

theorem eq_mk_support (f : ι →ₚ β) : f = mk f.support (λ i, f i.1) :=
by ext i; by_cases h : f i = 0; try {simp at h}; simp [h]

@[simp] lemma support_zero : (0 : ι →ₚ β).support = ∅ := rfl

@[simp] lemma mem_support_iff (f : ι →ₚ β) : ∀i:ι, i ∈ f.support ↔ f i ≠ 0 :=
f.mem_support_to_fun

@[simp] lemma support_eq_empty {f : ι →ₚ β} : f.support = ∅ ↔ f = 0 :=
⟨λ H, ext $ by simpa [finset.ext] using H, by simp {contextual:=tt}⟩

instance decidable_zero : decidable_pred (eq (0 : ι →ₚ β)) :=
λ f, decidable_of_iff _ $ support_eq_empty.trans eq_comm

lemma support_subset_iff {s : set ι} {f : ι →ₚ β} :
...
lemma map_range_def {f : Π i, β₁ i → β₂ i} {hf : ∀ i, f i 0 = 0} {g : ι →ₚ β₁} :
...
lemma support_map_range {f : Π i, β₁ i → β₂ i} {hf : ∀ i, f i 0 = 0} {g : ι →ₚ β₁} :
...
lemma zip_with_def {f : Π i, β₁ i → β₂ i → β i} {hf : ∀ i, f i 0 0 = 0} {g₁ : ι →ₚ β₁} {g₂ : ι →ₚ β₂} :
...
lemma support_zip_with {f : Π i, β₁ i → β₂ i → β i} {hf : ∀ i, f i 0 0 = 0} {g₁ : ι →ₚ β₁} {g₂ : ι →ₚ β₂} :
  (zip_with f hf g₁ g₂).support ⊆ g₁.support ∪ g₂.support :=
by simp [zip_with_def]

end map_range_and_zip_with

lemma erase_def (i : ι) (f : ι →ₚ β) :
...
@[simp] lemma support_erase (i : ι) (f : ι →ₚ β) :
...
@[simp] lemma support_subtype_domain {f : ι →ₚ β} :
...
lemma support_add [Π i, add_monoid (β i)] [Π i, decidable_pred (eq (0 : β i))] {g₁ g₂ : ι →ₚ β} :
  (g₁ + g₂).support ⊆ g₁.support ∪ g₂.support :=
support_zip_with

@[simp] lemma support_neg [Π i, add_group (β i)] [Π i, decidable_pred (eq (0 : β i))] {f : ι →ₚ β} :
  support (-f) = support f :=
by ext i; simp

instance [decidable_eq ι] [Π i, has_zero (β i)] [Π i, decidable_eq (β i)] : decidable_eq (ι →ₚ β) :=
...
lemma prod_map_range_index {β₁ : ι → Type v₁} {β₂ : ι → Type v₂}
  ...
  {f : Π i, β₁ i → β₂ i} {hf : ∀ i, f i 0 = 0} {g : ι →ₚ β₁} {h : Π i, β₂ i → γ} (h0 : ∀i, h i 0 = 1) :
...
lemma prod_neg_index [Π i, add_group (β i)] [Π i, decidable_pred (eq (0 : β i))] [comm_monoid γ]
  {g : ι →ₚ β} {h : Π i, β i → γ} (h0 : ∀i, h i 0 = 1) :
...
@[simp] lemma sum_single [Π i, add_comm_monoid (β i)]
  [Π i, decidable_pred (eq (0 : β i))] {f : ι →ₚ β} :
  f.sum single = f :=
begin
...
lemma prod_subtype_domain_index [Π i, has_zero (β i)] [Π i, decidable_pred (eq (0 : β i))]
  [comm_monoid γ] {v : ι →ₚ β} {p : ι → Prop} [decidable_pred p]
...
lemma subtype_domain_sum [Π i, add_comm_monoid (β i)] [Π i, decidable_pred (eq (0 : β i))]
  {s : finset γ} {h : γ → ι →ₚ β} {p : ι → Prop} [decidable_pred p] :
