...
begin
  ...
  { assume k, simp,
    cases k with k', 
      simp,
      rw [choose_zero_succ,choose_eq_zero_of_lt (dec_trivial : 1 < succ (succ k')),zero_mul]},
  assume k,
  cases k with k',
  { simp},
  ...
  { assume k hnk,
    ...
    { rw [choose_succ_succ,succ_sub_succ,add_mul,add_mul],
      ...
      { have : n' - k' = succ (n' - succ k') := by rw [←succ_sub (le_of_succ_le_succ hnk₁),succ_sub_succ],
        ...
        rw [←succ_sub (le_of_succ_le_succ hnk₁), nat.add_sub_cancel' hnk],unfold fact,rw mul_comm},
