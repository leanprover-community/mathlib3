...
parameters {abv' : Cauchy → α} [is_absolute_value abv'] (abv_eq_abv' : ∀ b : β, abv' (of_rat b) = abv b)
include abv_eq_abv'


local attribute [instance, priority 1] classical.prop_decidable

theorem rat_dense' (q : Cauchy) {ε : α} (hε : ε > 0) :
  ∃ r : β, abv' (q - of_rat r) < ε :=
quotient.induction_on q $ λ q',
  have ∃ N, ∀ m n ≥ N, abv (q' m - q' n) < ε, from cauchy₂ _ hε,
  let ⟨N, hN⟩ := this in
  ⟨ q' N,
    begin
      change abv' (⟦⟨λ i, q' i - q' N, _⟩⟧) < ε,
--      cases decidable.em (lim_zero (q' + -const abv (q' N))) with heq hne',
--      { rw [mk_eq_zero.2 heq, is_absolute_value.abv_zero abv'], exact hε },
--      {  }
    end ⟩
...
instance : is_complete Cauchy abv' :=
⟨λ s, _ ⟩
