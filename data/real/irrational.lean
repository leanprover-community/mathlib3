...
theorem irr_nrt_of_n_not_dvd_padic_val {x : ℝ} (n : ℕ) {m : ℤ} (hm : m ≠ 0) (p : ℕ)
  [hp : nat.prime p] (hxr : x ^ n = m)
  (hv : (padic_val (p : ℤ) m).get (finite_int_iff.2 ⟨hp.ne_one, hm⟩) % n ≠ 0) :
  irrational x :=
begin
  ...
  erw [padic_val.pow' (padic_val_rat.prime_int_of_nat_prime p)
    (finite_int_iff.2 ⟨hp.ne_one, this⟩), nat.mul_mod_right] at hv,
  exact hv rfl
end

theorem irr_sqrt_of_padic_val_odd (m : ℤ) (hm : 0 < m)
  (p : ℕ) [hp : nat.prime p]
  (Hpv : (padic_val (p : ℤ) m).get (finite_int_iff.2 ⟨hp.ne_one, ne.symm (ne_of_lt hm)⟩) % 2 = 1) :
  irrational (sqrt m) :=
irr_nrt_of_n_not_dvd_padic_val 2 (ne.symm (ne_of_lt hm)) p
...
by simp [padic_val_self (mt is_unit_iff_dvd_one.1 (mt int.coe_nat_dvd.1 hp.not_dvd_one) : _)];
  refl
