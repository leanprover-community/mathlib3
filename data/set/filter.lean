...
import order.filter data.set.lattice

@[reducible] def filter (α : Type*) := pfilter (set α)

variables {α : Type*} {ι : Sort*}
...
lemma univ_mem : univ ∈ f := f.top_mem

lemma mem_of_superset {x y : set α} : x ∈ f → x ⊆ y → y ∈ f := f.mem_of_le

lemma inter_mem : ∀{s t}, s ∈ f → t ∈ f → s ∩ t ∈ f :=
f.inter_sets

lemma univ_mem' (h : ∀ a, a ∈ s) : s ∈ f :=
mem_of_superset univ_mem (assume x _, h x)

lemma mp_mem {p q : α → Prop} (hs : {a | p a → q a} ∈ f)
  (h : {x | p a} ∈ f) : {x | q a} ∈ f :=
mem_of_superset (inter_mem hs h) $ assume x ⟨h₁, h₂⟩, h₂ h₁

lemma Inter_mem {β : Type v} {s : β → set α} {is : set β} (hf : finite is) :
  (∀i∈is, s i ∈ f) → (⋂i∈is, s i) ∈ f :=
finite.induction_on hf
  (assume hs, by simp only [univ_mem, mem_empty_eq, Inter_neg, Inter_univ, not_false_iff])
  (assume i is _ hf hi hs,
    ...
    by simp [inter_mem h₁ h₂])

lemma exists_sets_subset_iff : (∃t∈f, t ⊆ s) ↔ s ∈ f :=
⟨assume ⟨t, ht, ts⟩, mem_of_superset ht ts, assume hs, ⟨s, hs, subset.refl _⟩⟩

lemma monotone_mem {f : filter α} : monotone (λs, s ∈ f) :=
assume s t hst h, mem_of_superset h hst
