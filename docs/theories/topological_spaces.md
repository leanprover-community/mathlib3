...
topological groups and rings, and infinite sums.
...
look rather strange to a mathematician who is not a specialist in
point set topology. We have already seen a definition using filters
...
Translated, this says that a subset `Y` of a topological space `X` is compact if for every filter `F` on `X` other than the filter of all subsets of `X`, if `Y` is an element of `F` then there's an element `y` of `Y` such that the smallest filter containing both F and the neighbourhood filter of `y` is not the filter of all subsets of `X` either.
...
In Lean they chose the rather archaic terminology `t2_space` to mean Hausdorff:
...
The definition of a basis for a topology in mathlib is also quite
convoluted -- one of the axioms for a basis is that the topology
is generated from the basis in the sense above, which may make
it hard to prove that a given set is a basis. However we have
