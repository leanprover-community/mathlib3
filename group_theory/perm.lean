...
@[simp] lemma perms_of_list_nil : perms_of_list ([] : list α) = [1] := rfl
...
begin
  rintros ⟨s, hs⟩ f,
  cases quotient.exists_rep s with l hl,
  subst hl,
  exact mem_perms_of_list_iff
end
...
begin
  ...
  cases quotient.exists_rep s with l hl,
  subst hl,
  exact length_perms_of_list l
end

instance [fintype α] : fintype (perm α) :=
