...
section e
variables [fintype α] {R : Type v} [ring R]

def e (σ : perm α) : R := ((σ.sign : ℤ) : R)

@[simp] lemma e_mul (σ τ : perm α) : (e (σ * τ) : R) = e σ * e τ :=
by unfold e; rw ← int.cast_mul; congr; rw ← units.mul_coe; congr; apply is_group_hom.mul

@[simp] lemma e_swap {i j : α} : (e (swap i j) : R) = if i = j then 1 else -1 :=
by by_cases H : i = j; simp [e, H]

@[simp] lemma e_one : (e (1 : perm α) : R) = 1 :=
by unfold e; rw is_group_hom.one (sign : perm α → units ℤ); simp

@[simp] lemma e_inv (σ : perm α): (e σ⁻¹ : R) = e σ :=
by unfold e; rw is_group_hom.inv (sign : perm α → units ℤ);
cases int.units_eq_one_or σ.sign with H H; rw H; refl

lemma e_eq_one_or (σ : perm α) : (e σ : R) = 1 ∨ (e σ : R) = -1 :=
