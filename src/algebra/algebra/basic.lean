...
variables [add_comm_monoid M] [semimodule S M] [semimodule R M] [is_scalar_tower S R M]
variables [add_comm_monoid N] [semimodule S N] [semimodule R N] [is_scalar_tower S R N]
...
@[simp]
lemma restrict_scalars_bot : restrict_scalars S (⊥ : submodule R M) = ⊥ := rfl

@[simp]
lemma restrict_scalars_top : restrict_scalars S (⊤ : submodule R M) = ⊤ := rfl

/-- If `R` is an `S`-algebra, then the `S`-module generated by a set `X` is included in the
`R`-module generated by `X`. -/
lemma span_le_restrict_scalars (X : set M) : span S (X : set M) ≤ restrict_scalars S (span R X) :=
submodule.span_le.mpr submodule.subset_span

end submodule

@[simp]
lemma linear_map.ker_restrict_scalars (f : M →ₗ[R] N) :
  (f.restrict_scalars S).ker = f.ker.restrict_scalars S  :=
rfl
