...
/- none of this applies because currently all actions in mathlib need a `1`.
/-! ## Actions by `non_unital_subalgebra`s
...
/-- The action by a non_unital_subalgebra is the action by the underlying algebra. -/
instance [has_smul A α] (S : non_unital_subalgebra R A) : has_smul S α := S.to_non_unital_subsemiring.has_smul

lemma smul_def [has_smul A α] {S : non_unital_subalgebra R A} (g : S) (m : α) : g • m = (g : A) • m := rfl

instance smul_comm_class_left
  [has_smul A β] [has_smul α β] [smul_comm_class A α β] (S : non_unital_subalgebra R A) :
  smul_comm_class S α β :=
S.to_non_unital_subsemiring.smul_comm_class_left

instance smul_comm_class_right
  [has_smul α β] [has_smul A β] [smul_comm_class α A β] (S : non_unital_subalgebra R A) :
  smul_comm_class α S β :=
S.to_non_unital_subsemiring.smul_comm_class_right

/-- Note that this provides `is_scalar_tower S R R` which is needed by `smul_mul_assoc`. -/
instance is_scalar_tower_left
  [has_smul α β] [has_smul A α] [has_smul A β] [is_scalar_tower A α β] (S : non_unital_subalgebra R A) :
...
instance is_scalar_tower_mid {R S T : Type*} [comm_semiring R] [semiring S] [add_comm_monoid T]
  [algebra R S] [module R T] [module S T] [is_scalar_tower R S T] (S' : non_unital_subalgebra R S) :
  is_scalar_tower R S' T :=
⟨λ x y z, (smul_assoc _ (y : S) _ : _)⟩

instance [has_smul A α] [has_faithful_smul A α] (S : non_unital_subalgebra R A) :
  has_faithful_smul S α :=
S.to_non_unital_subsemiring.has_faithful_smul

/-- The action by a non_unital_subalgebra is the action by the underlying algebra. -/
instance [mul_action A α] (S : non_unital_subalgebra R A) : mul_action S α :=
S.to_non_unital_subsemiring.mul_action

/-- The action by a non_unital_subalgebra is the action by the underlying algebra. -/
instance [add_monoid α] [distrib_mul_action A α] (S : non_unital_subalgebra R A) : distrib_mul_action S α :=
S.to_non_unital_subsemiring.distrib_mul_action

/-- The action by a non_unital_subalgebra is the action by the underlying algebra. -/
instance [has_zero α] [smul_with_zero A α] (S : non_unital_subalgebra R A) : smul_with_zero S α :=
S.to_non_unital_subsemiring.smul_with_zero

/-- The action by a non_unital_subalgebra is the action by the underlying algebra. -/
instance [has_zero α] [mul_action_with_zero A α] (S : non_unital_subalgebra R A) : mul_action_with_zero S α :=
S.to_non_unital_subsemiring.mul_action_with_zero

/-- The action by a non_unital_subalgebra is the action by the underlying algebra. -/
instance module_left [add_comm_monoid α] [module A α] (S : non_unital_subalgebra R A) : module S α :=
S.to_non_unital_subsemiring.module

/-- The action by a non_unital_subalgebra is the action by the underlying algebra. -/
instance to_algebra {R A : Type*} [comm_semiring R] [comm_semiring A] [semiring α]
  [algebra R A] [algebra A α] (S : non_unital_subalgebra R A) : algebra S α :=
algebra.of_non_unital_subsemiring S.to_non_unital_subsemiring

lemma algebra_map_eq {R A : Type*} [comm_semiring R] [comm_semiring A] [semiring α]
  [algebra R A] [algebra A α] (S : non_unital_subalgebra R A) :
  algebra_map S α = (algebra_map A α).comp S.val := rfl

@[simp] lemma srange_algebra_map {R A : Type*} [comm_semiring R] [comm_semiring A]
  [algebra R A] (S : non_unital_subalgebra R A) :
  (algebra_map S A).srange = S.to_non_unital_subsemiring :=
by rw [algebra_map_eq, algebra.id.map_eq_id, ring_hom.id_comp, ← to_non_unital_subsemiring_subtype,
...
instance no_zero_smul_divisors_top [no_zero_divisors A] (S : non_unital_subalgebra R A) :
