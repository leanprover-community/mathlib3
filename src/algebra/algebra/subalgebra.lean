...
/-! ### Results about the subalgebra closure of a set (i.e., subalgebra generated by a set) -/
section closure

-- presentation in this section adapted almost directly from `subring.closure`

/-- The `subalgebra` generated by a set. -/
def closure  (s : set A) : subalgebra R A := Inf {S | s ⊆ S}

variables {R}

lemma mem_closure {x : A} {s : set A} : x ∈ closure R s ↔ ∀ S : subalgebra R A, s ⊆ S → x ∈ S :=
mem_Inf

/-- The subalgebra generated by a set includes the set. -/
@[simp] lemma subset_closure {s : set A} : s ⊆ closure R s := λ x hx, mem_closure.2 $ λ S hS, hS hx

/-- A subalgebra `t` includes `closure R s` if and only if it includes `s`. -/
@[simp]
lemma closure_le {s : set A} {t : subalgebra R A} : closure R s ≤ t ↔ s ⊆ t :=
⟨set.subset.trans subset_closure, λ h, Inf_le h⟩

/-- Subring closure of a set is monotone in its argument: if `s ⊆ t`,
then `closure R s ≤ closure R t`. -/
lemma closure_mono ⦃s t : set A⦄ (h : s ⊆ t) : closure R s ≤ closure R t :=
closure_le.2 $ set.subset.trans h subset_closure

lemma closure_eq_of_le {s : set A} {t : subalgebra R A} (h₁ : s ⊆ t) (h₂ : t ≤ closure R s) :
  closure R s = t :=
le_antisymm (closure_le.2 h₁) h₂

lemma span_le_closure (s : set A) : submodule.span R s ≤ (closure R s).to_submodule :=
by { rw submodule.span_le, exact subset_closure }

lemma subsemiring_closure_le_closure (s : set A) :
  subsemiring.closure s ≤ (closure R s).to_subsemiring :=
by { rw subsemiring.closure_le, exact subset_closure }

/-- An induction principle for closure membership. If `p` holds for `0`, `1`, the image of the
`algebra_map`, and all elements of `s`, and is preserved under addition and multiplication, then
`p` holds for all elements of the closure of `s`. -/
@[elab_as_eliminator]
lemma closure_induction {s : set A} {p : A → Prop} {x} (h : x ∈ closure R s)
  (Hs : ∀ x ∈ s, p x) (H0 : p 0) (H1 : p 1)
  (Hadd : ∀ x y, p x → p y → p (x + y))
  (Hmul : ∀ x y, p x → p y → p (x * y))
  (Hsmul : ∀ (c : R), p (algebra_map R A c)) : p x :=
(@closure_le R A _ _ _ _ ⟨p, H1, Hmul, H0, Hadd, Hsmul⟩).2 Hs h

lemma mem_closure_iff {s : set A} {x} :
  x ∈ closure R s ↔ x ∈ submodule.span R (submonoid.closure s : set A) :=
begin
  split,
  { refine λ h, closure_induction h
      (λ x hx, @submodule.subset_span R _ _ _ _ _ _ (submonoid.subset_closure hx))
      (submodule.span R _).zero_mem
      (submodule.subset_span (submonoid.closure s).one_mem)
      (λ x y hx hy, (submodule.span R _).add_mem hx hy) _ _,
    { refine λ x y hx hy, submodule.span_induction hy
        (λ q hq, submodule.span_induction hx
          (λ p hp, submodule.subset_span ((submonoid.closure s).mul_mem hp hq))
          _ (λ p₁ p₂ ihp₁ ihp₂, _) (λ c x hx, _))
        _ (λ q₁ q₂ ihq₁ ihq₂, _) (λ c z hz, _),
      { rw zero_mul q,
        exact (submodule.span R _).zero_mem },
      { rw add_mul p₁ p₂ q,
        exact (submodule.span R _).add_mem ihp₁ ihp₂ },
      { rw smul_mul_assoc,
        exact (submodule.span R _).smul_mem c hx },
      { rw mul_zero,
        exact (submodule.span R _).zero_mem },
      { rw mul_add x q₁ q₂,
        exact (submodule.span R _).add_mem ihq₁ ihq₂ },
      { rw mul_smul_comm c x z,
        exact (submodule.span R _).smul_mem c hz } },
    { intros c,
      rw algebra_map_eq_smul_one,
      exact (submodule.span R _).smul_mem c
        (submodule.subset_span (submonoid.closure s).one_mem) } },
  { exact λ h, submodule.span_induction h
      ...
      (λ c x hx, (closure R s).smul_mem hx c) },
end

lemma closure_le_of_submonoid_closure_le {s : set A} {t : submodule R A}
  (hs : ↑(submonoid.closure s) ⊆ (t : set A)) :
  (closure R s).to_submodule ≤ t :=
λ _ h, (submodule.span_le.mpr hs) (mem_closure_iff.mp h)

lemma closure_eq_span {s : set A} (hs : ↑(submonoid.closure s) ⊆ (submodule.span R s : set A)) :
  (closure R s).to_submodule = submodule.span R s :=
le_antisymm (closure_le_of_submonoid_closure_le hs) (span_le_closure s)

variables (R A)
/-- `closure` forms a Galois insertion with the coercion to set. -/
protected def gi : galois_insertion (@closure R A _ _ _) coe :=
{ choice := λ s _, closure R s,
  gc := λ s t, closure_le,
...
/-- Closure of a subalgebra `S` equals `S`. -/
lemma closure_eq (s : subalgebra R A) : closure R (s : set A) = s := (subalgebra.gi R A).l_u_eq s

@[simp] lemma closure_empty : closure R (∅ : set A) = ⊥ := (subalgebra.gi R A).gc.l_bot

@[simp] lemma closure_univ : closure R (set.univ : set A) = ⊤ := @coe_top R A _ _ _ ▸ closure_eq ⊤

lemma closure_union (s t : set A) : closure R (s ∪ t) = closure R s ⊔ closure R t :=
(subalgebra.gi R A).gc.l_sup

lemma closure_Union {ι} (s : ι → set A) : closure R (⋃ i, s i) = ⨆ i, closure R (s i) :=
(subalgebra.gi R A).gc.l_supr

lemma closure_sUnion (s : set (set A)) : closure R (⋃₀ s) = ⨆ t ∈ s, closure R t :=
(subalgebra.gi R A).gc.l_Sup
