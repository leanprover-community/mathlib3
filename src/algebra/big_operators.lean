...
/-- Geometric progression formulae with one variable;
  these work in noncommutative rings -/

theorem geom_sum_mul_add [semiring α] (x : α) :
  ∀ (n : ℕ), (geom_series (x + 1) n) * x + 1 = (x + 1) ^ n
| 0     := by simp [geom_series]
| (n+1) := calc (geom_series (x + 1) (n + 1)) * x + 1 =
        (x + 1)^n * x + ((geom_series (x + 1) n) * x + 1) :
    by simp [geom_series, range_add_one, add_mul]
  ... = (x + 1) ^ n * x + (x + 1) ^ n :
...
begin
  let h : (geom_series x n) * (x - 1) = x ^ n - 1 := geom_sum_mul x n,
  replace h := congr_arg has_neg.neg h,
  rw [neg_sub, ← mul_neg_eq_neg_mul_symm, neg_sub] at h,
  exact h
end

/-- Geometric progression formulae with two variables.
  These are valid provided that x and y commute, but we
  only prove them here under the assumption that the whole
  semiring is commutative.  (This prevents us from treating
  the one-variable version as a special case of the two-variable
  version.) -/

theorem geom_sum_mul_add₂ [comm_semiring α] (x y : α) (n : ℕ) :
  x * (geom_series₂ (x + y) y n) + y ^ n = (x + y) ^ n :=
begin
  let f := λ (m i : ℕ), (x + y) ^ i * y ^ (m - 1 - i),
  change x * ((range n).sum (f n)) + y ^ n = (x + y) ^ n,
  induction n with n ih,
  { rw [range_zero, sum_empty, mul_zero, zero_add, pow_zero, pow_zero] },
  { have f_last : f n.succ n = (x + y) ^ n :=
    ...
    have f_succ : ∀ i, i ∈ range n → f n.succ i = y * f n i :=
      λ i hi, by {
        dsimp [f],
        rw [← mul_assoc, mul_comm y, mul_assoc, ← pow_succ y (n - 1 - i)],
        congr' 2,
        ...
        rw [← this, nat.add_sub_cancel, add_comm i 1, ← add_assoc,
            nat.add_sub_cancel] },
    rw [pow_succ (x + y), add_mul, sum_range_succ, f_last, mul_add, add_assoc],
    congr' 1,
    rw [← ih, mul_add, pow_succ, ← mul_assoc, mul_comm y x, mul_assoc],
    congr' 2, rw[mul_sum], apply sum_congr rfl, exact f_succ }
end

theorem geom_sum_mul₂ [comm_ring α] (x y : α) (n : ℕ) :
  (geom_series₂ x y n) * (x - y) = x ^ n - y ^ n :=
begin
  have := geom_sum_mul_add₂ (x - y) y n,
  rw [sub_add_cancel, mul_comm] at this,
