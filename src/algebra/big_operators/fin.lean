...
@[to_additive] def mul_nth [has_mul G] (j : ℕ) (g : fin (n + 1) → G) (k : fin n) : G :=
...
@[to_additive] lemma mul_nth_lt_apply {j : ℕ} (g : fin (n + 1) → G) {k : fin n} (h : (k : ℕ) < j) :
  mul_nth j g k = g (cast_lt k (lt_trans k.2 $ lt_add_one _)) := if_pos h

@[to_additive] lemma mul_nth_eq_apply {j : ℕ} (g : fin (n + 1) → G) {k : fin n} (h : (k : ℕ) = j) :
...
@[to_additive] lemma mul_nth_neg_apply {j : ℕ} (g : fin (n + 1) → G) {k : fin n}
  (h : ¬(k : ℕ) < j) (h' : ¬(k : ℕ) = j) :
