...
- `char_p_zero_or_prime_power`: In a local ring the characteristics is either zero or a prime power.
...
begin
  ...
  { /- If `r` is prime, we can write it as `r = a * q^n`... -/
    ...
    begin
      ...
      rw ←(classical.some_spec a_unit.exists_left_inv),
      rw [mul_assoc, ←nat.cast_pow, ←nat.cast_mul, ←q_eq_a_mul_rn, char_p.cast_eq_zero R q],
      simp,
    end,
    have q_eq_rn := nat.dvd_antisymm ((char_p.cast_eq_zero_iff R q (r^n)).mp rn_cast_zero) rn_dvd_q,
    have n_pos : n ≠ 0 := begin
      by_contradiction n_zero,
      simp only [n_zero] at q_eq_rn,
  ...
  { /- `r = 0` is not possible as it would imply `q = 0`. -/
