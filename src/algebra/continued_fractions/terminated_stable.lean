...
by { rw [terminated_at_iff_s_none] at terminated_at_n,
     simp only [terminated_at_n, continuants_aux, option.elim] }
...
begin
  ...
  exact terminated_at_n.mono (nat.le_add_right _ _)
end
...
begin
  ...
  { refine (convergents'_aux_stable_step_of_terminated _).trans IH,
    exact terminated_at_n.mono n_le_m }
end

lemma continuants_stable_of_terminated (n_le_m : n ≤ m) (terminated_at_n : g.terminated_at n) :
  g.continuants.nth m = g.continuants.nth n :=
continuants_aux_stable_of_terminated (n_le_m.trans_lt m.lt_succ_self) terminated_at_n

lemma numerators_stable_of_terminated (n_le_m : n ≤ m) (terminated_at_n : g.terminated_at n) :
  g.numerators.nth m = g.numerators.nth n :=
...
lemma denominators_stable_of_terminated (n_le_m : n ≤ m) (terminated_at_n : g.terminated_at n) :
  g.denominators.nth m = g.denominators.nth n :=
...
lemma convergents_stable_of_terminated (n_le_m : n ≤ m) (terminated_at_n : g.terminated_at n) :
  g.convergents.nth m = g.convergents.nth n :=
by simp only [convergents, stream.nth_zip, denominators_stable_of_terminated n_le_m terminated_at_n,
