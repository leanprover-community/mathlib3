...
by { rw [terminated_at_iff_s_none] at terminated_at_n,
     simv only [terminated_at_n, continuants_aux] }
...
begin
  ...
  case nat.zero
  { simv only [convergents'_aux, terminated_at_n, seq.head] },
  case nat.succ
  { cases s_head_eq : s.head with gp_head,
    case option.none { simv only [convergents'_aux, s_head_eq] },
    case option.some
    { have : s.tail.terminated_at n, by simv only [seq.terminated_at, s.nth_tail, terminated_at_n],
      simv only [convergents'_aux, s_head_eq, (IH this)] } }
end
...
begin
  ...
  { refl },
  { cases s_head_eq : s.head with gp_head,
    case option.none { cases n; simv only [convergents'_aux, s_head_eq] },
    case option.some
...
by simv only [num_eq_conts_a, (continuants_stable_of_terminated n_le_m terminated_at_n)]

lemma denominators_stable_of_terminated (n_le_m : n ≤ m) (terminated_at_n : g.terminated_at n) :
  g.denominators m = g.denominators n :=
by simv only [denom_eq_conts_b, (continuants_stable_of_terminated n_le_m terminated_at_n)]

lemma convergents_stable_of_terminated (n_le_m : n ≤ m) (terminated_at_n : g.terminated_at n) :
  g.convergents m = g.convergents n :=
by simv only [convergents, (denominators_stable_of_terminated n_le_m terminated_at_n),
...
lemma convergents'_stable_of_terminated (n_le_m : n ≤ m) (terminated_at_n : g.terminated_at n) :
  g.convergents' m = g.convergents' n :=
by simv only [convergents', (convergents'_aux_stable_of_terminated n_le_m terminated_at_n)]
