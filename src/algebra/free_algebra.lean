...
Given a commutative semiring `R`, and a type `M`, we construct the free `R`-algebra on `M`.

## Notation

1. `free_algebra R M` is the free algebra itself. It is endowed with an R-algebra structure.
2. `free_algebra.ι R M` is the function `M → free_algebra R M`.
3. Given a function `f : M → A` to an R-algebra `A`, `lift R M f` is the lift of `f` to an
  `R`-algebra morphism `free_algebra R M → A`.

## Theorems

1. `ι_comp_lift` states that the composition `(lift R M f) ∘ (ι R M)` is identical to `f`.
2. `lift_unique` states that whenever an R-algebra morphism `g : free_algebra R M → A` is
...
As noted above, the free algebra on `M` is constructed as the free `R`-algebra generated by `M`.
...
1. We construct an inductive type `free_algebra.pre R M`, the terms of which should be thought
  of as representatives for the elements of `free_algebra R M`.
  It is the free type with maps from `R` and `M`, and with two binary operations `add` and `mul`.
2. We construct an inductive relation `free_algebra.rel R M` on `free_algebra.pre R M`.
...
3. The free algebra `free_algebra R M` is the quotient of `free_algebra.pre R M` by
  the relation `free_algebra.rel R M`.
-/
...
def has_coe_generator : has_coe M (pre R M) := ⟨of⟩
...
/--
Given a function from `M` to an `R`-algebra `A`, `lift_fun` provides a lift of `f` to a function
from `pre R M` to `A`. This is mainly used in the construction of `free_algebra.lift`.
-/
def lift_fun {A : Type*} [semiring A] [algebra R A] (f : M → A) : pre R M → A :=
...
/--
The free algebra for the type `M` over the commutative semiring `R`.
-/
def free_algebra := quot (free_algebra.rel R M)
...
instance : semiring (free_algebra R M) :=
...
instance : inhabited (free_algebra R M) := ⟨0⟩

instance : has_scalar R (free_algebra R M) :=
{ smul := λ r a, quot.lift_on a (λ x, quot.mk _ $ ↑r * x) $
  λ a b h, quot.sound (rel.mul_compat_right h) }

instance : algebra R (free_algebra R M) :=
...
/--
The canonical function `M → free_algebra R M`.
-/
...
-/
def lift {A : Type*} [semiring A] [algebra R A] (f : M → A) : free_algebra R M →ₐ[R] A :=
...
@[simp]
theorem ι_comp_lift {A : Type*} [semiring A] [algebra R A] (f : M → A) :
...
begin
  ...
  rcases x,
  induction x,
  { change ((g : free_algebra R M → A) ∘ (ι R M)) _ = _,
    rw hyp,
    refl },
