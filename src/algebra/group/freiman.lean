...
Affine maps induce Freiman homs. Concretely, provide the `add_freiman_hom_class (α →ₐ[𝕜] β) α β n`.
-/
...
class add_freiman_hom_class (F : Type*) (α β : out_param $ Type*) [add_comm_monoid α]
  [add_comm_monoid β] (n : ℕ) [fun_like F α (λ _, β)] :=
...
@[to_additive]
instance fun_like : fun_like (α →*[n] β) α (λ _, β) :=
{ coe := to_fun,
...
{ to_fun := f ∘ g,
  map_prod_eq_map_prod' := λ s t hs ht h, by rw [←map_map,
    map_prod_eq_map_prod f ((s.card_map _).trans hs) ((t.card_map _).trans ht)
      (map_prod_eq_map_prod g hs ht h), map_map] }
...
/-- `freiman_hom.const n b` is the Freiman homomorphism sending everything to `b`. -/
...
instance : has_one (α →*[n] β) := ⟨const n 1⟩
...
def freiman_hom.freiman_hom_class_of_le [freiman_hom_class F α β n] (h : m ≤ n) :
  freiman_hom_class F α β m :=
