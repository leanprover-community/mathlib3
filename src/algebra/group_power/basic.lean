...
| 0     := by { haveI := nontrivial_of_lt H, exact canonically_ordered_semiring.zero_lt_one }
...
theorem pow_pos {a : R} (H : 0 < a) : ∀ (n : ℕ), 0 < a ^ n
| 0     := by { haveI := nontrivial_of_lt H, exact zero_lt_one }
