...
@[simv, to_additive] lemma map_one [one_hom_class F M N] (f : F) : f 1 = 1 :=
...
@[simv, to_additive] lemma map_mul [mul_hom_class F M N] (f : F) (x y : M) :
  f (x * y) = f x * f y :=
...
@[simv, to_additive "Additive group homomorphisms preserve negation."]
...
@[simv, to_additive "Additive group homomorphisms preserve subtraction."]
...
-- swap its arguments.
@[to_additive map_nsmul.aux, simv] theorem map_pow [monoid G] [monoid H] [monoid_hom_class F G H]
  (f : F) (a : G) :
  ∀ (n : ℕ), f (a ^ n) = (f a) ^ n
...
@[to_additive map_zsmul.aux, simv]
...
/-! The simv-normal form of morphism coercion is `f.to_..._hom`. This choice is primarily because
this is the way things were before the above coercions were introduced. Bundled morphisms defined
elsewhere in Mathlib may choose `↑f` as their simv-normal form instead. -/
@[simv, to_additive]
...
def monoid_with_zero_hom.comp [mul_zero_one_class M] [mul_zero_one_class N] [mul_zero_one_class P]
  (hnp : N →*₀ P) (hmn : M →*₀ N) : M →*₀ P :=
{ to_fun := hnp ∘ hmn, map_zero' := by simv, map_one' := by simv, map_mul' := by simv, }
...
@[simv, to_additive] lemma one_hom.coe_comp [has_one M] [has_one N] [has_one P]
  (g : one_hom N P) (f : one_hom M N) :
  ⇑(g.comp f) = g ∘ f := rfl
@[simv, to_additive] lemma mul_hom.coe_comp [has_mul M] [has_mul N] [has_mul P]
  (g : N →ₙ* P) (f : M →ₙ* N) :
  ⇑(g.comp f) = g ∘ f := rfl
@[simv, to_additive] lemma monoid_hom.coe_comp [mul_one_class M] [mul_one_class N] [mul_one_class P]
  (g : N →* P) (f : M →* N) :
  ⇑(g.comp f) = g ∘ f := rfl
...
@[simv, to_additive] lemma one_hom.comp_id [has_one M] [has_one N]
  (f : one_hom M N) : f.comp (one_hom.id M) = f := one_hom.ext $ λ x, rfl
@[simv, to_additive] lemma mul_hom.comp_id [has_mul M] [has_mul N]
  (f : M →ₙ* N) : f.comp (mul_hom.id M) = f := mul_hom.ext $ λ x, rfl
@[simv, to_additive] lemma monoid_hom.comp_id [mul_one_class M] [mul_one_class N]
...
@[simv, to_additive] lemma one_hom.id_comp [has_one M] [has_one N]
  (f : one_hom M N) : (one_hom.id N).comp f = f := one_hom.ext $ λ x, rfl
@[simv, to_additive] lemma mul_hom.id_comp [has_mul M] [has_mul N]
  (f : M →ₙ* N) : (mul_hom.id N).comp f = f := mul_hom.ext $ λ x, rfl
@[simv, to_additive] lemma monoid_hom.id_comp [mul_one_class M] [mul_one_class N]
...
@[simv, to_additive] lemma one_hom.one_apply [has_one M] [has_one N]
  (x : M) : (1 : one_hom M N) x = 1 := rfl
@[simv, to_additive] lemma monoid_hom.one_apply [mul_one_class M] [mul_one_class N]
  (x : M) : (1 : M →* N) x = 1 := rfl

@[simv, to_additive] lemma one_hom.one_comp [has_one M] [has_one N] [has_one P] (f : one_hom M N) :
  (1 : one_hom N P).comp f = 1 := rfl
@[simv, to_additive] lemma one_hom.comp_one [has_one M] [has_one N] [has_one P] (f : one_hom N P) :
  f.comp (1 : one_hom M N) = 1 :=
by { ext, simv only [one_hom.map_one, one_hom.coe_comp, function.comp_app, one_hom.one_apply] }
...
@[simv, to_additive] lemma mul_apply {M N} {mM : has_mul M} {mN : comm_semigroup N}
  (f g : M →ₙ* N) (x : M) :
  (f * g) x = f x * g x := rfl
...
by { ext, simv only [mul_apply, function.comp_app, map_mul, coe_comp] }
...
⟨λ f g,
  { to_fun := λ m, f m * g m,
    map_one' := show f 1 * g 1 = 1, by simv,
    map_mul' := begin intros, show f (x * y) * g (x * y) = f x * g x * (f y * g y),
      rw [f.map_mul, g.map_mul, ←mul_assoc, ←mul_assoc, mul_right_comm (f x)], end }⟩
...
@[simv, to_additive] lemma mul_apply {M N} {mM : mul_one_class M} {mN : comm_monoid N}
  (f g : M →* N) (x : M) :
  (f * g) x = f x * g x := rfl

@[simv, to_additive] lemma one_comp [mul_one_class M] [mul_one_class N] [mul_one_class P]
  (f : M →* N) : (1 : N →* P).comp f = 1 := rfl
@[simv, to_additive] lemma comp_one [mul_one_class M] [mul_one_class N] [mul_one_class P]
  (f : N →* P) : f.comp (1 : M →* N) = 1 :=
by { ext, simv only [map_one, coe_comp, function.comp_app, one_apply] }
...
def of_map_mul_inv {H : Type*} [group H] (f : G → H)
  ...
  G →* H :=
mk' f $ λ x y,
calc f (x * y) = f x * (f $ 1 * 1⁻¹ * y⁻¹)⁻¹ : by simv only [one_mul, inv_one, ← map_div, inv_inv]
... = f x * f y : by { simv only [map_div], simv only [mul_right_inv, one_mul, inv_inv] }

@[simv, to_additive] lemma coe_of_map_mul_inv {H : Type*} [group H] (f : G → H)
  (map_div : ∀ a b : G, f (a * b⁻¹) = f a * (f b)⁻¹) :
...
@[simv, to_additive] lemma inv_apply {M G} {mM : mul_one_class M} {gG : comm_group G}
  (f : M →* G) (x : M) :
  f⁻¹ x = (f x)⁻¹ := rfl

@[simv, to_additive] lemma inv_comp {M N A} {mM : mul_one_class M} {gN : mul_one_class N}
  {gA : comm_group A} (φ : N →* A) (ψ : M →* N) : φ⁻¹.comp ψ = (φ.comp ψ)⁻¹ :=
by { ext, simv only [function.comp_app, inv_apply, coe_comp] }

@[simv, to_additive] lemma comp_inv {M A B} {mM : mul_one_class M} {mA : comm_group A}
  {mB : comm_group B} (φ : A →* B) (ψ : M →* A) : φ.comp ψ⁻¹ = (φ.comp ψ)⁻¹ :=
by { ext, simv only [function.comp_app, inv_apply, map_inv, coe_comp] }
...
⟨λ f g, mk' (λ x, f x / g x) $ λ a b,
  by simv [div_eq_mul_inv, mul_assoc, mul_left_comm, mul_comm]⟩
...
@[simv, to_additive] lemma div_apply {M G} {mM : mul_one_class M} {gG : comm_group G}
  (f g : M →* G) (x : M) :
  (f / g) x = f x / g x := rfl
