...
left_inv_eq_right_inv (inv_of_mul_self _ _) hac

lemma inv_of_eq_left_inv [monoid α] {a b : α} [invertible a] (hac : b * a = 1) : ⅟a = b :=
(left_inv_eq_right_inv hac (mul_inv_of_self _ _)).symm
...
⟨ a, mul_inv_of_self a _, inv_of_mul_self a _ ⟩

@[simp] lemma inv_of_inv_of [monoid α] (a : α) (_ : invertible a) (_ : invertible (⅟a))
    : ⅟(⅟a) = a :=
  inv_of_eq_right_inv (inv_of_mul_self _ _)
