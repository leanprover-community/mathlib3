...
@[simp] lemma mul_right_iterate [monoid M] (a : M) {n : ℕ} : (* a)^[n] = (* a ^ n) :=
begin
...
@[simp] lemma add_right_iterate [add_monoid M] (x : M) {n : ℕ} : (+ x)^[n] = (+ n •ℕ x) :=
begin
  induction n with d hd,
  { simpa },
  { simpa [hd] }
...
lemma mul_right_iterate_apply_one [monoid M] (a : M) {n : ℕ} : (* a)^[n] 1 = a ^ n :=
by simp [mul_right_iterate]

lemma add_right_iterate_apply_zero [add_monoid M] (x : M) {n : ℕ} : (+ x)^[n] 0 = n •ℕ x :=
