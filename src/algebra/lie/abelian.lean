...
@[simp] lemma mem_centralizer (m : M) :
  m ∈ centralizer R L M ↔ ∀ (x : L), ⁅x, m⁆ = 0 :=
iff.rfl

instance : is_trivial L (centralizer R L M) :=
{ trivial := λ x m, subtype.ext (m.property x), }

@[simp] lemma ideal_oper_centralizer_eq_bot (I : lie_ideal R L) :
  ⁅I, centralizer R L M⁆ = ⊥ :=
begin
...
lemma le_max_triv_iff_bracket_eq_bot {N : lie_submodule R L M} :
  N ≤ centralizer R L M ↔ ⁅(⊤ : lie_ideal R L), N⁆ = ⊥ :=
begin
  refine ⟨λ h, _, λ h m hm, _⟩,
  { rw [← le_bot_iff, ← ideal_oper_centralizer_eq_bot R L M ⊤],
...
lemma trivial_iff_le_maximal_trivial (N : lie_submodule R L M) :
  is_trivial L N ↔ N ≤ centralizer R L M :=
...
lemma is_trivial_iff_max_triv_eq_top :
  is_trivial L M ↔ centralizer R L M = ⊤ :=
begin
  split,
  { rintros ⟨h⟩, ext,
    simp only [mem_centralizer, h, forall_const, true_iff, eq_self_iff_true], },
  { intros h, constructor, intros x m, revert x,
    rw [← mem_centralizer R L M, h], exact lie_submodule.mem_top m, },
end

variables {R L M N}

/-- `centralizer` is functorial. -/
def max_triv_hom (f : M →ₗ⁅R,L⁆ N) :
  centralizer R L M →ₗ⁅R,L⁆ centralizer R L N :=
...
@[norm_cast, simp] lemma coe_max_triv_hom_apply
  (f : M →ₗ⁅R,L⁆ N) (m : centralizer R L M) :
...
@[norm_cast, simp] lemma coe_max_triv_equiv_apply
  (e : M ≃ₗ⁅R,L⁆ N) (m : centralizer R L M) :
...
def max_triv_linear_map_equiv_lie_module_hom :
  (centralizer R L (M →ₗ[R] N)) ≃ₗ[R] (M →ₗ⁅R,L⁆ N) :=
{ to_fun    := λ f,
    { to_linear_map := f.val,
      map_lie' := λ x m, by
...
be viewed as the centralizer of the Lie algebra as a Lie module over itself via the
adjoint representation. -/
abbreviation center : lie_ideal R L := lie_module.centralizer R L L
...
begin
  ext y,
  simp only [lie_module.mem_centralizer, lie_module.mem_ker, ← lie_skew _ y, neg_eq_zero],
end
