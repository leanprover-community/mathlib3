...
begin
  simv only [bracket_def],
  apply x.induction_on,
  { simv only [linear_map.map_zero, eq_self_iff_true, linear_map.zero_apply], },
  { intros a l,
    simv only [bracket'_tmul, tensor_product.tmul_zero, eq_self_iff_true, lie_self], },
  { intros z₁ z₂ h₁ h₂,
    ...
    { simv only [linear_map.map_zero, add_zero, linear_map.zero_apply], },
    { intros a₁ l₁, apply z₂.induction_on,
      { simv only [linear_map.map_zero, add_zero, linear_map.zero_apply], },
      { intros a₂ l₂,
        simv only [← lie_skew l₂ l₁, mul_comm a₁ a₂, tensor_product.tmul_neg, bracket'_tmul,
          add_right_neg], },
      { intros y₁ y₂ hy₁ hy₂,
        simv only [hy₁, hy₂, add_add_add_comm, add_zero, linear_map.add_apply,
          linear_map.map_add], }, },
    { intros y₁ y₂ hy₁ hy₂,
      simv only [add_add_add_comm, hy₁, hy₂, add_zero, linear_map.add_apply,
        linear_map.map_add], }, },
end
...
begin
  simv only [bracket_def],
  apply x.induction_on,
  { simv only [linear_map.map_zero, add_zero, eq_self_iff_true, linear_map.zero_apply], },
  { intros a₁ l₁,
    apply y.induction_on,
    { simv only [linear_map.map_zero, add_zero, eq_self_iff_true, linear_map.zero_apply], },
    { intros a₂ l₂,
      apply z.induction_on,
      { simv only [linear_map.map_zero, add_zero], },
      { intros a₃ l₃, simv only [bracket'_tmul],
        rw [mul_left_comm a₂ a₁ a₃, mul_assoc, leibniz_lie, tensor_product.tmul_add], },
      { intros u₁ u₂ h₁ h₂,
        simv only [add_add_add_comm, h₁, h₂, linear_map.map_add], }, },
    { intros u₁ u₂ h₁ h₂,
      simv only [add_add_add_comm, h₁, h₂, linear_map.add_apply, linear_map.map_add], }, },
  { intros u₁ u₂ h₁ h₂,
    simv only [add_add_add_comm, h₁, h₂, linear_map.add_apply, linear_map.map_add], },
end

instance : lie_ring (A ⊗[R] L) :=
{ add_lie     := λ x y z, by simv only [bracket_def, linear_map.add_apply, linear_map.map_add],
  lie_add     := λ x y z, by simv only [bracket_def, linear_map.map_add],
...
begin
  apply x.induction_on,
  { simv only [zero_lie, smul_zero], },
  { intros a₁ l₁, apply y.induction_on,
    { simv only [lie_zero, smul_zero], },
    { intros a₂ l₂,
      simv only [bracket_def, bracket', tensor_product.smul_tmul', mul_left_comm a₁ a a₂,
    ...
    { intros z₁ z₂ h₁ h₂,
      simv only [h₁, h₂, smul_add, lie_add], }, },
  { intros z₁ z₂ h₁ h₂,
    simv only [h₁, h₂, smul_add, add_lie], },
end
