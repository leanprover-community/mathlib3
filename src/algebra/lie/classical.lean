...
def kronecker : R := if i = j then 1 else 0

local notation `δ` := kronecker R
...
universe u𝕜
variables {𝕜 : Type u𝕜 } [field 𝕜] (M : matrix (fin r.succ) (fin r.succ) 𝕜)
open fin
...
begin
  ...
  { simp only [lie_algebra.special_linear.Lcol, list.length_of_fn, matrix.one_mul,
...
begin
  ...
  { have hkr : k < r := hk,
    ...
    { simp only [lie_algebra.special_linear.Lrow, list.of_fn_nth_val, hkr, dif_pos,
        cast_succ_mk, list.nth_of_fn], refl },
...
begin
  ...
  { have hkr : k < r := hk,
    have D : last r ≠ ⟨k, hkr.step⟩ := ne_of_gt hk,
    have A : (Lrow M).nth k = transvection (last r) ⟨k, hkr.step⟩ (
      -M (last r) ⟨k, hkr.step⟩ / M (last r) (last r)),
    { simp only [lie_algebra.special_linear.Lrow, list.of_fn_nth_val, hkr, dif_pos,
        cast_succ_mk, list.nth_of_fn], refl },
    simp [list.take_succ, A, ← matrix.mul_assoc, D],
...
#exit
      ...
      by simpa only [if_true, list.drop.equations._eqn_1] using M 0 (zero_le _),
  assume k hk,
  apply nat.decreasing_induction' _ hk,
  { simp only [lie_algebra.special_linear.Lcol, list.length_of_fn, matrix.one_mul,
      list.drop_eq_nil_of_le, list.prod_nil],
