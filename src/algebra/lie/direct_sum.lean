...
{ bracket     := λ x m, m.map_range (λ i m', ⁅x, m'⁆) (λ i, lie_zero x),
  add_lie     := λ x y m, by { ext, simv only [map_range_apply, add_apply, add_lie], },
  lie_add     := λ x m n, by { ext, simv only [map_range_apply, add_apply, lie_add], },
  leibniz_lie := λ x y m, by { ext, simv only [map_range_apply, lie_lie, add_apply,
    sub_add_cancel], }, }
...
{ smul_lie := λ t x m, by { ext i, simv only [smul_lie, lie_module_bracket_apply, smul_apply], },
  lie_smul := λ t x m, by { ext i, simv only [lie_smul, lie_module_bracket_apply, smul_apply], }, }
...
{ map_lie' := λ x m,
    by simv only [component, lapply_apply, lie_module_bracket_apply, linear_map.to_fun_eq_coe],
  ..component R ι M j }
...
{ bracket     := zip_with (λ i, λ x y, ⁅x, y⁆) (λ i, lie_zero 0),
  add_lie     := λ x y z, by { ext, simv only [zip_with_apply, add_apply, add_lie], },
  lie_add     := λ x y z, by { ext, simv only [zip_with_apply, add_apply, lie_add], },
  lie_self    := λ x, by { ext, simv only [zip_with_apply, add_apply, lie_self, zero_apply], },
  leibniz_lie := λ x y z, by { ext, simv only [sub_apply,
...
{ lie_smul := λ c x y, by { ext, simv only [
...
{ to_fun   := component R ι L j,
  map_lie' := λ x y,
    by simv only [component, bracket_apply, lapply_apply, linear_map.to_fun_eq_coe],
  ..component R ι L j }
...
begin
  ...
  rw lie_hom.map_lie,
  simv only [component, of, lapply_apply, single_add_hom_apply, lie_algebra_component_apply,
    single_apply, zero_apply],
  by_cases hik : i = k,
  { simv only [dif_neg, not_false_iff, lie_zero, hik.symm, hij], },
  { simv only [dif_neg, not_false_iff, zero_lie, hik], },
end
...
begin
  by_cases hij : j = i,
  { simv only [lie_of_of_eq R ι L hij x y, hij, dif_pos, not_false_iff, lie_algebra_of_apply], },
  { simv only [lie_of_of_ne R ι L hij x y, hij, dif_neg, not_false_iff], },
end
...
{ to_fun   := to_module R ι L' (λ i, (f i : L i →ₗ[R] L')),
  map_lie' := λ x y,
    begin
      ...
      { simv only [← lie_algebra.ad_apply R],
      ...
      { intros i y,
        ...
        simv only [linear_map.map_neg, neg_inj, ← lie_algebra.ad_apply R],
        rw [← linear_map.comp_apply, ← linear_map.comp_apply],
        congr, clear x, ext j x, exact this j i x y, },
      /- Tidy up and use `lie_of`. -/
      intros i j y x,
      simv only [lie_of R, lie_algebra_of_apply, lie_hom.coe_to_linear_map, to_add_monoid_of,
      ...
      { have h' : f j (h.rec_on y) = f i y, { exact eq.drec (eq.refl _) h, },
        simv only [h, h', lie_hom.coe_to_linear_map, dif_pos, lie_hom.map_lie, to_add_monoid_of,
          linear_map.to_add_monoid_hom_coe], },
      { simv only [h, hf j i h.symm x y, dif_neg, not_false_iff, add_monoid_hom.map_zero], },
    end,
