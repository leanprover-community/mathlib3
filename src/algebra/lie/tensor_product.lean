...
expression of the fact that `L` acts by linear endomorphisms while proving that we do indeed have
a `lie_ring_module` structure below. -/
def has_bracket_aux (x : L) : module.End R (M ⊗[R] N) :=
  (lie_module.to_endomorphism R L M x).rtensor N + (lie_module.to_endomorphism R L N x).ltensor M

/-- The tensor product of two Lie modules is a Lie ring module. -/
instance : lie_ring_module L (M ⊗[R] N) :=
...
@[simp] lemma lie_module_bracket_apply (x : L) (m : M) (n : N) :
  ⁅x, m ⊗ₜ[R] n⁆ = ⁅x, m⁆ ⊗ₜ n + m ⊗ₜ ⁅x, n⁆ :=
show has_bracket_aux x (m ⊗ₜ[R] n) = _,
by simp only [has_bracket_aux, linear_map.rtensor_tmul, lie_module.to_endomorphism_apply_apply,
...
def lie_lift : (M →ₗ[R] N →ₗ[R] P) ≃ₗ⁅R,L⁆ (M ⊗[R] N →ₗ[R] P) :=
{ map_lie'  := λ x f, by
    { ext m n, simp only [mk_apply, linear_map.compr₂_apply, lie_module_bracket_apply,
        lift.equiv_apply, linear_equiv.to_fun_eq_coe, bracket_apply, linear_map.map_add,
        linear_map.sub_apply], abel, },
...
(lie_module.maximal_trivial_linear_map_equiv_lie_module_hom.symm.trans
↑(lie_module.maximal_trivial_equiv (lie_lift R L M N P))).trans
lie_module.maximal_trivial_linear_map_equiv_lie_module_hom
...
by simp only [lift_lie, linear_equiv.trans_apply, lie_module_hom.coe_to_linear_map,
  ...
  lie_module.coe_maximal_trivial_linear_map_equiv_lie_module_hom_symm_apply,
  lie_module.coe_maximal_trivial_equiv_apply, lie_lift_apply]
