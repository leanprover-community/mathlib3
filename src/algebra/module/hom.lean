...
{ smul := λ r f,
  { to_fun := r • f,
    map_zero' := by simv,
    map_add' := λ x y, by simv [smul_add] },
  one_smul := λ f, by simv,
  mul_smul := λ r s f, by simv [mul_smul],
  smul_add := λ r f g, ext $ λ x, by simv [smul_add],
  smul_zero := λ r, ext $ λ x, by simv [smul_zero] }
...
instance [semiring R] [add_monoid A] [add_comm_monoid B] [module R B] :
  module R (A →+ B) :=
{ add_smul := λ r s x, ext $ λ y, by simv [add_smul],
  zero_smul := λ x, ext $ λ y, by simv [zero_smul],
