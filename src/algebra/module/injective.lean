...
/--An `R`-module `Q` satisfies Baer's criterion if any `R`-linear map from an `ideal R` extends to an
`R`-linear map `R ⟶ Q`-/
...
`pcik_submodule` picks that submodule-/
...
i.e. underlying submodule of some `extension_of` f, `pcik_extension_of` picks that extension.
-/
...
begin
  ...
  { rw ext_iff at eq_ineq,
    ...
    convert h2 _, },
  { rcases hchain (pick_extension_of_mem hchain hnonempty x) mem1 eq_ineq with
      ⟨⟨le1, h1⟩⟩|⟨⟨le1, h1⟩⟩,
    all_goals {
...
def extension_of_max_adjoin.extend_ideal_to (h : module.Baer R Q) (y : N) :
  R →ₗ[R] Q :=
...
begin
  rw extension_of_max_adjoin.extend_ideal_to_is_extension hi f h y r begin
    change r • y ∈ _,
    rw eq1,
...
begin
  rw extension_of_max_adjoin.extend_ideal_to_is_extension hi f h _ _ hr,
  refl,
end
...
begin
  ...
  have eq3 := extension_of_max_adjoin.extend_ideal_to_eq hi f h
    (r - extension_of_max_adjoin.snd x) begin
      rw ← eq2,
      exact submodule.sub_mem _ (extension_of_max_adjoin.fst x).2 a.2,
    end,
...
{ to_submodule := (extension_of_max hi f).to_submodule ⊔ submodule.span R {y},
    ...
    { to_fun := extension_of_max_adjoin.extension_to_fun hi f h,
      map_add' := λ a b, begin
        ...
        { change a.1 + b.1 = _ + _,
          ...
          change _ = _ + _ + _,
          abel, },
        rw [extension_of_max_adjoin.extension_to_fun_wd hi f h (a + b) _ _ eq1, map_add,
          map_add],
        unfold extension_of_max_adjoin.extension_to_fun,
        abel,
      end,
  ...
  is_extension := λ m, begin
    ...
    rw (extension_of_max hi f).is_extension,
    have eq1 : (⟨i m, mem⟩ : (extension_of_max hi f).to_submodule ⊔ submodule.span R {y}).1 =
      (⟨i m, (extension_of_max hi f).le ⟨_, rfl⟩⟩ :
        (extension_of_max hi f).to_submodule).1 + (0 : R) • y,
    { rw [zero_smul, add_zero], },
    rw [extension_of_max_adjoin.extension_to_fun_wd hi f h ⟨i m, mem⟩
      ⟨i m, (extension_of_max hi f).le ⟨_, rfl⟩⟩ 0 eq1, map_zero, add_zero],
  end }
...
⟨le_sup_left, λ m, begin
  generalize_proofs mem,
  have eq1 : (⟨m, mem⟩ : (extension_of_max hi f).to_submodule ⊔ submodule.span R {y}).1 =
      m.1 + (0 : R) • y,
    { rw [zero_smul, add_zero], refl, },
  change extension_of_max_adjoin.extension_to_fun hi f h _ = _,
...
begin
  ...
  have : (extension_of_max_adjoin hi f h y).to_submodule = _ :=
  by rw extension_of_max_eq hi f h,
...
{ out := λ X Y ins1 ins2 ins3 ins4 i hi f, begin
  ...
  { to_fun := λ y, (extension_of_max hi f).extension ⟨y.1, by { rw eq1, exact y.2}⟩,
  ...
  intros x,
  have is_extension := (extension_of_max hi f).is_extension x,
  simp only [linear_map.coe_mk],
  rw is_extension,
end }
