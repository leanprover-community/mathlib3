...
{ smul := λ a S, S.map (distrib_mul_action.to_linear_map R M a),
  one_smul := λ S,
    (congr_arg (λ f : M →ₗ[R] M, S.map f) (linear_map.ext $ by exact one_smul α)).trans S.map_id,
  mul_smul := λ a₁ a₂ S,
...
⟨λ a S, congr_arg (λ f : M →ₗ[R] M, S.map f) $ linear_map.ext $ by exact op_smul_eq_smul _⟩
