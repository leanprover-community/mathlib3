...
lemma add_left_cancel_of_covariant {A} [has_add A] [linear_order A] [covariant_class A A (+) (<)]
...
lemma add_right_cancel_of_covariant {A} [has_add A] [linear_order A]
...
{ add_right_cancel := begin
    ...
    exact λ a b c, add_right_cancel_of_covariant
  end,
...
begin
  ...
  { le          := eq,
    ...
    le_trans    := λ _ _ _, eq.trans,
    le_antisymm := λ _ _ h _, h },
  exact @add_monoid_algebra.no_zero_divisors.of_left_ordered R (lex (α →₀ N)) _ _ _
    finsupp.lex.linear_order _,
end
