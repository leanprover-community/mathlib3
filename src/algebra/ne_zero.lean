...
lemma ne_zero.ne' (n : ℕ) (R) [has_nat_cast R] [h : ne_zero (n : R)] :
  (n : R) ≠ 0 := h.out
...
instance char_zero [ne_zero n] [has_nat_cast M] [char_zero M] : ne_zero (n : M) :=
...
lemma of_not_dvd [has_nat_cast M] [char_p M p] (h : ¬ p ∣ n) : ne_zero (n : M) :=
...
lemma of_ne_zero_coe [has_nat_cast R] [h : ne_zero (n : R)] : ne_zero n :=
⟨by {casesI h, rintro rfl, by simpa using h}⟩

lemma not_char_dvd [has_nat_cast R] (p : ℕ) [char_p R p] (k : ℕ) [h : ne_zero (k : R)] :
  ¬ p ∣ k :=
by rwa [←not_iff_not.mpr $ char_p.cast_eq_zero_iff R p k, ←ne.def, ←ne_zero_iff]

lemma pos_of_ne_zero_coe [has_nat_cast R] [ne_zero (n : R)] : 0 < n :=
