...
structure mul_opposite (α : Type u) : Type u := (unop : α)

attribute [pp_using_anonymous_constructor] mul_opposite

/-- The element of `α` represented by `x : αᵐᵒᵖ`. -/
add_decl_doc mul_opposite.unop
...
⟨λ x ⟨y⟩ z, by simp [smul_mul_assoc]⟩
...
{ to_fun := λ u, op ⟨unop u, unop ↑(u⁻¹), op_injective u.4, op_injective u.3⟩,
  inv_fun := λ u, ⟨op ↑(unop u), op ↑(unop u⁻¹), congr_arg op (unop u).4, congr_arg op (unop u).3⟩,
...
add_monoid_hom.ext $ λ ⟨x⟩, (add_monoid_hom.congr_fun h : _) x
