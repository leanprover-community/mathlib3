...
let ⟨N, hN⟩ := pow_unbounded_of_one_lt x⁻¹ hy in
  have he: ∃ m : ℤ, y ^ m ≤ x, from
    ⟨-N, le_of_lt (by { rw [fzpow_neg y (↑N), zpow_coe_nat],
...
⟨-(m+1),
by rwa [fzpow_neg, inv_lt (fzpow_pos_of_pos hyp _) hx],
by rwa [neg_add, neg_add_cancel_right, fzpow_neg,
        le_inv hx (fzpow_pos_of_pos hyp _)]⟩
