...
/-!
# Use covariant_class and contravariant_class in the presence of addition and multiplication.

I am changing the file `algebra/order/ring.lean` incrementally, with the idea of merging the
final version with it.
-/
namespace new
open new function mul_opposite
...
instance ordered_semiring.to_covariant_class_scalar :
  covariant_class {x : α // 0 < x} α (•) (<) :=
{ elim := λ c a b ab, ordered_semiring.mul_lt_mul_of_pos_left a b c ab c.2 }

instance ordered_semiring.to_covariant_class_scalar_opp :
