...
{ ..(show covariant_class (with_zero α) (with_zero α) (*) (≤),
    from ⟨λ a b c h, with_zero.mul_le_mul_left _ _ h _⟩),
...
begin
  ...
  { rw le_antisymm h bot_le at h₂,
    exact ⟨_, h₂, le_refl _⟩ },
  cases b,
  { change a + 0 = some ca at h₂,
    simp at h₂, simp [h₂],
    exact ⟨_, rfl, by simpa using add_le_add_left (zero_le c) _⟩ },
  { simp at h,
    cases a; change some _ = _ at h₂;
...
begin
  ...
  rintros a b (_|c) h, { simp [none_eq_top] },
  rcases a with (_|a), { simp [none_eq_top] },
...
begin
  ...
  { intros a b c h ca h₂,
    cases a, {cases h₂},
    cases b; cases h₂,
    cases c with b, {cases le_antisymm h bot_le},
    simp at h,
