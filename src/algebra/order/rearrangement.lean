...
lemma equality_case' [no_zero_smul_divisors α β] (hfg : monovary_on f g s) (hσ : {x | σ x ≠ x} ⊆ s) :
  ∑ i in s, f i • g (σ i) = ∑ i in s, f i • g i ↔ ∀ i j ∈ {x | σ x ≠ x}, f i = f j ∨ g i = g j :=
begin
  classical,
  split,
  { revert σ hfg,
    apply finset.induction_on_max_value (λ i, to_lex (g i, f i)) s,
    { simp only [coe_empty, monovary_on.empty, ne.def, set.mem_set_of_eq, finset.sum_empty,
        eq_self_iff_true, not_true, forall_exists_index, and_imp, forall_true_left],
      intros σ hσ i hi j hj,
      simp only [set.subset_empty_iff, set.ext_iff, set.mem_set_of_eq, set.mem_empty_eq,
        iff_false, not_not] at hσ,
      specialize hσ i,
      contradiction },
    { intros a s has hamax hind σ hfg hσ heq i hi j hj,
      set τ : perm ι := σ.trans (swap a (σ a)) with hτ,
      have hτs : {x | τ x ≠ x} ⊆ s,
      { intros x hx,
        simp only [ne.def, set.mem_set_of_eq, equiv.coe_trans, equiv.swap_comp_apply] at hx,
        ...
        { exact (hx $ σ.injective h₂.symm).elim },
        { exact mem_of_mem_insert_of_ne (hσ hx) (ne_of_apply_ne _ h₂) } },
      by_cases h1 : ∑ (i : ι) in s, f i • g (τ i) = ∑ (i : ι) in s, f i • g i,
      { specialize hind (hfg.subset $ subset_insert _ _) hτs h1,
        have : (f a - f (σ⁻¹ a)) • (g a - g (σ a)) = 0 := by sorry,
        obtain hσa | hσa := eq_or_ne a (σ a),
        { simp only [equiv.coe_trans, function.comp_app, ne.def, set.mem_set_of_eq] at hind hi hj,
          rw [← hσa, swap_self] at hind,
          exact hind i hi j hj },
        { replace hi : i = σ a ∨ i = a ∨ i = σ⁻¹ a ∨ i ∈ {x | τ x ≠ x} := by sorry,
          replace hj : j = σ a ∨ j = a ∨ j = σ⁻¹ a ∨ j ∈ {x | τ x ≠ x} := by sorry,
          simp only [smul_eq_zero, sub_eq_zero] at this,
          -- split into cases of `hi` and `hj`
          sorry }},
      { replace h1 := lt_of_le_of_ne
        ((hfg.subset $ subset_insert _ _).sum_smul_comp_perm_le_sum_smul hτs) h1,
      simp_rw sum_insert has at heq,
      replace h1 := add_lt_add_left h1 (f a • g a),
      rw ← heq at h1,
      exfalso,
      ...
      { rw [ne.def, ←inv_eq_iff_eq] at hσa,
        refine mem_of_mem_insert_of_ne (hσ $ λ h, hσa _) hσa,
        rwa [apply_inv_self, eq_comm] at h },
      simp only [← s.sum_erase_add _ h1s, add_comm _ (f (σ⁻¹ a) • g (σ (σ⁻¹ a))),
        add_comm _ (f (σ⁻¹ a) • g (τ (σ⁻¹ a)))],
      rw [← add_assoc, ← add_assoc],
      refine add_le_add _ (sum_congr rfl $ λ x hx, _).le,
      { simp only [hτ, swap_apply_left, function.comp_app, equiv.coe_trans, apply_inv_self],
        suffices : 0 ≤ (f a - f (σ⁻¹ a)) • (g a - g (σ a)),
