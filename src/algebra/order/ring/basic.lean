...
and some structures to define ordered rings by specifying their non-negative elements:
* `positive_cone`: to define `ordered_ring`s, via `ordered_ring.mk_of_positive_cone`
* `total_positive_cone`: to define `linear_ordered_ring`s, via
