...
for typeclass inference: e.g. the typeclass system will only look through the `has_add_le_add_right`
instances, without extending the search to instances of `covariant` applied to different functions
or relations.
