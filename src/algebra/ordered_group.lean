...
@[simp] lemma sub_le_self (a : α) {b : α} : a - b ≤ a ↔ 0 ≤ b :=
by simp [sub_eq_add_neg]

@[simp] lemma sub_lt_self (a : α) {b : α} : a - b < a ↔ 0 < b :=
by simp [sub_eq_add_neg]
...
@[simp]
lemma sub_le_sub_right (c : α) : a - c ≤ b - c ↔ a ≤ b :=
...
⟨ begin
    obtain ⟨y, hy⟩ : ∃ (a:α), 0 < a := exists_gt_zero,
    exact λ a, ⟨a - y, (sub_lt_self a).2 hy⟩
  end ⟩
