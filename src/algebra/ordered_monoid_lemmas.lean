...
import order.basic
import algebra.order_operation_defs
/-!
...
lemma mul_le_mul_left' (h : a ≤ b) (c) :
  c * a ≤ c * b :=
has_mul_le_mul_left.mul_le_mul_left c h
...
variables [mul_one_class α] [has_mul_le_mul_left α]
...
-- here we start using properties of one.
section mul_one_class
variables [mul_one_class α] [has_mul_le_mul_right α]
...
variables [has_mul α]
  [has_mul_le_mul_left α] [has_mul_le_mul_right α]
...
lt_of_lt_of_le ((has_mul_le_mul_right.mul_le_mul_right c h₁.le).lt_of_ne
  (λ h, h₁.ne ((mul_left_inj c).mp h))) (mul_le_mul_left' h₂ b)
