...
multiplication with a positive number and addition are monotone. -/
@[prtect_proj] class linear_ordered_ring (α : Type u) extends ordered_ring α, linear_order α :=
(zero_lt_one : zero < one)
...
@[protect_proj] class nonneg_ring (α : Type*)
...
/-- Extend `nonneg_add_comm_group` to support linearly ordered rings
  specified by their nonnegative elements -/
@[protect_proj] class linear_nonneg_ring (α : Type*) extends domain α, nonneg_add_comm_group α :=
(mul_nonneg : ∀ {a b}, nonneg a → nonneg b → nonneg (a * b))
...
@[protect_proj] class canonically_ordered_comm_semiring (α : Type*) extends
