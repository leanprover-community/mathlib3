...
@[simp]
lemma star_empty [has_star α] : star (∅ : set α) = ∅ := rfl

@[simp]
lemma star_univ [has_star α] : star (univ : set α) = univ := rfl

@[simp]
lemma nonempty_star [has_involutive_star α] {s : set α} : (star s).nonempty ↔ s.nonempty :=
...
@[simp]
lemma mem_star [has_star α] : a ∈ star s ↔ star a ∈ s := iff.rfl

lemma star_mem_star [has_involutive_star α] : star a ∈ star s ↔ a ∈ s :=
by simp only [mem_star, star_star]

@[simp]
lemma star_preimage [has_star α] : has_star.star ⁻¹' s = star s := rfl

@[simp]
lemma image_star [has_involutive_star α] : has_star.star '' s = star s :=
by { simp only [← star_preimage], rw [image_eq_preimage_of_inverse]; intro; simp only [star_star] }

@[simp]
lemma inter_star [has_star α] : star (s ∩ t) = star s ∩ star t := preimage_inter

@[simp]
lemma union_star [has_star α] : star (s ∪ t) = star s ∪ star t := preimage_union

@[simp]
lemma Inter_star {ι : Sort*} [has_star α] (s : ι → set α) : star (⋂ i, s i) = ⋂ i, star (s i) :=
preimage_Inter

@[simp]
lemma Union_star {ι : Sort*} [has_star α] (s : ι → set α) : star (⋃ i, s i) = ⋃ i, star (s i) :=
preimage_Union

@[simp]
lemma compl_star [has_star α] : star (sᶜ) = (star s)ᶜ := preimage_compl
...
lemma finite.star [has_involutive_star α] {s : set α} (hs : finite s) : finite (star s) :=
hs.preimage $ star_injective.inj_on _

lemma star_singleton {β : Type*} [has_involutive_star β] (x : β) :
  star ({x} : set β) = {star x} :=
...
protected lemma star_inv [group α] [star_monoid α] (s : set α) : star s⁻¹ = (star s)⁻¹ :=
by { ext, simp only [mem_star, mem_inv, star_inv] }

protected lemma star_inv' [division_ring α] [star_ring α] (s : set α) : star s⁻¹ = (star s)⁻¹ :=
by { ext, simp only [mem_star, mem_inv, star_inv'] }
