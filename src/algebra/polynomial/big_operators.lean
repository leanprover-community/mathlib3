...
by simpa [-sum_neg_distrib] using multiset_prod_X_sub_C_next_coeff (s.1.map f)
...
by simpa [-sum_neg_distrib] using multiset_prod_X_sub_C_coeff_card_pred (s.1.map f)
  (by simpa using hs)
