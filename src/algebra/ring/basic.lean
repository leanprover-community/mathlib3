...
{ neg := has_neg.neg,
  neg_neg := neg_neg,
  neg_mul := λ a b,
    (neg_eq_of_add_eq_zero_left $ by rw [← right_distrib, add_right_neg, zero_mul]).symm,
  mul_neg := λ a b,
    (neg_eq_of_add_eq_zero_left $ by rw [← left_distrib, add_right_neg, mul_zero]).symm }
...
begin
  have : c = -(x * x - b * x) := (neg_eq_of_add_eq_zero_left h).symm,
