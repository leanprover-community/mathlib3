...
@[irreducible] private def smul (n : S) : ring_quot s → ring_quot s
| ⟨a⟩ := ⟨quot.map (λ a, n • a) (rel.smul n) a⟩

instance : has_scalar S (ring_quot s) := ⟨smul s⟩

lemma smul_quot {n : S} {a : A} : (n • ⟨quot.mk _ a⟩ : ring_quot s) = ⟨quot.mk _ (n • a)⟩ :=
by { show smul s _ _ = _, rw smul, refl }
