...
begin
  ...
  { simp only [hm_zero, square_zero, factorization_zero, finsupp.coe_zero, pi.zero_apply, forall_const, true_iff],
    use 0,
    exact mul_zero 2, },
  { split,
    { rintros ⟨c, hc⟩,
      by_cases hc' : c = 0,
      { rw [hc', mul_zero] at hc, rw ←hc, simp only [factorization_zero, finsupp.coe_zero, pi.zero_apply, forall_const], use 0, exact mul_zero 2, },
      { intros p,
...
begin
  ...
  { rw [square_iff_factorization_even, square_iff_factorization_even, square_iff_factorization_even],
    ...
    { intros p,
      ...
      { rw [←nat.factors_count_eq, list.count_eq_zero_of_not_mem hp], use zero, exact mul_zero 2, }, },
    { intros p,
