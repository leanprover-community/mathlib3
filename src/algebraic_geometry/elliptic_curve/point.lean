...
begin
  ...
  by_cases hx : x₁ = x₂,
  { rcases ⟨hx, Y_eq_of_Y_ne h₁ h₂ hx (hxy hx)⟩ with ⟨rfl, rfl⟩,
    let W_Y := (y₁ - W.neg_Y x₁ y₁) ^ 2,
    replace hxy := pow_ne_zero 2 (sub_ne_zero_of_ne $ hxy rfl),
    refine
      ⟨1 + C (C $ W_Y⁻¹ * 4) * W.polynomial,
        ⟨C $ C W_Y⁻¹ * (C 4 * X ^ 2 + C (4 * x₁ + W.b₂) * X + C (4 * x₁ ^ 2 + W.b₂ * x₁ + 2 * W.b₄)),
        0, C (C W_Y⁻¹) * (X - W.neg_polynomial), _⟩,
  ...
  { let W_X := x₁ - x₂,
    ...
    refine ⟨_, ⟨⟨C $ C W_X⁻¹, C $ C $ W_X⁻¹ * -1, 0, _⟩, map_one _⟩⟩,
    rw [← mul_right_inj' $ C_ne_zero.mpr $ C_ne_zero.mpr hx],
