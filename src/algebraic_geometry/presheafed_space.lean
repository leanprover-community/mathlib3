...
/-!
We now set up the category of spaces equipped with a presheaf in `C`,
with only open embeddings allowed as morphisms.
-/

variables (C)
/--
A type synonym for `PresheafedSpace C`,
for which we provide a category instance only allowing open embeddings.
-/
...
/-- An object in `PresheafedSpace.open_embeddings C` is just a `PresheafedSpace C`. -/
def forget_obj (X : open_embeddings C) : PresheafedSpace C := X

structure hom (X Y : open_embeddings C) extends to_hom : PresheafedSpace.hom X Y :=
(open_embedding : open_embedding to_hom.base)

/-- Identity morphism in `PresheafedSpace.open_embeddings C`. -/
def id (X : open_embeddings C) : hom X X :=
{ open_embedding := open_embedding_id,
    ..PresheafedSpace.id X }

/-- Composition of morphisms in `PresheafedSpace.open_embeddings C`. -/
def comp {X Y Z : open_embeddings C} (f : hom X Y) (g : hom Y Z) : hom X Z :=
{ open_embedding := open_embedding.comp g.open_embedding f.open_embedding,
  ..PresheafedSpace.comp f.to_hom g.to_hom }

/--
Every morphism in `PresheafedSpace.open_embeddings C` gives a morphism in `PresheafedSpace C`,
simply by forgetting that it is an open embedding.
-/
def forget_map {X Y : open_embeddings C} (f : hom X Y) : forget_obj X ⟶ forget_obj Y := f.to_hom

@[simp] lemma forget_map_id (X : open_embeddings C) : forget_map (id X) = 𝟙 _ := rfl
@[simp] lemma forget_map_comp {X Y Z : open_embeddings C} (f : hom X Y) (g : hom Y Z) :
  forget_map (comp f g) = forget_map f ≫ forget_map g := rfl

@[ext]
lemma hom_ext {X Y : open_embeddings C} {f g : hom X Y} (w : forget_map f = forget_map g) : f = g :=
...
/--
The forgetful functor from `PresheafedSpace.open_embeddings C` to `PresheafedSpace C`.
-/
def forget : open_embeddings C ⥤ PresheafedSpace C :=
{ obj := forget_obj,
  map := λ X Y f, forget_map f, }
