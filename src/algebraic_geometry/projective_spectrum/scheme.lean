...
open direct_sum set_like set_like.graded_monoid graded_algebra localization finset (hiding mk_zero)
...
begin
  ...
  { apply graded_algebra.proj_hom_mul, exact (hl hm f_deg 1 y).num_mem_deg, },
...
begin
  ...
  { apply graded_algebra.proj_hom_mul, exact (hl hm f_deg 0 y).num_mem_deg, },
  erw eq3 at eq2,
...
begin
  change (Proj_iso_Spec_Top_component.from_Spec.to_fun f_deg hm (Proj_iso_Spec_Top_component.to_Spec.to_fun 𝒜 f _)).1 = z.1,
...
begin
  ...
  { exact (hh.1 ⟨((Proj_iso_Spec_Top_component hm f_deg).inv hom_z).1, to_Spec.inv_mem ⟨hom_z, hom_z_mem_V⟩⟩).denom_mem, },
