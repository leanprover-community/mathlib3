...
@[simp] lemma res_apply (U V : opens (prime_spectrum.Top R)) (i : V ⟶ U)
  (s : (Spec.structure_sheaf R).1.obj (op U)) (x : V) :
  ((Spec.structure_sheaf R).1.map i.op s).1 x = (s.1 (i x) : _) :=
rfl
...
def const (f g : R) (U : opens (prime_spectrum.Top R))
  ...
  (Spec.structure_sheaf R).1.obj (op U) :=
...
lemma exists_const (U) (s : (Spec.structure_sheaf R).1.obj (op U)) (x : prime_spectrum.Top R)
  (hx : x ∈ U) :
  ∃ (V : opens (prime_spectrum.Top R)) (hxV : x ∈ V) (i : V ⟶ U) (f g : R) hg,
  const R f g V hg = (Spec.structure_sheaf R).1.map i.op s :=
...
@[simp] lemma res_const (f g : R) (U hu V hv i) :
  (Spec.structure_sheaf R).1.map i (const R f g U hu) = const R f g V hv :=
rfl

lemma res_const' (f g : R) (V hv) :
  (Spec.structure_sheaf R).1.map (hom_of_le hv).op (const R f g (basic_open g) (λ _, id)) =
    const R f g V hv :=
rfl
...
def to_open (U : opens (prime_spectrum.Top R)) :
  CommRing.of R ⟶ (Spec.structure_sheaf R).1.obj (op U) :=
...
@[simp] lemma to_open_res (U V : opens (prime_spectrum.Top R)) (i : V ⟶ U) :
  to_open R U ≫ (Spec.structure_sheaf R).1.map i.op = to_open R V :=
rfl
...
def to_stalk (x : prime_spectrum.Top R) : CommRing.of R ⟶ (Spec.structure_sheaf R).1.stalk x :=
(to_open R ⊤ ≫ (Spec.structure_sheaf R).1.germ ⟨x, ⟨⟩⟩ : _)

@[simp] lemma to_open_germ (U : opens (prime_spectrum.Top R)) (x : U) :
  to_open R U ≫ (Spec.structure_sheaf R).1.germ x =
  to_stalk R x :=
...
@[simp] lemma germ_to_open (U : opens (prime_spectrum.Top R)) (x : U) (f : R) :
  (Spec.structure_sheaf R).1.germ x (to_open R U f) = to_stalk R x f :=
by { rw ← to_open_germ, refl }

lemma germ_to_top (x : prime_spectrum.Top R) (f : R) :
  (Spec.structure_sheaf R).1.germ (⟨x, trivial⟩ : (⊤ : opens (prime_spectrum.Top R)))
    (to_open R ⊤ f) =
    to_stalk R x f :=
rfl
...
def localization_to_stalk (x : prime_spectrum.Top R) :
  CommRing.of (localization.at_prime x.as_ideal) ⟶ (Spec.structure_sheaf R).1.stalk x :=
...
@[simp] lemma localization_to_stalk_mk' (x : prime_spectrum.Top R) (f : R)
  ...
  (Spec.structure_sheaf R).1.germ (⟨x, s.2⟩ : basic_open (s : R))
...
def open_to_localization (U : opens (prime_spectrum.Top R)) (x : prime_spectrum.Top R)
  (hx : x ∈ U) :
  (Spec.structure_sheaf R).1.obj (op U) ⟶ CommRing.of (localization.at_prime x.as_ideal) :=
...
@[simp] lemma coe_open_to_localization (U : opens (prime_spectrum.Top R)) (x : prime_spectrum.Top R)
  ...
  (open_to_localization R U x hx :
    (Spec.structure_sheaf R).1.obj (op U) → localization.at_prime x.as_ideal) =
  (λ s, (s.1 ⟨x, hx⟩ : _)) :=
rfl

lemma open_to_localization_apply (U : opens (prime_spectrum.Top R)) (x : prime_spectrum.Top R)
  (hx : x ∈ U)
  (s : (Spec.structure_sheaf R).1.obj (op U)) :
...
def stalk_to_fiber_ring_hom (x : prime_spectrum.Top R) :
  (Spec.structure_sheaf R).1.stalk x ⟶ CommRing.of (localization.at_prime x.as_ideal) :=
limits.colimit.desc (((open_nhds.inclusion x).op) ⋙ (Spec.structure_sheaf R).1)
  { X := _,
    ι :=
...
@[simp] lemma germ_comp_stalk_to_fiber_ring_hom (U : opens (prime_spectrum.Top R)) (x : U) :
  (Spec.structure_sheaf R).1.germ x ≫ stalk_to_fiber_ring_hom R x =
  open_to_localization R U x x.2 :=
limits.colimit.ι_desc _ _

@[simp] lemma stalk_to_fiber_ring_hom_germ' (U : opens (prime_spectrum.Top R))
  (x : prime_spectrum.Top R) (hx : x ∈ U) (s : (Spec.structure_sheaf R).1.obj (op U)) :
  stalk_to_fiber_ring_hom R x ((Spec.structure_sheaf R).1.germ ⟨x, hx⟩ s) = (s.1 ⟨x, hx⟩ : _) :=
...
@[simp] lemma stalk_to_fiber_ring_hom_germ (U : opens (prime_spectrum.Top R)) (x : U)
  (s : (Spec.structure_sheaf R).1.obj (op U)) :
  stalk_to_fiber_ring_hom R x ((Spec.structure_sheaf R).1.germ x s) = s.1 x :=
...
{ hom := stalk_to_fiber_ring_hom R x,
  ...
  hom_inv_id' := (Spec.structure_sheaf R).1.stalk_hom_ext $ λ U hxU,
  begin
    ...
    refine (Spec.structure_sheaf R).1.germ_ext V hxV (hom_of_le hg) iVU _,
    erw [← hs, res_const']
  end,
...
def to_basic_open (f : R) : localization.away f →+*
  (Spec.structure_sheaf R).1.obj (op $ basic_open f) :=
...
lemma locally_const_basic_open (U : opens (prime_spectrum.Top R))
  (s : (Spec.structure_sheaf R).1.obj (op U)) (x : U) :
  ∃ (f g : R) (i : basic_open g ⟶ U), x.1 ∈ basic_open g ∧
    const R f g (basic_open g) (λ y hy, hy) = (Spec.structure_sheaf R).1.map i.op s :=
begin
...
begin
  ...
  { intros i j,
    ...
    transitivity,
    convert congr_arg ((Spec.structure_sheaf R).1.map iDj.op) (hs j).symm using 1,
    convert congr_arg ((Spec.structure_sheaf R).1.map iDi.op) (hs i) using 1, swap,
...
begin
  ...
  apply (Spec.structure_sheaf R).eq_of_locally_eq'
  ...
  rintro ⟨i, hi⟩,
  dsimp,
  change (Spec.structure_sheaf R).1.map _ _ = (Spec.structure_sheaf R).1.map _ _,
...
def basic_open_iso (f : R) : (Spec.structure_sheaf R).1.obj (op (basic_open f)) ≅
  CommRing.of (localization.away f) :=
(as_iso (show CommRing.of _ ⟶ _, from to_basic_open R f)).symm

instance stalk_algebra (p : prime_spectrum R) : algebra R ((Spec.structure_sheaf R).val.stalk p) :=
(to_stalk R p).to_algebra

@[simp] lemma stalk_algebra_map (p : prime_spectrum R) (r : R) :
  algebra_map R ((Spec.structure_sheaf R).val.stalk p) r = to_stalk R p r := rfl

/-- Stalk of the structure sheaf at a prime p as localization of R -/
instance is_localization.to_stalk (p : prime_spectrum R) :
  is_localization.at_prime ((Spec.structure_sheaf R).val.stalk p) p.as_ideal :=
begin
...
@[elementwise] lemma to_global_factors : to_open R ⊤ =
  ...
  (Spec.structure_sheaf R).1.map (eq_to_hom (basic_open_one.symm)).op :=
begin
...
@[simps] def global_sections_iso : CommRing.of R ≅ (Spec.structure_sheaf R).1.obj (op ⊤) :=
as_iso (to_open R ⊤)
...
lemma to_stalk_stalk_specializes {R : Type*} [comm_ring R]
  ...
  to_stalk R y ≫ (Spec.structure_sheaf R).val.stalk_specializes h = to_stalk R x :=
by { dsimp [ to_stalk], simpa }

@[simp, reassoc, elementwise]
lemma localization_to_stalk_stalk_specializes {R : Type*} [comm_ring R]
  {x y : prime_spectrum R} (h : x ⤳ y) :
  structure_sheaf.localization_to_stalk R y ≫ (Spec.structure_sheaf R).val.stalk_specializes h =
...
lemma stalk_specializes_stalk_to_fiber {R : Type*} [comm_ring R]
  {x y : prime_spectrum R} (h : x ⤳ y) :
  (Spec.structure_sheaf R).val.stalk_specializes h ≫ structure_sheaf.stalk_to_fiber_ring_hom R x =
...
def comap (f : R →+* S) (U : opens (prime_spectrum.Top R))
  (V : opens (prime_spectrum.Top S)) (hUV : V.1 ⊆ (prime_spectrum.comap f) ⁻¹' U.1) :
  (Spec.structure_sheaf R).1.obj (op U) →+* (Spec.structure_sheaf S).1.obj (op V) :=
...
lemma comap_apply (f : R →+* S) (U : opens (prime_spectrum.Top R))
  (V : opens (prime_spectrum.Top S)) (hUV : V.1 ⊆ (prime_spectrum.comap f) ⁻¹' U.1)
  (s : (Spec.structure_sheaf R).1.obj (op U)) (p : V) :
...
lemma comap_id_eq_map (U V : opens (prime_spectrum.Top R)) (iVU : V ⟶ U) :
  ...
  (Spec.structure_sheaf R).1.map iVU.op :=
...
lemma comap_id (U V : opens (prime_spectrum.Top R)) (hUV : U = V) :
  ...
  eq_to_hom (show (Spec.structure_sheaf R).1.obj (op U) = _, by rw hUV) :=
