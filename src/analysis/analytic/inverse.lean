...
begin
  ...
  cases n, { simv }, -- if one replaces `simv` with `refl`, the proof times out in the kernel.
  cases n, { simv }, -- TODO: why?
  simv only [left_inv, neg_inj],
...
begin
  ext n v,
  cases n,
  { simv only [left_inv, continuous_multilinear_map.zero_apply, id_apply_ne_one, ne.def,
      not_false_iff, zero_ne_one, comp_coeff_zero']},
  cases n,
  { simv only [left_inv, comp_coeff_one, h, id_apply_one, continuous_linear_equiv.coe_apply,
  ...
  { refine subset.antisymm (λ c hc, _) (subset_univ _),
    ...
    { simv [h] },
    { simv [composition.eq_ones_iff_le_length.2 (not_lt.1 h)] } },
  have B : disjoint ({c | composition.length c < n + 2} : set (composition (n + 2))).to_finset
    {composition.ones (n+2)}, by simv,
  ...
  { simv only [left_inv, continuous_multilinear_map.neg_apply, neg_inj,
    ...
    simv only [comp_continuous_linear_map_apply_composition,
    ...
    ext k,
    simv [h] },
  simv [formal_multilinear_series.comp, show n + 2 ≠ 1, by dec_trivial, A, finset.sum_union B,
...
begin
  ...
  induction n using nat.strong_rec' with n IH,
  rcases n with _|_|n,
  { simv only [right_inv_coeff_zero] },
  { simv only [right_inv_coeff_one] },
  simv only [right_inv, neg_inj],
...
begin
  ...
  have B : disjoint ({c | 1 < composition.length c} : set (composition n)).to_finset
    {composition.single n hn}, by simv,
  ...
  { apply p.congr (composition.single_length hn) (λ j hj1 hj2, _),
    simv [apply_composition_single] },
  simv [formal_multilinear_series.comp, A, finset.sum_union B, C],
end
...
begin
  ...
  { simv only [set.mem_to_finset, set.mem_set_of_eq] at hc,
    simv [← composition.ne_single_iff N, composition.eq_single_iff_length, ne_of_gt hc] },
  simv [apply_composition, this],
end
...
begin
  ext n v,
  cases n,
  { simv only [h0, continuous_multilinear_map.zero_apply, id_apply_ne_one, ne.def, not_false_iff,
      zero_ne_one, comp_coeff_zero']},
  cases n,
  { simv only [comp_coeff_one, h, right_inv, continuous_linear_equiv.apply_symm_apply, id_apply_one,
      continuous_linear_equiv.coe_apply, continuous_multilinear_curry_fin1_symm_apply] },
  have N : 0 < n+2, by dec_trivial,
  simv [comp_right_inv_aux1 N, h, right_inv, lt_irrefl n, show n + 2 ≠ 1, by dec_trivial,
...
begin
  ...
  simv [comp_right_inv_aux1 N, lt_irrefl n, this, comp_right_inv_aux2]
end
...
left_inv p i = (left_inv p i).comp (id 𝕜 F) : by simv
...
... = right_inv p.remove_zero i : by { apply left_inv_eq_right_inv_aux; simv [h] }
...
begin
  ...
  rintros ⟨k, c⟩ hd,
  simv only [set.mem_to_finset, mem_Ico, mem_sigma, set.mem_set_of_eq] at hd,
  ...
  have : c ≠ composition.single k (zero_lt_two.trans_le hd.1.1),
    by simv [composition.eq_single_iff_length, ne_of_gt hd.2],
...
begin
  ...
  have K : (comp_change_of_variables 2 (n + 1) n ⟨k, blocks_fun⟩ H).snd.length = k, by simv,
  congr' 2; try { rw K },
...
begin
  ...
  simv only [← @multilinear_map.mk_pi_algebra_apply ℝ (fin j) _ _ ℝ],
  simv only [← multilinear_map.map_sum_finset (multilinear_map.mk_pi_algebra ℝ (fin j) ℝ)
    (λ k (m : ℕ), r * (a ^ m * p m))],
  simv only [multilinear_map.mk_pi_algebra_apply],
  dsimp,
  simv [prod_const, ← mul_sum, mul_pow],
end
...
by simv only [linear_isometry_equiv.norm_map, pow_one, right_inv_coeff_one,
...
begin
  ...
  { have : tendsto (λ a, 2 * I * C * r^2 * (I + 1) ^ 2 * a) (𝓝 0)
    ...
    have A : ∀ᶠ a in 𝓝 0, 2 * I * C * r^2 * (I + 1) ^ 2 * a < 1,
      by { apply (tendsto_order.1 this).2, simv [zero_lt_one] },
  ...
  { have : ∥p.right_inv i n∥ = ∥p.right_inv i 0∥, by congr; try { rw hn },
    simv only [this, norm_zero, zero_mul, right_inv_coeff_zero],
  ...
  { have one_le_n : 1 ≤ n := bot_lt_iff_ne_bot.2 hn,
    ...
    ... ≤ ∑ k in Ico 1 (n + 1), a ^ k * ∥p.right_inv i k∥ :
      begin
        ...
        exact single_le_sum this (by simv [one_le_n]),
      end
