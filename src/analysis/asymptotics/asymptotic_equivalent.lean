...
definition only requires `β` to be a `normed_add_group`, most interesting properties require it to be a
`normed_field`.
...
variables {α β : Type*} [normed_add_group β]
...
lemma is_equivalent.smul {α E 𝕜 : Type*} [normed_field 𝕜] [normed_add_group E]
