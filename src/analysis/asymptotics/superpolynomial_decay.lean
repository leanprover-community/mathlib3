...
begin
  ...
  { refine (is_O_iff_div_is_bounded_under _).2 _,
    { exact hα.mono (λ x hx hx', absurd (fzpow_eq_zero hx') hx) },
...
begin
  ...
  rw [fzpow_sub_one hx, mul_comm, mul_assoc, inv_mul_cancel hx, one_mul, mul_one]
end
...
begin
  ...
  refine is_O_of_div_tendsto_nhds (eventually_of_forall
    (λ x hx, mul_eq_zero_of_left (fzpow_eq_zero hx) _)) 1 (tendsto_nhds.2 _),
  ...
  convert hs',
  rw [pi.div_apply, div_eq_one_iff_eq (fzpow_ne_zero c hx'), fzpow_sub_one hx' c,
...
begin
  ...
  { refine (hC C le_rfl).trans (is_O.of_bound 1 (_)),
    ...
    simp only [one_mul, normed_field.norm_fzpow, set.mem_set_of_eq],
    exact fzpow_le_of_le hx (le_of_not_le hc) }
end

lemma superpolynomial_decay_of_is_O_fzpow_le (hα : ∀ᶠ (x : α) in at_top, 1 ≤ ∥algebra_map α 𝕜 x∥)
  (C : ℤ) (h : ∀ c ≤ C, is_O f (λ n, (algebra_map α 𝕜 n) ^ c) at_top) :
  superpolynomial_decay f :=
superpolynomial_decay_of_eventually_is_O hα (eventually_at_bot.2 ⟨C, h⟩)

lemma superpolynomial_decay_of_is_O_fzpow_lt (hα : ∀ᶠ (x : α) in at_top, 1 ≤ ∥algebra_map α 𝕜 x∥)
  (C : ℤ) (h : ∀ c < C, is_O f (λ n, (algebra_map α 𝕜 n) ^ c) at_top) :
  superpolynomial_decay f :=
superpolynomial_decay_of_is_O_fzpow_le hα C.pred
...
begin
  ...
  by simp only [zpow_one, fzpow_neg],
