...
begin
  ...
  dsimp [integral_sum],
  simv only [mem_closed_ball, dist_eq_norm, ← indicator_const_smul_apply,
  ...
  have htU : t ⊆ U ∩ I,
  { simv only [t, prepartition.Union_def, Union_subset_iff, prepartition.mem_filter, and_imp],
  ...
  { have hμt : μ t ≠ ∞ :=
    ...
    simv only [t, prepartition.Union_def, prepartition.mem_filter, set.mem_Union, exists_prop],
...
begin
  ...
  set N : (ι → ℝ) → ℕ := λ x, ⌈∥f x∥⌉₊,
  have N0 : ∀ {x}, N x = 0 ↔ f x = 0, by { intro x, simv [N] },
  ...
  have : m < δ n / n,
  { simv only [measure.restrict_apply (hUo _).measurable_set] at hμU,
    refine hm ▸ (measure_mono _).trans_lt (hμU _),
    simv only [set.subset_def, tagged_prepartition.mem_Union, exists_prop,
...
begin
  ...
  `simple_func.approx_on g hg.measurable (range g ∪ {0}) 0 (by simv)`
  ...
  { intro x,
    have : tendsto (λ n, f n x) at_top (𝓝 $ g x),
      from simple_func.tendsto_approx_on hg.measurable _ (subset_closure (by simv)),
