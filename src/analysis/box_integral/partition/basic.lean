...
of pairwise disjoint boxes such that their union is exactly `I`. We use `boxes : finset (box n)` to
store the set of boxes.

Many lemmas about box integrals deal with pairwise disjoint collections of subboxes, so we define a
structure `box_integral.prepartition (I : box_integral.box n)` that stores a collection of boxes
such that
...
/-- A prepartition of `I : box_integral.box n` is a finite set of pairwise disjoint subboxes of
`I`. -/
structure prepartition (I : box n) :=
(boxes : finset (box n))
(le_of_mem' : ∀ J ∈ boxes, J ≤ I)
(pairwise_disjoint : set.pairwise ↑boxes (disjoint on (coe : box n → set ℝⁿ)))

namespace prepartition

variables {I J J₁ J₂ : box n} (π : prepartition I) {π₁ π₂ : prepartition I} {x : ℝⁿ}

instance : has_mem (box n) (prepartition I) := ⟨λ J π, J ∈ π.boxes⟩
...
lemma injective_boxes : function.injective (boxes : prepartition I → finset (box n)) :=
...
@[simps] def single (I J : box n) (h : J ≤ I) : prepartition I :=
⟨{J}, by simpa, by simp⟩
...
`2 ^ n` closed boxes of a prepartition. -/
lemma inj_on_set_of_mem_Icc_set_of_lower_eq (x : ℝⁿ) :
  inj_on (λ J : box n, {i | J.lower i = x i}) {J | J ∈ π ∧ x ∈ J.Icc} :=
begin
...
at most `2 ^ n`. -/
lemma card_filter_mem_Icc_le (x : ℝⁿ) :
  (π.boxes.filter (λ J : box n, x ∈ J.Icc)).card ≤ 2 ^ n :=
calc (π.boxes.filter (λ J : box n, x ∈ J.Icc)).card ≤ (fintype.card (set (fin n))) :
  finset.card_le_card_of_inj_on (λ J : box n, {i | J.lower i = x i}) (λ _ _, finset.mem_univ _) $
    by simpa only [finset.mem_filter] using π.inj_on_set_of_mem_Icc_set_of_lower_eq x
... = 2 ^ n : by rw [fintype.card_set, fintype.card_fin]

/-- Given a prepartition `π : box_integral.prepartition I`, `π.Union` is the part of `I` covered by
the boxes of `π`. -/
protected def Union : set ℝⁿ := ⋃ J ∈ π, ↑J
...
lemma le_iff_nonempty_imp_le_and_Union_subset : π₁ ≤ π₂ ↔
  (∀ (J ∈ π₁) (J' ∈ π₂), (J ∩ J' : set ℝⁿ).nonempty → J ≤ J') ∧ π₁.Union ⊆ π₂.Union :=
begin
  fsplit,
...
function. -/
@[simps] def bUnion (πi : Π J : box n, prepartition J) : prepartition I :=
...
variables {πi πi₁ πi₂ : Π J : box n, prepartition J}
...
@[simp] lemma Union_bUnion (πi : Π J : box n, prepartition J) :
...
@[simp] lemma sum_bUnion_boxes {M : Type*} [add_comm_monoid M] (π : prepartition I)
  (πi : Π J, prepartition J) (f : box n → M) :
...
def bUnion_index (πi : Π J, prepartition J) (J : box n) :
  box n :=
...
lemma bUnion_index_le (πi : Π J, prepartition J) (J : box n) : π.bUnion_index πi J ≤ I :=
begin
...
lemma bUnion_assoc (πi : Π J, prepartition J) (πi' : box n → Π J : box n, prepartition J) :
...
def of_with_bot (boxes : finset (with_bot (box n)))
  (le_of_mem : ∀ J ∈ boxes, (J : with_bot (box n)) ≤ I)
  (pairwise_disjoint : set.pairwise (boxes : set (with_bot (box n))) disjoint) :
  prepartition I :=
...
@[simp] lemma mem_of_with_bot {boxes : finset (with_bot (box n))} {h₁ h₂} :
  J ∈ (of_with_bot boxes h₁ h₂ : prepartition I) ↔ (J : with_bot (box n)) ∈ boxes :=
mem_erase_none

@[simp] lemma Union_of_with_bot (boxes : finset (with_bot (box n)))
  (le_of_mem : ∀ J ∈ boxes, (J : with_bot (box n)) ≤ I)
  (pairwise_disjoint : set.pairwise (boxes : set (with_bot (box n))) disjoint) :
  (of_with_bot boxes le_of_mem pairwise_disjoint).Union = ⋃ J ∈ boxes, ↑J :=
begin
  suffices : (⋃ (J : box n) (hJ : ↑J ∈ boxes), ↑J) = ⋃ J ∈ boxes, ↑J,
    by simpa [of_with_bot, prepartition.Union],
  simp only [← box.bUnion_coe_eq_coe, @Union_comm _ _ (box n), @Union_comm _ _ (@eq _ _ _),
    Union_Union_eq_right]
end

lemma of_with_bot_le {boxes : finset (with_bot (box n))}
  {le_of_mem : ∀ J ∈ boxes, (J : with_bot (box n)) ≤ I}
  {pairwise_disjoint : set.pairwise (boxes : set (with_bot (box n))) disjoint}
  (H : ∀ J ∈ boxes, J ≠ ⊥ → ∃ J' ∈ π, J ≤ ↑J') :
  of_with_bot boxes le_of_mem pairwise_disjoint ≤ π :=
have ∀ (J : box n), ↑J ∈ boxes → ∃ J' ∈ π, J ≤ J',
  from λ J hJ, by simpa only [with_bot.coe_le_coe] using H J hJ with_bot.coe_ne_bot,
by simpa [of_with_bot, le_def]

lemma le_of_with_bot {boxes : finset (with_bot (box n))}
  {le_of_mem : ∀ J ∈ boxes, (J : with_bot (box n)) ≤ I}
  {pairwise_disjoint : set.pairwise (boxes : set (with_bot (box n))) disjoint}
  (H : ∀ J ∈ π, ∃ J' ∈ boxes, ↑J ≤ J') :
...
begin
  ...
  lift J' to box n using ne_bot_of_le_ne_bot with_bot.coe_ne_bot hle,
  exact ⟨J', mem_of_with_bot.2 J'mem, with_bot.coe_le_coe.1 hle⟩
end

lemma of_with_bot_mono {boxes₁ : finset (with_bot (box n))}
  {le_of_mem₁ : ∀ J ∈ boxes₁, (J : with_bot (box n)) ≤ I}
  {pairwise_disjoint₁ : set.pairwise (boxes₁ : set (with_bot (box n))) disjoint}
  {boxes₂ : finset (with_bot (box n))}
  {le_of_mem₂ : ∀ J ∈ boxes₂, (J : with_bot (box n)) ≤ I}
  {pairwise_disjoint₂ : set.pairwise (boxes₂ : set (with_bot (box n))) disjoint}
...
def restrict (π : prepartition I) (J : box n) :
  prepartition J :=
...
@[simp] lemma mem_restrict : J₁ ∈ π.restrict J ↔ ∃ (J' ∈ π), (J₁ : with_bot (box n)) = J ⊓ J' :=
by simp [restrict, eq_comm]

lemma mem_restrict' : J₁ ∈ π.restrict J ↔ ∃ (J' ∈ π), (J₁ : set ℝⁿ) = J ∩ J' :=
...
@[simp] lemma mem_inf {π₁ π₂ : prepartition I} :
  J ∈ π₁ ⊓ π₂ ↔ ∃ (J₁ ∈ π₁) (J₂ ∈ π₂), (J : with_bot (box n)) = J₁ ⊓ J₂ :=
...
@[simps] def filter (π : prepartition I) (p : box n → Prop) : prepartition I :=
...
@[simp] lemma mem_filter {p : box n → Prop} : J ∈ π.filter p ↔ J ∈ π ∧ p J := finset.mem_filter

lemma filter_le (π : prepartition I) (p : box n → Prop) : π.filter p ≤ π :=
λ J hJ, let ⟨hπ, hp⟩ := π.mem_filter.1 hJ in ⟨J, hπ, le_rfl⟩

lemma filter_of_true {p : box n → Prop} (hp : ∀ J ∈ π, p J) : π.filter p = π :=
by { ext J, simpa using hp J }

@[simp] lemma filter_true : π.filter (λ _, true) = π := π.filter_of_true (λ _ _, trivial)

@[simp] lemma Union_filter_not (π : prepartition I) (p : box n → Prop) :
  (π.filter (λ J, ¬p J)).Union = π.Union \ (π.filter p).Union :=
begin
  simp only [prepartition.Union],
  convert (@set.bUnion_diff_bUnion_eq _ (box n) π.boxes (π.filter p).boxes coe _).symm,
  { ext J x, simp { contextual := tt } },
  { convert π.pairwise_disjoint, simp }
end

lemma sum_fiberwise {α M} [add_comm_monoid M] (π : prepartition I) (f : box n → α) (g : box n → M) :
...
{ boxes := π₁.boxes ∪ π₂.boxes,
  ...
  pairwise_disjoint :=
    suffices ∀ (J₁ ∈ π₁) (J₂ ∈ π₂), J₁ ≠ J₂ → disjoint (J₁ : set ℝⁿ) J₂,
...
@[simp] lemma sum_disj_union_boxes {M : Type*} [add_comm_monoid M]
  (h : disjoint π₁.Union π₂.Union) (f : box n → M) :
...
@[simp] lemma distortion_top (I : box n) : distortion (⊤ : prepartition I) = I.distortion :=
sup_singleton

@[simp] lemma distortion_bot (I : box n) : distortion (⊥ : prepartition I) = 0 := sup_empty
...
lemma is_partition_top (I : box n) : is_partition (⊤ : prepartition I) :=
...
lemma le_iff (h : π₂.is_partition) :
  π₁ ≤ π₂ ↔ ∀ (J ∈ π₁) (J' ∈ π₂), (J ∩ J' : set ℝⁿ).nonempty → J ≤ J' :=
