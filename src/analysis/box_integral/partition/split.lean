...
A hyperplane `{x : ℝⁿ | x i = a}` splits a rectangular box `I : box_integral.box n` into two
...
* `box_integral.prepartition.split_many I s`, where `s : finset (fin n × ℝ)` is a finite set of
  hyperplanes `{x : ℝⁿ | x i = a}` encoded as pairs `(i, a)`, is the partition of `I` made by
...
variables {I : box n} {i : fin n} {x : ℝ} {y : ℝⁿ}

/-- Given a box `I` and `x ∈ (I.lower i, I.upper i)`, the hyperplane `{y : ℝⁿ | y i = x}` splits
...
`with_bot (box_integral.box n)`. -/
def split_lower (I : box n) (i : fin n) (x : ℝ) : with_bot (box n) :=
mk' I.lower (update I.upper i (min x (I.upper i)))

@[simp] lemma coe_split_lower : (split_lower I i x : set ℝⁿ) = I ∩ {y | y i ≤ x} :=
begin
...
lemma split_lower_def {i x} (h : x ∈ Ioo (I.lower i) (I.upper i))
  (h' : ∀ j, I.lower j < update I.upper i x j :=
    (forall_update_iff I.upper (λ j y, I.lower j < y)).2 ⟨h.1, λ j hne, I.lower_lt_upper _⟩) :
  I.split_lower i x = (⟨I.lower, update I.upper i x, h'⟩ : box n) :=
by simpa only [split_lower, mk'_eq_coe, min_eq_left h.2.le] using and.intro rfl rfl

/-- Given a box `I` and `x ∈ (I.lower i, I.upper i)`, the hyperplane `{y : ℝⁿ | y i = x}` splits
`I` into two boxes. `box_integral.box.split_upper I i x` is the box `I ∩ {y | x < y i}`
(if it is nonempty). As usual, we represent a box that may be empty as
`with_bot (box_integral.box n)`. -/
def split_upper (I : box n) (i : fin n) (x : ℝ) : with_bot (box n) :=
mk' (update I.lower i (max x (I.lower i))) I.upper

@[simp] lemma coe_split_upper : (split_upper I i x : set ℝⁿ) = I ∩ {y | x < y i} :=
begin
...
lemma split_upper_def {i x} (h : x ∈ Ioo (I.lower i) (I.upper i))
  (h' : ∀ j, update I.lower i x j < I.upper j :=
    (forall_update_iff I.lower (λ j y, y < I.upper j)).2 ⟨h.2, λ j hne, I.lower_lt_upper _⟩) :
  I.split_upper i x = (⟨update I.lower i x, I.upper, h'⟩ : box n) :=
by simpa only [split_upper, mk'_eq_coe, max_eq_left h.1.le] using and.intro rfl rfl

lemma disjoint_split_lower_split_upper (I : box n) (i : fin n) (x : ℝ) :
...
lemma split_lower_ne_split_upper (I : box n) (i : fin n) (x : ℝ) :
...
variables {I J : box n} {i : fin n} {x : ℝ}

/-- The partition of `I : box n` into the boxes `I ∩ {y | y ≤ x i}` and `I ∩ {y | x i < y}`.
One of these boxes can be empty, then this partition is just the single-box partition `⊤`. -/
def split (I : box n) (i : fin n) (x : ℝ) : prepartition I :=
...
lemma mem_split_iff' : J ∈ split I i x ↔
  (J : set ℝⁿ) = I ∩ {y | y i ≤ x} ∨ (J : set ℝⁿ) = I ∩ {y | x < y i} :=
by simp [mem_split_iff, ← box.with_bot_coe_inj]

@[simp] lemma Union_split (I : box n) (i : fin n) (x : ℝ) : (split I i x).Union = I :=
by simp [split, ← inter_union_distrib_left, ← set_of_or, le_or_lt]

lemma is_partition_split (I : box n) (i : fin n) (x : ℝ) : is_partition (split I i x) :=
is_partition_iff_Union_eq.2 $ Union_split I i x

lemma sum_split_boxes {M : Type*} [add_comm_monoid M] (I : box n) (i : fin n) (x : ℝ)
  (f : box n → M) :
...
lemma coe_eq_of_mem_split_of_mem_le {y : ℝⁿ} (h₁ : J ∈ split I i x) (h₂ : y ∈ J) (h₃ : y i ≤ x) :
  (J : set ℝⁿ) = I ∩ {y | y i ≤ x} :=
(mem_split_iff'.1 h₁).resolve_right $ λ H,
  by { rw [← box.mem_coe, H] at h₂, exact h₃.not_lt h₂.2 }

lemma coe_eq_of_mem_split_of_lt_mem {y : ℝⁿ} (h₁ : J ∈ split I i x) (h₂ : y ∈ J) (h₃ : x < y i) :
  (J : set ℝⁿ) = I ∩ {y | x < y i} :=
...
begin
  ...
  have : ∀ s, (I ∩ s : set ℝⁿ) ⊆ J, from λ s, (inter_subset_left _ _).trans h,
  rintro J₁ ⟨J₂, (H₂|H₂), H₁⟩; [left, right]; simp [H₁, H₂, inter_left_comm ↑I, this],
end

lemma inf_split (π : prepartition I) (i : fin n) (x : ℝ) :
...
`(i x)`. -/
def split_many (I : box n) (s : finset (fin n × ℝ)) : prepartition I :=
s.inf (λ p, split I p.1 p.2)

@[simp] lemma split_many_empty (I : box n) : split_many I ∅ = ⊤ := finset.inf_empty

@[simp] lemma split_many_insert (I : box n) (s : finset (fin n × ℝ)) (p : fin n × ℝ) :
  split_many I (insert p s) = split_many I s ⊓ split I p.1 p.2 :=
by rw [split_many, finset.inf_insert, inf_comm, split_many]

lemma split_many_le_split (I : box n) {s : finset (fin n × ℝ)} {p : fin n × ℝ} (hp : p ∈ s) :
  split_many I s ≤ split I p.1 p.2 :=
finset.inf_le hp

lemma is_partition_split_many (I : box n) (s : finset (fin n × ℝ)) :
...
@[simp] lemma Union_split_many (I : box n) (s : finset (fin n × ℝ)) : (split_many I s).Union = I :=
(is_partition_split_many I s).Union_eq

lemma inf_split_many {I : box n} (π : prepartition I) (s : finset (fin n × ℝ)) :
...
/-- Let `s : finset (fin n × ℝ)` be a set of hyperplanes `{x : ℝⁿ | x i = r}` in `ℝⁿ` encoded as
...
`Js` is a subbox of `J`.  -/
lemma not_disjoint_imp_le_of_subset_of_mem_split_many {I J Js : box n} {s : finset (fin n × ℝ)}
  (H : ∀ i, {(i, J.lower i), (i, J.upper i)} ⊆ s) (HJs : Js ∈ split_many I s)
  (Hn : ¬disjoint (J : with_bot (box n)) Js) : Js ≤ J :=
begin
...
/-- Let `s` be a finite set of boxes in `ℝⁿ = fin n → ℝ`. Then there exists a finite set `t₀` of
...
lemma eventually_not_disjoint_imp_le_of_mem_split_many (s : finset (box n)) :
  ∀ᶠ t : finset (fin n × ℝ) in at_top, ∀ (I : box n) (J ∈ s) (J' ∈ split_many I t),
    ¬disjoint (J : with_bot (box n)) J' → J' ≤ J :=
begin
...
`split_many I s ≤ π`. -/
lemma is_partition.exists_split_many_le {I : box n} {π : prepartition I}
...
begin
  ...
  use (split_many I s).filter (λ J, ¬(J : set ℝⁿ) ⊆ π.Union),
  simp [← hs]
end
