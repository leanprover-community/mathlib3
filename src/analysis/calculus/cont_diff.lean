...
lemma norm_iterated_fderiv_within_comp_le_aux
  ...
  (hg : cont_diff_on 𝕜 n g t) (hf : cont_diff_on 𝕜 n f s) (hs : unique_diff_on 𝕜 s)
  (hst : maps_to f s t) (hx : x ∈ s)
  {C : ℝ} {D : ℝ} (hC : ∀ i, i ≤ n → ‖iterated_fderiv_within 𝕜 n g t (f x)‖ ≤ C)
  (hD : ∀ i, 1 ≤ i → i ≤ n → ‖iterated_fderiv_within 𝕜 n f s x‖ ≤ D^i) :
  ‖iterated_fderiv_within 𝕜 n (g ∘ f) s x‖ ≤ n! * C * D^n :=
begin
  unfreezingI { induction n using nat.case_strong_induction_on with n IH generalizing Fu Gu },
  ...
  have L : (1 : with_top ℕ) ≤ n.succ,
    by simpa only [enat.coe_one, nat.one_le_cast] using nat.succ_pos n,
  have J : iterated_fderiv_within 𝕜 n (λ (y : Eu), fderiv_within 𝕜 (g ∘ f) s y) s x
  ...
  rw [iterated_fderiv_within_succ_eq_comp_right hs hx, linear_isometry_equiv.norm_map, J],
  have : ‖iterated_fderiv_within 𝕜 n (λ (y : Eu), continuous_linear_map.compL 𝕜 Eu Fu Gu
        (fderiv_within 𝕜 g t (f y)) (fderiv_within 𝕜 f s y)) s x‖ ≤
    ∑ i in finset.range (n+1), (n.choose i : ℝ) *
      ‖iterated_fderiv_within 𝕜 i (λ y, fderiv_within 𝕜 g t (f y)) s x‖
      * ‖iterated_fderiv_within 𝕜 i (fderiv_within 𝕜 f s) s x‖,
  { have A : cont_diff_on 𝕜 n (λ y, fderiv_within 𝕜 g t (f y)) s, sorry,
    have B : cont_diff_on 𝕜 n (fderiv_within 𝕜 f s) s, sorry,
    have Z :=
     (continuous_linear_map.compL 𝕜 Eu Fu Gu).norm_iterated_fderiv_within_le_of_bilinear_of_le_one A B hs hx
       le_rfl,
