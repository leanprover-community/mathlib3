...
theorem has_fderiv_at_boundary_of_tendsto_fderiv {f : E → F} {s : set E} {x : E} {f' : E →L[ℝ] F}
  (f_diff : differentiable_on ℝ f s) (s_conv : convex 𝕜 s) (s_open : is_open s)
...
begin
  ...
  { rintros ⟨u, v⟩ ⟨u_in, v_in⟩,
    have conv : convex 𝕜 (B ∩ s) := (convex_ball _ _).inter s_conv,
...
begin
  ...
  have t_conv : convex 𝕜 t := convex_Ioo a b,
...
begin
  ...
  have t_diff : differentiable_on ℝ f t := f_diff.mono ts,
  have t_conv : convex 𝕜 t := convex_Ioo b a,
  have t_open : is_open t := is_open_Ioo,
