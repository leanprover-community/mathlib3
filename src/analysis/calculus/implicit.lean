...
structure implicit_function_data (𝕜 : Type*) [nondiscrete_normed_field 𝕜]
  (E : Type*) [normed_add_group E] [normed_space 𝕜 E] [complete_space E]
  (F : Type*) [normed_add_group F] [normed_space 𝕜 F] [complete_space F]
  (G : Type*) [normed_add_group G] [normed_space 𝕜 G] [complete_space G] :=
(left_fun : E → F)
(left_deriv : E →L[𝕜] F)
(right_fun : E → G)
(right_deriv : E →L[𝕜] G)
...
variables {𝕜 : Type*} [nondiscrete_normed_field 𝕜] [complete_space 𝕜]
  {E : Type*} [normed_add_group E] [normed_space 𝕜 E] [complete_space E]
  {F : Type*} [normed_add_group F] [normed_space 𝕜 F] [finite_dimensional 𝕜 F]
  (f : E → F) (f' : E →L[𝕜] F) {a : E}
