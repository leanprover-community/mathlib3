...
theorem lhopital_zero_right_on_Ioo
  ...
  (hg' : ∀ x ∈ Ioo a b, g' x ≠ 0)
  (hfa : tendsto f (𝓝ᵣ' a) (𝓝 0)) (hga : tendsto g (𝓝ᵣ' a) (𝓝 0))
  (hdiv : tendsto (λ x, (f' x) / (g' x)) (𝓝ᵣ' a) l) :
  tendsto (λ x, (f x) / (g x)) (𝓝ᵣ' a) l :=
begin
  have sub : ∀ x ∈ Ioo a b, Ioo a x ⊆ Ioo a b := λ x hx, Ioo_subset_Ioo (le_refl a) (le_of_lt hx.2),
  have hg : ∀ x ∈ (Ioo a b), g x ≠ 0,
  { intros x hx h,
    have : tendsto g (𝓝ₗ' x) (𝓝 0),
...
theorem lhopital_zero_right_on_Ioo
  ...
  (hfa : tendsto f (𝓝ᵣ' a) (𝓝 0)) (hga : tendsto g (𝓝ᵣ' a) (𝓝 0))
  (hdiv : tendsto (λ x, ((deriv f) x) / ((deriv g) x)) (𝓝ᵣ' a) l) :
  tendsto (λ x, (f x) / (g x)) (𝓝ᵣ' a) l :=
begin
...
theorem lhopital_zero_nhds_right
  (hff' : ∀ᶠ x in 𝓝ᵣ' a, has_deriv_at f (f' x) x)
  (hgg' : ∀ᶠ x in 𝓝ᵣ' a, has_deriv_at g (g' x) x)
  (hg' : ∀ᶠ x in 𝓝ᵣ' a, g' x ≠ 0)
  (hfa : tendsto f (𝓝ᵣ' a) (𝓝 0)) (hga : tendsto g (𝓝ᵣ' a) (𝓝 0))
  (hdiv : tendsto (λ x, (f' x) / (g' x)) (𝓝ᵣ' a) l) :
  tendsto (λ x, (f x) / (g x)) (𝓝ᵣ' a) l :=
begin
  ...
  let s := s₁ ∩ s₂ ∩ s₃,
  have hs : s ∈ 𝓝ᵣ' a := inter_mem (inter_mem hs₁ hs₂) hs₃,
...
theorem lhopital_zero_nhds_left
  (hff' : ∀ᶠ x in 𝓝ₗ' a, has_deriv_at f (f' x) x)
  (hgg' : ∀ᶠ x in 𝓝ₗ' a, has_deriv_at g (g' x) x)
  (hg' : ∀ᶠ x in 𝓝ₗ' a, g' x ≠ 0)
  (hfa : tendsto f (𝓝ₗ' a) (𝓝 0)) (hga : tendsto g (𝓝ₗ' a) (𝓝 0))
  (hdiv : tendsto (λ x, (f' x) / (g' x)) (𝓝ₗ' a) l) :
  tendsto (λ x, (f x) / (g x)) (𝓝ₗ' a) l :=
begin
  rw eventually_iff_exists_mem at *,
  rcases hff' with ⟨s₁, hs₁, hff'⟩,
  rcases hgg' with ⟨s₂, hs₂, hgg'⟩,
  rcases hg' with ⟨s₃, hs₃, hg'⟩,
  let s := s₁ ∩ s₂ ∩ s₃,
  have hs : s ∈ 𝓝ₗ' a := inter_mem (inter_mem hs₁ hs₂) hs₃,
...
theorem lhopital_zero_nhds_right
  (hdf : ∀ᶠ x in 𝓝ᵣ' a, differentiable_at ℝ f x)
  (hg' : ∀ᶠ x in 𝓝ᵣ' a, deriv g x ≠ 0)
  (hfa : tendsto f (𝓝ᵣ' a) (𝓝 0)) (hga : tendsto g (𝓝ᵣ' a) (𝓝 0))
  (hdiv : tendsto (λ x, ((deriv f) x) / ((deriv g) x)) (𝓝ᵣ' a) l) :
  tendsto (λ x, (f x) / (g x)) (𝓝ᵣ' a) l :=
begin
  have hdg : ∀ᶠ x in 𝓝ᵣ' a, differentiable_at ℝ g x,
    from hg'.mp (eventually_of_forall $
      λ _ hg', classical.by_contradiction (λ h, hg' (deriv_zero_of_not_differentiable_at h))),
  have hdf' : ∀ᶠ x in 𝓝ᵣ' a, has_deriv_at f (deriv f x) x,
    from hdf.mp (eventually_of_forall $ λ _, differentiable_at.has_deriv_at),
  have hdg' : ∀ᶠ x in 𝓝ᵣ' a, has_deriv_at g (deriv g x) x,
...
theorem lhopital_zero_nhds_left
  (hdf : ∀ᶠ x in 𝓝ₗ' a, differentiable_at ℝ f x)
  (hg' : ∀ᶠ x in 𝓝ₗ' a, deriv g x ≠ 0)
  (hfa : tendsto f (𝓝ₗ' a) (𝓝 0)) (hga : tendsto g (𝓝ₗ' a) (𝓝 0))
  (hdiv : tendsto (λ x, ((deriv f) x) / ((deriv g) x)) (𝓝ₗ' a) l) :
  tendsto (λ x, (f x) / (g x)) (𝓝ₗ' a) l :=
begin
  have hdg : ∀ᶠ x in 𝓝ₗ' a, differentiable_at ℝ g x,
    from hg'.mp (eventually_of_forall $
      λ _ hg', classical.by_contradiction (λ h, hg' (deriv_zero_of_not_differentiable_at h))),
  have hdf' : ∀ᶠ x in 𝓝ₗ' a, has_deriv_at f (deriv f x) x,
    from hdf.mp (eventually_of_forall $ λ _, differentiable_at.has_deriv_at),
  have hdg' : ∀ᶠ x in 𝓝ₗ' a, has_deriv_at g (deriv g x) x,
    from hdg.mp (eventually_of_forall $ λ _, differentiable_at.has_deriv_at),
