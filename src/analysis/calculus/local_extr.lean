...
on `(a, b)` and has the same limit `l` at `𝓝ᵣ' a` and `𝓝ₗ' b`, then `f' c = 0`
for some `c ∈ (a, b)`.  -/
lemma exists_has_deriv_at_eq_zero' (hab : a < b)
  (hfa : tendsto f (𝓝ᵣ' a) (𝓝 l)) (hfb : tendsto f (𝓝ₗ' b) (𝓝 l))
  (hff' : ∀ x ∈ Ioo a b, has_deriv_at f (f' x) x) :
  ∃ c ∈ Ioo a b, f' c = 0 :=
begin
...
`l` at `𝓝ᵣ' a` and `𝓝ₗ' b`, then `deriv f c = 0` for some `c ∈ (a, b)`. This version
