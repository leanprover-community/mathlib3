...
Then `∥f x∥ ≤ B x` everywhere on `[a, b]`. -/
lemma image_norm_le_of_liminf_right_slope_norm_lt_deriv_boundary {E : Type*} [normed_add_group E]
...
variables {𝕜 G : Type*} [is_R_or_C 𝕜] [normed_space 𝕜 E] [normed_add_group G] [normed_space 𝕜 G]
...
variables {𝕜 : Type*} [is_R_or_C 𝕜] {G : Type*} [normed_add_group G] [normed_space 𝕜 G]
  {H : Type*} [normed_add_group H] [normed_space 𝕜 H] {f : G → H} {f' : G → G →L[𝕜] H} {x : G}
