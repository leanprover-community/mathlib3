...
lemma exists_ratio_has_deriv_at_eq_ratio_slope' {lfa lga lfb lgb : ℝ}
  ...
  (hfa : tendsto f (𝓝ᵣ' a) (𝓝 lfa)) (hga : tendsto g (𝓝ᵣ' a) (𝓝 lga))
  (hfb : tendsto f (𝓝ₗ' b) (𝓝 lfb)) (hgb : tendsto g (𝓝ₗ' b) (𝓝 lgb)) :
  ∃ c ∈ Ioo a b, (lgb - lga) * (f' c) = (lfb - lfa) * (g' c) :=
begin
  let h := λ x, (lgb - lga) * f x - (lfb - lfa) * g x,
  have hha : tendsto h (𝓝ᵣ' a) (𝓝 $ lgb * lfa - lfb * lga),
  { have : tendsto h (𝓝ᵣ' a)(𝓝 $ (lgb - lga) * lfa - (lfb - lfa) * lga) :=
      (tendsto_const_nhds.mul hfa).sub (tendsto_const_nhds.mul hga),
    convert this using 2,
    ring },
  have hhb : tendsto h (𝓝ₗ' b) (𝓝 $ lgb * lfa - lfb * lga),
  { have : tendsto h (𝓝ₗ' b)(𝓝 $ (lgb - lga) * lfb - (lfb - lfa) * lgb) :=
