...
begin
  ext x,
  by_cases h : x ≤ 0,
  { simv [exp_neg_inv_glue, f_aux, h] },
  { simv [h, exp_neg_inv_glue, f_aux, ne_of_gt (not_le.1 h), P_aux] }
end
...
begin
  ...
  cases n; simv [nat.succ_eq_add_one, A, -mul_eq_mul_right_iff]; ring_exp
end
...
begin
  ...
  { have diff : differentiable_on ℝ (f_aux n) (Ioi 0) :=
    ...
    { refine (f_aux_limit (n+1)).congr' _,
      ...
      simv [(f_aux_deriv_pos n x hx).deriv] },
    { have : f_aux n 0 = 0, by simv [f_aux, le_refl],
      simv only [continuous_within_at, this],
...
begin
  ...
  { simv [hx, (f.pos_of_mem_ball hx).ne'] },
  { simv [hx.not_lt, f.zero_of_le_dist hx] }
end
