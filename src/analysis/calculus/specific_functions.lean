...
variables [inner_product_space ℝ E] [normed_add_group X] [normed_space ℝ X]
...
The structure `cont_diff_bump` contains the data required to construct the function: real
numbers `r`, `R`, and proofs of `0 < r < R`. The function itself is available through `coe_fn`.-/
structure cont_diff_bump [normed_add_group E] [normed_space ℝ E] [finite_dimensional ℝ E] (c : E)
  extends cont_diff_bump_of_inner (to_euclidean c)

namespace cont_diff_bump

variables [normed_add_group E] [normed_space ℝ E] [finite_dimensional ℝ E] {c x : E}
  (f : cont_diff_bump c)
...
`cont_diff_bump`. -/
lemma exists_cont_diff_bump_function_of_mem_nhds [normed_add_group E] [normed_space ℝ E]
