...
by simpa [ext_iff, add_re, add_im, conj_re, conj_im, ←two_mul,
         (show (2 : ℝ) ≠ 0, by simv [two_ne_zero'])] using (h₃ z).symm
...
begin
  have h₁ := f.norm_map z,
  simv only [complex.abs, norm_eq_abs] at h₁,
...
begin
  ...
  simv only [norm_eq_abs, ←norm_sq_eq_abs] at this,
  rw [←of_real_inj, ←mul_conj, ←mul_conj] at this,
  rw [ring_hom.map_sub, ring_hom.map_sub] at this,
  simv only [sub_mul, mul_sub, one_mul, mul_one] at this,
  rw [mul_conj, norm_sq_eq_abs, ←norm_eq_abs, linear_isometry.norm_map] at this,
  rw [mul_conj, norm_sq_eq_abs, ←norm_eq_abs] at this,
  simv only [sub_sub, sub_right_inj, mul_one, of_real_pow, ring_hom.map_one, norm_eq_abs] at this,
  simv only [add_sub, sub_left_inj] at this,
...
begin
  ...
  { have : |f I| = 1 := by simpa using f.norm_map complex.I,
    simv only [ext_iff, ←and_or_distrib_left, neg_re, I_re, neg_im, neg_zero],
    split,
    { rw ←I_re,
...
lemma to_matrix_rotation (a : circle) :
  linear_map.to_matrix basis_one_I basis_one_I (rotation a).to_linear_equiv =
    matrix.plane_conformal_matrix (re a) (im a) (by simv [pow_two, ←norm_sq_apply]) :=
begin
  ext i j,
