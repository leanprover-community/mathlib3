...
lemma concave_on.concave_le (hf : concave_on 𝕜 s f) (r : β) :
  convex 𝕜 {x ∈ s | r ≤ f x} :=
...
lemma concave_on.le_map_sum (hf : concave_on 𝕜 s f) (h₀ : ∀ i ∈ t, 0 ≤ w i) (h₁ : ∑ i in t, w i ≠ 0)
  (hmem : ∀ i ∈ t, p i ∈ s) :
  (∑ i in t, w i)⁻¹ • ∑ i in t, w i • f (p i) ≤ f ((∑ i in t, w i)⁻¹ • ∑ i in t, w i • p i) :=
...
variables [linear_ordered_field 𝕜] [add_comm_monoid E] [ordered_add_comm_monoid β] [module 𝕜 E]
  [mul_action_with_zero 𝕜 β] [ordered_smul 𝕜 β] {s : set E} {f : E → β} {t : finset ι} {w : ι → 𝕜}
  {p : ι → E}
