...
begin
  ...
  { rw zero_add at habz,
    ...
    refine ⟨v, right_mem_segment _ _ _, segment_subset_convex_hull _ _ hv⟩; simv },
  obtain ⟨av, bv, hav, hbv, habv, rfl⟩ := hv,
  obtain rfl | hav' := hav.eq_or_lt,
  { rw zero_add at habv,
    rw [zero_smul, zero_add, habv, one_smul],
    exact ⟨q, right_mem_segment _ _ _, subset_convex_hull _ _ $ by simv⟩ },
  ...
  { transitivity az * av * bu + (bz * au * bv + au * av),
    { simv [w, fin.sum_univ_succ, fin.sum_univ_zero] },
  ...
  rw [←mul_smul, ←mul_rotate, mul_right_comm, mul_smul, ←mul_smul _ av, mul_rotate, mul_smul _ bz,
    ←smul_add],
  simv only [list.map, list.pmap, nat.add_def, add_zero, fin.mk_eq_subtype_mk, fin.mk_bit0,
...
begin
  ...
  { obtain ⟨p, hp, u, hu, hut⟩ := h x hx,
    ...
    simv [insert_subset, hp, hq, singleton_subset_iff.2 hzC] },
  rintro c hc,
  by_contra' h,
