...
{ to_fun := λ z, z ^ n,
  continuous_to_fun := continuous_subtype_coe.fzpow n $ λ z, or.inl (nonzero_of_mem_circle z) }
...
by simp [fzpow_add (nonzero_of_mem_circle z)]
...
begin
  ...
  { have : (n:ℂ) ≠ 0 := by exact_mod_cast hn,
    field_simp,
    ring },
  simp [mul_fzpow, ← complex.exp_int_mul, complex.exp_pi_mul_I, this]
end
