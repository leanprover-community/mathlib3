...
begin
  ...
  { have : (n:ℂ) ≠ 0 := by exact_mod_cast hn,
    field_simp,
    ring },
  simv [mul_zpow, ← complex.exp_int_mul, complex.exp_pi_mul_I, this]
end
...
begin
  ...
  split_ifs,
  { simv [h, is_probability_measure.measure_univ, ← fourier_neg, ← fourier_add, -fourier_apply] },
  simv only [← fourier_add, ← fourier_neg],
  have hij : -i + j ≠ 0,
  { rw add_comm,
...
begin
  ...
  { simv [fourier_series.repr_apply_apply f i, measure_theory.L2.inner_def] },
...
begin
  ...
  { exact_mod_cast lp.norm_rpow_eq_tsum _ (fourier_series.repr f),
    norm_num },
  have H₂ : ∥fourier_series.repr f∥ ^ 2 = ∥f∥ ^2 := by simv,
  have H₃ := congr_arg is_R_or_C.re (@L2.inner_def circle ℂ ℂ _ _ _ _ f f),
  rw ← integral_re at H₃,
  { simv only [← norm_sq_eq_inner] at H₃,
    rw [← H₁, H₂],
    exact H₃ },
