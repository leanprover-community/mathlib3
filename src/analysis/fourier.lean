...
open topological_space continuous_map measure_theory measure_theory.measure submodule set
...
`z` and `conj z`. -/
def fourier_subalgebra : subalgebra ℂ C(circle, ℂ) := subalgebra.closure ℂ (range fourier)
...
begin
  apply subalgebra.closure_eq_span,
...
begin
  ...
  { exact subalgebra.subset_closure ⟨1, rfl⟩ },
  { simp [hxy] }
end
...
begin
  ...
  apply subalgebra.closure_induction hf,
  { rintros _ ⟨n, rfl⟩,
    suffices : fourier (-n) ∈ fourier_subalgebra,
    { convert this,
      ext1,
      simp, },
    exact subalgebra.subset_closure ⟨-n, rfl⟩ },
  { convert fourier_subalgebra.zero_mem,
    exact alg_hom.map_zero _, },
  { convert fourier_subalgebra.one_mem,
    exact alg_hom.map_one _, },
  { intros f g hf hg,
