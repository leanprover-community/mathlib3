...
begin
  ...
  rw [circle_measure, measurable_equiv.map_apply, measure_theory.measure.smul_apply, ne.def,
    smul_eq_zero, not_or_distrib, ←ne.def, ←ne.def],
  split, { simp [pi_pos] },
...
lemma fourier_norm (n : ℤ) : ∥fourier n∥ = 1 :=
begin
  apply eq_of_le_of_not_lt,
  { rw norm_le _ zero_le_one, intro z, rw fourier, simp },
  { rw not_lt, convert norm_coe_le_norm (fourier n) 1, simp },
end
...
/-- If the Fourier series of a continuous `f` is summable (in the uniform norm), then its sum must
be `f` itself. -/
lemma fourier_has_sum_of_summable {f : C(circle, ℂ)}
  (h : summable  (λ i, (fourier_series.repr (to_Lp 2 circle_measure ℂ f) i) • fourier i)) :
  has_sum (λ i, (fourier_series.repr (to_Lp 2 circle_measure ℂ f) i) • fourier i) f :=
begin
  cases h with g hg,
  suffices : f = g, { convert hg },
  have f_eq_g_Lp : to_Lp 2 circle_measure ℂ f = to_Lp 2 circle_measure ℂ g,
  { -- this times out if we don't make to_g first?
    have := (to_Lp 2 circle_measure ℂ).has_sum hg,
    exact (has_sum_fourier_series (to_Lp 2 circle_measure ℂ f)).unique this },
  have := (coe_fn_to_Lp circle_measure f).symm,
  rw f_eq_g_Lp at this,
  replace := this.trans (coe_fn_to_Lp circle_measure g),
  rw continuous.ae_eq_iff_eq at this,
  { ext1 a, rw this },
  -- can't inline these as then it doesn't pick up the `is_open_pos_measure` instance?
  { exact f.continuous }, { exact g.continuous },
end

lemma fourier_has_sum_of_coeffs_summable {f : C(circle, ℂ)}
  (h : summable  (fourier_series.repr (to_Lp 2 circle_measure ℂ f))) :
  has_sum (λ i, (fourier_series.repr (to_Lp 2 circle_measure ℂ f) i) • fourier i) f :=
begin
  refine fourier_has_sum_of_summable (summable_of_summable_norm _),
  simp_rw [norm_smul, fourier_norm, mul_one],
  rw ←summable_norm_iff at h, exact h,
end

lemma fourier_pointwise_sumable_of_coeffs_summable {f : C(circle, ℂ)} (z : circle)
  (h : summable  (fourier_series.repr (to_Lp 2 circle_measure ℂ f))) :
  has_sum (λ i, (fourier_series.repr (to_Lp 2 circle_measure ℂ f) i) * (fourier i z)) (f z) :=
begin
  let W := (linear_isometry_bounded_of_compact circle ℂ ℂ).to_linear_isometry,
  exact ((bounded_continuous_function.eval_clm ℂ z).comp W.to_continuous_linear_map).has_sum
    (fourier_has_sum_of_coeffs_summable h),
end
