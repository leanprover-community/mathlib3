...
def add_circle.to_circle := (@scaled_exp_map_periodic T).lift

lemma add_circle.to_circle_coe (x : ℝ) :
  @add_circle.to_circle T x = exp_map_circle (2 * π / T * x) := function.periodic.lift_coe _ _

lemma add_circle.to_circle_zero : @add_circle.to_circle T 0 = 1 :=
begin
  rw [←quotient_add_group.coe_zero, add_circle.to_circle_coe, mul_zero],
  exact exp_map_circle_zero,
end

lemma add_circle.to_circle_add (x : add_circle T) (y : add_circle T) :
  add_circle.to_circle (x + y) = add_circle.to_circle x * add_circle.to_circle y :=
begin
  induction x using quotient_add_group.induction_on',
  induction y using quotient_add_group.induction_on',
  simp_rw [←quotient_add_group.coe_add, add_circle.to_circle_coe, mul_add, exp_map_circle_add],
end

def add_circle.to_circle_hom : add_circle T →+ (additive circle) :=
{ to_fun := additive.of_mul ∘ add_circle.to_circle,
  map_zero' := add_circle.to_circle_zero,
  map_add' := add_circle.to_circle_add }

lemma add_circle.to_circle_continuous : continuous (@add_circle.to_circle T) :=
continuous_coinduced_dom.mpr (exp_map_circle.continuous.comp $ continuous_const.mul continuous_id')

lemma add_circle.injective_to_circle (hT : T ≠ 0) : function.injective (@add_circle.to_circle T) :=
begin
  intros a b h,
  induction a using quotient_add_group.induction_on',
  induction b using quotient_add_group.induction_on',
  simp_rw [add_circle.to_circle_coe] at h,
...
@[simp] lemma add_circle.to_circle_zsmul (n : ℤ) (x : add_circle T) :
  add_circle.to_circle (n • x) = (add_circle.to_circle x) ^ n :=
add_circle.to_circle_hom.map_zsmul _ _

end add_circle_to_circle
...
by simp only [fourier_apply, add_circle.to_circle_zsmul, coe_zpow_unit_sphere]
...
begin
  ...
  { rw [zsmul_eq_mul, add_circle.to_circle_coe, exp_map_circle_apply],
