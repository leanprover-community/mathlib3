...
class inner_product_space (𝕜 : Type*) (E : Type*) [is_R_or_C 𝕜]
  extends normed_add_group E, normed_space 𝕜 E, has_inner 𝕜 E :=
(norm_sq_eq_inner : ∀ (x : E), ∥x∥^2 = re (inner x x))
(conj_sym  : ∀ x y, conj (inner y x) = inner x y)
(add_left  : ∀ x y z, inner (x + y) z = inner x z + inner y z)
(smul_left : ∀ x y r, inner (r • x) y = (conj r) * inner x y)

attribute [nolint dangerous_instance] inner_product_space.to_normed_add_group
...
def to_normed_add_group : normed_add_group F :=
normed_add_group.of_core F
...
local attribute [instance] to_normed_add_group
...
begin
  letI : normed_add_group F := @inner_product_space.of_core.to_normed_add_group 𝕜 F _ _ _ c,
...
begin
  classical,
  simp only [summable_iff_cauchy_seq_finset, normed_add_group.cauchy_seq_iff, real.norm_eq_abs],
  split,
  { intros hf ε hε,
