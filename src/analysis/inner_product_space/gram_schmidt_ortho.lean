...
begin
  ...
  { have h₀ : ∀ x ∈ gram_schmidt 𝕜 f '' Iic c.succ, x ∈ span 𝕜 (f '' Iic c.succ),
    { intros x hx,
      ...
      have hp : span 𝕜 (f '' Iic c) ≤ span 𝕜 (f '' Iic (c + 1)),
        { apply span_mono,
          apply image_subset,
          simp only [Iic, set_of_subset_set_of],
          intros a ha,
          linarith, },
      have hq : gram_schmidt 𝕜 f '' Iic c ⊆
        ...
        { apply sum_mem _ _,
          ...
          { have h₂ : gram_schmidt 𝕜 f c₁ ∈ gram_schmidt 𝕜 f '' Iic c,
            ...
            have h₃ : gram_schmidt 𝕜 f c₁ ∈
              span 𝕜 (gram_schmidt 𝕜 f '' Iic c) := hq h₂,
            rw hc at h₃,
            exact hp h₃, },
    ...
    rwa ← span_le at h₁, },
  { have h₀ : ∀ x ∈ f '' Iic c.succ, x ∈ span 𝕜 (gram_schmidt 𝕜 f '' Iic c.succ),
    { intros x hx,
      simp only [mem_image, mem_Iic] at hx,
      rcases hx with ⟨y, hy₁, hy₂⟩,
      rw nat.succ_eq_add_one at hy₁ ⊢,
