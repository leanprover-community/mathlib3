...
open submodule finset

/-- Gram-Schmidt process preserves span -/
lemma span_gram_schmidt (f : ℕ → E) (c : ℕ) :
  span 𝕜 (gram_schmidt 𝕜 f '' range c) = span 𝕜 (f '' range c) :=
begin
  induction c with c hc,
  { simp only [finset.range_zero, finset.coe_empty, set.image_empty], },
  apply le_antisymm,
  { have h₀ : ∀ x ∈ gram_schmidt 𝕜 f '' range c.succ, x ∈ span 𝕜 (f '' range c.succ),
    { intros x hx,
      simp only [set.mem_image, mem_coe, mem_range] at hx,
      rcases hx with ⟨y, hy₁, hy₂⟩,
      rw nat.succ_eq_add_one at hy₁ ⊢,
      by_cases y < c,
      { have h₁ : x ∈ gram_schmidt 𝕜 f '' range c := ⟨y, mem_range.mpr h, hy₂⟩,
        have h₂ : gram_schmidt 𝕜 f '' range c ⊆
          span 𝕜 (gram_schmidt 𝕜 f '' range c) := subset_span,
        rw hc at h₂,
        have h₃ : span 𝕜 (f '' range c) ≤ span 𝕜 (f '' range (c + 1)),
        { have h₄ : f '' range c ⊆ f '' range (c + 1),
          { have h₅ : range c ⊆ range (c + 1) :=
              by simp only [range_subset, le_add_iff_nonneg_right, zero_le'],
            exact set.image_subset f h₅ },
          exact span_mono h₄ },
        exact h₃ (h₂ h₁) },
      { have h₁ : y = c  := by linarith,
        rw [h₁, gram_schmidt_def] at hy₂,
        simp only [orthogonal_projection_singleton] at hy₂,
        rw ← hy₂,
        apply sub_mem _ _ _,
        { have hc : f c ∈ f '' range (c + 1),
          { simp only [set.mem_image, mem_coe, mem_range],
            refine ⟨c, by linarith, by refl⟩ },
          have hc₁ : f '' range (c + 1) ⊆ span 𝕜 (f '' range (c + 1)) := subset_span,
          exact hc₁ hc },
        { apply sum_mem _ _,
          intros c₁ hc₁,
          have hc₂ : c₁ < c := mem_range.mp hc₁,
          have hc₃ : gram_schmidt 𝕜 f c₁ ∈ span 𝕜 (f '' range (c + 1)),
          { have h₂ : gram_schmidt 𝕜 f c₁ ∈ gram_schmidt 𝕜 f '' range c,
            { simp only [set.mem_image, mem_coe, mem_range],
              refine ⟨c₁, hc₂, by refl⟩ },
            have h₃ : gram_schmidt 𝕜 f '' range c ⊆
              span 𝕜 (gram_schmidt 𝕜 f '' range c) := subset_span,
            have h₄ : gram_schmidt 𝕜 f c₁ ∈
              span 𝕜 (gram_schmidt 𝕜 f '' range c) := h₃ h₂,
            rw hc at h₄,
            have h₅ : span 𝕜 (f '' range c) ≤ span 𝕜 (f '' range (c + 1)),
            { have h₆ : f '' range c ⊆ f '' range (c + 1),
              { have h₇ : range c ⊆ range (c + 1) :=
              ...
              exact span_mono h₆ },
            exact h₅ h₄ },
          exact smul_mem (span 𝕜 (f '' range (c + 1)))
            ((⟪gram_schmidt 𝕜 f c₁, f c⟫
              / ∥gram_schmidt 𝕜 f c₁∥ ^ 2) : 𝕜) hc₃ } } },
    have h₁ : gram_schmidt 𝕜 f '' range c.succ ⊆ span 𝕜 (f '' range c.succ) := h₀,
    rwa ← span_le at h₁ },
  { have h₀ : ∀ x ∈ f '' range c.succ,
      x ∈ span 𝕜 (gram_schmidt 𝕜 f '' range c.succ),
    { intros x hx,
      simp only [set.mem_image, mem_coe, mem_range] at hx,
      rcases hx with ⟨y, hy₁, hy₂⟩,
      rw nat.succ_eq_add_one at hy₁ ⊢,
      by_cases y < c,
      { have hx : x ∈ f '' range c := ⟨y, mem_range.mpr h, hy₂⟩,
        have hx₁ : x ∈ span 𝕜 (f '' range c),
        { have h₁ : f '' range c ⊆ span 𝕜 (f '' range c) := subset_span,
          exact h₁ hx },
        rw ← hc at hx₁,
        have h₂ : span 𝕜 (gram_schmidt 𝕜 f '' range c)
          ≤ span 𝕜 (gram_schmidt 𝕜 f '' range (c + 1)),
        { have h₃ : gram_schmidt 𝕜 f '' range c ⊆ gram_schmidt 𝕜 f '' range (c + 1),
          { have h₄ : range c ⊆ range (c + 1) :=
              by simp only [range_subset, le_add_iff_nonneg_right, zero_le'],
            exact set.image_subset (gram_schmidt 𝕜 f) h₄ },
          exact span_mono h₃ },
        exact h₂ hx₁ },
      { have hy₃ : y = c := by linarith,
        rw hy₃ at hy₂,
        have hx : x = gram_schmidt 𝕜 f c + ∑ i in finset.range c,
        ...
        rw hx,
        apply add_mem _ _ _,
        { have h₁ : gram_schmidt 𝕜 f c ∈ gram_schmidt 𝕜 f '' range c.succ,
          { simp only [set.mem_image, mem_coe, mem_range],
            refine ⟨c,
              by simp only [nat.succ_eq_add_one, lt_add_iff_pos_right, nat.lt_one_iff], by refl⟩ },
          have h₂ : gram_schmidt 𝕜 f '' range c.succ
            ⊆ span 𝕜 (gram_schmidt 𝕜 f '' range c.succ) := subset_span,
          exact h₂ h₁ },
        { apply sum_mem _ _,
          intros c₁ hc,
          have hc₁ : gram_schmidt 𝕜 f c₁ ∈
            span 𝕜 (gram_schmidt 𝕜 f '' range c.succ),
          { have hc₂ : gram_schmidt 𝕜 f c₁ ∈ gram_schmidt 𝕜 f '' range c.succ,
            { simp only [set.mem_image, mem_coe, mem_range],
              refine ⟨c₁, _, by refl⟩,
              rw mem_range at hc,
              simp only [nat.succ_eq_add_one],
              linarith },
            have hc₃ : gram_schmidt 𝕜 f '' range c.succ ⊆
              span 𝕜 (gram_schmidt 𝕜 f '' range c.succ) := subset_span,
            exact hc₃ hc₂ },
          exact smul_mem (span 𝕜 (gram_schmidt 𝕜 f '' range (c + 1)))
            ((⟪gram_schmidt 𝕜 f c₁, x⟫ / ∥gram_schmidt 𝕜 f c₁∥ ^ 2) : 𝕜)
              hc₁ } } },
    have h₁ : f '' range c.succ ⊆ span 𝕜 (gram_schmidt 𝕜 f '' range c.succ) := h₀,
    rwa ← span_le at h₁ }
end
  ...
  { by_contra h₂,
    ...
    have h₄ : ∑ (x : ℕ) in finset.range (n + 1),
      ((⟪gram_schmidt 𝕜 f x, f (n + 1)⟫ / ∥gram_schmidt 𝕜 f x∥ ^ 2) : 𝕜)
        • gram_schmidt 𝕜 f x ∈ span 𝕜 (gram_schmidt 𝕜 f '' range (n + 1)),
    { apply sum_mem _ _,
      intros a ha,
      have ha₁ : gram_schmidt 𝕜 f a ∈ gram_schmidt 𝕜 f '' range (n + 1),
      { simp only [set.mem_image, mem_coe, mem_range],
        rw mem_range at ha,
        refine ⟨a, ha, by refl⟩ },
      have h₄ : gram_schmidt 𝕜 f '' range (n + 1)
        ⊆ span 𝕜 (gram_schmidt 𝕜 f '' range (n + 1)) := subset_span,
      have h₅ : gram_schmidt 𝕜 f a
        ∈ span 𝕜 (gram_schmidt 𝕜 f '' range (n + 1)) := h₄ ha₁,
      exact smul_mem (span 𝕜 (gram_schmidt 𝕜 f '' range (n + 1)))
        ((⟪gram_schmidt 𝕜 f a, f (n + 1)⟫ / ∥gram_schmidt 𝕜 f a∥ ^ 2) : 𝕜) h₅ },
    have h₅ := span_gram_schmidt 𝕜 f (n + 1),
    rw [h₅, ← h₃] at h₄,
    have h₅ : linear_independent 𝕜 (f ∘ (coe : fin (n + 2) → ℕ)) := by simp only [h₀],
    have h₆ : ((n + 1) : fin (n + 2)) ∉ (coe : fin (n + 2) → ℕ) ⁻¹' (range (n + 1)),
    { simp only [set.mem_preimage, mem_coe, mem_range, not_lt],
      norm_cast,
    ...
    have h₈ : (range (n + 1) : set ℕ) ⊆ {x : ℕ | x < n + 2},
    { intros a ha,
      simp only [set.mem_set_of_eq],
      simp only [mem_coe, mem_range] at ha,
      linarith },
    have h₉ : (range (n + 1) : set ℕ) ∩ {x : ℕ | x < n + 2} = range (n + 1)
      := by simp only [set.inter_eq_left_iff_subset, h₈],
    rw h₉,
