...
universe u
def is_coercive {V : Type u} [inner_product_space ℝ V] [complete_space V] (B : V →L[ℝ] V →L[ℝ] ℝ) : Prop :=
∃ C, (0 < C) ∧ ∀ u, C * ∥u∥ * ∥u∥ ≤ B u u
...
comp
  (inner_product_space.to_dual ℝ V).symm.to_continuous_linear_equiv.to_continuous_linear_map
  B


lemma lax_milgram_map.apply : ∀ v w : V, B v w = inner (lax_milgram_map B v) w :=
begin
  intros x y,
...
begin
  ext v, split,
  {
    ...
    simp_rw [←ne.def],
    intro v_ne_0,
    --by_contradiction Av_eq_0,
    simp_rw [ ←norm_pos_iff] at ⊢ v_ne_0,
    rcases coercive with ⟨C, C_ge_0, coercivity⟩,
    have squared : C * ∥v∥ * ∥v∥ ≤ ∥lax_milgram_map B v ∥ * ∥v∥,
    by {
      exact calc C * ∥v∥*∥v∥ ≤ B v v                        : coercivity v
      ...                 = inner (lax_milgram_map B v) v : lax_milgram_map.apply B v v
  ...
  },
  {
    ...
    intro h, rw h,
    exact linear_map.map_zero (lax_milgram_map B),
  }
end
...
begin
  ...
  haveI := is_closed.complete_space_coe (closed_range B),
  ext v, split,
  {simp,},
  {
    simp only [submodule.mem_top, forall_true_left], -- we have to show that every v is in the range
    ...
    by {
      ...
      have : C * ∥w∥*∥w∥ ≤ 0 :=
      calc C * ∥w∥*∥w∥  ≤ B w w                          : coercivity w
      ...             = inner (lax_milgram_map B w) w  : lax_milgram_map.apply B w w
