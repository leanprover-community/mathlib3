...
* The elementwise norm:
  ...
  * `matrix.seminormed_add_group`
  * `matrix.normed_add_group`
  * `matrix.normed_space`

* The Frobenius norm:

  * `matrix.frobenius_seminormed_add_group`
  * `matrix.frobenius_normed_add_group`
...
* The $L^\infty$ operator norm:
  ...
  * `matrix.linfty_op_seminormed_add_group`
  * `matrix.linfty_op_normed_add_group`
...
variables [seminormed_add_group α] [seminormed_add_group β]

/-- Seminormed group instance (using sup norm of sup norm) for matrices over a seminormed group. Not
declared as an instance because there are several natural choices for defining the norm of a
matrix. -/
protected def seminormed_add_group : seminormed_add_group (matrix m n α) :=
pi.seminormed_add_group

local attribute [instance] matrix.seminormed_add_group
...
declared as an instance because there are several natural choices for defining the norm of a
matrix. -/
protected def normed_add_group [normed_add_group α] : normed_add_group (matrix m n α) :=
pi.normed_add_group

section normed_space
local attribute [instance] matrix.seminormed_add_group

variables [normed_field R] [seminormed_add_group α] [normed_space R α]
...
protected def linfty_op_seminormed_add_group [seminormed_add_group α] :
  seminormed_add_group (matrix m n α) :=
(by apply_instance : seminormed_add_group (m → pi_Lp 1 (λ j : n, α)))

/-- Normed group instance (using sup norm of L1 norm) for matrices over a normed ring.  Not
declared as an instance because there are several natural choices for defining the norm of a
matrix. -/
local attribute [instance]
protected def linfty_op_normed_add_group [normed_add_group α] :
  normed_add_group (matrix m n α) :=
(by apply_instance : normed_add_group (m → pi_Lp 1 (λ j : n, α)))

/-- Normed space instance (using sup norm of L1 norm) for matrices over a normed space.  Not
declared as an instance because there are several natural choices for defining the norm of a
matrix. -/
local attribute [instance]
protected def linfty_op_normed_space [normed_field R] [seminormed_add_group α] [normed_space R α] :
...
matrix. -/
local attribute [instance]
def frobenius_seminormed_add_group [seminormed_add_group α] :
  seminormed_add_group (matrix m n α) :=
(by apply_instance : seminormed_add_group (pi_Lp 2 (λ i : m, pi_Lp 2 (λ j : n, α))))

/-- Normed group instance (using frobenius norm) for matrices over a normed group.  Not
declared as an instance because there are several natural choices for defining the norm of a
matrix. -/
local attribute [instance]
def frobenius_normed_add_group [normed_add_group α] :
  normed_add_group (matrix m n α) :=
(by apply_instance : normed_add_group (pi_Lp 2 (λ i : m, pi_Lp 2 (λ j : n, α))))

/-- Normed space instance (using frobenius norm) for matrices over a normed space.  Not
declared as an instance because there are several natural choices for defining the norm of a
matrix. -/
local attribute [instance]
def frobenius_normed_space [normed_field R] [seminormed_add_group α] [normed_space R α] :
  normed_space R (matrix m n α) :=
...
lemma frobenius_nnnorm_one [decidable_eq n] [seminormed_add_group α] [has_one α] :
