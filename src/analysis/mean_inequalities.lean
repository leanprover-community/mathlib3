...
(`fzpow_arith_mean_le_arith_mean_fzpow`), and real exponents (`rpow_arith_mean_le_arith_mean_rpow` and
...
theorem fzpow_arith_mean_le_arith_mean_fzpow (w z : ι → ℝ) (hw : ∀ i ∈ s, 0 ≤ w i)
  (hw' : ∑ i in s, w i = 1) (hz : ∀ i ∈ s, 0 < z i) (m : ℤ) :
  (∑ i in s, w i * z i) ^ m ≤ ∑ i in s, (w i * z i ^ m) :=
(convex_on_fzpow m).map_sum_le hw hw' hz
