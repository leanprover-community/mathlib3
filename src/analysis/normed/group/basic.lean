...
* `seminormed_add_group`: a seminormed group is an additive group with a norm and a pseudo metric space
  structures that agree with each other: `∀ x y, dist x y = ∥x - y∥`;
* `normed_add_group`: a normed group is an additive group with a norm and a metric space structures that
...
class seminormed_add_group (E : Type*) extends has_norm E, add_comm_group E, pseudo_metric_space E :=
(dist_eq : ∀ x y : E, dist x y = norm (x - y))

/-- A normed group is an additive group endowed with a norm for which `dist x y = ∥x - y∥` defines
a metric space structure. -/
class normed_add_group (E : Type*) extends has_norm E, add_comm_group E, metric_space E :=
(dist_eq : ∀ x y : E, dist x y = norm (x - y))

/-- A normed group is a seminormed group. -/
@[priority 100] -- see Note [lower instance priority]
instance normed_add_group.to_seminormed_add_group [h : normed_add_group E] : seminormed_add_group E :=
{ .. h }

/-- Construct a seminormed group from a translation invariant pseudodistance. -/
def seminormed_add_group.of_add_dist [has_norm E] [add_comm_group E] [pseudo_metric_space E]
  (H1 : ∀ x : E, ∥x∥ = dist x 0)
  (H2 : ∀ x y z : E, dist x y ≤ dist (x + z) (y + z)) : seminormed_add_group E :=
{ dist_eq := λ x y, begin
...
def seminormed_add_group.of_add_dist' [has_norm E] [add_comm_group E] [pseudo_metric_space E]
  (H1 : ∀ x : E, ∥x∥ = dist x 0)
  (H2 : ∀ x y z : E, dist (x + z) (y + z) ≤ dist x y) : seminormed_add_group E :=
{ dist_eq := λ x y, begin
    rw H1, apply le_antisymm,
    { have := H2 (x - y) 0 y, rwa [sub_add_cancel, zero_add] at this },
    { rw [sub_eq_add_neg, ← add_right_neg y], apply H2 }
...
structure seminormed_add_group.core (E : Type*) [add_comm_group E] [has_norm E] : Prop :=
(norm_zero : ∥(0 : E)∥ = 0)
...
def seminormed_add_group.of_core (E : Type*) [add_comm_group E] [has_norm E]
  (C : seminormed_add_group.core E) : seminormed_add_group E :=
{ dist := λ x y, ∥x - y∥,
...
instance : normed_add_group punit :=
{ norm := function.const _ 0,
  dist_eq := λ _ _, rfl, }

@[simp] lemma punit.norm_eq_zero (r : punit) : ∥r∥ = 0 := rfl

noncomputable instance : normed_add_group ℝ :=
{ norm := λ x, |x|,
...
seminormed_add_group.dist_eq _ _
...
lemma normed_add_group.nhds_zero_basis_norm_lt :
  (𝓝 (0 : E)).has_basis (λ (ε : ℝ), 0 < ε) (λ (ε : ℝ), { y | ∥y∥ < ε }) :=
begin
  convert normed_add_group.nhds_basis_norm_lt (0 : E),
  simp,
end

lemma normed_add_group.uniformity_basis_dist :
...
@[priority 100] -- see Note [lower instance priority]
instance seminormed_add_group.to_has_nnnorm : has_nnnorm E := ⟨λ a, ⟨norm a, norm_nonneg a⟩⟩
...
@[reducible]
def seminormed_add_group.induced {E} [add_comm_group E] (f : E →+ F) : seminormed_add_group E :=
{ norm    := λ x, ∥f x∥,
  dist_eq := λ x y, by simpa only [add_monoid_hom.map_sub, ← dist_eq_norm],
  .. pseudo_metric_space.induced f seminormed_add_group.to_pseudo_metric_space, }

/-- A subgroup of a seminormed group is also a seminormed group,
with the restriction of the norm. -/
instance add_subgroup.seminormed_add_group (s : add_subgroup E) : seminormed_add_group s :=
seminormed_add_group.induced s.subtype

/-- If `x` is an element of a subgroup `s` of a seminormed group `E`, its norm in `s` is equal to
its norm in `E`. -/
@[simp] lemma add_subgroup.coe_norm {E : Type*} [seminormed_add_group E]
...
@[norm_cast] lemma add_subgroup.norm_coe {E : Type*} [seminormed_add_group E] {s : add_subgroup E}
  (x : s) :
  ∥(x : E)∥ = ∥(x : s)∥ :=
rfl
...
instance submodule.seminormed_add_group {𝕜 : Type*} {_ : ring 𝕜}
  {E : Type*} [seminormed_add_group E] {_ : module 𝕜 E} (s : submodule 𝕜 E) : seminormed_add_group s :=
{ norm := λx, norm (x : E),
...
@[simp] lemma submodule.coe_norm {𝕜 : Type*} {_ : ring 𝕜}
  {E : Type*} [seminormed_add_group E] {_ : module 𝕜 E} {s : submodule 𝕜 E} (x : s) :
  ∥(x : s)∥ = ∥(x : E)∥ :=
rfl
...
instance ulift.seminormed_add_group : seminormed_add_group (ulift E) :=
seminormed_add_group.induced ⟨ulift.down, rfl, λ _ _, rfl⟩
...
noncomputable instance prod.seminormed_add_group : seminormed_add_group (E × F) :=
{ norm := λx, max ∥x.1∥ ∥x.2∥,
...
using the sup norm. -/
noncomputable instance pi.seminormed_add_group {π : ι → Type*} [fintype ι]
  [Π i, seminormed_add_group (π i)] : seminormed_add_group (Π i, π i) :=
...
lemma pi.norm_def {π : ι → Type*} [fintype ι] [Π i, seminormed_add_group (π i)] (f : Π i, π i) :
  ∥f∥ = ↑(finset.univ.sup (λ b, ∥f b∥₊)) := rfl

lemma pi.nnnorm_def {π : ι → Type*} [fintype ι] [Π i, seminormed_add_group (π i)] (f : Π i, π i) :
  ∥f∥₊ = finset.univ.sup (λ b, ∥f b∥₊) := subtype.eta _ _

/-- The seminorm of an element in a product space is `≤ r` if and only if the norm of each
component is. -/
lemma pi_norm_le_iff {π : ι → Type*} [fintype ι] [∀i, seminormed_add_group (π i)] {r : ℝ}
  (hr : 0 ≤ r) {x : Πi, π i} : ∥x∥ ≤ r ↔ ∀i, ∥x i∥ ≤ r :=
by simp only [← dist_zero_right, dist_pi_le_iff hr, pi.zero_apply]

lemma pi_nnnorm_le_iff {π : ι → Type*} [fintype ι] [∀i, seminormed_add_group (π i)] {r : ℝ≥0}
  {x : Πi, π i} : ∥x∥₊ ≤ r ↔ ∀i, ∥x i∥₊ ≤ r :=
pi_norm_le_iff r.coe_nonneg

/-- The seminorm of an element in a product space is `< r` if and only if the norm of each
component is. -/
lemma pi_norm_lt_iff {π : ι → Type*} [fintype ι] [∀i, seminormed_add_group (π i)] {r : ℝ}
  (hr : 0 < r) {x : Πi, π i} : ∥x∥ < r ↔ ∀i, ∥x i∥ < r :=
by simp only [← dist_zero_right, dist_pi_lt_iff hr, pi.zero_apply]

lemma pi_nnnorm_lt_iff {π : ι → Type*} [fintype ι] [∀i, seminormed_add_group (π i)] {r : ℝ≥0}
  (hr : 0 < r) {x : Πi, π i} : ∥x∥₊ < r ↔ ∀i, ∥x i∥₊ < r :=
pi_norm_lt_iff hr

lemma norm_le_pi_norm {π : ι → Type*} [fintype ι] [∀i, seminormed_add_group (π i)] (x : Πi, π i)
  (i : ι) : ∥x i∥ ≤ ∥x∥ :=
(pi_norm_le_iff (norm_nonneg x)).1 le_rfl i

lemma nnnorm_le_pi_nnnorm {π : ι → Type*} [fintype ι] [∀i, seminormed_add_group (π i)] (x : Πi, π i)
  (i : ι) : ∥x i∥₊ ≤ ∥x∥₊ :=
norm_le_pi_norm x i
...
lemma pi.sum_norm_apply_le_norm {π : ι → Type*} [fintype ι] [∀i, seminormed_add_group (π i)]
  (x : Π i, π i) :
...
/-- The $L^1$ norm is less than the $L^\infty$ norm scaled by the cardinality. -/
lemma pi.sum_nnnorm_apply_le_nnnorm {π : ι → Type*} [fintype ι] [∀i, seminormed_add_group (π i)]
  (x : Π i, π i) :
  ∑ i, ∥x i∥₊ ≤ fintype.card ι • ∥x∥₊ :=
...
begin
  ...
  rw normed_add_group.tendsto_nhds_zero at hf ⊢,
  intros ε ε₀,
...
@[priority 100] -- see Note [lower instance priority]
instance seminormed_add_group.has_lipschitz_add : has_lipschitz_add E :=
...
lemma seminormed_add_group.mem_closure_iff {s : set E} {x : E} :
...
lemma norm_le_zero_iff' [t0_space E] {g : E} :
  ∥g∥ ≤ 0 ↔ g = 0 :=
begin
  letI : normed_add_group E := { to_metric_space := metric.of_t0_pseudo_metric_space E,
...
def normed_add_group.of_add_dist [has_norm E] [add_comm_group E] [metric_space E]
  (H1 : ∀ x : E, ∥x∥ = dist x 0)
  (H2 : ∀ x y z : E, dist x y ≤ dist (x + z) (y + z)) : normed_add_group E :=
{ dist_eq := λ x y, begin
    rw H1, apply le_antisymm,
    { rw [sub_eq_add_neg, ← add_right_neg y], apply H2 },
    { have := H2 (x-y) 0 y, rwa [sub_add_cancel, zero_add] at this }
...
formalised in this structure. -/
structure normed_add_group.core (E : Type*) [add_comm_group E] [has_norm E] : Prop :=
(norm_eq_zero_iff : ∀ x : E, ∥x∥ = 0 ↔ x = 0)
(triangle : ∀ x y : E, ∥x + y∥ ≤ ∥x∥ + ∥y∥)
(norm_neg : ∀ x : E, ∥-x∥ = ∥x∥)

/-- The `seminormed_add_group.core` induced by a `normed_add_group.core`. -/
lemma normed_add_group.core.to_seminormed_add_group.core {E : Type*} [add_comm_group E] [has_norm E]
  (C : normed_add_group.core E) : seminormed_add_group.core E :=
...
def normed_add_group.of_core (E : Type*) [add_comm_group E] [has_norm E]
  (C : normed_add_group.core E) : normed_add_group E :=
{ eq_of_dist_eq_zero := λ x y h,
  begin
    rw [dist_eq_norm] at h,
    exact sub_eq_zero.mp ((C.norm_eq_zero_iff _).1 h)
  end
  ..seminormed_add_group.of_core E (normed_add_group.core.to_seminormed_add_group.core C) }

variables [normed_add_group E] [normed_add_group F] {x y : E}
...
/-- An injective group homomorphism from an `add_comm_group` to a `normed_add_group` induces a
`normed_add_group` structure on the domain.

See note [reducible non-instances]. -/
@[reducible]
def normed_add_group.induced {E} [add_comm_group E]
  (f : E →+ F) (h : function.injective f) : normed_add_group E :=
{ .. seminormed_add_group.induced f,
  .. metric_space.induced f h normed_add_group.to_metric_space, }

/-- A subgroup of a normed group is also a normed group, with the restriction of the norm. -/
instance add_subgroup.normed_add_group (s : add_subgroup E) : normed_add_group s :=
normed_add_group.induced s.subtype subtype.coe_injective

/-- A submodule of a normed group is also a normed group, with the restriction of the norm.

See note [implicit instance arguments]. -/
instance submodule.normed_add_group {𝕜 : Type*} {_ : ring 𝕜}
  {E : Type*} [normed_add_group E] {_ : module 𝕜 E} (s : submodule 𝕜 E) : normed_add_group s :=
{ ..submodule.seminormed_add_group s }

instance ulift.normed_add_group : normed_add_group (ulift E) := { ..ulift.seminormed_add_group }

/-- normed group instance on the product of two normed groups, using the sup norm. -/
noncomputable instance prod.normed_add_group : normed_add_group (E × F) := { ..prod.seminormed_add_group }

/-- normed group instance on the product of finitely many normed groups, using the sup norm. -/
noncomputable instance pi.normed_add_group {π : ι → Type*} [fintype ι] [∀i, normed_add_group (π i)] :
  normed_add_group (Πi, π i) := { ..pi.seminormed_add_group }
