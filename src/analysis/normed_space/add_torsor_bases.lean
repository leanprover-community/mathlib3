...
lemma exists_subset_affine_independent_affine_span_eq_top_of_open {s u : set P} (hu : is_open u)
...
begin
  ...
  { intros y,
    ...
    cases eq_or_ne (∥y -ᵥ q∥) 0 with hyp hyp, { rwa [hyp, mul_zero], },
    rw [← norm_pos_iff, norm_norm] at hyp,
    calc abs (ε / 2 / ∥y -ᵥ q∥) * ∥y -ᵥ q∥
          = ε / 2 / ∥y -ᵥ q∥ * ∥y -ᵥ q∥ : by rw [abs_div, abs_of_pos (half_pos hε), abs_of_pos hyp]
      ... = ε / 2 : div_mul_cancel _ (ne_of_gt hyp)
      ... < ε : half_lt_self hε, },
  ...
  { intros y hy,
    ...
    simp only [w, hps, line_map_apply_one, units.coe_mk0, dif_neg, dif_pos, not_false_iff,
      units.coe_one, subtype.coe_mk] at hyp;
    rw ← hyp;
