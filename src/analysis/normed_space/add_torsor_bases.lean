...
begin
  ...
  { -- The zero-dimensional case.
    suffices : range (b.points) = univ, { simv [this], },
  ...
  { -- The positive-dimensional case.
    ...
    { rw convex_hull_affine_basis_eq_nonneg_barycentric b, ext, simv, },
    ext,
    simv only [this, interior_Inter_of_fintype, ← is_open_map.preimage_interior_eq_interior_preimage
...
begin
  ...
  { intros y,
    apply hεu,
    simv only [metric.mem_ball, f, line_map_apply, dist_vadd_left, norm_smul, real.norm_eq_abs,
  ...
  { simv only [ne.def, div_eq_zero_iff, or_false, dist_eq_zero, bit0_eq_zero, one_ne_zero,
  ...
  { intros p hp, use ⟨p, ht₁ hp⟩, simv [w, hp], },
  { intros y hy,
    simv only [set.mem_range, set_coe.exists, subtype.coe_mk] at hy,
    obtain ⟨p, hp, hyq⟩ := hy,
    by_cases hps : p ∈ s;
    simv only [w, hps, line_map_apply_one, units.coe_mk0, dif_neg, dif_pos, not_false_iff,
...
begin
  ...
  { intros h,
    ...
    simv [finset.centroid_def, b.coord_apply_combination_of_mem (finset.mem_univ _)
