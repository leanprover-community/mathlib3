...
structure is_bounded_linear_map (𝕜 : Type*) [normed_field 𝕜]
  {E : Type*} [normed_add_group E] [normed_space 𝕜 E]
  {F : Type*} [normed_add_group F] [normed_space 𝕜 F] (f : E → F)
...
lemma is_bounded_linear_map_prod_multilinear
  {E : ι → Type*} [∀ i, normed_add_group (E i)] [∀ i, normed_space 𝕜 (E i)] :
...
/-! We prove some computation rules for continuous (semi-)bilinear maps in their first argument.
  ...
  `continuous_linear_map.to_normed_add_group`, but we don't need to assume this for the first argument
  of `f`.
-/
...
variables {G' : Type*} [normed_add_group G'] [normed_space 𝕜₂ G'] [normed_space 𝕜' G']
...
lemma is_bounded_bilinear_map_smul {𝕜' : Type*} [normed_field 𝕜']
  [normed_algebra 𝕜 𝕜'] {E : Type*} [normed_add_group E] [normed_space 𝕜 E] [normed_space 𝕜' E]
...
[decidable_eq ι] [fintype ι] [∀ i, normed_add_group (E i)] [∀ i, normed_space 𝕜 (E i)] :
