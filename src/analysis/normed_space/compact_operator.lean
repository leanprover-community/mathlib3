...
* `compact_operator` : predicate for compact operators

## Main statements

* `compact_operator_iff_image_closed_ball_rel_compact` : the usual characterization of compact
  operators from a normed space to a T2 TVS.
* `compact_operator.comp_clm` : precomposing a compact operator by a continuous linear map gives
  a compact operator
* `compact_operator.clm_comp` : postcomposing a compact operator by a continuous linear map gives
  a compact operator
* `compact_operator.continuous` : compact operators are automatically continuous
* `is_closed_set_of_compact_operator` : the set of compact operators is closed for the operator
  norm

## Implementation details

We define `compact_operator` as a predicate, because the space of compact operators inherits all
...
`is_closed_set_of_compact_operator` should be generalized to this setup.
...
We prove the equivalence in `compact_operator_iff_exists_mem_nhds_image_in_compact`. -/
def compact_operator {M₁ M₂ : Type*} [has_zero M₁] [topological_space M₁]
  [topological_space M₂] (f : M₁ → M₂) : Prop :=
∃ K, is_compact K ∧ f ⁻¹' K ∈ (𝓝 0 : filter M₁)

lemma compact_operator_zero {M₁ M₂ : Type*} [has_zero M₁] [topological_space M₁]
...
lemma compact_operator.comp_clm [add_comm_monoid M₂] [module R₂ M₂] {f : M₂ → M₃}
  (hf : compact_operator f) (g : M₁ →SL[σ₁₂] M₂) :
  compact_operator (f ∘ g) :=
begin
...
lemma compact_operator.continuous_comp {f : M₁ → M₂} (hf : compact_operator f) {g : M₂ → M₃}
  (hg : continuous g) :
  compact_operator (g ∘ f) :=
begin
  rcases hf with ⟨K, hK, hKf⟩,
...
lemma compact_operator.clm_comp [add_comm_monoid M₂] [module R₂ M₂] [add_comm_monoid M₃]
  [module R₃ M₃] {f : M₁ → M₂} (hf : compact_operator f) (g : M₂ →SL[σ₂₃] M₃) :
  compact_operator (g ∘ f) :=
hf.continuous_comp g.continuous

lemma compact_operator.continuous_on_comp [t2_space M₂] {f : M₁ → M₂} (hf : compact_operator f)
  {g : M₂ → M₃} (hg : continuous_on g (closure $ range f)) :
  compact_operator (g ∘ f) :=
begin
  rw compact_operator_iff_exists_mem_nhds_image_rel_compact at hf,
...
compact. -/
lemma compact_operator.restrict_invariant {f: M₁ →ₗ[R₁] M₁} (hf : compact_operator f)
  {V : submodule R₁ M₁} (hV : ∀ v ∈ V, f v ∈ V) (h_closed : is_closed (V : set M₁)):
  compact_operator (f.restrict hV) :=
begin
  rcases hf with ⟨K, hK, hKf⟩,
...
compact. -/
lemma compact_operator.restrict_invariant' [separated_space M₂] {f: M₂ →ₗ[R₂] M₂}
  (hf : compact_operator f) {V : submodule R₂ M₂} (hV : ∀ v ∈ V, f v ∈ V)
  [hcomplete : complete_space V] : compact_operator (f.restrict hV) :=
