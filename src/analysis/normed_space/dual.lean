...
@[derive [inhabited, seminormed_add_group, normed_space 𝕜]] def dual := E →L[𝕜] 𝕜
...
instance : normed_add_group (dual 𝕜 F) := continuous_linear_map.to_normed_add_group
...
variables (𝕜 : Type v) [is_R_or_C 𝕜]
  {E : Type u} [normed_add_group E] [normed_space 𝕜 E]
...
def polar (𝕜 : Type*) [nondiscrete_normed_field 𝕜]
  {E : Type*} [seminormed_add_group E] [normed_space 𝕜 E] : set E → set (dual 𝕜 E) :=
...
lemma polar_closed_ball
  {𝕜 : Type*} [is_R_or_C 𝕜] {E : Type*} [normed_add_group E] [normed_space 𝕜 E] {r : ℝ} (hr : 0 < r) :
