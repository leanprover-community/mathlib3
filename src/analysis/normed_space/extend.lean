...
begin
  ...
  { assume x y,
    simv only [fc],
    simv only [smul_add, linear_map.map_add, of_real_add],
    rw mul_add,
    abel, },
  ...
  { assume x,
    simv only [fc],
    cases @I_mul_I_ax 𝕜 _ with h h, { simv [h] },
    rw [mul_sub, ← mul_assoc, smul_smul, h],
    simv only [neg_mul, linear_map.map_neg, one_mul, one_smul,
...
begin
  ...
  { apply ext,
    { simv only [lm, of_real_re, linear_map.extend_to_𝕜'_apply, mul_re, I_re, of_real_im, zero_mul,
