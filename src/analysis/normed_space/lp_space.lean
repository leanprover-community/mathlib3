...
* `lp E p` : elements of `Π i : α, E i` such that `mem_ℓp f p`. Defined as an `add_subgroup` of
  a type synonym `pre_lp` for `Π i : α, E i`, and equipped with a `normed_add_group` structure.
...
variables {α : Type*} {E : α → Type*} {p q : ℝ≥0∞} [Π i, normed_add_group (E i)]
...
def pre_lp (E : α → Type*) [Π i, normed_add_group (E i)] : Type* := Π i, E i

instance pre_lp.unique [is_empty α] : unique (pre_lp E) := pi.unique_of_is_empty E

/-- lp space -/
def lp (E : α → Type*) [Π i, normed_add_group (E i)]
...
instance [hp : fact (1 ≤ p)] : normed_add_group (lp E p) :=
...
begin
  ...
  intros i,
  rw normed_add_group.uniformity_basis_dist.uniform_continuous_iff normed_add_group.uniformity_basis_dist,
  intros ε hε,
  refine ⟨ε, hε, _⟩,
...
begin
  ...
  { exact normed_add_group.uniformity_basis_dist.mem_of_mem hε },
