...
def exponentiate_norm_has_dist {r : ℝ≥0} (hr : r ≤ 1) (α : Type*)
  (i : normed_add_comm_group α) :
  has_dist α :=
{ dist := λ x y, ∥x - y∥ ^ (r:ℝ) }

example (α : Type*) [normed_add_comm_group α] (a b : α) : ∥a - b∥ = ∥b - a∥ := norm_sub_rev a b
...
begin
  change uniform_space_of_edist _ _ _ _ = _,
end
...
{ dist := λ x y, ∥x - y∥ ^ (r:ℝ),
  ...
  to_uniform_space := i.to_metric_space.to_pseudo_metric_space.to_uniform_space,
