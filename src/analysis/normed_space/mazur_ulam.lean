...
/-- **Mazur-Ulam Theorem**: if `f` is an isometry_equiv bijection between two normed add-torsors
over normed vector spaces over `ℝ`, then `f` is an affine isometry equivalence. -/
