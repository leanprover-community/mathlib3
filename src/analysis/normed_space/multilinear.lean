...
universes u v wE wE' wF wF' wG wG'
variables {𝕜 : Type u} {ι : Type v} {n : ℕ}
  {E : fin n.succ → Type wE} {E' : fin n.succ → Type wE'}
  {F : ι → Type wF} {F' : ι → Type wF'}
  {G : Type wG} {G' : Type wG'}
...
variable (f : multilinear_map 𝕜 F G)
...
lemma bound_of_shell {ε : ι → ℝ} {C : ℝ} (hε : ∀ i, 0 < ε i) {c : ι → 𝕜} (hc : ∀ i, 1 < ∥c i∥)
  (hf : ∀ m : Π i, F i, (∀ i, ε i / ∥c i∥ ≤ ∥m i∥) → (∀ i, ∥m i∥ < ε i) → ∥f m∥ ≤ C * ∏ i, ∥m i∥)
  (m : Π i, F i) : ∥f m∥ ≤ C * ∏ i, ∥m i∥ :=
begin
...
begin
  ...
  resetI,
  obtain ⟨ε : ℝ, ε0 : 0 < ε, hε : ∀ m : Π i, F i, ∥m - 0∥ < ε → ∥f m - f 0∥ < 1⟩ :=
...
def mk_continuous (C : ℝ) (H : ∀ m, ∥f m∥ ≤ C * ∏ i, ∥m i∥) :
  continuous_multilinear_map 𝕜 F G :=
...
defines a normed space structure on `continuous_multilinear_map 𝕜 F G`.
-/
namespace continuous_multilinear_map

variables (c : 𝕜) (f g : continuous_multilinear_map 𝕜 F G) (m : Πi, F i)
...
instance has_op_norm : has_norm (continuous_multilinear_map 𝕜 F G) := ⟨op_norm⟩

lemma norm_def : ∥f∥ = Inf {c | 0 ≤ (c : ℝ) ∧ ∀ m, ∥f m∥ ≤ c * ∏ i, ∥m i∥} := rfl

-- So that invocations of `real.Inf_le` make sense: we show that the set of
-- bounds is nonempty and bounded below.
lemma bounds_nonempty {f : continuous_multilinear_map 𝕜 F G} :
  ∃ c, c ∈ {c | 0 ≤ c ∧ ∀ m, ∥f m∥ ≤ c * ∏ i, ∥m i∥} :=
let ⟨M, hMp, hMb⟩ := f.bound in ⟨M, le_of_lt hMp, hMb⟩

lemma bounds_bdd_below {f : continuous_multilinear_map 𝕜 F G} :
...
/-- Continuous multilinear maps themselves form a normed space with respect to
    the operator norm. -/
instance to_normed_group : normed_group (continuous_multilinear_map 𝕜 F G) :=
normed_group.of_core _ ⟨op_norm_zero_iff, op_norm_add_le, op_norm_neg⟩

instance to_normed_space : normed_space 𝕜' (continuous_multilinear_map 𝕜 F G) :=
⟨λ c f, f.op_norm_smul_le c⟩

lemma op_norm_prod (f : continuous_multilinear_map 𝕜 F G)
  (g : continuous_multilinear_map 𝕜 F G') :
  ∥f.prod g∥ = ∥(f, g)∥ :=
le_antisymm
...
def prodL :
  (continuous_multilinear_map 𝕜 F G) × (continuous_multilinear_map 𝕜 F G') ≃ₗᵢ[𝕜]
    continuous_multilinear_map 𝕜 F (G × G') :=
...
def comp_continuous_linear_mapL :
  continuous_multilinear_map 𝕜 (λ i : ι, F i →L[𝕜] F' i)
    (continuous_multilinear_map 𝕜 F' G →L[𝕜] continuous_multilinear_map 𝕜 F G) :=
multilinear_map.mk_continuous
  { to_fun := λ f, linear_map.mk_continuous
    { to_fun := λ g, g.comp_continuous_linear_map f,
      map_add' := λ g₁ g₂, rfl,
      map_smul' := λ c g, rfl } ∥f∥ (λ g, _) } 1 _

section restrict_scalars

variables [Π i, normed_space 𝕜' (F i)] [∀ i, is_scalar_tower 𝕜' 𝕜 (F i)]
...
lemma continuous_restrict_scalars :
  continuous (restrict_scalars 𝕜' : continuous_multilinear_map 𝕜 F G →
    continuous_multilinear_map 𝕜' F G) :=
...
where the other terms in the sum are the same products where `1` is replaced by any `i`.-/
lemma norm_image_sub_le' (m₁ m₂ : Π i, F i) :
  ∥f m₁ - f m₂∥ ≤
...
lemma continuous_eval :
  continuous (λ (p : (continuous_multilinear_map 𝕜 F G × (Π i, F i))), p.1 p.2) :=
begin
...
lemma continuous_eval_left (m : Π i, F i) :
  continuous (λ p : continuous_multilinear_map 𝕜 F G, p m) :=
continuous_eval.comp (continuous.prod_mk continuous_id continuous_const)

lemma has_sum_eval
  {α : Type*} {p : α → continuous_multilinear_map 𝕜 F G} {q : continuous_multilinear_map 𝕜 F G}
  (h : has_sum p q) (m : Π i, F i) : has_sum (λ a, p a m) (q m) :=
begin
  dsimp [has_sum] at h ⊢,
...
instance [complete_space G] : complete_space (continuous_multilinear_map 𝕜 F G) :=
begin
  have nonneg : ∀ (v : Π i, F i), 0 ≤ ∏ i, ∥v i∥ :=
  ...
  -- and establish that the evaluation at any point `v : Π i, F i` is Cauchy.
  ...
  let Fmult : multilinear_map 𝕜 F G :=
  { to_fun := F',
  ...
  -- Thus `F'` is continuous, and we propose that as the limit point of our original
  -- Cauchy sequence.
...
nonnegative. -/
lemma multilinear_map.mk_continuous_norm_le (f : multilinear_map 𝕜 F G) {C : ℝ} (hC : 0 ≤ C)
...
@[simp] lemma mk_pi_field_apply (z : G) (m : ι → 𝕜) :
  continuous_multilinear_map.mk_pi_field 𝕜 ι z m = (∏ i, m i) • z := rfl
...
linear isomorphism in `continuous_multilinear_curry_left_equiv_aux 𝕜 E G`.
The algebraic version (without continuity assumption on the maps) is
`multilinear_curry_left_equiv 𝕜 E G`, and the topological isomorphism (registering
additionally that the isomorphism is continuous) is
`continuous_multilinear_curry_left_equiv 𝕜 E G`.
...
`continuous_multilinear_curry_left_equiv 𝕜 E G`. The algebraic version (without topology) is given
in `multilinear_curry_left_equiv 𝕜 E G`.
...
unless you need the full framework of continuous linear equivs. -/
def continuous_multilinear_curry_right_equiv' :
  (continuous_multilinear_map 𝕜 (λ i : fin n, G) (G →L[𝕜] G')) ≃L[𝕜]
  (continuous_multilinear_map 𝕜 (λ i : fin n.succ, G) G') :=
...
@[simp] lemma continuous_multilinear_curry_right_equiv_apply'
  (f : (continuous_multilinear_map 𝕜 (λ i : fin n, G) (G →L[𝕜] G)))
  (v : Π (i : fin n.succ), G) :
  (continuous_multilinear_curry_right_equiv' 𝕜 n G G) f v = f (init v) (v (last n)) := rfl

@[simp] lemma continuous_multilinear_curry_right_equiv_symm_apply'
  (f : continuous_multilinear_map 𝕜 (λ(i : fin n.succ), G) G)
  (v : Π (i : fin n), G) (x : G) :
...
Therefore, the space of continuous multilinear maps on `(fin 0) → G` with values in `G` is
isomorphic (and even isometric) to `G`. As this is the zeroth step in the construction of iterated
...
variables (𝕜 G)
/-- Associating to an element `x` of a vector space `G` the continuous multilinear map in `0`
variables taking the (unique) value `x` -/
def continuous_multilinear_map.curry0 (x : G') :
...
@[simp] lemma continuous_multilinear_map.curry0_apply (x : G) (m : (fin 0) → G) :
  (continuous_multilinear_map.curry0 𝕜 G x : ((fin 0) → G) → G) m = x := rfl
...
@[simp] lemma continuous_multilinear_map.apply_zero_curry0
  (f : continuous_multilinear_map 𝕜 (λ (i : fin 0), G) G) {x : fin 0 → G} :
...
lemma continuous_multilinear_map.curry0_norm
  (f : continuous_multilinear_map 𝕜 (λ (i : fin 0), G) G) : ∥f.uncurry0∥ = ∥f∥ :=
by simp
...
def continuous_multilinear_curry_fin0_aux :
  (continuous_multilinear_map 𝕜 (λ (i : fin 0), G) G) ≃ₗ[𝕜] G :=
...
def continuous_multilinear_curry_fin0 :
  (continuous_multilinear_map 𝕜 (λ (i : fin 0), G) G) ≃L[𝕜] G :=
{ continuous_to_fun := begin
...
def continuous_multilinear_curry_fin1 :
  (continuous_multilinear_map 𝕜 (λ (i : fin 1), G) G) ≃L[𝕜] (G →L[𝕜] G) :=
...
@[simp] lemma continuous_multilinear_curry_fin1_apply
  (f : (continuous_multilinear_map 𝕜 (λ (i : fin 1), G) G)) (x : G) :
