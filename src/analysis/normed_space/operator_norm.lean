...
theory for `seminormed_add_group` and we specialize to `normed_add_group` at the end.
...
variables [seminormed_add_group E] [seminormed_add_group Eₗ] [seminormed_add_group F] [seminormed_add_group Fₗ]
variables [seminormed_add_group G] [seminormed_add_group Gₗ]
...
begin
  ...
  rcases normed_add_group.tendsto_nhds_nhds.1 (hf.tendsto 0) ε hε with ⟨δ, δ_pos, hδ⟩,
  replace hδ := hδ x,
...
begin
  rcases normed_add_group.tendsto_nhds_nhds.1 (hf.tendsto 0) 1 zero_lt_one with ⟨ε, ε_pos, hε⟩,
...
/-- Continuous linear maps themselves form a seminormed space with respect to
    the operator norm. -/
instance to_seminormed_add_group : seminormed_add_group (E →SL[σ₁₂] F) :=
seminormed_add_group.of_core _ ⟨op_norm_zero, λ x y, op_norm_add_le x y, op_norm_neg⟩
...
{ norm_mul := λ f g, op_norm_comp_le f g,
  .. continuous_linear_map.to_seminormed_add_group, .. continuous_linear_map.ring }
...
universes u₁ u₂ u₃ u₄
variables (M₁ : Type u₁) [seminormed_add_group M₁] [normed_space 𝕜 M₁]
          (M₂ : Type u₂) [seminormed_add_group M₂] [normed_space 𝕜 M₂]
          (M₃ : Type u₃) [seminormed_add_group M₃] [normed_space 𝕜 M₃]
          (M₄ : Type u₄) [seminormed_add_group M₄] [normed_space 𝕜 M₄]
...
namespace continuous_linear_map
variables {E' F' : Type*} [seminormed_add_group E'] [seminormed_add_group F']
...
variables [normed_add_group E] [normed_add_group F] [normed_add_group G] [normed_add_group Fₗ]
...
/-- Continuous linear maps themselves form a normed space with respect to
    the operator norm. -/
instance to_normed_add_group [ring_hom_isometric σ₁₂] : normed_add_group (E →SL[σ₁₂] F) :=
normed_add_group.of_core _ ⟨λ f, op_norm_zero_iff f, op_norm_add_le, op_norm_neg⟩

/-- Continuous linear maps form a normed ring with respect to the operator norm. -/
instance to_normed_ring : normed_ring (E →L[𝕜] E) :=
{ .. continuous_linear_map.to_normed_add_group, .. continuous_linear_map.to_semi_normed_ring }
...
variables {E' : Type*} [seminormed_add_group E'] [normed_space 𝕜 E'] [ring_hom_isometric σ₁₂]
...
variables {𝕜₂' : Type*} [nondiscrete_normed_field 𝕜₂'] {F' : Type*} [normed_add_group F']
...
variables {H : Type*} [normed_add_group H] [normed_space 𝕜₄ H] [normed_space 𝕜₃ G]
...
def arrow_congr {F H : Type*} [normed_add_group F] [normed_add_group H]
...
-/
def is_coercive
  [normed_add_group E] [normed_space ℝ E]
