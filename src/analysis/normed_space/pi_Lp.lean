...
norm. -/
instance seminormed_add_group [Π i, seminormed_add_group (β i)] : seminormed_add_group (pi_Lp p β) :=
{ norm := λf, (∑ i, ∥f i∥ ^ p) ^ (1/p),
  dist_eq := λ x y, by simp [pi_Lp.dist_eq, dist_eq_norm, sub_eq_add_neg],
  .. pi.add_comm_group }

/-- normed group instance on the product of finitely many normed groups, using the `L^p` norm. -/
instance normed_add_group [Π i, normed_add_group (α i)] : normed_add_group (pi_Lp p α) :=
{ ..pi_Lp.seminormed_add_group p α }

omit fact_one_le_p
lemma norm_eq {p : ℝ} [fact (1 ≤ p)] {β : ι → Type*}
  [Π i, seminormed_add_group (β i)] (f : pi_Lp p β) :
  ∥f∥ = (∑ i, ∥f i∥ ^ p) ^ (1/p) := rfl
...
lemma norm_eq_of_nat {p : ℝ} [fact (1 ≤ p)] {β : ι → Type*}
  [Π i, seminormed_add_group (β i)] (n : ℕ) (h : p = n) (f : pi_Lp p β) :
  ∥f∥ = (∑ i, ∥f i∥ ^ n) ^ (1/(n : ℝ)) :=
by simp [norm_eq, h, real.sqrt_eq_rpow, ←real.rpow_nat_cast]

lemma norm_eq_of_L2 {β : ι → Type*} [Π i, seminormed_add_group (β i)] (x : pi_Lp 2 β) :
  ∥x∥ = sqrt (∑ (i : ι), ∥x i∥ ^ 2) :=
by { rw [norm_eq_of_nat 2]; simp [sqrt_eq_rpow] }

lemma nnnorm_eq_of_L2 {β : ι → Type*} [Π i, seminormed_add_group (β i)] (x : pi_Lp 2 β) :
  ∥x∥₊ = nnreal.sqrt (∑ (i : ι), ∥x i∥₊ ^ 2) :=
subtype.ext $ by { push_cast, exact norm_eq_of_L2 x }

lemma dist_eq_of_L2 {β : ι → Type*} [Π i, seminormed_add_group (β i)] (x y : pi_Lp 2 β) :
  dist x y = (∑ i, dist (x i) (y i) ^ 2).sqrt :=
by simp_rw [dist_eq_norm, norm_eq_of_L2, pi.sub_apply]

lemma nndist_eq_of_L2 {β : ι → Type*} [Π i, seminormed_add_group (β i)] (x y : pi_Lp 2 β) :
  nndist x y = (∑ i, nndist (x i) (y i) ^ 2).sqrt :=
subtype.ext $ by { push_cast, exact dist_eq_of_L2 _ _ }

lemma edist_eq_of_L2 {β : ι → Type*} [Π i, seminormed_add_group (β i)] (x y : pi_Lp 2 β) :
...
instance normed_space [Π i, seminormed_add_group (β i)] [Π i, normed_space 𝕜 (β i)] :
...
instance finite_dimensional [Π i, seminormed_add_group (β i)] [Π i, normed_space 𝕜 (β i)]
...
variables {𝕜 p α} [Π i, seminormed_add_group (β i)] [Π i, normed_space 𝕜 (β i)] (c : 𝕜)
...
variables (p 𝕜) (E : Type*) [normed_add_group E] [normed_space 𝕜 E]
...
lemma nnnorm_equiv_symm_const {β} [seminormed_add_group β] (b : β) :
...
lemma norm_equiv_symm_const {β} [seminormed_add_group β] (b : β) :
  ∥(pi_Lp.equiv p (λ _ : ι, β)).symm (function.const _ b)∥ = fintype.card ι ^ (1 / p) * ∥b∥ :=
(congr_arg coe $ nnnorm_equiv_symm_const b).trans $ by simp

lemma nnnorm_equiv_symm_one {β} [seminormed_add_group β] [has_one β] :
  ∥(pi_Lp.equiv p (λ _ : ι, β)).symm 1∥₊ = fintype.card ι ^ (1 / p) * ∥(1 : β)∥₊ :=
(nnnorm_equiv_symm_const (1 : β)).trans rfl

lemma norm_equiv_symm_one {β} [seminormed_add_group β] [has_one β] :
  ∥(pi_Lp.equiv p (λ _ : ι, β)).symm 1∥ = fintype.card ι ^ (1 / p) * ∥(1 : β)∥ :=
