...
lemma _root_.div_div_cancel_left' {G₀} [comm_group_with_zero G₀] (a b : G₀) (ha : a ≠ 0) :
  a / b / a = b⁻¹ :=
...
begin
  ...
  { clear hs,
    simp_rw exp_series_apply_zero,
    simp_rw [norm_zero, div_zero, zero_smul, add_zero],
    simp_rw [norm_zero] at hc,
    convert has_sum_coe hc,
    ext (_ | n) : 1,
  ...
  { convert has_sum_coe hc using 1,
    ext n : 1,
    let k : ℝ := ↑(2 * n)!,
    calc k⁻¹ • q ^ (2 * n)
    ...
    { rw [←coe_mul_eq_smul, div_eq_mul_inv],
      norm_cast,
      ring_nf } },
  { convert @has_sum.smul_const _ _ _ _ _ _ _ _ _ _ q _ (hs.div_const ‖q‖) using 1,
    ext n : 1,
    let k : ℝ := ↑(2 * n + 1)!,
    calc k⁻¹ • q ^ (2 * n + 1)
