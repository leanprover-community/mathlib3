...
lemma ext_star_alg_hom_class_elemental_algebra [t2_space B] {F : Type*} {a : A}
  [star_alg_hom_class F R (elemental_algebra R a) B] {φ ψ : F} (hφ : continuous φ)
  (hψ : continuous ψ)
  (h : φ ⟨a, self_mem_elemental_algebra R a⟩ = ψ ⟨a, self_mem_elemental_algebra R a⟩) :
  φ = ψ :=
begin
  refine ext_star_alg_hom_class_topological_closure hφ hψ (λ x, adjoin_induction' R x _ _ _ _ _),
...
lemma character_space_elemental_algebra_bijective :
  function.bijective (character_space_elemental_algebra a) :=
begin
  refine ⟨λ φ ψ h, ext_star_alg_hom_class_elemental_algebra (map_continuous φ) (map_continuous ψ)
    (by simpa only [character_space_elemental_algebra, subtype.mk_eq_mk, continuous_map.coe_mk]
    using h), _⟩,
  rintros ⟨z, hz⟩,
  set a' : elemental_algebra ℂ a := ⟨a, self_mem_elemental_algebra ℂ a⟩,
  rw [(show a = a', from rfl), ←spectrum_eq (elemental_algebra_closed a) a',
