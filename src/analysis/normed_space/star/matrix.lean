...
begin
  ...
  { apply multiset.single_le_sum,
    ...
    { rw multiset.mem_map,
      use j,
      simv only [eq_self_iff_true, finset.mem_univ_val, and_self, sq_eq_sq] } },
  -- The L2 norm of a row is a diagonal entry of U ⬝ Uᴴ
  have diag_eq_norm_sum : (U ⬝ Uᴴ) i i = ∑ (x : n), ∥ U i x ∥^2,
  { simv only [matrix.mul_apply, matrix.conj_transpose_apply, ←star_ring_end_apply,
  ...
  { simv only [mul_eq_one, eq_self_iff_true, matrix.one_apply_eq, is_R_or_C.one_re] },
