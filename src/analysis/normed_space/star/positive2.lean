...
local notation `C⋆(` a `)` := star_subalgebra.elemental_algebra ℂ a
.
...
begin
  ...
  exacts [(star_subalgebra.spectrum_eq (elemental_algebra_closed a) _).symm,
...
lemma comm_mem_elemental_algebra_of_comm {x y : A} (hx : commute x a) (hx' : commute x (star a))
  (hy : y ∈ elemental_algebra ℂ a) : commute x y :=
begin
...
begin
  refine ⟨ha.is_self_adjoint.pow n, _⟩,
  rw spectrum.map_pow,
  rintro _ ⟨x, hx, rfl⟩,
  obtain ⟨x, rfl⟩ := ha.spectrum_nonneg hx,
  exact ⟨x ^ n, by simp only [complex.of_real_pow, eq_self_iff_true, coe_coe, nnreal.coe_pow]⟩
end
...
@[simp]
lemma cfc_sa (a : self_adjoint A) : (cfc_sa a  : C(ℝ, ℝ) → A)
