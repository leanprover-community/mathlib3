...
begin
  ...
  intros t ht,
  simp only [mem_ball, dist_zero_right] at ht,
  simp only [inverse_one_sub t ht, set.mem_set_of],
...
begin
  ...
  intros t ht,
  simp only [ball, dist_zero_right, set.mem_set_of] at ht,
  have ht' : ∥t∥ < 1,
  { have : (2:ℝ)⁻¹ < 1 := by cancel_denoms,
    linarith },
  simp only [inverse_one_sub t ht', norm_one, mul_one, set.mem_set_of],
  change ∥∑' n : ℕ, t ^ n∥ ≤ _,
