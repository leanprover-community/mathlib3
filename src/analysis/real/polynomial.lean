...
lemma polynomial_eval_zero_discrete {f : polynomial ℝ} (hf : f ≠ 0) :
  discrete_topology {a : ℝ // f.eval a = 0} := sorry
--begin
...
lemma metric.is_open_singleton_iff {X : Type*} [metric_space X] {x : X} :
  is_open ({x} : set X) ↔ ∃ ε > 0, ∀ y, dist y x < ε → y = x := sorry
--by simp [is_open_iff, subset_singleton_iff, mem_ball]


variables s : finset ℝ
#check (s.image (λ a, s.image (λ b, dist a b )))

lemma metric.is_open_finite_iff {X : Type*} [metric_space X] {s : finset X} :
  ∃ ε > 0, ∀ x y, x ∈ s → y ∈ s → dist y x < ε → y = x :=
begin
  set distances := finset.erase (s.image (λ a, s.image (λ b, dist a b ))) (0 : ℝ),

  exact 1,exact zero_lt_one,

  split;intros a,
  { simp at *, fsplit, work_on_goal 1 { fsplit, work_on_goal 1 { intros x y ᾰ ᾰ_1 } },
  sorry,sorry,sorry, },
  rcases a with ⟨U, hU, V⟩,
...
lemma non_root_interval_of_polynomial (α : ℝ) (f : polynomial ℝ) (h_f_nonzero : f ≠ 0) :
  ∃ B : ℝ, 0 < B ∧ ∀ x (hr : abs (α - x) < B) (hf : f.eval x = 0), x = α :=
--lemma non_root_interval_of_polynomial (α : ℝ) (f : polynomial ℝ) (h_f_nonzero : f ≠ 0) :
--  ∃ B : ℝ, 0 < B ∧ ∀ x (hr : abs (α - x) < B) (hn : x ≠ α), f.eval x ≠ 0 :=
begin
--  let r : f.roots,
  obtain F := metric.is_open_singleton_iff.mp,
  set f_roots := f.roots.to_finset.erase α,
  have : discrete_topology {a : ℝ // a ∈ f.roots.to_finset} := roots_discrete f,
