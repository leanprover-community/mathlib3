...
This file contains the computation of the integrals of various simple functions, including `pow`,
`exp`, `inv`/`one_div`, `sin`, `cos`, and `λ x, 1 / (1 + x^2)`.

With these lemmas, many simple integrals can be computed by `simp` or `norm_num`. Scroll to the
bottom of the file for examples.

This file is far from complete; we are working on adding more integrals.
-/
