...
lemma logb_npow {x : ℝ} (m : ℕ) :
  logb (x ^ m) = m * logb x :=
by rw [logb, log_pow, mul_div_assoc, logb]

lemma logb_zpow {x : ℝ} (m : ℤ) :
  logb (x ^ m) = m * logb x :=
...
def log_base2_goal (x₁ x₂ a₁ a₂ : ℝ) : Prop := 0 < x₁ → x₁ ≤ x₂ → a₁ < logb x₁ ∧ logb x₂ < a₂

lemma log_base2_square {x₁ x₂ a₁ a₂ : ℝ} (h : log_base2_goal (x₁^2) (x₂^2) (2 * a₁) (2 * a₂)) :
  log_base2_goal x₁ x₂ a₁ a₂ :=
λ hx₁ hx₂, by simpa [logb_npow] using h (pow_pos hx₁ _) (pow_le_pow_of_le_left hx₁.le hx₂ _)
...
begin
  ...
  exact ⟨t.1.trans_le (logb_le_logb h₃ h₁), (logb_le_logb (hx₁.trans_le hx₂) h₂).trans_lt t.2⟩,
end
...
begin
  intros hx₁ hx₂,
  have t := h (half_pos hx₁) (div_le_div_of_le zero_le_two hx₂),
  simpa [hx₁.ne', (hx₁.trans_le hx₂).ne', logb_div_two] using t,
end
...
begin
  ...
  simpa [logb_mul hx₁.ne' i.ne', logb_mul (hx₁.trans_le hx₂).ne' i.ne',
    logb_zpow, logb_two] using this,
end

lemma log_base2_start {x₁ x₂ a₁ a₂ : ℝ} (hx₁ : 0 < x₁) (hx₂ : x₁ ≤ x₂)
  (h : log_base2_goal x₁ x₂ a₁ a₂) : a₁ < logb x₁ ∧ logb x₂ < a₂ :=
h hx₁ hx₂
