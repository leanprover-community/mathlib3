...
lemma tendsto_algebra_map_inverse_at_top_nhds_0_nat 
  (𝕜 : Type*) [semiring 𝕜] [algebra ℝ 𝕜] [topological_space 𝕜] [topological_semiring 𝕜]
  [has_continuous_smul ℝ 𝕜] :
    tendsto ((algebra_map ℝ 𝕜) ∘ (λ n : ℕ, (n : ℝ)⁻¹)) at_top (nhds 0) :=
begin
  rw ← (map_zero (algebra_map ℝ 𝕜)),
  exact tendsto.comp (continuous_algebra_map ℝ 𝕜).continuous_at tendsto_inverse_at_top_nhds_0_nat
end
