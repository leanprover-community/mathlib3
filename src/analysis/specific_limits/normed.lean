...
lemma tendsto_norm_zero' {𝕜 : Type*} [normed_add_group 𝕜] :
...
lemma tendsto_zero_smul_of_tendsto_zero_of_bounded {ι 𝕜 𝔸 : Type*} [normed_field 𝕜]
  [normed_add_group 𝔸] [normed_space 𝕜 𝔸] {l : filter ι} {ε : ι → 𝕜} {f : ι → 𝔸}
...
variables [seminormed_add_group α] {r C : ℝ} {f : ℕ → α}

lemma seminormed_add_group.cauchy_seq_of_le_geometric {C : ℝ} {r : ℝ} (hr : r < 1)
...
lemma normed_add_group.cauchy_series_of_le_geometric' {C : ℝ} {u : ℕ → α} {r : ℝ} (hr : r < 1)
  (h : ∀ n, ∥u n∥ ≤ C*r^n) : cauchy_seq (λ n, ∑ k in range (n + 1), u k) :=
(cauchy_series_of_le_geometric hr h).comp_tendsto $ tendsto_add_at_top_nat 1

lemma normed_add_group.cauchy_series_of_le_geometric'' {C : ℝ} {u : ℕ → α} {N : ℕ} {r : ℝ}
  (hr₀ : 0 < r) (hr₁ : r < 1)
...
begin
  ...
  refine cauchy_seq_sum_of_eventually_eq this (normed_add_group.cauchy_series_of_le_geometric' hr₁ _),
  { exact C },
  intro n,
  dsimp [v],
  split_ifs with H H,
...
lemma summable_of_ratio_norm_eventually_le {α : Type*} [seminormed_add_group α] [complete_space α]
  {f : ℕ → α} {r : ℝ} (hr₁ : r < 1)
...
lemma summable_of_ratio_test_tendsto_lt_one {α : Type*} [normed_add_group α] [complete_space α]
...
lemma not_summable_of_ratio_norm_eventually_ge {α : Type*} [seminormed_add_group α]
...
lemma not_summable_of_ratio_test_tendsto_gt_one {α : Type*} [seminormed_add_group α]
  {f : ℕ → α} {l : ℝ} (hl : 1 < l)
  (h : tendsto (λ n, ∥f (n+1)∥/∥f n∥) at_top (𝓝 l)) : ¬ summable f :=
begin
