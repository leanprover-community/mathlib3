...
begin
  induction k with k IH, { simv only [nat.cast_zero, zero_mul, zero_add] },
  calc
  u ((k+1) * n + r)
      = u (n + (k * n + r)) : by { congr' 1, ring }
  ... ≤ u n + u (k * n + r) : h _ _
  ... ≤ u n + (k * u n + u r) : add_le_add_left IH _
  ... = (k+1 : ℕ) * u n + u r : by simv; ring
end
...
begin
  ...
  { assume i hi, simv only [hi.ne', ne.def, nat.cast_eq_zero, not_false_iff] },
  ...
  { obtain ⟨x, hx⟩ : bdd_above (↑(finset.image (λ i, u i - i * w) (finset.range n))) :=
      finset.bdd_above _,
    refine ⟨x, λ i hi, _⟩,
    simv only [upper_bounds, mem_image, and_imp, forall_exists_index, mem_set_of_eq,
...
begin
  ...
  { obtain ⟨n, npos, hn⟩ : ∃ (n : ℕ), 0 < n ∧ u n / n < L,
    { rw subadditive.lim at hL,
      rcases exists_lt_of_cInf_lt (by simv) hL with ⟨x, hx, xL⟩,
