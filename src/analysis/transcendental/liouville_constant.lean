...
begin
  ...
  by_cases n0 : n = 0,
  { simp [n0, zero_mul, add_zero],
    repeat { refine add_lt_add _ _ },
    apply (lt_mul_iff_one_lt_right _).mpr,
    any_goals { refine lt_of_lt_of_le _ (self_le_factorial _) },
  ...
  any_goals {
    refine (factorial_lt _).mpr ((lt_add_iff_pos_left _).mpr (lt_of_lt_of_le zero_lt_two hi)) },
  repeat { exact zero_lt_iff.mpr n0 }
end
end natural
...
begin
  ...
  have h_truncation_wd := liouville_constant_eq_first_k_terms_add_rest
    (by exact_mod_cast lt_of_lt_of_le one_lt_two hm : 1 < (m : ℝ)) n,
  ...
  rw [← hp, h_truncation_wd, add_sub_cancel', abs_of_pos (liouville_constant_terms_after_pos
    (by exact_mod_cast lt_of_lt_of_le one_lt_two hm : 1 < (m : ℝ)) _)],
  refine ⟨one_lt_pow (by exact_mod_cast (lt_of_lt_of_le one_lt_two hm)) (nat.factorial_pos _), _, _⟩,
  { rw [ne.def, add_right_eq_self],
    exact ne_of_gt (liouville_constant_terms_after_pos (by exact_mod_cast lt_of_lt_of_le one_lt_two hm) n) },
  { refine lt_of_lt_of_le (calc_liou_one (by exact_mod_cast (lt_of_lt_of_le one_lt_two hm)) n) _,
    apply calc_liou_two_zero,
    assumption_mod_cast }
end
