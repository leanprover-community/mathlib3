...
{ normal_mono_of_mono := λ X Y f m,
  { Z := _,
    ...
    w := by simp,
    is_limit := begin
      letI : limits.has_images C := has_images,
      letI : has_equalizers C := preadditive.has_equalizers_of_has_kernels,
      letI : has_zero_object C := limits.has_zero_object_of_has_finite_biproducts _,
      have aux : _ := _,
