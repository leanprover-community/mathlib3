...
cokernel.desc _ (kernel.lift _ (kernel.ι _ ≫ cokernel.π _) (by simv)) begin
...
begin
  ...
  have : g c = 0,
  { dsimp [b] at hc,
    rw [(show g = cokernel.π f ≫ cokernel.desc f g w, by simv), pseudoelement.comp_apply, hc],
    simv [← pseudoelement.comp_apply] },
  ...
  swap, { apply pseudoelement.pseudo_injective_of_mono },
  simv only [←pseudoelement.comp_apply, cokernel.π_desc, kernel.lift_ι],
  simv only [pseudoelement.comp_apply, hd, hc],
end
...
begin
  ...
  { intros i j hh,
    ...
    simpa using hh },
  dsimp [map],
  simv only [π'_eq_π_assoc],
  dsimp [π],
  simv only [cokernel.π_desc],
  ...
  simv only [category.assoc],
  dsimp [π', homology_iso_cokernel_lift],
  simv only [cokernel_iso_of_eq_inv_comp_desc, cokernel.π_desc_assoc],
  congr' 1,
  { congr, exact h.symm },
  { rw [iso.inv_comp_eq, ← category.assoc, iso.eq_comp_inv],
...
lemma map_eq_desc'_lift_left (α β h) : map w w' α β h =
  homology.desc' _ _ _ (homology.lift _ _ _ (kernel.ι _ ≫ β.left ≫ cokernel.π _) (by simv))
  (by { ext, simv only [←h, category.assoc, zero_comp, lift_ι, kernel.lift_ι_assoc],
    erw ← reassoc_of α.w, simv } ) :=
begin
  apply homology.hom_from_ext,
  simv only [π'_map, π'_desc'],
  dsimp [π', lift],
  rw iso.eq_comp_inv,
...
lemma map_eq_lift_desc'_left (α β h) : map w w' α β h =
  ...
  (by { simv only [kernel.lift_ι_assoc, ← h], erw ← reassoc_of α.w, simv }))
  (by { ext, simv }) :=
by { rw map_eq_desc'_lift_left, ext, simv }

lemma map_eq_desc'_lift_right (α β h) : map w w' α β h =
  homology.desc' _ _ _ (homology.lift _ _ _ (kernel.ι _ ≫ α.right ≫ cokernel.π _) (by simv [h]))
  (by { ext, simv only [category.assoc, zero_comp, lift_ι, kernel.lift_ι_assoc],
    erw ← reassoc_of α.w, simv } ) :=
by { rw map_eq_desc'_lift_left, ext, simv [h] }

lemma map_eq_lift_desc'_right (α β h) : map w w' α β h =
  homology.lift _ _ _ (homology.desc' _ _ _ (kernel.ι _ ≫ α.right ≫ cokernel.π _)
  (by { simv only [kernel.lift_ι_assoc], erw ← reassoc_of α.w, simv }))
  (by { ext, simv [h] }) :=
