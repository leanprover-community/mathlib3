...
{ obj := λ j, limit.π (F ⋙ Cat.objects) j X ⟶ limit.π (F ⋙ Cat.objects) j Y,
  ...
  map_id' := λ X, begin
    ext f, dsimp,
    simv [functor.congr_hom (F.map_id X) f],
  end,
  map_comp' := λ X Y Z f g, begin
    ext h, dsimp,
    simv [functor.congr_hom (F.map_comp f g) h, eq_to_hom_map],
    refl,
  end, }
...
{ hom := λ X Y, limit (hom_diagram X Y),
  id := λ X, types.limit.mk.{v v} (hom_diagram X X) (λ j, 𝟙 _) (λ j j' f, by simv),
...
{ lift := limit_cone_lift F,
  ...
  begin
    ...
    { intro X,
      ext,
      dsimp, simv only [types.limit.lift_π_apply', ←w j],
      refl, },
    { intros X Y f,
      dsimp, simv [(λ j, functor.congr_hom (w j).symm f)],
      congr, },
  end, }
