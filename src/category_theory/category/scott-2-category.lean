...
variables (obj : Type u₁) (hom : obj → obj → Type v₁)
local infix `⇒`:26 := hom

-- https://ncatlab.org/nlab/show/bicategory
class two_category_struct extends has_id obj (⇒), has_comp obj (⇒) :=
[hom_cats : Π (a b : obj), category.{w₁} (hom a b)]
(left_whisker : Π {a b c : obj} {f g : a ⇒ b} (η : f ⟶ g) (h : b ⇒ c), f ≫ h ⟶ g ≫ h)
(right_whisker : Π {a b c : obj} (f : a ⇒ b) {g h : b ⇒ c} (η : g ⟶ h), f ≫ g ⟶ f ≫ h)
(left_unitor : Π {a b : obj} (f : a ⇒ b), 𝟙 a ≫ f ≅ f)
...
instance two_category_struct_has_hom [two_category_struct obj (⇒)] : has_hom obj := ⟨(⇒)⟩
...
-- https://ncatlab.org/nlab/show/bicategory
class two_category extends
  two_category_struct.{w₁ v₁} obj (⇒) :=
