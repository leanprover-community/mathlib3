...
class concrete_category (C : Type (u+1)) [category.{u} C] :=
(forget : C ⥤ Type u)
...
@[reducible] def forget (C : Type (u+1)) [category.{u} C] [concrete_category C] : C ⥤ Type u :=
...
```
-/
def concrete_category.has_coe_to_sort (C : Type (u+1)) [category.{u} C] [concrete_category C] : has_coe_to_sort C :=
{ S := Type u, coe := (concrete_category.forget C).obj }

section
local attribute [instance] concrete_category.has_coe_to_sort

variables {C : Type (u+1)} [category.{u} C] [concrete_category C]
...
-/
class has_forget₂ (C D : Type (u+1)) [category.{u} C] [concrete_category C] [category.{u} D] [concrete_category D] :=
(forget₂ : C ⥤ D)
...
instance induced_category.concrete_category {C D : Type (u+1)} [category.{u} D] [concrete_category D] (f : C → D) :
  concrete_category (induced_category D f) :=
{ forget := induced_functor f ⋙ forget D }

instance induced_category.has_forget₂ {C D : Type (u+1)} [category.{u} D] [concrete_category D] (f : C → D) :
...
-/
def has_forget₂.mk' {C D : Type (u+1)} [category.{u} C] [concrete_category C] [category.{u} D] [concrete_category D]
...
instance has_forget_to_Type (C : Type (u+1)) [category.{u} C] [concrete_category C] : has_forget₂ C (Type u) :=
{ forget₂ := forget C,
