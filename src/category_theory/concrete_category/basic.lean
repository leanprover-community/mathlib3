...
class concrete_category (C : Type u) extends category.{u} C :=
(forget : C ⥤ Sort u)
[forget_faithful : faithful forget]

instance (C : Type u) [concrete_category C] : has_coe_to_sort C :=
...
-/
class has_forget (C D : Type u) [concrete_category C] [concrete_category D] :=
(forget₂ : C ⥤ D)
(forget_comp : forget₂ ⋙ (forget D) = forget C)

@[reducible] def forget₂ (C D : Type u) [concrete_category C] [concrete_category D]
  [has_forget C D] : C ⥤ D :=
has_forget.forget₂ C D

instance forget_faithful (C D : Type u) [concrete_category C] [concrete_category D]
  [has_forget C D] : faithful (forget₂ C D) :=
(has_forget.forget_comp C D).faithful_of_comp

instance induced_category.concrete_category {C D : Type u} [concrete_category D] (f : C → D) :
  concrete_category (induced_category f) :=
{ forget := induced_functor f ⋙ forget D }

instance induced_category.has_forget {C D : Type u} [concrete_category D] (f : C → D) :
...
-/
def has_forget.mk' {C D : Type u} [concrete_category C] [concrete_category D]
...
instance (C : Type u) [concrete_category C] : has_forget C (Sort u) :=
{ forget₂ := forget C,
