...
This file introduces various `simv` lemmas which in favourable circumstances
...
⟨eq_to_hom p, eq_to_hom p.symm, by simv, by simv⟩
...
by simv only [← is_iso.iso.inv_hom e, functor.map_inv, h₂, is_iso.inv_comp,
...
/--
See the note on `eq_to_hom_map` regarding using this as a `simv` lemma.
-/
...
by { rw [α.naturality_assoc], simv [eq_to_hom_map], }

lemma eq_conj_eq_to_hom {X Y : C} (f : X ⟶ Y) :
  f = eq_to_hom rfl ≫ f ≫ eq_to_hom rfl :=
by simv only [category.id_comp, eq_to_hom_refl, category.comp_id]
