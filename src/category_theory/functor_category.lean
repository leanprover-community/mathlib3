...
lemma mono_app_of_mono (α : F ⟶ G) (w : ∀ (X : C), mono (α.app X)) : mono α :=
⟨λ H g h eq, by { ext X, rw [←cancel_mono (w X), ←comp_app, eq, comp_app] }⟩

/-- A natural transformation is an epimorphism if each component is. -/
lemma epi_app_of_epi (α : F ⟶ G) (w : ∀ (X : C), epi (α.app X)) : epi α :=
⟨λ H g h eq, by { ext X, rw [←cancel_epi (w X), ←comp_app, eq, comp_app] }⟩
