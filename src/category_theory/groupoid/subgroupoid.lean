...
def is_graph_like := ∀ (c d : C), subsingleton (S.arrws c c)
...
begin
  ...
  { rintros h c d, constructor, rintro ⟨f,fS⟩ ⟨g,gS⟩,
    ...
    have ifgS : inv f ≫ g ∈ S.arrws c c := S.mul' (S.inv' fS) gS,
    have idS : 𝟙 c ∈ S.arrws c c := id_mem_of_nonempty_isotropy S c ⟨_,ifgS⟩,
    obtain ⟨ss⟩ := h c,
    have : inv f ≫ g = 𝟙 _, by
    { rw ←@subtype.mk_eq_mk _ (S.arrws c c) (inv f ≫ g) ifgS (𝟙 c) (idS),
      apply ss, },
    calc f
       = f ≫ (inv g ≫ g) : by simp
...
lemma is_graph_like.maximal_iff_full (T : subgroupoid C) (h : T ≤ S) (ht : T.is_graph_like):
  maximal_graph_like S T ↔ ∀ c d, nonempty (S.arrws c d) → nonempty (T.arrws c d) :=
begin
  sorry
end
...
* Now, the problem is reduced to showing that the free groupoid `F` on `X` is graph-like iff
  its image in `V` is graph-like.
* Now, let `φ` the lifted functor from `F` to `V` whose image is `generated X`.
  `φ` is injective on objects, so that if `F` is graph-like, then so is `generated X`.
  Conversely, if `generated X` is graph-like but `F` isn't, then there exists two words in `F`
  mapping to the same element of `generated X`.
* … and something is wrong here …
-/
lemma generated.is_graph_like_iff_forest :
  (generated X).is_graph_like ↔
  ∀ c d : C, subsingleton (  (@quiver.path C ⟨λ c d, subtype $ X c d⟩ d c)
                          ⊕ (@quiver.path C ⟨λ c d, subtype $ X c d⟩ d c) ) := sorry
