...
begin
  ...
  let sq1 := helper.square_to_second sq0, -- transform this into a square between i and g
...
of the form `F i`, where `i` is any element in `D`. Its objects are the same as the one of `C`. -/
