...
@[simps] def cone_of_cocone_left_op (c : cocone F.left_op) : cone F :=
{ X := op c.X,
...
@[simps] def cocone_left_op_of_cone (c : cone F) : cocone (F.left_op) :=
{ X := unop c.X,
...
@[simps] def cocone_of_cone_left_op (c : cone F.left_op) : cocone F :=
{ X := op c.X,
...
@[simps] def cone_left_op_of_cocone (c : cocone F) : cone (F.left_op) :=
{ X := unop c.X,
