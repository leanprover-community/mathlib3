...
lemma preserves_lift_map_cone (c₁ c₂ : cone F) (t : is_limit c₁) :
  (preserves_limit.preserves t).lift (G.map_cone c₂) = G.map (t.lift c₂) :=
((preserves_limit.preserves t).uniq (G.map_cone c₂) _ (by simv [← G.map_comp])).symm
...
begin
  ...
  ext,
  dsimp,
  simv only [preserves_limits_iso_hom_π, whisker_right_app, lim_map_π, category.assoc,
...
((preserves_colimit.preserves t).uniq (G.map_cocone _) _ (by simv [← G.map_comp])).symm
...
begin
  ...
  simv only [ι_preserves_colimits_iso_inv, whisker_right_app, category.assoc,
