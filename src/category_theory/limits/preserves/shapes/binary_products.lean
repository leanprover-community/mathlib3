...
binary_fan_map_cone_limit G f g (preserves_limit.preserves l)
...
reflects_limit.reflects ((binary_fan_map_cone_limit G f g).symm l)
...
is_limit.cone_point_unique_up_to_iso
  (is_limit_of_has_product_of_preserves_limit G X Y)
  (limit.is_limit _)

@[simp]
lemma iso_hom :
  (preserves_pair.iso G X Y).hom = prod_comparison G X Y :=
rfl
