...
@[reassoc] lemma map_comp_preserves_kernel_iso_inv {X' Y' : C} (g : X' ⟶ Y') [has_kernel g]
...
begin
  ...
  exact kernel_comparison_comp_map _ _ _ _ _ _,
end
...
@[reassoc] lemma preserves_cokernel_iso_comp_map {X' Y' : C} (g : X' ⟶ Y') [has_cokernel g]
  [has_cokernel (G.map g)] [preserves_colimit (parallel_pair g 0) G] (p : X ⟶ X') (q : Y ⟶ Y')
  (hpq : f ≫ q = p ≫ g) :
...
begin
  ...
  exact map_comp_cokernel_comparison _ _ _ _ _ _,
end
