...
-/
@[simps]
def raise_cone [is_connected.{v v v} J] {B : C} {F : J ⥤ over B} (c : cone (F ⋙ forget)) :
  cone F :=
...
lemma raised_cone_lowers_to_original [is_connected.{v v v} J] {B : C} {F : J ⥤ over B}
...
def raised_cone_is_limit
  [is_connected.{v v v} J] {B : C} {F : J ⥤ over B} {c : cone (F ⋙ forget)} (t : is_limit c) :
...
instance forget_creates_connected_limits
  [is_connected.{v v v} J] {B : C} : creates_limits_of_shape J (forget : over B ⥤ C) :=
{ creates_limit := λ K,
...
instance has_connected_limits
  {B : C} [is_connected.{v v v} J] [has_limits_of_shape J C] : has_limits_of_shape J (over B) :=
