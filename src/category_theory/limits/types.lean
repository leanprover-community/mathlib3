...
which are "coherent": `∀ (j j') (f : j ⟶ j'), F.map f (x j) = x j'`.
-/
def is_limit.mk {F : J ⥤ Type w} {c : cone F} (t : is_limit c)
  (x : Π j, F.obj j) (h : ∀ (j j') (f : j ⟶ j'), F.map f (x j) = x j') :
  (c.X : Type w) :=
t.lift ⟨punit, λ j _, x j, λ j j' f, funext $ λ _, (h _ _ f).symm⟩ punit.star

@[simp]
lemma is_limit.π_mk {F : J ⥤ Type w} {c : cone F} (t : is_limit c)
  (x : Π j, F.obj j) (h : ∀ (j j') (f : j ⟶ j'), F.map f (x j) = x j') (j) :
  c.π.app j (is_limit.mk t x h) = x j :=
congr_fun (t.fac ⟨_, _⟩ j) punit.star

lemma is_limit_ext {F : J ⥤ Type w} {c : cone F} (t : is_limit c) (x y : c.X)
  (w : ∀ j, c.π.app j x = c.π.app j y) : x = y :=
begin
  suffices : (λ (_ : punit), x) = (λ (_ : punit), y),
...
-/
def quot.rel (F : J ⥤ Type w) : (Σ j, F.obj j) → (Σ j, F.obj j) → Prop :=
...
def quot (F : J ⥤ Type w) : Type (max u w) :=
...
lemma jointly_surjective (F : J ⥤ Type w) {t : cocone F} (h : is_colimit t)
...
noncomputable def is_colimit_of (F : J ⥤ Type u) (t : cocone F)
  (hsurj : ∀ (x : t.X), ∃ i xi, x = t.ι.app i xi)
...
lemma colimit_eq_iff_aux (F : J ⥤ Type u) {i j : J} {xi : F.obj i} {xj : F.obj j} :
...
lemma is_colimit_eq_iff (ht : is_colimit t) {i j : J} {xi : F.obj i} {xj : F.obj j} :
  t.ι.app i xi = t.ι.app j xj ↔ ∃ k (f : i ⟶ k) (g : j ⟶ k), F.map f xi = F.map g xj :=
begin
  split,
  { change _ → _,
    intro q,
    let my_cocone : cocone F,
    { refine ⟨ulift Prop, λ k xk, _, _⟩,
      refine ⟨∃ (l : J) (il : i ⟶ l) (kl : k ⟶ l), F.map il xi = F.map kl xk⟩,
      intros k₁ k₂ f,
      ext xk,
      split,
      { rintro ⟨l, il, kl, p⟩,
        dsimp,
        rw ← functor_to_types.map_comp_apply at p,
        exact ⟨_, _, _, p⟩ },
      { rintro ⟨l', il', k₁l', p⟩,
        dsimp,
        rcases is_filtered_or_empty.cocone_objs l' k₂ with ⟨k₃, l'k₃, k₂k₃, ⟨⟩⟩,
        rcases is_filtered_or_empty.cocone_maps (k₁l' ≫ l'k₃) (f ≫ k₂k₃) with ⟨k₄, k₃k₄, z⟩,
        refine ⟨k₄, il' ≫ l'k₃ ≫ k₃k₄, k₂k₃ ≫ k₃k₄, _⟩,
        ...
        rw p,
        rw ←functor_to_types.map_comp_apply } },
    let z := ht.desc my_cocone,
    have : my_cocone.ι.app i xi = my_cocone.ι.app j xj,
    { rw ← ht.fac my_cocone j,
      rw ← ht.fac my_cocone i,
      dsimp,
      rw q },
    dsimp at this,
    have := ulift.up.inj this,
    rw ← this,
    refine ⟨_, 𝟙 _, 𝟙 _, rfl⟩ },
  { rintro ⟨k, f, g, q⟩,
    rw ← t.w f,
    rw ← t.w g,
    dsimp,
