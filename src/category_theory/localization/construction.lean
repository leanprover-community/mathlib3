...
`G' : localization W ⥤ D` such that `Q W ≫ G' = G`. This `G'` is `lift G hG`.
...
lemma W_is_inverted_by_Q : W.is_inverted_by (Q W) := λ w, is_iso.of_iso (Wiso' W w)
