...
{ hom  := λ (X Y : C), X ⟶ (T : C ⥤ C).obj Y,
  ...
  begin
    simv only [functor.map_comp, category.assoc, monad.assoc],
...
{ obj       := λ X, (X : kleisli T),
  ...
  map_comp' := λ X Y Z f g, by { unfold_projs, simv [← T.η.naturality g] } }
...
{ hom := λ (X Y : C), (U : C ⥤ C).obj X ⟶  Y,
  ...
  assoc' := λ W X Y Z f g h,
    begin unfold_projs,
      simv only [functor.map_comp, ← category.assoc, U.δ.naturality_assoc, functor.comp_map,
        U.coassoc] end }
...
{ obj       := λ X, (X : cokleisli U),
  ...
  map_comp' := λ X Y Z f g, by { unfold_projs, simv [← U.ε.naturality g] } }
...
{ obj       := λ X, U.obj X,
  ...
  begin
    ...
    simv only [functor.map_comp, ← category.assoc],
    rw comonad.coassoc,
    simv only [category.assoc, nat_trans.naturality, functor.comp_map],
  end }
...
{ hom_equiv := λ X Y, equiv.refl (U.obj X ⟶ Y),
  hom_equiv_naturality_right' := λ X Y Z f g,
    begin unfold_projs, dsimp, erw [← category.assoc (U.map f), U.ε.naturality], dsimp,
      simv only [← category.assoc, comonad.left_counit, category.id_comp] end }
