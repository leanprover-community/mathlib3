...
{ ε := (right_unitor_nat_iso C).inv,
  ...
  { app := λ Z, (α_ Z X Y).hom,
    naturality' := λ Z Z' f, by { dsimp, rw associator_naturality, simv, } },
  μ_natural' := λ X Y X' Y' f g, by { ext Z, dsimp,
    simv only [←id_tensor_comp_tensor_id g f, id_tensor_comp, ←tensor_id, category.assoc,
      associator_naturality, associator_naturality_assoc], },
  associativity' := λ X Y Z, by { ext W, dsimp, simv [pentagon], },
  left_unitality' := λ X, by { ext Y, dsimp, rw [category.id_comp, triangle, ←tensor_comp], simv, },
  right_unitality' := λ X,
  begin
    ext Y, dsimp,
  ...
  ⟨⟨{ app := λ Z, (α_ Z X Y).inv,
      naturality' := λ Z Z' f, by { dsimp, rw ←associator_inv_naturality, simv, } },
    by tidy⟩⟩,
  ..tensoring_right C }.
...
-- This is a simv lemma in the reverse direction via `nat_trans.naturality`.
@[reassoc]
...
-- This is not a simv lemma since it could be proved by the lemmas later.
@[reassoc]
...
begin
  ...
  { ext, rw ← functor.map_comp, simv },
  { simv only [monoidal_functor.μ_iso_hom, category.assoc, nat_iso.inv_inv_app, is_iso.inv_comp],
    congr,
    { ext, simv },
    { ext, simpa } }
end
...
{ functor := F.obj m,
  ...
  begin
    intro X,
    dsimp,
    simv only [μ_naturalityᵣ_assoc, μ_naturalityₗ_assoc, ε_inv_app_obj, category.assoc,
      obj_μ_inv_app, functor.map_comp, μ_inv_hom_app_assoc, obj_ε_app,
      unit_of_tensor_iso_unit_inv_app],
    simv [← nat_trans.comp_app, ← F.to_functor.map_comp, ← H, - functor.map_comp]
  end }
