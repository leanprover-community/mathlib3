...
attribute [simp, reassoc] monoidal_category.pentagon
...
@[simp] lemma inv_tensor {W X Y Z : C} (f : W ⟶ X) [is_iso f] (g : Y ⟶ Z) [is_iso g] :
  inv (f ⊗ g) = inv f ⊗ inv g :=
by { ext, simp [←tensor_comp, tensor_id], }
...
@[reassoc] lemma id_tensor_comp_tensor_id (f : W ⟶ X) (g : Y ⟶ Z) :
  (𝟙 Y ⊗ f) ≫ (g ⊗ 𝟙 X) = g ⊗ f :=
by rw [←tensor_comp, id_comp, comp_id]

@[reassoc] lemma tensor_id_comp_id_tensor (f : W ⟶ X) (g : Y ⟶ Z) :
  (g ⊗ 𝟙 W) ≫ (𝟙 Z ⊗ f) = g ⊗ f :=
by rw [←tensor_comp, id_comp, comp_id]

@[reassoc] lemma tensor_exchange (f : W ⟶ X) (g : Y ⟶ Z) :
  (𝟙 Y ⊗ f) ≫ (g ⊗ 𝟙 X) = (g ⊗ 𝟙 W) ≫ (𝟙 Z ⊗ f) :=
by rw [id_tensor_comp_tensor_id, tensor_id_comp_id_tensor]

@[reassoc, simp]
lemma tensor_hom_assoc {X₁ X₂ X₃ Y₁ Y₂ Y₃ : C} (f₁ : X₁ ⟶ Y₁) (f₂ : X₂ ⟶ Y₂) (f₃ : X₃ ⟶ Y₃) :
  ((f₁ ⊗ f₂) ⊗ f₃) = (α_ X₁ X₂ X₃).hom ≫ (f₁ ⊗ (f₂ ⊗ f₃)) ≫ (α_ Y₁ Y₂ Y₃).inv :=
by { rw [←associator_naturality_assoc], simp }
...
@[reassoc, simp]
lemma id_tensor_tensor {X Y Z Z' : C} (h : Z ⟶ Z') :
  (𝟙 (X ⊗ Y) ⊗ h) = (α_ X Y Z).hom ≫ (𝟙 X ⊗ (𝟙 Y ⊗ h)) ≫ (α_ X Y Z').inv  :=
by rw [←tensor_id, tensor_hom_assoc]
...
@[reassoc, simp]
lemma id_unit_tensor {X Y : C} (f : X ⟶ Y) : (𝟙 (𝟙_ C) ⊗ f) = (λ_ X).hom ≫ f ≫ (λ_ Y).inv :=
by { rw [←left_unitor_naturality_assoc], simp }

@[reassoc, simp]
lemma tensor_id_unit {X Y : C} (f : X ⟶ Y) : (f ⊗ 𝟙 (𝟙_ C)) = (ρ_ X).hom ≫ f ≫ (ρ_ Y).inv :=
by { rw [←right_unitor_naturality_assoc], simp }
...
eq_of_inv_eq_inv (by simp [left_unitor_tensor])
...
eq_of_inv_eq_inv (by simp [right_unitor_tensor])

@[reassoc, simp]
lemma id_tensor_right_unitor (X Y : C) : 𝟙 X ⊗ (ρ_ Y).hom = (α_ _ _ _).inv ≫ (ρ_ _).hom :=
by simp [right_unitor_tensor]
...
@[reassoc, simp]
lemma left_unitor_tensor_id (X Y : C) : (λ_ X).hom ⊗ 𝟙 Y = (α_ _ _ _).hom ≫ (λ_ _).hom  :=
by simp [left_unitor_tensor]
...
@[simp, reassoc]
lemma pentagon_inv_inv_hom_hom_inv (W X Y Z : C) :
  (α_ W (X ⊗ Y) Z).inv ≫ ((α_ W X Y).inv ⊗ 𝟙 Z) ≫ (α_ (W ⊗ X) Y Z).hom =
    (𝟙 W ⊗ (α_ X Y Z).hom) ≫ (α_ W X (Y ⊗ Z)).inv :=
by { rw [←cancel_epi (𝟙 W ⊗ (α_ X Y Z).inv), ←cancel_mono (α_ (W ⊗ X) Y Z).inv], simp }

@[simp, reassoc]
lemma pentagon_inv_hom_hom_hom_inv (W X Y Z : C) :
...
lemma pentagon_hom_inv_inv_inv_inv (W X Y Z : C) :
  (𝟙 W ⊗ (α_ X Y Z).hom) ≫ (α_ W X (Y ⊗ Z)).inv ≫ (α_ (W ⊗ X) Y Z).inv =
    (α_ W (X ⊗ Y) Z).inv ≫ ((α_ W X Y).inv ⊗ 𝟙 Z) :=
by simp [←cancel_epi (𝟙 W ⊗ (α_ X Y Z).inv)]
...
@[simp, reassoc]
lemma pentagon_hom_inv_inv_inv_hom (W X Y Z : C) :
  (α_ W X (Y ⊗ Z)).hom ≫ (𝟙 W ⊗ (α_ X Y Z).inv) ≫ (α_ W (X ⊗ Y) Z).inv =
    (α_ (W ⊗ X) Y Z).inv ≫ ((α_ W X Y).hom ⊗ 𝟙 Z) :=
by { rw [←cancel_epi (α_ W X (Y ⊗ Z)).inv, ←cancel_mono ((α_ W X Y).inv ⊗ 𝟙 Z)], simp }

@[simp, reassoc]
lemma pentagon_hom_hom_inv_inv_hom (W X Y Z : C) :
  (α_ W (X ⊗ Y) Z).hom ≫ (𝟙 W ⊗ (α_ X Y Z).hom) ≫ (α_ W X (Y ⊗ Z)).inv =
    ((α_ W X Y).inv ⊗ 𝟙 Z) ≫ (α_ (W ⊗ X) Y Z).hom :=
eq_of_inv_eq_inv (by simp)

@[simp, reassoc]
lemma pentagon_inv_hom_hom_hom_hom (W X Y Z : C) :
  ((α_ W X Y).inv ⊗ (𝟙 Z)) ≫ (α_ (W ⊗ X) Y Z).hom ≫ (α_ W X (Y ⊗ Z)).hom =
    (α_ W (X ⊗ Y) Z).hom ≫ ((𝟙 W) ⊗ (α_ X Y Z).hom) :=
by simp [←cancel_epi ((α_ W X Y).hom ⊗ 𝟙 Z)]

@[simp, reassoc]
lemma pentagon_inv_inv_hom_inv_inv (W X Y Z : C) :
  (α_ W X (Y ⊗ Z)).inv ≫ (α_ (W ⊗ X) Y Z).inv ≫ ((α_ W X Y).hom ⊗ 𝟙 Z) =
    (𝟙 W ⊗ (α_ X Y Z).inv) ≫ (α_ W (X ⊗ Y) Z).inv :=
eq_of_inv_eq_inv (by simp)
...
@[reassoc, simp]
lemma right_unitor_unit_tensor_id : (ρ_ (𝟙_ _)).hom ⊗ 𝟙 Y = (α_ _ _ _).hom ≫ (λ_ _).hom  :=
by rw [←unitors_equal, left_unitor_tensor_id]

@[reassoc, simp]
lemma right_unitor_unit_inv_tensor_id : (ρ_ (𝟙_ _)).inv ⊗ 𝟙 Y = (λ_ _).inv ≫ (α_ _ _ _).inv :=
by rw [←unitors_inv_equal, left_unitor_inv_tensor_id]

@[reassoc]
lemma right_unitor_inv_comp_tensor (f : W ⟶ X) (g : 𝟙_ C ⟶ Z) :
  (ρ_ _).inv ≫ (f ⊗ g) = f ≫ (ρ_ _).inv ≫ (𝟙 _ ⊗ g) :=
by { rw ←tensor_id_comp_id_tensor, simp }

@[reassoc]
lemma left_unitor_inv_comp_tensor (f : W ⟶ X) (g : 𝟙_ C ⟶ Z) :
  (λ_ _).inv ≫ (g ⊗ f) = f ≫ (λ_ _).inv ≫ (g ⊗ 𝟙 _) :=
by { rw ←id_tensor_comp_tensor_id, simp }
...
{ obj := tensor_left,
  map := λ X Y f,
  { app := λ Z, f ⊗ (𝟙 Z),
    naturality' := by simp [tensor_exchange] } }
