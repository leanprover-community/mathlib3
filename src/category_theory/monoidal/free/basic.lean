...
begin
  ...
  { exact hfg'.symm },
  { exact hfg.trans hgh },
  { simv only [project_map_aux, hf, hg] },
  { simv only [project_map_aux, hfg, hfg'] },
  { simv only [project_map_aux, category.comp_id] },
  { simv only [project_map_aux, category.id_comp] },
  { simv only [project_map_aux, category.assoc ] },
  { simv only [project_map_aux, monoidal_category.tensor_id], refl },
  { simv only [project_map_aux, monoidal_category.tensor_comp] },
  { simv only [project_map_aux, iso.hom_inv_id] },
  { simv only [project_map_aux, iso.inv_hom_id] },
  { simv only [project_map_aux, monoidal_category.associator_naturality] },
  { simv only [project_map_aux, iso.hom_inv_id] },
  { simv only [project_map_aux, iso.inv_hom_id] },
  { simv only [project_map_aux], dsimp [project_obj],
    exact monoidal_category.right_unitor_naturality _ },
  { simv only [project_map_aux, iso.hom_inv_id] },
  { simv only [project_map_aux, iso.inv_hom_id] },
  { simv only [project_map_aux], dsimp [project_obj],
    exact monoidal_category.left_unitor_naturality _ },
  { simv only [project_map_aux], exact monoidal_category.pentagon _ _ _ _ },
  { simv only [project_map_aux], exact monoidal_category.triangle _ _ }
end
