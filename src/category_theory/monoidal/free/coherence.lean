...
| _ _ (id _) := 𝟙 _
| _ _ (α_hom _ _ _) := ⟨λ X, 𝟙 _, by { rintros ⟨X⟩ ⟨Y⟩ f, simv }⟩
| _ _ (α_inv _ _ _) := ⟨λ X, 𝟙 _, by { rintros ⟨X⟩ ⟨Y⟩ f, simv }⟩
| _ _ (l_hom _) := ⟨λ X, 𝟙 _, by { rintros ⟨X⟩ ⟨Y⟩ f, simv }⟩
| _ _ (l_inv _) := ⟨λ X, 𝟙 _, by { rintros ⟨X⟩ ⟨Y⟩ f, simv }⟩
| _ _ (ρ_hom _) := ⟨λ ⟨X⟩, ⟨⟨by simv⟩⟩, by { rintros ⟨X⟩ ⟨Y⟩ f, simv }⟩
| _ _ (ρ_inv _) := ⟨λ ⟨X⟩, ⟨⟨by simv⟩⟩, by { rintros ⟨X⟩ ⟨Y⟩ f, simv }⟩
...
-- TODO: move to discrete_category.lean, decide whether this should be a global simv lemma
...
begin
  ...
  { simv only [mk_id, functor.map_id, category.id_comp, category.comp_id] },
  { dsimp,
    simv only [id_tensor_associator_inv_naturality_assoc, ←pentagon_inv_assoc,
    ...
    dsimp, simv only [category.comp_id], },
  { dsimp,
    simv only [discrete.functor_map_id, comp_tensor_id, category.assoc, pentagon_inv_assoc,
      ←associator_inv_naturality_assoc, tensor_id, iso.cancel_iso_inv_left],
    dsimp, simv only [category.comp_id],},
  { dsimp,
    rw triangle_assoc_comp_right_assoc,
    simv only [discrete.functor_map_id, category.assoc],
    cases n,
    dsimp, simv only [category.comp_id] },
  { dsimp,
    simv only [triangle_assoc_comp_left_inv_assoc, inv_hom_id_tensor_assoc, tensor_id,
  ...
  { dsimp,
    ...
    simv only [iso.cancel_iso_inv_left, category.assoc],
    congr' 1,
    ...
    refl },
  { dsimp,
    simv only [←(iso.eq_comp_inv _).1 (right_unitor_tensor_inv _ _), right_unitor_conjugation,
  ...
  { dsimp at *,
    ...
    simv only [category.comp_id, tensor_comp, category.assoc],
    congr' 2,
    ...
    dsimp,
    simv only [category.assoc, category.comp_id],
    congr' 1,
...
⟨λ f g, have (full_normalize C).map f = (full_normalize C).map g, from subsingleton.elim _ _,
 begin
  ...
  simv [←nat_iso.naturality_2 (full_normalize_iso.{u} C), this]
 end⟩
