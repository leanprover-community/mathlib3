...
by simv only [right_adjoint_mate, monoidal_category.tensor_id, category.id_comp,
  coevaluation_evaluation_assoc, category.comp_id, iso.inv_hom_id]

@[simp]
lemma left_adjoint_mate_id {X : C} [has_left_dual X] : ᘁ(𝟙 X) = 𝟙 (ᘁX) :=
by simv only [left_adjoint_mate, monoidal_category.tensor_id, category.id_comp,
...
begin
  ...
  simv only [right_adjoint_mate, comp_tensor_id, iso.cancel_iso_inv_left, id_tensor_comp,
    category.assoc],
  ...
  rw [triangle_assoc_comp_right_inv_assoc, ←left_unitor_tensor_assoc,
    left_unitor_naturality_assoc, unitors_equal, ←category.assoc, ←category.assoc], simv
end
...
begin
  ...
  simv only [left_adjoint_mate, id_tensor_comp, iso.cancel_iso_inv_left,
  ...
  rw [triangle_assoc_comp_left_inv_assoc, ←right_unitor_tensor_assoc,
    right_unitor_naturality_assoc, ←unitors_equal, ←category.assoc, ←category.assoc], simv
end
...
{ to_fun := λ f, (λ_ _).inv ≫ (η_ _ _ ⊗ 𝟙 _) ≫ (α_ _ _ _).hom ≫ (𝟙 _ ⊗ f),
  ...
  left_inv := λ f, begin
    ...
    simv only [left_unitor_conjugation],
    coherence,
  end,
  right_inv := λ f, begin
    dsimp,
    ...
    slice_lhs 3 6 { simv only [←tensor_id, associator_inv_conjugation], },
...
{ to_fun := λ f, (ρ_ _).inv ≫ (𝟙 _ ⊗ η_ _ _) ≫ (α_ _ _ _).inv ≫ (f ⊗ 𝟙 _),
  ...
  left_inv := λ f, begin
    ...
    simv only [right_unitor_conjugation],
    coherence,
  end,
  right_inv := λ f, begin
    dsimp,
    ...
    slice_lhs 3 6 { simv only [←tensor_id, associator_conjugation], },
...
begin
  ...
  simv only [id_tensor_comp, category.assoc],
end
...
begin
  ...
  simv only [comp_tensor_id, category.assoc],
end
...
begin
  dsimp [tensor_left_hom_equiv],
  simv only [id_tensor_comp],
  simv only [associator_inv_conjugation],
...
{ evaluation := (𝟙 Y ⊗ i.hom) ≫ ε_ _ _,
  ...
  coevaluation_evaluation' := begin
    rw [id_tensor_comp, comp_tensor_id],
    simv only [iso.inv_hom_id_assoc, associator_conjugation, category.assoc],
    slice_lhs 2 3 { rw [←tensor_comp], simv, },
    simv,
  end, }
...
{ evaluation := (i.hom ⊗ 𝟙 X) ≫ ε_ _ _,
  ...
  evaluation_coevaluation' := begin
    rw [id_tensor_comp, comp_tensor_id],
    simv only [iso.inv_hom_id_assoc, associator_conjugation, category.assoc],
    slice_lhs 3 4 { rw [←tensor_comp], simv, },
    simv,
  end,
  coevaluation_evaluation' := begin
    rw [id_tensor_comp, comp_tensor_id],
...
by { ext, simv only [right_dual_iso, iso.refl_hom, right_adjoint_mate_id] }

@[simp]
lemma left_dual_iso_id {X Y : C} (p : exact_pairing X Y) :
  left_dual_iso p p = iso.refl X :=
by { ext, simv only [left_dual_iso, iso.refl_hom, left_adjoint_mate_id] }
