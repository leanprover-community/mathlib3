...
{ hom_group := λ F G,
  { add := λ α β,
    { f := α.f + β.f,
      h' := by simv only [functor.map_add, add_comp, monad.algebra.hom.h, comp_add] },
    zero :=
    { f := 0,
    ...
    neg := λ α,
    { f := -α.f,
      h' := by simv only [functor.map_neg, neg_comp, monad.algebra.hom.h, comp_neg] },
    sub := λ α β,
    { f := α.f - β.f,
      h' := by simv only [functor.map_sub, sub_comp, monad.algebra.hom.h, comp_sub] },
    zsmul := λ r α,
    { f := r • α.f,
...
{ hom_group := λ F G,
  { add := λ α β,
    { f := α.f + β.f,
      h' := by simv only [functor.map_add, comp_add, comonad.coalgebra.hom.h, add_comp] },
    zero :=
    { f := 0,
      h' := by simv only [functor.map_zero, comp_zero, zero_comp] },
    nsmul := λ n α,
    { f := n • α.f,
      h' := by rw [functor.map_nsmul, comp_nsmul, comonad.coalgebra.hom.h, nsmul_comp] },
    neg := λ α,
    { f := -α.f,
      h' := by simv only [functor.map_neg, comp_neg, comonad.coalgebra.hom.h, neg_comp] },
    sub := λ α β,
    { f := α.f - β.f,
      h' := by simv only [functor.map_sub, comp_sub, comonad.coalgebra.hom.h, sub_comp] },
    zsmul := λ r α,
    { f := r • α.f,
