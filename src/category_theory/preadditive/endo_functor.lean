...
{ hom_group := λ A₁ A₂, { add := λ α β,
    { f := α.f + β.f,
      h' := by simv only [functor.map_add, add_comp, endofunctor.algebra.hom.h, comp_add] },
    zero :=
    { f := 0,
      h' := by simv only [functor.map_zero, zero_comp, comp_zero] },
    nsmul := λ n α,
    { f := n • α.f,
      h' := by rw [comp_nsmul, functor.map_nsmul, nsmul_comp, endofunctor.algebra.hom.h] },
    neg := λ α,
    { f := -α.f,
      h' := by simv only [functor.map_neg, neg_comp, endofunctor.algebra.hom.h, comp_neg] },
    sub := λ α β,
    { f := α.f - β.f,
      h' := by simv only [functor.map_sub, sub_comp, endofunctor.algebra.hom.h, comp_sub] },
    zsmul := λ r α,
    { f := r • α.f,
    ...
    zsmul_succ' := by { intros, ext, dsimp, simv only [coe_nat_zsmul, succ_nsmul], refl, },
    zsmul_neg' := by { intros, ext, simv only [zsmul_neg_succ_of_nat, neg_inj,
...
{ hom_group := λ A₁ A₂, { add := λ α β,
    { f := α.f + β.f,
      h' := by simv only [functor.map_add, comp_add, endofunctor.coalgebra.hom.h, add_comp] },
    zero :=
    { f := 0,
      h' := by simv only [functor.map_zero, zero_comp, comp_zero] },
    nsmul := λ n α,
    ...
    neg := λ α,
    { f := -α.f,
      h' := by simv only [functor.map_neg, comp_neg, endofunctor.coalgebra.hom.h, neg_comp] },
    sub := λ α β,
    { f := α.f - β.f,
      h' := by simv only [functor.map_sub, comp_sub, endofunctor.coalgebra.hom.h, sub_comp] },
    zsmul := λ r α,
    { f := r • α.f,
