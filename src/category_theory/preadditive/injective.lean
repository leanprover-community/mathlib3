...
{ factors := λ X Y g f mono, begin
  resetI,
  refine ⟨(@projective.factor_thru Cᵒᵖ _ P (op X) (op Y) _ g.op f.op _).unop, _⟩,
  convert congr_arg quiver.hom.unop (@projective.factor_thru_comp Cᵒᵖ _ P
    (op X) (op Y) _ g.op f.op _),
end }

instance {J : Cᵒᵖ} [injective J] : projective (unop J) :=
{ factors := λ E X f e epi, begin
  resetI,
  refine ⟨(@factor_thru Cᵒᵖ _ J _ _ _ f.op e.op _).unop, _⟩,
  convert congr_arg quiver.hom.unop (@comp_factor_thru Cᵒᵖ _ J _ _ _ f.op e.op _),
end }
...
{ factors := λ X Y g f mono, begin
  resetI,
  refine ⟨(@projective.factor_thru C _ P (unop X) (unop Y) _ g.unop f.unop _).op, _⟩,
  convert congr_arg quiver.hom.op (@projective.factor_thru_comp C _ P
    (unop X) (unop Y) _ g.unop f.unop _),
end }
