...
/-- `f +ₗ g` is the composite `X ⟶ Y ⊞ Y ⟶ Y`, where the first map is `(f, g)` and the second map
    is `(𝟙 𝟙)`. -/
@[simp] def left_addition (f g : X ⟶ Y) : X ⟶ Y :=
biprod.lift f g ≫ biprod.desc (𝟙 Y) (𝟙 Y)

/-- `f +ᵣ g` is the composite `X ⟶ X ⊞ X ⟶ Y`, where the first map is `(𝟙, 𝟙)` and the second map
    is `(f g)`. -/
@[simp] def right_addition (f g : X ⟶ Y) : X ⟶ Y :=
biprod.lift (𝟙 X) (𝟙 X) ≫ biprod.desc f g

local infixr ` +ₗ `:65 := left_addition X Y
local infixr ` +ᵣ `:65 := right_addition X Y

lemma is_unital_left : is_unital (+ₗ) 0 :=
...
lemma is_unital_right : is_unital (+ᵣ) 0 :=
...
begin
  ...
  rw [left_addition, h₁, category.assoc, h₂, right_addition]
end
...
{ add := (+ᵣ),
  add_assoc := (mul_assoc (is_unital_left X Y) (is_unital_right X Y) (distrib X Y)).assoc,
  zero := 0,
  zero_add := (is_unital_right X Y).left_id,
  add_zero := (is_unital_right X Y).right_id,
  add_comm := (mul_comm (is_unital_left X Y) (is_unital_right X Y) (distrib  X Y)).comm }
...
congr_fun₂ (mul (is_unital_left X Y) (is_unital_right X Y) (distrib  X Y)).symm f g
