...
-/
@[simps id, simps {not_recursive := []}] -- the second call generates simp lemmas like `id_fst`
instance prod : category.{max v₁ v₂} (C × D) :=
{ hom     := λ X Y, ((X.1) ⟶ (Y.1)) × ((X.2) ⟶ (Y.2)),
  id      := λ X, ⟨ 𝟙 (X.1), 𝟙 (X.2) ⟩,
  comp    := λ _ _ _ f g, (f.1 ≫ g.1, f.2 ≫ g.2) }

/-- One rfl lemma that cannot be generated by `@[simps]`. It is important that the domain and
  codomain are pairs, and not arbitrary elements of `C × D`. -/
