...
(λ W, begin
  ...
  ext,
  dsimp,
  simv only [functor.map_cone_π_app, multiequalizer.multifork_π_app_left,
...
(λ X, begin
  ...
  dsimp,
  simv only [has_colimit.iso_of_nat_iso_ι_hom_assoc,
...
begin
  ...
  intros W,
  dsimp [plus_obj, plus_map],
  simv only [ι_plus_comp_iso_hom, ι_colim_map, whisker_left_app, ι_plus_comp_iso_hom_assoc,
...
begin
  ...
  intros W,
  dsimp [plus_obj, plus_map],
  simv only [ι_colim_map, whisker_right_app, ι_plus_comp_iso_hom_assoc,
    grothendieck_topology.diagram_nat_trans_app],
  simv only [← category.assoc, ← F.map_comp],
  dsimp [colim_map, is_colimit.map],
  simv only [colimit.ι_desc],
  dsimp [cocones.precompose],
  simv only [functor.map_comp, category.assoc, ι_plus_comp_iso_hom],
  simv only [← category.assoc],
  congr' 1,
  ext,
  dsimp,
  simv only [diagram_comp_iso_hom_ι_assoc, multiequalizer.lift_ι,
...
begin
  ext,
  dsimp [to_plus],
  simv only [ι_plus_comp_iso_hom, functor.map_comp, category.assoc],
  simv only [← category.assoc],
  congr' 1,
  ext,
  delta cover.to_multiequalizer,
  simv only [diagram_comp_iso_hom_ι, category.assoc, ← F.map_comp],
...
by { apply J.plus_lift_unique, simv [iso.comp_inv_eq] }
