...
class representibly_flat (u : C ⥤ D) :=
(cofiltered : ∀ (X : D), is_cofiltered_or_empty (costructured_arrow u X))

def functor.flat_cofiltered (u : C ⥤ D) [representibly_flat u] (X : D) :
 is_cofiltered_or_empty (costructured_arrow u X) := representibly_flat.cofiltered X

variables (u : C ⥤ D) [representibly_flat u] {X : D} (Y₁ Y₂ : costructured_arrow u X)
...
lemma compatible.functor_pushforward {C : Type*} {D : Type*} [category.{v} C] [category.{v} D]
  {u : C ⥤ D} [representibly_flat u] {P : Dᵒᵖ ⥤ Type _} {Z : C} {T : presieve Z}
...
begin
  ...
  let f₁' := costructured_arrow.mk (u.map (functor_pushforward_prehom h₁)),
  let f₂' := costructured_arrow.mk (u.map (functor_pushforward_prehom h₂)),
  have := h (u.flat_min_left f₁' f₂').left (u.flat_min_right f₁' f₂').left
