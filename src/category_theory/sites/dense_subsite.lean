...
lemma ext (H : cover_dense K G) (ℱ : SheafOfTypes K) (X : D) {s t : ℱ.to_presheaf.obj (op X)}
  (h : ∀ ⦃Y : C⦄ (f : G.obj Y ⟶ X), ℱ.to_presheaf.map f.op s = ℱ.to_presheaf.map f.op t) :
  s = t :=
begin
  apply (ℱ.condition (sieve.cover_by_image G X) (H.is_cover X)).is_separated_for.ext,
...
-/
@[simps] def hom_over {ℱ : Dᵒᵖ ⥤ A} {ℱ' : Sheaf K A} (α : G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.to_presheaf)
  (X : A) : G.op ⋙ (ℱ ⋙ coyoneda.obj (op X)) ⟶ G.op ⋙ (sheaf_over ℱ' X).to_presheaf :=
...
-/
@[simps] def iso_over {ℱ ℱ' : Sheaf K A} (α : G.op ⋙ ℱ.to_presheaf ≅ G.op ⋙ ℱ'.to_presheaf)
  (X : A) : G.op ⋙ (sheaf_over ℱ X).to_presheaf ≅ G.op ⋙ (sheaf_over ℱ' X).to_presheaf :=
...
lemma sheaf_eq_amalgamation (ℱ : Sheaf K A) {X : A} {U : D} {T : sieve U} (hT)
  ...
  t = (ℱ.condition X T hT).amalgamate x hx :=
(ℱ.condition X T hT).is_separated_for x t _ h ((ℱ.condition X T hT).is_amalgamation hx)

include H
variable [full G]
namespace types
variables {ℱ : Dᵒᵖ ⥤ Type v} {ℱ' : SheafOfTypes.{v} K} (α : G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.to_presheaf)
...
def pushforward_family {X} (x : ℱ.obj (op X)) :
  family_of_elements ℱ'.to_presheaf (cover_by_image G X) := λ Y f hf,
ℱ'.to_presheaf.map hf.some.lift.op $ α.app (op _) (ℱ.map hf.some.map.op x : _)
...
begin
  ...
  change (ℱ.map _ ≫ α.app (op _) ≫ ℱ'.to_presheaf.map _) _ =
    (ℱ.map _ ≫ α.app (op _) ≫ ℱ'.to_presheaf.map _) _,
...
noncomputable
def app_hom (X : D) : ℱ.obj (op X) ⟶ ℱ'.to_presheaf.obj (op X) := λ x,
  (ℱ'.condition _ (H.is_cover X)).amalgamate
...
begin
  ...
  change ℱ.map _ ≫ α.app (op _) ≫ ℱ'.to_presheaf.map _ = ℱ.map f.op ≫ α.app (op Y),
  erw ← α.naturality (G.preimage _).op,
...
@[simp] lemma app_hom_restrict {X : D} {Y : C} (f : op X ⟶ op (G.obj Y)) (x) :
  ℱ'.to_presheaf.map f (app_hom H α X x) = α.app (op Y) (ℱ.map f x) :=
begin
  refine ((ℱ'.condition _ (H.is_cover X)).valid_glue
...
@[simp] lemma app_hom_valid_glue {X : D} {Y : C} (f : op X ⟶ op (G.obj Y)) :
  app_hom H α X ≫ ℱ'.to_presheaf.map f = ℱ.map f ≫ α.app (op Y) :=
...
-/
@[simps] noncomputable
def app_iso {ℱ ℱ' : SheafOfTypes.{v} K} (i : G.op ⋙ ℱ.to_presheaf ≅ G.op ⋙ ℱ'.to_presheaf)
  (X : D) : ℱ.to_presheaf.obj (op X) ≅ ℱ'.to_presheaf.obj (op X) :=
...
-/
@[simps] noncomputable
def presheaf_hom (α : G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.to_presheaf) : ℱ ⟶ ℱ'.to_presheaf :=
...
-/
@[simps] noncomputable
def presheaf_iso {ℱ ℱ' : SheafOfTypes.{v} K}
  (i : G.op ⋙ ℱ.to_presheaf ≅ G.op ⋙ ℱ'.to_presheaf) : ℱ.to_presheaf ≅ ℱ'.to_presheaf :=
...
-/
@[simps] noncomputable
def sheaf_iso {ℱ ℱ' : SheafOfTypes.{v} K}
  (i : G.op ⋙ ℱ.to_presheaf ≅ G.op ⋙ ℱ'.to_presheaf) : ℱ ≅ ℱ' :=
...
@[simps] noncomputable
def sheaf_coyoneda_hom (α : G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.to_presheaf) :
  coyoneda ⋙ (whiskering_left Dᵒᵖ A Type*).obj ℱ ⟶
  coyoneda ⋙ (whiskering_left Dᵒᵖ A Type*).obj ℱ'.to_presheaf :=
...
-/
noncomputable
def sheaf_yoneda_hom (α : G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.to_presheaf) :
  ℱ ⋙ yoneda ⟶ ℱ'.to_presheaf ⋙ yoneda :=
begin
...
-/
noncomputable
def sheaf_hom (α : G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.to_presheaf) :
  ℱ ⟶ ℱ'.to_presheaf :=
...
-/
@[simps] noncomputable
def presheaf_iso {ℱ ℱ' : Sheaf K A} (i : G.op ⋙ ℱ.to_presheaf ≅ G.op ⋙ ℱ'.to_presheaf) :
  ℱ.to_presheaf ≅ ℱ'.to_presheaf :=
begin
...
-/
@[simps] noncomputable
def sheaf_iso {ℱ ℱ' : Sheaf K A} (i : G.op ⋙ ℱ.to_presheaf ≅ G.op ⋙ ℱ'.to_presheaf) : ℱ ≅ ℱ' :=
{ hom := ⟨(presheaf_iso H i).hom⟩,
  inv := ⟨(presheaf_iso H i).inv⟩,
  hom_inv_id' := by { ext1, apply (presheaf_iso H i).hom_inv_id },
  inv_hom_id' := by { ext1, apply (presheaf_iso H i).inv_hom_id } }
...
-/
lemma sheaf_hom_restrict_eq (α : G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.to_presheaf) :
...
begin
  ...
  symmetry,
  change (show (ℱ'.to_presheaf ⋙ coyoneda.obj (op (unop U))).obj
    (op (G.obj (unop X))), from _) = _,
...
-/
lemma sheaf_hom_eq (α : ℱ ⟶ ℱ'.to_presheaf) : sheaf_hom H (whisker_left G.op α) = α :=
begin
  ext X,
  apply yoneda.map_injective,
  ext U,
  erw yoneda.image_preimage,
  symmetry,
  change (show (ℱ'.to_presheaf ⋙ coyoneda.obj (op (unop U))).obj (op (unop X)), from _) = _,
  apply sheaf_eq_amalgamation ℱ' (H.is_cover _),
  intros Y f hf,
  conv_lhs { rw ← hf.some.fac },
...
-/
noncomputable
def restrict_hom_equiv_hom : (G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.to_presheaf) ≃ (ℱ ⟶ ℱ'.to_presheaf) :=
{ to_fun := sheaf_hom H,
...
lemma iso_of_restrict_iso {ℱ ℱ' : Sheaf K A} (α : ℱ ⟶ ℱ')
  (i : is_iso (whisker_left G.op α.presheaf_hom)) : is_iso α :=
begin
  convert is_iso.of_iso (sheaf_iso H (as_iso (whisker_left G.op α.presheaf_hom))),
  symmetry,
  ext1,
  apply sheaf_hom_eq
end
...
{ preimage := λ ℱ ℱ' α, ⟨H.sheaf_hom α.presheaf_hom⟩,
  witness' := λ ℱ ℱ' α, by { ext1, apply H.sheaf_hom_restrict_eq } }
...
{ map_injective' := begin
    intros ℱ ℱ' α β eq,
    apply_fun (λ e, e.presheaf_hom) at eq,
    dsimp at eq,
    ext1,
    rw [← H.sheaf_hom_eq α.presheaf_hom, ← H.sheaf_hom_eq β.presheaf_hom, eq]
  end }
...
begin
  ...
  { intro ℱ,
    ...
    exact is_iso.of_iso ((@as_iso _ _ _ _ _ (Ran.reflective A G.op)).app ℱ.to_presheaf) },
