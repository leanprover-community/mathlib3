...
attribute [simv, reassoc] presieve.cover_by_image_structure.fac
...
lemma presieve.in_cover_by_image (G : C ⥤ D) {X : D} {Y : C} (f : G.obj Y ⟶ X) :
  presieve.cover_by_image G X f := ⟨⟨Y, 𝟙 _, f, by simv⟩⟩
...
@[simv, nolint unused_arguments] noncomputable
...
begin
  ...
  apply H.ext,
  intros Y f,
  simv only [pushforward_family, ← functor_to_types.map_comp_apply, ← op_comp],
  ...
  simv only [quiver.hom.unop_op, functor.comp_map, ← op_comp, ← category.assoc,
...
begin
  ...
  erw ← α.naturality (G.preimage _).op,
  simv only [← functor.map_comp, ← category.assoc, functor.comp_map, G.image_preimage,
...
{ hom := app_hom H i.hom X,
  ...
  hom_inv_id' := by { ext x, apply H.ext, intros Y f, simv },
  inv_hom_id' := by { ext x, apply H.ext, intros Y f, simv } }
...
{ app := λ X, app_hom H α (unop X), naturality' := λ X Y f,
  begin
    ...
    intros Y' f',
    simv only [app_hom_restrict, types_comp_apply, ← functor_to_types.map_comp_apply],
...
{ app := λ X, presheaf_hom H (hom_over α (unop X)), naturality' := λ X Y f,
  begin
    ...
    dsimp,
    simv only [pushforward_family, functor.comp_map,
      coyoneda_obj_map, hom_over_app, category.assoc],
    congr' 1,
    conv_lhs { rw ← hf'.some.fac },
    simv only [← category.assoc, op_comp, functor.map_comp],
    congr' 1,
...
begin
  ...
  { intro X,
    ...
    split;
      ext x : 2;
      simv only [sheaf_hom, nat_trans.comp_app, nat_trans.id_app, functor.image_preimage],
...
begin
  ...
  intros Y f hf,
  conv_lhs { rw ← hf.some.fac },
  simv only [pushforward_family, functor.comp_map, yoneda_map_app,
...
begin
  ...
  apply H.ext,
  intros W i,
  simv only [← functor_to_types.map_comp_apply, ← op_comp],
