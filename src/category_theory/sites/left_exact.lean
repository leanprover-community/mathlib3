...
{ lift := λ E, lift_to_diagram_limit_obj F E,
  fac' := begin
    ...
    dsimp [diagram_nat_trans],
    ext1,
    simv only [multiequalizer.lift_ι, multiequalizer.lift_ι_assoc, category.assoc],
    change (_ ≫ _) ≫ _ = _,
    dsimp [evaluate_combined_cones],
...
begin
  ...
  ext,
  dsimp [plus_map],
  simv only [has_colimit.iso_of_nat_iso_ι_hom_assoc, ι_colim_map],
...
begin
  ...
  { intros S m hm,
    ...
    ext,
    simv only [limit.lift_π, category.assoc, ← hm],
    congr' 1,
    ext,
    ...
    simv only [category.assoc],
    rw ι_colimit_limit_iso_limit_π_assoc,
    simv only [nat_iso.of_components_inv_app,
