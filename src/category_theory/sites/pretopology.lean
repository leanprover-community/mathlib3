...
def family_to_set_over (U : C) : family_with_target U → set (over U) :=
begin
  intros R f,
  apply ∃ (i : R.ι), over.mk (R.hom i) = f,
end
...
@[simps]
def pullback [has_pullbacks C] {A B : C} (f : A ⟶ B) : over B ⥤ over A :=
{ obj := λ g, over.mk (snd : pullback g.hom f ⟶ A),
  map := λ g h k, over.hom_mk (lift (fst ≫ k.left) snd (by simp [condition])) (by tidy)}
