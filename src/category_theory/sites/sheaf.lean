...
-- generalize to presieves and sieve generated by the presieve
-- separated iff subsingleton (at most one morphism)
...
{ to_fun := λ π, ⟨λ Y f h, π.app (op ⟨over.mk f, h⟩), λ Y Z f g hf, by
    { apply (id_comp _).symm.trans, dsimp,
...
begin
  ...
  { intros h c, let eqv := cones_equiv_sieve_compatible_family P S (op c.X),
    have : c = ⟨c.X, c.π⟩, { cases c, refl }, rw [this, ← eqv.left_inv c.π],
    erw (hom_equiv_amalgamation (eqv.to_fun c.π).2).unique_congr.nonempty_congr,
    apply (unique_subtype_iff_exists_unique _).2 (h _ _ (eqv.to_fun _).2) },
end
