...
variables {X : C} (S : sieve X) (P : Cᵒᵖ ⥤ Type v) (J : grothendieck_topology C)

def first_obj : Type v :=
∏ (λ (f : Σ Y, {f : Y ⟶ X // S f}), P.obj (opposite.op f.1))
...
def first_obj_eq_family : first_obj S P ≅ grothendieck_topology.family_of_elements P S :=
{ hom := λ t Y f hf, pi.π (λ (f : Σ Y, {f : Y ⟶ X // S f}), P.obj (opposite.op f.1)) ⟨_, _, hf⟩ t,
