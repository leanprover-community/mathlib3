...
@[simps] noncomputable def compatible_equiv_generate_sieve_compatible :
  ...
  {x : family_of_elements P (generate R) // x.sieve_compatible} :=
{ to_fun := λ x, ⟨λ Y f hf, P.map hf.some_spec.some.op (x.1 _ hf.some_spec.some_spec.some_spec.1),
    λ Y Z f g hf, by { change _ = (P.map hf.some_spec.some.op ≫ P.map g.op) _,
      rw [← P.map_comp, ← op_comp], apply x.2,
      simp [hf.some_spec.some_spec.some_spec.2,
        ((generate R).downward_closed hf g).some_spec.some_spec.some_spec.2] }⟩,
  inv_fun := λ x, ⟨λ Y f h, x.1 f (le_generate R Y h),
    λ Y, by { intros, apply (compatible_iff_sieve_compatible x.1).2 x.2, assumption } ⟩,
  left_inv := λ x, by { ext Y f h, apply (x.2 _ (𝟙 _) _ h _).trans, simp,
    simp [(le_generate R Y h).some_spec.some_spec.some_spec.2] },
  right_inv := λ x, by { ext Y f h, apply (x.2 _ _ _).symm.trans,
    congr, exact h.some_spec.some_spec.some_spec.2 } }
