...
@[simp] lemma sheafify_map_id (P : Cᵒᵖ ⥤ D) : J.sheafify_map (𝟙 P) = 𝟙 _ :=
by { dsimp only [sheafify_map, sheafify], simp_rw J.plus_map_id }

@[simp, reassoc] lemma sheafify_map_comp {P Q R : Cᵒᵖ ⥤ D} (η : P ⟶ Q) (γ : Q ⟶ R) :
  J.sheafify_map (η ≫ γ) = J.sheafify_map η ≫ J.sheafify_map γ :=
by { dsimp only [sheafify_map, sheafify], simp_rw J.plus_map_comp }

@[simp, reassoc]
lemma to_sheafify_naturality {P Q : Cᵒᵖ ⥤ D} (η : P ⟶ Q) :
  η ≫ J.to_sheafify Q = J.to_sheafify P ≫ J.sheafify_map η :=
begin
  dsimp only [sheafify_map, to_sheafify],
  simp_rw [← category.assoc, J.to_plus_naturality, category.assoc,
    ← J.plus_map_comp, J.to_plus_naturality]
end
...
lemma sheafification_map {P Q : Cᵒᵖ ⥤ D} (η : P ⟶ Q) :
  (J.sheafification D).map η = J.sheafify_map η := rfl
...
begin
  apply J.sheafify_lift_unique,
  erw [← category.assoc, ← J.to_sheafify_naturality, category.assoc, to_sheafify_sheafify_lift],
end
...
@[simps hom_equiv_apply hom_equiv_symm_apply_presheaf_hom unit_app counit_app_presheaf_hom]
