...
by { dsimp [factor_thru_kernel_subobject], simv, }
...
(cancel_mono (kernel.ι f)).1 $ by simv
...
lemma kernel_subobject_map_arrow (sq : arrow.mk f ⟶ arrow.mk f') :
  kernel_subobject_map sq ≫ (kernel_subobject f').arrow =
    (kernel_subobject f).arrow ≫ sq.left :=
by simv [kernel_subobject_map]

@[simp] lemma kernel_subobject_map_id : kernel_subobject_map (𝟙 (arrow.mk f)) = 𝟙 _ :=
by { ext, simv, dsimp, simv, } -- See library note [dsimp, simv].
...
subobject.le_mk_of_comm (kernel.lift f A.arrow h) (by simv)
...
by { simv [kernel_subobject_iso_comp], }
...
le_kernel_subobject _ _ (by simv)
...
le_antisymm
  (le_kernel_subobject _ _ ((cancel_mono h).mp (by simv)))
...
zero_of_epi_comp (factor_thru_image_subobject f) $ by simv [h]

lemma image_subobject_factors_comp_self {W : C} (k : W ⟶ X)  :
  (image_subobject f).factors (k ≫ f) :=
⟨k ≫ factor_thru_image f, by simv⟩
...
subobject.mk_le_mk_of_comm (image.pre_comp h f) (by simv)
...
by { rw ←image_subobject_arrow, simv, }

@[simp]
lemma image_subobject_zero {A B : C} :
  image_subobject (0 : A ⟶ B) = ⊥ :=
subobject.eq_of_comm
  (image_subobject_iso _ ≪≫ image_zero ≪≫ subobject.bot_coe_iso_zero.symm) (by simv)
...
eq_of_comm (image_subobject_iso f ≪≫ image_mono_iso_source f ≪≫ (underlying_iso f).symm) (by simv)
...
le_antisymm
  ...
  (subobject.mk_le_mk_of_comm (inv (image.pre_comp h f)) (by simv))
...
image_subobject_le f (h ≫ (subobject.underlying_iso g).inv) (by simv [w])
...
begin
  simv only [image_subobject_map, category.assoc, image_subobject_arrow'],
