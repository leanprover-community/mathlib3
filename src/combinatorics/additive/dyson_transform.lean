...
* `finset.mul_transform₁`/`finset.mul_transform₂`: Replaces `(s, t)` by
  `(s ∩ op e • s, t ∪ e⁻¹ • t)` and `(s ∪ op e • s, t ∩ e⁻¹ • t)`. Preserves the sum of the
  cardinalities (see `finset.mul_transform.card`).
...
@[to_additive, simps] def mul_e_transform : finset α × finset α := (x.1 ∪ e • x.2, x.2 ∩ e⁻¹ • x.1)
