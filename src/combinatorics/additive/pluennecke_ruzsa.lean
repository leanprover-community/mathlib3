...
begin
  ...
  refine le_trans _ (mul_le_mul (hUA _ hB') (cast_le.2 $ card_le_of_subset $
    mul_subset_mul_right hU.2) (cast_nonneg _) $ by positivity),
...
lemma card_mul_pow_le (hAB : ∀ A' ⊆ A, (A * B).card * A'.card ≤ (A' * B).card * A.card) (n : ℕ) :
  ((A * B ^ n).card : ℚ) ≤ ((A * B).card / A.card) ^ n * A.card :=
begin
  obtain rfl | hA := A.eq_empty_or_nonempty,
  { simp },
  induction n with n ih,
  { simp },
  rw [pow_succ, ←mul_assoc, pow_succ, @mul_assoc ℚ, ←mul_div_right_comm, le_div_iff, ←cast_mul],
  swap, exact (cast_pos.2 hA.card_pos),
...
lemma card_pow_div_pow_le (hA : A.nonempty) (B : finset α) (m n : ℕ) :
  ((B ^ m / B ^ n).card : ℚ) ≤ ((A * B).card / A.card) ^ (m + n) * A.card :=
begin
  ...
  refine (mul_le_mul (card_mul_pow_le (mul_aux hC.1 hC.2 hCA) _)
    (card_mul_pow_le (mul_aux hC.1 hC.2 hCA) _) (cast_nonneg _) $ by positivity).trans _,
...
lemma card_pow_le (hA : A.nonempty) (B : finset α) (n : ℕ) :
  ((B ^ n).card : ℚ) ≤ ((A * B).card / A.card) ^ n * A.card :=
