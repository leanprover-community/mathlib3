...
open finset probability_theory measure_theory measure_theory.measure
...
lemma cond_count_apply_finset {α} (s : finset α) :
  cond_count (s : set α) =
...
| (p + 1) (q + 1) h :=
  begin
    ...
    rw [← cond_count_disjoint_union (finite_to_set _) (finite_to_set _) (disjoint_bits' _ _),
      ← counted_succ_succ', cond_count, cond_count, cond_count,
      cond_apply _ (set.finite.measurable_set $ finite_to_set _),
      cond_apply _ (set.finite.measurable_set $ finite_to_set _),
      cond_apply _ (set.finite.measurable_set $ finite_to_set _)],
    -- , coe_map, function.embedding.coe_fn_mk, measurable_space.measurable_set_top,
