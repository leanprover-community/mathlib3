...
begin
  simp only [compress], split_ifs,
    suffices: U = ∅,
      rw [this, union_empty, union_empty, _root_.sdiff_idem],
...
/-- Compressing a set doesn't change its size, so compressing a family keeps all sets the same size.
-/
lemma compress_family_sized {r : ℕ} {U V : finset α}
...
shadow of `𝒜`. -/
theorem compression_reduces_shadow {U V : finset α}
  (h₁ : ∀ x ∈ U, ∃ y ∈ V, is_compressed (erase U x) (erase V y) 𝒜)
