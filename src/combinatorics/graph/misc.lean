...
lemma fin.fin_two_eq_or_eq_add_one (u v : fin 2):
  u = v ∨ u = v + 1 :=
begin
  refine (fin.forall_fin_two.mpr ⟨_,_⟩ u), 
  all_goals{refine (fin.forall_fin_two.mpr ⟨_,_⟩ v)},
  all_goals{tauto}, 
end 

lemma fin.zero_or_one_of_fin_two (u : fin 2):
  u = 0 ∨ u = 1 :=
...
begin
  simp_rw add_comm, 
  exact ⟨φ 0, fin.forall_fin_two.mpr ⟨by {rw add_zero}, or.elim (fin.fin_two_eq_or_eq_add_one (φ 1) (φ 0)) (λ h, false.elim (fin.zero_ne_one ((φ.injective h).symm))) id⟩⟩,
end 
