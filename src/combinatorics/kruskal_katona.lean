...
section
  ...
  begin
    ...
    have ne₂: filter (λ A, compress U V A ∉ 𝒜) 𝒜 ≠ ∅, intro z,
    ...
    conv_rhs {rw ← uA}, rw [sum_union, add_comm, add_lt_add_iff_left, sum_image],
        apply sum_lt_sum ne₂, intros A hA, rw [binary_iff, colex_hom_fin],
  ...
  begin
    ...
    { rw compress, split_ifs,
      rw [union_sdiff_self_eq_union, union_sdiff_distrib_right,
    ...
    { refine ⟨hU, hV, disj, _, smaller⟩,
      ...
      rwa [card_disjoint_union (sdiff_inter_inter _ _),
           card_disjoint_union (sdiff_inter_inter _ _),
