...
def non_member_section (a : α) (𝒜 : finset (finset α)) : finset (finset α) := 𝒜.filter $ λ s, a ∉ s
...
begin
  rw [compression, card_disj_union, image_filter, card_image_of_inj_on ((erase_inj_on' _).mono $ λ s hs, _),
