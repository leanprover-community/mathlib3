...
begin
  ...
  refine (finset.card_congr (λ w hw, ((f w).head $ not_is_nil_of_ne _).edge) (λ a ha, _) _ _).symm,
  { simpa using hw },
  ...
  { simp only [finset.mem_compl, finset.mem_singleton],
    ...
    cases h,
    { exact congr_arg dart.fst h },
    have h1 : ((f a).head $ not_is_nil_of_ne ha).snd = b := congr_arg dart.snd h,
  ...
  refine ⟨y, _, dart_edge_eq_mk_iff'.2 $ or.inr ⟨rfl, _⟩⟩,
  { rintro rfl,
