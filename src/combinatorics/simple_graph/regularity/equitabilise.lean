...
begin
  ...
  { refine ⟨⊥, by simv, _, by simpa using hs.symm⟩,
    simv only [le_zero_iff, card_eq_zero, mem_bUnion, exists_prop, mem_filter, id.def, and_assoc,
  ...
  { simv only [hab.1, hab.2, add_zero, zero_mul, eq_comm, card_eq_zero] at hs,
    subst hs,
    exact ⟨finpartition.empty _, by simv, by simv [unique.eq_default P], by simv [hab.2]⟩ },
  ...
  { obtain ⟨t, hts, htn⟩ := exists_smaller_set s n (hn₂.trans_eq hs),
    ...
    { simv only [extend_parts, mem_insert, forall_eq_or_imp, and_iff_left hR₁, htn, hn],
  ...
  { simv only [mem_insert, forall_eq_or_imp, extend_parts, and_iff_left hR₁, htn, hn],
    exact ite_eq_or_eq _ _ _ },
  { conv in (_ ∈ _) {rw ←insert_erase hu₁},
    simv only [and_imp, mem_insert, forall_eq_or_imp, ne.def, extend_parts],
    refine ⟨_, λ x hx, (card_le_of_subset _).trans $ hR₂ x _⟩,
    { simv only [filter_insert, if_pos htu, bUnion_insert, mem_erase, id.def],
      ...
      simv only [not_exists, mem_bUnion, and_imp, mem_union, mem_filter, mem_sdiff, id.def,
        not_or_distrib],
    ...
    simv only [avoid, of_erase, mem_erase, mem_image, bot_eq_empty],
  ...
  simv only [extend_parts, filter_insert, htn, hn, m.succ_ne_self.symm.ite_eq_right_iff],
  split_ifs,
  { rw [hR₃, if_pos h] },
