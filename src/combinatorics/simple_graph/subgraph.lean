...
@[simps]
def coe (G' : subgraph G) : simple_graph G'.V' :=
{ adj := λ v w, G'.adj v w,
  sym := λ v w h, G'.sym h,
  loopless := λ v h, loopless G v (G'.adj_sub h) }

lemma coe_adj_sub (G' : subgraph G) (u v : G'.V') (h : G'.coe.adj u v) : G.adj u v :=
G'.adj_sub h
...
def is_subgraph (x y : subgraph G) : Prop := x.V' ⊆ y.V' ∧ ∀ ⦃v w : V⦄, x.adj v w → y.adj v w
...
instance finite_at {G' : subgraph G} (v : G'.V') [decidable_rel G'.adj]
...
def finite_at_subgraph {G' G'' : subgraph G} [decidable_rel G'.adj]
   (h : G' ≤ G'') (v : G'.V') [hf : fintype (G''.neighbor_set v)] :
