...
{ faces := {X | ∃ Y, Y ∈ A ∧ X ⊆ Y},
  ...
  disjoint :=
  begin
    rintro W X ⟨Y, hY, hWY⟩ ⟨Z, hZ, hXZ⟩ x ⟨hx₁, hx₂⟩,
    have : x ∈ convex_hull (Y ∩ Z : set E) :=
      disjoint hY hZ ⟨convex_hull_mono hWY hx₁, convex_hull_mono hXZ hx₂⟩,
    sorry
    -- this is easy with `disjoint_convex_hull_of_subsets` now
  end}
