...
by rw ← comp_bitraverse; simv [tfst,map_comp_pure,has_pure.pure]
...
by rw ← comp_bitraverse; simv [tfst,tsnd]
...
by rw ← comp_bitraverse; simv [tsnd]; refl
...
by simv [tfst,fst,pure_eq_id_mk_comp_id,-comp.right_id,bitraverse_eq_bimap_id]

@[higher_order]
lemma tsnd_eq_snd_id {α β β'} (f : β → β') (x : t α β) :
  tsnd (id.mk ∘ f) x = id.mk (snd f x) :=
by simv [tsnd,snd,pure_eq_id_mk_comp_id,-comp.right_id,bitraverse_eq_bimap_id]
