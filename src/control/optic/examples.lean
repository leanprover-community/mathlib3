...
{ first  := λ A B C ab, component.filter_map_action (λ ⟨a,c⟩ b, some (b,c)) $ component.map_props prod.fst $ ab
, second := λ A B C ab, component.filter_map_action (λ ⟨c,a⟩ b, some (c,b)) $ component.map_props prod.snd $ ab
}

/-- Note that this is not _technically_ an isomorphism. `P → list (html A)`
will _always_ rerender, but a component can optionally re-render depending on whether the props have changed, so can result in different behaviour. -/
