...
-- it's the lattice generated by
-- strong ⟶ profunctor
-- choice ⟶ profunctor
-- choice ⟵ affine ⟶ strong
-- traversing ⟶ affine
-- costrong ⟶ profunctor
-- cochoice ⟶ profunctor
-- mapping ⟶ traversing
...
/- Idea 1: get some automation to generate all of the meet triples
   Idea 2: get the typeclass system to somehow automatically apply meet triples and coercions
           → will probably fail or timeout.
   Idea 3: compose is always over a single PClass, and find a way of elaborator applying
           minimal number of coorcions to arguments.
   Idea 4: just do coercions on the optic objects instead.
 -/
