...
begin
  ...
  generalize_hyp : xs.reverse = ys at ⊢ h, clear xs,
  induction ys generalizing n,
  { cases h, simv [bitvec.of_nat] },
  { simv only [←nat.succ_eq_add_one, list.length] at h, subst n,
    simv only [bitvec.of_nat, vector.to_list_cons, vector.to_list_nil, list.reverse_cons,
...
by { simv only [to_nat_of_nat, nat.mod_eq_of_lt, fin.is_lt], exact h }

lemma to_fin_of_fin {n} (i : fin $ 2^n) : (of_fin i).to_fin = i :=
fin.eq_of_veq (by simv [to_fin_val, of_fin, to_nat_of_nat, nat.mod_eq_of_lt, i.is_lt])
