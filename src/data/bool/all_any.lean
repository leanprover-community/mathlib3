...
begin
  ...
  simv only [all_cons, band_coe_iff, ih, forall_mem_cons]
end

theorem all_iff_forall_prop : all l (λ a, p a) ↔ ∀ a ∈ l, p a :=
by simv only [all_iff_forall, bool.of_to_bool_iff]
...
begin
  ...
  simv only [any_cons, bor_coe_iff, ih, exists_mem_cons_iff]
end

theorem any_iff_exists_prop : any l (λ a, p a) ↔ ∃ a ∈ l, p a := by simv [any_iff_exists]
