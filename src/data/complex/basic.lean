...
@[simp, norm_cast elim] lemma of_real_re (r : ℝ) : (r : ℂ).re = r := rfl
@[simp, norm_cast elim] lemma of_real_im (r : ℝ) : (r : ℂ).im = 0 := rfl

@[simp, norm_cast elim] theorem of_real_inj {z w : ℝ} : (z : ℂ) = w ↔ z = w :=
...
@[simp, norm_cast squash] lemma of_real_zero : ((0 : ℝ) : ℂ) = 0 := rfl
...
@[simp, norm_cast squash] lemma of_real_one : ((1 : ℝ) : ℂ) = 1 := rfl
...
@[simp, norm_cast move] lemma of_real_add (r s : ℝ) : ((r + s : ℝ) : ℂ) = r + s := ext_iff.2 $ by simp

@[simp, norm_cast move] lemma of_real_bit0 (r : ℝ) : ((bit0 r : ℝ) : ℂ) = bit0 r := ext_iff.2 $ by simp [bit0]
@[simp, norm_cast move] lemma of_real_bit1 (r : ℝ) : ((bit1 r : ℝ) : ℂ) = bit1 r := ext_iff.2 $ by simp [bit1]

instance : has_neg ℂ := ⟨λ z, ⟨-z.re, -z.im⟩⟩

@[simp] lemma neg_re (z : ℂ) : (-z).re = -z.re := rfl
@[simp] lemma neg_im (z : ℂ) : (-z).im = -z.im := rfl
@[simp, norm_cast move] lemma of_real_neg (r : ℝ) : ((-r : ℝ) : ℂ) = -r := ext_iff.2 $ by simp
...
@[simp, norm_cast move] lemma of_real_mul (r s : ℝ) : ((r * s : ℝ) : ℂ) = r * s := ext_iff.2 $ by simp
...
@[simp, norm_cast move] lemma of_real_sub (r s : ℝ) : ((r - s : ℝ) : ℂ) = r - s := ext_iff.2 $ by simp
@[simp, norm_cast move] lemma of_real_pow (r : ℝ) (n : ℕ) : ((r ^ n : ℝ) : ℂ) = r ^ n :=
...
@[simp, norm_cast move] lemma of_real_inv (r : ℝ) : ((r⁻¹ : ℝ) : ℂ) = r⁻¹ :=
ext_iff.2 $ begin
  simp,
  by_cases r = 0, {simp [h]},
...
@[simp, norm_cast move] lemma of_real_div (r s : ℝ) : ((r / s : ℝ) : ℂ) = r / s :=
is_ring_hom.map_div coe

@[simp, norm_cast move] lemma of_real_fpow (r : ℝ) (n : ℤ) : ((r ^ n : ℝ) : ℂ) = (r : ℂ) ^ n :=
is_ring_hom.map_fpow of_real r n

@[simp, norm_cast squash] theorem of_real_int_cast : ∀ n : ℤ, ((n : ℝ) : ℂ) = n :=
int.eq_cast (λ n, ((n : ℝ) : ℂ))
  (by rw [int.cast_one, of_real_one])
  (λ _ _, by rw [int.cast_add, of_real_add])

@[simp, norm_cast squash] theorem of_real_nat_cast (n : ℕ) : ((n : ℝ) : ℂ) = n :=
...
@[simp, norm_cast squash] theorem of_real_rat_cast : ∀ n : ℚ, ((n : ℝ) : ℂ) = n :=
...
@[simp, norm_cast elim] lemma nat_cast_im (n : ℕ) : (n : ℂ).im = 0 :=
by rw [← of_real_nat_cast, of_real_im]

@[simp, norm_cast elim] lemma int_cast_re (n : ℤ) : (n : ℂ).re = n :=
by rw [← of_real_int_cast, of_real_re]

@[simp, norm_cast elim] lemma int_cast_im (n : ℤ) : (n : ℂ).im = 0 :=
by rw [← of_real_int_cast, of_real_im]

@[simp, norm_cast elim] lemma rat_cast_re (q : ℚ) : (q : ℂ).re = q :=
by rw [← of_real_rat_cast, of_real_re]

@[simp, norm_cast elim] lemma rat_cast_im (q : ℚ) : (q : ℂ).im = 0 :=
...
@[simp, norm_cast elim] lemma abs_cast_nat (n : ℕ) : abs (n : ℂ) = n :=
