...
begin
  ...
  { rw [hk, zero_add, mul_right_comm, gwz.inv_pow _ _, ← div_eq_mul_inv, mul_div_cancel],
...
... = (nat.fact n)⁻¹ * (range (j - n)).sum (λ m, n.succ⁻¹ ^ m) :
  by simp [mul_inv', mul_sum.symm, sum_mul.symm, -nat.fact_succ, mul_comm, gwz.inv_pow]
