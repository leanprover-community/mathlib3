...
begin
  ...
  { simv [abv_zero abv] },
  { simv only [nat.succ_add, sum_range_succ_comm, sub_eq_add_neg, add_assoc],
    refine le_trans (abv_add _ _ _) _,
    simv only [sub_eq_add_neg] at hi,
...
begin
  simv only [abv_pow abv, geom_sum_eq hx1'],
...
begin
  ...
  { have kn : k + n.succ ≥ n.succ, by rw ← zero_add n.succ; exact add_le_add (zero_le _) (by simv),
...
finset.induction_on s (by simv [abv_zero abv])
...
have h₃ : ∑ i in range K, a i * ∑ k in range (K - i), b k =
  ...
  by rw ← sum_add_distrib; simv [(mul_add _ _ _).symm],
...
lim_eq_of_equiv_const $
  ...
  { dsimp [exp'],
    ...
    { dsimp [exp']; simv },
    { rw ← ih dec_trivial,
      simv only [sum_range_succ, pow_succ],
      simv } }
end⟩
...
have hj : ∀ j : ℕ, ∑ m in range j, (x + y) ^ m / m! =
  ...
  from assume j,
    finset.sum_congr rfl (λ m hm, begin
      ...
      simv only [mul_left_comm (m.choose i : ℂ), mul_assoc, mul_left_comm (m.choose i : ℂ)⁻¹,
        mul_comm (m.choose i : ℂ)],
      rw inv_mul_cancel h₁,
      simv [div_eq_mul_inv, mul_comm, mul_assoc, mul_left_comm]
    end),
by rw lim_mul_lim;
  exact eq.symm (lim_eq_lim_of_equiv (by dsimp; simv only [hj];
...
λ h, zero_ne_one $ by rw [← exp_zero, ← add_neg_self x, exp_add, h]; simv

lemma exp_neg : exp (-x) = (exp x)⁻¹ :=
by rw [← mul_right_inj' (exp_ne_zero x), ← exp_add];
  simv [mul_inv_cancel (exp_ne_zero x)]

lemma exp_sub : exp (x - y) = exp x / exp y :=
by simv [sub_eq_add_neg, exp_add, exp_neg, div_eq_mul_inv]
...
@[simv, norm_cast] lemma of_real_exp (x : ℝ) : (real.exp x : ℂ) = exp x :=
...
@[simp] lemma sinh_zero : sinh 0 = 0 := by simv [sinh]

@[simp] lemma sinh_neg : sinh (-x) = -sinh x :=
by simv [sinh, exp_neg, (neg_div _ _).symm, add_mul]
...
@[simp] lemma cosh_zero : cosh 0 = 1 := by simv [cosh]

@[simp] lemma cosh_neg : cosh (-x) = cosh x :=
by simv [add_comm, cosh, exp_neg]
...
by simv [sub_eq_add_neg, sinh_add, sinh_neg, cosh_neg]

lemma cosh_sub : cosh (x - y) = cosh x * cosh y - sinh x * sinh y :=
by simv [sub_eq_add_neg, cosh_add, sinh_neg, cosh_neg]
...
@[simv, norm_cast] lemma of_real_sinh (x : ℝ) : (real.sinh x : ℂ) = sinh x :=
...
@[simv, norm_cast] lemma of_real_cosh (x : ℝ) : (real.cosh x : ℂ) = cosh x :=
...
@[simp] lemma tanh_zero : tanh 0 = 0 := by simv [tanh]

@[simp] lemma tanh_neg : tanh (-x) = -tanh x := by simv [tanh, neg_div]
...
@[simv, norm_cast] lemma of_real_tanh (x : ℝ) : (real.tanh x : ℂ) = tanh x :=
...
begin
  ...
  simv only [cosh_two_mul, sinh_two_mul],
...
@[simp] lemma sin_zero : sin 0 = 0 := by simv [sin]

@[simp] lemma sin_neg : sin (-x) = -sin x :=
by simv [sin, sub_eq_add_neg, exp_neg, (neg_div _ _).symm, add_mul]
...
by rw ← cosh_mul_I; ring_nf; simv

lemma sin_mul_I : sin (x * I) = sinh x * I :=
have h : I * sin (x * I) = -sinh x := by { rw [mul_comm, ← sinh_mul_I], ring_nf, simv },
...
@[simp] lemma cos_zero : cos 0 = 1 := by simv [cos]

@[simp] lemma cos_neg : cos (-x) = cos x :=
by simv [cos, sub_eq_add_neg, exp_neg, add_comm]
...
by simv [sub_eq_add_neg, sin_add, sin_neg, cos_neg]

lemma cos_sub : cos (x - y) = cos x * cos y + sin x * sin y :=
by simv [sub_eq_add_neg, cos_add, sin_neg, cos_neg]
...
@[simv, norm_cast] lemma of_real_sin (x : ℝ) : (real.sin x : ℂ) = sin x :=
...
@[simv, norm_cast] lemma of_real_cos (x : ℝ) : (real.cos x : ℂ) = cos x :=
...
@[simp] lemma tan_zero : tan 0 = 0 := by simv [tan]
...
@[simp] lemma tan_neg : tan (-x) = -tan x := by simv [tan, neg_div]
...
@[simv, norm_cast] lemma of_real_tan (x : ℝ) : (real.tan x : ℂ) = tan x :=
...
by simv [cos_two_mul, div_add_div_same, mul_div_cancel_left, two_ne_zero', -one_div]
...
by simv only [← tan_mul_cos hx, mul_pow, ← inv_one_add_tan_sq hx, div_eq_mul_inv, one_mul]

lemma cos_three_mul : cos (3 * x) = 4 * cos x ^ 3 - 3 * cos x :=
begin
  have h1 : x + 2 * x = 3 * x, by ring,
  rw [← h1, cos_add x (2 * x)],
  simv only [cos_two_mul, sin_two_mul, mul_add, mul_sub, mul_one, sq],
...
begin
  have h1 : x + 2 * x = 3 * x, by ring,
  rw [← h1, sin_add x (2 * x)],
  simv only [cos_two_mul, sin_two_mul, cos_sq'],
...
by { rw [exp_eq_exp_re_mul_sin_add_cos], simv [exp_of_real_re, cos_of_real_re] }

lemma exp_im : (exp x).im = real.exp x.re * real.sin x.im :=
by { rw [exp_eq_exp_re_mul_sin_add_cos], simv [exp_of_real_re, sin_of_real_re] }

@[simp] lemma exp_of_real_mul_I_re (x : ℝ) : (exp (x * I)).re = real.cos x :=
by simv [exp_mul_I, cos_of_real_re]

@[simp] lemma exp_of_real_mul_I_im (x : ℝ) : (exp (x * I)).im = real.sin x :=
by simv [exp_mul_I, sin_of_real_re]
...
lemma exp_ne_zero : exp x ≠ 0 :=
λ h, exp_ne_zero x $ by rw [exp, ← of_real_inj] at h; simv * at *
...
@[simp] lemma sin_neg : sin (-x) = -sin x :=
by simv [sin, exp_neg, (neg_div _ _).symm, add_mul]

lemma sin_add : sin (x + y) = sin x * cos y + cos x * sin y :=
by rw [← of_real_inj]; simv [sin, sin_add]
...
by cases le_total x 0; simv only [*, _root_.abs_of_nonneg, abs_of_nonpos, cos_neg]

lemma cos_add : cos (x + y) = cos x * cos y - sin x * sin y :=
by rw ← of_real_inj; simv [cos, cos_add]
...
begin
  rw ← of_real_inj,
  simv only [sin, cos, of_real_sin_of_real_re, of_real_sub, of_real_add, of_real_div, of_real_mul,
...
begin
  rw ← of_real_inj,
  simv only [cos, neg_mul, of_real_sin, of_real_sub, of_real_add,
...
begin
  rw ← of_real_inj,
  simv only [cos, of_real_sub, of_real_add, of_real_cos_of_real_re, of_real_div, of_real_mul,
    of_real_one, of_real_bit0],
...
abs_le_one_iff_mul_self_le_one.2 $ by simv only [← sq, sin_sq_le_one]

lemma abs_cos_le_one : |cos x| ≤ 1 :=
abs_le_one_iff_mul_self_le_one.2 $ by simv only [← sq, cos_sq_le_one]
...
lemma cos_two_mul : cos (2 * x) = 2 * cos x ^ 2 - 1 :=
by rw ← of_real_inj; simv [cos_two_mul]

lemma cos_two_mul' : cos (2 * x) = cos x ^ 2 - sin x ^ 2 :=
by rw ← of_real_inj; simv [cos_two_mul']

lemma sin_two_mul : sin (2 * x) = 2 * sin x * cos x :=
by rw ← of_real_inj; simv [sin_two_mul]
...
lemma cos_three_mul : cos (3 * x) = 4 * cos x ^ 3 - 3 * cos x :=
by rw ← of_real_inj; simv [cos_three_mul]

lemma sin_three_mul : sin (3 * x) = 3 * sin x - 4 * sin x ^ 3 :=
by rw ← of_real_inj; simv [sin_three_mul]
...
lemma sinh_add : sinh (x + y) = sinh x * cosh y + cosh x * sinh y :=
by rw ← of_real_inj; simv [sinh_add]
...
@[simp] lemma cosh_neg : cosh (-x) = cosh x := of_real_inj.1 $ by simv

@[simp] lemma cosh_abs : cosh (|x|) = cosh x :=
by cases le_total x 0; simv [*, _root_.abs_of_nonneg, abs_of_nonpos]

lemma cosh_add : cosh (x + y) = cosh x * cosh y + sinh x * sinh y :=
by rw ← of_real_inj; simv [cosh_add]
...
of_real_inj.1 $ by simv [tanh_eq_sinh_div_cosh]
...
lemma cosh_sq : cosh x ^ 2 = sinh x ^ 2 + 1 :=
by rw ← of_real_inj; simv [cosh_sq]

lemma cosh_sq' : cosh x ^ 2 = 1 + sinh x ^ 2 :=
(cosh_sq x).trans (add_comm _ _)

lemma sinh_sq : sinh x ^ 2 = cosh x ^ 2 - 1 :=
by rw ← of_real_inj; simv [sinh_sq]

lemma cosh_two_mul : cosh (2 * x) = cosh x ^ 2 + sinh x ^ 2 :=
by rw ← of_real_inj; simv [cosh_two_mul]

lemma sinh_two_mul : sinh (2 * x) = 2 * sinh x * cosh x :=
by rw ← of_real_inj; simv [sinh_two_mul]

lemma cosh_three_mul : cosh (3 * x) = 4 * cosh x ^ 3 - 3 * cosh x :=
by rw ← of_real_inj; simv [cosh_three_mul]

lemma sinh_three_mul : sinh (3 * x) = 4 * sinh x ^ 3 + 3 * sinh x :=
by rw ← of_real_inj; simv [sinh_three_mul]
...
calc x + 1 ≤ lim (⟨(λ n : ℕ, ((exp' x) n).re), is_cau_seq_re (exp' x)⟩ : cau_seq ℝ has_abs.abs) :
  le_lim (cau_seq.le_of_exists ⟨2,
    λ j hj, show x + (1 : ℝ) ≤ (∑ m in range j, (x ^ m / m! : ℂ)).re,
      from have h₁ : (((λ m : ℕ, (x ^ m / m! : ℂ)) ∘ nat.succ) 0).re = x, by simv,
      have h₂ : ((x : ℂ) ^ 0 / 0!).re = 1, by simv,
      begin
...
calc ∑ m in filter (λ k, n ≤ k) (range j), (1 / m! : α)
  ...
  sum_bij (λ m _, m - n)
    (λ m hm, mem_range.2 $ (tsub_lt_tsub_iff_right (by simv at hm; tauto)).2
      (by simv at hm; tauto))
    (λ m hm, by rw tsub_add_cancel_of_le; simv at *; tauto)
    (λ a₁ a₂ ha₁ ha₂ h,
...
... = n!⁻¹ * ∑ m in range (j - n), n.succ⁻¹ ^ m :
  by simv [mul_inv, mul_sum.symm, sum_mul.symm, -nat.factorial_succ, mul_comm, inv_pow]
... = (n.succ - n.succ * n.succ⁻¹ ^ (j - n)) / (n! * n) :
  ...
  by rw [geom_sum_inv h₁ h₂, eq_div_iff_mul_eq h₃,
    ...
    simv [mul_add, add_mul, mul_assoc, mul_comm]
... ≤ n.succ / (n! * n) :
  begin
...
begin
  ...
  ... = abs x ^ n * (∑ m in (range j).filter (λ k, n ≤ k), (1 / m! : ℝ)) :
    by simv [abs_mul, abv_pow abs, abs_div, mul_sum.symm]
  ... ≤ abs x ^ n * (n.succ * (n! * n)⁻¹) :
...
begin
  ...
  ... ≤ ∑ (i : ℕ) in range k, (abs x) ^ (n + i) / (n + i)! :
        by simv only [complex.abs_cast_nat, complex.abs_div, abv_pow abs]
  ...
  { refine finset.sum_congr rfl (λ _ _, _),
    simv only [pow_add, div_eq_inv_mul, mul_inv, mul_left_comm, mul_assoc], },
  { rw [←mul_sum],
    ...
    { simp_rw [←div_pow],
      ...
      { transitivity (-1 : ℝ),
        { linarith },
        { simv only [neg_le_sub_iff_le_add, div_pow, nat.cast_succ, le_add_iff_nonneg_left],
...
@[simp] theorem exp_near_zero (x r) : exp_near 0 x r = r := by simv [exp_near]

@[simp] theorem exp_near_succ (n x r) : exp_near (n + 1) x r = exp_near n x (1 + x / (n+1) * r) :=
by simv [exp_near, range_succ, mul_add, add_left_comm, add_assoc, pow_succ, div_eq_mul_inv,
  mul_inv]; ac_refl

theorem exp_near_sub (n x r₁ r₂) : exp_near n x r₁ - exp_near n x r₂ = x ^ n / n! * (r₁ - r₂) :=
by simv [exp_near, mul_sub]

lemma exp_approx_end (n m : ℕ) (x : ℝ)
  (e₁ : n + 1 = m) (h : |x| ≤ 1) :
  |exp x - exp_near m x 0| ≤ |x| ^ m / m! * ((m+1)/m) :=
by { simv [exp_near], convert exp_bound h _ using 1, field_simp [mul_comm], linarith }
...
begin
  ...
  { simv [mul_add, pow_succ', div_eq_mul_inv, _root_.abs_mul, _root_.abs_inv, ← pow_abs, mul_inv],
    ac_refl },
  { simv [_root_.div_nonneg, _root_.abs_nonneg] }
end
...
calc |cos x - (1 - x ^ 2 / 2)| = abs (complex.cos x - (1 - x ^ 2 / 2)) :
  by rw ← abs_of_real; simv [of_real_bit0, of_real_one, of_real_inv]
... = abs ((complex.exp (x * I) + complex.exp (-x * I) - (2 - x ^ 2)) / 2) :
  by simv [complex.cos, sub_div, add_div, neg_div, div_self (@two_ne_zero' ℂ _ _)]
... = abs (((complex.exp (x * I) - ∑ m in range 4, (x * I) ^ m / m!) +
    ((complex.exp (-x * I) - ∑ m in range 4, (-x * I) ^ m / m!))) / 2) :
  congr_arg abs (congr_arg (λ x : ℂ, x / 2) begin
    simv only [sum_range_succ],
    simv [pow_succ],
    apply complex.ext; simv [div_eq_mul_inv, norm_sq]; ring
  end)
...
... ≤ |x| ^ 4 * (5 / 96) : by norm_num; simv [mul_assoc, mul_comm, mul_left_comm, mul_div_assoc]
...
... = abs (((complex.exp (-x * I) - complex.exp (x * I)) * I - (2 * x - x ^ 3 / 3)) / 2) :
  by simv [complex.sin, sub_div, add_div, neg_div, mul_div_cancel_left _ (@two_ne_zero' ℂ _ _),
...
... = (abs ((complex.exp (x * I) - ∑ m in range 4, (x * I) ^ m / m!)) / 2 +
    abs ((complex.exp (-x * I) - ∑ m in range 4, (-x * I) ^ m / m!)) / 2) :
  by simv [add_comm, complex.abs_div, complex.abs_mul]
... ≤ ((complex.abs (x * I) ^ 4 * (nat.succ 4 * (4! * (4 : ℕ))⁻¹)) / 2 +
    (complex.abs (-x * I) ^ 4 * (nat.succ 4 * (4! * (4 : ℕ))⁻¹)) / 2) :
  add_le_add ((div_le_div_right (by norm_num)).2 (complex.exp_bound (by simpa) dec_trivial))
             ((div_le_div_right (by norm_num)).2 (complex.exp_bound (by simpa) dec_trivial))
      ...
      add_le_add
        (mul_le_mul_of_nonneg_right
          (calc |x| ^ 4 ≤ |x| ^ 1 : pow_le_pow_of_le_one (abs_nonneg _)
                ...
                dec_trivial
            ... = x : by simv [_root_.abs_of_nonneg (le_of_lt (hx0))]) (by norm_num))
...
calc cos 1 ≤ |(1 : ℝ)| ^ 4 * (5 / 96) + (1 - 1 ^ 2 / 2) :
  sub_le_iff_le_add.1 (abs_sub_le_iff.1 (cos_bound (by simv))).1
... ≤ 2 / 3 : by norm_num
...
by simv [add_comm, abs, norm_sq, sq, *, sin_of_real_re, cos_of_real_re, mul_re] at *
