...
there are two ways to sum a `dfinsupp` (both defined in `algebra/big_operators/dfinsupp`):
  * `dfinsupp.sum`, which works over an arbitrary function but requires recomputation of the support
    and therefore a `decidable` argument;
