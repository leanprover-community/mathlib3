...
lemma range_coe (m : ℕ) : set.range (λ (i : fin m), (i : ℕ)) = set.Iio m := by simp [set.Iio]
...
lemma cast_lt_cast_lt {m n : ℕ} (i : fin n) (hm : i.val < m) (hn : i.val < n) :
...
lemma image_of_nat' (m : ℕ) [h : fact (0 < m)] :
