...
@[simp] theorem cons_eq_insert {α} [decidable_eq α] (a s h) :
  @cons α a s h = insert a s :=
ext $ λ a, by simp
...
@[simp] theorem finset.disj_union_eq_union {α} [decidable_eq α] (s t h) :
  @finset.disj_union α s t h = s ∪ t :=
finset.ext $ λ a, by simp
