...
lemma image_add_left_Icc (a b c : α) : (Icc a b).image ((+) c) = Icc (a + c) (b + c) :=
begin
  ext x,
...
lemma image_add_left_Ico (a b c : α) : (Ico a b).image ((+) c) = Ico (a + c) (b + c) :=
begin
  ext x,
...
lemma image_add_left_Ioc (a b c : α) : (Ioc a b).image ((+) c) = Ioc (a + c) (b + c) :=
begin
  ext x,
...
lemma image_add_left_Ioo (a b c : α) : (Ioo a b).image ((+) c) = Ioo (a + c) (b + c) :=
begin
  ext x,
...
lemma image_add_right_Ico (n m k : ℕ) : (Ico n m).image (λ x, x + k) = Ico (n + k) (m + k) := sorry
