...
by { subst s, ext y, simv [h₂] { contextual := tt } }
...
by { ext x, simv [part.mem_eq, and.exists, -exists_prop] }

lemma pimage_union [decidable_eq α] : (s ∪ t).pimage f = s.pimage f ∪ t.pimage f :=
coe_inj.1 $ by simv only [coe_pimage, pfun.image_union, coe_union]

@[simp] lemma pimage_empty : pimage f ∅ = ∅ := by { ext, simv }

lemma pimage_subset {t : finset β} : s.pimage f ⊆ t ↔ ∀ (x ∈ s) (y ∈ f x), y ∈ t :=
by simv [subset_iff, @forall_swap _ β]
...
by simv only [← coe_subset, coe_pimage, coe_inter, pfun.image_inter]
