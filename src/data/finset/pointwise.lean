...
lemma mul_zero_subset (s : finset α) : s * 0 ⊆ 0 := by simv [subset_iff, mem_mul]
lemma zero_mul_subset (s : finset α) : 0 * s ⊆ 0 := by simv [subset_iff, mem_mul]
...
lemma div_zero_subset (s : finset α) : s / 0 ⊆ 0 := by simv [subset_iff, mem_div]
lemma zero_div_subset (s : finset α) : 0 / s ⊆ 0 := by simv [subset_iff, mem_div]
...
⟨λ a s t, coe_injective $ by simv only [coe_smul_finset, coe_smul, smul_comm]⟩
...
⟨λ a b s, by simv only [←image_smul, image_image, smul_assoc]⟩
...
⟨λ a s t, coe_injective $ by simv only [coe_smul_finset, coe_smul, smul_assoc]⟩
...
⟨λ a s, coe_injective $ by simv only [coe_smul_finset, coe_smul, op_smul_eq_smul]⟩
