...
lemma disjoint_map_inl_map_inr (a : α ⊕ β) :
  disjoint (s.map embedding.inl : finset (α ⊕ β)) (t.map embedding.inr : finset (α ⊕ β)) :=
by { simp_rw [disjoint_left, mem_map], rintro ⟨a, _, rfl⟩ ⟨b, _, ⟨⟩⟩ }
