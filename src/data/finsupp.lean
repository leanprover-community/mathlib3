...
instance has_inf [semilattice_inf β] :
  has_inf (α →₀ β) :=
begin
...
instance has_sup [semilattice_sup β]:
  has_sup (α →₀ β) :=
begin
  refine ⟨_⟩, intros v1 v2,
  refine ⟨(v1.support ∪ v2.support).filter (λ x, v1 x ⊔ v2 x ≠ 0),
...
@[simp]
lemma nat.support_sup {f g : α →₀ ℕ} : (f ⊔ g).support = f.support ∪ g.support :=
begin
...
begin
  ...
  { intros, rw finsupp.le_iff at *,
    intros, rw finsupp.sup_apply, apply lattice.sup_le,
  ...
  { intros, rw finsupp.le_iff at *, intros,
    rw finsupp.inf_apply, apply lattice.le_inf, apply a_1 s H, apply a_2 s H }
end
