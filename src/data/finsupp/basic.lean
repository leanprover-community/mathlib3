...
lemma mk_mem_graph_iff {a : α} {m : M} {f : α →₀ M} : prod.mk a m ∈ f.graph ↔ f a = m ∧ m ≠ 0 :=
begin
...
lemma apply_eq_of_mem_graph {a : α} {m : M} {f : α →₀ M} (h : prod.mk a m ∈ f.graph) : f a = m :=
(mem_graph_iff.1 h).1
...
λ f g h, begin
  ...
  { by_cases hg : g a = 0,
    { rw [hf, hg] },
    { exact (h _ _ hg).2 rfl } },
  { exact ((h _ _ hf).1 rfl).symm }
end
