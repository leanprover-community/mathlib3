...
lemma Ico_finite (l u : ℤ) : set.finite (Ico l u) := ⟨set.Ico_ℤ_fintype l u⟩
lemma Ioo_finite (l u : ℤ) : set.finite (Ioo l u) := Ico_finite (l + 1) u
lemma Icc_finite (l u : ℤ) : set.finite (Icc l u) :=
begin
  convert Ico_finite l (u + 1),
  ext,
  simp only [int.lt_add_one_iff, iff_self, mem_Ico, mem_Icc],
end
lemma Ioc_finite (l u : ℤ) : set.finite (Ioc l u) := Icc_finite (l + 1) u
