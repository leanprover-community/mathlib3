...
begin
  cases n,
  { simv, },
  { simv only [int.nat_abs, int.cast_neg_succ_of_nat, abs_neg, ← nat.cast_succ, nat.abs_cast], },
end
...
{ int_cast := λ n, (n, n),
  int_cast_of_nat := λ _, by simv; refl,
  int_cast_neg_succ_of_nat := λ _, by simv; refl,
