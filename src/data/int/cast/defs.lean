...
-/
@[protect_proj]
class add_group_with_one (R : Type u) extends add_group R, add_monoid_with_one R :=
(int_cast : ℤ → R := int.cast_def)
...
protected def cast (i : ℤ) : R := add_group_with_one.int_cast i
