...
theorem map_pmap {p : α → Prop} (g : β → γ) (f : Π a, p a → β) (l : list α)
  (H : ∀ (a : α), a ∈ l → p a) :
  map g (pmap f l H) = pmap (λ a h, g (f a h)) l H :=
by induction l; [refl, simp only [*, pmap, map]]; split; refl

theorem pmap_map {p : β → Prop} (g : ∀ b, p b → γ) (f : α → β) (l : list α)
  (H : ∀ (a : β), a ∈ map f l → p a) :
  pmap g (map f l) H = pmap (λ a h, g (f a) h) l (λ a h, H _ (mem_map_of_mem _ h)) :=
by induction l; [refl, simp only [*, pmap, map]]; split; refl

theorem pmap_eq_map_attach {p : α → Prop} (f : Π a, p a → β) (l : list α)
  (H : ∀ (a : α), a ∈ l → p a) : pmap f l H = l.attach.map (λ x, f x.1 (H _ x.2)) :=
