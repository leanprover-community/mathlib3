...
begin
  ...
  { cases n,
    ...
    { have hn' : n.succ ≤ l.length.succ,
      ...
      { have : n < l.length := by simpa [succ_le_succ_iff] using hn'' ,
  ...
  { rcases n with _|_|n,
    ...
    { rw [prev_ne_cons_cons],
      { convert hl _ _ (nodup_of_nodup_cons h) _ using 1,
        ...
        simp only [length, succ_le_succ_iff] at hn,
        set k := l.length,
...
begin
  ...
  { simp },
  { simp },
  { simp only [mem_cons_iff, exists_prop, mem_coe_iff, list.length, ne.def, succ_le_succ_iff,
               zero_le, iff_true],
