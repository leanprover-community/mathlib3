...
lemma take_join_of_lt {L : list (list α)} {n : ℕ} (h : n < L.join.length) :
  ∃ (m k : ℕ) (hm : m < L.length), k < (L.nth_le m hm).length ∧
    L.join.take n = (L.take m).join ++ (L.nth_le m hm).take k :=
begin
  -- https://github.com/user7230724/lean-projects/blob/master/src/list_take_join/main.lean#L122
  -- `get_max` and `nat_get_max_spec` are used here
  sorry,
  -- It will take me a few days to adjust the proof to not require additional imports.
end

lemma take_join {L : list (list α)} {n : ℕ} (h : L ≠ []) :
  ∃ (m k : ℕ) (hm : m < L.length), L.join.take n = (L.take m).join ++ (L.nth_le m hm).take k :=
begin
