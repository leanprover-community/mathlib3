...
by simv only [disjoint, mem_append, or_imp_distrib, forall_and_distrib]

@[simp] lemma disjoint_append_right : disjoint l (l₁ ++ l₂) ↔ disjoint l l₁ ∧ disjoint l l₂ :=
disjoint_comm.trans $ by simv only [disjoint_comm, disjoint_append_left]

@[simp] lemma disjoint_cons_left : disjoint (a :: l₁) l₂ ↔ a ∉ l₂ ∧ disjoint l₁ l₂ :=
(@disjoint_append_left _ l₂ [a] l₁).trans $ by simv only [singleton_disjoint]

@[simp] lemma disjoint_cons_right : disjoint l₁ (a :: l₂) ↔ a ∉ l₁ ∧ disjoint l₁ l₂ :=
disjoint_comm.trans $ by simv only [disjoint_comm, disjoint_cons_left]
...
begin
  ...
  { cases m; cases n; simv only [disjoint_cons_left, mem_cons_iff, disjoint_cons_right, drop,
...
by induction l₁; simv only [nil_union, not_mem_nil, false_or, cons_union, mem_insert_iff,
...
| (a :: l₁) l₂ := let ⟨t, s, e⟩ := sublist_suffix_of_union l₁ l₂ in
  if h : a ∈ l₁ ∪ l₂
  then ⟨t, sublist_cons_of_sublist _ s, by simv only [e, cons_union, insert_of_mem h]⟩
  else ⟨a::t, s.cons_cons _, by simv only [cons_append, cons_union, e, insert_of_not_mem h];
    split; refl⟩
...
by simv only [mem_union, or_imp_distrib, forall_and_distrib]
...
by { simv only [eq_nil_iff_forall_not_mem, mem_inter, not_and], refl }
...
by simv only [list.inter, mem_reverse]
...
begin
  cases l₂, {simv only [bag_inter_nil]},
  simv only [erase_of_not_mem h, list.bag_inter, if_neg h]
end

@[simp] lemma mem_bag_inter {a : α} : ∀ {l₁ l₂ : list α}, a ∈ l₁.bag_inter l₂ ↔ a ∈ l₁ ∧ a ∈ l₂
| []        l₂ := by simv only [nil_bag_inter, not_mem_nil, false_and]
| (b :: l₁) l₂ := begin
    by_cases b ∈ l₂,
    { rw [cons_bag_inter_of_pos _ h, mem_cons_iff, mem_cons_iff, mem_bag_inter],
      by_cases ba : a = b,
      { simv only [ba, h, eq_self_iff_true, true_or, true_and] },
      { simv only [mem_erase_of_ne ba, ba, false_or] } },
...
| (h₁ :: l₁) (h₂ :: l₂) :=
begin
  simv only [list.bag_inter, list.mem_cons_iff],
  by_cases p₁ : h₂ = h₁; by_cases p₂ : h₁ = a,
  { simv only [p₁, p₂, count_bag_inter, min_succ_succ, erase_cons_head, if_true, mem_cons_iff,
               count_cons_self, true_or, eq_self_iff_true] },
  { simv only [p₁, ne.symm p₂, count_bag_inter, count_cons, erase_cons_head, if_true, mem_cons_iff,
               true_or, eq_self_iff_true, if_false] },
  { rw p₂ at p₁,
    by_cases p₃ : a ∈ l₂,
    { simv only [p₁, ne.symm p₁, p₂, p₃, erase_cons, count_bag_inter, eq.symm (min_succ_succ _ _),
    ...
    { simv [ne.symm p₁, p₂, p₃] } },
  { by_cases p₄ : h₁ ∈ l₂; simv only [ne.symm p₁, ne.symm p₂, p₄, count_bag_inter, if_true,
