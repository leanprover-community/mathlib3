...
begin
  ...
  { have : hd ∈ l₂,
    ...
    by_cases hx : x = hd,
    { subst hd,
      simpa [succ_le_succ_iff] using h },
    { simpa [hx] using h } },
end
...
begin
  ...
  { rw [permutations'_aux, count_cons_self],
    by_cases hx : x = y,
    { subst hx,
      simpa [take_while, succ_eq_succ_iff] using IH _ },
    { rw take_while,
      rw if_neg hx,
...
begin
  ...
  { rw [nth_le_insert_nth_of_lt _ _ _ _ H (H.trans hk),
        nth_le_insert_nth_of_lt _ _ _ _ (H.trans (lt_succ _))] },
  { rw [nth_le_insert_nth_self _ _ _ hk.le,
        nth_le_insert_nth_of_lt _ _ _ _ (lt_succ _) hk, hk'] },
  { rcases (nat.succ_le_of_lt H).eq_or_lt with rfl|H',
    ...
    { obtain ⟨m, rfl⟩ := nat.exists_eq_add_of_lt H',
      rw [length_insert_nth _ _ hk.le, succ_le_succ_iff, nat.succ_add] at hn,
...
begin
  ...
  { rw [permutations'],
    ...
    { refine IH.pairwise_of_forall_ne (λ as ha bs hb H, _),
      ...
      simp only [lt_succ_iff, length_permutations'_aux] at hn hm,
      rw nth_le_permutations'_aux at hn' hm',
      have hx : nth_le (insert_nth n x as) m
        (by rwa [length_insert_nth _ _ hn, lt_succ_iff, hl]) = x,
      { simp [hn', ←hm', hm] },
      have hx' : nth_le (insert_nth m x bs) n
        (by rwa [length_insert_nth _ _ hm, lt_succ_iff, ←hl]) = x,
      { simp [hm', ←hn', hn] },
