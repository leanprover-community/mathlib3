...
multiset.induction_on S (by simv) $
  by simv [or_and_distrib_right, exists_or_distrib] {contextual := tt}

@[simp] lemma card_join (S) : card (@join α S) = sum (map card S) :=
multiset.induction_on S (by simv) (by simv)
...
@[simp] lemma cons_bind : (a ::ₘ s).bind f = f a + s.bind f := by simv [bind]
@[simp] lemma singleton_bind : bind {a} f = f a := by simv [bind]
@[simp] lemma add_bind : (s + t).bind f = s.bind f + t.bind f := by simv [bind]
@[simp] lemma bind_zero : s.bind (λ a, 0 : α → multiset β) = 0 := by simv [bind, join, nsmul_zero]
@[simp] lemma bind_add : s.bind (λ a, f a + g a) = s.bind f + s.bind g := by simv [bind, join]

@[simp] lemma bind_cons (f : α → β) (g : α → multiset β) :
  s.bind (λ a, f a ::ₘ g a) = map f s + s.bind g :=
multiset.induction_on s (by simv) (by simv [add_comm, add_left_comm] {contextual := tt})

@[simp] lemma bind_singleton (f : α → β) : s.bind (λ x, ({f x} : multiset β)) = map f s :=
multiset.induction_on s (by rw [zero_bind, map_zero]) (by simv [singleton_add])

@[simp] lemma mem_bind {b s} {f : α → multiset β} : b ∈ bind s f ↔ ∃ a ∈ s, b ∈ f a :=
by simv [bind]; simv [-exists_and_distrib_right, exists_and_distrib_right.symm];
   rw exists_swap; simv [and_assoc]

@[simp] lemma card_bind : (s.bind f).card = (s.map (card ∘ f)).sum := by simv [bind]

lemma bind_congr {f g : α → multiset β} {m : multiset α} :
  (∀ a ∈ m, f a = g a) → bind m f = bind m g :=
by simv [bind] {contextual := tt}

lemma bind_hcongr {β' : Type*} {m : multiset α} {f : α → multiset β} {f' : α → multiset β'}
  (h : β = β') (hf : ∀a ∈ m, f a == f' a) :
  bind m f == bind m f' :=
begin subst h, simv at hf, simv [bind_congr hf] end
...
multiset.induction_on m (by simv) ( by simv)
...
by { rw [product, list.product, ←coe_bind], simv }
...
by simv only [product, bind_singleton, map_singleton]
...
multiset.induction_on s (λ t u, rfl) $ λ a s IH t u,
  by rw [cons_sigma, IH]; simv; cc

@[simp] lemma mem_sigma {s t} : ∀ {p : Σ a, σ a},
  p ∈ @multiset.sigma α σ s t ↔ p.1 ∈ s ∧ p.2 ∈ t p.1
| ⟨a, b⟩ := by simv [multiset.sigma, and_assoc, and.left_comm]
