...
@[simp] lemma cardinal_mk_eq_one [subsingleton R] : #(mv_polynomial σ R) = 1 :=
@mk_eq_one _ mv_polynomial.unique

lemma cardinal_lift_mk_le_max {σ : Type u} {R : Type v} [comm_semiring R] :
  #(mv_polynomial σ R) ≤ max (max (cardinal.lift.{u} $ #R) $ cardinal.lift.{v} $ #σ) ℵ₀ :=
begin
  casesI subsingleton_or_nontrivial R,
  { exact cardinal_mk_eq_one.trans_le (le_max_of_le_right one_le_aleph_0) },
