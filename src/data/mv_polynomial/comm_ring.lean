...
is_ring_hom.map_sub C.is_ring_hom

@[simp] lemma C_neg : (C (-a) : mv_polynomial σ R) = -C a := is_ring_hom.map_neg C.is_ring_hom
...
lemma hom_C {f : mv_polynomial σ ℤ → S} (hf : is_ring_hom f) (n : ℤ) : f (C n) = (n : S) :=
((ring_hom.of hf.to_is_semiring_hom).comp C).eq_int_cast n
...
(λ n, by { rw [hom_C f.to_is_ring_hom, eval₂_C], exact c.eq_int_cast n })
