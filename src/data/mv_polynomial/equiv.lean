...
of the variables. -/
def ring_equiv_congr_left (e : S₁ ≃ S₂) : mv_polynomial S₁ R ≃+* mv_polynomial S₂ R :=
ring_equiv_congr (ring_equiv.refl R) e

@[simp]
lemma ring_equiv_congr_left_apply (e : S₁ ≃ S₂) (p : mv_polynomial S₁ R) :
  ring_equiv_congr_left R e p = rename e p :=
begin
  have h : ((ring_equiv.refl R) : R →+* R) = ring_hom.id _ := rfl,
  simp only [ring_equiv_congr_left, h, map_id, ring_equiv_congr_apply, comp_app],
end

@[simp]
lemma ring_equiv_congr_left_symm_apply (e : S₁ ≃ S₂) (p : mv_polynomial S₂ R) :
  (ring_equiv_congr_left R e).symm p = rename e.symm p :=
begin
  have h : ((ring_equiv.refl R).symm : R →+* R) = ring_hom.id _ := rfl,
...
ring_equiv_congr e (equiv.cast rfl)
...
@[simp]
lemma alg_equiv_congr_left_apply (e : S₁ ≃ S₂) (p : mv_polynomial S₁ R) :
  alg_equiv_congr_left R e p = rename e p :=
begin
  have h : ((alg_equiv.refl : R ≃ₐ[R] R).to_ring_equiv : R →+* R) = ring_hom.id _ := rfl,
  simp only [alg_equiv_congr_left, h, map_id, ring_equiv.to_fun_eq_coe, ring_equiv_congr_apply,
    comp_app, alg_equiv_congr_apply]
end

@[simp]
lemma alg_equiv_congr_left_symm_apply (e : S₁ ≃ S₂) (p : mv_polynomial S₂ R) :
  (alg_equiv_congr_left R e).symm p = rename e.symm p :=
begin
  have h : ((alg_equiv.refl : R ≃ₐ[R] R).to_ring_equiv.symm : R →+* R) = ring_hom.id _ := rfl,
  simp only [alg_equiv_congr_left, ring_equiv_congr, h, map_id, alg_equiv_congr_symm_apply,
    comp_app],
end
...
(ring_equiv_congr_left R $ (equiv.option_equiv_sum_punit.{0} S₁).trans (equiv.sum_comm _ _)).trans $
...
(ring_equiv_congr_left R (fin_succ_equiv n)).trans
