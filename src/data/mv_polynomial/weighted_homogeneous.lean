...
-- TODO: move!
/-- Rewrite a finsupp.sum as a finsum. -/
...
begin
  rw [finsum_eq_sum_of_support_subset, finsupp.sum],
  intro i,
...
lemma weighted_total_degree'_eq_zero_iff (w : σ → M) (p : mv_polynomial σ R) :
...
begin
  rw [ne.def, ← weighted_total_degree'_eq_zero_iff w p, ← ne.def, with_bot.ne_bot_iff_exists] at hp,
  obtain ⟨m, hm⟩ := hp,
  apply le_antisymm,
...
/-- The product of weighted homogeneous polynomials of degrees `m` and `n` has weighted degree
  `m + n`. -/
...
/-- If `⊥` is the smallest element of `M`, then a polynomial of weighted_total_degree `⊥` is
  weighted_homogeneous of degree `⊥`. -/
...
/-- The weighted homogeneous submodules form a graded ring. -/
