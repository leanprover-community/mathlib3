...
/-- The same as binary_rec_eq, but that one unfortunately requires `f` to be the identity when
  ...
  supplying `n = 0 → b = tt`. -/
lemma binary_rec_eq' {C : nat → Sort*} {z : C 0} {f : ∀ b n, C n → C (bit b n)}
...
bits_append_bit n ff (λ hn', false.elim $ hn hn')
...
example : bits 3423 = [tt, tt, tt, tt, tt, ff, tt, ff, tt, ff, tt, tt] := by simp
...
begin
  induction n using nat.binary_rec' with b n h ih, { simp, },
  rw [size_bit, bits_append_bit],
  { simp [ih], }, { exact h, }, { simpa [bit_eq_zero_iff], }
end
