...
lemma add_factorial_le_factorial_add (i n : ℕ) (hn : n ≠ 0) : i + n! ≤ (i + n)! :=
begin
  induction i with i ih,
  { rw [zero_add, zero_add] },
  { simp only [nat.succ_eq_add_one, add_right_comm _ 1, factorial_succ],
    rw [add_mul (i + n), one_mul, add_comm _ 1],
    apply add_le_add _ ih,
    exact mul_pos (nat.add_pos_right _ (nat.pos_of_ne_zero hn)) (factorial_pos _) }
end
