...
| (n + 1) := begin
  rw [mul_add, mul_one, double_factorial_def, factorial, pow_succ,
      factorial_eq_two_pow_mul_double_factorial, succ_eq_add_one],
  ring,
end
...
| 0       := rfl
| (n + 1) := begin
  rw [finset.prod_range_succ, ← factorial_eq_prod_even, mul_comm (2 * n) ‼,
      (by ring : 2 * (n + 1) = 2 * n + 2)],
  refl,
end
...
| 0       := rfl
| (n + 1) := begin
  rw [finset.prod_range_succ, ← factorial_eq_prod_odd, mul_comm (2 * n + 1) ‼,
