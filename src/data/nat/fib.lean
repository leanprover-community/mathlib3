...
- `nat.fib_succ_eq_sum_choose` : is that `fib` is given by the sum of `nat.choose` along the antidiagonal.
...
{ rw [fib_add_two, h1, h2, finset.nat.antidiagonal_succ_succ'],
  simp [finset.nat.antidiagonal_succ', choose_succ_succ, finset.sum_add_distrib, add_left_comm] })
