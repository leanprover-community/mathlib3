...
begin
  ...
  { congr,
    ext a,
    simp only [set.finite.mem_to_finset, ne.def, set.mem_set_of, finset.mem_erase],
    refine ⟨λ ⟨hp, hlt⟩,
...
begin
  ...
  congr,
  ext a,
  simp only [set.mem_set_of, and.congr_right_iff],
  intro hpa,
...
begin
  ...
  { simp only [nat.subtype.succ, set.mem_set_of, subtype.coe_mk, subtype.val_eq_coe],
    ...
    simp only [exists_prop, not_and, not_lt, set.mem_set_of, not_forall],
