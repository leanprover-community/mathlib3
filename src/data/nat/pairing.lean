...
begin
  ...
  { have hl : n - s*s - s ≤ s :=
    ...
    simv [mkpair, hl.not_lt, add_assoc, add_tsub_cancel_of_le (le_of_not_gt h), sm] }
end
...
let s := sqrt n in begin
  simv [unpair], change sqrt n with s,
  by_cases h : n - s * s < s; simv [h],
...
begin
  by_cases h₁ : a₁ < b; simv [mkpair, h₁, add_assoc],
  { by_cases h₂ : a₂ < b; simv [mkpair, h₂, h],
    simv at h₂,
    apply add_lt_add_of_le_of_lt,
    exact mul_self_le_mul_self h₂,
    exact lt_add_right _ _ _ h },
  { simv at h₁,
    simv [not_lt_of_gt (lt_of_le_of_lt h₁ h)],
    apply add_lt_add,
