...
by simv only [eq_none_iff_forall_not_mem, not_exists, not_and, mem_def, bind_eq_some']
...
lemma join_eq_some {x : option (option α)} {a : α} : x.join = some a ↔ x = some (some a) := by simv

lemma join_ne_none {x : option (option α)} : x.join ≠ none ↔ ∃ z, x = some (some z) := by simv

lemma join_ne_none' {x : option (option α)} : ¬(x.join = none) ↔ ∃ z, x = some (some z) := by simv

lemma join_eq_none {o : option (option α)} : o.join = none ↔ o = none ∨ o = some none :=
by rcases o with _|_|_; simv

lemma bind_id_eq_join {x : option (option α)} : x >>= id = x.join := by simv
...
lemma map_eq_none {α β} {x : option α} {f : α → β} :
  f <$> x = none ↔ x = none :=
by { cases x; simv only [map_none, map_some, eq_self_iff_true] }
...
by { cases x; simv only [map_none', map_some', h, mem_def] }
...
by { cases x; simv only [map_none', map_some'] }
...
by { cases x; simv only [pbind, none_bind', some_bind'] }
...
by { cases x; simv only [pbind, map_none'] }
...
by { cases x; simv only [map_none', map_some', pmap] }
...
begin
  cases x,
  { simv only [not_mem_none, exists_false, pmap, not_false_iff, exists_prop_of_false] },
  { split,
    { intro h,
      simv only [pmap] at h,
      exact ⟨x, rfl, h⟩ },
...
by cases x; simv [is_some]; exact ⟨_, rfl⟩
...
by by_cases p a; simv [option.guard, h]; intro; contradiction

@[simp] theorem guard_eq_some' {p : Prop} [decidable p] (u) : _root_.guard p = some u ↔ p :=
begin
  cases u,
  by_cases p; simv [_root_.guard, h]; refl <|> contradiction,
end
...
begin
  cases o,
  { simv only [true_and, false_or, eq_self_iff_true, none_orelse] },
  { simv only [some_orelse, or_false, false_and] }
end
...
begin
  cases o,
  { simv only [true_and, none_orelse, eq_self_iff_true] },
  { simv only [some_orelse, false_and], }
end
