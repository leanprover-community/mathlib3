...
def restrict {p : Prop} (o : roption α) (H : p → o.dom) : roption α :=
⟨p, λh, o.get (H h)⟩

@[simp]
theorem mem_restrict {p : Prop} (o : roption α) (h : p → o.dom) (a : α) :
  a ∈ restrict o h ↔ p ∧ a ∈ o :=
begin
...
theorem restrict_res (f : α → β) (s : set α) {p : set α} (H : p ⊆ s) :
  (res f s).restrict H = res f p :=
rfl
