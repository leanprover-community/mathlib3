...
variables (P : ipfunctor₀ I)

inductive W : I → Type u
| mk {i : I} (a : P.A i) (f : P.B i a ⟶ W) : W i

def W_dest (P : ipfunctor I I) {i} : W P i → P.obj (W P) i
| ⟨a, f⟩ := ⟨a, f⟩

def W_mk {i} : P.obj (W P) i → W P i
| ⟨a, f⟩ := ⟨a, f⟩

@[simp] theorem W_dest_W_mk {i} (p : P.obj (W P) i) : W_dest P (W_mk P p) = p :=
by cases p; reflexivity

@[simp] theorem W_mk_W_dest {i} (p : W P i) : W_mk P (W_dest P p) = p :=
by cases p; reflexivity
...
| root {i} (a : P.A i) (f : P.last.B i a ⟶ P.last.W) (j : J) (c : P.drop.B i a j) :
    W_path ⟨a, f⟩ j
| child {i} (a : P.A i) (f : P.last.B i a ⟶ P.last.W) (j : I) (x : P.last.B i a j) (k : J)
  (c : W_path ((f : Π j, P.last.B i a j → P.last.W j) x) k) :
    W_path ⟨a, f⟩ k
...
def W_ind {C : Π i, P.W α i → Sort*}
  ...
  ∀ i x, C i x
| i ⟨a,f⟩ :=
@ipfunctor.Wp_ind _ _ P α (λ i a f, C _ ⟨a, f⟩) (λ i a f f',
...
begin
  ...
  simp [W_map,this,W_mk,W_mk_eq,ipfunctor.map_eq' P.Wp g,comp_W_path_cases_on],
end
...
by erw [apply_append1, apply_append1, ipfunctor.map_eq', ← fam.split_fun_comp]
...
P.W_ind (λ j a f f' _,
  show ipfunctor.obj P (fam.append1 α (W P α)) j,
