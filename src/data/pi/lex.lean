...
lemma lex.le_of_forall_le [linear_order ι]
  (wf : well_founded ((<) : ι → ι → Prop)) [∀ a, linear_order (β a)] {a b : Πₗ i, β i}
  (h : ∀ i, a i ≤ b i) : a ≤ b :=
begin
