...
theorem sub_coe (a b : ℕ+) : (((a - b) : ℕ+) : ℕ) = ite (a > b) ((a : ℕ) - (b : ℕ)) 1 :=
 by {change ((to_pnat' (a.val - b.val)) : ℕ) =
      ite (a.val > b.val) (a.val - b.val) 1,
     split_ifs with h,
...
@[simp] theorem gcd_val (n m : ℕ+) :
 (gcd n m).val = nat.gcd n.val m.val := rfl

@[simp] theorem lcm_val (n m : ℕ+) :
 (lcm n m).val = nat.lcm n.val m.val := rfl

theorem gcd_dvd_left (n m : ℕ+) : (gcd n m) ∣ n :=
 nat.gcd_dvd_left n.val m.val

theorem gcd_dvd_right (n m : ℕ+) : (gcd n m) ∣ m :=
 nat.gcd_dvd_right n.val m.val

theorem dvd_gcd {m n k : ℕ+} (hm : k ∣ m) (hn : k ∣ n) : k ∣ gcd m n :=
 @nat.dvd_gcd m.val n.val k.val hm hn

theorem dvd_lcm_left (n m : ℕ+) : n ∣ lcm n m :=
 nat.dvd_lcm_left n.val m.val

theorem dvd_lcm_right (n m : ℕ+) : m ∣ lcm n m :=
 nat.dvd_lcm_right n.val m.val
...
def to_nat  : nat.primes → ℕ  := subtype.val
def to_pnat : nat.primes → ℕ+ := λ p, ⟨p.val,p.property.pos⟩

instance coe_nat  : has_coe nat.primes ℕ  := ⟨to_nat⟩
instance coe_pnat : has_coe nat.primes ℕ+ := ⟨to_pnat⟩
...
theorem coe_pnat_inj (p q : nat.primes) : (p : ℕ+) = (q : ℕ+) → p = q :=
 λ h, by {
   ...
   rw[coe_pnat_nat,coe_pnat_nat] at h, exact subtype.eq h,}
...
theorem coe_pnat_nat (v : prime_multiset) :
 ...
 by {change (v.map nat.primes.to_pnat).map subtype.val = v.map subtype.val,
...
theorem coe_prod (v : prime_multiset) :
 (v.prod : ℕ) = (v : multiset ℕ).prod :=
by {
  let h : (v.prod : ℕ) = ((v.map nat.primes.to_pnat).map coe).prod :=
  (multiset.prod_hom coe v.to_pnat_multiset).symm,
  rw[multiset.map_map] at h,
  have : (coe : ℕ+ → ℕ) ∘ nat.primes.to_pnat = nat.primes.to_nat :=
   funext (λ p, rfl),
