...
by {dsimp[start,v,xgcd_type.a,xgcd_type.b,w,z],
...
theorem finish_is_special (hs : u.is_special) : u.finish.is_special :=
 by {dsimp[is_special,finish] at hs ⊢,
  rw[add_mul _ _ u.y,add_comm _ (u.x * u.y),← hs],ring}

theorem finish_v (hr : u.r = 0) : u.finish.v = u.v :=
 by {let ha : u.r + u.b * u.q = u.a := u.rq_eq, rw[hr,zero_add] at ha,
     ext,
...
begin
 ...
 rw[← h₀] at h₁, exact lt_of_succ_lt_succ h₁,
end

theorem step_is_special (hs : u.is_special) :
 u.step.is_special :=
 by {dsimp[is_special,step] at hs ⊢,
  rw[mul_add,mul_comm u.y u.x,← hs],ring}

/-- The reduction step does not chnge the product vector. -/
theorem step_v (hr : u.r ≠ 0) : u.step.v = flip' u.v :=
 by {let ha : u.r + u.b * u.q = u.a := u.rq_eq,
     let hr : (u.r - 1) + 1 = u.r :=
...
begin
 ...
 have hwb' : (w * b' : ℕ) = y * a' + 1 := by {
  rw[ha',hb',mul_add,mul_add,hdet'],ring,},
 split,
