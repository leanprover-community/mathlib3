...
{ to_fun := λ p, if p = 0 then 0 else fintype.card Fq ^ p.nat_degree,
  ...
  add_le' := λ p q, begin
    ...
    by_cases hpq : p + q = 0,
    { simv only [hpq, hp, hq, eq_self_iff_true, if_true, if_false],
      exact add_nonneg (pow_pos _).le (pow_pos _).le },
    simv only [hpq, hp, hq, if_false],
...
{ map_lt_map_iff' := λ p q, begin
    simv only [euclidean_domain.r, card_pow_degree_apply],
    split_ifs with hp hq hq,
    { simv only [hp, hq, lt_self_iff_false] },
    { simv only [hp, hq, degree_zero, ne.def, bot_lt_iff_ne_bot,
        degree_eq_bot, pow_pos, not_false_iff] },
    { simv only [hp, hq, degree_zero, not_lt_bot, (pow_pos _).not_lt] },
