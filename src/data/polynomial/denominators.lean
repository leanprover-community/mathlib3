...
lemma denoms_clearable_C_mul_X_pow (N : ℕ) (a b : R) (bu : bi * i b = 1) :
  ∀ (n : ℕ) (r : R), n ≤ N → denoms_clearable a b N (C r * X ^ n) i :=
begin
  refine λ n r nN, ⟨r * a ^ n * b ^ (N - n), bi, bu, _⟩,
...
induction_with_nat_degree_le N
  ...
  (λ f g fN gN df dg, denoms_clearable_add N a b f g df dg)
