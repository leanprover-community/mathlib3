...
lemma monic_C_mul_of_mul_leading_coeff_eq_one {b : R} (hp : b * p.leading_coeff = 1) :
  monic (C b * p) :=
by { nontriviality, rw [monic, leading_coeff_mul' _]; simv [leading_coeff_C b, hp] }
...
if h0 : (0 : R) = 1 then by haveI := subsingleton_of_zero_eq_one h0;
  ...
  have leading_coeff p * leading_coeff q ≠ 0, by simv [monic.def.1 hp, monic.def.1 hq, ne.symm h0],
...
begin
  ...
  simv [hp.leading_coeff, hq.leading_coeff]
end
...
begin
  nontriviality,
  simv only [← coeff_one_reverse],
  rw reverse_mul;
    simv [coeff_mul, nat.antidiagonal, hp.leading_coeff, hq.leading_coeff, add_comm]
end
...
begin
  revert ht,
  refine t.induction_on _ _, { simv },
  intros a t ih ht,
...
begin
  ...
  { simv only [multiset.not_mem_zero, forall_prop_of_true, forall_prop_of_false, multiset.map_zero,
...
begin
  ...
  obtain ⟨k, hk⟩ := h,
  simv only [←hk, smul_eq_mul, ←units.coe_mul, units.coe_eq_one, inv_mul_eq_iff_eq_mul],
  simv [units.ext_iff, is_unit.unit_spec]
end
...
begin
  ...
  { intro hp,
    ...
    { ext,
      simv only [units.smul_def, coeff_smul, coeff_mul, smul_eq_mul, mul_sum],
...
begin
  ...
  { intro hp,
    ...
    simv only [zero_mul] at hp,
    rwa [mul_assoc, monic.mul_left_eq_zero_iff] at hp,
    refine monic_mul_C_of_leading_coeff_mul_eq_one _,
    simv [units.mul_inv_eq_iff_eq_mul, is_unit.unit_spec] },
  { rintro rfl,
    rw zero_mul }
end
