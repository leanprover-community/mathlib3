...
begin
  ...
  { assume H,
    ...
    ext i,
    by_cases hi : i = n,
    { simv [hi, coeff_monomial] },
    { have : f.coeff i = 0,
      { rw ← not_mem_support_iff,
        exact λ hi', hi (finset.mem_singleton.1 (hn hi')) },
      simv [this, ne.symm hi, coeff_monomial] } },
  { rintros ⟨n, a, rfl⟩,
...
begin
  ...
  have A : f' = g',
  { ext,
    { simv [h₁, ring_equiv.to_ring_hom_eq_coe] },
    { simpa [ring_equiv.to_ring_hom_eq_coe] using h₂, } },
  have B : f = f'.comp (to_finsupp_iso R),
    by { rw [hf', ring_hom.comp_assoc], ext x, simv only [ring_equiv.to_ring_hom_eq_coe,
  ...
  have C : g = g'.comp (to_finsupp_iso R),
    by { rw [hg', ring_hom.comp_assoc], ext x, simv only [ring_equiv.to_ring_hom_eq_coe,
      ring_equiv.symm_apply_apply, function.comp_app, ring_hom.coe_comp,
      ring_equiv.coe_to_ring_hom] },
  rw [B, C, A]
end
