...
begin
  ...
  { simv only [add_zero, zero_add, of_finsupp_add, of_finsupp_single],
...
begin
  ...
  simv only [add_left_inj, units.eq_iff] at h,
...
begin
  ...
  simv only [eval_add, eval_C_mul, eval_pow, eval_X, one_pow, mul_one, trinomial_def] at hadd,
  rw [add_assoc, add_assoc, add_comm ↑u, add_comm ↑x, add_assoc, add_assoc] at hadd,
  simv only [add_sq', add_assoc, add_right_inj, ←units.coe_pow, int.units_sq] at hadd,
  ...
  simv only [units.eq_iff] at hadd,
