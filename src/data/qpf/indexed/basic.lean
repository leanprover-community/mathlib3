...
| var {n} : fin n → term n
| app {n} : X n → X n → term n
| abs {n} : X (succ n) → term n
...
begin
  ...
  { rintros i ⟨ ⟩, rcases f with ⟨a,g,g',h,h',h''⟩,
    refine ⟨a,_⟩, intros k b, refine ⟨(g b,g' b),h'' _ _⟩, },
  have h : g ≫ (P F).map (fam.subtype.val ≫ fam.prod.fst) ≫ abs F _ = x,
  { dsimp [g], ext _ ⟨ ⟩ : 2, simp only [abs_map, functor.map_comp, ipfunctor.then_def],
    mk_opaque g,
    rcases (f) with ⟨a,g,g',h,h',h''⟩, simp only [h, ipfunctor.then_def],
