...
by simp only [recF]; rw [pfunctor.W_ind_eq]; refl
...
begin
  ...
  ext : 2, dsimp, rw pfunctor.W_path_dest_right_W_path_cases_on, cases f x_2; refl,
end
...
begin
  ...
  case mvqpf.Wequiv.abs : j a₀ f'₀ f₀ a₁ f'₁ f₁ h
    { rw [recF_eq'], simp only [abs_map_assoc, pfunctor.W_dest'_W_mk, h, pfunctor.then_def] },
...
by { ext1, ext1 ⟨a,f⟩, simp only [pfunctor.W_mk', Wrepr_W_mk, abs_map', pfunctor.then_def], congr,
     ext1 ⟨ ⟩; ext1; refl }
...
begin
  ...
  case mvqpf.Wequiv.abs : j a₀ f'₀ f₀ a₁ f'₁ f₁ h
    { rw [q.P.W_map_W_mk, q.P.W_map_W_mk], apply Wequiv.abs,
      rw [pfunctor.append_contents_comp, pfunctor.append_contents_comp, ← pfunctor.map_eq', ← pfunctor.map_eq', abs_map', abs_map', h]},
  case mvqpf.Wequiv.trans : i x y z e₁ e₂ ih₁ ih₂
...
lemma fix.quot.mk_lift {γ : fam J} (g : q.P.W α ⟶ γ)
      (h : ∀ ⦃i : J⦄ (a b : pfunctor.W (P F) α i), Wequiv a b → g a = g b) :
...
lemma fix.quot.lift_comp {γ : fam J} (f : q.P.W α ⟶ β) (g : β ⟶ γ)
      (h : ∀ ⦃i : J⦄ (a b : pfunctor.W (P F) α i), Wequiv a b → f a = f b) :
...
begin
  ...
  rw [recF_eq', abs_map_assoc, pfunctor.W_dest'_W_mk'_assoc, abs_map_assoc, abs_repr_assoc,
...
have fix.mk (abs _ _ ⟨a, q.P.append_contents f' (λ i x, ⟦f x⟧)⟩) = ⟦Wrepr (q.P.W_mk a f' f)⟧,
  begin
    ...
    rw [pfunctor.W_dest'_W_mk'', abs_map', abs_repr', ←abs_map', pfunctor.map_eq'],
    conv { to_rhs, rw [Wrepr_W_mk, q.P.W_dest'_W_mk'', abs_repr', pfunctor.map_eq'] },
    congr' 2, rw [pfunctor.append_contents, pfunctor.append_contents],
...
begin
  ...
  -- specialize h _,
  specialize h (value _ ((P F).obj (append1 α (fix F α))) ⟨a,pfunctor.append_contents _ f' (λ i x, ⟦f x⟧)⟩ ≫ abs _ _) _,
  --   simp at h, exact h },
  -- { ext, cases x_2, },
  { replace h := congr_fun (congr_fun h j) unit.rfl, simp [value] at h, exact h },
  ext _ ⟨⟨⟨ rfl ⟩⟩⟩, simp only [value, pfunctor.append_contents, append_fun, pfunctor.then_def],
...
begin
  ...
  apply h i (value _ _ (abs _ (append1 α (fix F α))
          ⟨a,
           pfunctor.append_contents (P F) f' (λ (i_1 : J) (x : (pfunctor.last (P F)).B i a i_1), ⟦f x⟧)⟩))
          _ unit.rfl,
  rw [mvqpf.liftp_iff],
  rintros k ⟨⟨rfl⟩⟩,
  refine ⟨a, _, rfl, _⟩,
...
def vec_P (α : Type) : pfunctor (empty ⊕ ℕ) ℕ :=
...
def mut_P (α : Type) : pfunctor (empty ⊕ bool) bool :=
{ A := pair α bool,
  B := mut_children' α }
