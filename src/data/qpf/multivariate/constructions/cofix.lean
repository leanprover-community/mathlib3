...
have hh : subtype_val _ ⊚ to_subtype _ ⊚ from_append1_drop_last ⊚ c ⊚ b =
  ...
  by { dsimp [c,b],
       ...
       { dsimp,
         simv only [prod_map_id, drop_fun_prod, drop_fun_append_fun, drop_fun_diag, id_comp,
                    drop_fun_to_subtype],
         erw [to_subtype_of_subtype_assoc,id_comp],
         clear_except,
         ext i x : 2, induction i,
         refl,  apply i_ih, },
       simv only [h, last_fun_from_append1_drop_last, last_fun_to_subtype, last_fun_append_fun,
...
begin
  ...
  simv only [cofix.repr, M.dest_corec, abs_map, abs_repr],
  ...
  { simv only [←append_prod_append_fun, prod_map_id],
    apply eq_of_drop_last_eq,
    { dsimp, simv only [drop_fun_diag],
      erw subtype_val_diag_sub },
    ext1,
    simv only [cofix.abs, prod.mk.inj_iff, prod_map, function.comp_app, last_fun_append_fun,
...
begin
  ...
  { congr' with y, erw [append_fun_id_id], simv [mvfunctor.id_map] },
end
