...
def cofix_a.default [inhabited F.A] : Π n, cofix_a F n
| 0 := cofix_a.continue
...
/-- for a non-trivial approximation, return the first label of the tree -/
...
/-- relation between two approximations that state they both contain the same
...
/-- given an infinite series of approximation `approx`, `all_agree approx`
tells us that they are all consistent with each other -/
