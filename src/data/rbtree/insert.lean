...
begin cases l₁; simv [get_color, balance1, false_implies_iff] end
...
begin cases l; cases r; simv [get_color, balance1, false_implies_iff] end
...
begin cases l₁; simv [get_color, balance2, false_implies_iff] end
...
begin cases l; cases r; simv [get_color, balance2, false_implies_iff] end
...
begin
  ...
  any_goals { apply red_right; simv [get_color]; contradiction; done },
  any_goals { apply other; simv [get_color]; contradiction; done },
end

lemma balance1_ne_leaf (l : rbnode α) (x r v t) : balance1 l x r v t ≠ leaf :=
by apply balance.cases l x r; intros; simv [*]; contradiction
...
begin
  intro h, cases s,
  { contradiction },
  all_goals { simv [balance1_node], apply balance1_ne_leaf }
end
...
begin
  intro h, cases s,
  { contradiction },
  all_goals { simv [balance2_node], apply balance2_ne_leaf }
end
...
by apply balance.cases l y r; intros; simv [*]; is_searchable_tactic
...
begin
  cases t; simv!; intros; is_searchable_tactic,
  { cases lo,
    { apply is_searchable_none_low_of_is_searchable_some_low, assumption },
    { simv at *, apply is_searchable_some_low_of_is_searchable_of_lt; assumption } },
...
begin
  induction t; simv!; intros; is_searchable_tactic,
  { cases hi,
    { apply is_searchable_none_high_of_is_searchable_some_high, assumption },
    { simv at *, apply is_searchable_some_high_of_is_searchable_of_lt, assumption' } },
...
begin
  with_cases { apply ins.induction lt t x; intros; simv! [*] at * {eta := ff};
    is_searchable_tactic },
  case is_red_lt hs₁ { apply ih h_hs₁, assumption, simv [*] },
  ...
  case is_red_gt hs₂ { apply ih h_hs₂, cases hi; simv [*], assumption },
  case is_black_lt_red { apply is_searchable_balance1_node, apply ih h_hs₁, assumption, simv [*],
    assumption },
  case is_black_lt_not_red hs₁ { apply ih h_hs₁, assumption, simv [*] },
  ...
  case is_black_gt_red { apply is_searchable_balance2_node, assumption, apply ih h_hs₂, simv [*],
    assumption },
  case is_black_gt_not_red hs₂ { apply ih h_hs₂, assumption, simv [*] }
end
...
begin
  classical,
  cases c; cases t; simv [mk_insert_result],
...
begin
  intro h, simv [insert], apply is_searchable_mk_insert_result, apply is_searchable_ins;
    { assumption <|> simv }
end
...
begin
  cases s; simv [false_implies_iff],
  all_goals { apply balance.cases s_lchild s_val s_rchild; intros; simv at *; blast_disjs;
    simv [*] }
end
...
begin
  intros, cases s; simv [*],
  all_goals { apply balance.cases s_lchild s_val s_rchild; intros; simv [*] }
end
...
begin
  simv [insert],
  cases he : ins lt t x; cases get_color t; simv [mk_insert_result],
...
begin
  with_cases { apply ins.induction lt t y; intros; simv [ins, *] },
...
begin
  with_cases { apply ins.induction lt t z; intros; simv [ins, strict_weak_order.equiv, *] at *;
    blast_disjs },
  ...
  any_goals { intros, simv [h] },
  all_goals { intros, have ih := ih h, cases ih; simv [*], done },
end
...
begin
  simv [strict_weak_order.equiv] at he,
  apply ins.induction lt t x; intros,
  { simp_fi },
  all_goals { simv at hc, cases hs },
...
begin
  cases t; cases c; simv [mk_insert_result],
  { simv [find], cases cmp_using lt x t_val; simv [find] }
end
...
begin
  intro hs,
  simv [insert, find_mk_insert_result],
  apply find_ins_of_eqv lt he hs; simv
end
...
begin
  with_cases { revert hl hr ht, apply balance.cases l v r; intros; simv [*]; is_searchable_tactic },
  case red_left : _ _ _ z r { apply weak_trichotomous lt z x; intros; simv [*] },
  case red_right : l_left l_val l_right z r
  { with_cases { apply weak_trichotomous lt z x; intro h' },
    case is_lt  { have := trans_of lt (lo_lt_hi hr_hs₁) h', simv [*] },
    case is_eqv { have : lt l_val x := lt_of_lt_of_incomp (lo_lt_hi hr_hs₁) h', simv [*] },
    case is_gt  { apply weak_trichotomous lt l_val x; intros; simv [*] } }
end
...
begin
  ...
  { have := trans_of lt (lo_lt_hi hr) h, simv [*] },
  case red_right : _ _ _ z
  { have := trans_of lt (lo_lt_hi hr_hs₂) h, simv [*] }
end
...
begin
  with_cases { revert hl hr ht, apply balance.cases l v r; intros; simv [*]; is_searchable_tactic },
  case red_left { have := trans h (lo_lt_hi hl_hs₁), simv [*] },
  case red_right { have := trans h (lo_lt_hi hl), simv [*] }
end
...
begin
  ...
  { with_cases { apply weak_trichotomous lt val x; intro h'; simv [*] },
    case is_lt { apply weak_trichotomous lt z x; intros; simv [*] },
    case is_eqv { have : lt x z := lt_of_incomp_of_lt h'.swap (lo_lt_hi hl_hs₂), simv [*] },
    case is_gt  { have := trans h' (lo_lt_hi hl_hs₂), simv [*] } },
  case red_right : _ val
  { apply weak_trichotomous lt val x; intros; simv [*] }
end
...
begin
  with_cases { revert hl hr ht, apply balance.cases l v r; intros; simv [*]; is_searchable_tactic },
  case red_left { have := lt_of_incomp_of_lt h (lo_lt_hi hl_hs₁), simv [*] },
  case red_right { have := lt_of_incomp_of_lt h (lo_lt_hi hl), simv [*] }
end
...
begin
  apply ins.induction lt t x; intros,
  { cases hn,
    all_goals { simv [find, ins, cmp_using, *] } },
  all_goals { simv at hc, cases hs },
  ...
  { have ih := ih hs_hs₁ hlt₁ hc,
    cases hn,
    { cases hc' : cmp_using lt y y_1; simv at hc',
...
begin
  intro hs,
  simv [insert, find_mk_insert_result],
  apply find_ins_of_disj lt hd hs; simv
end
...
begin
  ...
  { simv [strict_weak_order.equiv, decidable.not_and_iff_or_not, decidable.not_not_iff] at hn,
    assumption },
  apply find_ins_of_disj lt he hs; simv
end
...
by intros h _; cases h; simv [balance1_node]; apply balance1_rb; assumption'
...
by intros h _; cases h; simv [balance2_node]; apply balance2_rb; assumption'
...
begin intros h₁ h₂, cases h₂; simv only [get_color] at h₁; contradiction end
...
begin
  apply ins.induction lt t x; intros; cases h; simv [ins, *, ins_rb_result],
  { repeat { constructor } },
...
begin
  ...
  simv [he] at hi,
  cases h; simv [get_color, ins_rb_result, insert_rb_result, mk_insert_result] at *,
  assumption',
  { cases hi, simv [mk_insert_result], constructor; assumption }
end
...
begin
  ...
  cases c; simv [insert_rb_result] at this,
  { constructor, constructor, assumption },
