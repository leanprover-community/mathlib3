...
lemma discriminant_le_zero {a b c : ℝ} (h : ∀ x : ℝ,  0 ≤ a*x*x + b*x + c) : b*b - 4*a*c ≤ 0 :=
...
begin
  ...
  { rw hb at *,
    ...
    { have : a * sqrt((-c-1)/a) * sqrt((-c-1)/a) + 0 * sqrt((-c-1)/a) + c = -1,
        have hac : 0 ≤ (-c-1)/a, from
          div_nonneg_of_nonpos_of_neg (by linarith) (lt_of_le_of_ne ha' ha),
        rw [mul_assoc, mul_self_sqrt hac, mul_div_cancel' _ ha], ring,
      have h := h (sqrt ((-c-1)/a)),
