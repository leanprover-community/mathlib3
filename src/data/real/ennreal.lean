...
We define `ennreal = [0..∞] := with_no ℝ≥0` to be the type of extended nonnegative real numbers,
...
In this file we define some algebraic operations and a linear order on `ennreal`
...
* `[0..∞]`: the type of the extended nonnegative real numbers;
...
instance : inhabited [0..∞] := ⟨0⟩

instance : has_coe ℝ≥0 [0..∞] := ⟨ option.some ⟩

instance : can_lift [0..∞] ℝ≥0 :=
{ coe := coe,
  cond := λ r, r ≠ ∞,
  prf := λ x hx, ⟨option.get $ option.ne_none_iff_is_some.1 hx, option.some_get _⟩ }

@[simp] lemma none_eq_top : (none : [0..∞]) = ∞ := rfl
@[simp] lemma some_eq_coe (a : ℝ≥0) : (some a : [0..∞]) = (↑a : [0..∞]) := rfl

/-- `to_nnreal x` returns `x` if it is real, otherwise 0. -/
protected def to_nnreal : [0..∞] → ℝ≥0
| (some r) := r
| none := 0

/-- `to_real x` returns `x` if it is real, `0` otherwise. -/
protected def to_real (a : [0..∞]) : real := coe (a.to_nnreal)

/-- `of_real x` returns `x` if it is nonnegative, `0` otherwise. -/
protected def of_real (r : real) : [0..∞] := coe (nnreal.of_real r)

@[simp, norm_cast] lemma to_nnreal_coe : (r : [0..∞]).to_nnreal = r := rfl

@[simp] lemma coe_to_nnreal : ∀{a:[0..∞]}, a ≠ ∞ → ↑(a.to_nnreal) = a
| (some r) h := rfl
| none     h := (h rfl).elim

@[simp] lemma of_real_to_real {a : [0..∞]} (h : a ≠ ∞) : ennreal.of_real (a.to_real) = a :=
...
lemma coe_nnreal_eq (r : ℝ≥0) : (r : [0..∞]) = ennreal.of_real r :=
by { rw [ennreal.of_real, nnreal.of_real], cases r with r h, congr, dsimp, rw max_eq_left h }

lemma of_real_eq_coe_nnreal {x : ℝ} (h : 0 ≤ x) :
  ennreal.of_real x = @coe ℝ≥0 [0..∞] _ (⟨x, h⟩ : ℝ≥0) :=
by { rw [coe_nnreal_eq], refl }

@[simp] lemma of_real_coe_nnreal : ennreal.of_real p = p := (coe_nnreal_eq p).symm

@[simp, norm_cast] lemma coe_zero : ↑(0 : ℝ≥0) = (0 : [0..∞]) := rfl
@[simp, norm_cast] lemma coe_one : ↑(1 : ℝ≥0) = (1 : [0..∞]) := rfl

@[simp] lemma to_real_nonneg {a : [0..∞]} : 0 ≤ a.to_real := by simp [ennreal.to_real]

@[simp] lemma top_to_nnreal : ∞.to_nnreal = 0 := rfl
@[simp] lemma top_to_real : ∞.to_real = 0 := rfl
@[simp] lemma one_to_real : (1 : [0..∞]).to_real = 1 := rfl
@[simp] lemma one_to_nnreal : (1 : [0..∞]).to_nnreal = 1 := rfl
@[simp] lemma coe_to_real (r : ℝ≥0) : (r : [0..∞]).to_real = r := rfl
@[simp] lemma zero_to_nnreal : (0 : [0..∞]).to_nnreal = 0 := rfl
@[simp] lemma zero_to_real : (0 : [0..∞]).to_real = 0 := rfl
@[simp] lemma of_real_zero : ennreal.of_real (0 : ℝ) = 0 :=
by simp [ennreal.of_real]; refl
@[simp] lemma of_real_one : ennreal.of_real (1 : ℝ) = (1 : [0..∞]) :=
by simp [ennreal.of_real]

lemma of_real_to_real_le {a : [0..∞]} : ennreal.of_real (a.to_real) ≤ a :=
if ha : a = ∞ then ha.symm ▸ le_top else le_of_eq (of_real_to_real ha)

lemma forall_ennreal {p : [0..∞] → Prop} : (∀a, p a) ↔ (∀r:ℝ≥0, p r) ∧ p ∞ :=
⟨assume h, ⟨assume r, h _, h _⟩,
  assume ⟨h₁, h₂⟩ a, match a with some r := h₁ _ | none := h₂ end⟩

lemma forall_ne_top {p : [0..∞] → Prop} : (∀ a ≠ ∞, p a) ↔ ∀ r : ℝ≥0, p r :=
option.ball_ne_none

lemma exists_ne_top {p : [0..∞] → Prop} : (∃ a ≠ ∞, p a) ↔ ∃ r : ℝ≥0, p r :=
option.bex_ne_none

lemma to_nnreal_eq_zero_iff (x : [0..∞]) : x.to_nnreal = 0 ↔ x = 0 ∨ x = ∞ :=
⟨begin
  cases x,
  { simp [none_eq_top] },
  { have A : some (0:ℝ≥0) = (0:[0..∞]) := rfl,
    simp [ennreal.to_nnreal, A] {contextual := tt} }
end,
by intro h; cases h; simp [h]⟩

lemma to_real_eq_zero_iff (x : [0..∞]) : x.to_real = 0 ↔ x = 0 ∨ x = ∞ :=
by simp [ennreal.to_real, to_nnreal_eq_zero_iff]

@[simp] lemma coe_ne_top : (r : [0..∞]) ≠ ∞ := with_top.coe_ne_top
@[simp] lemma top_ne_coe : ∞ ≠ (r : [0..∞]) := with_top.top_ne_coe
...
@[simp, norm_cast] lemma coe_eq_coe : (↑r : [0..∞]) = ↑q ↔ r = q := with_top.coe_eq_coe
@[simp, norm_cast] lemma coe_le_coe : (↑r : [0..∞]) ≤ ↑q ↔ r ≤ q := with_top.coe_le_coe
@[simp, norm_cast] lemma coe_lt_coe : (↑r : [0..∞]) < ↑q ↔ r < q := with_top.coe_lt_coe
lemma coe_mono : monotone (coe : ℝ≥0 → [0..∞]) := λ _ _, coe_le_coe.2

@[simp, norm_cast] lemma coe_eq_zero : (↑r : [0..∞]) = 0 ↔ r = 0 := coe_eq_coe
@[simp, norm_cast] lemma zero_eq_coe : 0 = (↑r : [0..∞]) ↔ 0 = r := coe_eq_coe
@[simp, norm_cast] lemma coe_eq_one : (↑r : [0..∞]) = 1 ↔ r = 1 := coe_eq_coe
@[simp, norm_cast] lemma one_eq_coe : 1 = (↑r : [0..∞]) ↔ 1 = r := coe_eq_coe
@[simp, norm_cast] lemma coe_nonneg : 0 ≤ (↑r : [0..∞]) ↔ 0 ≤ r := coe_le_coe
@[simp, norm_cast] lemma coe_pos : 0 < (↑r : [0..∞]) ↔ 0 < r := coe_lt_coe

@[simp, norm_cast] lemma coe_add : ↑(r + p) = (r + p : [0..∞]) := with_top.coe_add
@[simp, norm_cast] lemma coe_mul : ↑(r * p) = (r * p : [0..∞]) := with_top.coe_mul

@[simp, norm_cast] lemma coe_bit0 : (↑(bit0 r) : [0..∞]) = bit0 r := coe_add
@[simp, norm_cast] lemma coe_bit1 : (↑(bit1 r) : [0..∞]) = bit1 r := by simp [bit1]
lemma coe_two : ((2:ℝ≥0) : [0..∞]) = 2 := by norm_cast

protected lemma zero_lt_one : 0 < (1 : [0..∞]) :=
  canonically_ordered_semiring.zero_lt_one

@[simp] lemma one_lt_two : (1 : [0..∞]) < 2 :=
coe_one ▸ coe_two ▸ by exact_mod_cast (@one_lt_two ℕ _ _)
@[simp] lemma zero_lt_two : (0:[0..∞]) < 2 := lt_trans ennreal.zero_lt_one one_lt_two
lemma two_ne_zero : (2:[0..∞]) ≠ 0 := (ne_of_lt zero_lt_two).symm
lemma two_ne_top : (2:[0..∞]) ≠ ∞ := coe_two ▸ coe_ne_top

/-- The set of numbers in `[0..∞]` that are not equal to `∞` is equivalent to `ℝ≥0`. -/
...
lemma cinfi_ne_top [has_Inf α] (f : [0..∞] → α) : (⨅ x : {x // x ≠ ∞}, f x) = ⨅ x : ℝ≥0, f x :=
eq.symm $ infi_congr _ ne_top_equiv_nnreal.symm.surjective $ λ x, rfl

lemma infi_ne_top [complete_lattice α] (f : [0..∞] → α) : (⨅ x ≠ ∞, f x) = ⨅ x : ℝ≥0, f x :=
by rw [infi_subtype', cinfi_ne_top]

lemma csupr_ne_top [has_Sup α] (f : [0..∞] → α) : (⨆ x : {x // x ≠ ∞}, f x) = ⨆ x : ℝ≥0, f x :=
@cinfi_ne_top (order_dual α) _ _

lemma supr_ne_top [complete_lattice α] (f : [0..∞] → α) : (⨆ x ≠ ∞, f x) = ⨆ x : ℝ≥0, f x :=
@infi_ne_top (order_dual α) _ _

lemma infi_ennreal {α : Type*} [complete_lattice α] {f : [0..∞] → α} :
  (⨅ n, f n) = (⨅ n : ℝ≥0, f n) ⊓ f ∞ :=
le_antisymm
...
lemma supr_ennreal {α : Type*} [complete_lattice α] {f : [0..∞] → α} :
  (⨆ n, f n) = (⨆ n : ℝ≥0, f n) ⊔ f ∞ :=
...
/-- Coercion `ℝ≥0 → [0..∞]` as a `ring_hom`. -/
def of_nnreal_hom : ℝ≥0 →+* [0..∞] :=
...
@[simp, norm_cast] lemma coe_indicator {α} (s : set α) (f : α → ℝ≥0) (a : α) :
  ((s.indicator f a : ℝ≥0) : [0..∞]) = s.indicator (λ x, f x) a :=
(of_nnreal_hom : ℝ≥0 →+ [0..∞]).map_indicator _ _ _

@[simp, norm_cast] lemma coe_pow (n : ℕ) : (↑(r^n) : [0..∞]) = r^n :=
...
lemma not_lt_top {x : [0..∞]} : ¬ x < ∞ ↔ x = ∞ := by rw [lt_top_iff_ne_top, not_not]
...
lemma mul_lt_top_iff {a b : [0..∞]} : a * b < ∞ ↔ (a < ∞ ∧ b < ∞) ∨ a = 0 ∨ b = 0 :=
begin
  split,
...
lemma mul_self_lt_top_iff {a : [0..∞]} : a * a < ⊤ ↔ a < ⊤ :=
...
@[simp] lemma bot_eq_zero : (⊥ : [0..∞]) = 0 := rfl

@[simp] lemma coe_lt_top : coe r < ∞ := with_top.coe_lt_top r
@[simp] lemma not_top_le_coe : ¬ ∞ ≤ ↑r := with_top.not_top_le_coe r
lemma zero_lt_coe_iff : 0 < (↑p : [0..∞]) ↔ 0 < p := coe_lt_coe
@[simp, norm_cast] lemma one_le_coe_iff : (1:[0..∞]) ≤ ↑r ↔ 1 ≤ r := coe_le_coe
@[simp, norm_cast] lemma coe_le_one_iff : ↑r ≤ (1:[0..∞]) ↔ r ≤ 1 := coe_le_coe
@[simp, norm_cast] lemma coe_lt_one_iff : (↑p : [0..∞]) < 1 ↔ p < 1 := coe_lt_coe
@[simp, norm_cast] lemma one_lt_coe_iff : 1 < (↑p : [0..∞]) ↔ 1 < p := coe_lt_coe
@[simp, norm_cast] lemma coe_nat (n : ℕ) : ((n : ℝ≥0) : [0..∞]) = n := with_top.coe_nat n
@[simp] lemma of_real_coe_nat (n : ℕ) : ennreal.of_real n = n := by simp [ennreal.of_real]
@[simp] lemma nat_ne_top (n : ℕ) : (n : [0..∞]) ≠ ∞ := with_top.nat_ne_top n
...
lemma le_of_forall_pos_le_add : ∀{a b : [0..∞]}, (∀ε:ℝ≥0, 0 < ε → b < ∞ → a ≤ b + ε) → a ≤ b
| a    none     h := le_top
| none (some a) h :=
...
lemma lt_iff_exists_rat_btwn :
  a < b ↔ (∃q:ℚ, 0 ≤ q ∧ a < nnreal.of_real q ∧ (nnreal.of_real q:[0..∞]) < b) :=
⟨λ h,
  begin
...
lemma lt_iff_exists_real_btwn :
  a < b ↔ (∃r:ℝ, 0 ≤ r ∧ a < ennreal.of_real r ∧ (ennreal.of_real r:[0..∞]) < b) :=
...
lemma coe_nat_lt_coe {n : ℕ} : (n : [0..∞]) < r ↔ ↑n < r := ennreal.coe_nat n ▸ coe_lt_coe
lemma coe_lt_coe_nat {n : ℕ} : (r : [0..∞]) < n ↔ r < n := ennreal.coe_nat n ▸ coe_lt_coe
@[norm_cast] lemma coe_nat_lt_coe_nat {m n : ℕ} : (m : [0..∞]) < n ↔ m < n :=
ennreal.coe_nat n ▸ coe_nat_lt_coe.trans nat.cast_lt
lemma coe_nat_ne_top {n : ℕ} : (n : [0..∞]) ≠ ∞ := ennreal.coe_nat n ▸ coe_ne_top
lemma coe_nat_mono : strict_mono (coe : ℕ → [0..∞]) := λ _ _, coe_nat_lt_coe_nat.2
@[norm_cast] lemma coe_nat_le_coe_nat {m n : ℕ} : (m : [0..∞]) ≤ n ↔ m ≤ n :=
coe_nat_mono.le_iff_le

instance : char_zero [0..∞] := ⟨coe_nat_mono.injective⟩

protected lemma exists_nat_gt {r : [0..∞]} (h : r ≠ ∞) : ∃n:ℕ, r < n :=
begin
...
@[norm_cast] lemma coe_min : ((min r p:ℝ≥0):[0..∞]) = min r p :=
coe_mono.map_min

@[norm_cast] lemma coe_max : ((max r p:ℝ≥0):[0..∞]) = max r p :=
coe_mono.map_max

lemma le_of_top_imp_top_of_to_nnreal_le {a b : [0..∞]} (h : a = ⊤ → b = ⊤)
...
lemma coe_Sup {s : set ℝ≥0} : bdd_above s → (↑(Sup s) : [0..∞]) = (⨆a∈s, ↑a) := with_top.coe_Sup
lemma coe_Inf {s : set ℝ≥0} : s.nonempty → (↑(Inf s) : [0..∞]) = (⨅a∈s, ↑a) := with_top.coe_Inf

@[simp] lemma top_mem_upper_bounds {s : set [0..∞]} : ∞ ∈ upper_bounds s :=
assume x hx, le_top

lemma coe_mem_upper_bounds {s : set ℝ≥0} :
  ↑r ∈ upper_bounds ((coe : ℝ≥0 → [0..∞]) '' s) ↔ r ∈ upper_bounds s :=
...
@[simp] lemma add_sub_self : ∀{a b : [0..∞]}, b < ∞ → (a + b) - b = a
...
@[simp] lemma sub_add_cancel_of_le : ∀{a b : [0..∞]}, b ≤ a → (a - b) + b = a :=
begin
...
lemma sub_right_inj {a b c : [0..∞]} (ha : a < ∞) (hb : b ≤ a) (hc : c ≤ a) :
  a - b = a - c ↔ b = c :=
iff.intro
  begin
...
lemma prod_lt_top {s : finset α} {f : α → [0..∞]} (h : ∀a∈s, f a < ∞) : (∏ a in s, f a) < ∞ :=
with_top.prod_lt_top h

/-- A sum of finite numbers is still finite -/
lemma sum_lt_top {s : finset α} {f : α → [0..∞]} :
  (∀a∈s, f a < ∞) → ∑ a in s, f a < ∞ :=
with_top.sum_lt_top

/-- A sum of finite numbers is still finite -/
lemma sum_lt_top_iff {s : finset α} {f : α → [0..∞]} :
  ∑ a in s, f a < ∞ ↔ (∀a∈s, f a < ∞) :=
with_top.sum_lt_top_iff

/-- A sum of numbers is infinite iff one of them is infinite -/
lemma sum_eq_top_iff {s : finset α} {f : α → [0..∞]} :
  (∑ x in s, f x) = ∞ ↔ (∃a∈s, f a = ∞) :=
with_top.sum_eq_top_iff

/-- seeing `[0..∞]` as `ℝ≥0` does not change their sum, unless one of the `[0..∞]` is
infinity -/
lemma to_nnreal_sum {s : finset α} {f : α → [0..∞]} (hf : ∀a∈s, f a < ∞) :
...
/-- seeing `[0..∞]` as `real` does not change their sum, unless one of the `[0..∞]` is infinity -/
lemma to_real_sum {s : finset α} {f : α → [0..∞]} (hf : ∀a∈s, f a < ∞) :
...
variables {x y z : [0..∞]} {ε ε₁ ε₂ : [0..∞]} {s : set [0..∞]}
...
section inv
instance : has_inv [0..∞] := ⟨λa, Inf {b | 1 ≤ a * b}⟩

instance : div_inv_monoid [0..∞] :=
{ inv := has_inv.inv,
  .. (infer_instance : monoid [0..∞]) }

@[simp] lemma inv_zero : (0 : [0..∞])⁻¹ = ∞ :=
show Inf {b : [0..∞] | 1 ≤ 0 * b} = ∞, by simp; refl

@[simp] lemma inv_top : ∞⁻¹ = 0 :=
bot_unique $ le_of_forall_le_of_dense $ λ a (h : a > 0), Inf_le $ by simp [*, ne_of_gt h, top_mul]

@[simp, norm_cast] lemma coe_inv (hr : r ≠ 0) : (↑r⁻¹ : [0..∞]) = (↑r)⁻¹ :=
le_antisymm
...
lemma inv_involutive : function.involutive (λ a:[0..∞], a⁻¹) :=
λ a, ennreal.inv_inv

lemma inv_bijective : function.bijective (λ a:[0..∞], a⁻¹) :=
...
@[simp] lemma inv_lt_top {x : [0..∞]} : x⁻¹ < ∞ ↔ 0 < x :=
by { simp only [lt_top_iff_ne_top, inv_ne_top, pos_iff_ne_zero] }

lemma div_lt_top {x y : [0..∞]} (h1 : x < ∞) (h2 : 0 < y) : x / y < ∞ :=
...
lemma mul_le_iff_le_inv {a b r : [0..∞]} (hr₀ : r ≠ 0) (hr₁ : r ≠ ∞) : (r * a ≤ b ↔ a ≤ r⁻¹ * b) :=
by rw [← @ennreal.mul_le_mul_left _ a _ hr₀ hr₁, ← mul_assoc, mul_inv_cancel hr₀ hr₁, one_mul]

lemma le_of_forall_nnreal_lt {x y : [0..∞]} (h : ∀ r : ℝ≥0, ↑r < x → ↑r ≤ y) : x ≤ y :=
begin
...
lemma eq_top_of_forall_nnreal_le {x : [0..∞]} (h : ∀ r : ℝ≥0, ↑r ≤ x) : x = ∞ :=
top_unique $ le_of_forall_nnreal_lt $ λ r hr, h r

lemma div_add_div_same {a b c : [0..∞]} : a / c + b / c = (a + b) / c :=
...
lemma inv_two_add_inv_two : (2:[0..∞])⁻¹ + 2⁻¹ = 1 :=
by rw [← two_mul, ← div_eq_mul_inv, div_self two_ne_zero two_ne_top]

lemma add_halves (a : [0..∞]) : a / 2 + a / 2 = a :=
...
lemma half_pos {a : [0..∞]} (h : 0 < a) : 0 < a / 2 :=
by simp [ne_of_gt h]

lemma one_half_lt_one : (2⁻¹:[0..∞]) < 1 := inv_lt_one.2 $ one_lt_two

lemma half_lt_self {a : [0..∞]} (hz : a ≠ 0) (ht : a ≠ ∞) : a / 2 < a :=
begin
  lift a to ℝ≥0 using ht,
  have h : (2 : [0..∞]) = ((2 : ℝ≥0) : [0..∞]), from rfl,
...
lemma one_sub_inv_two : (1:[0..∞]) - 2⁻¹ = 2⁻¹ :=
by simpa only [div_eq_mul_inv, one_mul] using sub_half one_ne_top

lemma exists_inv_nat_lt {a : [0..∞]} (h : a ≠ 0) :
  ∃n:ℕ, (n:[0..∞])⁻¹ < a :=
...
begin
  ...
  have : 0 < (n : [0..∞]), from (zero_le _).trans_lt hn,
...
begin
  ...
  have : (n : [0..∞]) ≠ 0 := nat.cast_ne_zero.2 npos.lt.ne',
  use [n, npos],
...
lemma of_real_le_of_le_to_real {a : ℝ} {b : [0..∞]} (h : a ≤ ennreal.to_real b) :
  ennreal.of_real a ≤ b :=
...
lemma of_real_le_iff_le_to_real {a : ℝ} {b : [0..∞]} (hb : b ≠ ∞) :
...
lemma of_real_lt_iff_lt_to_real {a : ℝ} {b : [0..∞]} (ha : 0 ≤ a) (hb : b ≠ ∞) :
  ennreal.of_real a < b ↔ a < ennreal.to_real b :=
begin
  lift b to ℝ≥0 using hb,
...
lemma le_of_real_iff_to_real_le {a : [0..∞]} {b : ℝ} (ha : a ≠ ∞) (hb : 0 ≤ b) :
...
lemma to_real_le_of_le_of_real {a : [0..∞]} {b : ℝ} (hb : 0 ≤ b) (h : a ≤ ennreal.of_real b) :
  ennreal.to_real a ≤ b :=
have ha : a ≠ ∞, from ne_top_of_le_ne_top of_real_ne_top h,
(le_of_real_iff_to_real_le ha hb).1 h

lemma lt_of_real_iff_to_real_lt {a : [0..∞]} {b : ℝ} (ha : a ≠ ∞) :
  a < ennreal.of_real b ↔ ennreal.to_real a < b :=
begin
  lift a to ℝ≥0 using ha,
...
lemma to_real_of_real_mul (c : ℝ) (a : [0..∞]) (h : 0 ≤ c) :
...
@[simp] lemma to_nnreal_mul_top (a : [0..∞]) : ennreal.to_nnreal (a * ∞) = 0 :=
begin
  by_cases h : a = 0,
...
@[simp] lemma to_nnreal_top_mul (a : [0..∞]) : ennreal.to_nnreal (∞ * a) = 0 :=
by rw [mul_comm, to_nnreal_mul_top]

@[simp] lemma to_real_mul_top (a : [0..∞]) : ennreal.to_real (a * ∞) = 0 :=
by rw [ennreal.to_real, to_nnreal_mul_top, nnreal.coe_zero]

@[simp] lemma to_real_top_mul (a : [0..∞]) : ennreal.to_real (∞ * a) = 0 :=
...
lemma to_nnreal_mul {a b : [0..∞]}: (a * b).to_nnreal = a.to_nnreal * b.to_nnreal :=
to_nnreal_hom.map_mul a b

lemma to_nnreal_pow (a : [0..∞]) (n : ℕ) : (a ^ n).to_nnreal = a.to_nnreal ^ n :=
to_nnreal_hom.map_pow a n

lemma to_nnreal_prod {ι : Type*} {s : finset ι} {f : ι → [0..∞]} :
...
lemma to_real_pow (a : [0..∞]) (n : ℕ) : (a ^ n).to_real = a.to_real ^ n :=
to_real_hom.map_pow a n

lemma to_real_prod {ι : Type*} {s : finset ι} {f : ι → [0..∞]} :
...
lemma Inf_add {s : set [0..∞]} : Inf s + a = ⨅b∈s, b + a :=
by simp [Inf_eq_infi, infi_add]

lemma add_infi {a : [0..∞]} : a + infi f = ⨅b, a + f b :=
...
lemma infi_sum {f : ι → α → [0..∞]} {s : finset α} [nonempty ι]
...
lemma infi_mul {ι} [nonempty ι] {f : ι → [0..∞]} {x : [0..∞]} (h : x ≠ ∞) :
  infi f * x = ⨅i, f i * x :=
begin
...
lemma mul_infi {ι} [nonempty ι] {f : ι → [0..∞]} {x : [0..∞]} (h : x ≠ ∞) :
  x * infi f = ⨅i, x * f i :=
by { rw [mul_comm, infi_mul h], simp only [mul_comm], assumption }

/-! `supr_mul`, `mul_supr` and variants are in `topology.instances.[0..∞]`. -/

end infi

section supr

lemma supr_coe_nat : (⨆n:ℕ, (n : [0..∞])) = ∞ :=
(supr_eq_top _).2 $ assume b hb, ennreal.exists_nat_gt (lt_top_iff_ne_top.1 hb)

end supr

/-- `le_of_add_le_add_left` is normally applicable to `ordered_cancel_add_comm_monoid`,
but it holds in `[0..∞]` with the additional assumption that `a < ∞`. -/
lemma le_of_add_le_add_left {a b c : [0..∞]} : a < ∞ →
  a + b ≤ a + c → b ≤ c :=
