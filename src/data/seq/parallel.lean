...
begin
  ...
  { cases o with a a, { exact terminates_parallel.aux a T },
    ...
    { apply destruct_eq_inl.1, simp only [parallel.aux1, destruct_corec], rw [h], simp },
    { rw C, apply_instance },
    { apply destruct_eq_inr.1, simp only [parallel.aux1, destruct_corec], rw [h, H], simp },
    { rw C, apply @computation.think.terminates _ _ _,
      apply terminates_parallel.aux _ T, simp } },
  { cases o with a a, { exact terminates_parallel.aux a T },
    induction h : parallel.aux2 l with a l';
    have C : corec parallel.aux1 (l, S) = _,
    { apply destruct_eq_inl.1, simp [parallel.aux1], rw [h], simp },
    ...
    { rw C, apply @computation.think.terminates _ _ _,
      ...
      induction e : S.head with o,
      { obtain rfl := seq.head_eq_none.1 e,
        exact TT l' },
