...
/-- Bad instance for `has_neg (set α)` used to generate errors ASAP. -/
@[priority 2000]
instance set.has_neg_bad {α : Type*} : has_neg (set α) := ⟨classical.choice ⟨id⟩⟩
...
lemma inter_compl_nonempty_iff {s t : set α} : (s ∩ ∁ t).nonempty ↔ ¬ s ⊆ t :=
begin
  split,
  { rintros ⟨x ,xs, xt⟩ sub,
    exact xt (sub xs) },
...
lemma nonempty_compl {s : set α} : (∁ s).nonempty ↔ s ≠ univ :=
...
lemma subset_compl_singleton_iff {a : α} {s : set α} : s ⊆ ∁ {a} ↔ a ∉ s :=
