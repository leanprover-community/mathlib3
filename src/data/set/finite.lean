...
* `set.mk_finite` to prove `set.finite` for a `set` from a `fintype` instance.
...
theorem mk_finite (s : set α) [h : fintype s] : s.finite := ⟨h⟩
...
The implementation of these constructors ideally should be no more than `set.mk_finite`, after
...
by { classical, apply mk_finite }

theorem finite_univ [fintype α] : (@univ α).finite := mk_finite _

theorem finite.union {s t : set α} (hs : s.finite) (ht : t.finite) : (s ∪ t).finite :=
by { classical, casesI hs, casesI ht, apply mk_finite }
...
by { classical, casesI hs, apply mk_finite }
...
by { classical, casesI hs, haveI := set.fintype_subset _ ht, apply mk_finite }
...
theorem finite_Union [fintype (plift ι)] {f : ι → set α} (H : ∀ i, (f i).finite) :
  (⋃ i, f i).finite :=
by { classical, haveI := λ i, (H i).fintype, apply mk_finite }

theorem finite.sUnion {s : set (set α)} (hs : s.finite) (H : ∀ t ∈ s, finite t) : (⋃₀ s).finite :=
by { classical, casesI hs, haveI := λ (i : s), (H i i.2).fintype, apply mk_finite }

theorem finite.bUnion {ι} {s : set ι} (hs : s.finite)
  {t : ι → set α} (ht : ∀ i ∈ s, (t i).finite) : (⋃(i ∈ s), t i).finite :=
by { classical, casesI hs,
     haveI := fintype_bUnion s t (λ i hi, (ht i hi).fintype), apply mk_finite }
...
@[simp] theorem finite_empty : (∅ : set α).finite := mk_finite _

@[simp] theorem finite_singleton (a : α) : ({a} : set α).finite := mk_finite _

theorem finite_pure (a : α) : (pure a : set α).finite := mk_finite _
...
lemma finite_lt_nat (n : ℕ) : finite {i | i < n} := mk_finite _

lemma finite_le_nat (n : ℕ) : finite {i | i ≤ n} := mk_finite _
...
theorem finite.seq {f : set (α → β)} {s : set α} (hf : f.finite) (hs : s.finite) :
  finite (f.seq s) :=
by { classical, casesI hf, casesI hs, apply mk_finite }

theorem finite.seq' {α β : Type u} {f : set (α → β)} {s : set α} (hf : f.finite) (hs : s.finite) :
  finite (f <*> s) :=
hf.seq hs

theorem finite_mem_finset (s : finset α) : finite {a | a ∈ s} := mk_finite _
...
begin
  ...
  exact mk_finite (fintype.pi_finset t),
end

/-- A finite union of finsets is finite. -/
lemma union_finset_finite_of_range_finite (f : α → finset β) (h : (range f).finite) :
  (⋃ a, (f a : set β)).finite :=
by { rw ← bUnion_range, exact h.bUnion (λ y hy, mk_finite y) }
...
lemma infinite.exists_not_mem_finset {s : set α} (hs : s.infinite) (f : finset α) :
  ∃ a ∈ s, a ∉ f :=
let ⟨a, has, haf⟩ := (hs.diff (mk_finite f)).nonempty in ⟨a, has, λ h, haf $ finset.mem_coe.1 h⟩
...
(mk_finite ↑(finset.range (b + 1))).subset range_find_greatest_subset
...
This is a wrapper around `set.mk_finite`. -/
lemma finite_to_set (s : finset α) : (s : set α).finite := set.mk_finite _
