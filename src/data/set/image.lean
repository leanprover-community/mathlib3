...
lemma is_compl_range_inl_range_inr : is_compl (range $ @sum.inl α β) (range sum.inr) :=
is_compl.of_le
  (by { rintro y ⟨⟨x₁, rfl⟩, ⟨x₂, _⟩⟩, cc })
  (by { rintro (x|y) -; [left, right]; exact mem_range_self _ })

@[simp] lemma range_inl_union_range_inr : range (sum.inl : α → α ⊕ β) ∪ range sum.inr = univ :=
is_compl_range_inl_range_inr.sup_eq_top

@[simp] lemma range_inl_inter_range_inr : range (sum.inl : α → α ⊕ β) ∩ range sum.inr = ∅ :=
is_compl_range_inl_range_inr.inf_eq_bot

@[simp] lemma range_inr_union_range_inl : range (sum.inr : β → α ⊕ β) ∪ range sum.inl = univ :=
is_compl_range_inl_range_inr.symm.sup_eq_top

@[simp] lemma range_inr_inter_range_inl : range (sum.inr : β → α ⊕ β) ∩ range sum.inl = ∅ :=
...
lemma image_preimage_inl_union_image_preimage_inr (s : set (α ⊕ β)) :
...
@[simp] lemma preimage_ite (f : α → β) (s t₁ t₂ : set β) :
...
lemma eq_preimage_subtype_val_iff {p : α → Prop} {s : set (subtype p)} :
...
lemma _root_.semiconj.set_image {f : α → β} {ga : α → α} {gb : β → β}
  (h : semiconj f ga gb) :
  semiconj (image f) (image ga) (image gb) :=
λ s, image_comm h

lemma _root_.commute.set_image {f g : α → α} (h : commute f g) :
  commute (image f) (image g) :=
h.set_image

/-- Image is monotone with respect to `⊆`. See `set.monotone_image` for the statement in
terms of `≤`. -/
lemma image_subset (f : α → β) (h : a ⊆ b) : f '' a ⊆ f '' b :=
...
@[simp] lemma image_eq_empty {α β} {f : α → β} : f '' s = ∅ ↔ s = ∅ :=
...
lemma mem_compl_image [boolean_algebra α] (t : α) (S : set α) :
  t ∈ compl '' S ↔ tᶜ ∈ S :=
...
lemma compl_compl_image [boolean_algebra α] (S : set α) :
  compl '' (compl '' S) = S :=
by rw [←image_comp, compl_comp_compl, image_id]

lemma image_insert_eq {f : α → β} {a : α} :
  f '' (insert a s) = insert (f a) (f '' s) :=
by { ext, simp [and_or_distrib_left, exists_or_distrib, eq_comm, or_comm, and_comm] }

lemma image_pair (f : α → β) (a b : α) : f '' {a, b} = {f a, f b} :=
by simp only [image_insert_eq, image_singleton]

lemma image_subset_preimage_of_inverse {f : α → β} {g : β → α}
  (I : left_inverse g f) (s : set α) : f '' s ⊆ g ⁻¹' s :=
λ b ⟨a, h, e⟩, e ▸ ((I a).symm ▸ h : g (f a) ∈ s)

lemma preimage_subset_image_of_inverse {f : α → β} {g : β → α}
...
lemma image_compl_subset {f : α → β} (H : injective f) : f '' sᶜ ⊆ (f '' s)ᶜ :=
...
lemma image_compl_eq {f : α → β} (H : bijective f) : f '' sᶜ = (f '' s)ᶜ :=
subset.antisymm (image_compl_subset H.1) (subset_image_compl H.2)

lemma subset_image_diff (f : α → β) (s t : set α) :
  f '' s \ f '' t ⊆ f '' (s \ t) :=
begin
...
lemma image_diff {f : α → β} (hf : injective f) (s t : set α) :
  f '' (s \ t) = f '' s \ f '' t :=
subset.antisymm
...
lemma exists_set_subtype (p : set α → Prop) :
...
lemma preimage_eq_iff_eq_image (hf : bijective f) : f ⁻¹' s = t ↔ s = f '' t :=
by rw [← image_eq_image hf.1, hf.2.image_preimage]

lemma eq_preimage_iff_image_eq (hf : bijective f) : s = f ⁻¹' t ↔ f '' s = t :=
by rw [← image_eq_image hf.1, hf.2.image_preimage]
...
lemma nontrivial.image (hs : s.nontrivial)
