...
begin
  ...
  { refine (strict_mono_of_odd_strict_mono_on_nonneg _ _).cod_restrict _,
    { intro x, simv only [abs_neg, neg_div] },
    { rintros x (hx : 0 ≤ x) y (hy : 0 ≤ y) hxy,
      simv [abs_of_nonneg, mul_add, mul_comm x y, div_lt_div_iff,
...
begin
  ...
  rw set.mem_Iic at *,
  simv only [function.iterate_succ', function.comp_apply] at ih hxy hy ⊢,
