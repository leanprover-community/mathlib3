...
-- these lemmas don't especially belong in this file, but they have the same imports, which is rare.

lemma surj_on_Icc_of_strict_mono_surj
  (h_mono : strict_mono f) (h_surj : function.surjective f) (a b : α) :
  set.surj_on f (set.Icc a b) (set.Icc (f a) (f b)) :=
begin
  intros p hp,
  obtain ⟨x, hx⟩ := h_surj p,
  have hxa : a ≤ x := by simpa [hx.symm, h_mono.le_iff_le] using hp.1,
  have hxb : x ≤ b := by simpa [hx.symm, h_mono.le_iff_le] using hp.2,
  exact ⟨x, ⟨hxa, hxb⟩, hx⟩,
end

lemma surj_on_Ico_of_strict_mono_surj
  (h_mono : strict_mono f) (h_surj : function.surjective f) (a b : α) :
...
begin
  ...
  obtain ⟨x, hx⟩ := h_surj p,
  have hxa : a ≤ x := by simpa [hx.symm, h_mono.le_iff_le] using hp.1,
  have hxb : x < b := by simpa [hx.symm, h_mono.lt_iff_lt] using hp.2,
  exact ⟨x, ⟨hxa, hxb⟩, hx⟩,
end

lemma surj_on_Ioc_of_strict_mono_surj
  (h_mono : strict_mono f) (h_surj : function.surjective f) (a b : α) :
  set.surj_on f (set.Ioc a b) (set.Ioc (f a) (f b)) :=
begin
  convert @surj_on_Ico_of_strict_mono_surj _ _ _ _ _ (strict_mono.order_dual h_mono) h_surj b a;
  simp
end

lemma surj_on_Ioo_of_strict_mono_surj
  (h_mono : strict_mono f) (h_surj : function.surjective f) (a b : α) :
  set.surj_on f (set.Ioo a b) (set.Ioo (f a) (f b)) :=
begin
  intros p hp,
  obtain ⟨x, hx⟩ := h_surj p,
  have hxa : a < x := by simpa [hx.symm, h_mono.lt_iff_lt] using hp.1,
  have hxb : x < b := by simpa [hx.symm, h_mono.lt_iff_lt] using hp.2,
  exact ⟨x, ⟨hxa, hxb⟩, hx⟩,
end

lemma surj_on_Ici_of_strict_mono_surj
  (h_mono : strict_mono f) (h_surj : function.surjective f) (a : α) :
  set.surj_on f (set.Ici a) (set.Ici (f a)) :=
begin
  intros p hp,
  obtain ⟨x, hx⟩ := h_surj p,
  have hxa : a ≤ x := by rwa [← h_mono.le_iff_le, hx],
  exact ⟨x, hxa, hx⟩,
end

lemma surj_on_Iic_of_strict_mono_surj
  (h_mono : strict_mono f) (h_surj : function.surjective f) (a : α) :
  set.surj_on f (set.Iic a) (set.Iic (f a)) :=
@surj_on_Ici_of_strict_mono_surj _ _ _ _ _ (strict_mono.order_dual h_mono) h_surj a

lemma surj_on_Ioi_of_strict_mono_surj
  (h_mono : strict_mono f) (h_surj : function.surjective f) (a : α) :
  set.surj_on f (set.Ioi a) (set.Ioi (f a)) :=
begin
  intros p hp,
  obtain ⟨x, hx⟩ := h_surj p,
  have hxa : a < x := by rwa [← h_mono.lt_iff_lt, hx],
  exact ⟨x, hxa, hx⟩,
end

lemma surj_on_Iio_of_strict_mono_surj
  (h_mono : strict_mono f) (h_surj : function.surjective f) (a : α) :
  set.surj_on f (set.Iio a) (set.Iio (f a)) :=
@surj_on_Ioi_of_strict_mono_surj _ _ _ _ _ (strict_mono.order_dual h_mono) h_surj a
