...
{ prf  := λ l H,
    begin
      rw [← mem_range, range_map],
      exact λ a ha, can_lift.prf a (H a ha),
    end}

@[simp] lemma range_nth_le : set.range (λ k : fin l.length, l.nth_le k k.2) = {x | x ∈ l} :=
begin
  ext x,
...
lemma range_nth : set.range l.nth = insert none (some '' {x | x ∈ l}) :=
begin
  rw [← range_nth_le, ← range_comp],
...
@[simp] lemma range_nthd (d : α) : set.range (λ n, l.nthd n d) = insert d {x | x ∈ l} :=
...
... = insert d {x | x ∈ l} :
  by simp only [range_nth, image_insert_eq, option.get_or_else, image_image, image_id']
