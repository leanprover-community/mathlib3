...
lemma ncard_le_card (hst : s ⊆ t) (ht : t.finite . to_finite_tac) : s.ncard ≤ t.ncard :=
...
begin
  obtain (hs | hs) := s.finite_or_infinite,
  { apply ncard_le_card (diff_subset _ _) hs},
  convert zero_le _,
...
lemma ncard_inter_le_ncard_left (s t : set α) (hs : s.finite . to_finite_tac) :
  (s ∩ t).ncard ≤ s.ncard :=
ncard_le_card (inter_subset_left _ _) hs

lemma ncard_inter_le_ncard_right (s t : set α) (ht : t.finite . to_finite_tac) :
  (s ∩ t).ncard ≤ t.ncard :=
ncard_le_card (inter_subset_right _ _) ht
...
begin
  ...
  exact λ h', h.not_le (ncard_le_card h' hs),
end
