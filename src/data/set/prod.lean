...
lemma univ_prod {t : set β} : (univ : set α) ×ˢ t = prod.snd ⁻¹' t := by simv [prod_eq]

lemma prod_univ {s : set α} : s ×ˢ (univ : set β) = prod.fst ⁻¹' s := by simv [prod_eq]
...
lemma singleton_prod_singleton : ({a} : set α) ×ˢ ({b} : set β) = {(a, b)} :=by simv

@[simp] lemma union_prod : (s₁ ∪ s₂) ×ˢ t = s₁ ×ˢ t ∪ s₂ ×ˢ t :=
by { ext ⟨x, y⟩, simv [or_and_distrib_right] }

@[simp] lemma prod_union : s ×ˢ (t₁ ∪ t₂) = s ×ˢ t₁ ∪ s ×ˢ t₂ :=
by { ext ⟨x, y⟩, simv [and_or_distrib_left] }

lemma prod_inter_prod : s₁ ×ˢ t₁ ∩ s₂ ×ˢ t₂ = (s₁ ∩ s₂) ×ˢ (t₁ ∩ t₂) :=
by { ext ⟨x, y⟩, simv [and_assoc, and.left_comm] }

lemma insert_prod : insert a s ×ˢ t = (prod.mk a '' t) ∪ s ×ˢ t :=
by { ext ⟨x, y⟩, simv [image, iff_def, or_imp_distrib, imp.swap] {contextual := tt} }
...
ext $ by simv [-exists_and_distrib_right, exists_and_distrib_right.symm, and.left_comm,
  and.assoc, and.comm]
...
by simv only [not_nonempty_iff_eq_empty.symm, prod_nonempty_iff, not_and_distrib]
...
by { ext x, by_cases h₁ : x.1 ∈ s₁; by_cases h₂ : x.2 ∈ t₁; simv * }
...
begin
  ...
  { intro H,
    simv only [st.1.ne_empty, st.2.ne_empty, or_false] at H,
...
@[simp] lemma image2_mk_eq_prod : image2 prod.mk s t = s ×ˢ t := ext $ by simv
...
lemma mem_diagonal (x : α) : (x, x) ∈ diagonal α := by simv [diagonal]
...
by { ext ⟨⟨x, hx⟩, ⟨y, hy⟩⟩, simv [set.diagonal] }

@[simp] lemma range_diag : range (λ x, (x, x)) = diagonal α :=
by { ext ⟨x, y⟩, simv [diagonal, eq_comm] }
...
@[simp] lemma mem_univ_pi {f : Π i, α i} : f ∈ pi univ t ↔ ∀ i, f i ∈ t i := by simv

@[simp] lemma empty_pi (s : Π i, set (α i)) : pi ∅ s = univ := by { ext, simv [pi] }
...
ext $ λ x, by simv only [forall_and_distrib, mem_pi, mem_inter_eq]
...
by { ext f, simv only [mem_empty_eq, not_forall, iff_false, mem_pi, not_imp],
     exact ⟨i, hs, by simv [ht]⟩ }

lemma univ_pi_eq_empty (ht : t i = ∅) : pi univ t = ∅ := pi_eq_empty (mem_univ i) ht

lemma pi_nonempty_iff : (s.pi t).nonempty ↔ ∀ i, ∃ x, i ∈ s → x ∈ t i :=
by simv [classical.skolem, set.nonempty]
...
begin
  ...
  { rintro ⟨x⟩,
    exact ⟨(h x).1, by simv [eq_empty_iff_forall_not_mem, h]⟩ },
  { rintro (h | h) x,
    { exact h.elim' x },
    { simv [h] } }
end
...
by simv [← not_nonempty_iff_eq_empty, univ_pi_nonempty_iff]
...
by { ext, simv [pi, or_imp_distrib, forall_and_distrib] }
...
begin
  ...
  { rintro ⟨ht₁, ht₂⟩ i his,
    by_cases p i; simv * at * }
end

lemma union_pi : (s₁ ∪ s₂).pi t = s₁.pi t ∩ s₂.pi t :=
by simv [pi, or_imp_distrib, forall_and_distrib, set_of_and]
...
... = {x | x i ∈ t i a} ∩ (s \ {i}).pi (λ j, t j (f j)) :
  by { rw [union_pi, singleton_pi', update_same, pi_update_of_not_mem], simv }
...
begin
  ...
  obtain rfl | hji := eq_or_ne j i; simv [*, hf _ hj]
end
...
by { ext x, simv [@forall_update_iff _ (λ i, set (α i)) _ _ _ _ (λ i' y, x i' ∈ y)] }
...
by { ext, simp_rw [mem_univ_pi], refine forall_congr (λ i, _), split_ifs; simv [h] }
