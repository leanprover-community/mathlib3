...
/--
An attempt at the proper recursor for `sym α n`, using vectors to represent a given element.
-/
@[reducible, recursor 6]
protected def rec {α : Type u} {β : sym α n → Sort v}
   (f : Π (a : vector α n), β ↑a)
   (h : ∀ (a b : vector α n) (p : a.val ~ b.val), (eq.rec (f a) (sound p) : β ↑b) = f b)
   (q : sym α n) : β q :=
begin
  cases q with m h, induction m, change m.length = n at h,
  have key : (⟨quot.mk setoid.r m, h_1⟩ : sym α n) = of_vector ⟨m, h_1⟩ := rfl,
  rw key, apply f,
  ext hb,
  change m_b.length = n at hb,
  dsimp,
  have ha := list.perm.length_eq p, rw hb at ha,
  have h' := h ⟨m_a, ha⟩ ⟨m_b, hb⟩ p,
  set q := quot.sound p,
  have q' : (↑m_a : multiset α) = ↑m_b := q,
  refine eq.trans _ h',
  dsimp,
  have key : ∀ {m_b hb e}, @eq.rec (multiset α) ↑m_a
      (λ (m : multiset α), Π (h : m.card = n), β ⟨m, h⟩)
      (λ (h_1 : m_a.length = n), f ⟨m_a, h_1⟩)
      m_b
      e
      hb =
    @eq.rec (sym α n) (of_vector ⟨m_a, ha⟩) β
      (f ⟨m_a, ha⟩)
      ⟨m_b, hb⟩
      ((λ e, _) e),
  rotate 2, {
    dsimp [of_vector], tidy,
  },
  { intros m_b hb e, subst m_b, subst n, tidy, },
  exact key,
end
...
lemma cons_equiv_eq_equiv_cons (α : Type*) (n : ℕ) (a : α) (s : sym α n) : a :: sym_equiv_sym' s = sym_equiv_sym' (a :: s) :=
by tidy
