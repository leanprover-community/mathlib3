...
def of {X : Type*} (bipointing : X × X) : Bipointed := ⟨X, bipointing⟩
...
(to_fun : X → Y)
(map_fst : to_fun X.bipointing.1 = Y.bipointing.1)
(map_snd : to_fun X.bipointing.2 = Y.bipointing.2)
...
instance : large_category Bipointed :=
{ hom := hom,
  id := hom.id,
  comp := @hom.comp,
...
instance : concrete_category Bipointed :=
