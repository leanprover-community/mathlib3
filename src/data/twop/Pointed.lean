...
@[simps] def id (X : Pointed) : Pointed.hom X X := ⟨id, rfl⟩

instance (X : Pointed) : inhabited (Pointed.hom X X) := ⟨id X⟩

/-- Composition of morphisms of `Pointed`. -/
@[simps] def comp {X Y Z : Pointed.{u}} (f : Pointed.hom X Y) (g : Pointed.hom Y Z) :
  Pointed.hom X Z :=
...
{ forget := { obj := Pointed.X, map := @Pointed.hom.to_fun },
