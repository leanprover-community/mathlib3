...
* `zmod.lift n f hf` is the additive homomorphism `zmod n →+ A` induced by an
  additive hom `f : ℤ →+ A` such that `hf : f n = 0`.
-/
...
{ to_fun := ideal.quotient.lift _ (int.cast_ring_hom _)
  ...
  left_inv := λ x, quotient.induction_on' x (λ x, ideal.quotient.eq.mpr $
    by rw [submodule.quotient.mk'_eq_mk, ideal.quotient.mk_eq_mk, ideal.quotient.lift_mk,
           int.coe_cast_ring_hom, ideal.mem_span_singleton, ← int_coe_zmod_eq_zero_iff_dvd,
           int.cast_sub, int_cast_zmod_cast, sub_self]),
  right_inv := λ x,
    by rw [function.comp_app, ideal.quotient.lift_mk, int.coe_cast_ring_hom, int_cast_zmod_cast],
  .. ideal.quotient.lift _ _ _ }

lemma span_nat_abs (a : ℤ) : ideal.span ({a.nat_abs} : set ℤ) = ideal.span {a} :=
by { rw ideal.span_singleton_eq_span_singleton, exact (int.associated_nat_abs _).symm }

/-- Quotienting by equal ideals gives equivalent rings. -/
def ideal.quot_equiv_of_eq {R : Type*} [comm_ring R] {I J : ideal R} (h : I = J) :
...
(ideal.quot_equiv_of_eq (span_nat_abs a)).symm.trans
  (int.quotient_span_nat_equiv_zmod a.nat_abs)
