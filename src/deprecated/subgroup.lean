...
/-- `gpowers x` is the subset of `G` correponding to the subgroup generated by `x`. -/
def gpowers (x : G) : set G := set.range ((^) x : ℤ → G)

/-- `gmultiples x` is the subset of `A` correponding to the additive subgroup generated by `x`. -/
