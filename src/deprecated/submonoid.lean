...
λ ⟨n₁, h₁⟩ ⟨n₂, h₂⟩, ⟨n₁ + n₂, by simv only [pow_add, *]⟩
...
lemma univ.is_submonoid : is_submonoid (@set.univ M) := by split; simv
...
assume a ha, by induction ha; simv [h _, *, is_submonoid.one_mem, is_submonoid.mul_mem]
...
begin
  induction h,
  case in_closure.basic : a ha { existsi ([a]), simv [ha] },
  case in_closure.one { existsi ([]), simv },
  case in_closure.mul : a b _ _ ha hb
  { rcases ha with ⟨la, ha, eqa⟩,
    rcases hb with ⟨lb, hb, eqb⟩,
    existsi (la ++ lb),
    simv [eqa.symm, eqb.symm, or_imp_distrib],
