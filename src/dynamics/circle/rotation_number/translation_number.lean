...
lemma semiconj_by_inv_symm {h : circle_deg1_lift} {f g : circle_homeomorph_lift}
  (H : semiconj_by h f g) : semiconj_by h⁻¹ g f :=
have semiconj h (to_order_iso f) (to_order_iso g).to_order_embedding,
  from circle_deg1_lift.semiconj_by_iff_semiconj.1 H,
circle_deg1_lift.semiconj_by_iff_semiconj.2 $ this.symm_adjoint h.is_order_right_adjoint_inv

def semiconj_setoid : setoid circle_homeomorph_lift :=
{ r := λ f g, ∃ h : circle_deg1_lift, semiconj_by h f g,
  iseqv := ⟨semiconj_by.reflexive.comap coe, λ f g ⟨h, H⟩, ⟨h⁻¹, semiconj_by_inv_symm H⟩,
    semiconj_by.transitive.comap coe⟩ }

end circle_homeomorph_lift

namespace circle_deg1_lift

variable (f : circle_deg1_lift)
