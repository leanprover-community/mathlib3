...
open filter encodable set emetric ennreal

section Baire_theorem_refactor

variables {α : Type*} {β : Type*} {γ : Type*} {ι : Type*}


/-- The property `baire_space α` means that the topological space `α` has the Baire property: 
any countable intersection of open dense subsets is dense. Formulated here when the source space is ℕ 
(and subsumed below by `dense_Inter_of_open` working with any encodable source space).-/
...
theorem dense_Inter_of_open_nat [topological_space α] [baire_space α] {f : ℕ → set α} (ho : ∀ n, is_open (f n)) (hd : ∀ n, dense (f n)) :
  dense (⋂ n, f n) :=
begin
...
theorem baire_category_theorem_emetric_complete [pseudo_emetric_space α] [complete_space α] : baire_space α :=
{ baire_property :=
  begin
    intros f ho hd,
...
theorem baire_category_theorem_locally_compact [topological_space α] [t2_space α] [locally_compact_space α] : baire_space α :=
{ baire_property :=
  begin
    intros f ho hd,
    have nonempty_open_has_compact : ∀ (V : set α), is_open V → V.nonempty → ∃ (F : set α),  is_compact F ∧ (interior F).nonempty ∧ F ⊆ V :=
    begin
    ...
    begin
      ...
      specialize h ((interior (compact_seq n)) ∩ (f n))
        (is_open.inter is_open_interior (ho n))
        (dense_iff_inter_open.1 (hd n) (interior (compact_seq n)) is_open_interior (compact_seq_is_compact_nonempty n).2),
      replace h := h.2.2,
      simp at h,
...
theorem dense_sInter_of_open {S : set (set α)} (ho : ∀s∈S, is_open s) (hS : countable S) (hd : ∀s∈S, dense s) :
  dense (⋂₀S) :=
begin
...
theorem dense_bInter_of_open {S : set β} {f : β → set α} (ho : ∀s∈S, is_open (f s)) (hS : countable S) (hd : ∀s∈S, dense (f s)) :
  dense (⋂s∈S, f s) :=
begin
  rw ← sInter_image,
...
theorem dense_sInter_of_Gδ {S : set (set α)} (ho : ∀s∈S, is_Gδ s) (hS : countable S) (hd : ∀s∈S, dense s) :
  dense (⋂₀S) :=
begin
...
/-- Baire theorem: a countable intersection of dense Gδ sets is dense. Formulated here with
an index set which is a countable set in any type. -/
theorem dense_bInter_of_Gδ {S : set β} {f : Π x ∈ S, set α} (ho : ∀s∈S, is_Gδ (f s ‹_›)) (hS : countable S) (hd : ∀s∈S, dense (f s ‹_›)) :
  dense (⋂s∈S, f s ‹_›) :=
begin
  rw bInter_eq_Inter,
...
theorem dense.inter_of_Gδ {s t : set α} (hs : is_Gδ s) (ht : is_Gδ t) (hsc : dense s) (htc : dense t) :
  dense (s ∩ t) :=
begin
  rw [inter_eq_Inter],
  apply dense_Inter_of_Gδ _,
  repeat { simp [bool.forall_bool, *] },
  exact bool.encodable,
end
...
theorem dense_bUnion_interior_of_closed {S : set β} {f : β → set α} (hc : ∀s∈S, is_closed (f s)) (hS : countable S) (hU : (⋃s∈S, f s) = univ) :
...
theorem dense_sUnion_interior_of_closed {S : set (set α)} (hc : ∀s∈S, is_closed s) (hS : countable S) (hU : (⋃₀ S) = univ) :
...
theorem dense_Union_interior_of_closed [encodable β] {f : β → set α} (hc : ∀s, is_closed (f s)) (hU : (⋃s, f s) = univ) :
  dense (⋃s, interior (f s)) :=
begin
  rw ← bUnion_univ,
...
theorem nonempty_interior_of_Union_of_closed [nonempty α] [encodable β] {f : β → set α} (hc : ∀s, is_closed (f s)) (hU : (⋃s, f s) = univ) :
