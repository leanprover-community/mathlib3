...
begin
  ...
  have hs₁ : ∀ (i : ι) (k : ι →₀ ℕ), k ∈ (ps i).support →
    ...
    from λ i k hk, algebra.subset_adjoin
      (finset.mem_union_left _ (finset.mem_bUnion.2
        ⟨i, finset.mem_univ _, finset.mem_image_of_mem _ hk⟩)),
  have hs : ∀ i, mv_polynomial.eval v (ps i) ∈ algebra.adjoin K (s : set R),
    from λ i, mv_polynomial.eval_mem (hs₁ _) hv,
  ...
  let res : (ι → algebra.adjoin K (s : set R)) →
    ...
    λ x i, ⟨mv_polynomial.eval (λ j : ι, (x j : R)) (ps i),
      mv_polynomial.eval_mem (hs₁ _) (λ i, (x i).2)⟩,
