...
begin
  ...
  { rw eq_univ_iff_forall at hd', push_neg at hd', rcases hd' with ⟨i₀, hi₀⟩,
    let stratified : finset (σ → α) := univ.bind (λ a : α, univ.filter (λ x, x i₀ = a)),
    suffices : stratified = univ,
    { rw [← this, sum_bind],
      { suffices aux : (λ (a : α), (univ.filter (λ (x : σ → α), x i₀ = a)).sum
            (λ (x : σ → α), finset.prod (d.support) (λ (j : σ), x j ^ d j))) =
          (λ (a : α), (univ.filter (λ (x : σ → α), x i₀ = 0)).sum
            (λ (x : σ → α), finset.prod (d.support) (λ (j : σ), x j ^ d j))),
