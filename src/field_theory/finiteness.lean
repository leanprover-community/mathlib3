...
begin
  ...
  rw [← b.mk_eq_dim'', lt_aleph_0_iff_finite_set],
  split,
  { introI,
...
lemma finite_basis_index {ι : Type*} [is_noetherian K V] (b : basis ι K V) :
  _root_.finite ι :=
...
instance finite_basis.of_vector_space_index [is_noetherian K V] :
  finite (basis.of_vector_space_index K V) :=
finite_basis_index (basis.of_vector_space K V)

/-- In a noetherian module over a division ring,
if a basis is indexed by a set, that set is finite. -/
lemma finite_set_basis_index {ι : Type*} {s : set ι} [is_noetherian K V] (b : basis s K V) :
  s.finite :=
b.finite_set_index_of_dim_lt_aleph_0 (dim_lt_aleph_0 K V)
...
noncomputable def finset_basis_index [is_noetherian K V] :
  finset V :=
(finite_set_basis_index (basis.of_vector_space K V)).to_finset
...
begin
  ...
  { rintros ⟨s, hs⟩,
    ...
    exact lt_of_le_of_lt (dim_span_le _) (lt_aleph_0_of_finite _) }
end
