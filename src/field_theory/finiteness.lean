...
-/
lemma iff_rank_lt_omega : is_noetherian K V ↔ module.rank K V < omega.{v} :=
begin
  let b := basis.of_vector_space K V,
  rw [← b.mk_eq_rank'', lt_omega_iff_finite],
  split,
  { introI,
...
lemma rank_lt_omega : ∀ [is_noetherian K V], module.rank K V < omega.{v} :=
is_noetherian.iff_rank_lt_omega.1
...
noncomputable def fintype_basis_index {ι : Type*} [is_noetherian K V] (b : basis ι K V) :
  fintype ι :=
b.fintype_index_of_rank_lt_omega (rank_lt_omega K V)
...
lemma finite_basis_index {ι : Type*} {s : set ι} [is_noetherian K V] (b : basis s K V) :
  s.finite :=
b.finite_index_of_rank_lt_omega (rank_lt_omega K V)
...
begin
  ...
  { rintros ⟨s, hs⟩,
    rw [is_noetherian.iff_rank_lt_omega, ← rank_top, ← hs],
    exact lt_of_le_of_lt (rank_span_le _) (lt_omega_iff_finite.2 (set.finite_mem_finset s)) }
end
