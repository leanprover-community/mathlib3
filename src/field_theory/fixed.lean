...
@[simp] lemma coe_to_alg_hom_over_fixed_points (G : Type u) (F : Type v) [group G] [field F]
  [faithful_mul_semiring_action G F] :
  (to_alg_hom_over_fixed_points G F : G → F →ₐ[fixed_points G F] F) =
    λ g, { commutes' := λ x, x.2 g,
      .. mul_semiring_action.to_semiring_hom G F g } :=
rfl

lemma to_alg_hom_over_fixed_points_apply {G : Type u} {F : Type v} [group G] [field F]
  [faithful_mul_semiring_action G F] (g : G) (x : F) :
  to_alg_hom_over_fixed_points G F g x = g • x :=
rfl
