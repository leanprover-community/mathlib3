...
have h : val (1 : α) * val (1 : α) = val (1 : α), by rw[←valued_ring.val_mul, mul_one],
...
have val (-1 : α) * val (-1 : α) = 1, from
calc val (-1 : α) * val (-1 : α) = val ((-1 : α) * -1) : by rw[←val_mul]
                             ... = val (1 : α) : by rw[neg_one_mul, neg_neg]
                             ... = 1 : val_one,
or.resolve_right
...
instance : valued_ring (valuation_ring α) :=
...
/--
A *henselian field* is a valued field `α` such that any irreducible polyomial over this field .
-/
