...
lemma ultrafilter.generator_of_pushforward_comp_inclusion (f : ultrafilter (L →ₐ[K] L))
  ...
  (f.generator_of_pushforward hF).comp (subalgebra.inclusion hEF) =
  ultrafilter.generator_of_pushforward hE f :=
begin
  refine ultrafilter.pure_injective _,
  transitivity map (λ ϕ : ↥F →ₐ[K] L, ϕ.comp $ subalgebra.inclusion hEF)
  (pure $ f.generator_of_pushforward hF),
  { refl },
  rw [←f.generator_of_pushforward_spec hE, ←f.generator_of_pushforward_spec hF],
  refl,
end
...
f.generator_of_pushforward (intermediate_field.adjoin.finite_dimensional $ h_int x)
...
(f : ultrafilter (L →ₐ[K] L))
  ...
  f.glued_generators_of_pushforwards_function h_int x = f.generator_of_pushforward hE ⟨x, hx⟩ :=
...
lemma ultrafilter.glued_generators_of_pushforwards_function_spec' (h_int : algebra.is_integral K L)
  ...
  f.glued_generators_of_pushforwards_function h_int x = f.generator_of_pushforward hE x :=
...
{ to_fun := ultrafilter.glued_generators_of_pushforwards_function h_int f,
  ...
  begin
    ...
    exact map_mul (f.generator_of_pushforward hE) ⟨x, hx⟩ ⟨y, hy⟩,
  end,
  map_add' := λ x y,
  begin
    let E := intermediate_field.adjoin K ({x, y} : set L),
    ...
    exact map_add (f.generator_of_pushforward hE) ⟨x, hx⟩ ⟨y, hy⟩,
  end,
  commutes' := λ r,
  begin
...
lemma ultrafilter.glued_generators_of_pushforwards_alg_hom_bijection (h_int : algebra.is_integral
  K L)
  (f : ultrafilter (L →ₐ[K] L)) :
function.bijective (ultrafilter.glued_generators_of_pushforwards_alg_hom f h_int) :=
