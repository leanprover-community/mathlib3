...
lemma ultrafilter.alg_hom_spec (h_findim : finite_dimensional K E) (f : ultrafilter (L →ₐ[K] L)) :
  f.map (λ σ : L →ₐ[K] L, σ.comp $ intermediate_field.val E) =
...
lemma alg_hom_of_ultrafilter_injective (h_int : algebra.is_integral K L)
  (f : ultrafilter (L →ₐ[K] L)) : function.injective (alg_hom_of_ultrafilter f h_int) :=
begin
  intros x y hxy,
  let E := intermediate_field.adjoin K ({x, y} : set L),
  let S := ({x, y} : finset L),
  ...
  simpa using ring_hom.injective (f.alg_hom hE).to_ring_hom hxy,
end
...
begin
  ...
  { rintros x ⟨a, ha, hax⟩,
    rw ← hax,
    simp only [finset.mem_coe, multiset.mem_to_finset],
    rw polynomial.mem_roots,
    { rw [polynomial.is_root.def, ← polynomial.eval₂_eq_eval_map,
        ← polynomial.alg_hom_eval₂_algebra_map, (show 0 = σ 0, by simp)],
      apply congr_arg σ,
      rwa [finset.mem_coe, multiset.mem_to_finset, polynomial.mem_roots, polynomial.is_root.def,
        ← polynomial.eval₂_eq_eval_map] at ha,
  ...
  have h2 : intermediate_field.adjoin K (σ '' S) ≤ E := intermediate_field.gc.l_le hσSE,
  rw ← h1 at h2,
  have h3 := eq_of_map_le hE_findim h2,
  have hyE : y ∈ E,
  { have hyS : y ∈ S,
    { rw [multiset.mem_to_finset, polynomial.mem_roots],
      { rw [polynomial.is_root.def, ← polynomial.eval₂_eq_eval_map, ← polynomial.aeval_def],
...
lemma alg_hom_of_ultrafilter_bijection (h_int : algebra.is_integral K L)
  (f : ultrafilter (L →ₐ[K] L)) :
function.bijective (alg_hom_of_ultrafilter f h_int) :=
⟨alg_hom_of_ultrafilter_injective h_int f, alg_hom_of_ultrafilter_surjective h_int f⟩
...
begin
  ...
  rw h,
  exact ultrafilter.function_spec' _ _ _ _,
end
...
(f : filter (L ≃ₐ[K] L)) ≤ nhds (equiv_of_ultrafilter h_int (f.map (λ (σ : L ≃ₐ[K] L),
σ.to_alg_hom))) :=
begin
  let σ := equiv_of_ultrafilter h_int (f.map (λ (e : L ≃ₐ[K] L), e.to_alg_hom)),
  intros A hA,
  ...
  { have h : p σ = (f.map (λ (e : L ≃ₐ[K] L), e.to_alg_hom)).alg_hom h_findim :=
    equiv_restricts_to_alg_hom_of_finite_dimensional h_int
    (f.map (λ (e : L ≃ₐ[K] L), e.to_alg_hom)) h_findim,
    rw h,
    have h2 : f.map p = pure (((f.map (λ (e : L ≃ₐ[K] L), e.to_alg_hom))).alg_hom h_findim) :=
    (f.map (λ (e : L ≃ₐ[K] L), e.to_alg_hom)).alg_hom_spec h_findim,
    ext,
    split,
    { intro hs,
    ...
    { intro hs,
      ...
      have h3 : s ∈ (pure ((f.map (λ (e : L ≃ₐ[K] L), e.to_alg_hom)).alg_hom h_findim) :
      ultrafilter (↥E →ₐ[K] L)),
...
is_compact (set.univ : set (L ≃ₐ[K] L)) := is_compact_iff_ultrafilter_le_nhds.2
  (λ f _,  ⟨equiv_of_ultrafilter h_int (f.map (λ (σ : L ≃ₐ[K] L), σ.to_alg_hom)),
  set.mem_univ (equiv_of_ultrafilter h_int (f.map (λ (σ : L ≃ₐ[K] L), σ.to_alg_hom))),
