...
begin
  by_cases hp : p = 0,
  { letI : is_splitting_field F F p, { rw hp, exact ⟨splits_zero _, subsingleton.elim _ _⟩ },
  ...
  letI hFE : finite_dimensional F E := is_splitting_field.finite_dimensional E p,
  have Hx : is_integral F x := is_integral_of_noetherian (is_noetherian.iff_fg.2 hFE) x,
  refine ⟨Hx, or.inr _⟩,
  rintros q q_irred ⟨r, hr⟩,
  let D := adjoin_root q,
  letI := fact.mk q_irred,
  let pbED := adjoin_root.power_basis q_irred.ne_zero,
  letI : finite_dimensional E D := power_basis.finite_dimensional pbED,
  ...
  letI : is_scalar_tower F E D := of_algebra_map_eq (λ _, rfl),
  letI : finite_dimensional F D := finite_dimensional.trans F E D,
  ...
  letI Hx_irred := fact.mk (minpoly.irreducible Hx),
  ...
  letI : is_scalar_tower F C D := of_algebra_map_eq (λ x, (adjoin_root.lift_of _).symm),
  letI : is_scalar_tower F C E := of_algebra_map_eq (λ x, (adjoin_root.lift_of _).symm),
