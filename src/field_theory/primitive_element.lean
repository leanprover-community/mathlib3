...
lemma primitive_element_two_inf (F_sep : is_separable F E) :
...
variables {F E : Type u} [field F] [field E] [algebra F E] [finite_dimensional F E]

/-- Primitive element theorem for infinite fields in the same universe. -/
theorem primitive_element_inf (F_sep : is_separable F E) (F_inf : infinite F)
...
theorem primitive_element_aux (F_sep : is_separable F E) : ∃ α : E, F⟮α⟯ = ⊤ :=
begin
...
begin
  ...
  obtain ⟨α, hα⟩ := primitive_element_aux F'_sep,
  have : (F'⟮α⟯ : set E) = F⟮α⟯,
