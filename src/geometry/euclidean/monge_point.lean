...
mk' (({i₁, i₂}ᶜ : finset (fin (n + 3))).centroid ℝ s.points)
  (ℝ ∙ (s.points i₁ -ᵥ s.points i₂)).orthogonal ⊓
...
lemma direction_monge_plane {n : ℕ} (s : simplex ℝ P (n + 2)) {i₁ i₂ : fin (n + 3)} (h : i₁ ≠ i₂) :
  (s.monge_plane i₁ i₂).direction = (ℝ ∙ (s.points i₁ -ᵥ s.points i₂)).orthogonal ⊓
...
begin
  ...
  have h' : ∀ i₂, i₁ ≠ i₂ → p -ᵥ s.monge_point ∈
    (ℝ ∙ (s.points i₁ -ᵥ s.points i₂)).orthogonal ⊓ vector_span ℝ (set.range s.points),
  { intros i₂ hne,
