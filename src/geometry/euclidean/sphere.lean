...
lemma mul_norm_eq_abs_sub_sq_norm_of_smul_and_norm_eq_norm {x y z : V}
...
begin
  ...
  calc  abs (∥z + y∥ ^ 2 - ∥z - x∥ ^ 2)
      = abs (∥x∥ ^ 2 - ∥y∥ ^ 2)            : by simp [norm_add_pow_two_real, norm_sub_pow_two_real,
                                                    hzy, hzx, abs_sub]
  ... = abs (r ^ 2 * ∥y∥ ^ 2 - ∥y∥ ^ 2)    : by rw [hxy, norm_smul, mul_pow, norm_eq_abs, sqr_abs]
...
`AP * BP = abs (BQ ^ 2 - PQ ^ 2)`. -/
lemma mul_dist_eq_abs_sub_sq_dist_of_smul_and_dist_eq_dist {a b p q : P}
...
begin
  ...
  exact mul_norm_eq_abs_sub_sq_norm_of_smul_and_norm_eq_norm hp hq,
end

/-- If `A`, `B`, `C`, `D` are cospherical and `P` is on both lines `AB` and `CD`, then
`AP * BP = CP * DP`. -/
lemma mul_dist_eq_mul_dist_of_cospherical_and_smul {a b c d p : P}
...
begin
  ...
  { rw ← hd at hc,
    rw ← hb at ha,
    rw [mul_dist_eq_abs_sub_sq_dist_of_smul_and_dist_eq_dist hapb ha, hb,
        mul_dist_eq_abs_sub_sq_dist_of_smul_and_dist_eq_dist hcpd hc, hd] },
  all_goals { simp },
end

/-- Intersecting Chords Theorem. -/
theorem mul_dist_eq_mul_dist_of_cospherical_and_angle_eq_pi {a b c d p : P}
  (h : cospherical ({a, b, c, d} : set P))
...
begin
  ...
  exact mul_dist_eq_mul_dist_of_cospherical_and_smul
          h ⟨k₁, (by linarith), hab⟩ ⟨k₂, (by linarith), hcd⟩,
end

/-- Intersecting Secants Theorem. -/
theorem mul_dist_eq_mul_dist_of_cospherical_and_angle_eq_zero {a b c d p : P}
  (h : cospherical ({a, b, c, d} : set P))
...
begin
  ...
  refine mul_dist_eq_mul_dist_of_cospherical_and_smul h ⟨k₁, _, hab₁⟩ ⟨k₂, _, hcd₁⟩;
  by_contra hnot;
