...
it, we get a space `Z.to_topological_vector_bundle_core.total_space` (which as a Type is just `Σ (x :
...
(coord_change_smooth : ∀ i j : atlas H M, times_cont_diff_on 𝕜 ∞ (λ p : E × F, coord_change i j
  (I.symm p.1) p.2) ((I '' (i.1.symm.trans j.1).source).prod (univ : set F)))
...
{ base_set := λ i, i.1.source,
  ...
  coord_change_continuous := λ i j, begin
    have A : continuous_on (λ p : E × F, Z.coord_change i j (I.symm p.1) p.2)
    ...
    have C : continuous_on (λ p : M × F, (⟨I (i.1 p.1), p.2⟩ : E × F))
...
begin
  ...
  { assume e e' he he',
    ...
    { -- the coordinate change on the base is just a coordinate change for `M`, smooth since
      ...
      have B : times_cont_diff_on 𝕜 ∞ (λ p : E × F, p.1)
        ((I.symm ⁻¹' (e.symm.trans e').source ∩ range I).prod univ) :=
