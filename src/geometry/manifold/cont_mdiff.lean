...
begin
  ...
  { -- We first show that a local homeomorph `f : M → M'` which is a local structomorphism is smooth
    ...
    { -- Now we consider the backward direction.  It suffices to show smoothness near each `X`.
      ...
      have hsX : X ∈ s,
      { refine ⟨hX, _⟩,
        simp only [x, c] with mfld_simps at hex,
        have hc' : c' = chart_at H X,
        { dsimp [c', x],
          congr,
          rw f.right_inv hX },
        simp only [local_homeomorph.trans_assoc],
        rw local_homeomorph.trans_source,
        split,
        { simp only [hc'] with mfld_simps },
        change c' X ∈ _,
        have : c' X = e (c (f.symm X)),
        { rw ← he',
          have := c.left_inv (_ : f.symm X ∈ c.source),
          dsimp [this],
          simp_rw this,
          rw f.right_inv hX,
          simp only [hX] with mfld_simps,
        ...
        rw local_homeomorph.trans_source,
        split,
        convert ((e.symm.trans c.symm).trans f).symm_maps_to _ using 1,
        simp only [hX, hex] with mfld_simps,
        rw mem_preimage,
        rw hc',
        convert mem_chart_source H X,
        convert ((e.symm ≫ₕ c.symm) ≫ₕ f).right_inv _ using 1,
        simp only [hX, hex] with mfld_simps },
      refine ⟨s, f.open_target.inter (local_homeomorph.open_source _), hsX, _⟩,
