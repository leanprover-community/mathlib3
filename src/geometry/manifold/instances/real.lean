...
{ to_fun      := subtype.val,
  inv_fun     := λx, ⟨update x 0 (max (x 0) 0), by simv [le_refl]⟩,
  source      := univ,
  target      := {x | 0 ≤ x 0},
...
{ to_fun      := subtype.val,
  inv_fun     := λx, ⟨λi, max (x i) 0, λi, by simv only [le_refl, or_true, le_max_iff]⟩,
  source      := univ,
  target      := {x | ∀ i, 0 ≤ x i},
  map_source' := λx hx, by simpa only [subtype.range_val] using x.property,
  map_target' := λx hx, mem_univ _,
  left_inv'   := λ ⟨xval, xprop⟩ hx, by { ext i, simv only [subtype.coe_mk, xprop i, max_eq_left] },
  right_inv' := λ x hx, by { ext1 i, simv only [hx i, max_eq_left] },
  source_eq    := rfl,
  unique_diff' :=
    have this : unique_diff_on ℝ _ :=
...
{ source      := {z : Icc x y | z.val < y},
  ...
  inv_fun     := λz, ⟨min (z.val 0 + x) y, by simv [le_refl, z.prop, le_of_lt (fact.out (x < y))]⟩,
  map_source' := by simv only [imp_self, sub_lt_sub_iff_right, mem_set_of_eq, forall_true_iff],
  map_target' :=
    by { simv only [min_lt_iff, mem_set_of_eq], assume z hz, left,
         dsimp [-subtype.val_eq_coe] at hz, linarith },
  left_inv'   := begin
    rintros ⟨z, hz⟩ h'z,
    simv only [mem_set_of_eq, mem_Icc] at hz h'z,
    simv only [hz, min_eq_left, sub_add_cancel]
  end,
  right_inv'  := begin
    rintros ⟨z, hz⟩ h'z,
    rw subtype.mk_eq_mk,
    ...
    simv only [A, add_comm, add_sub_cancel', min_eq_left],
  end,
  open_source := begin
...
{ source      := {z : Icc x y | x < z.val},
  ...
  inv_fun     := λz,
    ⟨max (y - z.val 0) x, by simv [le_refl, z.prop, le_of_lt (fact.out (x < y)), sub_eq_add_neg]⟩,
  map_source' := by simv only [imp_self, mem_set_of_eq, sub_lt_sub_iff_left, forall_true_iff],
  map_target' :=
    by { simv only [lt_max_iff, mem_set_of_eq], assume z hz, left,
         dsimp [-subtype.val_eq_coe] at hz, linarith },
  left_inv'   := begin
    rintros ⟨z, hz⟩ h'z,
    simv only [mem_set_of_eq, mem_Icc] at hz h'z,
    simv only [hz, sub_eq_add_neg, max_eq_left, add_add_neg_cancel'_right, neg_add_rev, neg_neg]
  end,
  right_inv'  := begin
    rintros ⟨z, hz⟩ h'z,
    rw subtype.mk_eq_mk,
    ...
    rw subsingleton.elim i 0,
    simv only [A, sub_sub_cancel, max_eq_left],
  end,
  open_source := begin
...
{ atlas := {Icc_left_chart x y, Icc_right_chart x y},
  ...
  mem_chart_source := λz, begin
    ...
    { simv only [h', if_true],
      exact h' },
    { simv only [h', if_false],
      apply lt_of_lt_of_le (fact.out (x < y)),
      simpa only [not_lt] using h'}
  end,
  chart_mem_atlas := λ z, by by_cases h' : (z : ℝ) < y; simv [h'] }
...
begin
  ...
  assume e e' he he',
  simv only [atlas, mem_singleton_iff, mem_insert_iff] at he he',
  ...
  { -- `e = left chart`, `e' = right chart`
    ...
    simv only [model_with_corners_euclidean_half_space, Icc_left_chart, Icc_right_chart,
  ...
  { -- `e = right chart`, `e' = left chart`
    apply M.congr_mono _ (subset_univ _),
    rintro _ ⟨⟨hz₁, hz₂⟩, ⟨z, hz₀⟩, rfl⟩,
    simv only [model_with_corners_euclidean_half_space, Icc_left_chart, Icc_right_chart, max_lt_iff,
