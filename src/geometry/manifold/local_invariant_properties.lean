...
-- move this
lemma foo {H : Type*} [topological_space H] {G : structure_groupoid H} {f : local_homeomorph H H}
  (hf : f ∈ G) :
  f ∈ maximal_atlas H G :=
begin
...
lemma lift_prop_at_of_mem_groupoid (hG : G.local_invariant_prop G Q) (hQ : ∀ y, Q id univ y)
  ...
  lift_prop_at Q f x :=
lift_prop_at_of_mem_maximal_atlas hG hQ (foo hf) hx
...
lift_prop_on_of_mem_maximal_atlas hG hQ (foo hf)
