...
corners `I` on `(E, H)` and `I'` on `(E', H')`), and let `f : M → M'`. We say that `f` is
differentiable at `x` (`mdifferentiable_at`) if `C_{f x}^{-1} ∘ f ∘ C_x` is differentiable,
where `C_y` is the chart around `y`.
...
* `unique_mdiff_on I s` : predicate saying that, at each point of the set `s`, a function can have
  ...
  `mfderiv_within` in the file `mfderiv`, as otherwise there is an arbitrary choice in the
