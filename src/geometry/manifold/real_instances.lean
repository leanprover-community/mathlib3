...
{ source := {z : Icc x y | z.val < y},
  ...
  continuous_inv_fun := begin
    ...
    have A : continuous (λ z : ℝ, min (z + x) y) :=
      continuous_min (continuous.add continuous_id continuous_const) continuous_const,
...
{ source := {z : Icc x y | x < z.val},
  ...
  continuous_inv_fun := begin
    ...
    have A : continuous (λ z : ℝ, max (y - z) x) :=
      continuous_max (continuous.sub continuous_const continuous_id) continuous_const,
    have B : continuous (λz : fin 1 → ℝ, z 0) := continuous_apply 0,
    exact (A.comp B).comp continuous_subtype_val
  end }
