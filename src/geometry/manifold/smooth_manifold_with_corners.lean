...
variables {𝕜 : Type*} [nontrivially_normed_field 𝕜]
  {E : Type*} [normed_add_comm_group E] [normed_space 𝕜 E]
  {M : Type*} [topological_space M]
  {H : Type*} [topological_space H] (f : local_homeomorph M H) (I : model_with_corners 𝕜 E H)
  {E' : Type*} [normed_add_comm_group E'] [normed_space 𝕜 E']
  {H' : Type*} [topological_space H'] (I' : model_with_corners 𝕜 E' H')
  {M' : Type*} [topological_space M']
  (x : M) {s t : set M}
...
lemma extend_open_source : is_open (f.extend I).source :=
...
(extend_open_source f I).mem_nhds h
...
f.continuous_on_symm.comp I.continuous_on_symm $
  (maps_to_preimage _ _).mono_left (inter_subset_right _ _)

lemma extend_map_nhds {x y : M} (hy : y ∈ (f.extend I).source) :
  map (f.extend I) (𝓝 y) = 𝓝[range I] (f.extend I y) :=
begin
...
begin
  rw [← local_equiv.image_source_eq_target, ← extend_map_nhds I hy],
...
lemma nhds_within_ext_chart_target_eq' {y : M} (hy : y ∈ (f.extend I).source) :
...
lemma nhds_within_ext_chart_target_eq :
  𝓝[(f.extend I).target] ((f.extend I) x) =
  𝓝[range I] ((f.extend I) x) :=
nhds_within_ext_chart_target_eq' f I (mem_ext_chart_source f I)

lemma ext_chart_continuous_at_symm'' {y : E} (h : y ∈ (f.extend I).target) :
  continuous_at (f.extend I).symm y :=
continuous_at.comp (f.continuous_at_symm h.2) (I.continuous_symm.continuous_at)

lemma ext_chart_continuous_at_symm' {x' : M} (h : x' ∈ (f.extend I).source) :
  continuous_at (f.extend I).symm (f.extend I x') :=
ext_chart_continuous_at_symm'' I _ $ (f.extend I).map_source h

lemma ext_chart_continuous_at_symm :
  continuous_at (f.extend I).symm ((f.extend I) x) :=
ext_chart_continuous_at_symm' f I (mem_ext_chart_source f I)
...
(extend_continuous_on f I).preimage_open_of_open (extend_open_source _ _) hs

lemma ext_chart_preimage_open_of_open {s : set E} (hs : is_open s) :
  is_open (f.source ∩ f.extend I ⁻¹' s) :=
by { rw ← extend_source I, exact ext_chart_preimage_open_of_open' f I hs }
...
... = 𝓝[e '' (e.source ∩ s)] (e y) :
  ((f.extend I).left_inv_on.mono $ inter_subset_left _ _).map_nhds_within_eq
    ...
    (ext_chart_continuous_at_symm' f I hy).continuous_within_at
    (extend_continuous_at f I hy).continuous_within_at

lemma extend_map_nhds_within_eq_image :
  map (f.extend I) (𝓝[s] x) =
    𝓝[f.extend I '' ((f.extend I).source ∩ s)] (f.extend I x) :=
extend_map_nhds_within_eq_image' f I (mem_ext_chart_source f I)
...
lemma extend_map_nhds_within :
  map (f.extend I) (𝓝[s] x) =
    𝓝[(f.extend I).symm ⁻¹' s ∩ range I] (f.extend I x) :=
extend_map_nhds_within' f I (mem_ext_chart_source f I)
...
lemma extend_symm_map_nhds_within :
  map (f.extend I).symm
    (𝓝[(f.extend I).symm ⁻¹' s ∩ range I] (f.extend I x)) = 𝓝[s] x :=
extend_symm_map_nhds_within' f I (mem_ext_chart_source f I)

lemma extend_symm_map_nhds_within_range :
  map (f.extend I).symm (𝓝[range I] (f.extend I x)) = 𝓝 x :=
extend_symm_map_nhds_within_range' f I (mem_ext_chart_source f I)

/-- Technical lemma ensuring that the preimage under an extended chart of a neighborhood of a point
in the source is a neighborhood of the preimage, within a set. -/
lemma ext_chart_preimage_mem_nhds_within' {x' : M} (h : x' ∈ (f.extend I).source)
  (ht : t ∈ 𝓝[s] x') :
  (f.extend I).symm ⁻¹' t ∈
    𝓝[(f.extend I).symm ⁻¹' s ∩ range I] ((f.extend I) x') :=
...
lemma ext_chart_preimage_mem_nhds_within (ht : t ∈ 𝓝[s] x) :
  (f.extend I).symm ⁻¹' t ∈
    𝓝[(f.extend I).symm ⁻¹' s ∩ range I] ((f.extend I) x) :=
ext_chart_preimage_mem_nhds_within' f I (mem_ext_chart_source f I) ht

lemma ext_chart_preimage_mem_nhds' {x' : M} (h : x' ∈ (f.extend I).source) (ht : t ∈ 𝓝 x') :
  (f.extend I).symm ⁻¹' t ∈ 𝓝 (f.extend I x') :=
begin
  apply (ext_chart_continuous_at_symm' f I h).preimage_mem_nhds,
...
lemma ext_chart_preimage_mem_nhds (ht : t ∈ 𝓝 x) :
  (f.extend I).symm ⁻¹' t ∈ 𝓝 ((f.extend I) x) :=
begin
  apply (ext_chart_continuous_at_symm f I).preimage_mem_nhds,
  rwa (f.extend I).left_inv (mem_ext_chart_source _ _)
end
...
/-! We use the name `ext_coord_change` for `(f.extend I').symm ≫ f.extend I`. -/

lemma ext_coord_change_source (x x' : M) :
  ((f.extend I').symm ≫ f.extend I).source =
  I '' ((f x').symm ≫ₕ f).source :=
...
lemma cont_diff_within_at_ext_coord_change [smooth_manifold_with_corners I M] (x x' : M) {y : E}
  (hy : y ∈ ((f.extend I').symm ≫ f.extend I).source) :
  cont_diff_within_at 𝕜 ⊤ (f.extend I ∘ (f.extend I').symm) (range I) y :=
begin
  apply (cont_diff_on_ext_coord_change f I x' y hy).mono_of_mem,
...
lemma extend_self_eq {x : H} : ⇑(f.extend I) = I := rfl
lemma extend_self_apply {x y : H} : f.extend I y = I y := rfl

/-- In the case of the manifold structure on a vector space, the extended charts are just the
identity.-/
lemma ext_chart_model_space_eq_id (x : E) : extend 𝓘(𝕜, E) x = local_equiv.refl E :=
by simp only with mfld_simps

lemma ext_chart_model_space_apply {x y : E} : extend 𝓘(𝕜, E) x y = y := rfl

variable {𝕜}

lemma extend_prod (x : M × M') :
  extend (I.prod I') x = (f.extend I.1).prod (f.extend I' x.2) :=
by simp only with mfld_simps

lemma extend_source : (f.extend I).source = f.source :=
by rw [local_homeomorph.extend, local_equiv.trans_source, I.source_eq, preimage_univ, inter_univ]

lemma extend_target : (f.extend I).target = I '' f.target :=
