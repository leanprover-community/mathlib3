...
def of {a b : V} : G.edge a b → G.path a b := path.app (path.nil _)
lemma of.def {a b : V} (e : G.edge a b) : of e = path.app (path.nil _) e := rfl
...
@[simp]
lemma comp_nil {a b : V} {p : G.path a b} : comp p (path.nil _) = p := rfl

@[simp]
lemma nil_comp : ∀ {a b : V} {p : G.path a b}, comp (path.nil _) p = p
| _ _ (path.nil _) := rfl
...
lemma comp_induction {C : Π {a b} (p : G.path a b), Prop}
...
| _ _ (path.nil _) := h_nil _
| _ _ (path.app p e) := by {rw app_eq_comp_of, exact h_comp (comp_induction p) (h_of e)}

end path
open path

def path_category : category V :=
{ hom := G.path,
  id := @path.nil _ _,
  comp := @comp _ _,
  assoc' := @comp_assoc _ _}

section to_category

variables [G] {C : Type u₁} [category.{v₁} C] (f_obj : V → C)
  (f_edge : Π {a b}, G.edge a b → (f_obj a ⟶ f_obj b))
include G
local attribute [instance]
def mydef : category V := path_category G

lemma comp_def {a b c : V} (p : a ⟶ b) (q : b ⟶ c) :
  comp p q = p ≫ q := rfl
...
@[simp]
lemma to_hom.nil {a} : to_hom f_obj @f_edge (@path.nil _ _ a) = 𝟙 _ := rfl
...
by { rw [of.def, to_hom.app, to_hom.nil], simp }
...
lemma to_category.unique {F : V ⥤ C}
  (h_obj : ∀ a, F.obj a = f_obj a) (h_edge : ∀ {a b} (e : G.edge a b),
...
by {rw [←add_left_inj (length $ path.nil _), ←length_comp], refl }
