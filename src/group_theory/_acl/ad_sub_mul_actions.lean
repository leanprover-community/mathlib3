...
variables {G X}
lemma mul_action.mem_smul [decidable_eq X] {g : G} {a : X} {s : finset X} :
  a ∈ g • s ↔ ∃ (b : X) (H : b ∈ s), g • b = a :=
begin
  change a ∈ finset.image (λ x, g • x) s ↔ ∃ (b : X) (H : b ∈ s), (λ x, g • x) b = a,
...
variables {G X}
lemma mul_action.smul_mem_iff {g : G} {a : X} {s : finset X} :
  g • a ∈ s ↔ a ∈ g⁻¹ • s :=
begin
  split,
  intro gas, rw mul_action.mem_smul,
  use g • a, apply and.intro gas, simp only [inv_smul_smul],
  intro ags,
  obtain ⟨b, hb, hb'⟩ := mul_action.mem_smul.mp ags,
  rw ← hb', simpa using hb,
end

lemma mul_action.smul_subset_smul_inf (g : G) (s t : finset X) :
  g • s ⊆ t ↔ s ⊆ g⁻¹ • t :=
begin
  split,
  intro gst,
  intros a ha, apply mul_action.smul_mem_iff.mp, apply gst,
  rw mul_action.mem_smul, use a, exact ⟨ha, rfl⟩,

  intro sgt,
  intros a ha,
  obtain ⟨b, hb, rfl⟩ := mul_action.mem_smul.mp ha,
  obtain ⟨c, hc, rfl⟩ := mul_action.mem_smul.mp (sgt hb),
  simpa using hc,
end
