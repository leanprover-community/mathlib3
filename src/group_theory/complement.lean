...
@[to_additive] noncomputable def to_equiv (hS : S ∈ subgroup.left_transversals (H : set G)) :
  G ⧸ H ≃ S :=
...
@[to_additive] noncomputable def to_equiv (hS : S ∈ subgroup.right_transversals (H : set G)) :
