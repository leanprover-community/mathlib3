import group_theory.submonoid order.order_iso tactic.default data.dfinsupp algebra.pi_instances data.equiv.algebra
...
-- not sure where to put these; I'd need to import data.equiv.algebra to put them in group_theory.submonoid
...
-- I can't tell what the benefits of the latter tags are
@[simp, refl] lemma refl (c : con M) (x) : c.1 x x := c.2.1 x
@[simp, symm] lemma symm (c : con M) : ∀ {x y}, c x y → c.1 y x := λ _ _ h, c.2.2.1 h
@[simp, trans] lemma trans (c : con M) : ∀ {x y z}, c x y → c y z → c.1 x z := 
...
@[extensionality] lemma ext {c d : con M} (H : ∀ x y, c x y ↔ d x y) :
