...
⟨λ h, let ⟨S, hS, hf⟩ := (monoid.fg_def _).1 h in (add_monoid.fg_def _).mpr
  ⟨additive.to_mul ⁻¹' S, by simpa [← submonoid.to_add_submonoid_closure, hS], hf⟩,
 λ h, let ⟨T, hT, hf⟩ := (add_monoid.fg_def _).1 h in (monoid.fg_def _).mpr
  ⟨multiplicative.of_add ⁻¹' T, by simpa [← add_submonoid.to_submonoid'_closure, hT], hf⟩⟩

lemma add_monoid_fg_iff_mul_fg : add_monoid.fg N ↔ monoid.fg (multiplicative N) :=
(monoid_fg_iff_add_fg $ multiplicative N).symm
