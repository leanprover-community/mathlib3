...
* `is_freely_generated_by`: states that a group is a freely generated group, i.e. equivalent to
   the `free_group`.
...
/-- `is_freely_generated_by G X` means that `G` is equialent to the free group generated by `X`.  -/
class is_freely_generated_by (G : Type*) [group G] (X : out_param (Type*)) :=
  ( mul_equiv : free_group X ≃* G )

instance free_group_is_freely_generated (X : Type*) : is_freely_generated_by (free_group X) X :=
...
@[ext]
lemma ext_hom ⦃f g : G →* H⦄ (h : ∀ (a : X), f (of a) = g (of a)) :
  f = g :=
lift.symm.injective $ funext h

lemma unique_lift (f : X → H) :
  ∃! F : G →* H, ∀ a, F (of a) = f a :=
begin
  have := (@lift G _ X _ H _).symm.bijective.exists_unique f,
