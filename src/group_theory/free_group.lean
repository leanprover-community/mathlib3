...
* `is_free_group`: A class for groups that are free groups
...
@[simps] def to_free_group : G ≃* free_group (generators G) := is_free_group.mul_equiv.symm
...
{ generators := X,
  ...
  let lift' {H : Type u} [group H] : by exactI (X → H) ≃ (G →* H) := by exactI
    { to_fun := λ f, classical.some (h f),
      ...
      right_inv := λ F, ((classical.some_spec (h (F ∘ of))).right F (λ _, rfl)).symm }
  in
  let lift'_of {H : Type u} [group H] (f : X → H) (a : X) : by exactI (lift' f) (of a) = f a :=
