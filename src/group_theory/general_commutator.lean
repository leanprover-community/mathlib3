...
begin
  ...
  {
    rintros ⟨x1, x2⟩ ⟨hx1, hx2⟩,
    rw general_commutator_def at hx1,
    simp only [coe_to_submonoid, set_like.mem_coe, exists_prop] at hx1 hx2,
    revert hx2,
    refine closure_induction hx1 _ _ _ _; clear hx1,
    {
      sorry,
    },
    {
      intro hx2,
      rw general_commutator_def at hx2,
      refine closure_induction hx2 _ _ _ _; clear hx2,
      { rintros x ⟨p, hp, q, hq, rfl⟩,
        simpa using @general_commutator_containment _ _ (H₁.prod H₂) (K₁.prod K₂) (1, p) (1, q)
          ⟨one_mem _, hp⟩ ⟨one_mem _, hq⟩, },
      { apply one_mem _, },
      { intros x y hx hy,
        simpa only [prod.mk_mul_mk, mul_one] using mul_mem _ hx hy, },
      { intros x hx,
        simpa only [prod.inv_mk, one_inv] using inv_mem _ hx }, }
