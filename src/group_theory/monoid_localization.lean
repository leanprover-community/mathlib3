...
We next define a predicate, `char_pred`, characterising `comm_monoid`s isomorphic to the
...
cannot make use of some appropriate lemmas from the `monoid_localization` namespace.
...
/-- Given `comm_monoid`s `X, Z`, a submonoid `Y ⊆ X`, and a homomorphism `f : X → Z` satsifying
    the predicate characterising monoids isomorphic to `monoid_localization X Y`, the homomorphism
