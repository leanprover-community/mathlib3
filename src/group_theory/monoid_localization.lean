...
@[simp, to_additive] lemma lift_mk'_spec (x v) (y : S) :
...
@[simp, to_additive] lemma lift_of (x : M) :
  f.lift hg (f.1 x) = g x :=
...
⟨units.map j $ is_unit.lift_right (f.1.restrict S) f.2 y,
  show j _ = j _, from congr_arg j $ is_unit.coe_lift_right (f.1.restrict S) f.2⟩
...
def localization_map_of_mul_equiv (k : N ≃* P) :
...
def localization_map_of_mul_equiv' {k : P ≃* M} (H : k.to_monoid_hom.map T = S) :
...
noncomputable def mul_equiv_map
  ...
  N ≃* Q :=
f.mul_equiv_of_localizations $ k.localization_map_of_mul_equiv' H

-- Localization at a submonoid generated by one element

variables (x : M) (N)

/-- Given `x : M`, the type of monoid homs satisfying the predicate characterizing the
    localization of `M` at the submonoid generated by `x`. -/
@[reducible] def away_map := localization_map (submonoid.powers x) N

variables {N}

section away

  /-- Given a localization map `φ : M →* N` for the submonoid generated by `x : M`,
      `away.inv φ x n := (φ x)⁻ⁿ.` -/
  noncomputable def away.inv (φ : away_map N x) (n : ℕ) : N :=
  ↑(is_unit.lift_right (φ.1.restrict $ submonoid.powers x) φ.2 ⟨(x ^ n), n, rfl⟩)⁻¹

  /-- Given a localization map `φ : M →* N` for the submonoid generated by `x : M`,
      `away.mk φ x y n := φ y * (φ x)⁻ⁿ.` -/
  noncomputable def away.mk (φ : away_map N x) (y : M) (n : ℕ) : N := φ.1 y * away.inv x φ n

  variables {x}

  @[simp] lemma away.mk_eq_mk {φ : away_map N x} {y : M} {n : ℕ} :
    @mk' M _ (submonoid.powers x) _ _ φ y ⟨x ^ n, ⟨n, rfl⟩⟩ = away.mk x φ y n := rfl

  variables (φ : away_map N x) {ψ : M →* P}

  /-- Given a localization map `φ : M →* N` for the submonoid generated by `x : M` and a
      `comm_monoid` homomorphism `ψ : M →* U` such that `ψ x` is invertible, the homomorphism
      induced from `N` to `U` sending `z : N` to `ψ y * (ψ x)⁻ⁿ`, where `(y, n) : M × ℕ` are
      such that `z = φ y * (φ x)⁻ⁿ`. -/
  noncomputable def away.lift (hx : is_unit (ψ x)) : N →* P :=
  φ.lift $ λ ⟨y, n, hn⟩, show is_unit (ψ y), from let ⟨u, hu⟩ := hx in
    ⟨u ^ n, by rw [←hn, ψ.map_pow, hu, units.coe_pow]⟩

  @[simp] lemma away.lift_of (hx : is_unit (ψ x)) (y) :
    away.lift φ hx (φ.1 y) = ψ y :=
  lift_of _ _ _

  @[simp] lemma away.lift_comp_of (hx : is_unit (ψ x)) :
    (away.lift φ hx).comp φ.1 = ψ :=
  lift_comp_of _ _

  /-- Given a localization map `φ : M →* N` for the submonoid generated by `x : M` and a
      localization map `ρ : M →* U` for the submonoid generated by `x * y : M`, the induced map of
      localizations `N →* U`. -/
  noncomputable def away_to_away_right {y : M}
    (ρ : away_map P (x * y)) : N →* P :=
  away.lift φ $ is_unit_of_mul_one (ρ.1 x) (away.mk (x * y) ρ y 1) $ by
    erw mul_mk'_eq_mk'_of_mul; convert mk'_self ρ _; rw [subtype.coe_mk, pow_one]
