...
/-- The localization map at a submonoid `S` consisting only of units. As a monoid homomorphism, this
is just the identity `M → M`. -/
@[to_additive] def at_units (hS : ∀ x ∈ S, is_unit x) : S.localization_map M :=
{ map_units' := λ y, hS y.1 y.2,
  surj' := λ z, ⟨⟨z, 1⟩, mul_one z⟩,
  eq_iff_exists' := λ x y, ⟨λ h, ⟨1, by simpa only [mul_one, submonoid.coe_one] using h⟩,
    λ ⟨c, hc⟩, (is_unit.mul_left_inj (hS c.1 c.2)).mp hc⟩,
  ..monoid_hom.id M}

@[to_additive, simp] lemma at_units_to_map (hS : ∀ x ∈ S, is_unit x) :
  (at_units hS).to_map = monoid_hom.id M := rfl
...
@[simp] lemma at_units_mk' (hS : ∀ x ∈ S, is_unit x) (x : M) (y : S) :
  (at_units hS).mk' x y = x * ↑((hS y.1 y.2).unit ⁻¹) := rfl
...
/-- The localization map for a single unit element `x : M`. As a monoid homomorphism, this is just
the identity `M → M`. -/
def away_map_of_unit (hx : is_unit x) : away_map x M :=
@localization_map.at_units _ _ (powers x) (λ y ⟨n, hn⟩, hn ▸ is_unit.pow _ hx)
