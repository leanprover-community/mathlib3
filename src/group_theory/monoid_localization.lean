...
define commutative ring localizations does not rely on the ring's addition.
We briefly define the quotient of `X × Y` by the unique congruence relation `r` on
...
We then characterize the localization of a commutative monoid `X` at a submonoid `Y` up to isomorphism;
that is, a commutative monoid `Z` is the localization of `X` at `Y` iff we can find a monoid homomorphism
`f : X →* Z` satisfying 3 properties:
1. For all `y : Y`, there exists `z : Z` such that `f y * z = 1`;
2. For all `z : Z`, there exists `(x, y) : X × Y` such that `z * f y = f x`;
3. For all `x, y : X`, `f x = f y` iff there exists `c : Y` such that `c * x = c * y`.

The majority of lemmas in the file are written in terms of localizations up to isomorphism, using
maps which satisfy the characteristic predicate.
...
We prove that the localization as a quotient type satisfies this characteristic predicate.

The last section of the file concerns commutative groups isomorphic to the localization of `X` at
itself. This localization, known as the Grothendieck group of a commutative monoid, defines a functor from the
category of commutative monoids to the category of commutative groups which is left adjoint to the
forgetful functor, and whose action on morphisms is defined by `completion_map`.
...
Most of the infrastructure starts after the definition of `is_localization_map`; we define the
quotient type first in order to use `monoid_localization` as a namespace.
...
/-- An alternate form of the `add_monoid` localization relation, stated here for readability of the
    next few lemmas. -/
private def r'.rel (Y : add_submonoid A) (a b : A × Y) :=
∃ c : Y, (c : A) + (a.1 + b.2) = c + (b.1 + a.2)

lemma r'.transitive {Y : add_submonoid A} : transitive (r'.rel Y) :=
...
lemma r'.add {Y : add_submonoid A} {a b c d} :
...
def r' (Y : add_submonoid A) : add_con (A × Y) :=
{ r := λ a b, ∃ c : Y, (c : A) + (a.1 + b.2) = c + (b.1 + a.2),
...
@[to_additive]
lemma r_iff_exists {x y : X × Y} : Y.r x y ↔ ∃ c : Y, (c : X) * (x.1 * y.2) = c * (y.1 * x.2) :=
by rw Y.r_eq_r'; refl

@[to_additive]
lemma one_rel (y : Y) : Y.r 1 (y, y) := Y.r_iff_exists.2 ⟨1, by simp⟩
...
@classical.epsilon (X →* Z) ⟨monoid_hom.one⟩ (λ f, monoid_localization.is_localization_map Y f)
...
def is_localization := ∃ f : X →* Z, is_localization_map Y f
...
/-- Given a localization map `f : X →* Z`, the quotient map sending `(x, y) : X × Y` to
    `(f x)(f y)⁻¹`. -/
...
@[simp, to_additive] lemma mk_mul (x₁ x₂) (y₁ y₂ : Y) :
  (mk f (x₁ * x₂) (y₁ * y₂) : Z) = mk f x₁ y₁ * mk f x₂ y₂ :=
...
@[to_additive] lemma localization_map_spec (h : is_localization Y Z) :
  is_localization_map Y (Y.localization_map Z) :=
classical.epsilon_spec h

variables (hf : is_localization_map Y f)
include hf

@[to_additive] lemma eq_iff_exists (x y) :
  f x = f y ↔ ∃ c : Y, (c : X) * x = c * y :=
hf.2.2 x y
...
@[to_additive] protected lemma eq {a₁ b₁} {a₂ b₂ : Y} :
  mk f a₁ a₂ = mk f b₁ b₂ ↔ ∃ c : Y, (c : X) * (a₁ * b₂) = c * (b₁ * a₂) :=
(mk_eq_iff_eq hf).trans $ hf.2.2 _ _

@[to_additive] protected lemma eq' {a₁ b₁} {a₂ b₂ : Y} :
  mk f a₁ a₂ = mk f b₁ b₂ ↔ Y.r (a₁, a₂) (b₁, b₂) :=
(monoid_localization.eq hf).trans $ iff.symm Y.r_iff_exists

@[to_additive] lemma eq_iff_eq {g : X →* V} (hg : is_localization_map Y g) {x y} :
  f x = f y ↔ g x = g y :=
(hf.2.2 _ _).trans (hg.2.2 _ _).symm

@[to_additive] lemma mk_eq_iff_mk_eq {g : X →* V} (hg : is_localization_map Y g)
  {x₁ x₂} {y₁ y₂ : Y} :
...
@[to_additive] lemma exists_of_sec_mk (x) (y : Y) :
  ∃ c : Y, (c : X) * (x * (sec Y f $ mk f x y).2) = c * ((sec Y f $ mk f x y).1 * y) :=
(eq_iff_exists hf _ _).1 $ (mk_eq_iff_eq hf).1 $ (mk_sec hf _).symm

@[to_additive] lemma exists_of_sec (x) :
  ∃ c : Y, (c : X) * (x * (sec Y f $ f x).2) = c * (sec Y f $ f x).1 :=
(eq_iff_exists hf _ _).1 $ by rw f.map_mul; exact sec_spec hf _
...
let ⟨c, hc⟩ := (eq_iff_exists hf _ _).1 h in
  by rw [←one_mul (g _), ←inv_spec_right' hg c, mul_comm (g c), mul_assoc, ←g.map_mul, hc,
         g.map_mul, ←mul_assoc, mul_comm _ (g ↑c), inv_spec_right' hg, one_mul]

omit hg

@[to_additive] lemma comp_eq_of_eq {W : submonoid V} {U : Type*} [comm_monoid U]
  (hg : ∀ y : Y, g y ∈ W) {k : V →* U} (hk : is_localization_map W k)
  {x y} (h : f x = f y) : k (g x) = k (g y) :=
let ⟨c, hc⟩ := (hf.2.2 _ _).1 h in (hk.2.2 _ _).2
  ⟨⟨g c, hg c⟩, show g _ * _ = g _ * _, by rw [←g.map_mul, hc, g.map_mul]⟩
...
variables (hy : ∀ y : Y, g y ∈ W) {U : Type*} [comm_monoid U] {k : V →* U}
          (hk : is_localization_map W k)
omit hf

/-- Given a `comm_monoid` homomorphism `g : X →* V` where for submonoids `Y ⊆ X, W ⊆ V` we have
    `g(Y) ⊆ W`, the monoid homomorphism from the localization of `X` at `Y` to the localization of
    `V` at `W`: if `f : X →* Z` and `k : V →* U` are localization maps, we send `z : Z` to
    `(k (g x)) * (k (g y))⁻¹, where `(x, y) : X × Y` are such that `z = (f x) * (f y)⁻¹`. -/
@[to_additive "Given an `add_comm_monoid` homomorphism `g : X →+ V` where for submonoids `Y ⊆ X, W ⊆ V` we have `g(Y) ⊆ W`, the `add_monoid` homomorphism from the localization of `X` at `Y` to the localization of `V` at `W`: if `f : X →+ Z` and `k : V →+ U` are localization maps, we send `z : Z` to `(k (g x)) - (k (g y)), where `(x, y) : X × Y` are such that `z = (f x) - (f y)`."]
...
@[to_additive] lemma map_comp_map {R T} [comm_monoid R] {S : submonoid R} [comm_monoid T]
  {j : R →* T} (hj : is_localization_map S j) {l : V →* R} (hl : ∀ w : W, l w ∈ S) :
...
@[to_additive] lemma map_map (hk : is_localization_map W k) {R T} [comm_monoid R]
  {S : submonoid R} [comm_monoid T] {j : R →* T} (hj : is_localization_map S j)
...
@[to_additive] lemma lift_inv {R} [comm_monoid R] {k : X →* R}
  (hk : is_localization_map Y k) (z : Z) :
...
noncomputable def mul_equiv_of_localizations
  {R} [comm_monoid R] {k : X →* R} (hk : is_localization_map Y k) : Z ≃* R :=
...
@[to_additive] lemma is_localization_map_of_mul_equiv (k : Z ≃* V) :
  is_localization_map Y (k.to_monoid_hom.comp f) :=
...
@[to_additive] lemma is_localization_map_of_mul_equiv'
  ...
  is_localization_map W (f.comp k.to_monoid_hom) :=
...
/-- Given a localization maps `f : X →* Z` for a submonoid `Y` and `k : V →* U` for a
    submonoid `W`, an isomorphism `j : X ≃* V` such that `j(Y) = W` induces an
    isomorphism of localizations `Z ≃* U`. -/
@[to_additive "Given a localization maps `f : X →+ Z` for a submonoid `Y` and `k : V →+ U` for a submonoid `W`, an isomorphism `j : X ≃+ V` such that `j(Y) = W` induces an isomorphism of localizations `Z ≃+ U`."]
noncomputable def mul_equiv_map
  (hk : is_localization_map W k) (j : X ≃* V) (H : j.to_monoid_hom.map Y = W) :
  Z ≃* U :=
...
section away
  ...
  /-- Given `φ : X →* Z` a localization map for the submonoid generated by `x : X`,
  ...
  /-- Given `φ : X →* Z` a localization map for the submonoid generated by `x : X` and a
      `comm_monoid` homomorphism `ψ : X →* U` such that `ψ x` is invertible, the homomorphism
      induced from `Z` to `U` sending `z : Z` to `(ψ y)(ψ x)⁻ⁿ`, where `(x, y) : X × Y` are
  ...
  /-- Given `φ : X →* Z` a localization map for the submonoid generated by `x : X` and `ρ : X →* U`
      a localization map for the submonoid generated by `x * y : X`, the induced map of
      localizations `Z →* U`. -/
...
attribute [to_additive add_monoid_localization.is_away_map] monoid_localization.is_away_map
...
localized "infix ` • ` := add_monoid.smul" in add_monoid
  ...
  /-- Given `φ : X →+ Z` a localization map for the submonoid generated by `x : X` and an
      `add_comm_monoid` homomorphism `ψ : X →+ U` such that `ψ x` is invertible, the homomorphism
      induced from `Z` to `U` sending `z : Z` to `(ψ y) - n • (ψ x)`, where `(x, y) : X × Y` are
      such that `z = (φ y) - n • (φ x)`. -/
...
@[to_additive] lemma is_localization_map_of : is_localization_map Y (of Y) :=
⟨λ y, ⟨mk' 1 y, Y.r.eq.2 $ show Y.r (_, 1 * y) 1, by simpa using Y.r.symm (Y.one_rel y)⟩,
 λ z, induction_on z $ λ x, ⟨x, Y.r.eq.2 $ show Y.r (x.1 * x.2, x.2 * 1) (x.1, 1), by
   rw [mul_comm x.2, ←mul_one (x.1, (1 : Y))];
   exact Y.r.mul (Y.r.refl (x.1, 1)) (Y.r.symm $ Y.one_rel x.2)⟩,
λ x y, Y.r.eq.trans $ Y.r_iff_exists.trans $
  show (∃ (c : Y), (c : X) * (x * 1) = c * (y * 1)) ↔ _, by rw [mul_one, mul_one]⟩

@[to_additive] lemma is_localization_of_quotient :
  is_localization Y (monoid_localization X Y) := ⟨of Y, is_localization_map_of⟩
...
noncomputable def mul_equiv_of_quotient (hf : is_localization_map Y f) :
...
@[to_additive] lemma is_completion_mk {f : X →* Z} (h1 : ∀ x : X, ∃ z : Z, f x * z = 1)
  ...
  (h3 : ∀ x y : X, f x = f y ↔ ∃ c : X, c * x = c * y) : is_completion_map f :=
...
noncomputable def completion.mk (x y : X) : Z :=
mk f x (⟨y, trivial⟩ : (⊤ : submonoid X))

/-- Given a completion map `f : X →* Z`, sends `z : Z` to `(x, y) : X × X`
    such that `z = (f x) * (f y)⁻¹. -/
@[to_additive "Given a completion map `f : X →+ Z`, sends `z : Z` to `(x, y) : X × X` such that `z = (f x) - (f y)."]
noncomputable def completion.sec (z : Z) : X × X :=
...
noncomputable def completion.inv (z : Z) :=
completion.mk f (completion.sec f z).2 (completion.sec f z).1

/-- Given a completion map of `comm_monoid`s, `f : X →* Z`, `Z` is also a `comm_group`. -/
@[to_additive "Given a completion map of `add_comm_monoid`s, `f : X →+ Z`, `Z` is also an `add_comm_group`."]
noncomputable def completion.comm_group
...
def completion.of := of (⊤ : submonoid X)

@[to_additive] noncomputable instance completion.comm_group' :
  comm_group (monoid_localization X (⊤ : submonoid X)) :=
...
@[to_additive] lemma completion.id_is_localization_map {G : Type*} [comm_group G] :
  is_completion_map (monoid_hom.id G) :=
is_completion_mk ex_inv (λ x, ⟨(x, 1), mul_one x⟩) $
  λ x y, ⟨λ h, ⟨1, by simpa using h⟩, λ ⟨c, hc⟩, mul_left_cancel hc⟩
...
noncomputable def completion.mul_equiv_of_group (G : Type*) [comm_group G] {g : G →* Z}
  (hg : is_completion_map g) : G ≃* Z :=
mul_equiv_of_localizations completion.id_is_localization_map hg

/-- Given a completion map `g : X →* G` and any homomorphism `h : Z →* H`, a homomorphism
    `f : X →* Z` induces a homomorphism from `G` to `H` sending `z : G` to
    `h (f x) * (h (f y))⁻¹` where `(x, y) : X × X` are such that `z = (g x) * (g y)⁻¹`. -/
@[to_additive "Given a completion map `g : X →+ G` and any homomorphism `h : Z →+ H`, a homomorphism `f : X →+ Z` induces a homomorphism from `G` to `H` sending `z : G` to `h (f x) - h (f y)` where `(x, y) : X × X` are such that `z = g x - g y`."]
noncomputable def completion.map {G : Type*} {H : Type*} [comm_group G] [comm_group H]
  {g : X →* G} (hg : is_completion_map g) (h : Z →* H) (f : X →* Z) :
  G →* H := lift hg $ λ z, ⟨(h.comp f z)⁻¹, mul_right_inv _⟩

@[to_additive] lemma completion.map_id {G : Type*} [comm_group G] {g : X →* G}
...
@[to_additive] lemma completion.map_comp {G : Type*} {H : Type*} {J : Type*}
...
begin
  ...
  { show h (k _) * h (k (j _)) = h (k _) * h (k (j _)),
    ...
    rw [g.map_mul, ←sec_spec hg, g.map_mul, mul_assoc, completion.map, lift_spec_mul,
        mul_comm, mul_comm _ (g (j (sec ⊤ f x).2)), mul_assoc],
    refl,
    { tauto }},
    { exact (λ y, ex_inv _) }
end
...
noncomputable def completion.hom_equiv : (X →* H) ≃* (G →* H) :=
  ...
  map_mul' := λ j k, monoid_hom.ext $ λ x, show _ * _ = (_ * _) * (_ * _),
    begin
      rw [mul_inv_left, ←mul_assoc, ←mul_assoc, mul_inv_right, mul_assoc, mul_assoc,
          mul_comm _ (k (sec ⊤ g x).1), ←mul_assoc, ←mul_assoc, mul_inv_right, mul_assoc],
      { show _ * (k * j) _ = _,
...
@[to_additive] lemma completion.hom_equiv_naturality_left_symm {J : Type*} [comm_group J]
...
begin
  ...
  { rw [j.map_mul, j.map_mul, ←sec_spec hj, mul_assoc],
    erw lift_spec_mul,
    rw [mul_comm, mul_comm _ (j (f (sec ⊤ g a).2)), mul_assoc],
    refl },
  { exact (λ y, ex_inv _) },
end
...
theorem is_completion_pnat_to_pos_rat :
  monoid_localization.is_completion_map pnat_to_pos_rat :=
...
λ x y, ⟨λ h, ⟨⟨1, trivial⟩, by rw [subtype.coe_mk, one_mul, subtype.ext,
  nat.cast_inj.1 (show (x.1 : ℚ) = _, from subtype.ext.1 h), one_mul]; refl⟩,
    λ ⟨⟨c, _⟩, hc⟩, mul_left_cancel hc ▸ rfl⟩⟩

/-- Natural inclusion homomorphism from the naturals to the integers. -/
def nat_to_int : ℕ →+ ℤ :=
⟨λ n, n, rfl, λ x y, rfl⟩

theorem is_completion_of_nat_to_int :
  add_monoid_localization.is_completion_map nat_to_int :=
...
(λ z, ⟨(0, ⟨z + 1, trivial⟩), show -((z + 1) : ℤ) + ((z + 1) : ℤ) = 0, from add_left_neg _⟩),
  λ x y, ⟨λ h, ⟨⟨0, trivial⟩, (add_left_inj _).2 $ int.of_nat_inj h⟩,
    λ ⟨c, hc⟩, add_left_cancel hc ▸ rfl⟩⟩