...
localized "infix ` • ` := add_monoid.smul" in add_monoid
  ...
  /-- Given a localization map `φ : X →+ Z` for the submonoid generated by `x : X` and an
...
theorem is_completion_pnat_to_pos_rat :
  submonoid.monoid_localization.is_completion_map pnat_to_pos_rat :=
...
theorem is_completion_nat_to_int :
  add_submonoid.monoid_localization.is_completion_map nat_to_int :=
