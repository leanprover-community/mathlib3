...
of_monoid_hom (k.to_monoid_hom.comp f.to_map) (λ y, is_unit_comp f k.to_monoid_hom y)
...
@[to_additive] lemma symm_of_mul_equiv_of_localizations_apply {k : N ≃* P} (x) :
...
@[to_additive] lemma mul_equiv_of_localizations_right_inv_apply {k : localization_map S P} {x} :
...
@[to_additive] lemma mul_equiv_of_localizations_left_inv_apply {k : N ≃* P} (x) :
...
of_monoid_hom (f.to_map.comp k.to_monoid_hom)
...
@[to_additive, simp] lemma of_mul_equiv_of_dom_eq
  {k : P ≃* M} (H : T.map k.to_monoid_hom = S) :
  (f.of_mul_equiv_of_dom H).to_map = f.to_map.comp k.to_monoid_hom := rfl

@[to_additive] lemma symm_of_mul_equiv_of_dom_apply {k : P ≃* M}
...
@[to_additive, simp] lemma of_mul_equiv_of_mul_equiv
  {k : localization_map T Q} {j : M ≃* P} (H : S.map j.to_monoid_hom = T) :
