...
/-!
  ...
  - Given a subgroup `H` of a free group `G`, observe that `H` is the stabilizer of `1H` in
    ...
    automorphism groups in this groupoid. (This corresponds to the observation that `H` is a
  ...
  - Show that if a groupoid `G` is freely generated by `D` and `D` has a spanning tree `T`,
    then an automorphism groups in `G` is freely generated by certain elements coming from
...
have foo : f = monoid_hom.id G := (u _ (λ _, rfl)).trans (u _ (by simp [h])).symm,
monoid_hom.ext_iff.mp foo
...
@[simp] lemma foo_apply {G A X : Type*} [group G] [mul_action G A] [has_mul X]
  (g : Gᵒᵖ) (F : A → X) (a : A) : foo G A X g F a = F (g.unop • a) := rfl
...
def monoid_hom_of_functor {G A X} [group G] [mul_action G A] [group X]
  (F : action_category G A ⥤ single_obj X) : G →* ((A → Xᵒᵖ) ⋊[foo G A Xᵒᵖ] Gᵒᵖ)ᵒᵖ :=
have F_map_eq : ∀ {a b} {f : a ⟶ b}, F.map f = (F.map (lift_to_hom a.snd f.val) : X) :=
  action_category.cases (λ _ _, rfl),
{ to_fun := λ g, op ⟨λ a, op (F.map (lift_to_hom a g)), op g⟩,
  map_one' := begin
  ...
  map_mul' := begin
    ...
    exact (F_map_eq.congr rfl).mp (F.map_comp (lift_to_hom a h) (lift_to_hom (h • a) g)),
  end }

def functor_of_sane_monoid_hom {G A X} [group G] [mul_action G A] [group X]
  (F : G →* ((A → Xᵒᵖ) ⋊[foo G A Xᵒᵖ] Gᵒᵖ)ᵒᵖ)
...
{ obj := λ _, (),
  ...
  map_comp' := by {
    ...
    rw [action_category.comp_val, F.map_mul, unop_mul, mul_left, pi.mul_apply, foo_apply, sane],
    refl } }
...
{ generators := ⟨λ a b, { e : generators G // (of e • a.snd : A) = b.snd }⟩,
  ...
  unique_lift := begin
    introsI X _ f,
    let X' := ((A → Xᵒᵖ) ⋊[foo G A Xᵒᵖ] Gᵒᵖ)ᵒᵖ,
    let f' : generators G → X' := λ e, op ⟨λ a, op (@f a (_ : A) ⟨e, rfl⟩), op (of e)⟩,
    rcases unique_lift f' with ⟨F', hF', uF'⟩,
    let F : action_category G A ⥤ single_obj X := functor_of_sane_monoid_hom F' _,
    refine ⟨F, _, _⟩,
    { rintros ⟨⟨⟩, a : A⟩ ⟨⟨⟩, b⟩ ⟨e, h : of e • a = b⟩,
      change ((F' (of _)).unop.left _).unop = _,
      rw hF', cases h, refl },
    { intros E hE,
      let E' := monoid_hom_of_functor E,
      have : E' = F',
      { apply uF',
        intro e,
