...
instance action_groupoid_is_free
  {G A : Type u} [group G] {X : Type*} [is_freely_generated_by G X] [mul_action G A] :
  is_free_groupoid (action_category G A) :=
{ quiver_generators := ⟨λ a b, { e : X // (fgp.of e : G) • a.back = b.back }⟩,
  of := λ a b e, ⟨fgp.of e, e.property⟩,
  unique_lift := begin
    introsI Y _ f,
    let f' : X → (A → Y) ⋊[mul_aut_arrow] G :=
...
def End_is_free : Σ (X : Type u), is_freely_generated_by (End (root' T)) X :=
fgp.of_unique_lift
...
lemma End_is_free_of_connected_free {G : Type u} [groupoid.{u u} G] [is_connected G]
  [is_free_groupoid G] (r : G) : Σ (Y : Type u), is_freely_generated_by (End r) Y :=
...
instance subgroup_is_free_of_is_free {G : Type u} [group G] {X : Type*}
  [is_freely_generated_by G X] (H : subgroup G) :
  Σ Y, is_freely_generated_by H Y :=
let e := End_mul_equiv_subgroup H in
match is_free_groupoid.End_is_free_of_connected_free ((obj_equiv G (G ⧸ H)) ↑(1:G)) with
  ⟨Y, h⟩ := ⟨Y, @fgp.of_mul_equiv _ _ _ h _ _ e⟩
end
