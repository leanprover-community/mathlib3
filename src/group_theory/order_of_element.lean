...
@[to_additive gsmul_eq_mod_add_order_of]
...
@[to_additive fin_equiv_gmultiples
"The equivalence between `fin (add_order_of a)` and `subgroup.gmultiples a`, sending `i`
to `i • a`."]
...
@[simp, to_additive fin_equiv_gmultiples_apply]
lemma fin_equiv_gpowers_apply {n : fin (order_of x)} :
  fin_equiv_gpowers x n = ⟨x ^ (n : ℕ), n, gpow_coe_nat x n⟩ := rfl

@[simp, to_additive fin_equiv_gmultiples_symm_apply]
lemma fin_equiv_gpowers_symm_apply (x : G) (n : ℕ)
  {hn : ∃ (m : ℤ), x ^ m = x ^ n} :
...
/-- The equivalence between `subgroup.gpowers` of two elements `x, y` of the same order, mapping
  `x ^ i` to `y ^ i`. -/
@[to_additive gmultiples_equiv_gmultiples
...
@[simp, to_additive gmultiples_equiv_gmultiples_apply]
...
@[to_additive add_order_eq_card_gmultiples]
