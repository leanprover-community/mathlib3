...
lemma add_order_of_nsmul_eq_zero (a : A) : (add_order_of a) •ℕ a = 0 :=
begin
...
lemma nsmul_eq_mod_add_order_of {n : ℕ} : n •ℕ a = (n % add_order_of a) •ℕ a :=
begin
...
lemma add_order_of_dvd_iff_nsmul_eq_zero : add_order_of a ∣ n ↔ n •ℕ a = 0 :=
...
lemma order_of_dvd_iff_pow_eq_one : order_of x ∣ n ↔ x ^ n = 1 :=
...
lemma add_order_of_nsmul' (h : n ≠ 0) :
  add_order_of (n •ℕ a) = add_order_of a / gcd (add_order_of a) n :=
...
lemma nsmul_injective_aux (h : n ≤ m)
  (hm : m < add_order_of a) (eq : n •ℕ a = m •ℕ a) : n = m :=
pow_injective_aux (multiplicative.of_add a) h hm eq

attribute [to_additive nsmul_injective_aux] pow_injective_aux

lemma nsmul_injective_of_lt_add_order_of
  (hn : n < add_order_of a) (hm : m < add_order_of a) (eq : n •ℕ a = m •ℕ a) : n = m :=
(le_total n m).elim
...
lemma sum_card_add_order_of_eq_card_nsmul_eq_zero [decidable_eq A] (hn : 0 < n) :
  ...
  = (finset.univ.filter (λ a : A, n •ℕ a = 0)).card :=
...
... = _ : congr_arg finset.card (finset.ext (begin
  assume a,
  suffices : add_order_of a ≤ n ∧ add_order_of a ∣ n ↔ n •ℕ a = 0,
...
exists_pow_eq_one (multiplicative.of_add a)
...
finset.le_card_of_inj_on_range (•ℕ a)
...
lemma mem_multiples_iff_mem_range_add_order_of [decidable_eq A] {b : A} :
  ...
  b ∈ (finset.range (add_order_of a)).image ((•ℕ a) : ℕ → A)  :=
...
begin
  assume b,
  apply decidable_of_iff' (b ∈ (finset.range (add_order_of a)).image (•ℕ a)),
...
⟨λ ⟨n, hn⟩, ⟨n, by simp * at *⟩, λ ⟨i, hi⟩, ⟨(i % add_order_of a).nat_abs,
  by { convert hi,
       change (i % (add_order_of a)).nat_abs •ℕ a = (λ b, b •ℤ a) i,
       rwa [← gsmul_coe_nat,
...
lemma mem_gmultiples_iff_mem_range_add_order_of [decidable_eq A] {b : A} :
  b ∈ add_subgroup.gmultiples a ↔ b ∈ (finset.range (add_order_of a)).image (•ℕ a) :=
...
@[simp] lemma card_nsmul_eq_zero {a : A} : fintype.card A •ℕ a = 0 :=
...
lemma image_range_add_order_of [decidable_eq A] :
  finset.image (λ i, i •ℕ a) (finset.range (add_order_of a)) =
...
lemma gcd_nsmul_card_eq_zero_iff : n •ℕ a = 0 ↔ (gcd n (fintype.card A)) •ℕ a = 0 :=
