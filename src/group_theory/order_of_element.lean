...
--lemma one_mul_eq_id : ((*) (1 : G)) = id := funext one_mul

@[to_additive]
lemma mul_one_eq_id {M : Type u} [mul_one_class M] : (* (1 : G)) = id := funext mul_one

@[to_additive]
lemma semiconj_by.function_semiconj_mul_left {b c : G} (h : semiconj_by a b c) :
  function.semiconj ((*)a) ((*)b) ((*)c) :=
λ j, by rw [← mul_assoc, h.eq, mul_assoc]

@[to_additive]
lemma commute.function_commute_mul_left {b : G} (h : commute a b) :
...
@[to_additive]
lemma semiconj_by.function_semiconj_mul_right_swap {b c : G} (h : semiconj_by a b c) :
  function.semiconj (*a) (*c) (*b) :=
λ j, by simp_rw [mul_assoc, ← h.eq]

@[to_additive]
lemma commute.function_commute_mul_right {b : G} (h : commute a b) : function.commute (*a) (*b) :=
