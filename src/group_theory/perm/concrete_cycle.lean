...
* `equiv.perm.to_list`: the `list` representation of the cycle generated by
  `f : perm α` starting from `x : α`
* `equiv.perm.to_cycle`: the `cycle` representation of the cycle generated by
  `f : perm α` where `hf : f.is_cycle`
...
* `equiv.perm.iso_cycle'`: the same equivalence as `equiv.perm.iso_cycle`
  but with faster evaluation via `#eval`
...
For some reason, this works faster than the `equiv.perm.to_cycle`, which relies
...
lemma two_le_length_to_list_of_mem_support (h : x ∈ p.support) : 2 ≤ length (to_list p x) :=
by simpa using is_cycle.two_le_card_support (is_cycle_cycle_of _ (mem_support.mp h))

lemma length_to_list_pos_of_mem_support (h : x ∈ p.support) : 0 < length (to_list p x) :=
zero_lt_two.trans_le (two_le_length_to_list_of_mem_support _ _ h)
...
begin
  ...
  { rw [←hc.mem_support_pos_pow_iff_of_lt_order_of n.zero_lt_succ hn,
        mem_support, cycle_of_pow_apply_self] at hx,
    simp [hx] },
  rw ←hc.pow_inj_iff (p.cycle_of x),
  { intro h,
...
begin
  ...
  { rw [cycle_of_apply_of_not_same_cycle hy, form_perm_apply_not_mem],
...
begin
  ...
  { refine ⟨f x, x, hx, _, _⟩,
    ...
    { rw [cycle.mem_coe_iff, mem_to_list_iff],
      exact ⟨⟨0, by simp⟩, mem_support.mpr hx⟩ } },
...
begin
  obtain ⟨x, hx, -⟩ := id hf,
  simpa [to_cycle_eq_to_list f hf x hx, hx, cycle.nontrivial_coe_nodup_iff (nodup_to_list _ _)]
    using two_le_length_to_list_of_mem_support _ _ (mem_support.mpr hx)
end
...
For a faster-computing function, use `equiv.perm.iso_cycle'`.
-/
...
For a slower, but more explicit computing function, use `equiv.perm.iso_cycle'`.
-/
def iso_cycle' : {f : perm α // is_cycle f} ≃ {s : cycle α // s.nodup ∧ s.nontrivial} :=
{ to_fun := λ f, fintype.choose _ f.prop.exists_unique_cycle,
  inv_fun := λ s, ⟨(s : cycle α).form_perm s.prop.left,
    (s : cycle α).is_cycle_form_perm _ s.prop.right⟩,
  left_inv := λ f, by simpa [subtype.ext_iff]
    using fintype.choose_spec _ f.prop.exists_unique_cycle,
...
⟨λ f, repr (multiset.pmap (λ (g : perm α) (hg : g.is_cycle),
  iso_cycle' ⟨g, hg⟩) -- to_cycle is slower
