...
let ⟨x, hx⟩ := hf in
⟨x, by { simv only [inv_eq_iff_eq, *, forall_prop_of_true, ne.def] at *, cc },
  λ y hy, let ⟨i, hi⟩ := hx.2 y (by { simv only [inv_eq_iff_eq, *, forall_prop_of_true,
      ne.def] at *, cc }) in
...
begin
  obtain ⟨a, ha1, ha2⟩ := hf,
  refine ⟨g a, by simv [ha1], λ b hb, _⟩,
...
| -[1+ n] := λ b x f hb h,
  ...
  have hb : (swap x (f⁻¹ x) * f⁻¹) (f⁻¹ b) ≠ f⁻¹ b,
    by { rw [mul_apply, swap_apply_def],
      split_ifs;
      simv only [inv_eq_iff_eq, perm.mul_apply, zpow_neg_succ_of_nat, ne.def,
...
if hyx : y = x then by simv [hyx]
else if hfyx : y = f x then by simv [hfyx, hffx]
else begin
...
⟨f x, by { simv [swap_apply_def, mul_apply, if_neg hffx, f.injective.eq_iff, if_neg hx, hx], },
  λ y hy,
  ...
  then
    have h : swap x (f x) * f = 1,
      begin
        ...
        simv only [perm.mul_def, perm.one_def, swap_apply_left, swap_swap]
      end,
    ...
    by { rw [sign_mul, sign_swap hx.1.symm, (hf.swap_mul hx.1 h1).sign, ← h],
      simv only [pow_add, mul_one, neg_neg, one_mul, mul_neg, eq_self_iff_true,
        pow_one, neg_mul_neg] }
...
begin
  ...
  { simv only [le_eq_subset, zpow_neg_succ_of_nat, perm.support_inv] at h1 h2,
...
begin
  ...
  { intro h,
    ...
    { rw hf.support_pow_eq_iff,
      rintro ⟨k, rfl⟩,
      refine h.ne_one _,
      simv [pow_mul, pow_order_of_eq_one] },
...
begin
  ...
  { rintro ⟨x, hx, hx'⟩,
    ...
    { refine ⟨(f ^ a) x, mem_support.mp hfa, _⟩,
      simv only [pow_sub _ hab, equiv.perm.coe_mul, function.comp_app,
        inv_apply_self, ← hx'] },
    { have h := @equiv.perm.zpow_apply_comm _ f 1 a x,
      simv only [zpow_one, zpow_coe_nat] at h,
...
equiv.ext $ λ y, begin
  ...
  split_ifs; simv [*, same_cycle_inv, same_cycle_inv_apply] at *
end
...
begin
  ...
  { obtain ⟨z, rfl⟩ := hxy,
    ...
    simv [h.mul_zpow, zpow_apply_eq_self_of_apply_eq_self hx] },
...
begin
  ...
  { simv [h.commute.eq, cycle_of_mul_of_apply_right_eq_self h.symm.commute, hfx] },
  { simv [cycle_of_mul_of_apply_right_eq_self h.commute, hgx] }
end
...
begin
  ...
  { rw [mem_support, cycle_of_apply],
    split_ifs with hy,
    { simv only [hx, hy, iff_true, ne.def, not_false_iff, and_self, mem_support],
...
| []     f h := ⟨[], by { simv only [imp_false, list.pairwise.nil, list.not_mem_nil, forall_const,
...
if hx : f x = x then
  ...
  (λ y hy, list.mem_of_ne_of_mem
    ...
    ⟨(cycle_of f x) :: m, by { rw [list.prod_cons, hm₁], simv },
...
begin
  ...
  simv only [noncomm_prod_to_finset, hl, exists_prop, list.mem_to_finset, and.congr_left_iff,
...
begin
  ...
  intros x hx,
  simv only [mem_cycle_factors_finset_iff, inf_eq_inter, mem_inter, mem_support] at hx,
  obtain ⟨⟨⟨a, ha, -⟩, hf⟩, -, hg⟩ := hx,
  refine h (_ : a ∈ f.support ∩ g.support),
  simv [ha, ←hf a ha, ←hg a ha]
end
...
begin
  ...
  split,
  { simv only [mem_cycle_factors_finset_iff, mem_union],
    rintro _ (⟨h, -⟩ | ⟨h, -⟩);
    exact h },
  { refine ⟨_, _⟩,
    { simp_rw mem_union,
...
begin
  ...
  simv only [hfc2.eq, inv_mul_cancel_right],
...
begin
  ...
  { simv },
  { intros σ hσ f hf,
    simv only [cycle_factors_finset_eq_singleton_self_iff.mpr hσ, mem_singleton] at hf ⊢,
    simv [hf] },
  ...
  { obtain ⟨k, hk, hk'⟩ := same_cycle.nat_of_mem_support f h hx,
    ...
    { refine ⟨(f.cycle_of x).support.card, _, self_le_add_right _ _, _⟩,
      { refine zero_lt_one.trans (one_lt_card_support_of_ne_one _),
        simpa using hx },
      { simv only [perm.coe_one, id.def, pow_zero] at hk',
        subst hk',
        rw [←order_of_is_cycle (is_cycle_cycle_of _ (mem_support.mp hx)),
            ←cycle_of_pow_apply_self, pow_order_of_eq_one, one_apply] } },
    { exact ⟨k + 1, by simv, nat.le_succ_of_le hk.le, hk'⟩ } },
  { refine ⟨1, zero_lt_one, by simv, _⟩,
    obtain ⟨k, rfl⟩ := h,
...
begin
  ...
  intros x hx,
  simv only [perm.mul_apply, equiv.trans_apply, equiv.sum_congr_apply],
  ...
  simv only [ne.def, is_cycle.zpowers_equiv_support_apply,
...
begin
  ext,
  simv only [mem_map_equiv, perm.coe_mul, comp_app, ne.def, perm.mem_support, equiv.eq_symm_apply],
  refl,
end
...
begin
  ...
  { refine ((equiv.set.of_eq hd1').trans (equiv.set.union hd1'')).trans
    ...
    { simv only [set.mem_image, to_embedding_apply, exists_eq_right,
        support_conj, coe_map, apply_eq_iff_eq] } },
  { intros x hx,
    simv only [trans_apply, symm_trans_apply, set.of_eq_apply,
    ...
    { rw [mem_coe, mem_support] at hxσ,
      ...
      simv only [subtype_equiv_apply, perm.coe_mul, sum.map_inl, comp_app,
    ...
    { rw [mem_coe, ← apply_mem_support, mem_support] at hxτ,
      ...
      simv only [subtype_equiv_apply, perm.coe_mul, sum.map_inr, comp_app,
