...
begin
  ...
  { rintro ⟨n, hx, rfl⟩,
    ...
    simp [hx] },
  { rintro ⟨h, hx⟩,
    simpa using same_cycle.exists_pow_eq_of_mem_support _ h hx }
end
...
begin
  ...
  obtain ⟨k, hk, hk'⟩ := hy.left.exists_pow_eq_of_mem_support _ hy.right,
...
begin
  ...
  { obtain ⟨_ | k, hk, hy⟩ := h.exists_pow_eq_of_mem_support _ hx,
...
begin
  ...
  { obtain ⟨k, hk, rfl⟩ := hy.exists_pow_eq_of_mem_support _ (mem_support.mpr hx),
