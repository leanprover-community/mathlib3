...
begin
  ext (_ | i),
  { simv [swap_apply_of_ne_of_ne] },
  { by_cases hx : i = x,
    simv [hx, swap_apply_of_ne_of_ne],
    by_cases hy : i = y;
    simv [hx, hy, swap_apply_of_ne_of_ne], }
end
...
begin
  ...
  { simv [perm.one_def] },
  { intros f x y hne h,
    simv [h, hne, perm.mul_def, ←equiv.option_congr_trans] }
end
...
begin
  ...
  { cases x,
    { simv },
    { cases h : σ (some x),
      { simv [remove_none_none _ h], },
      { have hn : σ (some x) ≠ none := by simv [h],
        have hσn : σ (some x) ≠ σ none := σ.injective.ne (by simv),
        simv [remove_none_some _ ⟨_, h⟩, ←h, swap_apply_of_ne_of_ne hn hσn] } } },
  simpa using this,
end
...
{ to_fun := λ σ, (σ none, remove_none σ),
  ...
  left_inv := λ σ, by simv,
  right_inv := λ ⟨x, σ⟩, begin
    have : remove_none (swap none x * σ.option_congr) = σ :=
      equiv.option_congr_injective (by simv [←mul_assoc]),
    simv [←perm.eq_inv_iff_eq, this],
  end }
