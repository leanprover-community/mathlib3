...
@[simp] lemma sign_sum_congr {α β : Type*} [decidable_eq α] [decidable_eq β]
  [fintype α] [fintype β] (σa : equiv.perm α) (σb : equiv.perm β) :
  (sum_congr σa σb).sign = σa.sign * σb.sign :=
begin
  suffices : (sum_congr σa 1).sign = σa.sign ∧ (sum_congr 1 σb).sign = σb.sign,
  { rw [←this.1, ←this.2, ←sign_mul, sum_congr_mul],
    congr, },
  split,
  { apply σa.swap_induction_on _ (λ σa' a₁ a₂ ha ih, _),
    { simp },
    { rw [←one_mul (1 : equiv.perm β), ←sum_congr_mul, sign_mul, sign_mul, ih],
      congr,
      rw [perm.sign_swap ha, sum_congr_swap_one],
      exact sign_swap (sum.injective_inl.ne_iff.mpr ha), }, },
  { apply σb.swap_induction_on _ (λ σb' b₁ b₂ hb ih, _),
    { simp },
    { rw [←one_mul (1 : equiv.perm α), ←sum_congr_mul, sign_mul, sign_mul, ih],
      congr,
      rw [perm.sign_swap hb, sum_congr_one_swap],
      exact sign_swap (sum.injective_inr.ne_iff.mpr hb), }, }
end
