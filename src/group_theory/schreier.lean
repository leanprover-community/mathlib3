...
lemma card_commutator_dvd_index_center_pow [hG0 : finite (commutators G)] :
  nat.card (commutator G) ∣
    (center G).index ^ ((center G).index * nat.card (commutators G) + 1) :=
begin
...
noncomputable def quotient_centralizer_embedding (g : G) :
  G ⧸ centralizer (zpowers (g : G)) ↪ commutators G :=
...
-- PR ready
lemma index_center_ne_zero [finite (commutators G)] [group.fg G] :
  (center G).index ≠ 0 :=
begin
...
-- PR ready
lemma index_center_le_pow [finite (commutators G)] [group.fg G] :
  (center G).index ≤ (nat.card (commutators G)) ^ group.rank G :=
begin
  obtain ⟨S, hS1, hS2⟩ := group.rank_spec G,
...
set.range (λ g : commutators G, (classical.some g.2, classical.some (classical.some_spec g.2)))

instance [finite (commutators G)] : finite (commutator_representatives G) :=
...
instance closure_commutator_representatives_fg [finite (commutators G)] :
...
lemma mylem2 : (closure_commutator_representatives G).subtype ''
  (commutators (closure_commutator_representatives G)) = commutators G :=
begin
...
lemma card_commutators_closure_commutator_representatives :
  nat.card (commutators (closure_commutator_representatives G)) = nat.card (commutators G) :=
begin
  rw ← mylem2 G,
...
begin
  rw [commutator_eq_closure G, ←commutators_def, ←mylem2, ←monoid_hom.map_closure,
    commutators_def, ←commutator_eq_closure],
  refine nat.card_congr (equiv.set.image _ _ (subtype_injective _)),
end

instance [finite (commutators G)] : finite (commutators (closure_commutator_representatives G)) :=
begin
  apply nat.finite_of_card_ne_zero,
  rw card_commutators_closure_commutator_representatives,
...
lemma key_lemma [finite (commutators G)] :
  nat.card (commutator G) ≤ myfun (nat.card (commutators G)) :=
begin
  have hn₀ : 0 < nat.card (commutators G) := finite.card_pos,
  ...
  rw card_commutators_closure_commutator_representatives at h1 h2,
  ...
  rw ← pow_succ' at h2,
  rwa ← card_commutator_closure_commutator_representatives,
end
