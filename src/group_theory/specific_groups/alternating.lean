...
* `three_cycle_is_commutator` shows that every 3-cycle is a commutator
of elements in the alternating group, while `three_cycle_mem_commutator`
shows that every 3-cycle belongs to the commutator subgroup.
(NB : `commutator G` is defined in abelianization.lean as the *normal
subgroup* generated by the commutators.)

* `alternating_group_is_perfect`shows that the alternating group on `α`
is equal to its commutator subgroup if `α ≤ fintype.card α`

* `alternating_group.is_simple_group_five` shows that the alternating
group on `fin 5` is simple.
  The proof shows that the normal closure of any non-identity element
  of this group contains a 3-cycle.
...
lemma three_cycle_mem_commutator
  (h5: 5 ≤ fintype.card α) {g : alternating_group α} :
   is_three_cycle (g:perm α)
