...
* `three_cycle_is_commutator` shows that every 3-cycle is a commutator of elements in the alternating group,
while `three_cycle_mem_commutator` shows that every 3-cycle belongs to the commutator subgroup.
(NB : `commutator G` is defined in abelianization.lean as the *normal subgroup* generated by the commutators.)
