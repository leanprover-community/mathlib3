...
A group `G` is called cyclic if there exists an element `g : G` such that it generates the group.
This file only deals with the predicate on a group to be cyclic. For the concrete cyclic group of
order `n`, see `data.zmod.basic`.
...
@[to_additive group.is_cyclic_add] class group.is_cyclic (α : Type u) [group α] : Prop :=
(exists_generator [] : ∃ g : α, ∀ x, x ∈ gpowers g)

@[priority 100]
instance is_cyclic_of_subsingleton [group α] [subsingleton α] : group.is_cyclic α :=
⟨⟨1, λ x, by { rw subsingleton.elim x 1, exact mem_gpowers 1 }⟩⟩

/-- A cyclic group is always commutative. This is not an `instance` because often we have a better
proof of `comm_group`. -/
def is_cyclic.comm_group [hg : group α] [group.is_cyclic α] : comm_group α :=
...
lemma monoid_hom.map_cyclic {G : Type*} [group G] [h : group.is_cyclic G] (σ : G →* G) :
  ∃ m : ℤ, ∀ g : G, σ g = g ^ m :=
begin
...
lemma is_cyclic_of_order_of_eq_card [fintype α]  (x : α)
   (hx : order_of x = fintype.card α) : group.is_cyclic α :=
begin
  classical,
  use x,
...
lemma is_cyclic_of_prime_card {α : Type u} [group α] [fintype α] {p : ℕ} [hp : fact p.prime]
  (h : fintype.card α = p) : group.is_cyclic α :=
⟨begin
  obtain ⟨g, hg⟩ : ∃ g : α, g ≠ 1,
...
instance bot.is_cyclic {α : Type u} [group α] : group.is_cyclic (⊥ : subgroup α) :=
⟨⟨1, λ x, ⟨0, subtype.eq $ eq.symm (subgroup.mem_bot.1 x.2)⟩⟩⟩

instance subgroup.is_cyclic {α : Type u} [group α] [group.is_cyclic α] (H : subgroup α) : group.is_cyclic H :=
...
lemma is_cyclic.card_pow_eq_one_le [decidable_eq α] [fintype α] [group.is_cyclic α] {n : ℕ}
...
[group.is_cyclic α] : ∃ x : α, ∀ y : α, y ∈ submonoid.powers x :=
...
lemma is_cyclic_of_card_pow_eq_one_le : group.is_cyclic α :=
...
lemma is_cyclic.card_order_of_eq_totient [group.is_cyclic α] [fintype α]
...
lemma commutative_of_cyclic_center_quotient [group.is_cyclic H] (f : G →* H)
...
/-- A group is commutative if the quotient by the center is cyclic. -/
def comm_group_of_cycle_center_quotient [group.is_cyclic H] (f : G →* H)
...
theorem comm_group.is_simple_iff_is_cyclic_and_prime_card [fintype α] [comm_group α] :
  is_simple_group α ↔ group.is_cyclic α ∧ (fintype.card α).prime :=
begin
  split,
  { introI h,
    exact ⟨is_simple_group.group.is_cyclic, is_simple_group.prime_card⟩ },
  { rintro ⟨hc, hp⟩,
