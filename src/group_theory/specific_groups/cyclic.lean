...
begin
  ...
  { rw [zero_dvd_iff, fintype.card_eq_zero_iff] at hd, cases not_is_empty_of_nonempty α hd },
  ...
  have h0 : (univ.filter (λ a : α , order_of a = d)).card = 0, { simp [h0'] },
  clear h0',

  apply lt_irrefl c,
  have h1 : c = (univ.filter (λ a : α, a ^ c = 1)).card, { apply congr_arg card, simp },
