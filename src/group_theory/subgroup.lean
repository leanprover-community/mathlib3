...
begin ext, simp [submonoid.exists_multiplicative_iff] end
...
begin ext, simp [submonoid.exists_multiplicative_iff, gpowers] end
...
begin
  show ∀ x, (∃ n, n •ℤ a = x) → x ∈ B,
  have : ∀ x, (∃ n, multiplicative.of_add a ^ n = x) → x ∈ B.to_subgroup :=
    subgroup.gpowers_subset (show multiplicative.of_add a ∈ B.to_subgroup, by simpa),
  rintros x ⟨n, h⟩,
  replace this := this (multiplicative.of_add x) ⟨n, by simpa [← of_add_gsmul] using h⟩,
  simpa using this,
end
