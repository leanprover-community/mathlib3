...
This is more definitionally convenient than `(gpowers_hom G g).mrange` as it does not involve
`multiplicative ℤ`. -/
def gpowers (g : G) : subgroup G :=
(gpowers_hom G g).range.copy (set.range ((^) g : ℤ → G)) $
  set.ext $ λ i, multiplicative.of_add.exists_congr_left

lemma coe_gpowers (g : G) : (gpowers g : set G) = set.range ((^) g : ℤ → G) := rfl
...
(gmultiples_hom A a).range

lemma coe_gmultiples (a : A) : (gmultiples a : set A) = set.range ((• a) : ℤ → A) := rfl
...
attribute [to_additive add_subgroup.coe_gmultiples] subgroup.coe_gpowers
...
begin
  ...
  split,
  { rintro ⟨z, ⟨m, rfl⟩, rfl⟩,
    refine ⟨m, (of_mul_gpow _ _).symm⟩, },
  { rintros ⟨n, rfl⟩,
    refine ⟨x ^ n, ⟨n, rfl⟩, of_mul_gpow _ _⟩, }
end
