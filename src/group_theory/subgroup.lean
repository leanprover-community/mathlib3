...
@[simp] lemma coe_pow (x : H) (n : ℕ) : ((x ^ n : H) : G) = x ^ n :=
coe_subtype H ▸ monoid_hom.map_pow _ _ _
@[simp] lemma coe_gpow (x : H) (n : ℤ) : ((x ^ n : H) : G) = x ^ n :=
...
{ carrier := set.range ((^) g : ℤ → G),
  one_mem' := ⟨0, rfl⟩,
  mul_mem' := by { rintros g h ⟨n, rfl⟩ ⟨m, rfl⟩, exact ⟨n + m, gpow_add _ _ _⟩ },
  inv_mem' := by { rintros g ⟨n, rfl⟩, exact ⟨-n, gpow_neg _ _⟩ } }
...
{ carrier := set.range ((•ℤ a) : ℤ → A),
  zero_mem' := ⟨0, rfl⟩,
  add_mem' := by { rintros a b ⟨n, rfl⟩ ⟨m, rfl⟩, exact ⟨n + m, add_gsmul _ _ _⟩ },
  neg_mem' := by { rintros g ⟨n, rfl⟩, exact ⟨-n, neg_gsmul _ _⟩ } }

@[simp] lemma mem_gpowers (a : A) : a ∈ gmultiples a := ⟨1, one_gsmul _⟩

lemma gpowers_eq_closure (a : A) : gmultiples a = closure {a} :=
by { ext, exact mem_closure_singleton.symm }

attribute [to_additive gmultiples] subgroup.gpowers
