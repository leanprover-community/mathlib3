...
@[simp, norm_cast, to_additive] theorem coe_multiset_prod {G} [comm_group G] (S : subgroup G)
  (m : multiset S) : (m.prod : G) = (m.map coe).prod :=
S.to_submonoid.coe_multiset_prod m

@[simp, norm_cast, to_additive] theorem coe_finset_prod {ι G} [comm_group G] (S : subgroup G)
  (f : ι → S) (s : finset ι) :
  ↑(∏ i in s, f i) = (∏ i in s, f i : G) :=
S.to_submonoid.coe_finset_prod f s
