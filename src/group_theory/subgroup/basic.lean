...
lemma single_mem_pi [decidable_eq η] {I : set η} {H : Π i, subgroup (f i)}
  (i : η) (x : f i) :
  monoid.single f i x ∈ pi I H ↔ (i ∈ I → x ∈ H i) :=
begin
  split,
...
lemma pi_mem_of_single_mem_aux [decidable_eq η] (I : finset η) {H : subgroup (Π i, f i) } (x : Π i, f i)
  (h1 : ∀ i, i ∉ I → x i = 1) (h2 : ∀ i, i ∈ I → pi.single1 i (x i) ∈ H ) :
  x ∈ H :=
begin
  ...
  { have : x = 1,
    ...
    simp [this, one_mem], },
  { have : x = function.update x i 1 * pi.single1 i (x i),
    { ext j,
      by_cases heq : j = i,
      { subst heq, simp, },
      { simp [heq], }, },
...
lemma pi_le_iff [decidable_eq η] [fintype η] {H : Π i, subgroup (f i)} {J : subgroup (Π i, f i)} :
  pi univ H ≤ J ↔ (∀ i : η , map (monoid.single f i) (H i) ≤ J) :=
begin
  split,
