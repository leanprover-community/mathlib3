...
@[to_additive]
instance : is_bundled_set (submonoid M) :=
{ mem_type := M,
  set_coe' := submonoid.carrier,
...
theorem ext {S T : submonoid M}
  (h : ∀ x, x ∈ S ↔ x ∈ T) : S = T := is_bundled_set.ext' $ set.ext h
...
@[to_additive] lemma copy_eq {s : set M} (hs : s = S) : S.copy s hs = S := is_bundled_set.ext' hs
...
{ le           := (≤),
  ...
  .. complete_lattice_of_Inf (submonoid M) $ λ s,
    is_glb.of_image (λ S T,
      show (S : set M) ≤ T ↔ S ≤ T, from begin
        convert is_bundled_set.coe_subset_coe,
        refl,
      end) is_glb_binfi }
