...
/-- An additive submonoid of an additive monoid `A` is a subset containing 0 and
  closed under addition. -/
structure add_submonoid (A : Type*) [has_zero A] [has_add A] :=
(carrier : set A)
(zero_mem' : (0 : A) ∈ carrier)
...
variables {M : Type*} [has_one M] [has_mul M] {s : set M}
variables {A : Type*} [has_zero A] [has_add A] {t : set A}
...
instance {M : Type*} [mul_one_class M] : has_bot (submonoid M) :=
⟨{ carrier := {1},
   one_mem' := set.mem_singleton 1,
   mul_mem' := λ a b ha hb, by { simp only [set.mem_singleton_iff] at *, rw [ha, hb, mul_one] }}⟩

@[to_additive]
instance {M : Type*} [mul_one_class M] : inhabited (submonoid M) := ⟨⊥⟩

@[simp, to_additive] lemma mem_bot {M : Type*} [mul_one_class M] {x : M} :
  x ∈ (⊥ : submonoid M) ↔ x = 1 :=
...
@[simp, to_additive] lemma coe_bot {M : Type*} [mul_one_class M] :
  ((⊥ : submonoid M) : set M) = {1} :=
rfl
...
instance {M : Type*} [mul_one_class M] : complete_lattice (submonoid M) :=
{ le           := (≤),
  lt           := (<),
  bot          := (⊥),
...
@[to_additive]
lemma subsingleton_iff {M : Type*} [mul_one_class M] :
  subsingleton M ↔ subsingleton (submonoid M) :=
...
@[to_additive]
lemma nontrivial_iff {M : Type*} [mul_one_class M] : nontrivial M ↔ nontrivial (submonoid M) :=
not_iff_not.mp (
  (not_nontrivial_iff_subsingleton.trans subsingleton_iff).trans
  not_nontrivial_iff_subsingleton.symm)

@[to_additive]
instance {M : Type*} [mul_one_class M] [subsingleton M] : unique (submonoid M) :=
⟨⟨⊥⟩, λ a, @subsingleton.elim _ (subsingleton_iff.mp ‹_›) a _⟩

@[to_additive]
instance {M : Type*} [mul_one_class M] [nontrivial M] : nontrivial (submonoid M) :=
nontrivial_iff.mp ‹_›
...
lemma closure_le {M : Type*} [mul_one_class M] {s : set M} {S : submonoid M} :
  closure s ≤ S ↔ s ⊆ S :=
...
then `closure s ≤ closure t`"]
lemma closure_mono {M : Type*} [mul_one_class M] ⦃s t : set M⦄ (h : s ⊆ t) : closure s ≤ closure t :=
closure_le.2 $ subset.trans h subset_closure

@[to_additive]
lemma closure_eq_of_le {M : Type*} [mul_one_class M] {s : set M} {S : submonoid M}
...
lemma closure_induction {M : Type*} [mul_one_class M] {s : set M}
  {p : M → Prop} {x} (h : x ∈ closure s)
  (Hs : ∀ x ∈ s, p x) (H1 : p 1)
...
@[to_additive]
lemma dense_induction {M : Type*} [mul_one_class M] {p : M → Prop} (x : M) {s : set M} (hs : closure s = ⊤)
  (Hs : ∀ x ∈ s, p x) (H1 : p 1)
  (Hmul : ∀ x y, p x → p y → p (x * y)) : p x :=
...
protected def gi (M : Type*) [mul_one_class M] : galois_insertion (@closure M _ _) coe :=
...
lemma closure_eq {M : Type*} [mul_one_class M] (S : submonoid M) :
  closure (S : set M) = S :=
(submonoid.gi M).l_u_eq S

@[simp, to_additive] lemma closure_empty {M : Type*} [mul_one_class M] : closure (∅ : set M) = ⊥ :=
(submonoid.gi M).gc.l_bot

@[simp, to_additive] lemma closure_univ {M : Type*} [mul_one_class M] :
  closure (univ : set M) = ⊤ :=
@coe_top M _ _ ▸ closure_eq ⊤

@[to_additive]
lemma closure_union {M : Type*} [mul_one_class M] (s t : set M) :
  closure (s ∪ t) = closure s ⊔ closure t :=
(submonoid.gi M).gc.l_sup

@[to_additive]
lemma closure_Union {M : Type*} [mul_one_class M] {ι} (s : ι → set M) :
...
variables {M : Type*} {N : Type*} {P : Type*} [monoid M] [monoid N] [monoid P] (S : submonoid M)
variables {s : set M}
