...
This is more definitionally convenient than `(powers_hom M n).mrange` as it does not involve
`multiplicative ℕ`. -/
def powers (n : M) : submonoid M :=
(powers_hom M n).mrange.copy (set.range ((^) n : ℕ → M)) $
set.ext $ λ i, multiplicative.of_add.exists_congr_left

lemma coe_powers (n : M) : (powers n : set M) = set.range ((^) n : ℕ → M) := rfl
...
(multiples_hom A x).mrange

lemma coe_multiples (x : A) : (multiples x : set A) = set.range (λ i : ℕ, i • x) := rfl
...
attribute [to_additive add_submonoid.coe_multiples] submonoid.coe_powers
