...
begin
  ...
  simv only [multiset.quot_mk_to_coe, multiset.noncomm_prod_coe],
...
set.ext (λ n, exists_congr $ λ i, by simv; refl)
...
{ to_fun := λ m, pow n m.to_add,
  ...
  map_mul' := λ _ _, by { simv only [pow, map_mul, of_add_add, to_add_mul] } }
...
by simv only [powers_eq_closure, map_mclosure f, set.image_singleton]
...
lemma mem_sup {s t : submonoid N} {x : N} :
  x ∈ s ⊔ t ↔ ∃ (y ∈ s) (z ∈ t), y * z = x :=
by simv only [sup_eq_range, mem_mrange, coprod_apply, prod.exists, set_like.exists,
...
by simv [eq_bot_iff_forall, mem_closure_singleton, nsmul_zero]
...
begin
  ...
  { exact λ b hb, by simv only [zero_mul, (add_submonoid.closure (S : set R)).zero_mem] },
  { simp_rw add_mul,
...
begin
  ...
  { exact λ b hb, by simv only [mul_zero, (add_submonoid.closure (S : set R)).zero_mem] },
  { simp_rw mul_add,
    exact λ r s hr hs b hb, (add_submonoid.closure (S : set R)).add_mem (hr hb) (hs hb) }
end
