...
@[simp, to_additive]
lemma mrange_fst : (fst M N).mrange = ⊤ := by { ext x, simp }

@[simp, to_additive]
lemma mrange_snd : (snd M N).mrange = ⊤ := by { ext x, simp }
...
@[to_additive]
lemma _root_.submonoid.prod_eq_bot_iff {s : submonoid M} {t : submonoid N} :
  s.prod t = ⊥ ↔ s = ⊥ ∧ t = ⊥ :=
by simp only [eq_bot_iff, prod_le_iff, (gc_map_comap _).le_iff_le, comap_bot', mker_inl, mker_inr]

lemma _root_.submonoid.prod_eq_top_iff {s : submonoid M} {t : submonoid N} :
  s.prod t = ⊤ ↔ s = ⊤ ∧ t = ⊤ :=
