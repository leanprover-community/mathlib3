...
@[to_additive nsmul_mem] lemma pow_mem {M : Type*} [monoid M] (S : submonoid M) {x : M}
  (hx : x ∈ S) (n : ℕ) : x ^ n ∈ S :=
begin
...
@[simp, norm_cast, to_additive coe_nsmul] theorem coe_pow  {M : Type*} [monoid M] {S : submonoid M}
  (x : S) (n : ℕ) : ↑(x ^ n) = (x ^ n : M) :=
rfl
