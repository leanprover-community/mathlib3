...
* `subsemigroup.copy` : copy of a submonoid with `carrier` replaced by a set that is equal but
...
/-- A submonoid of a monoid `M` can be considered as a subsemigroup of `M`-/
@[to_additive "An additive submonoid of an `add_monoid M` can be considered as an
`add_subsemigroup` of `M`", simps]
def to_subsemigroup {M : Type*} [mul_one_class M] (s : submonoid M) : subsemigroup M :=
{ ..s }
