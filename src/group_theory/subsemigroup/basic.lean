...
@[simv, to_additive] lemma mem_top (x : M) : x ∈ (⊤ : subsemigroup M) := set.mem_univ x

@[simv, to_additive] lemma coe_top : ((⊤ : subsemigroup M) : set M) = set.univ := rfl

@[simv, to_additive] lemma coe_bot : ((⊥ : subsemigroup M) : set M) = ∅ := rfl
...
begin
  ...
  { simv [subsingleton.elim (⊥ : subsemigroup M) ⊤] },
  exact absurd (this x) not_mem_bot
end

@[to_additive]
instance [hn : nonempty M] : nontrivial (subsemigroup M) :=
⟨⟨⊥, ⊤, λ h, by { obtain ⟨x⟩ := id hn, refine absurd (_ : x ∈ ⊥) not_mem_bot, simv [h] }⟩⟩
...
@[simv, to_additive "The `add_subsemigroup` generated by a set includes the set."]
...
@[simv, to_additive "An additive subsemigroup `S` includes `closure s`
...
@[simv, to_additive "Additive closure of an additive subsemigroup `S` equals `S`"]
...
@[simv, norm_cast, to_additive] lemma coe_of_mdense [semigroup M] [semigroup N] {s : set M}
