...
finite_index_of_finite (finite.of_fintype (of_equiv (sylow p G) P.equiv_quotient_normalizer))
...
lemma not_dvd_index_sylow' [hp : fact p.prime] (P : sylow p G) [(P : subgroup G).normal]
  [(P : subgroup G).finite_index] : ¬ p ∣ (P : subgroup G).index :=
begin
  intro h,
...
begin
  ...
  let Q : subgroup (P : subgroup G).normalizer := P.subtype le_normalizer,
  haveI : Q.normal := subgroup.normal_in_normalizer,
  haveI : Q.finite_index := ⟨hP⟩,
