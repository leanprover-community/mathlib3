...
`Π i, β i`.
* `hamming.normed_add_group`: the Hamming norm forms a normed group on `hamming β`.
-/
...
the Hamming metric and norm, instead of `pi.normed_add_group` which uses the sup norm. -/
...
instance [Π i, add_comm_group (β i)] : seminormed_add_group (hamming β) :=
...
instance [Π i, add_comm_group (β i)] : normed_add_group (hamming β) := { ..hamming.seminormed_add_group }
