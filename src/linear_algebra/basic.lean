...
/-- Alternative definition of an ideal generated by a set-/
def span' (s : set M) : submodule R M :=
{ carrier := { a | ∃ (t : finset s) (r : s → R), a = ∑ i in t, (r i) • i },
  zero_mem' := begin
    refine ⟨∅, ⟨λ t, 0, _⟩⟩,
    simp only [zero_smul, finset.sum_const_zero],
  end,
  add_mem' := λ a₁ a₂ ha₁ ha₂, begin
    obtain ⟨s₁, r₁, rfl⟩ := ha₁,
    obtain ⟨s₂, r₂, rfl⟩ := ha₂,
    use s₁ ∪ s₂,
    ...
    rw [show ∑ x in s₁ \ s₂, r₁ x • ↑x + (∑ x in s₁ ∩ s₂, r₁ x • ↑x + ∑ x in s₁ ∩ s₂, r₂ x • ↑x) +
            ...
            (∑ x in s₁ ∩ s₂, r₂ x • ↑x + ∑ x in s₂ \ s₁, r₂ x • ↑x), from _],
  ...
  smul_mem' := λ c a ha, begin
    obtain ⟨s, r, rfl⟩ := ha,
    rw [finset.smul_sum],
    use s, use c • r, congr, ext, simp only [smul_eq_mul, pi.smul_apply],
    exact smul_smul c (r x) ↑x,
  end }
end classical
