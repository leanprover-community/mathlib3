...
section
variables [ring_hom_surjective σ₁₂] {F : Type*} [mc : semilinear_map_class F σ₁₂ M M₂]
...
lemma map_to_add_submonoid (f : F) (p : submodule R M) :
...
variables {F : Type*} [mc : semilinear_map_class F σ₁₂ M M₂]
...
@[simp] protected lemma map_finsupp_sum (f : M →ₛₗ[σ₁₂] M₂) {t : ι →₀ γ} {g : ι → γ → M} :
...
variables {F : Type*} [mc : semilinear_map_class F τ₁₂ M M₂]
...
include mc
@[simp] theorem mem_range [ring_hom_surjective τ₁₂] {f : F} {x} : x ∈ range f ↔ ∃ y, f y = x :=
iff.rfl

lemma range_eq_map [ring_hom_surjective τ₁₂] (f : F) : range f = map f ⊤ :=
by { ext, simp }

theorem mem_range_self [ring_hom_surjective τ₁₂] (f : F) (x : M) : f x ∈ range f := ⟨x, rfl⟩
...
variables [ring_hom_comp_triple τ₁₂ τ₂₃ τ₁₃]
variables {F : Type*} [mc : semilinear_map_class F τ₁₂ M M₂] {f : F}
include R
open submodule
...
variables {τ₁₂ : R →+* R₂} {F : Type*} [mc : semilinear_map_class F τ₁₂ M M₂]
open linear_map
