...
/-- The perpendicular submodule of a submodule `N` is the set of elements all of which are
  orthogonal to all elements of `N`. -/
...
-- ↓ Move
lemma matrix.dot_product_vec_mul_eq_mul_vec
...
/-- Let `A` be a symmetric matrix. Then `A` has trivial kernel, that is it is invertible if and only
  if the induced bilinear form of `A` is nondegenerate. -/
...
lemma span_inf_ortho_eq_bot {B : bilin_form K V} {x : V} (hx : B x x ≠ 0) :
  submodule.span K ({x} : set V) ⊓ B.ortho (submodule.span K ({x} : set V)) = ⊥ :=
begin
