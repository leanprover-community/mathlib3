...
lemma injective_to_linear_map :
  function.injective (bilin_form.to_lin B₂) ↔ function.injective B₂ :=
...
{ to_fun := λ l,
  { to_fun := λ r,
    { to_fun := λ B, B.comp l r,
      map_add' := λ B B',
        by { ext, simp only [comp_apply, linear_map.add_apply, add_apply] },
      map_smul' := λ c B,
        by { ext, simp only [comp_apply, linear_map.smul_apply, smul_apply, smul_eq_mul] } },
    map_add' := λ r r',
      by { ext, simp only [linear_map.coe_mk, comp_apply, add_apply, add_right,
                           linear_map.add_apply] },
    map_smul' := λ c r,
      by { ext, simp only [linear_map.coe_mk, comp_apply, smul_apply, smul_right,
                           linear_map.smul_apply, smul_eq_mul] } },
  map_add' := λ r l',
    by { ext, simp only [linear_map.coe_mk, add_apply, comp_apply, add_left,
                         linear_map.add_apply] },
  map_smul' := λ c l,
    by { ext, simp only [linear_map.coe_mk, smul_apply, comp_apply, smul_left,
                         linear_map.smul_apply, smul_eq_mul] } }

@[simp] lemma comp_lin_apply (B : bilin_form R₂ M₂') (l r : M₂ →ₗ[R₂] M₂') :
