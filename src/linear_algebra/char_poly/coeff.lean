...
begin
  ...
  transitivity polynomial.sum (mat_poly_equiv M) (λ (e : ℕ) (a : matrix n n R),
    (a * (scalar n) r ^ e) i j),
  { unfold polynomial.sum matrix,
    -- `rw sum_apply` gets stuck here
    convert sum_apply j _ _,
    refine congr_fun _ j,
    dsimp,
    convert sum_apply i _ _, },
  { simp_rw ←ring_hom.map_pow,
    simp_rw ← (matrix.scalar.commute _ _).eq,
