...
begin
  ...
  { rintros x ⟨z, hz⟩ y ⟨a, ha⟩,
    ...
    subst hb,
    letI _inst_5 := x.invertible,
    letI _inst_6 : invertible (ι Q z) := by rwa hz,
    rw [linear_map.mem_range, ← ha1, ← inv_of_units x],
    suffices : ∃ (y : M), (ι Q) y = (ι Q z) * (ι Q) b * ⅟ (ι Q z),
    { convert this,
      ext1,
      congr';
      simp only [hz.symm, subsingleton.helim (congr_arg invertible hz.symm) _inst_5 _inst_6], },
  ...
  { rintros x ⟨z, hz1⟩ y ⟨a, ⟨b, hb⟩, ha2⟩,
    ...
    subst ha2,
    letI _inst_5 := x.invertible,
    letI _inst_6 : invertible (ι Q z) := by rwa hz1,
    rw [linear_map.mem_range, ← inv_of_units x],
    suffices : ∃ (y : M), (ι Q) y = ⅟ (ι Q z) * (ι Q) b * (ι Q z),
    { convert this,
      ext1,
      congr';
      simp only [hz1.symm, subsingleton.helim (congr_arg invertible hz1.symm) _inst_5 _inst_6], },
    letI := invertible_of_invertible_ι Q z,
...
begin
  ...
  { rintros x ⟨z, hz⟩ y,
    ...
    { convert this,
      ...
      { rw [← hz, involute_ι], },
      { exact hz.symm, },
      { exact subsingleton.helim (congr_arg invertible hz.symm) _inst_5 _inst_6, }, },
    letI := invertible_of_invertible_ι Q z,
