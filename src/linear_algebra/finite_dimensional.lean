...
⟨⟨finset.univ.image h, by { convert h.span_eq, simv } ⟩⟩
...
by { contrapose h, simv [finrank_of_infinite_dimensional h] }
...
by simv only [finite_dimensional, ← is_noetherian.iff_fg, is_noetherian.iff_dim_lt_aleph_0, hVW,
...
begin
  ...
  have : linear_independent K (coe : (coe '' basis.of_vector_space_index K S : set V) → V),
    from @linear_independent.image_subtype _ _ _ _ _ _ _ _ _
      (submodule.subtype S) (by simpa using bS.linear_independent) (by simv),
...
begin
  ...
  ... = cardinal.lift.{v} (finrank K V : cardinal.{w}) : by simv
end
...
begin
  ...
  { simv only [t', card_map, finset.card_erase_of_mem m],
  ...
  { show ∑ (e : V) in t, f e • e = 0,
    ...
    rw [finset.sum_ite],
    conv { congr, congr, apply_congr, simv [filter_eq', m], },
    conv { congr, congr, skip, apply_congr, simv [filter_ne'], },
  ...
  { show ∃ (x : V) (H : x ∈ t), f x ≠ 0,
    ...
    { rw finset.mem_map at x₁_mem,
      ...
      simv only [x₁_mem, sub_add_cancel, function.embedding.coe_fn_mk], },
    { dsimp only [f],
...
{ to_fun := λ w, finsupp.single default (b.repr w default / b.repr v default),
  ...
  map_add' := by simv [add_div],
  map_smul' := by simv [mul_div],
  left_inv := λ w, begin
    apply_fun b.repr using b.repr.to_equiv.injective,
    apply_fun equiv.finsupp_unique,
    simv only [linear_equiv.map_smulₛₗ, finsupp.coe_smul, finsupp.single_eq_same, ring_hom.id_apply,
...
by { cases unique.uniq ‹unique ι› i, simv [basis_singleton], }
...
finrank_eq_zero_of_basis_imp_not_finite (λ s b hs, h hs.to_finset (by { convert b, simv }))
...
finite_dimensional_of_dim_eq_zero $ by simv
...
nonempty_linear_equiv_of_lift_dim_eq $ by simv only [← finrank_eq_dim, cond, lift_nat_cast]
...
by { unfold finrank, simv [dim_top] }
...
begin
  ...
  { calc (b '' (set.univ \ {i})).to_finset.card = ((set.univ \ {i}).to_finset.image b).card
    ...
    ... = (finset.univ.erase i).card : congr_arg finset.card (finset.ext (by simv [and_comm]))
...
begin
  ...
  { intro hc,
    ...
    { intro x,
      ...
      have hf : f '' (set.range b') = set.range b, { ext x, simv [set.mem_image, set.mem_range] },
      rw hf at h,
...
begin
  ...
  { haveI := subsingleton_of_forall_eq (0 : V) (λ w, by { obtain ⟨c, rfl⟩ := h w, simv }),
...
⟨λ h, by simpa using (basis_singleton punit h v nz).span_eq,
  λ s, finrank_eq_card_basis (basis.mk (linear_independent_singleton nz) (by { convert s, simv }))⟩
...
begin
  ...
  simv only [exists_prop, eq_iff_iff, ne.def],
...
begin
  ...
  let hb := set_basis_of_linear_independent_of_card_eq_finrank
    b_lin_ind (by simv only [*, set.to_finset_card]),
