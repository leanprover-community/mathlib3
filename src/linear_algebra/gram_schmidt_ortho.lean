...
@[simp] lemma gram_schmidt_process_zero (f : ℕ → E) :
  gram_schmidt_process 𝕜 E f 0 = f 0 := by simp [gram_schmidt_process]
...
lemma span_gram_schmidt_process (f : ℕ → E) (c : ℕ) :
  span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c) = span 𝕜 (f '' Iic c) :=
begin
  induction c with c hc,
  { simp [Iic, gram_schmidt_process_zero] },
  apply le_antisymm,
  { have h₀ : ∀ x ∈ gram_schmidt_process 𝕜 E f '' Iic c.succ, x ∈ span 𝕜 (f '' Iic c.succ),
    { intros x hx,
      simp only [mem_image, mem_Iic] at hx,
      rcases hx with ⟨y, hy₁, hy₂⟩,
      rw nat.succ_eq_add_one at hy₁ ⊢,
      by_cases y ≤ c,
      { have h₁ : x ∈ gram_schmidt_process 𝕜 E f '' Iic c := ⟨y, h, hy₂⟩,
        have h₂ : gram_schmidt_process 𝕜 E f '' Iic c ⊆
          span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c) := subset_span,
        rw hc at h₂,
      ...
      { have h₁ : y = c + 1 := by linarith,
        ...
        { apply sum_mem _ _,
          ...
          { rw finset.mem_range at hc₁,
            linarith },
          have hc₃ : gram_schmidt_process 𝕜 E f c₁ ∈ span 𝕜 (f '' Iic (c + 1)),
          { have h₂ : gram_schmidt_process 𝕜 E f c₁ ∈ gram_schmidt_process 𝕜 E f '' Iic c,
            { simp only [mem_image, mem_Iic],
              refine ⟨c₁, hc₂, by refl⟩ },
            have h₃ : gram_schmidt_process 𝕜 E f '' Iic c ⊆
              span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c) := subset_span,
            have h₄ : gram_schmidt_process 𝕜 E f c₁ ∈
              span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c) := h₃ h₂,
            rw hc at h₄,
            have h₅ : span 𝕜 (f '' Iic c) ≤ span 𝕜 (f '' Iic (c + 1)),
            { have h₆ : f '' Iic c ⊆ f '' Iic (c + 1),
              { have h₇ : Iic c ⊆ Iic (c + 1),
          ...
          exact smul_mem (span 𝕜 (f '' Iic (c + 1)))
            ((inner (gram_schmidt_process 𝕜 E f c₁) (f (c + 1))
              / ∥gram_schmidt_process 𝕜 E f c₁∥ ^ 2) : 𝕜) hc₃ }}},
    have h₁ : gram_schmidt_process 𝕜 E f '' Iic c.succ ⊆ span 𝕜 (f '' Iic c.succ) := h₀,
    rwa ← span_le at h₁ },
  { have h₀ : ∀ x ∈ f '' Iic c.succ,
      x ∈ span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c.succ),
    { intros x hx,
      simp only [mem_image, mem_Iic] at hx,
      rcases hx with ⟨y, hy₁, hy₂⟩,
      rw nat.succ_eq_add_one at hy₁ ⊢,
      ...
      { have hx : x ∈ f '' Iic c := ⟨y, h, hy₂⟩,
        ...
        { have h₁ : f '' Iic c ⊆ span 𝕜 (f '' Iic c) := subset_span,
          exact h₁ hx },
        rw ← hc at hx₁,
        have h₂ : span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c)
          ≤ span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic (c + 1)),
        { have h₃ : gram_schmidt_process 𝕜 E f '' Iic c ⊆ gram_schmidt_process 𝕜 E f '' Iic (c+1),
          { have h₄ : Iic c ⊆ Iic (c + 1),
            { simp [Iic],
              intros a ha,
              linarith },
            exact image_subset (gram_schmidt_process 𝕜 E f) h₄ },
          exact span_mono h₃ },
        exact h₂ hx₁ },
      { have hy₃ : y = c + 1 := by linarith,
        rw hy₃ at hy₂,
        have hx : x = gram_schmidt_process 𝕜 E f (c + 1) + ∑ i in finset.range (c + 1),
          orthogonal_projection (𝕜 ∙ gram_schmidt_process 𝕜 E f i) x :=
            by simp [gram_schmidt_process_def, hy₂],
        simp only [orthogonal_projection_singleton] at hx,
        rw hx,
        apply add_mem _ _ _,
        { have h₁ : gram_schmidt_process 𝕜 E f (c + 1) ∈ gram_schmidt_process 𝕜 E f '' Iic c.succ,
          { simp only [mem_image, mem_Iic],
            refine ⟨c + 1, by simp only [nat.succ_eq_add_one], by refl⟩ },
          have h₂ : gram_schmidt_process 𝕜 E f '' Iic c.succ
            ⊆ span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c.succ) := subset_span,
          exact h₂ h₁ },
        { apply sum_mem _ _,
          intros c₁ hc,
          have hc₁ : gram_schmidt_process 𝕜 E f c₁ ∈
            span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c.succ),
          { have hc₂ : gram_schmidt_process 𝕜 E f c₁ ∈ gram_schmidt_process 𝕜 E f '' Iic c.succ,
            { simp only [mem_image, mem_Iic, nat.succ_eq_add_one],
              refine ⟨c₁, _, by refl⟩,
              rw finset.mem_range at hc,
              linarith },
            have hc₃ : gram_schmidt_process 𝕜 E f '' Iic c.succ ⊆
              span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c.succ) := subset_span,
            exact hc₃ hc₂ },
          exact smul_mem (span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic (c + 1)))
            ((inner (gram_schmidt_process 𝕜 E f c₁) x / ∥gram_schmidt_process 𝕜 E f c₁∥ ^ 2) : 𝕜)
              hc₁ }}},
    have h₁ : f '' Iic c.succ ⊆ span 𝕜 (gram_schmidt_process 𝕜 E f '' Iic c.succ) := h₀,
    rwa ← span_le at h₁ },
end
