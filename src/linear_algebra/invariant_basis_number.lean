...
The fact that any commutative ring satisfies the `strong_rank_condition` is proved in
...
/-!
  The following results were used to prove that commutative rings have the invariant basis number
  property. In `linear_algebra/free_module/strong_rank_condition` we prove more generally that
  commutative rings have the strong rank condition. We keep the following results as interesting
  in their own.
