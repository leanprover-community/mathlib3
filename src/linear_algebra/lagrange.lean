...
by { rw [basis, ← (s.erase x).prod_hom (eval x), finset.prod_eq_one],
...
by { rw [basis, ← (s.erase x).prod_hom (eval y), finset.prod_eq_zero (finset.mem_erase.2 ⟨h2, h1⟩)],
...
theorem nat_degree_basis (x : F) (hx : x ∈ s) : (basis s x).nat_degree = s.card - 1 :=
by { unfold basis, generalize hsx : s.erase x = sx,
...
theorem eval_interpolate (x ∈ s) : eval x (interpolate s f) = f ⟨x, H⟩ :=
by { rw [interpolate, ← finset.sum_hom _ (eval x), finset.sum_eq_single (⟨x, H⟩ : { x // x ∈ s })],
