...
The module `lc ι α β v` of linear combinations over `β`, indexed by ι via v (`α` is the scalar ring)
-/
...
/-- The type of linear combinations over an indexed family -/
@[reducible] def lc (ι : Type*) (α : Type*) [has_zero α] := ι →₀ α
...
instance : add_comm_group (lc ι α) := finsupp.add_comm_group

instance : has_scalar α (lc ι α) := finsupp.to_has_scalar

instance : module α (lc ι α) := finsupp.to_module ι α

variables (α)
def supported (s : set ι) : submodule α (lc ι α) :=
...
theorem mem_supported {s : set ι} {l : lc ι α} :
...
theorem supported_eq_span_single (s : set ι) :
  lc.supported α s = span α ((λ i, finsupp.single i (1 : α)) '' s) :=
begin
  refine (span_eq_of_le _ _ (le_def'.2 $ λ l hl, _)).symm,
  { rintro _ ⟨l, hl, rfl⟩, exact single_mem_supported (1:α) hl },
  { rw ← l.sum_single,
    refine sum_mem _ (λ i il, _),
    convert @smul_mem α (lc ι α) _ _ _ _ (finsupp.single i (1 : α)) (l i) _,
    { simp },
    apply subset_span,
...
variables (α)
def restrict_dom (s : set ι) : lc ι α →ₗ supported α s :=
...
@[simp] theorem restrict_dom_apply (s : set ι) (l : lc ι α) :
  ↑((restrict_dom α s : lc ι α →ₗ supported α s) l) = finsupp.filter (∈ s) l := rfl
...
theorem range_restrict_dom (s : set ι) : (restrict_dom α s).range = ⊤ :=
begin
...
@[simp] theorem supported_empty : supported α (∅ : set ι) = ⊥ :=
eq_bot_iff.2 $ λ l h, (submodule.mem_bot α).2 $
by ext; simp [*, mem_supported'] at *

@[simp] theorem supported_univ : supported α (set.univ : set ι) = ⊤ :=
...
def apply (i : ι) : lc ι α →ₗ α :=
⟨λ l, l i, λ _ _, finsupp.add_apply, λ _ _, finsupp.smul_apply⟩

@[simp] theorem apply_apply (i : ι) (l : lc ι α) :
  (lc.apply i : lc ι α →ₗ α) l = l i := rfl

protected def lsum (f : ι → α →ₗ[α] γ) : lc ι α →ₗ[α] γ :=
⟨λ d, d.sum (λ i, f i),
...
@[simp] theorem lsum_apply (f : ι → α →ₗ γ) (l : lc ι α) :
  (lc.lsum f : lc ι α →ₗ γ) l = l.sum (λ b, f b) := rfl

section
variables (ι α β v)
protected def total : lc ι α →ₗ β := lc.lsum (λ i, linear_map.id.smul_right (v i))
end

theorem total_apply (l : lc ι α) :
...
begin
  ...
  intros l hl,
  refine ⟨(lc.map α (inv_fun_on f s) : lc ι' α →ₗ lc ι α) l, λ x hx, _, _⟩,
...
theorem map_total (f : ι → ι') (g : β →ₗ[α] β') (h : ∀ i, g (v i) = v' (f i)):
  (lc.total ι' α β' v').comp (lc.map α f) = g.comp (lc.total ι α β v) :=
...
instance : vector_space α (lc ι α) := { .. lc.module }
...
theorem span_eq_map_lc : span α (v '' s) = (lc.supported α s).map (lc.total ι α β v) :=
begin
  apply span_eq_of_le,
  { intros x hx,
    ...
    intros i hi,
    exact ⟨_, lc.single_mem_supported 1 hi.1, by simp [hi.2]⟩ },
...
theorem mem_span_iff_lc :
  x ∈ span α (v '' s) ↔ ∃ l ∈ lc.supported α s, lc.total ι α β v l = x :=
by rw span_eq_map_lc; simp

variables (α) (β) (v)
def lc.total_on (s : set ι) : lc.supported α s →ₗ span α (v '' s) :=
