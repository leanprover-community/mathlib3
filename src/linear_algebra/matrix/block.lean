...
variables {α m n o : Type*} [decidable_eq n] [fintype n] [decidable_eq m] [fintype m]
...
def block_triangular [has_lt α] (M : matrix o o R) (b : o → α) : Prop :=
∀ ⦃i j⦄, b j < b i → M i j = 0
...
lemma det_of_block_triangular (M : matrix m m R) (b : m → ℕ) (h : M.block_triangular b) :
  ∀ (n : ℕ) (hn : ∀ i, b i < n), M.det = ∏ k in range n, (to_square_block' M b k).det :=
begin
  intros n hn,
  ...
  { intros i hi j hj,
    apply h, simp only [not_not] at hi,
    rw hi,
    exact hj.lt_of_le (nat.lt_succ_iff.mp $ hn j) }
end

lemma det_of_block_triangular'' (M : matrix m m R) (b : m → ℕ) (h : block_triangular M b) :
  M.det = ∏ k in univ.image b, (to_square_block' M b k).det :=
begin
...
begin
  ...
  { intros i j hij, exact h (fin.coe_fin_lt.mp hij) },
  { intro i, exact fin.is_lt (b i) }
end

lemma det_of_upper_triangular {n : ℕ} (M : matrix (fin n) (fin n) R) (h : M.block_triangular id) :
  M.det = ∏ i : (fin n), M i i :=
begin
  rw det_of_block_triangular M coe h _ _,
  ext i,
