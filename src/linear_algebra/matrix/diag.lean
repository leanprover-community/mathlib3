...
@[simp] lemma is_diag.apply_ne [has_zero α] {A : matrix n n α}
  (ha : A.is_diag) {i j : n} (h : ¬ i = j) :
A i j = 0 := ha h

/-- Non-diagonal entries equal zero. -/
@[simp] lemma is_diag.apply_ne' [has_zero α] {A : matrix n n α}
  (ha : A.is_diag) {i j : n} (h : ¬ j = i) :
...
@[simp] lemma is_diag_unit [has_zero α] (A : matrix unit unit α) : A.is_diag :=
by {intros i j h, have h':= @unit.ext i j, contradiction}
...
begin
  ext,
  by_cases g : i = j, {rw g},
  simp [h.apply_ne g, h.apply_ne' g],
end
