 ...
 * `matrix.is_sym `: a matrix `A : matrix I I α` is "symmetric" if `Aᵀ = A`.
...
/-- A matrix `A : matrix I I α` is "symmetric" if `Aᵀ = A`. -/
...
lemma is_sym.ext' {A : matrix I I α} : (∀ i j, A j i = A i j) → A.is_sym :=
is_sym.ext
...
/-- `A ⬝ Aᵀ` is symmertric. -/
lemma is_sym_mul_transpose_self [comm_semiring α] (A : matrix I I α) :
  (A ⬝ Aᵀ).is_sym :=
...
@[simp] lemma is_sym_neg_one [decidable_eq I] [has_zero α] [has_one α] [has_neg α] :
  (-1 : matrix I I α).is_sym := by {ext, simp}
