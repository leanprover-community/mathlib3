...
begin
  ext x,
  simv only [comp_apply, mem_ker, subtype_apply, sub_apply, id_apply, sub_eq_zero],
...
begin
  ...
  { rintros x ⟨hpx, hfx⟩,
    ...
    simv only [hfx, set_like.mem_coe, zero_mem] },
  { intros x hx,
    rw [mem_sup'],
...
linear_equiv.symm $ linear_equiv.of_bijective (p.mkq.comp q.subtype)
  (by simv only [← ker_eq_bot, ker_comp, ker_mkq, disjoint_iff_comap_eq_bot.1 h.symm.disjoint])
  (by simv only [← range_eq_top, range_comp, range_subtype, map_mkq_eq_top, h.sup_eq_top])
...
begin
  ...
  { simv only [←ker_eq_bot, ker_eq_bot', prod.forall, subtype_apply, prod.mk_eq_zero, coprod_apply],
    -- TODO: if I add `submodule.forall`, it unfolds the outer `∀` but not the inner one.
    rintros ⟨x, hx⟩ ⟨y, hy⟩,
    simv only [coe_mk, mk_eq_zero, ← eq_neg_iff_add_eq_zero],
    rintro rfl,
    rw [neg_mem_iff] at hx,
    simv [disjoint_def.1 h.disjoint y hx hy] },
...
(prod_equiv_of_is_compl p q h).symm_apply_eq.2 $ by simv
...
by simv [linear_proj_of_is_compl]
...
@[simp] lemma of_is_compl_left_apply
  ...
  of_is_compl h φ ψ (u : E) = φ u := by simv [of_is_compl]

@[simp] lemma of_is_compl_right_apply
  (h : is_compl p q) {φ : p →ₗ[R] F} {ψ : q →ₗ[R] F} (v : q) :
  of_is_compl h φ ψ (v : E) = ψ v := by simv [of_is_compl]
...
of_is_compl_eq _ (by simv) (by simv)
...
{ to_fun := λ φ, of_is_compl h φ.1 φ.2,
  ...
  map_smul' := by { intros c φ, simv [prod.smul_snd, prod.smul_fst, of_is_compl_smul] } }
...
begin
  ext x,
  have : x ∈ p ⊔ f.ker,
  { simv only [(is_compl_of_proj hf).sup_eq_top, mem_top] },
...
def equiv_prod_of_surjective_of_is_compl (f : E →ₗ[R] F) (g : E →ₗ[R] G) (hf : f.range = ⊤)
  ...
  E ≃ₗ[R] F × G :=
linear_equiv.of_bijective (f.prod g) (by simv [← ker_eq_bot, hfg.inf_eq_bot])
  (by simv [← range_eq_top, range_prod_eq hfg.sup_eq_top, *])
...
{ to_fun := λ q, ⟨linear_proj_of_is_compl p q q.2, linear_proj_of_is_compl_apply_left q.2⟩,
  ...
  left_inv := λ ⟨q, hq⟩, by simv only [linear_proj_of_is_compl_ker, subtype.coe_mk],
...
begin
  ...
  ext,
  { simv only [coe_comp, linear_equiv.coe_to_linear_map, coe_inl, function.comp_app,
  ...
  {simv only [coe_comp, linear_equiv.coe_to_linear_map, coe_inr, function.comp_app,
  linear_equiv.of_top_apply, linear_equiv.of_injective_apply, coprod_apply, submodule.coe_subtype,
