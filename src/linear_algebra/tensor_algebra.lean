...
2. `canon R M` is the canonical R-linear map `M → tensor_algebra R M`.
...
1. `canon_comp_lift` states that the composition
  `(lift R M f) ∘ (canon R M)` is identical to `f`.
2. `lift_unique` states that whenever an R-algebra morphism
  `g : tensor_algebra R M → A` is given whose composision with `canon R M` is `f`,
...
variables (M : Type u₂) [add_comm_group M] [semimodule R M]
...
-/
def lift_fun {A : Type u₃} [semiring A] [algebra R A] (f : M →ₗ[R] A) : pre R M → A :=
...
-/
def canon : M →ₗ[R] (tensor_algebra R M) :=
...
`tensor_algebra R M → A`.
-/
def lift {A : Type u₃} [semiring A] [algebra R A] (f : M →ₗ[R] A) : tensor_algebra R M →ₐ[R] A :=
...
theorem canon_comp_lift {A : Type u₃} [semiring A] [algebra R A] (f : M →ₗ[R] A) :
  (lift R M f).to_linear_map.comp (canon R M) = f := by {ext, refl}

theorem lift_unique {A : Type u₃} [semiring A] [algebra R A] (f : M →ₗ[R] A)
  (g : tensor_algebra R M →ₐ[R] A) : g.to_linear_map.comp (canon R M) = f → g = lift R M f :=
begin
  intro hyp,
  ext,
  ...
  induction x,
  { change (g.to_linear_map.comp (canon R M)) _ = _,
    rw hyp, refl },
...
theorem hom_ext {A : Type u₃} [semiring A] [algebra R A] {f g : tensor_algebra R M →ₐ[R] A} :
  f.to_linear_map.comp (canon R M) = g.to_linear_map.comp (canon R M) → f = g := λ hyp,
begin
  let h := g.to_linear_map.comp (canon R M),
  rw (show g = lift R M h, by apply lift_unique; refl),
  apply lift_unique, rw hyp,
end
