...
@[simp] lemma tpower_apply {n : ℕ} (x : fin n → M) :
  tpower R M n x = (list.of_fn (λ i, ι R (x i))).prod := rfl

variables {R M}

/-- The canonical embedding from a tensor power to the tensor algebra -/
def _root_.tensor_power.to_tensor_algebra {n} : ⨂[R]^n M →ₗ[R] tensor_algebra R M :=
pi_tensor_product.lift (tpower R M n)

@[simp]
lemma _root_.tensor_power.to_tensor_algebra_tprod {n} (x : fin n → M) :
  tensor_power.to_tensor_algebra (pi_tensor_product.tprod R x) = tpower R M n x :=
pi_tensor_product.lift.tprod _

@[simp]
lemma _root_.tensor_power.to_tensor_algebra_ghas_one :
  (@graded_monoid.ghas_one.one _ (λ n, ⨂[R]^n M) _ _).to_tensor_algebra = 1 :=
tensor_power.to_tensor_algebra_tprod _

@[simp]
lemma _root_.tensor_power.to_tensor_algebra_ghas_mul {i j} (a : ⨂[R]^i M) (b : ⨂[R]^j M) :
...
begin
  ...
  simp only [linear_map.compr₂_apply, algebra.lmul_apply, alg_hom.to_linear_map_apply,
    ...
    tensor_power.to_tensor_algebra_tprod, tpower_apply],
...
@[simp]
lemma _root_.tensor_power.to_tensor_algebra_galgebra_to_fun (r : R) :
...
@[simp] lemma of_direct_sum_of_tprod {n} (x : fin n → M) :
  of_direct_sum (direct_sum.of _ n (pi_tensor_product.tprod R x)) = tpower R M n x :=
...
begin
  ext,
  simp [direct_sum.lof_eq_of, tpower_apply],
end
...
lemma to_direct_sum_tensor_power_tpower {n} (x : fin n → M) :
  to_direct_sum (tpower R M n x) = direct_sum.of _ n (pi_tensor_product.tprod R x) :=
begin
  rw [tpower_apply, alg_hom.map_list_prod, list.map_of_fn, function.comp],
...
begin
  ext,
  simp [direct_sum.lof_eq_of, -tpower_apply, to_direct_sum_tensor_power_tpower],
end
...
@[simps]
def equiv_direct_sum :
  tensor_algebra R M ≃ₐ[R] ⨁ n, ⨂[R]^n M :=
