...
@[simp] lemma and_self_left (a b : Prop) : a ∧ a ∧ b ↔ a ∧ b :=
⟨λ h, ⟨h.1, h.2.2⟩, λ h, ⟨h.1, h.1, h.2⟩⟩

@[simp] lemma and_self_right (a b : Prop) : (a ∧ b) ∧ b ↔ a ∧ b :=
...
@[simp] lemma or_self_left (a b : Prop) : a ∨ a ∨ b ↔ a ∨ b :=
⟨λ h, h.elim or.inl id, λ h, h.elim or.inl (or.inr ∘ or.inr)⟩

@[simp] lemma or_self_right (a b : Prop) : (a ∨ b) ∨ b ↔ a ∨ b :=
