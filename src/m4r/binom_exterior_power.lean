import linear_algebra.free_module linear_algebra.basis linear_algebra.direct_sum.finsupp m4r.inductive_koszul m4r.exterior_power
universes u w v
...
lemma is_basis.tensor_power {b : ι → M} (hb : is_basis R b) (n : ℕ) :
  is_basis R (λ (i : fin n → ι), tensor_power.mk R M n (b ∘ i)) :=
begin
...
lemma exists_same2_span_disjoint (n i : ℕ) :
  disjoint (submodule.span R ((λ (j : fin i → fin n), tensor_power.mk R (fin n → R) i
    (λ (k : fin i), (finsupp.single (j k) 1 : fin n → R))) '' (exists_same2 n i)))
    (submodule.span R ((λ (j : fin i → fin n), tensor_power.mk R (fin n → R) i
...
@[simp] lemma multiply_repr_apply {i n : ℕ} {y : fin i.succ → fin n → R}
  {z : fin i.succ → fin n} :
  multiply_repr R i.succ n y z = y 0 (z 0) * multiply_repr R i n (fin.tail y) (fin.tail z) :=
by unfold multiply_repr; refl

noncomputable def multiply_repr' (i n : ℕ) (y : fin i → fin n → R) :
  ((fin i → fin n) →₀ R) :=
{ support := set.to_finset {f : fin i → fin n | finset.univ.prod (λ j, y j (f j)) ≠ 0 },
  to_fun := λ f, finset.univ.prod (λ j, y j (f j)),
  mem_support_to_fun := λ x, by simp }

def smul_tail {i n : ℕ} {y : fin i.succ → fin n → R} (z : fin i.succ → fin n)
  (h : z ∈ (multiply_repr R i.succ n y).support) :
  fin i → fin n :=
sorry
/-
lemma smul_tail_multiply {i n : ℕ} {y : fin i.succ → fin n → R}
  (z : fin i.succ → fin n) (h : z ∈ (multiply_repr R i.succ n y).support) :
  multiply_repr R i n (fin.tail y) (smul_tail R z h) = multiply_repr R i.succ n y z :=
begin
  unfold multiply_repr,
  show multiply_repr R i n (fin.tail y) (smul_tail R z h) =
    y 0 (z 0) * multiply_repr R i n (fin.tail y) (fin.tail z),

end
-/

--lemma multiply_repr_support {n i : ℕ} {y : fin i.succ → fin n → R} :
--  (multiply_repr R i.succ n y).support =

lemma tensor_power_repr_eq (n i : ℕ) (z : fin i → fin n → R) :
  (is_basis.tensor_power (is_basis_single_one' R n) i).repr
    (tensor_power.mk R (fin n → R) i z) =
  multiply_repr R i n z :=
begin
  funext,
  refine linear_independent.repr_eq _ _,
  induction i with i hi,
  { unfold tensor_power.mk multiply_repr,
    rw finsupp.total_unique,
    exact one_smul _ _, },
  { rw finsupp.total_apply,
    unfold finsupp.sum,
    simp only [multiply_repr_apply],
    sorry  },
  --simp only [linear_map.id_coe, id.def, linear_map.cod_restrict_apply],
  -- (is_basis.tensor_power (is_basis_single_one' R n) i).1.repr_eq,
 -- have := (is_basis.tensor_power (is_basis_single_one' R n) i).total_repr (tensor_power.mk R (fin n → R) i z),
 -- unfold multiply_repr,
  --ext f,
  --unfold is_basis.repr,
  --refine linear_independent.repr_eq _ _,


 /- induction i with i hi,
  { unfold multiply_repr tensor_power.mk,
    ext,
...
begin
  ...
  induction i with i hi,
  { exact fin.elim0 j },
  { rw multiply_repr_apply at hx, sorry
  }
end

lemma tensor_power_exists_same2 (n i : ℕ) :
  submodule.span R ((λ (j : fin i → fin n), tensor_power.mk R (fin n → R) i
    (λ (k : fin i), (finsupp.single (j k) 1 : fin n → R))) '' (exists_same2 n i)) =
  exterior_power_ker R (fin n → R) i :=
begin
...
lemma tensor_power_exists_same2_compl (n i : ℕ) :
  submodule.span R ((λ (j : fin i → fin n), tensor_power.mk R (fin n → R) i
    (λ (k : fin i), (finsupp.single (j k) 1 : fin n → R))) '' (exists_same2 n i)ᶜ) =
  submodule.span R (tensor_power.mk R (fin n → R) i '' (exists_same (fin n → R) i)ᶜ) :=
sorry

lemma disjoint_exterior_power_ker_compl (n i : ℕ) :
  disjoint (exterior_power_ker R (fin n → R) i)
  (submodule.span R
    (tensor_power.mk R (fin n → R) i '' (exists_same (fin n → R) i)ᶜ)) :=
begin
  have := @linear_independent.disjoint_span_image (fin i → fin n) R
    (tensor_power R (fin n → R) i) (λ (j : fin i → fin n), tensor_power.mk R (fin n → R) i
    ...
    (le_bot_iff.2 (set.inter_compl_self _)),
  rwa [tensor_power_exists_same2_compl, tensor_power_exists_same2] at this,
end

def aux_fun (n i : ℕ) : (exists_same2 n i)ᶜ → tensor_power R (fin n → R) i :=
(λ (j : fin i → fin n), tensor_power.mk R (fin n → R) i
    (λ (k : fin i), (finsupp.single (j k) 1 : fin n → R))) ∘ subtype.val

lemma lemma_i_actually_want (n i : ℕ) :
  disjoint (submodule.span R (set.range (aux_fun R n i))) (exterior_power_ker R (fin n → R) i) :=
begin
  rw ←tensor_power_exists_same2 R n i,
  have := @linear_independent.disjoint_span_image (fin i → fin n) R
    (tensor_power R (fin n → R) i) (λ (j : fin i → fin n), tensor_power.mk R (fin n → R) i
      ((λ (i : fin n), (finsupp.single i 1 : fin n → R)) ∘ j)) _ _ _
    (is_basis.tensor_power (is_basis_single_one' R n) i).1
...
def exterior_power.to_basis (n i : ℕ) (l : list (fin n)) (hli : l.length = i) :=
exterior_power.mk R (fin n → R) i (λ j, finsupp.single (l.nth_le j (hli.symm ▸ j.2)) (1 : R))

def exterior_power.basis (n i : ℕ) (j : fin (n.choose i)) : exterior_power R (fin n → R) i :=
exterior_power.to_basis R n i
  ((list.sublists_len i (list.fin_range n)).nth_le j -- the `j`th entry in the list of all sublists of `[0, ..., n - 1]` of length `i`
...
theorem exterior_power_eq_zero_iff (n i : ℕ) (f : fin i → fin n) :
  exterior_power.mk R (fin n → R) i (λ j, function.update 0 (f j) 1) = 0 ↔
...
theorem exterior_power.basis_li (n i : ℕ) : linear_independent R (exterior_power.basis R n i) :=
begin
  have := jfc R (λ (j : fin i → fin n), tensor_power.mk R (fin n → R) i
    (λ (k : fin i), (finsupp.single (j k) 1 : fin n → R))) (exists_same2 n i)ᶜ
    (is_basis.tensor_power (is_basis_single_one' R n) i).1,
  have hhh := lemma_i_actually_want R n i,
  rw ←(exterior_power_ker R (fin n → R) i).ker_mkq at hhh,
...
theorem exterior_power.basis_span (n i : ℕ) : submodule.span R (set.range $ exterior_power.basis R n i) = ⊤ :=
begin
  have : submodule.span R (set.range $ exterior_power.basis R n i) =
    (submodule.span R (set.range (aux_fun R n i))).map (exterior_power_ker R (fin n → R) i).mkq :=
  begin
  ...
  rw [this, submodule.map_mkq_eq_top, ←tensor_power_exists_same2 R n i, ←submodule.span_union],
...
lemma exterior_power.is_basis (n i : ℕ) : is_basis R (exterior_power.basis R n i) :=
⟨exterior_power.basis_li R n i, exterior_power.basis_span R n i⟩

def exterior_power.equiv_choose (n i : ℕ) : exterior_power R (fin n → R) i ≃ₗ[R] (fin (n.choose i) → R) :=
is_basis.equiv_fun (exterior_power.is_basis R n i)

def exterior_power.equiv_succ (n i : ℕ) :
  exterior_power R (fin n.succ → R) i.succ ≃ₗ[R] exterior_power R (fin n → R) i × exterior_power R (fin n → R) i.succ :=
(exterior_power.equiv_choose R n.succ i.succ).trans $ (rk_free_prod_eq_add R (n.choose i) (n.choose i.succ)).symm.trans
  (linear_equiv.prod (exterior_power.equiv_choose R n i).symm (exterior_power.equiv_choose R n i.succ).symm)

def Koszul_isom_inductive_Koszul (n : ℕ) (x : fin n.succ → R) :
  Koszul R (fin n.succ → R) x ≅ inductive_Koszul R n x :=
{ hom :=
  { f := λ i, int.rec_on i
      (λ i, ((exterior_power.equiv_choose R n.succ i).trans
        (inductive_Koszul_isom_choose R i x).to_linear_equiv.symm).to_linear_map) 0,
    comm := λ i j,
  begin
    ext y,
