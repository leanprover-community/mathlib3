...
lemma linear_map.map_sum₂ {P : Type*} [add_comm_group P] [module R P]
...
begin
  ...
  congr' 2,
  rw sum_apply',
  rw mul_def, rw mul_def,
  rw talg.sum_mul, rw talg.mul_sum,
...
{ left_distrib := by exact talg.mul_add R M,
  right_distrib := by exact talg.add_mul R M, ..direct_sum.add_comm_group _, ..talg.monoid _ _ }
...
lemma talg.of_scalar_apply {x : R} : talg.of_scalar R M x = direct_sum.lof R ℕ (tpow R M) 0 x := rfl
...
{ to_fun := direct_sum.to_module R ℕ A $ λ n, @tpow.lift R _ M _ _ n A _ _ (alg_prod f n),
  ...
  map_mul' := λ x y,
    begin
      ...
      congr' 1,
      ext z,
      simp only [function.comp_app, algebra.smul_mul_assoc],
      congr,
      erw tpow.lift_mk_apply,
      ...
      congr' 1,
      ext w,
      simp only [algebra.mul_smul_comm, function.comp_app],
      congr,
      erw tpow.lift_mk_apply,
      erw mul_apply,
      erw to_module_lof,
...
begin
  ...
  rw map_sum,
  simp only [map_smul_eq_smul_map],
  rw alg_hom.map_sum',
  rw multiset.map_map,
  rw alg_hom.map_sum',
  rw multiset.map_map,
  ...
  have := hi (fin.init y2),
  rw fin.comp_init at this,
  rw fin.comp_init at this,
  rw list.concat_eq_append,
  rw list.prod_append,
  rw this,
  rw function.comp_app, rw function.comp_app,
...
begin
  ...
  rw tpow.lift_mk_apply,
  show list.prod _ = _,
  simp only [list.repeat, mul_one, function.comp_const, list.prod_cons, list.prod_nil, list.of_fn_const],
  { intros y z hy hz,
    simp only [alg_hom.map_add, hy, hz] },
