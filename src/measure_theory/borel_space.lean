...
class borel_space (α : Type*) [topological_space α] [measurable_space α] :=
...
lemma is_open.is_measurable : is_open s → is_measurable s :=
(borel_space.measurable_eq α).symm ▸ generate_measurable.basic _
...
lemma continuous.measurable {f : α → β} (hf : continuous f) : measurable f :=
‹borel_space β›.1.symm ▸ ‹borel_space α›.1.symm ▸ hf.borel_measurable
...
le_antisymm prod_le_borel_prod begin
    ...
    exact generate_from_le (assume p ⟨u, hu, v, hv, eq⟩,
      ...
      eq.symm ▸ is_measurable_set_prod hu.is_measurable hv.is_measurable)
end⟩
...
@[to_additive]
lemma measurable_inv_iff [group α] [topological_group α] [measurable_space γ] {f : γ → α} :
...
lemma measurable.smul [semiring α] [second_countable_topology α]
  [add_comm_monoid β] [second_countable_topology β]
  [semimodule α β] [topological_semimodule α β]
  {f : δ → α} {g : δ → β} (hf : measurable f) (hg : measurable g) :
  measurable (λ c, f c • g c) :=
continuous_smul.measurable2 hf hg

lemma measurable.const_smul [semiring α] [second_countable_topology α]
  [add_comm_monoid β] [second_countable_topology β]
  [semimodule α β] [topological_semimodule α β]
  {f : δ → β} (hf : measurable f) (c : α) :
  measurable (λ x, c • f x) :=
measurable_const.smul hf

lemma measurable_const_smul_iff [division_ring α] [second_countable_topology α]
  [add_comm_monoid β] [second_countable_topology β]
  [semimodule α β] [topological_semimodule α β]
  {f : δ → β} {c : α} (hc : c ≠ 0) :
...
lemma measurable.supr [complete_linear_order α] [order_topology α] [second_countable_topology α]
  {ι} [encodable ι] {f : ι → δ → α} (hf : ∀ i, measurable (f i)) :
  measurable (λ b, ⨆ i, f i b) :=
measurable.is_lub hf $ λ b, is_lub_supr

lemma measurable.infi [complete_linear_order α] [order_topology α] [second_countable_topology α]
  {ι} [encodable ι] {f : ι → δ → α} (hf : ∀ i, measurable (f i)) :
  measurable (λ b, ⨅ i, f i b) :=
...
variables [metric_space α] [measurable_space α] [borel_space α] {x : α} {ε : ℝ}
...
variables [emetric_space α] [measurable_space α] [borel_space α] {x : α} {ε : ennreal}
...
{ to_fun    := λr, ennreal.to_nnreal r.1,
  ...
  measurable_inv_fun := measurable.subtype_mk measurable_id.ennreal_coe }
...
variables [measurable_space α] [normed_group α] [borel_space α] [measurable_space β]
