...
lemma cardinal_generate_measurable_rec_le_aux {s : set (set α)} (hs : #s ≤ 𝔠) (i : ω₁) :
...
begin
  ...
  { apply (mk_Union_le _).trans,
    ...
    have B : cardinal.sup.{u u} (λ (j : {j // j <₁ i}), #(generate_measurable_rec s j.1)) ≤ 𝔠,
...
begin
  ...
  { rcases mem_Union.1 IH with ⟨i, hi⟩,
    obtain ⟨j, hj⟩ : ∃ j, i <₁ j,
    { apply ordinal.has_succ_of_is_limit,
      simp [ord_aleph_is_limit 1] },
