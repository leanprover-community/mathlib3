...
lemma cardinal_generate_measurable_rec_le_aux {s : set (set α)} (hs : 2 ≤ #s) (i : ω₁) :
  #(generate_measurable_rec s i) ≤ #s ^ omega.{u} :=
begin
  apply well_founded.induction (aleph 1 : cardinal.{u}).ord.out.wo.wf i,
  assume i IH,
  have A := omega_le_aleph 1,
  have B : aleph 1 ≤ #s ^ omega.{u} := aleph_one_le_continuum.trans (power_le_power_right hs),
  have C : omega.{u} ≤ #s ^ omega.{u} := A.trans B,
  have J : #(⋃ (j : {j // j <₁ i}), generate_measurable_rec s j.1) ≤ #s ^ omega.{u},
  { apply (mk_Union_le _).trans,
    have D : # {j // j <₁ i} ≤ aleph 1 := (mk_subtype_le _).trans (le_of_eq (aleph 1).mk_ord_out),
    have E : cardinal.sup.{u u}
      (λ (j : {j // j <₁ i}), #(generate_measurable_rec s j.1)) ≤ #s ^ omega.{u} :=
  ...
  { nth_rewrite 0 ←@power_one (#s),
    have : (0 : cardinal.{u}) < 2 := by simp,
    exact power_le_power_left (this.trans_le hs).ne' one_lt_omega.le },
  { rw [mk_singleton],
...
lemma cardinal_generate_measurable_rec_le {s : set (set α)} (hs : 2 ≤ #s) :
  #(⋃ i, generate_measurable_rec s i) ≤ #s ^ omega.{u} :=
begin
  apply (mk_Union_le _).trans,
  rw [(aleph 1).mk_ord_out],
  refine le_trans (mul_le_mul' aleph_one_le_continuum
    (cardinal.sup_le.2 (λ i, cardinal_generate_measurable_rec_le_aux hs i))) _,
  have := power_le_power_right hs,
...
theorem cardinal_generate_measurable_le {s : set (set α)} (hs : 2 ≤ #s) :
  #{t | generate_measurable s t} ≤ #s ^ omega.{u} :=
(mk_subtype_le_of_subset (generate_measurable_subset_rec s)).trans
  (cardinal_generate_measurable_rec_le hs)

/-- If a sigma-algebra is generated by a set of sets with cardinality at most `𝔠`, then the sigma
algebra satisfies the same cardinality bound. -/
theorem cardinal_measurable_set_le {s : set (set α)} (hs : 2 ≤ #s) :
  #{t | @measurable_set α (generate_from s) t} ≤ #s ^ omega.{u} :=
