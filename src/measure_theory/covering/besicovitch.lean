...
begin
  ...
  { simp only [not_exists, true_and, exists_prop, mem_Union, mem_singleton_iff, mem_closed_ball,
      not_and, mem_univ, mem_sdiff, subtype.exists, subtype.coe_mk],
    assume j ji hj,
  ...
  { assume k hk,
    rw ← Inf_eq_N at hk,
    have : k ∈ A,
      by simpa only [true_and, mem_univ, not_not, mem_sdiff] using nat.not_mem_of_lt_Inf hk,
    simp at this,
  ...
  { -- show that balls of the same color are disjoint
    ...
    { rw color_j,
      ...
      simp only [not_exists, true_and, exists_prop, mem_Union, mem_singleton_iff, not_and, mem_univ,
        mem_sdiff, subtype.exists, subtype.coe_mk],
      assume k hk H,
      exact (p.color_lt (hk.trans jy_lt) hN).ne' },
    simp only [not_exists, true_and, exists_prop, mem_Union, mem_singleton_iff, not_and, mem_univ,
      mem_sdiff, subtype.exists, subtype.coe_mk] at this,
...
begin
  ...
  { have : μ s = 0 := le_bot_iff.1 hμs,
    ...
    { simp only [this, sdiff_empty, Union_false, Union_empty, nonpos_iff_eq_zero, mul_zero] },
...
begin
  ...
  { assume t ht,
    ...
    { assume x hx,
      ...
      { let R := inf_dist x B,
        have : 0 < min R 1 :=
          lt_min ((B_closed.not_mem_iff_inf_dist_pos hB).1 ((mem_sdiff x).1 hx).2) zero_lt_one,
    ...
    { convert hμv using 2,
      rw [finset.set_bUnion_union, ← sdiff_sdiff, finset.set_bUnion_finset_image] } },
  ...
  { assume x hx,
    ...
    { obtain ⟨i, y, ySi, xy⟩ : ∃ (i : fin N) (y : ↥s') (ySi : y ∈ S i), x ∈ ball (y : α) (r1 y),
      { have A : x ∈ range q.c, by simpa only [not_exists, exists_prop, mem_Union, mem_closed_ball,
          not_and, not_le, mem_set_of_eq, subtype.range_coe_subtype, mem_sdiff] using h'x,
