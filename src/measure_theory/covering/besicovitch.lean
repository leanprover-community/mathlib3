...
begin
  ...
  simp only [nonempty_def, not_exists, exists_prop, not_and, not_lt, not_le, mem_set_of,
    not_forall] at h,
  specialize h y,
...
begin
  ...
  { have : p.last_step ∈ {i | ¬ ∃ (b : β), p.c b ∉ p.Union_up_to i ∧ p.R i ≤ p.τ * p.r b} :=
    ...
    simpa only [not_exists, mem_set_of, not_and_distrib, not_le, not_not_mem] },
  by_contra,
...
begin
  ...
  { assume n hn,
    ...
    have : ∃ t, p.c t ∉ p.Union_up_to (G n) ∧ p.R (G n) ≤ p.τ * p.r t,
      by simpa only [not_exists, exists_prop, not_and, not_lt, not_le, mem_set_of,
  ...
  { assume x hx,
    ...
    { obtain ⟨i, y, ySi, xy⟩ : ∃ (i : fin N) (y : ↥s') (ySi : y ∈ S i), x ∈ ball (y : α) (r1 y),
      { have A : x ∈ range q.c, by simpa only [not_exists, exists_prop, mem_Union, mem_closed_ball,
          not_and, not_le, mem_set_of, subtype.range_coe_subtype, mem_diff] using h'x,
