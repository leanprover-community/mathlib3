...
theorem with_densityᵥ_rn_deriv_eq
  (c : complex_measure α) (μ : measure α) [sigma_finite μ]
  (h : c ≪ᵥ μ.to_ennreal_vector_measure) :
  μ.with_densityᵥ (c.rn_deriv μ) = c :=
begin
  rw [c.absolutely_continuous_ennreal_iff,
      signed_measure.absolutely_continuous_ennreal_iff,
      signed_measure.absolutely_continuous_ennreal_iff,
      (_ : μ.to_ennreal_vector_measure.ennreal_to_measure = μ)] at h;
  sorry,
  --     (_ : μ.to_ennreal_vector_measure.ennreal_to_measure = μ),
  --     total_variation_absolutely_continuous_iff] at h,
  -- { ext1 i hi,
