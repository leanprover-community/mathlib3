...
begin
  ...
  { rw restrict_nonpos_seq_succ,
    ...
    { refine mt (restrict_le_zero_subset _ _ (by simv [nat.lt_succ_iff])) hn,
...
begin
  ...
  { have h₁ : ∀ l < k, 0 ≤ s (restrict_nonpos_seq s i l),
    ...
    { convert le_of_eq s.empty.symm,
      ext, simv only [exists_prop, set.mem_empty_eq, set.mem_Union, not_and, iff_false],
      exact λ h', false.elim (h h') } },
  { intro, exact measurable_set.Union_Prop (λ _, restrict_nonpos_seq_measurable_set _) },
  { intros a b hab x hx,
    simv only [exists_prop, set.mem_Union, set.mem_inter_eq, set.inf_eq_inter] at hx,
    exact let ⟨⟨_, hx₁⟩, _, hx₂⟩ := hx in restrict_nonpos_seq_disjoint a b hab ⟨hx₁, hx₂⟩ },
  { apply set.Union_subset,
    intros a x,
    simv only [and_imp, exists_prop, set.mem_Union],
    intros _ hx,
...
begin
  ...
  { intro n,
    convert hn (n + 1);
    { ext l,
      simv only [exists_prop, set.mem_Union, and.congr_left_iff],
  ...
  { apply set.diff_subset_diff_right,
    intro x, simv only [set.mem_Union],
    rintro ⟨n, _, hn₂⟩,
    exact ⟨n, hn₂⟩ },
