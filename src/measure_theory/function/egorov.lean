...
begin
  ...
  simv only [mem_inter_eq, mem_Inter, ge_iff_le, mem_not_convergent_seq_iff],
  push_neg,
  rintro ⟨hmem, hx⟩,
...
begin
  ...
  { have : (λ j, μ (s ∩ not_convergent_seq f g n j)) = λ j, 0,
      by simv only [eq_iff_true_of_subsingleton],
    rw this,
...
begin
  ...
  simv only [mem_diff, egorov.Union_not_convergent_seq, not_exists, mem_Union, mem_inter_eq,
