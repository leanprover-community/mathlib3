...
* `α →₁[μ] β` is the type of `L¹` space, where `α` is a `measure_space` and `β` is a `normed_add_group`
...
* Let `f : α → β` be a function, where `α` is a `measure_space` and `β` a `normed_add_group`.
...
variables [normed_add_group β]
variables [normed_add_group γ]
...
variables {H : Type*} [normed_add_group H] {m0 : measurable_space α} {μ' : measure α} {f : α → H}
...
variables {E : Type*} {m0 : measurable_space α} [normed_add_group E]
...
variables {E : Type*} [normed_add_group E]
          ...
          {H : Type*} [normed_add_group H] [normed_space 𝕜 H]
