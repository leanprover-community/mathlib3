...
variables {α E F G : Type*} {m m0 : measurable_space α} {p : ℝ≥0∞} {q : ℝ} {μ ν : measure α}
  [normed_add_group E] [normed_add_group F] [normed_add_group G]
...
@[simp] lemma snorm_ae_eq_fun {α E : Type*} [measurable_space α] {μ : measure α}
  [normed_add_group E] {p : ℝ≥0∞} {f : α → E} (hf : ae_strongly_measurable f μ) :
...
lemma mem_ℒp.snorm_mk_lt_top {α E : Type*} [measurable_space α] {μ : measure α}
  [normed_add_group E] {p : ℝ≥0∞} {f : α → E} (hfp : mem_ℒp f p μ) :
  snorm (ae_eq_fun.mk f hfp.1) p μ < ∞ :=
by simp [hfp.2]

/-- Lp space -/
def Lp {α} (E : Type*) {m : measurable_space α} [normed_add_group E]
...
lemma snorm_indicator_le {E : Type*} [normed_add_group E] (f : α → E) :
...
lemma lipschitz_with.comp_mem_ℒp {α E F} {K} [measurable_space α] {μ : measure α}
  [normed_add_group E] [normed_add_group F] {f : α → E} {g : E → F} (hg : lipschitz_with K g)
...
lemma measure_theory.mem_ℒp.of_comp_antilipschitz_with {α E F} {K'}
  [measurable_space α] {μ : measure α} [normed_add_group E] [normed_add_group F]
...
lemma mem_ℒp_comp_iff_of_antilipschitz {α E F} {K K'} [measurable_space α] {μ : measure α}
  [normed_add_group E] [normed_add_group F]
...
lemma snorm'_lim_le_liminf_snorm' {E} [normed_add_group E] {f : ℕ → α → E} {p : ℝ} (hp_pos : 0 < p)
...
lemma snorm_lim_le_liminf_snorm {E} [normed_add_group E]
