...
* `tendsto_approx_on_univ_Lp` (Lᵖ convergence): If `E` is a `normed_add_group` and `f` is measurable
...
variables [measurable_space E] [normed_add_group E] [normed_add_group F] {q : ℝ} {p : ℝ≥0∞}
...
section integrable
variables [measurable_space β]
variables [measurable_space E] [normed_add_group E]
...
variables [normed_add_group E] [normed_add_group F]
...
variables [measurable_space α] [normed_add_group E] [normed_add_group F] (p : ℝ≥0∞) (μ : measure α)
...
variables [measurable_space α] [normed_add_group E] {f : α → E} {p : ℝ≥0∞} {μ : measure α}
