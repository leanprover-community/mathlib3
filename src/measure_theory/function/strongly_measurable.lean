...
begin
  ...
  { to_fun := λ x, if hx : x ∈ s then hc.approx n ⟨x, hx⟩
    ...
    begin
      ...
      ext1 y,
      simp only [mem_union_iff, mem_preimage, mem_singleton_iff, mem_image, set_coe.exists,
  ...
  { to_fun := g_seq_s n,
    measurable_set_fiber' := λ x, begin
      ...
      { suffices : (g_seq_s n) ⁻¹' {x} ∩ sᶜ = ∅, by { rw this, exact measurable_set.empty, },
        ext1 y,
        simp only [mem_inter_iff, mem_preimage, mem_singleton_iff, mem_compl_iff, mem_empty_iff_false,
