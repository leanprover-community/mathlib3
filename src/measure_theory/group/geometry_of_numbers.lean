...
lemma exists_pair_mem_lattice_not_disjoint_vadd_of_volume_lt_volume
  ...
  (μ : measure E) [vadd_invariant_measure L E μ]
  {F S : set E} (fund : is_add_fundamental_domain L F μ) (hS : measurable_set S) (h : μ F < μ S) :
...
lemma exists_ne_zero_mem_lattice_of_measure_mul_two_pow_finrank_lt_measure
  ...
  [countable L] {F T : set E} (fund : is_add_fundamental_domain L F μ) (hT : measurable_set T)
...
begin
  ...
  obtain ⟨x, y, hxy, h⟩ := exists_pair_mem_lattice_not_disjoint_vadd_of_volume_lt_volume μ
    fund (hT.const_smul₀ _) h_vol,
