...
variables {ι 𝕜 E F : Type*} [normed_add_group E]
...
lemma integrable_on_Icc_iff_integrable_on_Ioc'
  {E : Type*} [normed_add_group E] {f : ℝ → E} (ha : μ {a} ≠ ∞) :
...
lemma interval_integrable_iff_integrable_Icc_of_le {E : Type*} [normed_add_group E]
...
lemma integrable_on_Ici_iff_integrable_on_Ioi
  {E : Type*} [normed_add_group E] [has_no_atoms μ] {f : ℝ → E} :
  integrable_on f (Ici a) μ ↔ integrable_on f (Ioi a) μ :=
...
lemma mono_fun [normed_add_group F] {g : ℝ → F}
...
variables [normed_add_group F] [complete_space F] [normed_space ℝ F]
