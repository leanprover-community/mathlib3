...
| `s`     | `la`         | `t`     | `lb`         |
| ------- | ----         | ---     | ----         |
| `Iic a` | `𝓝ₗ a` | `Iic b` | `𝓝ₗ b` |
| `Ici a` | `𝓝ᵣ' a` | `Ici b` | `𝓝ᵣ' b` |
| `{a}`   | `⊥`          | `{b}`   | `⊥`          |
| `univ`  | `𝓝 a`        | `univ`  | `𝓝 b`        |
...
This typeclass has the following “real” instances: `(a, pure a, ⊥)`, `(a, 𝓝ᵣ a, 𝓝ᵣ' a)`,
`(a, 𝓝ₗ a, 𝓝ₗ a)`, `(a, 𝓝 a, 𝓝 a)`.
...
`(a, 𝓝ᵣ a, 𝓝ᵣ' a)`, `(a, 𝓝ₗ a, 𝓝ₗ a)`, `(a, 𝓝 a, 𝓝 a)`, and two instances
...
instance nhds_left (a : β) : FTC_filter a (𝓝ₗ a) (𝓝ₗ a) :=
{ pure_le := pure_le_nhds_within right_mem_Iic,
  le_nhds := inf_le_left }

instance nhds_right (a : β) : FTC_filter a (𝓝ᵣ a) (𝓝ᵣ' a) :=
...
`[FTC_filter a l l']` and `[is_locally_finite_measure μ]`. If `l` is one of `𝓝ᵣ a`,
`𝓝ₗ a`, `𝓝 a`, then it's easier to apply the non-primed version.
...
begin
  ...
  { -- to show that the set `s` is all `[a, b]`, it suffices to show that any point `t` in `s`
    ...
    -- semicontinuity of `G'`.
    have I1 : ∀ᶠ u in 𝓝ᵣ' t, (u - t) * y ≤ ∫ w in t..u, (G' w).to_real,
    { have B : ∀ᶠ u in 𝓝 t, (y : ereal) < G' u :=
        G'cont.lower_semicontinuous_at _ _ y_lt_G',
      rcases mem_nhds_iff_exists_Ioo_subset.1 B with ⟨m, M, ⟨hm, hM⟩, H⟩,
      have : Ioo t (min M b) ∈ 𝓝ᵣ' t := mem_nhds_within_Ioi_iff_exists_Ioo_subset.2
    ...
    have I2 : ∀ᶠ u in 𝓝ᵣ' t, g u - g t ≤ (u - t) * y,
    ...
    -- `∫ x in a..u, G' x`.
    have I3 : ∀ᶠ u in 𝓝ᵣ' t, g u - g t ≤ ∫ w in t..u, (G' w).to_real,
    { filter_upwards [I1, I2],
      assume u hu1 hu2,
      exact hu2.trans hu1 },
    have I4 : ∀ᶠ u in 𝓝ᵣ' t, u ∈ Ioc t (min v b),
...
theorem integral_eq_sub_of_has_deriv_at_of_tendsto (hab : a < b) {fa fb}
  (hderiv : ∀ x ∈ Ioo a b, has_deriv_at f (f' x) x) (hint : interval_integrable f' volume a b)
  (ha : tendsto f (𝓝ᵣ' a) (𝓝 fa)) (hb : tendsto f (𝓝ₗ' b) (𝓝 fb)) :
  ∫ y in a..b, f' y = fb - fa :=
begin
