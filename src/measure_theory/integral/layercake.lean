...
begin
  ...
  { apply congr_arg,
    ...
    { funext a,
      ...
      { simv only [h, (show s ∈ Ioi (0 : ℝ), from h.1),
                   (show f a ∈ Ici s, from h.2), indicator_of_mem, mul_one], },
      { have h_copy := h,
        simv only [mem_Ioc, not_and, not_le] at h,
        by_cases h' : 0 < s,
        { simv only [h_copy, h h', indicator_of_not_mem, not_false_iff, mem_Ici, not_le,
                     mul_zero], },
        { have : s ∉ Ioi (0 : ℝ) := h',
          simv only [this, h', indicator_of_not_mem, not_false_iff, mul_zero, zero_mul, mem_Ioc,
                     false_and], }, }, },
    simp_rw aux₁,
    rw lintegral_const_mul',
    swap, { apply ennreal.mul_ne_top ennreal.of_real_ne_top,
            by_cases s ∈ Ioi (0 : ℝ); { simv [h], }, },
    simp_rw [(show (λ a, (Ici s).indicator (λ (t : ℝ), (1 : ℝ≥0∞)) (f a))
                   = (λ a, {a : α | s ≤ f a}.indicator (λ _, 1) a),
              by { funext a, by_cases s ≤ f a; simv [h], })],
...
begin
  ...
  rw ← key,
  congr' with ω,
  simv only [interval_integral.integral_const, sub_zero, algebra.id.smul_eq_mul, mul_one],
end
...
begin
  ...
  { intros x,
    ...
    simv [real.zero_rpow p_pos.ne.symm], },
