...
lemma integral_diff (ht : measurable_set t) (hfs : integrable_on f s μ)
  ...
  ∫ x in s \ t, f x ∂μ = ∫ x in s, f x ∂μ - ∫ x in t, f x ∂μ :=
begin
  rw [eq_sub_iff_add_eq, ← integral_union, sdiff_union_of_subset hts],
  exacts [disjoint_sdiff.symm, ht, hfs.mono_set (sdiff_subset _ _), hft]
end
...
begin
  rw [← set.union_sdiff_self, union_comm, integral_union,
    set_integral_eq_zero_of_forall_eq_zero _ (λ x hx, ht_eq x (sdiff_subset _ _ hx)), zero_add],
  exacts [hf, disjoint_sdiff.symm, hs, hfi.integrable_on, hfi.integrable_on]
end
