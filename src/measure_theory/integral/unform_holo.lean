...
(abs r)⁻¹ * abs(b-x)+ (abs r)⁻¹ * abs ((c-d) -(y-x)), by {
  ring_nf, apply mul_mono_nonneg, rw inv_nonneg, apply abs_nonneg,
   ...
   by {set X : ℂ := (a-y) - (b-x), set Y: ℂ :=-((c-d) - (y-x)),
  have := complex.abs_add X Y,
  have ho : abs (c - d - (y - x)) = abs Y, by {simp_rw Y, rw abs_neg,},
  rw ho,
  convert this,},
  have h44 : abs ((a-y) - (b-x)) ≤ abs (a-y) + abs (b-x), by {
    have:= complex.abs_sub_le (a-y) 0 (b-x),
  simp at this,
  have hcd :abs (b-x)= abs (x-b), by {apply complex.abs_sub_comm,},
  rw hcd,
  apply this,},
  apply le_trans h4, simp_rw ← add_assoc, simp [h44],
},
...
lemma auxfin (a b c d e f r: ℂ) (ε : ℝ) (hε: 0 < ε) (h1: abs (a- b) < 8⁻¹*abs(r)*ε)
 ...
 (abs r)⁻¹ *abs ((y -x)- (e -f) ) < 8⁻¹*ε) : (abs r)⁻¹ * abs ((a-b) - (c-d) + (b-d) - (e-f) ) < ε :=
begin
apply aux2 ,
apply hε,
apply h1,
