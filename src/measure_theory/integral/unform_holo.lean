...
lemma int_diff0_int_abs (R : ℝ) (hR: 0 < R) (F : ℂ → ℂ) (z : ℂ)
  (F_cts :  continuous_on (F ) (closed_ball z R))
  (w : ball z R) :
  integrable (complex.abs ∘ (int_diff0 R hR (F) z w)) (volume.restrict (Ioc 0  (2*π))) :=
begin
  apply integrable_on.integrable,
  rw ← interval_integrable_iff_integrable_Ioc_of_le,
  apply continuous_on.interval_integrable,
...
begin
  ...
  abs_mul, abs_div, abs_inv, abs_two, norm_eq_abs], ring,},
  rw hg,
  simp_rw ← r,
  have haa := ha b hb,
...
def bound2 (R : ℝ) (hR: 0 < R) (F : ℕ → ℂ → ℂ) (f : ℂ → ℂ) (z : ℂ) (w : ball z R) (a : ℕ) :
ℝ → ℝ := λ θ, (∑ (i : finset.range (a+1) ),complex.abs ((int_diff0 R hR (F i) z w) θ))  +
...
begin
  ...
  tendsto (λ n, ((int_diff0 R hR (F n) z w)) a)
  at_top (𝓝 (((int_diff0 R hR f z w)) a)),
  ...
  ∥(int_diff0 R hR (F n) z w) a∥ ≤ bound a,
  by {intro n,
...
begin
  ...
  simp only [one_div, algebra.id.smul_eq_mul, nat.cast_bit0, real_smul, integral_const_mul,
  ...
  have HF2 := HF x,
  clear HF,
  simp only [hx, forall_true_left] at HF2,
  ...
  gt_iff_lt,
  mem_closed_ball,
  normed_field.norm_mul,
  ge_iff_le,
  nonempty_of_inhabited,
  sub_zero, zero_lt_bit0,
  zero_lt_mul_left,
  ...
  by {simp_rw U, simp_rw metric.mem_nhds_within_iff at *,
  simp only [exists_prop, mem_ball, and_true, gt_iff_lt, inf_eq_inter,
  inter_subset_right, subset_inter_iff] at *, exact hS1,},
  ...
  simp_rw abs_norm,
  have hyz: y ∈ ball z R, by {apply half_ball_sub R hR z, simp only [mem_ball], apply hy.2,},
  have keyy:= keyb y hyz,
  have h8': 0 < 8⁻¹*t*ε, by {apply mul_pos, apply mul_pos,
  simp only [zero_lt_bit0, zero_lt_one, inv_pos], rw abs_pos, simp at ht, apply ht, apply hε,},
  ...
  by {intro y,simp only [sub_left_inj], ring,},
  simp_rw HH,
  apply auxfin,
  apply hε,
  clear keyb key keyy HH,
  apply ha A haA,
  apply hyz.le,
  apply ha A haA,
  apply hxx.le,
  ...
  simp_rw int_diff,
  have hyy: y ∈ ball z (2⁻¹*R), by {simp only [mem_ball], apply hy.2,},
  have hxz: x ∈ ball z (2⁻¹*R), by {simp only [mem_ball], apply hx,},
  split,
  have:= F_alt A ⟨y,hyy⟩,
  simp at this,
  rw this,
