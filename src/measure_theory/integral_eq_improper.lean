...
lemma ae_cover.Union_nat_ae_cover {φ : ι → set α} (hφ : ae_cover μ φ) :
  ae_cover μ (λ (n : ℕ), ⋃ k ≤ n, φ k) :=
{ ae_eventually_mem := hφ.ae_eventually_mem.mono (λ x h,
    (tendsto_coe_nat_at_top_at_top.eventually h).mono
