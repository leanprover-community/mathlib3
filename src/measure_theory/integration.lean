...
begin
  revert f,
  have h_mul_indicator:∀ g, @measurable α ℝ≥0∞ M _ g → @measurable α ℝ≥0∞ M ennreal.measurable_space (g * (λ (a : α), T.indicator (λ (_x : α), c) a)) :=
...
begin
  ...
  { intros S T h_S h_T, cases h_S with A h_S, cases h_T with B h_T,
    rw [← h_S.right, ← h_T.right],
    apply h_ind _ _ h_S.left h_T.left },
