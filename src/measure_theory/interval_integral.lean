...
Note `a..b` is the set of real numbers lying between `a` and `b`, and so `∫ x in a...b, f x` is
different from `∫ x in a..b, f x`
-/
...
begin
  ...
  { cases le_total b c with bc cb,
    ...
    { exact hbc.subinterval (mem_interval_of_mem_Icc ⟨ba, ac⟩) right_mem_interval },
    { exact hab.subinterval left_mem_interval (mem_interval_of_mem_Icc ⟨ac, cb⟩) } },
  let h := interval_swap,
...
begin
  rw interval, split_ifs,
  { simp only [imp_self] },
  rw Icc_eq_empty (lt_of_not_ge h), simp only [integrable_on_empty, forall_true_iff]
end
...
begin
  ...
  { cases le_total b c with bc cb,
    ...
    { exact hfi'.subinterval (mem_interval_of_mem_Icc ⟨ba, ac⟩) right_mem_interval },
    { exact hfi.subinterval left_mem_interval (mem_interval_of_mem_Icc ⟨ac, cb⟩) } },
...
begin
  ...
  { have acm : measurable_on a..c f := hfm.interval_combine hfm',
    ...
    { rw [add_neg_eq_iff_eq_add, eq_comm],
      ...
      rw [interval_of_le ab, interval_of_le' cb, interval_of_le ac],
    ...
    { rw [neg_add_eq_iff_eq_add, eq_comm],
      ...
      rw [interval_of_le' ba, interval_of_le ac, interval_of_le bc],
