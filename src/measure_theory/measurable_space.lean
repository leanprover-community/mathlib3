...
/-- If `g` collection of subsets of `α` such that the `σ`-algebra generated from `g` contains the
...
/-- We get a Galois insertion between `σ`-algebras on `α` and `set (set α)` by using `generate_from`
  on one side and the collection of measurable sets on the other sides. -/
...
/-- `α` is equivalent to it's image in `α ⊕ β` as measurable spaces. -/
...
/-- `β` is equivalent to it's image in `α ⊕ β` as measurable spaces. -/
...
/-- A Dynkin system is a collection of subsets of a type `α` that conitains the empty set,
