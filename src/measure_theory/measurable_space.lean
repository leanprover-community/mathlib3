      ...
      { have hfs : (⋃ (n : ℕ), ht_f n) = s,
        { rw not_exists at huniv hsc,
          ...
          { refine set.Union_subset (λ i, _),
            have hfi : ht_f i = ∅ ∨ ht_f i = s,
            { simpa [hsc i, huniv i] using ht_ih i, },
            cases hfi; rw hfi,
            exact set.empty_subset s, },
          { rw ← hs,
            exact set.subset_Union _ n, }, },
        simp only [hfs, set.mem_insert_iff, true_or, eq_self_iff_true, or_true], }, },
    { rw not_exists at huniv hs,
      by_cases hsc : ∃ n, ht_f n = sᶜ,
      { have hfs : (⋃ (n : ℕ), ht_f n) = sᶜ,
        { cases hsc with n hsc,
          refine set.subset.antisymm _ _,
          { refine set.Union_subset (λ i, _),
            have hfi : ht_f i = ∅ ∨ ht_f i = sᶜ,
            { simpa [hs i, huniv i] using ht_ih i, },
            cases hfi; rw hfi,
            { exact set.empty_subset sᶜ, },
            { exact set.subset.rfl, }, },
          { rw ← hsc,
            exact set.subset_Union _ n, }, },
        simp only [hfs, set.mem_insert_iff, set.mem_singleton, or_true], },
      { have hf_empty : (⋃ (n : ℕ), ht_f n) = ∅,
        { refine set.Union_eq_empty.mpr (λ n, _),
          simpa [huniv n, hs n, (not_exists.mp hsc) n] using ht_ih n, },
