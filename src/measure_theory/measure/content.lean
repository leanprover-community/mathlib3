...
begin
  ...
  letI : nonempty {L : compacts G // (L : set G) ⊆ U' ∩ U} := ⟨⟨⊥, empty_subset _⟩⟩,
  rw [ennreal.supr_add],
  ...
  simp only [inner_content, supr_subtype'], rw [opens.coe_mk],
  letI : nonempty {M : compacts G // (M : set G) ⊆ ↑U' \ L} := ⟨⟨⊥, empty_subset _⟩⟩,
...
begin
  ...
  { refine ⟨λ A hA r (hr : _ < _), _⟩,
    ...
    exact ⟨U, hAU, hUo, hr⟩ },
  letI : is_finite_measure_on_compacts μ.measure,
  { refine ⟨λ K hK, _⟩,
