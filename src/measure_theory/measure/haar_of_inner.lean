...
lemma orientation.add_haar_orthonormal_basis
  ...
  o.volume_form.add_haar (parallelogram b) = 1 :=
begin
  have e : ι ≃ fin n,
  ...
  rw [A, parallelogram_comp, alternating_map.add_haar_parallelogram,
    o.abs_volume_form_apply_of_orthonormal, ennreal.of_real_one],
end

/-- In an oriented inner product space, the canonical volume form associated to an orientation
coincides with the volume. -/
lemma orientation.add_haar_eq_volume (o : orientation ℝ F (fin n)) :
  o.volume_form.add_haar = volume :=
begin
  have A : o.volume_form.add_haar ((std_orthonormal_basis ℝ F).to_basis.parallelogram) = 1,
    from orientation.add_haar_orthonormal_basis o (std_orthonormal_basis ℝ F),
  rw [add_haar_measure_unique o.volume_form.add_haar
...
/-- The volume form of a finite-dimensional inner product space gives measure `1` to the
