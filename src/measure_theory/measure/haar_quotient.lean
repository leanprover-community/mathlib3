...
{ measure_preimage_smul :=
  begin
    ...
    rw this,
    letI : smul_invariant_measure ↥(Γ.opposite) G μ := subgroup.smul_invariant_measure,
    letI : smul_invariant_measure G G μ := ⟨λ c s hs, measure_preimage_mul μ c s⟩,
    -- Lean can generate the next instance but it has no additive version of the autogenerated proof
    letI : smul_comm_class G Γ.opposite G := ⟨λ a b c, (mul_assoc _ _ _).symm⟩,
...
{ map_mul_left_eq_self := begin
    ...
    letI := h𝓕.smul_invariant_measure_map,
...
begin
  ...
  have 𝓕meas : null_measurable_set 𝓕 μ := h𝓕.null_measurable_set,
  letI : is_finite_measure (μ.restrict 𝓕) :=
    ⟨by { rw [measure.restrict_apply₀' 𝓕meas, univ_inter], exact h𝓕_finite }⟩,
  -- the measure is left-invariant, so by the uniqueness of Haar measure it's enough to show that
  -- it has the stated size on the reference compact set `K`.
  letI : (measure.map (quotient_group.mk' Γ) (μ.restrict 𝓕)).is_mul_left_invariant :=
