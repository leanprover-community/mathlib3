...
lemma map_matrix_volume_pi_eq_smul_volume [decidable_eq ι] {M : matrix ι ι ℝ} (hM : det M ≠ 0) :
...
lemma map_linear_map_volume_pi_eq_smul_volume {f : (ι → ℝ) →ₗ[ℝ] (ι → ℝ)} (hf : f.det ≠ 0) :
...
begin
  ...
  rw [A, B],
  apply map_matrix_volume_pi_eq_smul_volume,
  rwa A at hf
end
