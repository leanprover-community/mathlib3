...
begin
  refine measure_mono_null _ (measure_limsup_eq_zero hs),
  simp only [limsup_eq_infi_supr_of_nat, eventually_at_top, set_of_exists, set_of_forall,
    compl_Union, compl_Inter, compl_set_of, not_not],
  exact subset.rfl
end

lemma measure_set_of_frequently_eq_zero {p : ℕ → α → Prop} (hp : ∑' i, μ {x | p i x} ≠ ∞) :
  μ {x | ∃ᶠ n in at_top, p n x} = 0 :=
ae_eventually_not_mem hp
