...
theorem sigma_finite.out {μ : measure α} (h : sigma_finite μ) :
  nonempty (μ.finite_spanning_sets_in {s | is_measurable s}) := h.1
...
⟨by rintro ⟨t, z, rfl, ht, hz⟩; exact
  is_measurable.union ht (c.out _ hz),
...
begin
  ...
  { have h_g_to_f : (g ⁻¹' s) ∩ t = (f ⁻¹' s) ∩ t,
    { ext,
      ...
      exact λ hx, by rw hx, },
    rw h_g_to_f,
    exact is_measurable.inter (hf hs) (is_measurable.compl_iff.mp (hμ.out tᶜ ht_compl)), },
