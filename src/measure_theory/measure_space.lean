...
def of_measurable (m : Π (s : set α), is_measurable s → ℝ≥0∞)
...
lemma of_measurable_apply {m : Π (s : set α), is_measurable s → ℝ≥0∞}
  {m0 : m ∅ is_measurable.empty = 0}
  {mU : ∀ {{f : ℕ → set α}} (h : ∀ i, is_measurable (f i)), pairwise (disjoint on f) →
...
begin
  ...
  choose f hf using show ∀ q : {q : ℚ // q > 0}, ∃ t : set α,
    z ⊆ t ∧ is_measurable t ∧ μ t < (nnreal.of_real q.1 : ℝ≥0∞),
  { rintro ⟨ε, ε0⟩,
