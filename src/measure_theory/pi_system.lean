...
lemma generate_from_pi_Union_Inter'_le {α ι} {m : measurable_space α}
...
lemma mem_pi_Union_Inter'_of_measurable_set {α ι} (m : ι → measurable_space α)
...
lemma generate_from_pi_Union_Inter'_measurable_space {α ι} (m : ι → measurable_space α)
  (S : set ι) :
  generate_from (pi_Union_Inter' (λ n, {s | measurable_set[m n] s}) S)
...
lemma generate_from_pi_Union_Inter'_subsets {α ι} (m : ι → measurable_space α) (S : set ι) :
  generate_from (pi_Union_Inter' (λ i, {t | measurable_set[m i] t}) S)
    = ⨆ i ∈ S, m i :=
begin
  rw generate_from_pi_Union_Inter'_measurable_space,
  simp only [set.mem_set_of_eq, exists_prop, supr_exists],
  congr' 1,
  ext1 i,
  by_cases hiS : i ∈ S,
