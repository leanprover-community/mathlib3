...
When there are multiple anonymous binders, they are named `a_1`, `a_2` etc.
...
/--
Parse a name that may have been generated by Lean to name an anonymous binder.
Such names are either `a` or `a_n` for some natural `n`. See
[note likely generated binder names].
-/
meta def likely_generated_binder_name_p : parser unit := do
  parser.ch 'a',
  optional (parser.ch '_' *> parser.nat),
  pure ()
...
| anonymous := ff
| mk_numeral _ _ := ff
| mk_string s anonymous := (likely_generated_binder_name_p.run_string s).is_right
| mk_string _ _ := ff
end
