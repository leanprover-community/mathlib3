...
-/
meta def is_likely_generated_binder_name_component : string → bool
| "a" := tt
| n :=
  match n.get_rest "a_" with
  | none := ff
...
| anonymous := ff
| mk_numeral _ _ := ff
| mk_string s anonymous := is_likely_generated_binder_name_component s
| mk_string _ _ := ff
end
