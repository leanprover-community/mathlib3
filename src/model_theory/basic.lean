...
* A `first_order.language.equiv`, denoted `M ↪[L] N`, is an equivalence from the `L`-structure `M`
  to the `L`-structure `N` that commutes with the interpretations of functions, and which preserves
  the interpretations of relations in both directions.
* A `first_order.language.substructure` of a structure `M` is a subset of the structure closed
  under application of function symbols.
* The `substructure.closure` of a set `s` in a structure `M` is the smallest substructure of `M`
  containing `s`.
...
instance const_coe : has_coe_t L.const M :=
...
lemma closed_under_inter (hs : closed_under f s) (ht : closed_under f t) :
  closed_under f (s ∩ t) :=
λ x h, mem_inter (hs x (λ i, mem_of_mem_inter_left (h i)))
  (ht x (λ i, mem_of_mem_inter_right (h i)))

lemma closed_under_inf (hs : closed_under f s) (ht : closed_under f t) :
  closed_under f (s ⊓ t) := closed_under_inter hs ht

variables {S : set (set M)}

lemma closed_under_Inf (hS : ∀ s, s ∈ S → closed_under f s) : closed_under f (Inf S) :=
...
⟨λ S₁ S₂,
  { carrier := S₁ ∩ S₂,
    fun_mem := λ n f, closed_under_inf (S₁.fun_mem f) (S₂.fun_mem f) }⟩
...
instance : has_Inf (L.substructure M) :=
⟨λ s, { carrier := ⋂ t ∈ s, ↑t,
        fun_mem := λ n f, closed_under_Inf begin
          rintro _ ⟨t, rfl⟩,
          simp only,
          by_cases h : t ∈ s,
          { rw [set.Inter_pos h],
...
@[simp]
lemma subset_closure : s ⊆ closure L s := λ x hx, mem_closure.2 $ λ S hS, hS hx
...
@[simp]
lemma closure_le : closure L s ≤ S ↔ s ⊆ S :=
