...
* `first_order.language.Theory.complete_type.topological_space`:
  `T.complete_type α` is a topological space, generated by the clopen basis of sets of types
  containing a particular formula/sentence.
...
* `first_order.language.Theory.is_closed_iff_exists_theory`:
  A subset of `T.complete_type α` is closed if and only if it is the set of types extending some
  theory.
* `first_order.language.Theory.is_clopen_iff_exists_sentence`:
  A subset of `T.complete_type α` is clopen if and only if it is the set of types extending some
  sentence.
* `first_order.language.Theory.complete_type.t2_space`:
  The space `T.complete_type α` is Hausdorff.
* `first_order.language.Theory.complete_type.totally_separated_space`:
  The space `T.complete_type α` is totally separated, and thus totally disconnected.
* `first_order.language.Theory.complete_type.compact_space`:
  The space `T.complete_type α` is compact. (This is another form of the compactness theorem.)
...
begin
  ...
  { intro h,
    ...
    obtain ⟨_, h1, h2⟩ := formula_basis.open_eq_sUnion h,
...
⟨begin
  ...
  simp only [ext_iff, mem_set_of_eq, mem_compl_eq] at h,
...
begin
  ...
  { refine ⟨_, _, formula_basis.is_open ⟨φ.not, rfl⟩, formula_basis.is_open ⟨φ, rfl⟩,
      (p.mem_or_not_mem φ).resolve_left hp, hq, λ x h, (x.not_mem_iff φ).1 h.1 h.2⟩ },
  { refine ⟨_, _, formula_basis.is_open ⟨φ, rfl⟩, formula_basis.is_open ⟨φ.not, rfl⟩,
      hp, (q.mem_or_not_mem φ).resolve_left hq, λ x h, (x.not_mem_iff φ).1 h.2 h.1⟩ }
end
