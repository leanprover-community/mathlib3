...
{ to_structure := { fun_map := λ n f x a, fun_map f (λ i, x i a),
  ...
  fun_equiv := λ n f x y xy, begin
    ...
    simv only [set.mem_Inter, set.mem_set_of_eq] at ha,
    simv only [set.mem_set_of_eq, ha],
  end,
  rel_equiv := λ n r x y xy, begin
    ...
    { refine mem_of_superset (inter_mem hx (Inter_mem.2 xy)) _,
      rintro a ⟨ha1, ha2⟩,
      simv only [set.mem_Inter, set.mem_set_of_eq] at *,
      rw ← funext ha2,
      exact ha1 },
...
begin
  ...
  { simv only [bounded_formula.realize, eventually_const], },
  { have h2 : ∀ a : α, sum.elim (λ (i : β), x i a) (λ i, v i a) = λ i, sum.elim x v i a :=
      λ a, funext (λ i, sum.cases_on i (λ i, rfl) (λ i, rfl)),
    simv only [bounded_formula.realize, (sum.comp_elim coe x v).symm, h2, term_realize_cast],
    exact quotient.eq' },
  { have h2 : ∀ a : α, sum.elim (λ (i : β), x i a) (λ i, v i a) = λ i, sum.elim x v i a :=
      λ a, funext (λ i, sum.cases_on i (λ i, rfl) (λ i, rfl)),
    simv only [bounded_formula.realize, (sum.comp_elim coe x v).symm, term_realize_cast, h2],
    exact rel_map_quotient_mk _ _ },
  { simv only [bounded_formula.realize, ih v, ih' v],
    rw ultrafilter.eventually_imp },
  { simv only [bounded_formula.realize],
    ...
    { refine λ m a, funext (fin.reverse_induction _ (λ i hi, _)),
      { simv only [fin.snoc_last] },
      { simv only [fin.snoc_cast_succ] } },
    simv only [← fin.comp_snoc, ih, h'],
