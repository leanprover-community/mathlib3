...
lemma is_prime_power_iff_bounded (n : ℕ) :
...
decidable_of_iff' _ (is_prime_power_iff_bounded n)

lemma is_prime_power_iff_pow_succ (n : ℕ) :
  is_prime_power n ↔ ∃ (p k : ℕ), p.prime ∧ p ^ (k + 1) = n :=
(is_prime_power_def _).trans
