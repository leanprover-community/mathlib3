...
begin
  ext x,
  cases x,
  { simp only [divisors_zero, sum_empty, ne.def, not_false_iff,
      coe_mul_zeta_apply, zero_ne_one, one_apply_ne] },
  cases x,
  ...
  suffices :
    ∑ (i : finset ℕ) in (unique_factorization_monoid.factors x.succ.succ).to_finset.powerset,
    ite (squarefree i.val.prod) ((-1:ℤ) ^ Ω i.val.prod) 0 = 0,
  { have h := @sum_divisors_filter_squarefree _ (nat.succ_ne_zero _) _ _
      (λ (i : ℕ), ite (squarefree i) ((-1:ℤ) ^ Ω i) 0),
    exact (eq.trans (by congr') h).trans this },
  refine (sum_congr rfl _).trans (sum_powerset_neg_one_pow_card_of_nonempty _),
  { intros y hy,
