...
lemma dvd_sub_pow_of_prime_of_dvd_sub (R : Type*) [comm_ring R] (p : ℕ) [hp : fact p.prime]
  (a b : R) (h : (p : R) ∣ a - b) (k : ℕ) :
  (p^(k+1) : R) ∣ a^(p^k) - b^(p^k) :=
begin
  ...
  have p_eq_zero : (p : ideal.quotient I) = 0,
  ...
  rw [← mem_span_singleton, ← eq_zero_iff_mem, mk_eq_mk_hom, ring_hom.map_geom_series₂,
      ring_hom.map_pow, ring_hom.map_pow, h, geom_series₂_self, p_eq_zero, zero_mul],
end
