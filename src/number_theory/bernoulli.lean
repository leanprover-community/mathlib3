...
begin
  ...
  { simp only [mul_eq_mul_right_iff] at h, cases h,
    { simp only [power_series.ext_iff, eval_neg_hom, coeff_mk,
        coeff_eval_mul_hom, linear_map.map_sub] at h,
      specialize h n, rw coeff_X n at h, split_ifs at h,
      { rw h_1 at hlt, exfalso, norm_num at *, },
      rw [←mul_div_assoc, sub_eq_zero_iff_eq, div_eq_iff] at h,
      { rw div_mul_cancel at h,
        { rw [neg_one_pow_eq_pow_mod_two, h_odd] at h,
