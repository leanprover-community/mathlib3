...
begin
  ...
  { simp only [mul_eq_mul_right_iff] at h, cases h,
     { simp only [eval_neg_hom, eval_mul_hom, coeff_mk, coe_mk, power_series.ext_iff,
      ...
      specialize h n, rw coeff_X n at h, split_ifs at h with h2,
      { rw h2 at hlt, exfalso, exact false.elim (lt_irrefl _ hlt), },
      have hn : (n.factorial : ℚ) ≠ 0, { simp [factorial_ne_zero], },
      rw [←mul_div_assoc, sub_eq_zero_iff_eq, div_eq_iff hn] at h,
      { rw div_mul_cancel _ hn at h,
        { simp only [neg_one_pow_eq_pow_mod_two, h_odd,neg_mul_eq_neg_mul_symm, one_mul,
         pow_one] at h, exact eq_zero_of_neg_eq h.symm, }, }, },
    { exfalso, rw [sub_eq_zero_iff_eq, power_series.ext_iff] at h,
