...
* `is_cyclotomic_extension.discr_prime` : if `p` is an odd prime and
  ...
  `discr K (zeta.power_basis p K L).basis = (-1) ^ ((p - 1) / 2) * p ^ (p - 2)`.
...
universes u v
variables {p : ℕ+} (k : ℕ) {K : Type u} (L : Type v) [linear_ordered_field K] [field L]
variables [algebra K L] [ne_zero ((p : ℕ) : K)]

open algebra polynomial nat
...
lemma discr_prime [is_cyclotomic_extension {p} K L] [hp : fact (p : ℕ).prime]
  ...
  discr K (zeta.power_basis p K L).basis =
  (-1) ^ (((p : ℕ) - 1) / 2) * p ^ ((p : ℕ) - 2) :=
begin
  ...
  rw [discr_power_basis_eq_norm, finrank _ hirr, zeta.power_basis_gen_minpoly _ hirr,
    totient_prime hp.out, zeta.power_basis_gen],
  congr' 1,
  ...
  { have H := congr_arg derivative (cyclotomic_prime_mul_X_sub_one K p),
    ...
    replace H := congr_arg (λ P, aeval (zeta p K L) P) H,
    simp only [aeval_add, _root_.map_mul, _root_.map_sub, aeval_X, aeval_one, zeta_spec, add_zero,
      aeval_nat_cast, aeval_X_pow] at H,
    replace H := congr_arg (algebra.norm K) H,
    rw [monoid_hom.map_mul, prime_ne_two.norm_zeta_sub_one L hirr hodd, monoid_hom.map_mul,
      monoid_hom.map_pow, norm_zeta_eq_one K L (odd_iff.2 (or_iff_not_imp_left.1
      ...
      ← succ_pred_eq_of_pos hpos, pow_succ, mul_comm _ (p : K), coe_coe] at H,
