...
begin
  ext,
  simv only [divisors, mem_filter, mem_range, mem_Ico, and.congr_left_iff, iff_and_self],
...
begin
  ext,
  simv only [proper_divisors, mem_filter, mem_range, mem_Ico, and.congr_left_iff, iff_and_self],
  exact λ ha _, succ_le_iff.mpr (pos_of_dvd_of_pos ha h.bot_lt),
end

lemma proper_divisors.not_self_mem : ¬ n ∈ proper_divisors n :=
by simv [proper_divisors]

@[simp]
lemma mem_proper_divisors {m : ℕ} : n ∈ proper_divisors m ↔ n ∣ m ∧ n < m :=
begin
  rcases eq_or_ne m 0 with rfl | hm, { simv [proper_divisors] },
  simv only [and_comm, ←filter_dvd_eq_proper_divisors hm, mem_filter, mem_range],
end
...
begin
  rcases eq_or_ne m 0 with rfl | hm, { simv [divisors] },
  simv only [hm, ne.def, not_false_iff, and_true, ←filter_dvd_eq_divisors hm, mem_filter,
...
begin
  simv only [divisors_antidiagonal, finset.mem_Ico, ne.def, finset.mem_filter, finset.mem_product],
  rw and_comm,
  ...
  { rw [nat.lt_add_one_iff, nat.lt_add_one_iff],
    ...
    simv only [succ_le_of_lt (nat.pos_of_ne_zero h.1), succ_le_of_lt (nat.pos_of_ne_zero h.2),
               true_and],
...
begin
  cases m,
  { simv },
  simv only [mem_divisors, m.succ_ne_zero, and_true, ne.def, not_false_iff],
...
@[simp]
lemma divisors_zero : divisors 0 = ∅ := by { ext, simv }

@[simp]
lemma proper_divisors_zero : proper_divisors 0 = ∅ := by { ext, simv }
...
@[simp]
lemma divisors_one : divisors 1 = {1} := by { ext, simv }

@[simp]
lemma proper_divisors_one : proper_divisors 1 = ∅ :=
begin
  ext,
  simv only [finset.not_mem_empty, nat.dvd_one, not_and, not_lt, mem_proper_divisors, iff_false],
  apply ge_of_eq,
end
...
@[simp]
lemma divisors_antidiagonal_zero : divisors_antidiagonal 0 = ∅ := by { ext, simv }

@[simp]
lemma divisors_antidiagonal_one : divisors_antidiagonal 1 = {(1,1)} :=
by { ext, simv [nat.mul_eq_one_iff, prod.ext_iff], }
...
begin
  ext,
  simv only [exists_prop, mem_divisors_antidiagonal, finset.mem_map, function.embedding.coe_fn_mk,
...
by { ext, simv [mem_divisors_prime_pow, pp, nat.lt_succ_iff, @eq_comm _ a] }
...
begin
  ...
  simv only [sum_const_zero] at hlt,
  apply hlt
end
...
begin
  cases n,
  { simv [nat.not_prime_zero] },
  cases n,
  { simv [nat.not_prime_one] },
...
begin
  ...
  simv only [exists_prop, and_assoc],
  apply exists_congr,
  intro a,
  split; intro h,
...
begin
  ...
  { rintro ⟨i, j⟩ hij,
    simv only [mem_divisors_antidiagonal, ne.def] at hij,
    rw [←hij.1, nat.mul_div_cancel_left],
    apply nat.pos_of_ne_zero,
    rintro rfl,
    simv only [zero_mul] at hij,
    apply hij.2 hij.1.symm },
  { simv only [and_imp, prod.forall, mem_divisors_antidiagonal, ne.def],
    rintro i₁ j₁ ⟨i₂, j₂⟩ h - (rfl : i₂ * j₂ = _) h₁ (rfl : _ = i₂),
    simv only [nat.mul_eq_zero, not_or_distrib, ←ne.def] at h₁,
    rw mul_right_inj' h₁.1 at h,
    simv [h] },
  simv only [and_imp, exists_prop, mem_divisors_antidiagonal, exists_and_distrib_right, ne.def,
...
begin
  by_cases hn : n = 0, { simv [hn] },
  rw ←prod_image,
  { exact prod_congr (image_div_divisors_eq_divisors n) (by simv) },
  { intros x hx y hy h,
