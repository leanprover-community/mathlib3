...
lemma comm {a b c : ℤ} : (fermat_42 a b c) ↔ (fermat_42 b a c) :=
begin
  refine ⟨_, _⟩;
  { rintro ⟨a, b, h⟩,
    exact ⟨b, a, by rwa add_comm⟩ }
end

@[simp]
lemma nat.two_mul_two : 2 * 2 = 4 := rfl

lemma mul_smul {α : Type*} [semiring α] (n : ℕ) : n • (1 : α) = (n : α) :=
nsmul_one n

@[simp]
lemma two_mul_two {α : Type*} [semiring α] : (2 : α) * 2 = (4 : α) :=
by norm_num
...
begin
  delta fermat_42,
  simp_rw [mul_pow, ← pow_mul, ← mul_add],
  simp [hk0]
end

lemma ne_zero {a b c : ℤ} (h : fermat_42 a b c) : c ≠ 0 :=
begin
  refine ne_zero_pow two_ne_zero (ne_of_gt _),
  rcases h with ⟨a0, b0, h⟩,
  rw [← h, ← two_mul_two, pow_mul, pow_mul],
  refine add_pos _ _;
  exact sq_pos_of_ne_zero _ (pow_ne_zero 2 ‹_›),
end
...
begin
  ...
  have S_nonempty : S.nonempty := ⟨int.nat_abs c, ⟨a, b, c⟩, h, rfl⟩,
  let m : ℕ := nat.find S_nonempty,
  have m_mem : m ∈ S := nat.find_spec S_nonempty,
  rcases m_mem with ⟨s0, hs0, hs1⟩,
  refine ⟨s0.1, s0.2.1, s0.2.2, hs0, λ a1 b1 c1 h1, _⟩,
  rw ← hs1,
  exact nat.find_min' _ ⟨⟨a1, b1, c1⟩, h1, rfl⟩,
end
...
begin
  ...
  have hpc : (p : ℤ) ^ 2 ∣ c,
  { apply (int.pow_dvd_pow_iff zero_lt_two).mp,
    rw [← h.1.2.2, mul_pow, mul_pow, ← mul_add, ← pow_mul],
    exact dvd.intro (a1 ^ 4 + b1 ^ 4) rfl },
  obtain ⟨c1, rfl⟩ := hpc,
  have hf : fermat_42 a1 b1 c1 :=
     (fermat_42.mul (int.coe_nat_ne_zero.mpr (nat.prime.ne_zero hp))).mpr h.1,
...
lemma neg_of_minimal {a b c : ℤ} : (minimal a b c) → (minimal a b (-c)) :=
λ ⟨⟨ha, hb, heq⟩, h2⟩, ⟨⟨ha, hb, heq.trans (neg_sq _).symm⟩, λ x y z f,
  (int.nat_abs_neg c).le.trans (h2 x y z f)⟩

lemma int.dvd_one (n : ℤ) : n ∣ 1 ↔ n = 1 ∨ n = -1 :=
is_unit_iff_dvd_one.symm.trans int.is_unit_iff
...
begin
  ...
  { cases int.mod_two_eq_zero_or_one b0 with hbp hbp,
    { have h1 : 2 ∣ (int.gcd a0 b0 : ℤ) :=
      ...
      rcases (int.dvd_one _).mp h1 with ⟨⟨⟩⟩ | ⟨⟨⟩⟩ },
...
begin
  ...
  { exact ⟨a0, b0, c0, hf, hc, h1⟩ },
  { exact (ne_zero hf.1 rfl).elim },
  { exact ⟨a0, b0, -c0, neg_of_minimal hf, hc, neg_pos.mpr h1⟩ }
end
...
begin
  refine is_coprime.mul_right (int.coprime_of_sq_sum (is_coprime_comm.mp h)) _,
  rw add_comm,
  exact int.coprime_of_sq_sum h
end
...
begin
  ...
  have ht : pythagorean_triple (a ^ 2) (b ^ 2) c :=
    calc ((a ^ 2) * (a ^ 2)) + ((b ^ 2) * (b ^ 2))
          = a ^ 4 + b ^ 4 : by rw [← two_mul_two, pow_mul, pow_mul, ← pow_two, ← pow_two]
      ... = c ^ 2 : h.1.2.2
      ... = c * c : sq _ ,
  -- coprime requirement:
  ...
  have ha22 : a ^ 2 % 2 = 1, { rw [sq, int.mul_mod, ha2], refl },
  ...
  { apply int.gcd_eq_one_iff_coprime.mpr,
    ...
    rw [← sq, ht1, sq n, sub_eq_add_neg, neg_mul_eq_neg_mul],
  ...
  have h4 : 0 < m,
  { refine lt_of_le_of_ne ht6 _,
    rintro rfl,
    refine hb20 (ht2.trans _),
  ...
  have hb2 : 2 ∣ b,
  { refine @int.prime.dvd_pow' _ 2 _ nat.prime_two _,
    rw [ht2, mul_assoc],
  ...
  have hs : b' ^ 2 = m * (r * s),
  { apply (mul_right_inj'
      (by { rw [ne.def, bit0_eq_zero, bit0_eq_zero], exact one_ne_zero } : (4 : ℤ) ≠ 0)).mp,
