...
/- Providing a construction for all larger values. -/
...
begin
  simp_rw [is_frobenius, add_submonoid.mem_closure_pair],
  push_neg,
  exact ⟨mul_add_mul_ne_max_val cop hm hn, λ k hk, not_lt.mp (mt (not_in_clos_nat_pair cop hm hn k)
    (λ ⟨a, b, H⟩, hk a b H))⟩,
end
