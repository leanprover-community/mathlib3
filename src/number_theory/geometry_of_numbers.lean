...
namespace subtype
variables {V : Type*} [measure_space V] {p : V → Prop} {s t : set V}

instance measure_space : measure_space (subtype p) :=
{ volume := measure.comap (coe : subtype p → V) volume,
  ..subtype.measurable_space }

--TODO: Fix `measure.comap` so that this works for `null_measurable_set s`
@[simp] lemma volume_preimage_coe (hs : measurable_set s) (ht : measurable_set t) :
  volume ((coe : s → V) ⁻¹' t) = volume (t ∩ s) :=
begin
  dsimp only [measure_space.volume],
  rw [measure.comap_apply _ subtype.coe_injective (λ x, hs.subtype_image) _
    (measurable_subtype_coe ht), image_preimage_eq_inter_range, subtype.range_coe],
end

@[simp] lemma volume_univ (hs : measurable_set s) : volume (univ : set s) = volume s :=
...
begin
  ...
  refine Exists₃.imp _ (exists_nonempty_inter_of_measure_univ_lt_tsum_measure
    subtype.measure_space.volume (_ : (∀ p : X, measurable_set ((coe : F → Y) ⁻¹' (p • S)))) _),
  { exact λ i j hij ⟨x, hi, hj⟩, ⟨x, ⟨hi, x.2⟩, hj, x.2⟩ },
  { exact λ p, measurable_id'.subtype_coe (hS.smul p) },
  have hF : measurable_set F := sorry, -- false but we eventually won't need it.
  rw ←subtype.volume_univ hF at hlt,
  simp_rw subtype.volume_preimage_coe hF (hS.smul _),
