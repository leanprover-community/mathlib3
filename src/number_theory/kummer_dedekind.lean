...
variables  [algebra R S] {pb : power_basis R S} {I : ideal R}

noncomputable instance [hI : is_maximal I] : field (R ⧸ I) :=
...
((normalized_factors_equiv_of_quot_equiv ↑(pb.quotient_equiv_quotient_minpoly_map I) hI'
  (by by_contra ; exact hpb (span_singleton_eq_bot.mp h))).trans
...
theorem multiplicity_factors_map_eq_multiplicity (hI : is_maximal I)
  ...
  multiplicity J (I.map (algebra_map R S)) =
    multiplicity ↑(normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb ⟨J, hJ⟩)
...
theorem normalized_factors_ideal_map_eq_normalized_factors_min_poly_mk_map
  ...
  normalized_factors (I.map (algebra_map R S)) =
    multiset.map (λ f,
    ((normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb).symm f : ideal S))
...
begin
  ...
  { rw [multiset.count_eq_zero.mpr hJ, eq_comm, multiset.count_eq_zero, multiset.mem_map],
    ...
    rintros J' rfl,
    exact hJ
      ((normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb).symm J').prop },

  -- Then we just have to compare the multiplicities, which we already proved are equal.
  have := multiplicity_factors_map_eq_multiplicity hI hI' hpb hJ,
  ...
  generalize hJ' : (normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb)
    ⟨J, hJ⟩ = J',
  have : ((normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb).symm
    J' : ideal S) = J,
  ...
  rw [multiset.count_map_eq_count' (λ f,
      ((normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb).symm f
        : ideal S)),
      multiset.attach_count_eq_count_coe],
  { exact subtype.coe_injective.comp (equiv.injective _) },
  { exact (normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb _).prop },
  { exact irreducible_of_normalized_factor _
    (normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb _).prop },
  { assumption },
...
begin
  ...
  rw normalized_factors_ideal_map_eq_normalized_factors_min_poly_mk_map hI hI' hpb,
  use ((normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hpb).symm
