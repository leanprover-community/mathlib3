...
lemma unlikely_strong_probable_prime_of_composite (n : ℕ) [hn_pos : fact (0 < n)] (not_prime : ¬ n.prime) :
  ((finset.univ : finset (zmod n)).filter (strong_probable_prime n)).card ≤ n / 4 :=
begin
  -- TODO(Sean): it would be good to split up these two cases into their own lemmas. Make two lemmas above this one, with the same arguments and hypotheses. Then add the extra hypothesis (h : (∃ (n0 n1 : ℕ), nat.coprime n0 n1 ∧ n0 * n1 = n ∧ 1 < n0 ∧ 1 < n1)) for the first and the extra hypothesis (h : (∃ (p k : ℕ), p.prime ∧ p^k = n)) for the second.
  -- Then put the partial proof below for the first subgoal generated by the cases statment as the first lemma proof.
  -- After that we should be able to prove this lemma by just calling those two lemmas after the cases line.
