...
begin
  letI : finite_dimensional F K := finite_dimensional.right ℚ  _ _ ,
  ...
  { intro ha,
    ...
    letI : fact (irreducible $ minpoly F x) := ⟨minpoly.irreducible hx⟩,
    ...
    letI : finite_dimensional Fx K := finite_dimensional.right ℚ  _ _ ,
    let ψ₀ : K →ₐ[Fx] A := is_alg_closed.lift (algebra.is_algebraic_of_finite _ _),
    letI : is_scalar_tower F Fx K := is_scalar_tower.of_ring_hom (adjoin_root.lift_hom _ _ hK),
    letI : is_scalar_tower F Fx A := is_scalar_tower.of_ring_hom (adjoin_root.lift_hom _ _ hA),
