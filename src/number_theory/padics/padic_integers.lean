...
begin
  ...
  use k,
  rw ← inv_lt_inv hε (_root_.fzpow_pos_of_pos _ _),
  { rw [fzpow_neg, inv_inv₀, zpow_coe_nat],
...
begin
  ...
  rw [← neg_nonpos, ← (fzpow_strict_mono h).le_iff_le],
...
begin
  ...
  rwa [norm_eq_pow_val aux, norm_p_pow, norm_eq_pow_val hc,
      ← fzpow_add, ← neg_add, fzpow_inj, neg_inj] at this,
  { exact_mod_cast hp_prime.1.pos },
...
have hu : ∥u∥ = 1,
by simp [hx, nat.fzpow_ne_zero_of_pos (by exact_mod_cast hp_prime.1.pos) x.valuation,
         norm_eq_pow_val, fzpow_neg, inv_mul_cancel, -cast_eq_of_rat_of_nat],
mk_units hu
...
begin
  ...
  { rw [unit_coeff_coe, mul_assoc, ← fzpow_add],
    { simp },
    { exact_mod_cast hp_prime.1.ne_zero } },
...
begin
  ...
  simp only [int.coe_nat_le, fzpow_neg, zpow_coe_nat],
...
begin
  by_cases hx : x = 0,
  { subst hx,
    simp only [norm_zero, fzpow_neg, zpow_coe_nat, inv_nonneg, iff_true, submodule.zero_mem],
...
{ prec' := λ x hx,
  begin
    ...
    { refine ⟨x'.lim, λ n, _⟩,
      have : (0:ℝ) < p ^ (-n : ℤ), { apply fzpow_pos_of_pos, exact_mod_cast hp_prime.1.pos },
