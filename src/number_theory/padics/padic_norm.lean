...
-/
@[simp] protected lemma eq_fzpow_of_nonzero {q : ℚ} (hq : q ≠ 0) :
...
begin
  rw padic_norm.eq_fzpow_of_nonzero p hq,
  apply fzpow_ne_zero_of_ne_zero,
...
if hq : q = 0 then
  ...
  by simp [padic_norm, *, padic_val_rat.mul, fzpow_add this, mul_comm]
...
begin
  unfold padic_norm,
  rw [if_neg _],
  { refine fzpow_le_one_of_nonpos _ _,
...
begin
  ...
  { norm_cast at hz,
    ...
    simp [hz, this] },
  { rw [fzpow_le_iff_le, neg_le_neg_iff, padic_val_rat_of_int _ hp.1.ne_one _],
    { norm_cast,
