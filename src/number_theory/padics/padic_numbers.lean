...
instance field : field (ℚ_[p]) := cau_seq.completion.field
...
@[simp] lemma of_rat_neg : ∀ x : ℚ, of_rat p (-x) = -of_rat p x := cau_seq.completion.of_rat_neg
...
lemma cast_eq_of_rat_of_int (n : ℤ) : (n : ℚ_[p]) = of_rat p n :=
