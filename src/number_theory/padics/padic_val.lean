...
lemma one_le_padic_val_nat_of_dvd {n : ℕ} [prime : fact p.prime] (n_pos : 0 < n) (div : p ∣ n) :
  1 ≤ padic_val_nat p n :=
begin
...
protected lemma defn (p : ℕ) [hp : fact p.prime] {q : ℚ} {n d : ℤ} (hqz : q ≠ 0)
  (qdf : q = n /. d) : padic_val_rat p q = (multiplicity (p : ℤ) n).get
...
begin
  ...
  simp [hc1, hc2, multiplicity.mul' (nat.prime_iff_prime_int.1 hp.1),
    (ne.symm (ne_of_lt hp.1.one_lt)), hqz, pos_iff_ne_zero],
...
begin
  ...
  calc _ ≤ min (multiplicity (p : ℤ) (q.num * r.denom * q.denom))
    ...
    (by rw [mul_assoc, @multiplicity.mul _ _ _ _ (q.denom : ℤ)
        (_ * _) (nat.prime_iff_prime_int.1 hp.1)];
...
protected lemma mul {q r : ℕ} (hq : q ≠ 0) (hr : r ≠ 0) :
...
protected lemma div_of_dvd {a b : ℕ} (h : b ∣ a) :
...
protected lemma div {b : ℕ} (dvd : p ∣ b) :
...
protected lemma pow {q : ℕ} (n : ℕ) (hq : q ≠ 0) :
  padic_val_nat p (q ^ n) = n * padic_val_nat p q :=
begin
  apply @nat.cast_injective ℤ,
  push_cast,
  exact padic_val_rat.pow (cast_ne_zero.mpr hq)
end
...
protected lemma div_pow {b k : ℕ} (dvd : p ^ k ∣ b) :
...
lemma padic_val_int_dvd_iff (n : ℕ) (a : ℤ) :
  (p : ℤ) ^ n ∣ a ↔ a = 0 ∨ n ≤ padic_val_int p a :=
...
begin
  rw @padic_val_int.mul p _ _ _ ha (int.coe_nat_ne_zero.mpr (hp.out).ne_zero),
