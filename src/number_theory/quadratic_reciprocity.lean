...
begin
  ...
  { intros b hb,
    rw [Ico.mem, succ_le_iff, ← succ_sub hp, succ_sub_one, pos_iff_ne_zero] at hb,
...
begin
  have he : ∀ {x}, x ∈ Ico 1 (p / 2).succ → x ≠ 0 ∧ x ≤ p / 2,
    by simp [lt_succ_iff, succ_le_iff, pos_iff_ne_zero] {contextual := tt},
...
have hx2 : ∀ x ∈ Ico 1 (p / 2).succ, (2 * x : zmod p).val = 2 * x,
  from λ x hx, have h2xp : 2 * x < p,
      ...
      ... < _ : by conv_rhs {rw [← div_add_mod p 2, hp1.1]}; exact lt_succ _,
