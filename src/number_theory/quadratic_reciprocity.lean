...
begin
  ...
  { unfreezingI { subst p }, refine iff_of_true ⟨1, _⟩ _; apply subsingleton.elim  },
...
begin
  cases nat.prime.eq_two_or_odd ‹p.prime› with hp2 hp_odd,
  { unfreezingI { subst p }, exact dec_trivial },
...
begin
  cases nat.prime.eq_two_or_odd ‹p.prime› with hp2 hp_odd,
  { unfreezingI { subst p }, revert a ha, exact dec_trivial },
...
if hpq : p = q then by unfreezingI { subst hpq } else
have h1 : ((p / 2) * (q / 2)) % 2 = 0,
