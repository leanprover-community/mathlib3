...
lemma finrank_quotient_map.span_eq_top [is_domain R] [is_domain S] [algebra K L] [is_noetherian R S]
  ...
  (hRK : function.injective (algebra_map R K))
  (hp : p ≠ ⊤)
...
begin
  ...
  { rw is_scalar_tower.algebra_map_eq R K L,
    exact (algebra_map K L).injective.comp hRK },
...
begin
  ...
  { rw set.range_comp,
    refine finrank_quotient_map.span_eq_top p (is_fraction_ring.injective R K) hp.ne_top _
           (top_le_iff.mp _),
