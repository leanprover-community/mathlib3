...
* A `circular_order` is a ternary betweenness relation `btw : α → α → α → Prop`, which is
  - reflexive: `btw a a a`
...
What's next is to define circular groups and provide instances for `zmod` and the usual circle group
`ℝ/ℤ`.
