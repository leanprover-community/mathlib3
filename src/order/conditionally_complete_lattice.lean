...
mul_inv_le_iff_le_mul.mp $ le_cinfi $ λ gi, mul_inv_le_iff_le_mul.mpr $
  inv_mul_le_iff_le_mul.mp $ le_cinfi $ λ hi, inv_mul_le_iff_le_mul.mpr $ H _ _

@[to_additive]
lemma csupr_mul_csupr_le {a : α} {g h : ι → α} (H : ∀ i j, g i * h j ≤ a) : supr g * supr h ≤ a :=
@le_cinfi_mul_cinfi (order_dual α) _ _ _ _ _ _ _ _ _ H
