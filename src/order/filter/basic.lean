...
mem_sets_of_superset univ_mem_sets (λ x _, h x)

lemma mp_sets (hs : s ∈ f) (h : {x | x ∈ s → x ∈ t} ∈ f) : t ∈ f :=
mem_sets_of_superset (inter_mem_sets hs h) $ λ x ⟨h₁, h₂⟩, h₂ h₁
...
⟨λ ⟨t, ht, ts⟩, mem_sets_of_superset ht ts, λ hs, ⟨s, hs, subset.rfl⟩⟩

lemma monotone_mem_sets {f : filter α} : monotone (λ s, s ∈ f) :=
λ s t hst h, mem_sets_of_superset h hst
...
{ sets             := {s | {t : filter α | s ∈ t} ∈ f},
  ...
  sets_of_superset := λ x y hx xy,
    mem_sets_of_superset hx $ λ f h, mem_sets_of_superset h xy,
  inter_sets       := λ x y hx hy,
    mem_sets_of_superset (inter_mem_sets hx hy) $ λ f ⟨h₁, h₂⟩, inter_mem_sets h₁ h₂ }
...
iff.intro
  ...
  (λ h u hu, hu.rec_on h univ_mem_sets
    (λ x y _ hxy hx, mem_sets_of_superset hx hxy)
    (λ x y _ _ hx hy, inter_mem_sets hx hy))
...
instance : has_inf (filter α) := ⟨λ f g : filter α,
{ sets             := {s | ∃ (a ∈ f) (b ∈ g), a ∩ b ⊆ s },
  univ_sets        := ⟨_, univ_mem_sets, _, univ_mem_sets, inter_subset_left _ _⟩,
  sets_of_superset := λ x y ⟨a, ha, b, hb, h⟩ xy, ⟨a, ha, b, hb, h.trans xy⟩,
  inter_sets       := λ x y ⟨a, ha, b, hb, hx⟩ ⟨c, hc, d, hd, hy⟩,
...
instance : complete_lattice (filter α) := original_complete_lattice.copy
  ...
  (top_unique $ λ s hs, by simp [mem_top_sets.1 hs])
  /- bot -/ _ rfl
  /- sup -/ _ rfl
  ...
  begin
    ext f g : 2,
    exact le_antisymm
      (le_inf (λ s, mem_inf_sets_of_left) (λ s, mem_inf_sets_of_right))
      (λ s ⟨a, ha, b, hb, hs⟩, show s ∈ complete_lattice.inf f g, from
...
show (∀ {t}, s ⊆ t → t ∈ f) ↔ s ∈ f,
  from ⟨λ h, h (subset.refl s), λ hs t ht, mem_sets_of_superset hs ht⟩
...
⟨λ h, bot_unique $ λ s _, mem_sets_of_superset h (empty_subset s),
...
empty_in_sets_eq_bot.mp $ univ_mem_sets' $ λ x, false.elim (ne ⟨x⟩)
...
by filter_upwards [hs₁] λ a ha, classical.by_contradiction $ λ ha', hs ⟨ha, hs₂ ha'⟩
...
filter.ext $ λ x, by simp only [mem_supr_sets, mem_join_sets]
...
{ le_sup_inf :=
  begin
    ...
    exact ⟨s ∪ t₁,
      ...
      (@le_sup_inf (set α) _ _ _ _).trans (union_subset subset.rfl hts)⟩
  end,
...
begin
  ...
  { intros t₁ ht₁ t₂ p hp ht₂ ht,
    ...
    have eq : s.inf (λ j, function.update p a t₁ j) = s.inf (λ j, p j) :=
      finset.inf_congr rfl this,
    simp only [this, ht₁, hp, function.update_same, true_and, imp_true_iff, eq]
      {contextual := tt},
    exact (inter_subset_inter subset.rfl ht₂).trans ht },
  { exact λ p hpa hp ht, ⟨p a, hpa, (s.inf p), ⟨p, hp, le_rfl⟩, ht⟩ }
end
...
begin
  ...
  { intros s hs, rwa [mem_top_sets.1 hs] },
...
filter.ext $ λ x, by simp only [mem_supr_sets, mem_principal_sets, Union_subset_iff]
...
begin
  ...
  { rintro ⟨p, hp, ht⟩,
    calc (⋂ (i : ι) (H : i ∈ s), f i) ≤ (⋂ (i : ι) (H : i ∈ s), p i) :
      infi_le_infi (λ i, infi_le_infi (λ hi, mem_principal_sets.1 (hp i hi)))
    ... ≤ t : ht },
  { exact λ h, ⟨f, λ i hi, subset.rfl, h⟩ }
end
...
{ sets             := preimage m ⁻¹' f.sets,
  ...
  sets_of_superset := λ s t hs st, mem_sets_of_superset hs $ preimage_mono st,
  inter_sets       := λ s t hs ht, inter_mem_sets hs ht }
...
iff.intro
  ...
  (λ ⟨s, hs, ht⟩, mem_sets_of_superset (image_mem_map hs) ht)
...
{ sets             := { s | ∃ t ∈ f, m ⁻¹' t ⊆ s },
  ...
  sets_of_superset := λ a b ⟨a', ha', ma'a⟩ ab,
    ⟨a', ha', ma'a.trans ab⟩,
...
le_antisymm (λ s, preimage_mem_comap) (λ s ⟨t, ht, hst⟩, mem_sets_of_superset ht hst)
...
⟨λ h s ⟨t, ht, hts⟩, mem_sets_of_superset (h ht) hts, λ h s ht, h ⟨_, ht, subset.rfl⟩⟩
...
le_antisymm
  (λ s hs,
    ...
    ⟨⋃ i, t i, mem_supr_sets.2 $ λ i, (f i).sets_of_superset (ht i).1 (subset_Union _ _),
      begin
...
lemma comap_map {f : filter α} {m : α → β} (h : function.injective m) :
  comap m (map m f) = f :=
le_antisymm
  (λ s hs, mem_sets_of_superset (preimage_mem_comap $ image_mem_map hs) $
...
λ t ht, by filter_upwards [hsf, h $ image_mem_map (inter_mem_sets hsg ht)]
λ a has ⟨b, ⟨hbs, hb⟩, h⟩,
...
begin
  refine map_inf_le.antisymm (λ s hs, _),
  ...
  { filter_upwards [h₁, htf] λ a h₁ h₂, mem_image_of_mem _ ⟨h₁, h₂⟩ },
  { filter_upwards [h₂, htg] λ a h₁ h₂, mem_image_of_mem _ ⟨h₁, h₂⟩ },
  { rw [image_inter_on],
...
map_inf' univ_mem_sets univ_mem_sets (λ x _ y _ hxy, h hxy)
...
lemma le_map {f : filter α} {m : α → β} {g : filter β} (h : ∀ s ∈ f, m '' s ∈ g) :
  g ≤ f.map m :=
λ s hs, mem_sets_of_superset (h _ hs) $ image_preimage_subset _ _
...
begin
  ...
  { refine sets_of_superset (map (λ g : α→β, g a) f) (image_mem_map hs) _,
...
begin
  ...
  { rcases mem_seq_sets_iff.1 ht with ⟨u, hu, v, hv, ht⟩,
    refine mem_sets_of_superset _ (set.seq_mono subset.rfl ht),
    rw set.seq_seq,
...
begin
  ...
  { rcases mem_map_sets_iff.1 hs with ⟨u, hu, hs⟩,
    refine mem_sets_of_superset _ (set.seq_mono hs subset.rfl),
...
begin
  ...
  split,
  exact λ ⟨t₁, ⟨s₁, hs₁, hts₁⟩, t₂, ⟨s₂, hs₂, hts₂⟩, h⟩,
    ⟨s₁, hs₁, s₂, hs₂, (inter_subset_inter hts₁ hts₂).trans h⟩,
  exact λ ⟨t₁, ht₁, t₂, ht₂, h⟩,
    ⟨prod.fst ⁻¹' t₁, ⟨t₁, ht₁, subset.rfl⟩, prod.snd ⁻¹' t₂, ⟨t₂, ht₂, subset.rfl⟩, h⟩
end
