...
begin
  ...
  { rw eventually_countable_forall,
    ...
    exact (ennreal.add_lt_add_iff_left hx_top).mpr (by simv), },
...
begin
  ...
  from function.bijective_iff_has_inverse.mpr ⟨(λ x, a⁻¹ * x),
    ⟨λ x, by simv [←mul_assoc, inv_mul_cancel ha_zero ha_top],
    λ x, by simv [g, ←mul_assoc, mul_inv_cancel ha_zero ha_top]⟩⟩,
...
begin
  ...
  { have hau : (λ x, a * (u x)) =ᶠ[f] 0,
    { refine hu.mono (λ x hx, _),
      ...
      simv [hx], },
    simv only [limsup_congr hu, limsup_congr hau, pi.zero_apply, ← bot_eq_zero, limsup_const_bot],
    simv, },
  { simp_rw [ha_top, top_mul],
    ...
    simv only [h_top_le, hfu, if_false], },
end
