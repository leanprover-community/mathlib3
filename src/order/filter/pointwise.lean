...
This file defines pointwise operations on filters. This is useful because usual filter operations
...
* `0 : filter α`: Principal filter at `0 : α`.
* `1 : filter α`: Principal filter at `1 : α`.
* `f + g`: Addition, filter generated by all `s + t` where `s ∈ f` and `t ∈ g`.
* `f * g`: Multiplication, filter generated by all `s * t` where `s ∈ f` and `t ∈ g`.
