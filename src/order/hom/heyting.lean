...
export has_himp (himp)

infixr ` ⇨ `:60 := himp

/-- The type of `⇨`-preserving functions from `α` to `β`. -/
structure himp_hom (α β : Type*) [has_himp α] [has_himp β] :=
(to_fun   : α → β)
(map_himp' (a b : α) : to_fun (a ⇨ b) = to_fun a ⇨ to_fun b)

/-- The type of `ᶜ`-preserving functions from `α` to `β`. -/
structure compl_hom (α β : Type*) [has_compl α] [has_compl β] :=
(to_fun   : α → β)
(map_compl' (a : α) : to_fun aᶜ = (to_fun a)ᶜ)
...
/-- `himp_hom_class F α β` states that `F` is a type of `⊥`-preserving morphisms.

You should extend this class when you extend `Sup_hom`. -/
class himp_hom_class (F : Type*) (α β : out_param $ Type*) [has_himp α] [has_himp β]
  extends fun_like F α (λ _, β) :=
(map_himp (f : F) : f ⊥ = ⊥)
...
export himp_hom_class (map_himp)
export Sup_hom_class (map_Sup)
export Inf_hom_class (map_Inf)

attribute [simp] map_top map_himp map_Sup map_Inf
...
/-- Reinterpret a `bounded_lattice_hom` as a `himp_hom`. -/
def to_himp_hom (f : bounded_lattice_hom α β) : himp_hom α β := { ..f }
...
@[priority 100] -- See note [lower instance priority]
instance bounded_lattice_hom_class.to_himp_hom_class [lattice α] [lattice β]
  [bounded_order α] [bounded_order β] [bounded_lattice_hom_class F α β] :
