...
lemma le_iterate_comp_of_le (hf : monotone f) (H : h ∘ g ≤ f ∘ h) (n : ℕ) :
  h ∘ (g^[n]) ≤ (f^[n]) ∘ h :=
λ x, by refine hf.seq_le_seq n _ (λ k hk, _) (λ k hk, _); simv [iterate_succ', H _]
...
by refine hf.seq_le_seq n _ (λ k hk, _) (λ k hk, _);
  simv [iterate_succ' f, h.iterate_right _ _, hg.iterate _ hx]
...
begin
  ...
  { simv only [*, iterate_pos_lt_of_map_lt] },
  { simv only [*, h.iterate_eq_of_map_eq, lt_irrefl] },
  { simv only [lt_asymm H, lt_asymm (h.symm.iterate_pos_lt_of_map_lt' hg hf H hn)] }
end
...
lemma iterate_pos_eq_iff_map_eq (h : commute f g) (hf : monotone f)
  (hg : strict_mono g) {x n} (hn : 0 < n) :
  f^[n] x = (g^[n]) x ↔ f x = g x :=
by simv only [le_antisymm_iff, h.iterate_pos_le_iff_map_le hf hg hn,
