...
variables {α : Type*} [partial_order α] [succ_order α] [pred_order α] [succ_pred_archimedean α]
...
@refl_trans_gen_of_succ_of_le αᵒᵈ _ _ _ _ r n m (λ x hx, h x ⟨hx.2, hx.1⟩) hnm
...
@refl_trans_gen_of_succ_of_ge αᵒᵈ _ _ _ _ r n m (λ x hx, h x ⟨hx.2, hx.1⟩) hmn
...
@trans_gen_of_succ_of_lt αᵒᵈ _ _ _ _ r _ _ (λ x hx, h x ⟨hx.2, hx.1⟩) hnm
...
@trans_gen_of_succ_of_gt αᵒᵈ _ _ _ _ r _ _ (λ x hx, h x ⟨hx.2, hx.1⟩) hmn

end partial_order

section linear_order
variables {α : Type*} [linear_order α] [succ_order α] [pred_order α] [succ_pred_archimedean α]
...
@refl_trans_gen_of_succ αᵒᵈ _ _ _ _ r n m (λ x hx, h1 x ⟨hx.2, hx.1⟩)
...
lemma trans_gen_of_pred_of_ne (r : α → α → Prop) {n m : α}
  ...
  (hnm : n ≠ m) : trans_gen r n m :=
@trans_gen_of_succ_of_ne αᵒᵈ _ _ _ _ r n m (λ x hx, h1 x ⟨hx.2, hx.1⟩)
...
@trans_gen_of_succ_of_reflexive αᵒᵈ _ _ _ _ r n m hr (λ x hx, h1 x ⟨hx.2, hx.1⟩)
  (λ x hx, h2 x ⟨hx.2, hx.1⟩)
