...
lemma sdiff_symm_diff : c \ (a Δ b) = (a ⊓ b) ⊔ ((c \ a) ⊓ (c \ b)) :=
calc c \ (a Δ b) = ((c \ a) ⊔ b) ⊓ ((c \ b) ⊔ a)         : by simp only [(Δ), sdiff_sdiff_sup_sdiff]
...
lemma compl_symm_diff : (a Δ b)ᶜ = (a ⊓ b) ⊔ (aᶜ ⊓ bᶜ) := by simp only [←top_sdiff, sdiff_symm_diff]
