...
This file defines two type synonyms for order theory:
...
One should not abuse definitional equality between `α` and `order_dual α`/`lex α`. Instead, explicit
coercions should be inserted:
* `order_dual`: `order_dual.to_dual : α → order_dual α` and `order_dual.of_dual : order_dual α → α`
...
def to_dual : α ≃ order_dual α := equiv.refl _

/-- `of_dual` is the identity function from the `order_dual` of a linear order.  -/
def of_dual : order_dual α ≃ α := equiv.refl _
...
@[simp] lemma of_dual_inj {a b : order_dual α} : of_dual a = of_dual b ↔ a = b := iff.rfl
...
@[simp] lemma of_dual_le_of_dual [has_le α] {a b : order_dual α} : of_dual a ≤ of_dual b ↔ b ≤ a :=
iff.rfl

@[simp] lemma of_dual_lt_of_dual [has_lt α] {a b : order_dual α} : of_dual a < of_dual b ↔ b < a :=
iff.rfl

lemma le_to_dual [has_le α] {a : order_dual α} {b : α} : a ≤ to_dual b ↔ b ≤ of_dual a := iff.rfl
lemma lt_to_dual [has_lt α] {a : order_dual α} {b : α} : a < to_dual b ↔ b < of_dual a := iff.rfl
lemma to_dual_le [has_le α] {a : α} {b : order_dual α} : to_dual a ≤ b ↔ of_dual b ≤ a := iff.rfl
lemma to_dual_lt [has_lt α] {a : α} {b : order_dual α} : to_dual a < b ↔ of_dual b < a := iff.rfl
