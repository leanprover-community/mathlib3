...
/-!
# Transport bot to top

This file defines an attribute `to_order_dual` that can be used to
...
Usage information is contained in the doc string of `to_order_dual.attr`.
...
that have been processed by `to_order_dual`. -/
@[user_attribute]
meta def aux_attr : user_attribute (name_map name) name :=
{ name      := `to_order_dual_aux,
  descr     := "Auxiliary attribute for `to_order_dual`. DON'T USE IT",
  parser    := failed,
  cache_cfg := ⟨λ ns,
                ns.mfoldl
                  (λ dict n', do
                   let n := match n' with
                            | name.mk_string s pre := if s = "_to_order_dual" then pre else n'
                            | _ := n'
                            end,
...
/--
An attribute that tells `@[to_order_dual]` that certain arguments of this definition are not
involved when using `@[to_order_dual]`.
This helps the heuristic of `@[to_order_dual]` by also transforming definitions if `ℕ` or another
fixed type occurs as one of these arguments.
-/
@[user_attribute]
meta def ignore_args_attr : user_attribute (name_map $ list ℕ) (list ℕ) :=
{ name      := `to_order_dual_ignore_args,
  descr     :=
    "Auxiliary attribute for `to_order_dual` stating that certain arguments are not additivized.",
  cache_cfg :=
    ⟨λ ns, ns.mfoldl
      (λ dict n, do
...
/--
An attribute that is automatically added to declarations tagged with `@[to_order_dual]`, if needed.
...
See the Heuristics section of `to_order_dual.attr` for more details.

If a declaration is not tagged, it is presumed that the first argument is relevant.
`@[to_order_dual]` uses the function `to_order_dual.first_multiplicative_arg` to automatically tag
...
Warning: adding `@[to_order_dual_reorder]` with an equal or smaller number than the number in this
attribute is currently not supported.
-/
@[user_attribute]
meta def relevant_arg_attr : user_attribute (name_map ℕ) ℕ :=
{ name      := `to_order_dual_relevant_arg,
  descr     :=
    "Auxiliary attribute for `to_order_dual` stating which arguments are the types with a " ++
...
applying `@[to_order_dual]`. Currently, we only support swapping consecutive arguments.
...
Example: `@[to_order_dual_reorder 1 4]` swaps the first two arguments and the arguments in
positions 4 and 5.
-/
@[user_attribute]
meta def reorder_attr : user_attribute (name_map $ list ℕ) (list ℕ) :=
{ name      := `to_order_dual_reorder,
  descr     :=
    "Auxiliary attribute for `to_order_dual` that stores arguments that need to be reordered.",
  cache_cfg :=
    ⟨λ ns, ns.mfoldl
      (λ dict n, do
...
meta def first_multiplicative_arg (nm : name) : tactic ℕ := do
  ...
  { let tgt := bi.type.pi_codomain,
    ...
    tt ← has_attribute' `to_order_dual tgt.get_app_fn.const_name | return none,
...
/-- A command that can be used to have future uses of `to_order_dual` change the `src` namespace
to the `tgt` namespace.

For example:
```
run_cmd to_order_dual.map_namespace `quotient_group `quotient_add_group
```

Later uses of `to_order_dual` on declarations in the `quotient_group` namespace will be created
in the `quotient_add_group` namespaces.
-/
meta def map_namespace (src tgt : name) : command :=
do let n := src.mk_string "_to_order_dual",
   let decl := declaration.thm n [] `(unit) (pure (reflect ())),
   add_decl decl,
   aux_attr.set n tgt tt

/-- `value_type` is the type of the arguments that can be provided to `to_order_dual`.
`to_order_dual.parser` parses the provided arguments:
...
* if `allow_auto_name` is `ff` (default) then `@[to_order_dual]` will check whether the given name
...
/-- Dictionary used by `to_order_dual.guess_name` to autogenerate names. -/
meta def tr : bool → list string → list string
| is_comm ("bot" :: s)                := add_comm_prefix is_comm "top"       :: tr ff s
| is_comm ("inf" :: s)                := add_comm_prefix is_comm "sup"       :: tr ff s
| is_comm ("min" :: s)                := add_comm_prefix is_comm "max"       :: tr ff s
...
/-- Autogenerate target name for `to_order_dual`. -/
...
(if tgt.get_prefix ≠ name.anonymous ∨ allow_auto_name -- `tgt` is a full name
 then pure tgt
      ...
      | (name.mk_string s pre) :=
        do let tgt_auto := guess_name s,
           guard (tgt.to_string ≠ tgt_auto ∨ tgt = src)
             <|> trace ("`to_order_dual " ++ src.to_string ++ "`: correctly autogenerated target " ++
      ...
      | _ := fail ("to_order_dual: can't transport " ++ src.to_string)
      end) >>=
(λ res,
  if res = src ∧ tgt ≠ src
  then fail ("to_order_dual: can't transport " ++ src.to_string ++ " to itself.
Give the desired additive name explicitly using `@[to_order_dual additive_name]`. ")
  else pure res)

/-- the parser for the arguments to `to_order_dual`. -/
...
so that future uses of `to_order_dual` will map them to the corresponding `tgt` fields. -/
...
/--
The attribute `to_order_dual` can be used to automatically transport theorems
...
```
@[to_order_dual add_foo "add_foo doc string"]
/-- foo doc string -/
theorem foo := sorry
```
...
copied to the additive version, then `to_order_dual` should come last:

```
@[simp, to_order_dual] lemma mul_one' {G : Type*} [group G] (x : G) : x * 1 = x := mul_one x
```

The following attributes are supported and should be applied correctly by `to_order_dual` to
...
The exception to this rule is the `simps` attribute, which should come after `to_order_dual`:
...
Additionally the `mono` attribute is not handled by `to_order_dual` and should be applied afterwards
...
that have previously been labeled with `to_order_dual`.

In the `mul_comm'` example above, `to_order_dual` maps:
...
`to_order_dual` uses heuristics to determine whether a particular identifier has to be
...
* Identifiers that have the `@[to_order_dual]` attribute are ignored.
  For example, multiplication in `↥Semigroup` is replaced by addition in `↥AddSemigroup`.
* If an identifier `d` has attribute `@[to_order_dual_relevant_arg n]` then the argument
  in position `n` is checked for a fixed type, instead of checking the first argument.
  `@[to_order_dual]` will automatically add the attribute `@[to_order_dual_relevant_arg n]` to a
  declaration when the first argument has no multiplicative type-class, but argument `n` does.
* If an identifier has attribute `@[to_order_dual_ignore_args n1 n2 ...]` then all the arguments in
  positions `n1`, `n2`, ... will not be checked for unapplied identifiers (start counting from 1).
  For example, `times_cont_mdiff_map` has attribute `@[to_order_dual_ignore_args 21]`, which means
...
If `@[to_order_dual]` fails because the additive declaration raises a type mismatch, there are
various things you can try.
The first thing to do is to figure out what `@[to_order_dual]` did wrong by looking at the type
mismatch error.
...
* Option 1: It additivized a declaration `d` that should remain multiplicative. Solution:
  * Make sure the first argument of `d` is a type with a multiplicative structure. If not, can you
    ...
    The reason is that `@[to_order_dual]` doesn't additivize declarations if their first argument
    contains fixed types like `ℕ` or `ℝ`. See section Heuristics.
    If the first argument is not the argument with a multiplicative type-class, `@[to_order_dual]`
    should have automatically added the attribute `@[to_order_dual_relevant_arg]` to the declaration.
    You can test this by running the following (where `d` is the full name of the declaration):
    ```
      run_cmd to_order_dual.relevant_arg_attr.get_param `d >>= tactic.trace
    ```
    ...
    the attribute `@[to_order_dual_relevant_arg n]` manually, where `n` is an argument with a
    multiplicative structure.
* Option 2: It didn't additivize a declaration that should be additivized.
  ...
  * If the fixed type has an additive counterpart (like `↥Semigroup`), give it the `@[to_order_dual]`
    attribute.
  * If the fixed type occurs inside the `k`-th argument of a declaration `d`, and the
    `k`-th argument is not connected to the multiplicative structure on `d`, consider adding
    attribute `[to_order_dual_ignore_args k]` to `d`.
  * If you want to disable the heuristic and replace all multiplicative
    identifiers with their additive counterpart, use `@[to_order_dual!]`.
* Option 3: Arguments / universe levels are incorrectly ordered in the additive version.
  ...
  * If this is not possible, add the `[to_order_dual_reorder k]` to the multiplicative declaration
    to indicate that the `k`-th and `(k+1)`-st arguments are reordered in the additive version.

If neither of these solutions work, and `to_order_dual` is unable to automatically generate the
...
```
attribute [to_order_dual foo_add_bar] foo_bar
```

This will allow future uses of `to_order_dual` to recognize that
`foo_bar` should be replaced with `foo_add_bar`.

### Handling of hidden definitions

Before transporting the “main” declaration `src`, `to_order_dual` first
scans its type and value for names starting with `src`, and transports
them. This includes auxiliary definitions like `src._match_1`,
`src._proof_1`.

In addition to transporting the “main” declaration, `to_order_dual` transports
...
If `src` is a structure, then `to_order_dual` automatically adds
structure fields to its mapping, and similarly for constructors of
inductive types.

For new structures this means that `to_order_dual` automatically handles
coercions, and for old structures it does the same, if ancestry
information is present in `@[ancestor]` attributes. The `ancestor`
attribute must come before the `to_order_dual` attribute, and it is
...
* If `@[to_order_dual]` is called without a `name` argument, then the
  new name is autogenerated.  First, it takes the longest prefix of
  the source name that is already known to `to_order_dual`, and replaces
...
* Namespaces can be transformed using `map_namespace`. For example:
  ...
  Later uses of `to_order_dual` on declarations in the `quotient_group`
  namespace will be created in the `quotient_add_group` namespaces.

* If `@[to_order_dual]` is called with a `name` argument `new_name`
  /without a dot/, then `to_order_dual` updates the prefix as described
  above, then replaces the last part of the name with `new_name`.

* If `@[to_order_dual]` is called with a `name` argument
  `new_namespace.new_name` /with a dot/, then `to_order_dual` uses this
  new name as is.

As a safety check, in the first case `to_order_dual` double checks
...
{ name      := `to_order_dual,
  ...
  after_set := some $ λ src prio persistent, do
    guard persistent <|> fail "`to_order_dual` can't be used as a local attribute",
    env ← get_env,
    ...
    then proceed_fields env src tgt prio
      ...
      mwhen (has_attribute' `simps src)
        (trace "Apply the simps attribute after the to_order_dual attribute"),
      mwhen (has_attribute' `mono src)
        (trace $ "to_order_dual does not work with mono, apply the mono attribute to both" ++
          "versions after"),
      match val.doc with
...
/- map operations -/
attribute [to_order_dual_reorder] has_le
attribute [to_order_dual] has_inf
attribute [to_order_dual] has_inf.inf
attribute [to_order_dual] semilattice_inf
attribute [to_order_dual] semilattice_inf.to_has_inf
-- attribute [to_order_dual] semilattice_inf.inf_le_left
-- attribute [to_order_dual] semilattice_inf.inf_le_right
-- attribute [to_order_dual] semilattice_inf.le_inf

-- attribute [to_order_dual_reorder 10] le_inf
attribute [to_order_dual] le_inf

/- the following types are supported by `@[to_order_dual]` and mapped to themselves. -/
attribute [to_order_dual empty] empty
attribute [to_order_dual pempty] pempty
attribute [to_order_dual punit] punit
attribute [to_order_dual unit] unit
