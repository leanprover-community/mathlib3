...
begin
  ...
  let π' := λ i, π i ∪ {set.univ},
  have h_subset : ∀ i, π i ⊆ π' i := λ i, set.subset_union_left (π i) _,
  have h_pi' : ∀ n, is_pi_system (π' n) := λ n, (h_pi n).union_univ,
  have h_univ' : ∀ i, set.univ ∈ π' i,
    from λ i, set.mem_union_right _ (set.mem_singleton _),
  have h_gen' : ∀ i, m i = generate_from (π' i),
  { intros i,
    rw [h_generate i, generate_from_union_univ (π i)], },
