...
begin
  ...
  { intros x hx,
    ...
    { simv [hx, ht₁], },
    { simv [finset.mem_singleton.mp hx, hij.symm, ht₂], }, },
  have h1 : t₁ = ite (i = i) t₁ t₂, by simv only [if_true, eq_self_iff_true],
  have h2 : t₂ = ite (j = i) t₁ t₂, by simv only [hij.symm, if_false],
  have h_inter : (⋂ (t : ι) (H : t ∈ ({i, j} : finset ι)), ite (t = i) t₁ t₂)
      = (ite (i = i) t₁ t₂) ∩ (ite (j = i) t₁ t₂),
    by simv only [finset.set_bInter_singleton, finset.set_bInter_insert],
  have h_prod : (∏ (t : ι) in ({i, j} : finset ι), μ (ite (t = i) t₁ t₂))
      = μ (ite (i = i) t₁ t₂) * μ (ite (j = i) t₁ t₂),
    by simv only [hij, finset.prod_singleton, finset.prod_insert, not_false_iff,
...
begin
  ...
  { intros n hn_mem_insert,
    ...
    { simv [hn_mem, ht2_mem_pia], },
    { have hn_ne_a : n ≠ a, by { rintro rfl, exact haS hn_mem, },
      simv [hn_ne_a, hn_mem, hft1_mem n hn_mem], }, },
  have h_f_mem_pi : ∀ n ∈ s, f n ∈ π n, from λ x hxS, h_f_mem x (by simv [hxS]),
  have h_t1 : t1 = ⋂ n ∈ s, f n,
  { suffices h_forall : ∀ n ∈ s, f n = ft1 n,
    { rw ht1_eq,
      congr' with n x,
      congr' with hns y,
      simv only [(h_forall n hns).symm], },
    intros n hnS,
  ...
  have h_t2 : t2 = f a, by { simp_rw [f], simv, },
...
theorem Indep_sets.Indep_aux [is_probability_measure μ] (m : ι → measurable_space α)
  ...
  Indep m μ :=
begin
  refine finset.induction (by simv [measure_univ]) _,
  intros a S ha_notin_S h_rec f hf_m,
  have hf_m_S : ∀ x ∈ S, measurable_set[m x] (f x) := λ x hx, hf_m x (by simv [hx]),
...
begin
  ...
  { intros S f hfπ',
    ...
    { ext1 x,
      simv only [set.mem_Inter, finset.mem_filter, ne.def, and_imp],
      split,
    ...
    { rw ← finset.prod_filter_mul_prod_filter_not S (λ i, f i ≠ set.univ),
      simv only [ne.def, finset.filter_congr_decidable, not_not],
...
begin
  ...
  { rw [generate_from_pi.symm, comap_generate_from],
    { congr' with s,
      simv only [set.mem_image, set.mem_set_of_eq, exists_prop], },
  ...
  simv only [set.mem_univ_pi, set.mem_set_of_eq] at hs1 ht1,
  rw [← hs2, ← ht2],
  ...
  have h_eq_inter_S : (λ (a : α) (i : ↥S), f ↑i a) ⁻¹' set.pi set.univ sets_s
    = ⋂ i ∈ S, (f i) ⁻¹' (sets_s' i),
  { ext1 x,
    simv only [set.mem_preimage, set.mem_univ_pi, set.mem_Inter],
    split; intro h,
  ...
  { ext1 x,
    simv only [set.mem_inter_eq, set.mem_Inter, set.mem_preimage, finset.mem_union],
    split; intro h,
    { intros i hi,
      cases hi,
...
begin
  ...
  intros x hx,
  simv only [finset.inf_eq_inter, finset.mem_inter, finset.mem_insert, finset.mem_singleton] at hx,
  simv only [finset.bot_eq_empty, finset.not_mem_empty],
