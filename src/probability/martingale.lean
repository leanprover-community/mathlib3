...
begin
  ...
  { intro n,
    ...
    exact hint.snorm_one_condexp_le_snorm (hℱ n) },
  ...
  exact (integrable.snorm_one_condexp_le_snorm (hint.indicator (hmeas n C)) (hℱ n)),
end

/-- Given a integrable function `g`, the conditional expectations of `g` with respect to a
filtration is uniformly integrable. -/
lemma mem_ℒp.uniform_integrable_condexp_filtration [is_finite_measure μ]
  {g : α → ℝ} (hg : mem_ℒp g 1 μ) :
  uniform_integrable (λ i, μ[g | ℱ i]) 1 μ :=
hg.uniform_integrable_condexp ℱ.le
