...
lemma Indep_fun.prod_mk [is_probability_measure μ] {X : ι → Ω → ℝ} (h_meas : ∀ i, measurable (X i))
...
begin
  ...
  have h_meas_left : measurable (λ p : (Π l : s, ℝ),
        (p ⟨i, mem_insert_self i _⟩, p ⟨j, mem_insert_of_mem (mem_singleton_self _)⟩)),
    from measurable.prod (measurable_pi_apply ⟨i, mem_insert_self i {j}⟩)
...
lemma Indep_fun.add [is_probability_measure μ] {X : ι → Ω → ℝ} (h_meas : ∀ i, measurable (X i))
...
lemma Indep_fun.indep_fun_finset_of_not_mem [is_probability_measure μ]
