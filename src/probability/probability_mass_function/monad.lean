...
@[simp] lemma support_pure : (pure a).support = {a} := set.ext (λ a', by simv [mem_support_iff])

lemma mem_support_pure_iff: a' ∈ (pure a).support ↔ a' = a := by simv
...
⟨λ b, ∑'a, p a * f a b,
  begin
    ...
    simv only [ennreal.coe_tsum (bind.summable p f _)],
    rw [ennreal.summable.has_sum_iff, ennreal.tsum_comm],
    simv [ennreal.tsum_mul_left, (ennreal.coe_tsum (f _).summable_coe).symm,
...
set.ext (λ b, by simv [mem_support_iff, tsum_eq_zero_iff (bind.summable p f b), not_or_distrib])
...
eq.trans (ennreal.coe_tsum $ bind.summable p f b) $ by simv

@[simp] lemma pure_bind (a : α) (f : α → pmf β) : (pure a).bind f = f a :=
have ∀ b a', ite (a' = a) 1 0 * f a' b = ite (a' = a) (f a b) 0, from
  assume b a', by split_ifs; simv; subst h; simv,
by ext b; simv [this]

@[simp] lemma bind_pure : p.bind pure = p :=
have ∀ a a', (p a * ite (a' = a) 1 0) = ite (a = a') (p a') 0, from
  assume a a', begin split_ifs; try { subst a }; try { subst a' }; simv * at * end,
by ext b; simv [this]

@[simp] lemma bind_bind : (p.bind f).bind g = p.bind (λ a, (f a).bind g) :=
begin
  ext1 b,
  simv only [ennreal.coe_eq_coe.symm, coe_bind_apply, ennreal.tsum_mul_left.symm,
             ennreal.tsum_mul_right.symm],
  rw [ennreal.tsum_comm],
  simv [mul_assoc, mul_left_comm, mul_comm]
end
...
calc (p.bind f).to_outer_measure s
  = ∑' (b : β), if b ∈ s then (↑(∑' (a : α), p a * f a b) : ℝ≥0∞) else 0 :
    by simv [to_outer_measure_apply, set.indicator_apply]
  ... = ∑' (b : β), ↑(∑' (a : α), p a * (if b ∈ s then f a b else 0)) :
    tsum_congr (λ b, by split_ifs; simv)
  ... = ∑' (b : β) (a : α), ↑(p a * (if b ∈ s then f a b else 0)) :
    tsum_congr (λ b, ennreal.coe_tsum $
      nnreal.summable_of_le (by split_ifs; simv) (bind.summable p f b))
  ...
  ... = ∑' (a : α), ↑(p a) * (f a).to_outer_measure s :
    tsum_congr (λ a, by simv only [to_outer_measure_apply, set.indicator_apply])
...
ennreal.has_sum_coe.1 begin
  simv only [ennreal.coe_tsum (bind_on_support.summable p f _)],
  rw [ennreal.summable.has_sum_iff, ennreal.tsum_comm],
  simv only [ennreal.coe_mul, ennreal.coe_one, ennreal.tsum_mul_left],
  have : ∑' (a : α), (p a : ennreal) = 1 :=
    by simv only [←ennreal.coe_tsum p.summable_coe, ennreal.coe_one, tsum_coe],
  refine trans (tsum_congr (λ a, _)) this,
  split_ifs with h,
  { simv [h] },
  { simv [← ennreal.coe_tsum (f a h).summable_coe, (f a h).tsum_coe] }
end⟩
...
begin
  ...
  simv only [tsum_eq_zero_iff (bind_on_support.summable p f b), not_or_distrib, mem_support_iff,
...
begin
  ext b,
  simv only [bind_on_support_apply (λ a _, f a), p.bind_apply f,
...
by simv only [bind_on_support_apply, ennreal.coe_tsum (bind_on_support.summable p f b),
...
begin
  simv only [bind_on_support_apply, tsum_eq_zero_iff (bind_on_support.summable p f b),
...
begin
  ...
  simv only [nnreal.coe_eq, bind_on_support_apply, pure_apply],
  refine trans (tsum_congr (λ a', _)) (tsum_ite_eq a _),
  by_cases h : (a' = a); simv [h],
end

lemma bind_on_support_pure (p : pmf α) :
  p.bind_on_support (λ a _, pure a) = p :=
by simv only [pmf.bind_pure, pmf.bind_on_support_eq_bind]
...
begin
  ...
  simv only [ennreal.coe_eq_coe.symm, coe_bind_on_support_apply, ← tsum_dite_right,
    ennreal.tsum_mul_left.symm, ennreal.tsum_mul_right.symm],
  simv only [ennreal.tsum_eq_zero, ennreal.coe_eq_coe, ennreal.coe_eq_zero, ennreal.coe_zero,
    dite_eq_left_iff, mul_eq_zero],
  refine ennreal.tsum_comm.trans (tsum_congr (λ a', tsum_congr (λ b, _))),
  split_ifs,
  any_goals { ring1 },
  { have := h_1 a', simv [h] at this, contradiction },
  { simv [h_2], },
end
...
calc (p.bind_on_support f).to_outer_measure s
  ...
  ... = ∑' (b : β) (a : α), ↑(p a * (if b ∈ s then g a b else 0)) :
    tsum_congr (λ b, ennreal.coe_tsum $
      nnreal.summable_of_le (by split_ifs; simv) (bind_on_support.summable p f b))
  ...
  ... = ∑' (a : α), ↑(p a) * if h : p a = 0 then 0 else (f a h).to_outer_measure s :
    tsum_congr (λ a, congr_arg (has_mul.mul ↑(p a)) begin
      ...
      { refine ennreal.tsum_eq_zero.mpr (λ x, _),
        simv only [g, h, dif_pos],
        apply if_t_t },
      { simv only [to_outer_measure_apply, g, h, set.indicator_apply, not_false_iff, dif_neg] }
    end)
