...
begin
  ...
  have h_f1_mem : ∀ n ∈ insert a S, f1 n ∈ π n,
  { intros n hn_mem,
    simp_rw [f1],
  ...
  have h_f1_mem_pi : ∀ n ∈ S, f1 n ∈ π n, from λ x hxS, h_f1_mem x (by simp [hxS]),
  have h_t1 : t1 = ⋂ n ∈ S, f1 n,
  { suffices h_forall : ∀ n ∈ S, f1 n = ft1 n,
    { rw ht1_eq,
      congr' with n x,
      congr' with _,
      { rw hs_mem, },
    ...
    simp_rw [f1],
    simp [hnS, hn_ne_a], },
  have h_μ_t1 : μ t1 = ∏ n in S, μ (f1 n), by rw [h_t1, ←hp_ind S h_f1_mem_pi],
  have h_t2 : t2 = f1 a, by { simp_rw [f1], simp, },
  have h_μ_inter : μ (t1 ∩ t2) = ∏ n in insert a S, μ (f1 n),
  { have h_t1_inter_t2 : t1 ∩ t2 = ⋂ n ∈ insert a S, f1 n,
