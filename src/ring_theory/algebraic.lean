...
See the `instance_diamonds` test for details. -/
def polynomial.has_op_smul_pi [semiring R'] [has_smul R'ᵐᵒᵖ S'] :
  has_smul (R'[X]ᵐᵒᵖ) (R' → S') :=
⟨λ p f x, op (eval x p.unop) • f x⟩

/-- This is not an instance as it forms a diamond with `pi.has_smul`.

See the `instance_diamonds` test for details. -/
noncomputable def polynomial.has_op_smul_pi' [comm_semiring R'] [semiring S'] [algebra R' S']
  [has_smul S'ᵐᵒᵖ T'] :
  has_smul (R'[X]ᵐᵒᵖ) (S' → T') :=
...
{ to_fun := λ p z, algebra_map S' T' (aeval z p),
  ...
  op_smul_def' := λ p f, funext $ λ z, by
    simp [algebra.algebra_map_eq_smul_one, op_smul_eq_smul],
