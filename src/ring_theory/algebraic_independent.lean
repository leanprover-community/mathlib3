...
let e : mv_polynomial (option ι) R ≃+* polynomial (adjoin R (set.range x)) :=
  (mv_polynomial.option_equiv_left _ _).to_ring_equiv.trans
    (polynomial.map_equiv hx.aeval_equiv.to_ring_equiv) in
have ring_hom.comp (polynomial.aeval a :
      polynomial (adjoin R (set.range x)) →ₐ[_] A).to_ring_hom
    e.to_ring_hom = (mv_polynomial.aeval (λ o : option ι, o.elim a x)).to_ring_hom,
  begin
    ...
    dsimp only [e, ring_equiv.to_ring_hom_eq_coe,
      ring_equiv.to_ring_hom_trans, alg_hom.to_ring_hom_eq_coe],
