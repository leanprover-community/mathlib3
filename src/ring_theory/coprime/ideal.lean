...
begin
  ...
  { simv only [finset.sum_singleton, finset.coe_singleton, set.pairwise_singleton, iff_true],
    refine λ a, ⟨λ i, if h : i = a then ⟨1, _⟩ else 0, _⟩,
    { rw h, simv only [finset.mem_singleton, ne.def, infi_infi_eq_left, eq_self_iff_true,
        not_true, infi_false]},
    { simv only [dif_pos, dif_ctx_congr, submodule.coe_mk, eq_self_iff_true] } },
  intros a t hat h ih,
  rw [finset.coe_cons,
  ...
  { rintro ⟨μ, hμ⟩, rw finset.sum_cons at hμ,
    ...
    { have := submodule.coe_mem (μ a), rw mem_infi at this ⊢,
      --for some reason `simv only [mem_infi]` times out
    ...
    { have := submodule.coe_mem (μ i), simv only [mem_infi] at this ⊢,
    ...
    { rw [eq_top_iff_one, submodule.mem_sup],
      ...
      { refine sum_mem _ (λ x hx, _),
        have := submodule.coe_mem (μ x), simv only [mem_infi] at this,
        apply this _ (finset.mem_cons_self _ _), rintro rfl, exact hat hx },
      { have := submodule.coe_mem (μ a), simv only [mem_infi] at this,
