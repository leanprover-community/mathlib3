...
begin
  ...
  { suffices h_Iincl_f : (↑M : fractional_ideal f) ≤ (↑I : fractional_ideal f),
    { intros x hx,
      ...
      { simp only [exists_prop, fractional_ideal.mem_coe], use x, exact ⟨hx, defy⟩ },
      replace hy : y ∈ (↑I : fractional_ideal f),
      apply fractional_ideal.le_iff.mp h_Iincl_f, assumption,
      obtain ⟨a, ⟨ ha, hfa ⟩ ⟩ : ∃ (x' ∈ I), f.to_map x' = y,
      apply fractional_ideal.mem_coe.mp hy,
      have hax : a = x,
  ...
  { replace hx : ∃ x' ∈ (1 : ideal R),  f.to_map x' = x,
    apply fractional_ideal.mem_coe.mp hx,
    rw h_top, simp only [submodule.mem_top, fractional_ideal.mem_coe, exists_prop_of_true],
    rcases hx with ⟨a, ⟨ha,hfa⟩⟩,
    use a, exact hfa },
  rw ← hI, exact h_okfI,
end
