...
begin
  ...
  { refine ⟨ p^(k - n), not_is_unit_of_not_is_unit_dvd hp.not_unit
      (dvd_pow (dvd_refl p) (ne_of_gt (nat.sub_pos_of_lt hlt))), _⟩,
...
lemma comap_map_map_ne_bot_of_lt  (f : T ⧸ I ≃+* S ⧸ J) {p : ideal T} (hJ : J ≠ ⊥) :
 (comap J^.quotient.mk (map (f : T ⧸ I →+* S ⧸ J) (map I^.quotient.mk p))) ≠ ⊥ :=
begin
...
lemma comap_map_map_mem_normalized_factors_of_mem_normalized_factors (hI : I ≠ ⊥) (hJ : J ≠ ⊥)
  (f : T ⧸ I ≃+* S ⧸ J) {p : ideal T} (hp : p ∈ normalized_factors I) :
...
def ideal_quotient_equiv : ideal (T ⧸ I) ≃o {p : ideal T // I ≤ p} :=
...
def ideal_quotient_equiv_divisors : ideal (T ⧸ I) ≃o {p : ideal T // p ∣ I} :=
...
@[simps]
def ideal_correspondence (hI : I ≠ ⊥) (hJ : J ≠ ⊥) (f : T ⧸ I ≃+* S ⧸ J) :
...
lemma shifted_seq_pow_constant_iff_ideal_correspondence_shifted_eq_pow_constant (hI : I ≠ ⊥)
  (hJ : J ≠ ⊥) (f : T ⧸ I ≃+* S ⧸ J) {p : ideal T} (hp : p ∈ normalized_factors I)
...
begin
  ...
  { rw [shifted_seq_pow_constant, shifted_seq_pow_constant],
    intros hn m hm,
    specialize hn m hm,
    apply_fun map (f : T ⧸ I →+* S ⧸ J) at hn,
    rw [ideal_correspondence_apply_coe,
        map_comap_of_surjective J^.quotient.mk quotient.mk_surjective,
        subtype.coe_mk, ← ideal.map_pow, hn, ideal.map_pow] },
  { rw [shifted_seq_pow_constant, shifted_seq_pow_constant],
    intros hn m hm,
    specialize hn m hm,
    rw [ideal_correspondence_apply_coe,
        map_comap_of_surjective J^.quotient.mk quotient.mk_surjective, ← ideal.map_pow,
        ← ideal.map_pow _ _ m]
      at hn,
...
lemma irreducible_ideal_correspondence (hI : I ≠ 0) (hJ : J ≠ 0) (f : T ⧸ I ≃+* S ⧸ J)
...
theorem multiplicity_eq_of_quot_equiv (hI : I ≠ 0) (hJ : J ≠ 0) (f : T ⧸ I ≃+* S ⧸ J)
  {p : ideal T} (hp : p ∈ normalized_factors I):
