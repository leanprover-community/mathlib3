...
begin
  ...
  let ϖ := classical.some (exists_irreducible R),
  have hϖ : ϖ ≠ 0, sorry,
  obtain ⟨π, hup⟩ : is_unit (f.to_map ϖ ^ v), sorry,
  -- have hup' : ↑π⁻¹ = (f.to_map ϖ)^(-v), sorry,
  obtain ⟨εᵣ, hr⟩ : ∃ (εᵣ : units R), r = εᵣ * ϖ^v, sorry,--veramente utile?
  obtain ⟨εₜ, ht⟩ : ∃ (εₜ : units R), t = εₜ * ϖ^e, sorry,
  obtain ⟨ηᵣ, hur⟩ : is_unit (f.to_map ↑εᵣ), sorry,
  have hur' : ↑ηᵣ⁻¹ = (f.to_map ↑εᵣ⁻¹), sorry,
  use f.to_map (ϖ^(e * n - v) * ↑εₜ^n * ↑εᵣ⁻¹),
  simp_rw ht at hn,
  rw [hr, hn] at h,
