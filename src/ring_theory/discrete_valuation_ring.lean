...
begin
  ...
  { intros q hq,
    ...
    intros a b h,
    by_cases ha : a = 0,
    { rw ha, simv only [true_or, dvd_zero], },
    ...
    by_cases hm : m = 0,
    { simv only [hm, one_mul, pow_zero] at h ⊢, right, exact h },
    left,
...
begin
  ...
  simv only [true_and, and_imp, multiset.card_map, eq_self_iff_true,
...
begin
  constructor,
  intro I,
  by_cases I0 : I = ⊥, { rw I0, use 0, simv only [set.singleton_zero, submodule.span_zero], },
  ...
  { intros r hr,
    by_cases hr0 : r = 0,
    { simv only [hr0, submodule.zero_mem], },
    obtain ⟨n, u, rfl⟩ := H hr0,
    simv only [mem_span_singleton, units.is_unit, is_unit.dvd_mul_right],
    apply pow_dvd_pow,
    apply nat.find_min',
...
{ haus' := λ x hx,
  begin
    ...
    simv only [← ideal.one_eq_top, smul_eq_mul, mul_one, smodeq.zero,
