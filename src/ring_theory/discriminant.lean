...
begin
  ...
  { simp only [finset.mem_mk, mem_sigma],
    rw [multiset.mem_erase_of_ne (λ h, _)],
    { exact hroots _ },
    { simp only [mem_singleton, ne.def, mem_sigma] at hi,
  ...
  { simp only [ finset.mem_mk, mem_sigma] at hσ,
    simp only [sigma.exists, exists_prop, mem_singleton, ne.def, mem_sigma],
