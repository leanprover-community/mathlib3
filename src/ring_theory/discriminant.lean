...
## Main definition
* `discriminant A b` : the discriminant of `b : ι → B`.

## Main results
* `zero_of_not_linear_independent` : if `b` is not linear independent, then
  `discriminant A b = 0`.
* `of_matrix_vec_mul` and `of_matrix_mul_vec` : formulas relating `discriminant A ι b` with
  `discriminant A ((P.map (algebra_map A B)).vec_mul b)` and
  `discriminant A ((P.map (algebra_map A B)).mul_vec b)`.
* `not_zero_of_linear_independent` : over a field, if b` is linear independent, then
  `discriminant K b ≠ 0`.
* `eq_det_embeddings_matrix_reindex_pow_two` if `L/K` is a fields extension and `b : ι → L`, then
  `discriminant K b` is the square of the determinant of the matrix whose `(i, j)` coefficient is
...
* `of_power_basis_eq_prod` : the discriminant of a power basis.
