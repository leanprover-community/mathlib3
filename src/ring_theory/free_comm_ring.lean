...
import group_theory.free_comm_monoid
...
free_abelian_group.of $ free_comm_monoid.of x
...
free_abelian_group.induction_on z
  ...
  (λ m, free_comm_monoid.induction_on m h1 $ λ a m ih, hm _ _ (hb a) ih)
  (λ m ih, hn _ ih)
  ha
...
free_comm_monoid.lift.trans free_abelian_group.ring_lift

@[simp] lemma lift_eval_of (x : α) : lift f (of x) = f x :=
(free_abelian_group.lift_eval_of _ _).trans $ mul_one _
