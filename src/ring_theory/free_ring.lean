...
/-- The bare function `free_ring α → R` induced from a map `α → R`. For the ring homomorphism,
    see `lift_hom`. -/
...
/-- Lift of a map `f : α → R` to `free_ring α` as a ring homomorphism.
We don't use it as the canonical form because Lean fails to coerce it to a function. -/
def lift_hom : free_ring α →+* R :=
...
/-- The canonical map `free_ring α → free_ring β` generated by a map `α → β`, as a bare map.
    For the ring homomorphism, see `map_hom`. -/
