...
lemma graded_algebra.nsmul_mem (n : ℕ) {i : ι} {a : A} (ha : a ∈ 𝒜 i) : n • a ∈ 𝒜 i :=
begin
  induction n with n ih,
  { simp, },
  { rw [succ_nsmul],
    exact submodule.add_mem _ ha ih, }
end
...
instance graded_algebra.grade_zero_semiring (𝒜 : ι → submodule R A) [graded_algebra 𝒜] :
  semiring (𝒜 0) :=
{ add := (+),
  add_assoc := add_assoc,
  zero := 0,
  zero_add := zero_add,
  add_zero := add_zero,
  nsmul := λ n x, ⟨n • x.1, graded_algebra.nsmul_mem 𝒜 n x.2⟩,
  nsmul_zero' := λ x, by { simp only [subtype.ext_iff_val, zero_nsmul], refl, },
  nsmul_succ' := λ n x, by { simp only [subtype.ext_iff_val, succ_nsmul], refl, },
  add_comm := add_comm,
  mul := λ x y, ⟨x.1 * y.1, by { convert set_like.graded_monoid.mul_mem x.2 y.2, rw zero_add, }⟩,
  left_distrib := λ x y z, by { rw [subtype.ext_iff_val], dsimp, rw mul_add },
  right_distrib := λ x y z, by { rw [subtype.ext_iff_val], dsimp, rw add_mul },
  zero_mul := λ x, by { rw [subtype.ext_iff_val], dsimp, rw zero_mul },
  mul_zero := λ x, by { rw [subtype.ext_iff_val], dsimp, rw mul_zero },
  mul_assoc := λ x y z, by { rw [subtype.ext_iff_val], dsimp, rw mul_assoc },
  one := ⟨1, set_like.graded_monoid.one_mem⟩,
  one_mul := λ x, by { rw [subtype.ext_iff_val], dsimp, rw one_mul },
  mul_one := λ x, by { rw [subtype.ext_iff_val], dsimp, rw mul_one },
  npow := λ n x, ⟨x.1^n, begin
    induction n with n ih,
    { rw [pow_zero],
      exact set_like.graded_monoid.one_mem },
    { rw [pow_succ],
      convert set_like.graded_monoid.mul_mem x.2 ih,
      rw zero_add }
  end⟩,
  npow_zero' := λ x, by { rw [subtype.ext_iff_val], dsimp, rw pow_zero, refl },
  npow_succ' := λ n x, by { rw [subtype.ext_iff_val], dsimp, rw pow_succ, refl }, }
