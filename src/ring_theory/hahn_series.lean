...
def support (x : hahn_series Γ R) : set Γ := {a | x.coeff a ≠ 0}
...
@[simp]
lemma support_zero : support (0 : hahn_series Γ R) = ∅ := by { ext, simp }
...
begin
  ext b,
  simp only [set.mem_singleton_iff, ne.def, mem_support],
  split,
  { contrapose!,
