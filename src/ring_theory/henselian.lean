...
begin
  ...
  { intros H, constructor, intros f hf a₀ h₁ h₂,
    ...
    simv only [aeval_def, ring_hom.algebra_map_to_algebra, eval₂_at_apply,
  ...
  { introsI hR K _K φ hφ f hf a₀ h₁ h₂,
    ...
    simv only [← ker_eq_maximal_ideal φ hφ, eval₂_at_apply, φ.mem_ker] at H h₁ h₂,
...
{ jac := is_adic_complete.le_jacobson_bot _,
  ...
  begin
    ...
    { intro n,
      ...
      simv only [nat.succ_eq_add_one],
      rw [← taylor_eval_sub (c n), hc],
      simv only [sub_eq_add_neg, add_neg_cancel_comm],
      ...
      { simv only [finset.sum_range_succ, taylor_coeff_one, mul_one, pow_one, taylor_coeff_zero,
      ...
      { refine submodule.sum_mem _ _, simv only [finset.mem_Ico],
        rintro i ⟨h2i, hi⟩,
