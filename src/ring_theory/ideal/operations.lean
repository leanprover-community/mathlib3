...
section quotient_module

variables {R S : Type*} [comm_ring R] [comm_ring S] (f : R →+* S) (I : ideal R)
variables (M : Type*) [add_comm_group M] [module R M]

/-- If an ideal `I` annihilates an `R`-module `M`, there is a `R/I`-module structure on `M`. -/
...
instance ideal.quotient_is_scalar_tower_of_le_ann (h : I ≤ (⊤ : submodule R M).annihilator) :
  @@is_scalar_tower R (R ⧸ I) M _ (ideal.quotient_module_of_le_ann I M h).to_has_smul _ :=
{ smul_assoc := λ r y m, quotient.induction_on' y $ λ y, mul_smul _ _ _ }

instance : module (R ⧸ I) (M ⧸ I • (⊤ : submodule R M)) :=
ideal.quotient_module_of_le_ann I _ (λ r hr, begin
  ext ⟨x, _⟩,
  dsimp, clear_,
