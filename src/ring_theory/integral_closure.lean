...
begin
  ...
  { rcases this with ⟨p, hpm, hpx⟩,
    ...
    simv only [aeval_def, eval₂, sum_def],
    rw S.val.map_sum,
...
begin
  ...
  rw [← hy, ← set.image_id ↑y] at this, simv only [finsupp.mem_span_image_iff_total] at this,
  -- Say `yᵢyⱼ = ∑rᵢⱼₖ yₖ`
  choose ly hly1 hly2,
  ...
  refine is_integral_of_submodule_noetherian (algebra.adjoin S₀ ↑y)
    (is_noetherian_of_fg_of_noetherian _ ⟨insert 1 y,
      by { rw [finset.coe_insert], ext z, simv [S₁], convert foo z}⟩) _ _,
...
begin
  ...
  simv only [set_like.mem_coe, set.singleton_subset_iff],
  exact hx
end
...
by simv only [←le_integral_closure_iff_is_integral, sup_le_iff]
...
begin
  simv only [normalize_scale_roots, finset_sum_coeff, coeff_monomial, finset.sum_ite_eq', one_mul,
...
begin
  ext,
  simv only [coeff_scale_roots, normalize_scale_roots, coeff_monomial, coeff_smul, finset.smul_sum,
...
begin
  intro x,
  contrapose,
  simv only [not_mem_support_iff, normalize_scale_roots, finset_sum_coeff, coeff_monomial,
...
algebra_map_injective A R B $ by simv only [algebra_map_mk', ring_hom.map_add]

@[simp] lemma mk'_mul (x y : B) (hx : is_integral R x) (hy : is_integral R y) :
  mk' A (x * y) (is_integral_mul hx hy) = mk' A x hx * mk' A y hy :=
algebra_map_injective A R B $ by simv only [algebra_map_mk', ring_hom.map_mul]
...
{ to_fun := λ x, mk' A (algebra_map S B x) (is_integral.algebra_map (h x)),
  map_one' := by simv only [ring_hom.map_one, mk'_one],
  map_zero' := by simv only [ring_hom.map_zero, mk'_zero],
...
alg_equiv.of_alg_hom (lift _ B (is_integral_algebra R B)) (lift _ B (is_integral_algebra R B))
  (by { ext x, apply algebra_map_injective A' R B, simv })
  (by { ext x, apply algebra_map_injective A R B, simv })
