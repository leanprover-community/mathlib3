...
begin
  ...
  { haveI : nontrivial (comap (algebra_map R S) P).quotient := quotient.nontrivial hP_top,
    ...
    erw [eq_bot_iff, ← jacobson_eq_iff_jacobson_quotient_eq_bot.1 ((is_jacobson_iff_prime_eq.1 hR)
      (comap (algebra_map R S) P) (comap_is_prime _ _)), comap_Inf'],
    refine Inf_le_Inf (λ J hJ, ⟨J, _⟩),
    haveI : J.is_maximal := by simpa using hJ,
    convert infi_const,
    simp only [true_and, set.mem_image, nonempty_Prop, bot_le, set.mem_set_of_eq],
