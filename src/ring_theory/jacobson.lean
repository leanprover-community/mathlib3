...
begin
  ...
  rintros x ⟨hx, hx'⟩,
  cases submonoid.mem_closure_singleton.1 hx with n hn,
  rw [← hn, ← hI] at hx',
  refine absurd (hI ▸ mem_radical_of_pow_mem hx' : y ∈ I.carrier) h,
end

/-- The submonoid generated by the single element `y` -/
local notation `M` y := (submonoid.closure {y} : submonoid R)
...
begin
  ...
  have hP : P.jacobson = P := H (comap f.to_map P') (is_prime.radical hP'),
  ...
  { intros x hx,
    ...
    { refine this ⟨((mul_comm y x) ▸ I₁.smul_mem y hx : x * y ∈ I₁), I₂.smul_mem x _⟩,
      rw mem_Inf,
      rintros J ⟨hJ_max, hJ⟩,
    ...
    { exact hxy },
    { exfalso,
      refine hPM ⟨submonoid.mem_closure_singleton.2 ⟨1, pow_one y⟩, hxy⟩ } },
  refine le_trans _ this,
  rw [ideal.jacobson, comap_Inf', Inf_eq_infi],
  refine infi_le_infi_of_subset (λ I hI, _),
  simp,
  ...
  { exact localization_map.comap_map_of_is_prime_disjoint f I
      (is_maximal.is_prime hI.2.1) hI.2.2 }
end
