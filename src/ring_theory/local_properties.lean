...
lemma localization_is_reduced [is_localization M R'] [hR : is_reduced R] : is_reduced R' :=
begin
  constructor,
  rintro x ⟨(_|n), e⟩,
  ...
  rw [← (algebra_map R R').map_zero] at hx',
  obtain ⟨m', hm'⟩ := (is_localization.eq_iff_exists M R').mp hx',
  apply_fun (*m'^n) at hm',
  simp only [mul_assoc, zero_mul] at hm',
  rw [mul_comm, ← pow_succ, ← mul_pow] at hm',
  replace hm' := is_nilpotent.eq_zero ⟨_, hm'.symm⟩,
  rw [← (is_localization.map_units R' m).mul_left_inj, hx, zero_mul,
    is_localization.map_eq_zero_iff M],
  exact ⟨m', by rw [← hm', mul_comm]⟩
end

instance [is_reduced R] : is_reduced (localization M) := localization_is_reduced M _

lemma is_reduced_of_localization (h : ∀ (J : ideal R) (hJ : J.is_maximal),
  by exactI is_reduced (localization.at_prime J)) : is_reduced R :=
begin
  constructor,
  intros x hx,
