...
begin
  introv R hf,
  -- Setting up the `algebra` and `smul_assoc_class` instances needed
  resetI,
  letI := f.to_algebra,
  letI := ((algebra_map S S').comp f).to_algebra,
  let f' : R' →+* S' := is_localization.map S' f (submonoid.le_comap_map M),
  letI := f'.to_algebra,
  haveI : smul_assoc_class R R' S' :=
    smul_assoc_class.of_algebra_map_eq' (is_localization.map_comp _).symm,
...
lemma is_localization.smul_mem_finset_integer_multiple_span [algebra R S]
  [algebra R S'] [smul_assoc_class R S S']
...
lemma multiple_mem_span_of_mem_localization_span [algebra R' S] [algebra R S]
  [smul_assoc_class R R' S] [is_localization M R']
...
begin
  ...
  simp_rw [ring_hom.map_mul, ← smul_assoc_class.algebra_map_apply, mul_comm (algebra_map R' S y),
...
begin
  ...
  haveI : ∀ r : s, smul_assoc_class R (localization.away (r : R)) (localization.away (f r)) :=
    λ r, smul_assoc_class.of_algebra_map_eq' (is_localization.map_comp _).symm,
  ...
  rw [submonoid.smul_def, algebra.smul_def, smul_assoc_class.algebra_map_apply R S,
...
lemma is_localization.exists_smul_mem_of_mem_adjoin [algebra R S]
  [algebra R S'] [smul_assoc_class R S S'] (M : submonoid S)
  ...
  (hx : algebra_map S S' x ∈ algebra.adjoin R (s : set S')) :
    ∃ m : M, m • x ∈ A :=
begin
  let g : S →ₐ[R] S' := smul_assoc_class.to_alg_hom R S S',
