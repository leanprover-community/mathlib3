...
lemma mem_coe (I : ideal R) {x : S} :
  x ∈ (I : f.submodule) ↔ ∃ y : R, y ∈ I ∧ f.to_map y = x :=
iff.rfl

theorem mul_mem_mul {A B : f.submodule} {m n} (hm : m ∈ A) (hn : n ∈ B) : m * n ∈ A * B :=
@submodule.mul_mem_mul _ _ _ _ f.to_map.to_algebra _ _ _ _ hm hn

theorem mul_le {A B C : f.submodule} : A * B ≤ C ↔ ∀ (m ∈ A) (n ∈ B), m * n ∈ C :=
@submodule.mul_le _ _ _ _ f.to_map.to_algebra _ _ _

variables {f}
@[elab_as_eliminator] theorem mul_induction_on
  {A B : f.submodule}
  {C : S → Prop} {r : S} (hr : r ∈ A * B)
  (hm : ∀ (m ∈ A) (n ∈ B), C (m * n))
  (h0 : C 0) (ha : ∀ x y, C x → C y → C (x + y))
  (hs : ∀ (r : R) x, C x → C (f.to_map r * x)) : C r :=
@submodule.mul_induction_on _ _ _ _ f.to_map.to_algebra _ _ _ _ hr hm h0 ha hs
