...
begin
  classical, haveI : fintype L := is_localization.fintype' S L,
  intro x, rcases is_localization.surj S x with ⟨⟨r,s⟩,h⟩,
  rcases (is_of_fin_order_iff_pow_eq_one _).1
    (exists_pow_eq_one (is_localization.map_units L s).unit) with ⟨n,hn,hp⟩,
