...
| n m := λ a b ha hb ⟨s, hs⟩,
  ...
  (hp.2.2 a b this).elim
    (λ ⟨x, hx⟩, have hn0 : 0 < n,
      ...
      have hpx : ¬ p ^ (n - 1 + 1) ∣ x,
        from λ ⟨y, hy⟩, ha (hx.symm ▸ ⟨y, (mul_left_inj' hp.1).1
      ...
      finite_mul_aux hpx hb ⟨s, (mul_left_inj' hp.1).1 begin
    ...
    (λ ⟨x, hx⟩, have hm0 : 0 < m,
      ...
      have hpx : ¬ p ^ (m - 1 + 1) ∣ x,
        from λ ⟨y, hy⟩, hb (hx.symm ▸ ⟨y, (mul_left_inj' hp.1).1
          $ by rw [nat.sub_add_cancel hm0] at hy;
            simp [hy, _root_.pow_add, mul_comm, mul_assoc, mul_left_comm]⟩),
      finite_mul_aux ha hpx ⟨s, (mul_left_inj' hp.1).1 begin
...
eq_some_iff.2 ⟨by simp, λ ⟨b, hb⟩, ha (is_unit_iff_dvd_one.2
  ⟨b, (mul_right_inj' ha0).1 $ by clear _fun_match;
