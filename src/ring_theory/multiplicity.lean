...
/-- `enat.find` is the least `n : ℕ` such that `P n`, or `⊤` if no `n` satisfy `P`. -/
def enat.find (P : ℕ → Prop) : ℕ∞ :=
if h : ∃ x, P x then nat.find h else ⊤

lemma enat.find_ne_top {P : ℕ → Prop} : enat.find P ≠ ⊤ ↔ ∃ x, P x :=
begin
  unfold enat.find,
  split_ifs with h; simp [h]
end

lemma enat.ext {x y : ℕ∞} (h₁ : x = ⊤ ↔ y = ⊤) (h₂ : ∀ (n : ℕ), x = n ↔ y = n) : x = y :=
_
...
/-- `multiplicity a b` returns the largest natural number `n` such that
  ...
  then it returns `⊤`-/
def multiplicity [monoid α] [decidable_rel ((∣) : α → α → Prop)] (a b : α) : enat :=
...
lemma finite_iff_ne_top [decidable_rel ((∣) : α → α → Prop)] {a b : α} :
  finite a b ↔ (multiplicity a b) ≠ ⊤ :=
enat.find_ne_top.symm

lemma eq_top_iff_not_finite [decidable_rel ((∣) : α → α → Prop)] {a b : α} :
  (multiplicity a b) = ⊤ ↔ ¬ (finite a b) :=
(not_iff_comm.mp (finite_iff_ne_top.symm)).symm
...
by { unfold multiplicity, norm_cast }
...
lemma pow_dvd_of_le_multiplicity {a b : α} {k : ℕ} :
  (k : enat) ≤ multiplicity a b → a ^ k ∣ b :=
nat.cases_on k (λ _, by { rw pow_zero, exact one_dvd _ })
  (λ k h, by_contradiction (λ hk, _))
