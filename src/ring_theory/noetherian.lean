...
begin
  let P := λ J, ∀ (Y : multiset (prime_spectrum R)), ¬ multiset.prod (Y.map (coe : subtype _ → ideal R)) ≤ J,
  let Ω := {J : ideal R | P J },
  ...
  obtain ⟨Wx, h_Wx⟩ : ∃ (Wx : multiset (prime_spectrum R)), multiset.prod (Wx.map (coe : subtype _ → ideal R)) ≤ Jx,
  { have Jx_P : ¬ P Jx, simp [*, mem_set_of_eq, not_forall] at *,
    dsimp [P] at Jx_P, from not_forall_not.mp Jx_P },
  obtain ⟨Wy, h_Wy⟩ : ∃ (Wy : multiset (prime_spectrum R)), multiset.prod (Wy.map (coe : subtype _ → ideal R)) ≤ Jy,
  { have Jy_P : ¬ P Jy, simp [*, mem_set_of_eq, not_forall] at *,
    dsimp [P] at Jy_P, from not_forall_not.mp Jy_P },
  let W := Wx + Wy,
  have h_W : multiset.prod (W.map (coe : subtype _ → ideal R)) ≤ M,
  { suffices h_JJ : multiset.prod (W.map (coe : subtype _ → ideal R)) ≤ Jx * Jy,
    { suffices h_JM : Jx * Jy ≤ M, from le_trans h_JJ h_JM,
      { dsimp only *,
        rw add_mul,
        have this : M * (M + span R {y}) ≤ M, from ideal.mul_le_right,
        apply sup_le this,
        rw mul_add,
        replace this : span R {x} * M ≤ M, from ideal.mul_le_left,
        apply sup_le this,
        rw [span_mul_span, singleton_mul_singleton, span_singleton_le_iff_mem],
        exact h_xy } },
    { rw [multiset.map_add, multiset.prod_add],
      apply submodule.mul_le_mul h_Wx h_Wy }, },
  have h_absM : ¬ P M,
  { dsimp only [P], rw not_forall_not,
    use W, exact h_W },
