...
begin
  ...
  rw [← ht1,← set.image_id ↑t1, mem_span_iff_lc] at this,
  rcases this with ⟨l, hl1, hl2⟩,
  refine mem_sup.2 ⟨lc.total β α β id ((lc.map α g : lc γ α → lc β α) l), _,
    x - lc.total β α β id ((lc.map α g : lc γ α → lc β α) l), _, add_sub_cancel'_right _ _⟩,
  { rw [← set.image_id (g '' ↑t1), mem_span_iff_lc], refine ⟨_, _, rfl⟩,
    haveI : inhabited γ := ⟨0⟩,
...
begin
  ...
  rw [← hs, ← set.image_id ↑s, mem_span_iff_lc] at hn,
