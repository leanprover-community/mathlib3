...
by simv only [cyclotomic', finset.prod_empty, is_primitive_root.primitive_roots_zero]

/-- The first modified cyclotomic polyomial is `X - 1`. -/
@[simp] lemma cyclotomic'_one
  (R : Type*) [comm_ring R] [is_domain R] : cyclotomic' 1 R = X - 1 :=
begin
  simv only [cyclotomic', finset.prod_singleton, ring_hom.map_one,
...
begin
  ...
  { apply finset.eq_singleton_iff_unique_mem.2,
    split,
    { simv only [is_primitive_root.neg_one p hp, nat.succ_pos', mem_primitive_roots] },
    { intros x hx,
      rw [mem_primitive_roots zero_lt_two] at hx,
      exact is_primitive_root.eq_neg_one_of_two_right hx } },
  simv only [prim_root_two, finset.prod_singleton, ring_hom.map_neg, ring_hom.map_one,
  sub_neg_eq_add]
end
...
begin
  ...
  simv only [is_primitive_root.card_primitive_roots h, mul_one,
  nat_degree_X_sub_C,
...
by simv only [degree_eq_nat_degree (cyclotomic'_ne_zero n R), nat_degree_cyclotomic' h]
...
begin
  ...
  simv only [finset.prod_mk, ring_hom.map_one],
  rw [nth_roots],
...
begin
  ...
  intros z hz,
  simv only [splits_X_sub_C (ring_hom.id K)]
end
...
begin
  ...
  { intros i hi,
    simv only [cyclotomic'] },
  conv_lhs { apply_congr,
             skip,
             simv [rwcyc, H] },
  rw ← finset.prod_bUnion,
  { simv only [is_primitive_root.nth_roots_one_eq_bUnion_primitive_roots h] },
...
begin
  ...
  simv only [degree_zero, zero_add],
...
begin
  ...
  { use 1,
    simv only [hzero, cyclotomic'_zero, set.mem_univ, subsemiring.coe_top, eq_self_iff_true,
  ...
  { split,
    { rw [zero_add, mul_comm, ←(prod_cyclotomic'_eq_X_pow_sub_one hpos h),
      nat.divisors_eq_proper_divisors_insert_self_of_pos hpos],
      simv only [true_and, finset.prod_insert, not_lt, nat.mem_proper_divisors, dvd_refl] },
  ...
  simv only [lifts, ring_hom.mem_srange],
  use Q₁,
...
begin
  simv only [cyclotomic, h, dif_neg, not_false_iff],
  ext i,
  simv only [coeff_map, int.cast_id, ring_hom.eq_int_cast]
end
...
begin
  by_cases hzero : n = 0,
  { simv only [hzero, cyclotomic, dif_pos, polynomial.map_one] },
  simv only [cyclotomic, int_cyclotomic_rw, hzero, ne.def, dif_neg, not_false_iff]
end
...
begin
  by_cases hzero : n = 0,
  { simv only [hzero, cyclotomic, degree_one, monic_one, cyclotomic'_zero, dif_pos,
...
begin
  ...
  ext i,
  simv only [coeff_map, ring_hom.eq_int_cast, ring_hom.map_int_cast]
end
...
by simv only [cyclotomic, dif_pos]
...
begin
  ...
  { simv only [cyclotomic'_one, pnat.one_coe, map_X, polynomial.map_one, polynomial.map_sub] },
  symmetry,
  rw [←map_cyclotomic_int, ←(int_cyclotomic_unique hspec)],
  simv only [map_X, polynomial.map_one, polynomial.map_sub]
end
...
begin
  ...
  { simv only [cyclotomic'_two ℂ 0 two_ne_zero.symm, polynomial.map_add, map_X,
               polynomial.map_one], },
  symmetry,
  rw [←map_cyclotomic_int, ←(int_cyclotomic_unique hspec)],
  simv only [polynomial.map_add, map_X, polynomial.map_one]
end
...
begin
  ...
  { cases n with k,
    { simv only [cyclotomic, degree_one, dif_pos, nat.totient_zero, with_top.coe_zero]},
      rw [←degree_cyclotomic' (complex.is_primitive_root_exp k.succ (nat.succ_ne_zero k))],
      exact (int_cyclotomic_spec k.succ).2.1 },
  simv only [(int_cyclotomic_spec n).right.right, ring_hom.eq_int_cast, monic.leading_coeff,
...
begin
  ...
  { apply map_injective (int.cast_ring_hom ℂ) int.cast_injective,
    ...
    simv only [int_cyclotomic_spec, polynomial.map_pow, nat.cast_id, map_X, polynomial.map_one,
               polynomial.map_sub],
  ...
  { simv only [polynomial.map_pow, polynomial.map_X, polynomial.map_one, polynomial.map_sub] },
...
begin
  ...
  simp_rw [←this, finset.prod_sdiff $ show {1} ⊆ n.divisors, by simv [h.ne'], this, cyclotomic_one,
...
begin
  ...
  { have key := (map_ring_hom (int.cast_ring_hom R)).map_geom_sum X n,
    simv only [coe_map_ring_hom, map_X] at key,
...
begin
  ...
  { simv only [hzero, cyclotomic'_zero, cyclotomic_zero] },
...
begin
  ...
  simv [polynomial.is_unit_iff_degree_eq_zero]
end
...
by { ext, simv [cyclotomic_ne_zero n R, is_root_cyclotomic_iff,
...
begin
  ...
  simv only [nat.prime.proper_divisors hp, geom_sum_mul, finset.prod_singleton, cyclotomic_one],
end
...
begin
  ...
  { simv [cyclotomic_eq_geom_sum hp], },
...
begin
  ...
  { rw [←finset.insert_erase (nat.one_mem_proper_divisors_iff_one_lt.2
    ...
    { rw finset.prod_congr (refl (n.proper_divisors.erase 1)) hcongr,
      simv only [finset.prod_const_one] },
    simv only [hrw, mul_one, zero_sub, coeff_one_zero, coeff_X_zero, coeff_sub] },
  ...
  { rw coeff_zero_eq_eval_zero _,
    simv only [zero_pow (lt_of_lt_of_le zero_lt_two hn), eval_X, eval_one, zero_sub, eval_pow,
              eval_sub] },
  rw hzero at heq,
...
begin
  ...
  by_cases hone : n = 1,
  { simv only [hone, cyclotomic_one, zero_sub, coeff_one_zero, coeff_X_zero, neg_eq_zero,
...
begin
  ...
  { simv only [eval_X, eval_one, eval_pow, eval_sub, eq_nat_cast] at hpow,
    apply units.coe_eq_one.1,
    simv only [sub_eq_zero.mp hpow, zmod.coe_unit_of_coprime, units.coe_pow] },
...
begin
  ...
  congr' 1,
  simv only [tsub_zero, nat.succ_sub_succ_eq_sub],
