...
lemma cyclotomic_mul_prime_eq_pow_of_not_dvd (R : Type*) {p n : ℕ} [hp : fact (nat.prime p)] [ring R]
...
/-- If `R` is of characterist `p` and `¬p ∣ m`, then `ζ` is a root of `cyclotomic (p ^ k * m) R` if
