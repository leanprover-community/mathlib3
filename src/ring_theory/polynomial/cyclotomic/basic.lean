...
@[simp]
lemma cyclotomic_epand_eq_cyclotomic_mul {p n : ℕ} (hp : nat.prime p) (hdiv : ¬p ∣ n) (R : Type*)
...
begin
  ...
  { refine (is_primitive.int.dvd_iff_map_cast_dvd_map_cast _ _ (is_primitive.mul
    ...
    { simp only at h,
      nth_rewrite 1 [← mul_one n] at h,
      exact nat.prime.ne_one hp (nat.eq_of_mul_eq_mul_left hnpos h) },
    { have hpos : 0 < n * p := mul_pos hnpos (nat.prime.pos hp),
