...
begin
  ...
  rw [← smul_sum, ← smul_sub, smul_def, algebra_map_apply R K L, _root_.map_mul, map_pow,
    norm_algebra_map, smul_def, _root_.map_mul, algebra_map_apply R K L, norm_algebra_map, ← hr,
    finrank B, power_basis.norm_gen_eq_coeff_zero_minpoly, mul_pow, ← map_pow, ← map_pow,
    minpoly.gcd_domain_eq_field_fractions K hBint, coeff_map, ← map_pow, ← pow_mul,
    show (-1 : K) = algebra_map R K (-1), by simp, ← map_pow, ← _root_.map_mul, ← _root_.map_mul,
    ← _root_.map_mul] at hQ,
  ...
  rw [← is_fraction_ring.injective R K hQ, mul_comm, mul_assoc, ← units.coe_neg_one,
    ← units.coe_pow, is_unit.dvd_mul_left _ _ _ ⟨_, rfl⟩, mul_comm] at hppdiv,
  convert hppdiv,
