...
/- The isomorphism `R[α]/pR[α] ≅ (R/p)[X]/(f mod p)` for `α` a root of `f` -/

def l₁ : (adjoin_root f) ⧸ (p.map (of f)) ≃+*
  (adjoin_root f) ⧸ (p.map (((span ({f} : set (polynomial R)))^.quotient.mk).comp
  (C : R →+* polynomial R))) :=
ideal.quot_equiv_of_eq (by rw [of, adjoin_root.mk])

def l₂ : (adjoin_root f) ⧸(p.map (((span ({f} : set (polynomial R)))^.quotient.mk).comp
  (C : R →+* polynomial R))) ≃+* (adjoin_root f) ⧸
  (((p.map (C : R →+* polynomial R))).map (span ({f} : set (polynomial R)))^.quotient.mk) :=
ideal.quot_equiv_of_eq (by rw ideal.map_map)

def l₃ : (adjoin_root f) ⧸ (((p.map (C : R →+* polynomial R))).map
  (span ({f} : set (polynomial R)))^.quotient.mk) ≃+*
  (polynomial R ⧸ map C p) ⧸ ((span ({f} : set (polynomial R))).map
  (p.map (C : R →+* polynomial R))^.quotient.mk)
:= quot_quot_equiv_comm (span ({f} : set (polynomial R))) (p.map (C : R →+* polynomial R))

def l₄ : (polynomial R ⧸ map C p) ⧸ ((span ({f} : set (polynomial R))).map
  (p.map (C : R →+* polynomial R))^.quotient.mk) ≃+*
...
def l₅ : polynomial (R ⧸ p) ⧸  (span ({f.map (p^.quotient.mk)} : set (polynomial (R ⧸ p)))) ≃+*
  (polynomial R ⧸ map C p) ⧸ (span ({(ideal.quotient.mk
  (p.map polynomial.C)) f} : set (polynomial R ⧸ map C p))) :=
...
lemma quot_adjoin_root_equiv_quot_polynomial_quot_mk_of (x : R) :
  ...
  ideal.quotient.mk (span ({f.map (p^.quotient.mk)} : set (polynomial (R ⧸ p))))
  (C (ideal.quotient.mk p x))
  := sorry
