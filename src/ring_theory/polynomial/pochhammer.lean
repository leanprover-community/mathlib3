...
that is focused on `nat` can be found in `data.nat.factorial` as `asc_fact`.
...
lemma pochhammer_nat_eq_asc_fact (n : ℕ) : ∀ k, (pochhammer ℕ k).eval (n + 1) = nat.asc_fact n k
| 0 := by erw [eval_one]; refl
| (t + 1) := begin
  rw [pochhammer_succ_right, eval_mul, pochhammer_nat_eq_asc_fact t],
  suffices : n.asc_fact t * (n + 1 + t) = n.asc_fact (t + 1), by simpa,
  rw [nat.asc_fact_succ, add_right_comm, mul_comm]
end
...
by rw_mod_cast [pochhammer_nat_eq_asc_fact, nat.zero_asc_fact]

lemma factorial_mul_pochhammer (S : Type*) [semiring S] (r n : ℕ) :
  (r! : S) * (pochhammer S n).eval (r + 1) = (r + n)! :=
by rw_mod_cast [pochhammer_nat_eq_asc_fact, nat.factorial_mul_asc_fact]
...
| (k + 1) := by simp only [pochhammer_nat_eq_asc_fact, nat.succ_asc_fact, add_right_comm]
