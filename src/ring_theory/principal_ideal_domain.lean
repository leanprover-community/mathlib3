...
begin
  ...
  have hSupJ : Sup c = J,
  { exact le_antisymm (by simp [hx, ideal.span_le, hxJ]) (le_Sup hJc) },
  replace hs := hs hJc,
...
begin
  ...
  refine (non_principals_def _).1 Ibad ⟨⟨a * b, le_antisymm (λ i hi, _)
    (ideal.span_le.2 (set.singleton_subset_iff.2 (set_like.mem_coe.2 _)))⟩⟩,
  { have hisup : i ∈ I ⊔ (span {y}) := ideal.mem_sup_left hi,
    have : y ∈ I ⊔ (span {y}) := ideal.mem_sup_right (ideal.mem_span_singleton_self y),
    rw ha at hisup this,
    obtain ⟨v, hv⟩ := mem_span_singleton'.1 this,
    obtain ⟨u, hu⟩ := mem_span_singleton'.1 hisup,
    have hucolon : u ∈ I.colon (span {y}),
    { rw [ideal.mem_colon_singleton, ← hv, mul_comm v, ← mul_assoc, hu],
      exact mul_mem_right _ _ hi },
    rw hb at hucolon,
    obtain ⟨z, hz⟩ := mem_span_singleton'.1 hucolon,
    rw ← hz at hu,
    rw ← hu,
    refine mem_span_singleton'.2 ⟨z, by ring⟩ },
  { have hbI : b ∈ I.colon (span {y}) := (hb.symm ▸ submodule.mem_span_singleton_self b),
    rw [ideal.mem_colon_singleton] at hbI,
    suffices : span {a * b} ≤ I,
    { refine this (ideal.mem_span_singleton_self _) },
      rw [← span_singleton_mul_span_singleton,
          show span {a} = I ⊔ span {y}, from ha.symm, ideal.sup_mul, sup_le_iff],
      refine ⟨mul_le_right, _⟩,
      rw [span_singleton_mul_span_singleton, mul_comm, ideal.span_singleton_le_iff_mem],
      exact hbI },
end
