...
instance : is_bundled_set (subsemiring R) :=
{ mem_type := R,
  set_coe' := subsemiring.carrier,
...
@[ext] theorem ext {S T : subsemiring R} (h : ∀ x, x ∈ S ↔ x ∈ T) : S = T :=
is_bundled_set.ext h
...
is_bundled_set.set_coe_inj' $ set.image_image _ _ _
...
{ bot := (⊥),
  ...
  .. complete_lattice_of_Inf (subsemiring R)
    (λ s, is_glb.of_image (λ s t, show (s : set R) ≤ t ↔ s ≤ t, by {
      convert is_bundled_set.coe_subset_coe, refl }) is_glb_binfi)}
...
lemma mem_supr_of_directed {ι} [hι : nonempty ι] {S : ι → subsemiring R} (hS : directed (≤) S)
  ...
  x ∈ (⨆ i, S i) ↔ ∃ i, x ∈ S i :=
begin
  refine ⟨_, λ ⟨i, hi⟩, (is_bundled_set.le_def.1 $ le_supr S i) hi⟩,
...
set.ext $ λ x, by { erw is_bundled_set.mem_coe, simp [mem_supr_of_directed hS], refl }
...
lemma coe_Sup_of_directed_on {S : set (subsemiring R)} (Sne : S.nonempty) (hS : directed_on (≤) S) :
  (↑(Sup S) : set R) = ⋃ s ∈ S, ↑s :=
set.ext $ λ x, by { erw is_bundled_set.mem_coe, simp [mem_Sup_of_directed_on Sne hS], refl}
...
is_bundled_set.ext'_iff.trans $ iff.trans (by rw [coe_srange, coe_top]) set.range_iff_surjective
...
closure_le.2 $ λ x hx, is_bundled_set.mem_coe.2 $ mem_comap.2 $ subset_closure hx
...
is_bundled_set.ext' $ (coe_srange _).trans subtype.range_coe
...
assume p hp, prod.fst_mul_snd p ▸ mul_mem _
  ((le_sup_left : s.prod ⊥ ≤ s.prod ⊥ ⊔ prod ⊥ t) ⟨hp.1, is_bundled_set.mem_coe.2 $ one_mem ⊥⟩)
  ((le_sup_right : prod ⊥ t ≤ s.prod ⊥ ⊔ prod ⊥ t) ⟨is_bundled_set.mem_coe.2 $ one_mem ⊥, hp.2⟩)
...
{ map_mul' :=  λ _ _, rfl, map_add' := λ _ _, rfl, ..equiv.set_congr $ is_bundled_set.ext'_iff.1 h }
