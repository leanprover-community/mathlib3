...
lemma to_subsemiring_eq_closure : M.to_subsemiring = subsemiring.closure (M : set R) :=
begin
  ext,
  refine ⟨λ hx, _, λ hx, (subsemiring.mem_closure.mp hx) M.to_subsemiring (λ s sM, _)⟩;
  rintros - ⟨H1, rfl⟩;
  rintros - ⟨H2, rfl⟩,
...
by simp only [←to_subsemiring_to_add_submonoid, submonoid.to_subsemiring_eq_closure,
