...
/-- The `subsemiring.closure` of a multiplicative submonoid coincides with the `subsemiring`
generated by the submonoid itself. -/
lemma closure_eq_submonoid_to_subsemiring : subsemiring.closure (M : set R) = M.to_subsemiring :=
begin
  ext,
  refine ⟨λ hx, (subsemiring.mem_closure.mp hx) M.to_subsemiring (λ s sM, _), λ hx, _⟩;
  rintros - ⟨H1, rfl⟩;
  rintros - ⟨H2, rfl⟩,
  { exact H2 sM },
...
begin
  rw [← add_submonoid.mem_coe, ← submonoid.to_subsemiring_coe, ← closure_eq_submonoid_to_subsemiring],
