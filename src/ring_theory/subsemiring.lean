...
closure_le.2 $ λ x hx, (set_like.mem_coe _).2 $ mem_comap.2 $ subset_closure hx
...
assume p hp, prod.fst_mul_snd p ▸ mul_mem _
  ((le_sup_left : s.prod ⊥ ≤ s.prod ⊥ ⊔ prod ⊥ t) ⟨hp.1, (set_like.mem_coe _).2 $ one_mem ⊥⟩)
  ((le_sup_right : prod ⊥ t ≤ s.prod ⊥ ⊔ prod ⊥ t) ⟨(set_like.mem_coe _).2 $ one_mem ⊥, hp.2⟩)
