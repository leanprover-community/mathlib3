...
lemma associates.is_atom_iff [cancel_comm_monoid_with_zero M] {p : associates M} (h₁ : p ≠ 0) :
...
lemma pow_prime₅' {M : Type*} [cancel_comm_monoid_with_zero M] [unique_factorization_monoid M]
...
begin
  ...
  { have : (finset.image (λ m, (c ⟨1, finset.mem_range.2 (nat.lt_succ_of_le hn)⟩)^m)
    ...
    { conv_rhs {rw [← finset.card_range (i+1)] },
      ...
      apply pow_injective_of_not_unit (pow_primeₐ' n 1 ⟨le_refl 1, hn⟩ c h₁)
