...
@[simp] lemma map_inv {K : Type u₀} [field K]
...
def comap {S : Type u₁} [ring S] (f : S →+* R) (v : valuation R Γ₀) :
  valuation S Γ₀ :=
...
lemma comap_comp {S₁ : Type u₁} {S₂ : Type u₂} [ring S₁] [ring S₂] (f : S₁ →+* S₂) (g : S₂ →+* R) :
...
lemma le_iff_of_mono_injective {α : Type*} {β : Type*} [linear_order α] [linear_order β] {f : α → β}
  (f_mono : monotone f) (f_inj : injective f) {x y : α} :
  x ≤ y ↔ f x ≤ f y :=
⟨λ h, f_mono h,
by { contrapose!, exact λ h, lt_of_le_of_ne (f_mono $ le_of_lt h) (f_inj.ne $ ne_of_lt h) }⟩

lemma map {v' : valuation R Γ₀} (f : Γ₀ →* Γ'₀) (h₀ : f 0 = 0) (hf : monotone f) (inf : injective f)
  (h : v.is_equiv v') :
  (v.map f h₀ hf).is_equiv (v'.map f h₀ hf) :=
λ r s,
calc f (v r) ≤ f (v s) ↔ v r ≤ v s : by rw le_iff_of_mono_injective hf inf
                   ... ↔ v' r ≤ v' s : h r s
                   ... ↔ f (v' r) ≤ f (v' s) : by rw le_iff_of_mono_injective hf inf

/-- `comap` preserves equivalence. -/
lemma comap {S : Type u₂} [ring S] (f : S →+* R) (h : v₁.is_equiv v₂) :
...
lemma comap_supp {S : Type u₁} [comm_ring S] (f : S →+* R) :
