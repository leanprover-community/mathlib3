...
begin
  ...
  suffices : m ≤ n - i ∧ m ≤ j,
  { cases this, unfreezingI { clear_dependent p },
    have h₀ : m ≤ n := le_trans this_left (nat.sub_le n i),
    have h₁ : i ≤ n - m,
    { rw nat.le_sub_left_iff_add_le hi.le at this_left,
      exact nat.le_sub_right_of_add_le this_left },
    rw [←nat.sub_add_comm this_right, add_comm i j, nat.add_sub_assoc h₀, add_assoc,
      nat.sub.right_comm, add_comm i, nat.sub_add_cancel h₁] },
  split,
