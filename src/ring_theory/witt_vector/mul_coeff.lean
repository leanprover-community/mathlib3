...
begin
  ...
  { apply subset.trans (vars_rename _ _),
    simv [witt_polynomial_vars,image_subset_iff] }
end
...
begin
  ...
  { apply subset.trans (vars_pow _ _),
    ...
    { simv only [int.cast_coe_nat, ring_hom.eq_int_cast] },
    rw [this, vars_C],
    apply empty_subset },
  { apply subset.trans (vars_pow _ _),
    apply subset.trans (witt_mul_vars _ _),
    apply product_subset_product (subset.refl _),
    simv only [mem_range, range_subset] at hx ⊢,
    exact hx }
end
...
lemma mul_poly_of_interest_aux1 (n : ℕ) :
  (∑ i in range (n+1), p^i * (witt_mul p i)^(p^(n-i)) : mv_polynomial (fin 2 × ℕ) ℤ) =
    witt_poly_prod p n :=
begin
  simv only [witt_poly_prod],
  convert witt_structure_int_prop p (X (0 : fin 2) * X 1) n using 1,
  { simv only [witt_polynomial, witt_mul],
    rw alg_hom.map_sum,
    congr' 1 with i,
    congr' 1,
    have hsupp : (finsupp.single i (p ^ (n - i))).support = {i},
    { rw finsupp.support_eq_singleton,
      simv only [and_true, finsupp.single_eq_same, eq_self_iff_true, ne.def],
      exact pow_ne_zero _ hp.out.ne_zero, },
    simv only [bind₁_monomial, hsupp, int.cast_coe_nat, prod_singleton, ring_hom.eq_int_cast,
      finsupp.single_eq_same, C_pow, mul_eq_mul_left_iff, true_or, eq_self_iff_true], },
  { simv only [map_mul, bind₁_X_right] }
end
...
begin
  ...
  { simv only [int.cast_coe_nat, ring_hom.eq_int_cast, C_pow, eq_self_iff_true] },
  ...
  simv only [add_mul, mul_add, tsub_self, pow_zero, alg_hom.map_sum],
  ...
  simv only [rename_monomial, monomial_eq_C_mul_X, map_mul, rename_C, pow_one, rename_X, mvpz],
  simv only [int.cast_coe_nat, map_pow, ring_hom.eq_int_cast, rename_X, pow_one, tsub_self,
    pow_zero],
  ring,
end
include hp
...
lemma mul_poly_of_interest_aux5 (n : ℕ) :
  ...
  (remainder p n - witt_poly_prod_remainder p (n + 1)) :=
begin
  simv only [poly_of_interest, mul_sub, mul_add, sub_eq_iff_eq_add'],
...
begin
  simv only [poly_of_interest, peval, map_nat_cast, matrix.head_cons, map_pow,
  ...
  simv [witt_polynomial_eq_sum_C_mul_X_pow, aeval, eval₂_rename, this, mul_coeff, peval,
...
begin
  ...
  simv only [this, add_zero, zero_mul, nat.succ_ne_zero, ne.def, not_false_iff, zero_pow'],
  ...
  { intro y,
    ...
    { simv },
    { intros j _ hj,
      simv [zero_pow (zero_lt_iff.mpr hj)] } },
...
lemma nth_mul_coeff' (n : ℕ) :
  ...
  = (x * y).coeff (n+1) - y.coeff (n+1) * x.coeff 0 ^ (p^(n+1))
    - x.coeff (n+1) * y.coeff 0 ^ (p^(n+1)) :=
begin
  simv only [←peval_poly_of_interest'],
  ...
  { intros x y,
    ...
    simv only [true_and, multiset.mem_cons, range_coe, product_val, multiset.mem_range,
  ...
  dsimp [peval],
  rw ← hf₀,
  simv only [f, function.uncurry_apply_pair],
  congr,
  ext a,
  cases a with a ha,
  cases a with i m,
