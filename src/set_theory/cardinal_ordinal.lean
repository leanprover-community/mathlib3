...
theorem mk_list_eq_mk_add_omega (α : Type u) [nonempty α] : #(list α) = #α + ω :=
begin
  casesI fintype_or_infinite α,
  { haveI : encodable α := fintype.encodable α,
    rw [mk_list_eq_omega, eq_comm],
    exact add_eq_right le_rfl mk_le_omega },
  { rw [mk_list_eq_mk, eq_comm],
    exact add_eq_left (omega_le_mk α) (omega_le_mk α) }
end
