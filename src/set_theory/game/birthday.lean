...
theorem birthday_add : ∀ a b : pgame,
  (a + b).birthday = of_nat_ordinal (to_nat_ordinal a.birthday + to_nat_ordinal b.birthday)
| a b := begin
...
@[simp] theorem birthday_add_one (a : pgame) : (a + 1).birthday = a.birthday + 1 :=
by { rw birthday_add, simpa [add_one] }

@[simp] theorem birthday_one_add (a : pgame) : (1 + a).birthday = a.birthday + 1 :=
by rw [birthday_add_comm, birthday_add_one]
...
@[simp] theorem birthday_add_nat (a : pgame) (n : ℕ) : (a + n).birthday = a.birthday + n :=
by { rw birthday_add, simp [add_nat] }

@[simp] theorem birthday_nat_add (a : pgame) (n : ℕ) : (↑n + a).birthday = a.birthday + n :=
by rw [birthday_add_comm, birthday_add_nat]
