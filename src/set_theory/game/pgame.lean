...
equivalence relations at the level of pregames, we introduce the notion of `identical` games, and
show, for example, that the games `x + (y + z)` and `(x + y) + z` are identical.
...
inductive restricted : pgame.{u} → pgame.{u} → Prop
...
@[refl] theorem restricted.refl : Π (x : pgame), restricted x x
...
-/
inductive identical : pgame.{u} → pgame.{u} → Prop
| mk : Π {x y : pgame} (Lxy : x.left_moves → y.left_moves) (Lyx : y.left_moves → x.left_moves)
         ...
         (∀ i, identical (x.move_left (Lyx i)) (y.move_left i) ) →
         (∀ j, identical (x.move_right j) (y.move_right (Rxy j))) →
         (∀ j, identical (x.move_right (Ryx j)) (y.move_right j) ) → identical x y

local infix ` ≡ `:50 := identical

/-- A constructor for `identical` better suited for whenever the games are destructured. -/
theorem identical.mk' {xl xr yl yr} {xL : xl → pgame} {xR : xr → pgame}
  {yL : yl → pgame} {yR : yr → pgame}
  (Lxy : xl → yl) (Lyx : yl → xl) (Rxy : xr → yr) (Ryx : yr → xr)
  (HLxy : ∀ i, (xL i).identical (yL (Lxy i))) (HLyx : ∀ i, (xL (Lyx i)).identical (yL i))
  (HRxy : ∀ j, (xR j).identical (yR (Rxy j))) (HRyx : ∀ j, (xR (Ryx j)).identical (yR j)) :
  identical (mk xl xr xL xR) (mk yl yr yL yR) :=
@identical.mk (mk _ _ _ _) (mk _ _ _ _) _ _ _ _ HLxy HLyx HRxy HRyx

@[simp, refl] theorem identical.rfl : Π {x : pgame}, x ≡ x
| ⟨xl, xr, xL, xR⟩ := by apply identical.mk' _ _ _ _; exact λ i, identical.rfl
theorem identical.refl (x : pgame) : x ≡ x := identical.rfl

@[symm] theorem identical.symm : Π {x y : pgame}, x ≡ y → y ≡ x
| ⟨xl, xr, xL, xR⟩ ⟨yl, yr, yL, yR⟩ ⟨Lxy, Lyx, Rxy, Ryx, HLxy, HLyx, HRxy, HRyx⟩ :=
⟨_, _, _, _, λ i, (HLyx i).symm, λ i, (HLxy i).symm, λ j, (HRyx j).symm, λ j, (HRxy j).symm⟩
using_well_founded { dec_tac := pgame_wf_tac }

theorem identical.symm_iff {x y : pgame} : x ≡ y ↔ y ≡ x := ⟨identical.symm, identical.symm⟩

-- Is it the case that `restricted x y → restricted y x → identical x y`?

@[trans] theorem identical.trans : Π {x y z : pgame}, x ≡ y → y ≡ z → x ≡ z
| ⟨xl, xr, xL, xR⟩ ⟨yl, yr, yL, yR⟩ ⟨zl, zr, zL, zR⟩
  ⟨Lxy, Lyx, Rxy, Ryx, HLxy, HLyx, HRxy, HRyx⟩
  ⟨Lyz, Lzy, Ryz, Rzy, HLyz, HLzy, HRyz, HRzy⟩ :=
⟨_, _, _, _,
  λ i, (HLxy i).trans (HLyz _), λ i, ((HLyx _).trans (HLzy i)),
  λ j, (HRxy j).trans (HRyz _), λ j, ((HRyx _).trans (HRzy j))⟩
using_well_founded { dec_tac := pgame_wf_tac }
...
/-- If `x` is identical to `y`, then Left and Right have the same moves in either game,
    so `x` is a restriction of `y`. -/
def identical.restricted : Π {x y : pgame} (r : identical x y), restricted x y
| ⟨xl, xr, xL, xR⟩ ⟨yl, yr, yL, yR⟩ ⟨Lxy, Lyx, Rxy, Ryx, HLxy, HLyx, HRxy, HRyx⟩ :=
⟨Lxy, Ryx, λ i, (HLxy i).restricted, λ j, (HRyx j).restricted⟩
using_well_founded { dec_tac := pgame_wf_tac }

theorem identical.le {x y : pgame} (r : x ≡ y) : x ≤ y := r.restricted.le
theorem identical.ge {x y : pgame} (r : x ≡ y) : y ≤ x := r.symm.restricted.le

/-- An identity lets us prove equivalence of games. -/
theorem identical.equiv {x y : pgame} (r : x ≡ y) : x ≈ y := ⟨r.le, r.ge⟩
theorem identical.equiv' {x y : pgame} (r : x ≡ y) : y ≈ x := ⟨r.ge, r.le⟩

/-- Prove two games to be identical by providing an equivalence instead of bare functions between
the left and right games. -/
theorem identical_of_equiv {x y : pgame}
  (L : x.left_moves ≃ y.left_moves) (R : x.right_moves ≃ y.right_moves)
  (HL : ∀ i, identical (x.move_left i) (y.move_left (L i)))
  (HR : ∀ j, identical (x.move_right j) (y.move_right (R j))) : x ≡ y :=
⟨L, L.symm, R, R.symm,
  HL, by simpa using λ i, (HL (L.symm i)), HR, by simpa using λ j, (HR (R.symm j))⟩

/-- Any game without left or right moves is identical to 0. -/
def identical.is_empty (x : pgame) [is_empty x.left_moves] [is_empty x.right_moves] : x ≡ 0 :=
by split; exact is_empty_elim

theorem equiv.is_empty (x : pgame) [is_empty (x.left_moves)] [is_empty (x.right_moves)] : x ≈ 0 :=
(identical.is_empty x).equiv

instance {x y : pgame} : has_coe (identical x y) (x ≈ y) := ⟨identical.equiv⟩
...
/-- If `x` is identical to `y`, then `-x` is identical to `-y`. -/
theorem identical.neg_congr : ∀ {x y : pgame}, x ≡ y → -x ≡ -y
| (mk xl xr xL xR) (mk yl yr yL yR) ⟨Lxy, Lyx, Rxy, Ryx, HLxy, HLyx, HRxy, HRyx⟩ :=
⟨_, _, _, _,
  λ j, (HRxy j).neg_congr, λ j, ((HRyx j).neg_congr),
  λ i, (HLxy i).neg_congr, λ i, ((HLyx i).neg_congr)⟩
using_well_founded { dec_tac := pgame_wf_tac }
...
/-- `x + 0` is identical to `x`. -/
theorem add_zero_identical : Π (x : pgame.{u}), x + 0 ≡ x
| (mk xl xr xL xR) :=
begin
  refine identical_of_equiv (equiv.sum_empty xl pempty) (equiv.sum_empty xr pempty) _ _;
  rintro (⟨i⟩|⟨⟨⟩⟩);
  apply add_zero_identical
end

/-- `x + 0` is equivalent to `x`. -/
lemma add_zero_equiv (x : pgame.{u}) : x + 0 ≈ x := (add_zero_identical x).equiv

/-- `0 + x` is identical to `x`. -/
theorem zero_add_identical : Π (x : pgame.{u}), 0 + x ≡ x
| (mk xl xr xL xR) :=
begin
  refine identical_of_equiv (equiv.empty_sum pempty xl) (equiv.empty_sum pempty xr) _ _;
  rintro (⟨⟨⟩⟩|⟨i⟩);
  apply zero_add_identical
end

/-- `0 + x` is equivalent to `x`. -/
lemma zero_add_equiv (x : pgame.{u}) : 0 + x ≈ x := (zero_add_identical x).equiv
...
/-- If `w` is identical to `x` and `y` is identical to `z`, then `w - y` is identical to `x - z`. -/
def identical.sub_congr {w x y z : pgame} (h₁ : w ≡ x) (h₂ : y ≡ z) : w - y ≡ x - z :=
h₁.add_congr h₂.neg_congr

/-- `-(x + y)` is identical to `-x + -y`. -/
def neg_add_identical : Π (x y : pgame), -(x + y) ≡ -x + -y
| ⟨xl, xr, xL, xR⟩ ⟨yl, yr, yL, yR⟩ :=
begin
  refine identical_of_equiv (equiv.refl _) (equiv.refl _) _ _,
  all_goals
  { rintro (i | i),
    { apply neg_add_identical },
    { apply neg_add_identical ⟨xl, xr, xL, xR⟩ } }
end
using_well_founded { dec_tac := pgame_wf_tac }

theorem neg_add_le {x y : pgame} : -(x + y) ≤ -x + -y := (neg_add_identical x y).le
theorem neg_add_ge {x y : pgame} : -x + -y ≤ -(x + y) := (neg_add_identical x y).ge
theorem neg_add_equiv {x y : pgame} : -(x + y) ≈ -x + -y := (neg_add_identical x y).equiv

/-- `x + y` is identical to `y + x`. -/
def add_comm_identical : Π (x y : pgame.{u}), x + y ≡ y + x
| ⟨xl, xr, xL, xR⟩ ⟨yl, yr, yL, yR⟩ :=
begin
  refine identical_of_equiv (equiv.sum_comm _ _) (equiv.sum_comm _ _) _ _;
  rintros (_|_);
  { simp [left_moves_add, right_moves_add], apply add_comm_identical }
end
using_well_founded { dec_tac := pgame_wf_tac }

theorem add_comm_le {x y : pgame} : x + y ≤ y + x := (add_comm_identical x y).le
theorem add_comm_equiv {x y : pgame} : x + y ≈ y + x := (add_comm_identical x y).equiv

/-- `(x + y) + z` is identical to `x + (y + z)`. -/
def add_assoc_identical : Π (x y z : pgame.{u}), x + y + z ≡ x + (y + z)
| ⟨xl, xr, xL, xR⟩ ⟨yl, yr, yL, yR⟩ ⟨zl, zr, zL, zR⟩ :=
begin
  refine identical_of_equiv (equiv.sum_assoc _ _ _) (equiv.sum_assoc _ _ _) _ _,
  all_goals
  { rintro (⟨i|i⟩|i),
    { apply add_assoc_identical },
    { apply add_assoc_identical ⟨xl, xr, xL, xR⟩ },
    { apply add_assoc_identical ⟨xl, xr, xL, xR⟩ ⟨yl, yr, yL, yR⟩ } }
end
using_well_founded { dec_tac := pgame_wf_tac }

theorem add_assoc_equiv {x y z : pgame} : (x + y) + z ≈ x + (y + z) :=
(add_assoc_identical x y z).equiv
...
suffices z + x ≤ y + x → z ≤ y, by { rw ←pgame.not_le at ⊢ h, exact mt this h }, λ w,
  calc z ≤ z + 0        : (add_zero_identical _).symm.le
     ... ≤ z + (x + -x) : add_le_add_left (zero_le_add_right_neg x) _
     ... ≤ z + x + -x   : (add_assoc_identical _ _ _).ge
     ... ≤ y + x + -x   : add_le_add_right w _
     ... ≤ y + (x + -x) : (add_assoc_identical _ _ _).le
     ... ≤ y + 0        : add_le_add_left (add_right_neg_le_zero x) _
     ... ≤ y            : (add_zero_identical _).le
