...
theorem bsup_succ_mono {o : ordinal} {f : Π a < succ o, ordinal}
...
theorem blsub_succ_mono {o : ordinal} {f : Π a < succ o, ordinal}
  (hf : ∀ {i j} hi hj, i ≤ j → f i hi ≤ f j hj) : blsub _ f = succ (f o (lt_succ_self o)) :=
bsup_succ_mono $ λ i j hi hj h, succ_le_succ.2 (hf hi hj h)
