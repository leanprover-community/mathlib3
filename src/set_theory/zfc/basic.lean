...
begin
  ...
  { intros H,
    ...
    cases h with z hz,
    rcases H z (mem_hom_left.2 hz) with ⟨y, rfl, hzy⟩,
    refine ⟨y, rfl, λ w hxw, _⟩ ,
    have := H w (mem_hom_left.2 hxw),
...
λ ⟨y, hy⟩, ⟨_, sep_eq_of_subset $ sInter_subset $ mem_hom_left.2 hy⟩
