...
- `Set.transitive`: every element of a set is a subset.
...
def transitive (x : Set) : Prop := ∀ y ∈ x, y ⊆ x

@[simp] theorem empty_transitive : transitive ∅ := λ y hy, (not_mem_empty y hy).elim

theorem transitive.subset_of_mem (h : x.transitive) : y ∈ x → y ⊆ x := h y

theorem transitive_iff_mem_trans : z.transitive ↔ ∀ ⦃x y : Set⦄, x ∈ y → y ∈ z → x ∈ z :=
⟨λ h x y hx hy, h.subset_of_mem hy hx, λ H x hx y hy, H hy hx⟩

theorem transitive.mem_trans (h : z.transitive) : ∀ {x y : Set}, x ∈ y → y ∈ z → x ∈ z :=
transitive_iff_mem_trans.mp h

protected theorem transitive.inter (hx : x.transitive) (hy : y.transitive) :
  (x ∩ y).transitive :=
λ z hz w hw, by { rw mem_inter at hz ⊢, exact ⟨hx.mem_trans hw hz.1, hy.mem_trans hw hz.2⟩ }

protected theorem transitive.sUnion (h : x.transitive) : (⋃₀ x).transitive :=
λ y hy z hz, begin
  rcases mem_sUnion.1 hy with ⟨w, hw, hw'⟩,
  exact mem_sUnion_of_mem hz (h.mem_trans hw' hw)
end

theorem transitive.sUnion' (H : ∀ y ∈ x, transitive y) : (⋃₀ x).transitive :=
λ y hy z hz, begin
  rcases mem_sUnion.1 hy with ⟨w, hw, hw'⟩,
  exact mem_sUnion_of_mem ((H w hw).mem_trans hz hw') hw
end

theorem transitive_iff_sUnion_subset : x.transitive ↔ ⋃₀ x ⊆ x :=
⟨λ h y hy, by { rcases mem_sUnion.1 hy with ⟨z, hz, hz'⟩, exact h.mem_trans hz' hz },
  λ H y hy z hz, H $ mem_sUnion_of_mem hz hy⟩

alias transitive_iff_sUnion_subset ↔ transitive.sUnion_subset _

theorem transitive_iff_subset_powerset : x.transitive ↔ x ⊆ powerset x :=
⟨λ h y hy, mem_powerset.2 $ h.subset_of_mem hy, λ H y hy z hz, mem_powerset.1 (H hy) hz⟩

alias transitive_iff_subset_powerset ↔ transitive.subset_powerset _
...
def is_ordinal (x : Set) : Prop := x.hereditarily transitive

theorem is_ordinal_iff {x : Set} : x.is_ordinal ↔ x.transitive ∧ ∀ y ∈ x, is_ordinal y :=
hereditarily_iff
...
theorem mem_trans (h : z.is_ordinal) : x ∈ y → y ∈ z → x ∈ z := h.transitive.mem_trans
...
begin
  refine is_ordinal_iff.mpr ⟨transitive.sUnion' $ λ y hy, (H y hy).transitive, λ y hy, _⟩,
