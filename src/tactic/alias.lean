...
do old ← ident,
  ...
  let doc := λ al ol : name, meta_info.doc_string.get_or_else $
    "**Alias** of `" ++ to_string ol ++ "`.",
  do
  { tk "←" <|> tk "<-",
    aliases ← many ident,
    ↑(aliases.mmap' $ λ al, alias_direct d (doc al old) al) } <|>
  do
  { tk "↔" <|> tk "<->",
    (left, right) ←
    ...
    alias_iff d (doc left $ old.append `mp) left `iff.mp,
    alias_iff d (doc right $ old.append `mpr) right `iff.mpr }
