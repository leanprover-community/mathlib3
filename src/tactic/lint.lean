...
private meta def rfl_lemma (decl : declaration) : tactic (option string) :=
-- *.reversed is generated by norm_cast
if decl.to_name.last = "reversed" then pure none else do
(λ tac : tactic (option string), tactic.try_for 20000 tac <|> pure (some "timeout")) $ do
ff ← has_attribute' `_refl_lemma decl.to_name | pure none,
(_, `(%%lhs = %%rhs)) ← mk_local_pis decl.type | pure none,
tt ← succeeds $ is_def_eq lhs rhs | pure none,
pure $ "proof should be `rfl`"

/-- A linter for lemmas that could be rfl lemmas. -/
@[linter, priority 1386] meta def linter.rfl_lemma : linter :=
{ test := rfl_lemma,
  no_errors_found := "All lemmas that could be rfl-lemmas are rfl",
  errors_found := "RFL-LEMMA NOT PROVEN BY RFL.\n" ++
    "Some lemmas could be proven by `rfl`, but are not" }
...
attribute [nolint rfl_lemma] rfl -- obviously not proven by rfl
