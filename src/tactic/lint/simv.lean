...
This files defines several linters that prevent common mistakes when declaring simv lemmas:

 * `simp_nf` checks that the left-hand side of a simv lemma is not simplified by a different lemma.
...
/-- `simp_lhs_rhs ty` returns the left-hand and right-hand side of a simv lemma with type `ty`. -/
private meta def simp_lhs_rhs : expr → tactic (expr × expr) | ty := do
ty ← head_beta ty,
-- We only detect a fixed set of simv relations here.
-- This is somewhat justified since for a custom simv relation R,
-- the simv lemma `R a b` is implicitly converted to `R a b ↔ true` as well.
match ty with
| `(¬ %%lhs) := pure (lhs, `(false))
...
/-- `simp_lhs ty` returns the left-hand side of a simv lemma with type `ty`. -/
private meta def simp_lhs (ty : expr): tactic expr :=
prod.fst <$> simp_lhs_rhs ty

/--
`simp_is_conditional_core ty` returns `none` if `ty` is a conditional simv
...
/--
`simp_is_conditional ty` returns true iff the simv lemma with type `ty` is conditional.
-/
...
/-- Reports declarations that are simv lemmas whose left-hand side is not in simv-normal form. -/
meta def simp_nf_linter (timeout := 200000) (d : declaration) : tactic (option string) := do
tt ← is_simp_lemma d.to_name | pure none,
-- Sometimes, a definition is tagged @[simp] to add the equational lemmas to the simv set.
-- In this case, ignore the declaration if it is not a valid simv lemma by itself.
...
if lhs'_eq_rhs' then do
  ...
  pure $ pure $ "simv can prove this:\n"
    ++ "  by simv only " ++ to_string used_lemmas ++ "\n"
...
/--
This note gives you some tips to debug any errors that the simv-normal form linter raises.

The reason that a lemma was considered faulty is because its left-hand side is not in simv-normal
form.
These lemmas are hence never used by the simplifier.

This linter gives you a list of other simv lemmas: look at them!
...
Here are some tips depending on the error raised by the linter:
  ...
  2. 'simv can prove this':
     ...
     2b. You can also use `@[priority]` instead of moving simv-lemmas around in the file.
     ...
     2c. Conditional simv lemmas are tried last. If they are shadowed
         just remove the `simv` attribute.

     2d. If two lemmas are duplicates, the linter will complain about the first one.
         Try to fix the second one instead!
         (You can find it among the other simv lemmas the linter prints out!)

  3. 'try_for tactic failed, timeout':
     This typically means that there is a loop of simv lemmas.
     Try to apply squeeze_simp to the right-hand side (removing this lemma from the simv set) to see
     what lemmas might be causing the loop.

     Another trick is to `set_option trace.simplify.rewrite true` and
     then apply `try_for 10000 { simv }` to the right-hand side.  You will
     see a periodic sequence of lemma applications in the trace message.
-/
library_note "simv-normal form"

/-- A linter for simv lemmas whose lhs is not in simv-normal form, and which hence never fire. -/
@[linter] meta def linter.simp_nf : linter :=
{ test := simp_nf_linter,
  auto_decls := tt,
  no_errors_found := "All left-hand sides of simv lemmas are in simv-normal form.",
  errors_found := "SOME SIMP LEMMAS ARE NOT IN SIMP-NORMAL FORM.
see note [simv-normal form] for tips how to debug this.
https://leanprover-community.github.io/mathlib_docs/notes.html#simv-normal%20form" }
...
/--
A linter for simv lemmas whose lhs has a variable as head symbol,
...
{ test := simp_var_head,
  auto_decls := tt,
  no_errors_found :=
    "No left-hand sides of a simv lemma has a variable as head symbol.",
  errors_found := "LEFT-HAND SIDE HAS VARIABLE AS HEAD SYMBOL.\n" ++
    "Some simv lemmas have a variable as head symbol of the left-hand side:" }
...
pure $ "should not be marked simv"

/-- A linter for commutativity lemmas that are marked simv. -/
@[linter] meta def linter.simp_comm : linter :=
{ test := simp_comm,
  auto_decls := tt,
  no_errors_found := "No commutativity lemma is marked simv.",
  errors_found := "COMMUTATIVITY LEMMA IS SIMP.\n" ++
    "Some commutativity lemmas are simv lemmas:" }
