...
/-!
# `move_op`: a tactic for moving operands

Calling `move_op [a, ← b, c]`, recursively looks inside the goal for expressions formed as a
succession of applications of a binary operation.  E.g. if the operation is `+`, it looks for
summands, if the operation is `*`, it looks for factors.  We call "operands" the terms that it
finds.
Whenever it finds one, it moves the operands that unify to `a, b, c`, removing all parentheses.

See the doc-string for `tactic.interactive.move_op` for more information.

##  Implementation notes

The implementation of `move_op` only moves the terms specified by the user (and rearranges
parentheses).
...
The scope of `move_op` is different: it is designed to make it easy to move individual terms
around a sum.
...
/--  Takes an `expr` and returns a list of its operands. -/
meta def get_ops (op : name) : expr → list expr
| (expr.app (expr.app ef a) b) := if ef.get_app_fn.const_name = op then
                                      (get_ops a ++ get_ops b) else
                                      [expr.app (expr.app ef a) b] --([a] ++ [b])
| a            := [a]

--review the doc-string
/-- `sorted_op` takes an optional location name `hyp` for where it will be applied, a list `ll` of
`bool × pexpr` (arising as the user-provided input to `move_op`) and an expression `e`.

`sorted_op hyp ll e` returns an ordered operation of the terms of `e`, where the order is
...
meta def sorted_op (op : name) (hyp : option name) (ll : list (bool × pexpr)) (e : expr) :
  tactic (list bool) :=
do
  (sli, is_unused) ← ll.combined (get_ops op e),
  match sli with
  | []       := return is_unused
  | (eₕ::es) := do
    e' ← es.mfoldl (λ eₗ eᵣ, mk_app op [eₗ, eᵣ]) eₕ,
    e_eq ← mk_app `eq [e, e'],
    n ← get_unused_name,
    assert n e_eq,
    e_eq_fmt ← pp e_eq,
    reflexivity <|>
      ...
      mul_comm, mul_assoc, mul_left_comm], done, }] <|>
...
/-- Partially traverses an expression in search for an iterated application of a binary operation.
When `recurse_on_expr` finds such a term, it sorts it using `sorted_op`. -/
meta def recurse_on_expr (op : name) (hyp : option name) (ll : list (bool × pexpr)) :
  expr → tactic (list bool)
| e@(expr.app (expr.app ef a) b) := if ef.get_app_fn.const_name = op then
                                      sorted_op op hyp ll e else
                                      do  li_unused ← e.get_app_args.mmap recurse_on_expr,
...
| e                  := do li_unused ← e.get_app_args.mmap recurse_on_expr,
                          let unused_summed := li_unused.transpose.map list.band,
                          return unused_summed
...
/--
Calling `move_op [a, ← b, c]`, recursively looks inside the goal for expressions involving a sum.
Whenever it finds one, it moves the operands that unify to `a, b, c`, removing all parentheses.
...
right.  Unnamed terms stay in place.  Due to re-parenthesizing, doing `move_op` with no argument
...
```lean
import tactic.move_op

example {a b c d : ℕ} (h : c = d) : c + b + a = b + a + d :=
begin
  move_op [← a, b],  -- Goal: `a + c + b = a + d + b`  -- both sides changed
  congr,
  exact h
end
...
begin
  move_op [_ * c, ← _ * c], -- Goal: `a * c + c + b * c = a * d + d + b * d`
...
The list of expressions that `move_op` takes is optional and a single expression can be passed
without brackets.  Thus `move_op ← f` and `move_op [← f]` mean the same.

Finally, `move_op` can also target one or more hypotheses.  If `hp₁, hp₂` are in the
local context, then `move_op [f, ← g] at hp₁ hp₂` performs the rearranging at `hp₁` and `hp₂`.
...
1. `move_op [vars]? at *` reports gloally unused variables and whether *all* goals
   are unchanged, not *each unchanged goal*.
2. If a target of `move_op` is left unchanged by the tactic, then this will be flagged (unless
   we are using `at *`).
...
* `tactive.interactive.abel`
  ...
  success rate than `move_op`.  If the goal is an equality of two sums that are simply obtained by
  reparenthesizing and permuting operands, then `move_op [appropriate terms]` can close the goal.
  Compared to `abel`, `move_op` has the advantage of allowing the user to specify the beginning and
  the end of the final sum, so that from there the user can continue with the proof.

* `tactic.interactive.ac_change`
  supports a wide variety of operations.  At the moment, `move_op` only works with addition.
  Still, on several experiments, `move_op` had a much quicker performance than `ac_change`.
  Also, for `move_op` the user need only specify a few terms: the tactic itself takes care of
  producing the full rearrangement and proving it "behind the scenes".

###  Remark:
It is still possible that the same output of `move_op [exprs]` can be achieved by a proper sublist
...
`a + b = 0`, applying either one of `move_op [b,a]`, or `move_op a`, or `move_op ← b` has the
same effect and changes the goal to `b + a = 0`.  These are all valid uses of `move_op`.
-/
meta def move_op (op : parse ident)
  (args : parse move_pexpr_list_or_texpr) (locat : parse location) :
  tactic unit :=
match locat with
| loc.wildcard := do
  ctx ← local_context,
  err_rep ← ctx.mmap (λ e, move_op_with_errors op args e.local_pp_name),
  er_t ← move_op_with_errors op args none,
  if ff ∉ ([er_t.1] ++ err_rep.map (λ e, e.1)) then
    trace "'move_op at *' changed nothing" else skip,
...
| loc.ns names := do
  err_rep ← names.mmap $ move_op_with_errors op args,
...
{ name := "move_op",
  ...
  decl_names := [`tactic.interactive.move_op],
  tags := ["arithmetic"] }
