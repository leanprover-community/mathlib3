...
meta structure eval_info :=
  ...
  (csr_instance : expr) (cr_instance : option expr) (hp_instance : expr) 
/--
...
/--
  ...
  The number of implicit arguments must be explicitly passed so we can determine
  where the instances should go.
 -/
meta def mk_app_csr (f : name) (implicit : ℕ) (args : list (option expr)) : ring_exp_m expr := do
  ctx <- get_context,
  lift $ mk_mapp f ([some ctx.info_b.α, some ctx.info_b.csr_instance] ++ list.repeat none implicit ++ args)
...
/--
  ...
  where the instances should go.
  -/
meta def mk_proof (lem : name) (implicit : ℕ) (args : list (option expr)) (hs : list ex_info) : ring_exp_m expr := do
  hs' <- traverse ex_info.proof_term hs,
  mk_app_csr lem implicit (args ++ hs'.map some)
/--
  Use the proof terms as arguments to the given lemma.
  ...
  where the instances should go.
-/
meta def mk_proof_refl (term : expr) (lem : name) (implicit : ℕ) (args : list (option expr)) (hs : list ex_info) : ring_exp_m expr := do
  hs_full <- none_or_proof_term hs,
  match hs_full with
  | none := lift $ mk_eq_refl term
  | (some hs') := mk_app_csr lem implicit (args ++ hs'.map some)
  end
...
| sum pps@(ex.sum pps_i p (ex.zero _)) := do
  (p_p, p_pf) <- p.simple,
  pf <- mk_app_csr ``pretty_pf_sum_zero 2 [p_pf],
  pure $ (p_p, pf)
| sum (ex.sum pps_i p ps) := do
  (p_p, p_pf) <- p.simple,
  (ps_p, ps_pf) <- ps.simple,
  pps_p <- mk_app_csr ``has_add.add 0 [p_p, ps_p],
  pf <- mk_app_csr ``sum_congr 4 [p_pf, ps_pf],
  pure $ (pps_p, pf)
| prod (ex.prod pps_i p (ex.coeff _ ⟨⟨1, 1, _, _⟩⟩)) := do
  (p_p, p_pf) <- p.simple,
  pf <- mk_app_csr ``pretty_pf_prod_one 2 [p_pf],
  pure $ (p_p, pf)
| prod (ex.prod pps_i p (ex.coeff _ ⟨⟨-1, 1, _, _⟩⟩)) := do
  (p_p, p_pf) <- p.simple,
  p' <- lift $ mk_app ``has_neg.neg [p_p],
  pf <- mk_app_csr ``pretty_pf_prod_neg_one 3 [p_pf],
  pure $ (p', pf)
| prod (ex.prod pps_i p ps) := do
  (p_p, p_pf) <- p.simple,
  (ps_p, ps_pf) <- ps.simple,
  pps_p <- lift $ mk_app ``has_mul.mul [p_p, ps_p],
  pf <- mk_app_csr ``prod_congr 4 [p_pf, ps_pf],
  pure $ (pps_p, pf)
| base (ex.sum_b pps_i ps) := ps.simple
| exp (ex.exp pps_i p (ex.coeff _ ⟨⟨1, 1, _, _⟩⟩)) := do
  (p_p, p_pf) <- p.simple,
  pf <- mk_app_csr ``pretty_pf_exp_one 2 [p_pf],
  pure $ (p_p, pf)
| exp (ex.exp pps_i p ps) := do
  ...
  pf <- mk_app_csr ``exp_congr 4 [p_pf, ps_pf],
  pure $ (pps_p, pf)
| et ps := do
  pf <- ps.proof_term,
  pure $ (ps.pretty, pf)
...
meta def ex_sum (p : ex_pf prod) (ps : ex_pf sum) : ring_exp_m (ex_pf sum) := do
  ...
  pps_pf <- mk_proof_refl pps_p ``sum_congr 4 [] [p.info, ps.info],
...
meta def ex_prod (p : ex_pf exp) (ps : ex_pf prod) : ring_exp_m (ex_pf prod) := do
  ...
  pps_pf <- mk_proof_refl pps_p ``prod_congr 4 [] [p.info, ps.info],
...
meta def ex_exp (p : ex_pf base) (ps : ex_pf prod) : ring_exp_m (ex_pf exp) := do
  ...
  pps_pf <- mk_proof_refl pps_p ``exp_congr 4 [] [p.info, ps.info],
...
meta def base_to_exp (p : ex_pf base) : ring_exp_m (ex_pf exp) := do
  ...
  ps <- ex_exp p o,
  pf <- mk_proof ``base_to_exp_pf 2 [] [p.info],
...
meta def atom_to_sum (p : atom) : ring_exp_m (ex_pf sum) := do
  ...
  z <- ex_zero,
  p' <- ex_sum p' z,
  pf <- mk_proof ``atom_to_sum_pf 0 [p.1] [],
  pure $ p'.set_proof pf
...
| (ex.prod _ p ps) (ex.prod _ q qs) := if p.eq atom.eq q
  then do
    ...
    | (overlap.nonzero pq) := do
      pqs <- ex_prod p pq,
      pf <- mk_proof ``add_overlap_pf 3 [p.pretty] [pq.info],
      pure $ overlap.nonzero (pqs.set_proof pf)
    | (overlap.zero pq) := do
      z <- ex_zero,
      pf <- mk_proof ``add_overlap_pf_zero 2 [p.pretty] [pq.info],
...
| ps@(ex.zero ps_i) qs := do
  pf <- mk_proof ``add_pf_z_sum 3 [] [ps.info, qs.info],
  pure $ qs.set_proof pf
| ps qs@(ex.zero qs_i) := do
  pf <- mk_proof ``add_pf_sum_z 3 [] [ps.info, qs.info],
  pure $ ps.set_proof pf
| pps@(ex.sum pps_i p ps) qqs@(ex.sum qqs_i q qs) := do
  ...
  | (overlap.nonzero pq) := do
    pqs <- add ps qs,
    pqqs <- ex_sum pq pqs,
    pf <- mk_proof ``add_pf_sum_overlap 8 [] [pps.info, qqs.info, pq.info, pqs.info],
    pure $ pqqs.set_proof pf
  | (overlap.zero pq) := do
    pqs <- add ps qs,
    pf <- mk_proof ``add_pf_sum_overlap_zero 7 [] [pps.info, qqs.info, pq.info, pqs.info],
    pure $ pqs.set_proof pf
  | overlap.none := if p.lt atom.eq atom.lt q
  then do
    pqs <- add ps qqs,
    ppqs <- ex_sum p pqs,
    pf <- mk_proof ``add_pf_sum_lt 5 [] [pps.info, pqs.info],
    pure $ ppqs.set_proof pf
  else do
    pqs <- add pps qs,
    pqqs <- ex_sum q pqs,
    pf <- mk_proof ``add_pf_sum_gt 5 [] [qqs.info, pqs.info],
    pure $ pqqs.set_proof pf
  end
...
| ps@(ex.coeff ps_i x) qs@(ex.coeff qs_i y) := do
  pq <- mul_coeff ps_i.pretty qs_i.pretty x y,
  pf <- mk_proof_refl pq.pretty ``mul_pf_c_c 5 [] [ps.info, qs.info, pq.info],
  pure $ pq.set_proof pf
| ps@(ex.coeff ps_i x) qqs@(ex.prod qqs_i q qs) := do
  pqs <- mul_pp ps qs,
  pqqs <- ex_prod q pqs,
  pf <- mk_proof ``mul_pf_c_prod 5 [] [qqs.info, pqs.info],
  pure $ pqqs.set_proof pf
| pps@(ex.prod pps_i p ps) qs@(ex.coeff qs_i y) := do
  pqs <- mul_pp ps qs,
  ppqs <- ex_prod p pqs,
  pf <- mk_proof ``mul_pf_prod_c 5 [] [pps.info, pqs.info],
  pure $ ppqs.set_proof pf
| pps@(ex.prod pps_i p ps) qqs@(ex.prod qqs_i q qs) := do
  ...
  then do
    pqs <- mul_pp ps qqs,
    ppqs <- ex_prod p pqs,
    pf <- mk_proof ``mul_pp_pf_prod_lt 5 [] [pps.info, pqs.info],
    pure $ ppqs.set_proof pf
  else do
    pqs <- mul_pp pps qs,
    pqqs <- ex_prod q pqs,
    pf <- mk_proof ``mul_pp_pf_prod_gt 5 [] [qqs.info, pqs.info],
    pure $ pqqs.set_proof pf
...
| ps@(ex.zero ps_i) qs := do
  z <- ex_zero,
  pf <- mk_proof ``mul_p_pf_zero 3 [] [ps.info, qs.info],
  pure $ z.set_proof pf
| pps@(ex.sum pps_i p ps) qs := do
  ...
  pf <- mk_proof ``mul_p_pf_sum 5 [] [pps.info, ppsqs.info],
...
| ps qs@(ex.zero qs_i) := do
  z <- ex_zero,
  pf <- mk_proof ``mul_pf_zero 3 [] [ps.info, qs.info],
  pure $ z.set_proof pf
| ps qqs@(ex.sum qqs_i q qs) := do
  psq <- mul_p ps q,
  psqs <- mul ps qs,
  psqqs <- add psq psqs,
  pf <- mk_proof ``mul_pf_sum 5 [] [qqs.info, psqqs.info],
...
| pps@(ex.exp pps_i p ps) qs := do
  ...
  ppsqs <- ex_exp p psqs,
  pf <- mk_proof ``pow_e_pf_exp 5 [] [pps.info, psqs.info],
  pure $ ppsqs.set_proof pf
...
| ps@(ex.coeff ps_i ⟨⟨1, 1, _, _⟩⟩) qs := do
  o <- ex_one,
  pf <- mk_proof ``pow_pp_pf_one 3 [] [ps.info, qs.info],
  pure $ o.set_proof pf
| ps@(ex.coeff ps_i x) qs := do
  ps' <- ex_coeff x.1,
  ps'' <- (prod_to_sum $ ps'.set_proof ps.proof) >>= ex_sum_b,
  pqs <- ex_exp ps'' qs,
  pf <- mk_proof_refl pqs.pretty ``pow_pp_pf_c 5 [] [ps.info, qs.info, pqs.info],
  exp_to_prod $ pqs.set_proof pf
| pps@(ex.prod pps_i p ps) qs := do
  pqs <- pow_e p qs,
  psqs <- pow_pp ps qs,
  ppsqs <- ex_prod pqs psqs,
  pf <- mk_proof ``pow_pp_pf_prod 6 [] [pps.info, pqs.info, psqs.info],
  pure $ ppsqs.set_proof pf
...
| ps qs@(ex.zero qs_i) := do
  o <- ex_one,
  pf <- mk_proof ``pow_pf_zero 3 [] [ps.info, qs.info],
  prod_to_sum $ o.set_proof pf
| ps qqs@(ex.sum qqs_i q qs) := do
  psq <- pow_p ps q,
  psqs <- pow ps qs,
  psqqs <- mul psq psqs,
  pf <- mk_proof ``pow_pf_sum 5 [] [qqs.info, psqqs.info],
  pure $ psqqs.set_proof pf
...
def inverse_pf [has_inv α] {ps ps_u ps_p e' e'' : α} :
...
meta def inverse (ps : ex_pf sum) : ring_exp_m (ex_pf sum) := do
  ...
  e'' <- eval_atom e',
  ps_pf <- ps.proof_term,
  pf <- mk_proof_refl e''.pretty ``inverse_pf 6 [ps_pf, ps_pretty_pf, e_pf] [e''.info],
  pure $ e''.set_proof pf
...
| e@`(%%ps - %%qs) := (do
  ...
  lift $ trace e >> trace ps'.repr >> trace qs'.repr,
  psqs <- add ps' qs',
  pf <- mk_proof_refl psqs.pretty ``sub_pf 4 [] [psqs.info],
...
| e@`(%%ps / %%qs) := (do
  ...
  lift $ trace e >> trace ps'.repr >> trace qs'.repr,
  psqs <- mul ps' qs',
  pf <- mk_proof_refl psqs.pretty ``div_pf 4 [] [psqs.info],
  pure (psqs.set_proof pf)) <|> eval_atom e
...
meta def make_eval_info (α : expr) : tactic eval_info := do
  ...
  pure (eval_info.mk α u csr_instance cr_instance hp_instance)
