...
/--
This is a "finishing" tactic modification of `simv`. It has two forms.
...
* `simpa [rules, ...] using e` will simplify the goal and the type of
  ...
  more robust under changes to the simv lemma set.
...
let simp_at lc (close_tac : tactic unit) := focus1 $
  simv use_iota_eqn trace_lemmas no_dflt hs attr_names (loc.ns lc)
...
| some e := focus1 $ do
  ...
  { ty ← target,
    ...
    pty ← pp ty, ptgt ← pp e,
    -- Fail deliberately, to advise regarding `simv; exact` usage
