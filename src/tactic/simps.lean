...
-/
import tactic.core category.traversable.basic
/-!
  # simps attribute
...
do decl_value ← match pr with
                | none :=
                  do refl_ap ← mk_app `eq.refl [type, lhs],
                     pure $ refl_ap.lambdas args
                | (some pr) := pure pr
                end,
  ...
  if e.is_structure str then do
    ...
    if is_constant_of (get_app_fn rhs_ap) intro then do -- if the value is a constructor application
      ...
      when ¬(todo = [""] ∨ (eta.is_some ∧ todo = [])) $ do
        ...
        projs'.mmap' $ λ ⟨proj,lmm⟩,
        try $
        do { d ← get_decl lmm,
             lmm ← mk_const lmm,
             new_lhs ← mk_mapp proj $ t_params.map some,
             (new_rhs, prf, _) ← rewrite lmm (new_lhs rhs_ap),
...
meta def simps_tac (nm : name) (add_simp : bool) (short_nm : bool := ff)
  ...
  simps_add_projections e nm "" d.type lhs d.value none [] d.univ_params add_simp tt short_nm todo
