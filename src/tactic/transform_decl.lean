...
private meta def transform_decl_with_prefix_fun_aux (f : name → option name) (ignore_fixed : bool)
...
-/
meta def transform_decl_with_prefix_fun (f : name → option name) (ignore_fixed : bool)
  (src tgt : name) (attrs : list name) : command :=
do transform_decl_with_prefix_fun_aux f ignore_fixed src tgt attrs src,
   ls ← get_eqn_lemmas_for tt src,
   ls.mmap' $ transform_decl_with_prefix_fun_aux f ignore_fixed src tgt attrs
...
This is used to implement `@[to_additive]`.
-/
meta def transform_decl_with_prefix_dict (dict : name_map name) (ignore_fixed : bool)
  (src tgt : name) (attrs : list name) : command :=
transform_decl_with_prefix_fun dict.find ignore_fixed src tgt attrs
