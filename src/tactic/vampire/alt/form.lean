...
| (# k)    := 0
| (t &t s) := max t.fresh_vdx s.fresh_vdx
| (t &v k) := max t.fresh_vdx (k + 1)

def fresh_fdx : term → nat
| (# k)    := k + 1
...
| ($ k)    := 0
| (a ^t t) := max a.fresh_vdx t.fresh_vdx
| (a ^v k) := max a.fresh_vdx (k + 1)

def fresh_rdx : atom → nat
| ($ k)    := k + 1
| (a ^t _) := a.fresh_rdx
| (a ^v _) := a.fresh_rdx

def fresh_fdx : atom → nat
| ($ k)    := 0
| (a ^t t) := max a.fresh_fdx t.fresh_fdx
| (a ^v _) := a.fresh_fdx
...
def vdx_occ (k : nat) : atom → Prop
| ($ _)    := false
| (a ^t t) := a.vdx_occ ∨ t.vdx_occ k
| (a ^v m) := a.vdx_occ ∨ m = k
...
def fresh_rdx : lit → nat
| (-* a) := a.fresh_rdx
| (+* a) := a.fresh_rdx

def fresh_fdx : lit → nat
| (-* a) := a.fresh_fdx
| (+* a) := a.fresh_fdx

def fresh_vdx : lit → nat
| (-* a) := a.fresh_vdx
| (+* a) := a.fresh_vdx

def incr_fdx : lit → lit
| (-* a) := -* a.incr_fdx
| (+* a) := +* a.incr_fdx
...
def fresh_rdx : form → nat
| (form.lit l)     := l.fresh_rdx
| (form.bin _ f g) := max f.fresh_rdx g.fresh_rdx
| (form.qua _ f)   := f.fresh_rdx

def fresh_fdx : form → nat
| (form.lit l)     := l.fresh_fdx
| (form.bin _ f g) := max (f.fresh_fdx) (g.fresh_fdx)
| (form.qua _ f)   := f.fresh_fdx

def fresh_vdx : form → nat
| (form.lit l)     := l.fresh_vdx
...
def incr_fdx : form → form
| (form.lit l)     := form.lit l.incr_fdx
| (form.bin b f g) := form.bin b f.incr_fdx g.incr_fdx
| (form.qua b f)   := form.qua b f.incr_fdx
...
-- | (k + 1) R m F f := ∀ x : rl α, f.frxffx k (R ₀↦ x) m F
...
def vdx_occ : form → nat → Prop
| (form.lit l)     k := l.vdx_occ k
| (form.bin _ f g) k := f.vdx_occ k ∨ g.vdx_occ k
| (form.qua _ f)   k := f.vdx_occ (k + 1)
...
def holds (R : rls α) (F : fns α) (V : vas α) : qf → Prop
| (qf.lit l)      := l.holds R F V
| (qf.bin tt f g) := f.holds ∨ g.holds
| (qf.bin ff f g) := f.holds ∧ g.holds

def incr_fdx : qf → qf
| (qf.lit l)     := qf.lit l.incr_fdx
| (qf.bin b f g) := qf.bin b f.incr_fdx g.incr_fdx

def subst (k : nat) (t : term) : qf → qf
...
def pnf.get_qf : pnf → qf
| (pnf.qf f)    := f
| (pnf.qua _ f) := f.get_qf

namespace pnf

def incr_fdx : pnf → pnf
...
def qua_count : pnf → nat
| (pnf.qf f)    := 0
| (pnf.qua b f) := f.qua_count + 1
...
def univ : pnf → Prop
| (pnf.qf _)    := true
| (pnf.qua b f) := b = ff ∧ univ f

def holds (R : rls α) (F : fns α) : vas α → pnf → Prop
| V (pnf.qf f)     := f.holds R F V
| V (pnf.qua ff f) := ∀ x : α, f.holds (V ₀↦ x)
| V (pnf.qua tt f) := ∃ x : α, f.holds (V ₀↦ x)
...
lemma term.val_eq_val (V W : vas α) :
  ∀ t : term, (∀ m : nat, t.vdx_occ m → V m = W m) →
  (t.val F V = t.val F W)
| (# _)    _  := rfl
| (t &t s) h0 :=
  begin
...
lemma atom.val_eq_val (R) (F) (V W : vas α) :
  ∀ a : atom, (∀ m : nat, a.vdx_occ m → V m = W m) →
...
lemma term.lt_of_fresh_vdx_le (k m : nat) :
  ∀ t : term, t.fresh_vdx ≤ k →
  t.vdx_occ m → m < k
| (# _)    h0 h1 := by cases h1
| (t &t s) h0 h1 :=
  begin
    cases h1;
    apply term.lt_of_fresh_vdx_le _ _ h1,
...
lemma atom.lt_of_fresh_vdx_le (k m : nat) :
  ∀ a : atom, a.fresh_vdx ≤ k →
  a.vdx_occ m → m < k
| ($ _)    h0 h1 := by cases h1
| (a ^t t) h0 h1 :=
  begin
    cases h1,
    { apply atom.lt_of_fresh_vdx_le _ _ h1,
      apply le_of_max_le_left h0 },
    apply term.lt_of_fresh_vdx_le _ _ _ _ h1,
    apply le_of_max_le_right h0
  end
| (a ^v n) h0 h1 :=
  begin
...
lemma form.lt_of_fresh_vdx_le :
  ∀ {f : form}, ∀ {k m : nat},
  f.fresh_vdx ≤ k → f.vdx_occ m → m < k
| (form.lit l) k m h0 h1 :=
  by { cases l with a a;
       apply atom.lt_of_fresh_vdx_le _ _ _ h0 h1 }
| (form.bin _ f g) k m h0 h1 :=
  begin
    cases h1;
    apply form.lt_of_fresh_vdx_le _ h1,
    { apply le_of_max_le_left h0 },
    apply le_of_max_le_right h0
  end
| (form.qua _ f) k m h0 h1 :=
  begin
    ...
    apply @form.lt_of_fresh_vdx_le f (k + 1) (m + 1) _ h1,
    unfold form.fresh_vdx at h0,
...
-- lemma lit.holds_neg (M : model α) (v : nat → α) (l : lit) :
...
lemma holds_of_forall_vxt_holds {k : nat} {f : form} :
  f.fresh_vdx ≤ k → (∀^ V' ∷ k ⇒ V, f.holds R F V') →
  ∀ W : vas α, (R ; F ; W ⊨ f) :=
  begin
    intros h0 h1 W,
    ...
    unfold prepend,
    rw if_pos,
    apply form.lt_of_fresh_vdx_le h0 h2,
  end

--   #exit
-- lemma fv_of_fvx (k : nat) (f : form) :
--   f.fresh_vdx ≤ k → f.fvx R F k V →
--   ∀ W : vas α, (R ; F ; W ⊨ f) :=
--   begin
--     intros h0 h1 W,
--     rw holds_iff_holds f W (prepend k W V) _,
--     { apply forall_ext_holds_of_fvx f k V h1
--         (prepend k W V) (ext_prepend _ _ _) },
--     intros m h2,
--     unfold prepend,
--     rw if_pos,
--     apply form.lt_of_fresh_vdx_le _ _ _ h0 h2
--   end
...
lemma forall_vxt_succ {k : nat} {f : form} :
  (∀^ V' ∷ k ⇒ V, (∀* f).holds R F V') →
...
begin
  ...
  { apply funext,
    rintro ⟨m⟩; refl },
  rw h2,
  apply h0 (V' ∘ nat.succ) (ext_comp_of_ext_succ h1) (V' 0)
end

lemma fresh_vdx_neg :
  ∀ f : form, f.neg.fresh_vdx = f.fresh_vdx
| (form.lit l) := by {cases l; refl}
| (form.bin _ f g) :=
  by simp only [form.neg, form.fresh_vdx,
       fresh_vdx_neg f, fresh_vdx_neg g]
| (form.qua _ f)   :=
  by simp only [form.neg, form.fresh_vdx, fresh_vdx_neg f]
...
| (∀* f) k h0 h1 h2 W :=
  begin
    ...
    unfold form.fresh_vdx at h1,
    unfold form.strip_fa,
    have h3 := forall_vxt_succ h2,
    apply holds_strip_fa h0 _ h3,
    cases f.fresh_vdx,
    { apply nat.zero_le },
...
lemma holds_strip_fa (f : form) :
  ...
  ∀ W : vas α, (R ; F ; W ⊨ f.strip_fa) :=
begin
  intros h0 W, unfold form.strip_fa,
  by_cases h1 : (f.AF ∧ form.fresh_vdx f = 0),
  { rw if_pos h1,
    apply holds_strip_fa_core f 0 h1.left (le_of_eq h1.right) h0 },
  rw if_neg h1,
  apply classical.em
end
...
local notation `∀^` := fa_ext


local notation `∃^` := ex_ext

#check @list
-- notation `∃^` k `;` f `;` binders  `,` r:(scoped p, ex_ext k f p) := r


  #exit
| 0        F f h := h
| (fn + 1) F f h := λ x, ffx_of_ffx (h x.unfix)
...
lemma efx_neg :
  ∀ fn : nat, ∀ F : fns α, ∀ f : form,
  f.neg.efx R fn F ↔ ¬ (f.ffx R fn F)
| 0       F f := holds_neg
| (k + 1) F f :=
  begin
    unfold form.efx,
    ...
    apply exists_congr,
    intro a, apply efx_neg,
  end

lemma erxefx_neg :
  ∀ (rn : nat) (R : rls α) (fn : nat) (F : fns α) (f : form),
  f.neg.erxefx rn R fn F ↔ ¬ (f.frxffx rn R fn F)
| 0       R fn F f := efx_neg _ _ _
| (k + 1) R fn F f :=
  begin
    unfold form.erxefx,
    unfold form.frxffx,
    rw classical.not_forall,
    apply exists_congr,
    intro a, apply erxefx_neg,
  end

lemma ffx_of_ffx [inhabited α] {R : rls α} :
  ∀ {fn : nat} {F : fns α} {f : form},
  f.ffx R fn F → f.ffx R fn F
| 0        F f h := h
| (fn + 1) F f h := λ x, ffx_of_ffx (h x.unfix)

lemma frxffx_of_frxffx [inhabited α] :
  ∀ {rn : nat} {R : rls α} {fn : nat} {F : fns α} {f : form},
  f.frxffx rn R fn F → f.frxffx rn R fn F
| 0        R fn F f h := ffx_of_ffx h
| (rn + 1) R fn F f h := λ x, frxffx_of_frxffx (h x.unfix)
