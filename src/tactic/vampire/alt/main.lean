...
tactic.vampire.misc
data.list.min_max
tactic.vampire.alt.pull
tactic.vampire.alt.reify
tactic.vampire.alt.cnf
tactic.vampire.alt.skolemize
...
lemma cla.holds_substs {μs : mappings} {c : cla} :
  ...
  :=
by { apply (@list.exists_mem_iff_exists_mem_map lit lit),
...
begin
  ...
  rw ← prenexify_eqv at h3,
  rcases holds_skolemize h3 with ⟨F'', h4, h5⟩,
  have hAF : (skolemize (prenexify (form.neg f))).AF,
  { apply AF_skolemize,
    apply QF_prenexify },
...
do πx ← build_proof_core m m.to_expr [] chs,
   let rnx : expr := f.fresh_rdx.to_expr,
   let fnx : expr := f.fresh_fdx.to_expr,
   let Rx : expr := `(rls.default %%αx),
   let Fx : expr := `(@fns.default %%αx %%ix),
   let fx : expr := f.to_expr,
   let x : expr := expr.mk_app `(@frxffx_of_proof) [αx, ix, rnx, Rx, fnx, Fx, fx, πx],
   return x
...
#exit
local notation `#`      := term.fn
local notation t `&a` s := term.app t s
local notation t `&v` k := term.vpp t k
