...
This file provides lemmas about the interaction of infinite sums and (scalar) multiplication.
-/

open filter finset function
open_locale big_operators classical

variables {R α β γ : Type*}

section const_smul
variables [monoid R] [topological_space α] [add_comm_monoid α] [distrib_mul_action R α]
  [has_continuous_const_smul R α] {f : β → α}
...
section smul_const
variables [semiring R] [topological_space R] [topological_space α] [add_comm_monoid α] [module R α]
  [has_continuous_smul R α] {f : β → R}
...
lemma has_sum.mul_left (a₂) (h : has_sum f a₁) : has_sum (λ b, a₂ * f b) (a₂ * a₁) :=
by simpa only using h.map (add_monoid_hom.mul_left a₂) (continuous_const.mul continuous_id)

lemma has_sum.mul_right (a₂) (hf : has_sum f a₁) : has_sum (λ b, f b * a₂) (a₁ * a₂) :=
by simpa only using hf.map (add_monoid_hom.mul_right a₂) (continuous_id.mul continuous_const)

lemma summable.mul_left (a) (hf : summable f) : summable (λ b, a * f b) :=
(hf.has_sum.mul_left _).summable

lemma summable.mul_right (a) (hf : summable f) : summable (λ b, f b * a) :=
...
variables [division_semiring α] [topological_space α] [topological_semiring α] {f g : β → α}
  {a a₁ a₂ : α}
...
lemma has_sum_mul_left_iff (h : a₂ ≠ 0) : has_sum (λ b, a₂ * f b) (a₂ * a₁) ↔ has_sum f a₁ :=
⟨λ H, by simpa only [inv_mul_cancel_left₀ h] using H.mul_left a₂⁻¹, has_sum.mul_left _⟩

lemma has_sum_mul_right_iff (h : a₂ ≠ 0) : has_sum (λ b, f b * a₂) (a₁ * a₂) ↔ has_sum f a₁ :=
⟨λ H, by simpa only [mul_inv_cancel_right₀ h] using H.mul_right a₂⁻¹, has_sum.mul_right _⟩

lemma has_sum_div_const_iff (h : a₂ ≠ 0) : has_sum (λ b, f b / a₂) (a₁ / a₂) ↔ has_sum f a₁ :=
by simpa only [div_eq_mul_inv] using has_sum_mul_right_iff (inv_ne_zero h)

lemma summable_mul_left_iff (h : a ≠ 0) : summable (λ b, a * f b) ↔ summable f :=
⟨λ H, by simpa only [inv_mul_cancel_left₀ h] using H.mul_left a⁻¹, λ H, H.mul_left _⟩

lemma summable_mul_right_iff (h : a ≠ 0) : summable (λ b, f b * a) ↔ summable f :=
⟨λ H, by simpa only [mul_inv_cancel_right₀ h] using H.mul_right a⁻¹, λ H, H.mul_right _⟩

lemma summable_div_const_iff (h : a ≠ 0) : summable (λ b, f b / a) ↔ summable f :=
