...
/-- If a scalar is central, then its right action is continuous automatically if its left action
is. -/
instance mul_opposite.has_continuous_smul [has_scalar Mᵐᵒᵖ α] [is_central_scalar M α] :
