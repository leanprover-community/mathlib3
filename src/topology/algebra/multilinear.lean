...
instance : has_coe_to_fun (continuous_multilinear_map R M₁ M₂) := ⟨_, λf, f.to_multilinear_map.to_fun⟩
...
⟨λf f', {cont := f.cont.add f'.cont, ..(f.to_multilinear_map + f'.to_multilinear_map)}⟩
...
lemma cons_smul (f : continuous_multilinear_map R M M₂) (m : Π(i : fin n), M i.succ) (c : R) (x : M 0) :
