  ...
  { rw [← map_le_iff_le_comap],
    ...
    { refine le_infi (λ x, le_infi $ λ h, le_principal_iff.2 _),
      ...
      { refine mem_infi_of_mem (Ioi ⟨x, hx⟩) (mem_infi_of_mem ⟨h, ⟨⟨x, hx⟩, or.inl rfl⟩⟩ _),
        exact λ _, id },
      simp only [set_coe.exists, mem_set_of, mem_map'],
      convert univ_sets _,
...
begin
  ...
  { have : s ⊆ ⋃ (a ∈ countable_basis α), {x | x ∈ s ∧ x ∈ a ∧ y x ∉ a},
    { assume x hx,
      ...
      simp only [mem_set_of, mem_Union],
      exact ⟨a, ab, hx, xa, ya⟩ },
  ...
  { have : {x | x ∈ s ∧ x ∈ a ∧ y x ∉ a} ⊆
    ...
    { assume x hx,
      ...
      { simp only [h'x, mem_set_of, mem_union_eq, not_true, and_false, false_or] },
      { simpa only [h'x, hx.2.1, hx.2.2, mem_set_of, mem_union_eq,
