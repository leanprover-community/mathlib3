...
/-
lemma F_condition [nonempty α] (F : set α) : F = univ ↔ (upper_closure F : set α)ᶜ = ∅ :=
begin
  split,
  { intro h,
    rw h,
    simp only [upper_closure_univ, upper_set.coe_bot, compl_univ], },
  { intro h,
    --simp at h,
  sorry,}
 --rw upper_closure_empty
end
-/
...
begin
  ...
  { rw subset_antisymm_iff,
    ...
    { refine subset_sUnion_of_mem _,
      ...
      use ∅,
      rw univ_empty,
      simp only [finite_empty, eq_self_iff_true, and_self],
       } },
