...
instance [p : nonempty α] : nonempty (lower α) := p
instance [p : inhabited α] : inhabited (lower α) := p
...
instance lower.topological_space : topological_space (lower α) := lower_topology (lower α)
...
lemma is_upper_set_of_is_closed {s : set (lower α)} (h : is_closed s) : is_upper_set s :=
