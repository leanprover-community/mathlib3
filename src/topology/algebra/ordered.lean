...
equivalently by those of the form `(-∞, a)` and `(b, +∞)`). We introduce a mixin `order_topology α`
saying that on the type α having a topological space structure and a preorder structure, the
topological structure corresponds precisely to the order topology.

We also introduce a mixin `order_closed_topology α` saying that the set of points `(x, y)` with
`x ≤ y` is closed in the product space. This is automatically satisfied on a linear order with the
order topology.
...
vs `order_topology`, `preorder` vs `partial_order` vs `linear_order` etc) see their
statements.
...
definition `order_topology α` though, that can be registered as an instance when necessary, or for
specific types.
-/
...
variables [topological_space α] [decidable_linear_order α] [t : order_closed_topology α] {f g : β → α}
include t
...
@[nolint] lemma tendsto_order {f : β → α} {a : α} {x : filter β} :
  tendsto f x (𝓝 a) ↔ (∀a'<a, {b | a' < f b} ∈ x) ∧ (∀a'>a, {b | f b < a'} ∈ x) :=
...
tendsto_order.2
  ⟨assume a' h',
    have {b : β | a' < g b} ∈ b, from (tendsto_order.1 hg).left a' h',
    by filter_upwards [this, hgf] assume a, lt_of_lt_of_le,
    assume a' h',
    have {b : β | h b < a'} ∈ b, from (tendsto_order.1 hh).right a' h',
...
@[nolint] theorem induced_order_topology' {α : Type u} {β : Type v}
...
variables [topological_space α] [linear_order α] [t : order_topology α]
include t
...
@[nolint] lemma is_cobounded_ge_nhds (a : α) : (𝓝 a).is_cobounded (≥) :=
is_cobounded_of_is_bounded nhds_neq_bot (is_bounded_le_nhds a)

@[nolint] lemma is_cobounded_under_ge_of_tendsto {f : filter β} {u : β → α} {a : α}
...
@[nolint] theorem gt_mem_sets_of_Liminf_gt : ∀ {f : filter α} {b}, f.is_bounded (≥) → f.Liminf > b →
  {a | a > b} ∈ f :=
...
@[nolint] theorem le_nhds_of_Limsup_eq_Liminf {f : filter α} {a : α}
...
@[nolint] lemma order_topology_of_nhds_abs
...
@[nolint] lemma decidable_linear_ordered_comm_group.tendsto_nhds
  [decidable_linear_ordered_comm_group α] [topological_space α] [order_topology α] {β : Type*}
  (f : β → α) (x : filter β) (a : α) :
  filter.tendsto f x (nhds a) ↔ ∀ ε > (0 : α), {b : β | |f b - a| < ε} ∈ x :=
begin
