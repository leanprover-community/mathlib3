...
the left and to the right, either open or closed, i.e., members of `𝓝ᵣ' a` and
`𝓝ᵣ a` on the right, and similarly on the left. Here we simply prove that all
...
lemma tfae_mem_nhds_within_Ioi {a b : α} (hab : a < b) (s : set α) :
  tfae [s ∈ 𝓝ᵣ' a, -- 0 : `s` is a neighborhood of `a` within `(a, +∞)`
...
lemma mem_nhds_within_Ioi_iff_exists_Ioc_subset [no_top_order α] [densely_ordered α]
  {a : α} {s : set α} : s ∈ 𝓝ᵣ' a ↔ ∃u ∈ Ioi a, Ioc a u ⊆ s :=
begin
...
lemma tfae_mem_nhds_within_Iio {a b : α} (h : a < b) (s : set α) :
  tfae [s ∈ 𝓝ₗ' b, -- 0 : `s` is a neighborhood of `b` within `(-∞, b)`
...
lemma mem_nhds_within_Ici_iff_exists_Icc_subset' [no_top_order α] [densely_ordered α]
  {a : α} {s : set α} : s ∈ 𝓝ᵣ a ↔ ∃u ∈ Ioi a, Icc a u ⊆ s :=
begin
...
lemma mem_nhds_within_Iic_iff_exists_Icc_subset' [no_bot_order α] [densely_ordered α]
  {a : α} {s : set α} : s ∈ 𝓝ₗ a ↔ ∃l ∈ Iio a, Icc l a ⊆ s :=
begin
...
with `a < u`. -/
lemma mem_nhds_within_Ici_iff_exists_Icc_subset [no_top_order α] [densely_ordered α]
  {a : α} {s : set α} : s ∈ 𝓝ᵣ a ↔ ∃u, a < u ∧ Icc a u ⊆ s :=
begin
  rw mem_nhds_within_Ici_iff_exists_Ico_subset,
  split,
  { rintros ⟨u, au, as⟩,
...
with `l < a`. -/
lemma mem_nhds_within_Iic_iff_exists_Icc_subset [no_bot_order α] [densely_ordered α]
  {a : α} {s : set α} : s ∈ 𝓝ₗ a ↔ ∃l, l < a ∧ Icc l a ⊆ s :=
begin
...
lemma tendsto_abs_nhds_within_zero : tendsto (abs : α → α) (𝓝' 0) (𝓝ᵣ' 0) :=
...
lemma tendsto_inv_zero_at_top : tendsto (λx:α, x⁻¹) (𝓝ᵣ' (0:α)) at_top :=
begin
...
lemma tendsto_inv_at_top_zero' : tendsto (λr:α, r⁻¹) at_top (𝓝ᵣ' (0:α)) :=
begin
...
lemma filter.tendsto.inv_tendsto_zero (h : tendsto f l (𝓝ᵣ' 0)) :
  tendsto (f⁻¹) l at_top :=
...
lemma comap_coe_Ioo_nhds_within_Iio (a b : α) :
  comap (coe : Ioo a b → α) (𝓝ₗ' b) = at_top :=
...
lemma comap_coe_Ioo_nhds_within_Ioi (a b : α) :
  comap (coe : Ioo a b → α) (𝓝ᵣ' a) = at_bot :=
comap_coe_nhds_within_Ioi_of_Ioo_subset Ioo_subset_Ioi_self $
  λ h, ⟨b, nonempty_Ioo.1 h, subset.refl _⟩

lemma comap_coe_Ioi_nhds_within_Ioi (a : α) : comap (coe : Ioi a → α) (𝓝ᵣ' a) = at_bot :=
...
@[simp] lemma tendsto_Iio_at_top {f : β → Iio a} :
  tendsto f l at_top ↔ tendsto (λ x, (f x : α)) l (𝓝ₗ' a) :=
