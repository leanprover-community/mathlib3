...
lemma exists_seq_strict_mono_tendsto_of_is_lub_of_not_mem' {t : set α} {x : α}
...
lemma exists_seq_monotone_tendsto_of_is_lub' {t : set α} {x : α}
...
begin
  ...
  { rcases exists_seq_strict_mono_tendsto_of_is_lub_of_not_mem' htx h ht hx with ⟨u, hu⟩,
    exact ⟨u, hu.1.monotone, λ n, (hu.2.1 n).le, hu.2.2⟩ }
end

lemma exists_seq_strict_mono_tendsto_of_is_lub_of_not_mem [first_countable_topology α]
  {t : set α} {x : α} (htx : is_lub t x) (ht : t.nonempty) (not_mem : x ∉ t) :
  ∃ u : ℕ → α, strict_mono u ∧ (∀ n, u n < x) ∧ tendsto u at_top (𝓝 x) ∧ (∀ n, u n ∈ t) :=
exists_seq_strict_mono_tendsto_of_is_lub_of_not_mem' htx not_mem ht (is_countably_generated_nhds x)

lemma exists_seq_monotone_tendsto_of_is_lub [first_countable_topology α]
  {t : set α} {x : α} (htx : is_lub t x) (ht : t.nonempty) :
  ∃ u : ℕ → α, monotone u ∧ (∀ n, u n ≤ x) ∧ tendsto u at_top (𝓝 x) ∧ (∀ n, u n ∈ t) :=
exists_seq_monotone_tendsto_of_is_lub' htx ht (is_countably_generated_nhds x)
...
begin
  ...
  rcases exists_seq_strict_mono_tendsto_of_is_lub_of_not_mem is_lub_Iio ht hx with ⟨u, hu⟩,
...
begin
  rcases exists_seq_monotone_tendsto_of_is_lub (is_lub_cSup hS hS') hS with ⟨u, hu⟩,
  exact ⟨u, hu.1, hu.2.2⟩,
end

lemma exists_seq_strict_mono_tendsto_of_is_glb_of_not_mem' {t : set α} {x : α}
...
@exists_seq_strict_mono_tendsto_of_is_lub_of_not_mem' (order_dual α) _ _ _ t x htx not_mem ht hx

lemma exists_seq_monotone_tendsto_of_is_glb' {t : set α} {x : α}
  (htx : is_glb t x) (ht : t.nonempty) (hx : is_countably_generated (𝓝 x)) :
  ∃ u : ℕ → α, (∀ m n, m ≤ n → u n ≤ u m) ∧ (∀ n, x ≤ u n)
    ∧ tendsto u at_top (𝓝 x) ∧ (∀ n, u n ∈ t) :=
@exists_seq_monotone_tendsto_of_is_lub' (order_dual α) _ _ _ t x htx ht hx

lemma exists_seq_strict_mono_tendsto_of_is_glb_of_not_mem [first_countable_topology α]
  {t : set α} {x : α} (htx : is_glb t x) (ht : t.nonempty) (not_mem : x ∉ t) :
  ∃ u : ℕ → α, (∀ m n, m < n → u n < u m) ∧ (∀ n, x < u n)
    ∧ tendsto u at_top (𝓝 x) ∧ (∀ n, u n ∈ t) :=
exists_seq_strict_mono_tendsto_of_is_glb_of_not_mem' htx not_mem ht (is_countably_generated_nhds x)

lemma exists_seq_monotone_tendsto_of_is_glb [first_countable_topology α]
  {t : set α} {x : α} (htx : is_glb t x) (ht : t.nonempty) :
  ∃ u : ℕ → α, (∀ m n, m ≤ n → u n ≤ u m) ∧ (∀ n, x ≤ u n)
    ∧ tendsto u at_top (𝓝 x) ∧ (∀ n, u n ∈ t) :=
exists_seq_monotone_tendsto_of_is_glb' htx ht (is_countably_generated_nhds x)
