...
lemma tendsto_const_mul_fzpow_at_top_zero {n : ℤ} {c : α} (hn : n < 0) :
  tendsto (λ x, c * x ^ n) at_top (𝓝 0) :=
(mul_zero c) ▸ (filter.tendsto.const_mul c (tendsto_fzpow_at_top_zero hn))
...
lemma tendsto_const_mul_fzpow_at_top_zero_iff {n : ℤ} {c d : α} (hc : c ≠ 0) :
...
begin
  ...
  { by_cases hn : 0 ≤ n,
    { lift n to ℕ using hn,
      ...
      exact or.inl h },
    { rw not_le at hn,
      refine or.inr ⟨hn, tendsto_nhds_unique h (tendsto_const_mul_fzpow_at_top_zero hn)⟩ } },
  { cases h,
    { simp only [h.left, h.right, zpow_zero, mul_one],
      exact tendsto_const_nhds },
    { exact h.2.symm ▸ tendsto_const_mul_fzpow_at_top_zero h.1} }
end
