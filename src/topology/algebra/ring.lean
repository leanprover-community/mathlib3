...
@[simp] lemma to_topological_space_le {x y : ring_topology α} :
  x.to_topological_space ≤ y.to_topological_space ↔ x ≤ y := iff.rfl

local notation `cont` := @continuous _ _

instance : has_Inf (ring_topology α) :=
⟨λ S, let Inf_S' := Inf (to_topological_space '' S) in
...
complete_lattice_of_Inf (ring_topology α) $ λ S, ⟨
  λ a haS, to_topological_space_le.1 $ Inf_le ⟨a, ⟨haS, rfl⟩⟩,
  λ a hab, to_topological_space_le.1 $ le_Inf $ by { rintros _ ⟨b, hbS, rfl⟩, exact hab hbS }
⟩ 
