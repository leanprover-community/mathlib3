...
lemma ring_sep_rel (α) [comm_ring α] [uniform_space α] [uniform_add_group α]
  [topological_semiring α] : separation_setoid α = submodule.quotient_rel (ideal.closure ⊥) :=
...
/-- Given a topological semiring `α` equipped with a uniform structure that makes subtraction
