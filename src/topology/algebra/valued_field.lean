 ...
 begin
  ...
  { have preimage_one : v ⁻¹' {(1 : Γ₀)} ∈ 𝓝 (1 : K),
    { have : (v (1 : K) : Γ₀) ≠ 0, { rw valuation.map_one, exact zero_ne_one.symm },
      ...
      ext x,
      rw [valuation.map_one, mem_preimage, mem_singleton_iff, mem_set_of] },
...
begin
  ...
  { cases eq_or_ne γ₀ 0,
    { simp only [h, (valuation.zero_iff _).mp h, mem_set_of, valuation.map_zero, units.zero_lt,
        iff_true],
      apply subset_closure,
      exact ⟨0, by simpa only [mem_set_of, valuation.map_zero, units.zero_lt, true_and]⟩, },
    { exact this h, }, },
  intros h,
