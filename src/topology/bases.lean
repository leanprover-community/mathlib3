...
such that taking the union of these sets generates all open sets, without the need to take
...
## Main definitions
* `is_topological_basis s`: Indicates whether the topology `t` is generated by `s`, but only
  allowing unions of sets in `s`.

* `separable_space α`: Indicates whether there is a countable, dense subset of `t`.

* `first_countable_topology α`: A topology which, at any point `x`, `𝓝 x` is countably generated.
...
* `second_countable_topology.countable_cover_nhds`: Consider a function `f` with the property that
...
/-- A point `a` is in the closure of `s` iff all basis sets containing `a` overlap `s`. -/
...
/-- In a first-countable space, all neighbourhoods are countably generated. -/
...
/-- A countable topological basis of `α`. The next few lemmas show that it behaves as expected. -/
