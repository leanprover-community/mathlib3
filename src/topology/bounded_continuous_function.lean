...
/-- Constructing an element of a normed group from a continuous bounded function -/
def normed_group.mk {α : Type u} {β : Type v} [topological_space α] [normed_group β]
...
/-- Constructing an element of a normed group from a bounded function on a
    discrete space -/
def normed_group.mk' {α : Type u} {β : Type v}
  [topological_space α] [discrete_topology α] [normed_group β]
  (f : α  → β) (C : ℝ) (H : ∀x, norm (f x) ≤ C) : α →ᵇ β :=
normed_group.mk f C H continuous_of_discrete_topology
