...
begin
  ...
  refine ⟨j0, ggg, _⟩,
  have : f.map ι = locally_constant.unflip (f.map ι).flip, by simv,
  rw this, clear this,
  have : locally_constant.comap (C.π.app j0) ggg =
    locally_constant.unflip (locally_constant.comap (C.π.app j0) ggg).flip, by simv,
  rw this, clear this,
  congr' 1,
  ext1 a,
  change ff a = _,
...
begin
  ...
  { suffices : ∃ j, is_empty (F.obj j),
    ...
    simv only [← not_nonempty_iff, ← not_forall],
    intros h,
