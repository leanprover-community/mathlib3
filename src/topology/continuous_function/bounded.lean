...
variables [topological_space α] [seminormed_add_group β]
variables (f g : α →ᵇ β) {x : α} {C : ℝ}
...
def of_normed_add_group {α : Type u} {β : Type v} [topological_space α] [seminormed_add_group β]
  (f : α → β) (Hf : continuous f) (C : ℝ) (H : ∀x, ∥f x∥ ≤ C) : α →ᵇ β :=
⟨⟨λn, f n, Hf⟩, ⟨_, dist_le_two_norm' H⟩⟩

@[simp] lemma coe_of_normed_add_group
  {α : Type u} {β : Type v} [topological_space α] [seminormed_add_group β]
  (f : α → β) (Hf : continuous f) (C : ℝ) (H : ∀x, ∥f x∥ ≤ C) :
  (of_normed_add_group f Hf C H : α → β) = f := rfl

lemma norm_of_normed_add_group_le {f : α → β} (hfc : continuous f) {C : ℝ} (hC : 0 ≤ C)
  (hfC : ∀ x, ∥f x∥ ≤ C) : ∥of_normed_add_group f hfc C hfC∥ ≤ C :=
(norm_le hC).2 hfC

/-- Constructing a bounded continuous function from a uniformly bounded
function on a discrete space, taking values in a normed group -/
def of_normed_add_group_discrete {α : Type u} {β : Type v}
  [topological_space α] [discrete_topology α] [seminormed_add_group β]
  (f : α  → β) (C : ℝ) (H : ∀x, norm (f x) ≤ C) : α →ᵇ β :=
of_normed_add_group f continuous_of_discrete_topology C H

@[simp] lemma coe_of_normed_add_group_discrete
  {α : Type u} {β : Type v} [topological_space α] [discrete_topology α] [seminormed_add_group β]
  (f : α → β) (C : ℝ) (H : ∀x, ∥f x∥ ≤ C) :
  (of_normed_add_group_discrete f C H : α → β) = f := rfl

/-- Taking the pointwise norm of a bounded continuous function with values in a `seminormed_add_group`,
...
⟨λf, of_normed_add_group (-f) f.continuous.neg ∥f∥ $ λ x,
  trans_rel_right _ (norm_neg _) (f.norm_coe_le_norm x)⟩

/-- The pointwise difference of two bounded continuous functions is again bounded continuous. -/
instance : has_sub (α →ᵇ β) :=
⟨λf g, of_normed_add_group (f - g) (f.continuous.sub g.continuous) (∥f∥ + ∥g∥) $ λ x,
...
instance : seminormed_add_group (α →ᵇ β) :=
{ dist_eq := λ f g, by simp only [norm_eq, dist_eq, dist_eq_norm, sub_apply] }

instance {α β} [topological_space α] [normed_add_group β] : normed_add_group (α →ᵇ β) :=
{ ..bounded_continuous_function.seminormed_add_group }
...
instance [normed_field 𝕜] [normed_space 𝕜 β] : normed_space 𝕜 (α →ᵇ β) := ⟨λ c f, begin
  refine norm_of_normed_add_group_le _ (mul_nonneg (norm_nonneg _) (norm_nonneg _)) _,
...
variables [seminormed_add_group γ] [normed_space 𝕜 γ]
...
linear_map.mk_continuous
  { to_fun := λ f, of_normed_add_group
      (g ∘ f)
  ...
  ∥g∥
  (λ f, norm_of_normed_add_group_le _ (mul_nonneg (norm_nonneg g) (norm_nonneg f)) _)
...
{ mul := λ f g, of_normed_add_group (f * g) (f.continuous.mul g.continuous) (∥f∥ * ∥g∥) $ λ x,
...
{ norm_mul := λ f g, norm_of_normed_add_group_le _ (mul_nonneg (norm_nonneg _) (norm_nonneg _)) _,
  .. bounded_continuous_function.seminormed_add_group }
...
{ .. bounded_continuous_function.non_unital_semi_normed_ring,
  .. bounded_continuous_function.normed_add_group }
...
{ .. bounded_continuous_function.comm_ring, .. bounded_continuous_function.seminormed_add_group }

instance [normed_comm_ring R] : normed_comm_ring (α →ᵇ R) :=
{ .. bounded_continuous_function.comm_ring, .. bounded_continuous_function.normed_add_group }
...
variables [topological_space α] [seminormed_add_group β] [normed_space 𝕜 β]
...
⟨λ (f : α →ᵇ 𝕜) (g : α →ᵇ β), of_normed_add_group (λ x, (f x) • (g x))
...
norm_of_normed_add_group_le _ (mul_nonneg (norm_nonneg _) (norm_nonneg _)) _
...
variables [topological_space α] [seminormed_add_group β] [star_add_monoid β] [normed_star_group β]
...
{ inf := λ f g,
  { to_fun := λ t, f t ⊓ g t,
    ...
    map_bounded' := begin
      ...
      refine ⟨C₁ + C₂, λ x y, _⟩,
      simp_rw normed_add_group.dist_eq at hf hg ⊢,
