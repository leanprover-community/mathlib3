...
begin
  ...
  { intros x z,
    ...
    dsimp [h],
    simp only [@coe_fn_coe_base' _ C(X, ℝ), subtype.coe_mk, sup'_coe, finset.sup'_apply,
               finset.lt_sup'_iff],
    exact ⟨y, ym, zm⟩ },
  have h_eq : ∀ x, h x x = f x,
  { intro x, simp only [@coe_fn_coe_base' _ C(X, ℝ)] at w₁,
    simp [@coe_fn_coe_base' _  C(X, ℝ), w₁], },
