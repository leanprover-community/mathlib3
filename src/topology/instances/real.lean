...
/-- Given a subgroup `G ⊆ ℝ`, if `a` is the minimum of `G ∩ ℝ_{>0}` then `G` is generated by `a`. -/
lemma real.subgroup_cyclic_of_min {G : add_subgroup ℝ} {a : ℝ}
  (ha : is_least {g : ℝ | g ∈ G ∧ 0 < g} a) : G = add_subgroup.closure {a} :=
begin
  ext g,
  rw add_subgroup.mem_closure_singleton,
  change g ∈ (G : set ℝ) ↔ _,
  simp [real.subgroup_eq_of_min ha]
end
...
`real.subgroup_cyclic_of_min` for more precise statements. -/
...
begin
  ...
  { let G_pos := {g : ℝ | g ∈ G ∧ 0 < g},
    ...
    { right,
      rcases H' with ⟨a, ha⟩,
      exact ⟨a, real.subgroup_cyclic_of_min ha⟩ },
    { left,
