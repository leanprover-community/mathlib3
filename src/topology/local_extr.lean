...
lemma is_local_min_on.not_nhds_le_map [topological_space β]
  (hf : is_local_min_on f s a) [ne_bot (𝓝ₗ' (f a))] :
  ¬𝓝 (f a) ≤ map f (𝓝[s] a) :=
λ hle,
have ∀ᶠ y in 𝓝ₗ' (f a), f a ≤ y,
...
lemma is_local_max_on.not_nhds_le_map [topological_space β]
  (hf : is_local_max_on f s a) [ne_bot (𝓝ᵣ' (f a))] :
  ¬𝓝 (f a) ≤ map f (𝓝[s] a) :=
...
lemma is_local_extr_on.not_nhds_le_map [topological_space β]
  (hf : is_local_extr_on f s a) [ne_bot (𝓝ₗ' (f a))] [ne_bot (𝓝ᵣ' (f a))] :
  ¬𝓝 (f a) ≤ map f (𝓝[s] a) :=
