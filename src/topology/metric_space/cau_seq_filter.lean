...
/-
 ...
 specific to norm. Furthermore, norm only instantiates is_absolute_value on valued_field.
 This needs to be fixed, since it prevents showing that ℤ_[hp] is complete
-/

instance valued_field.is_absolute_value : is_absolute_value (norm : β → ℝ) :=
...
lemma cau_seq_iff_cauchy_seq {α : Type u} [valued_field α] {u : ℕ → α} :
