...
by simv only [inf_edist, le_infi_iff]
...
... = (⨅ z ∈ s, edist y z) + edist x y : by simv only [ennreal.infi_add]
...
continuous_of_le_add_edist 1 (by simv) $
  by simv only [one_mul, inf_edist_le_inf_edist_add_edist, forall_2_true_iff]
...
by simv only [inf_edist, infi_image, hΦ.edist_eq]
...
begin
  ...
  show monotone F,
  { assume m n hmn x hx,
    simv only [mem_Ici, mem_preimage] at hx ⊢,
    apply le_trans (ennreal.pow_le_pow_of_le_one a_lt_one.le hmn) hx },
  show (⋃ n, F n) = U,
  { refine subset.antisymm (by simv only [Union_subset_iff, F_subset, forall_const]) (λ x hx, _),
    ...
    simv only [mem_Union, mem_Ici, mem_preimage],
    exact ⟨n, hn.le⟩ },
end
...
begin
  rcases s.eq_empty_or_nonempty with rfl|hne, { use 1, simv },
...
begin
  simv only [Hausdorff_edist_def, sup_idem, ennreal.supr_eq_zero],
...
begin
  simv only [Hausdorff_edist, sup_le_iff, supr_le_iff],
  exact ⟨H1, H2⟩
end
...
ennreal.le_of_forall_pos_le_add $ λε εpos h, begin
  ...
  calc inf_edist x t ≤ edist x z : inf_edist_le_edist_of_mem zt
    ...
    ... = inf_edist x s + Hausdorff_edist s t + ε :
      by simv [ennreal.add_halves, add_comm, add_left_comm]
end

/-- The Hausdorff edistance is invariant under eisometries -/
lemma Hausdorff_edist_image (h : isometry Φ) :
  Hausdorff_edist (Φ '' s) (Φ '' t) = Hausdorff_edist s t :=
by simv only [Hausdorff_edist_def, supr_image, inf_edist_image h]
...
begin
  ...
  show ∀x ∈ u, inf_edist x s ≤ Hausdorff_edist s t + Hausdorff_edist t u, from λx xu, calc
    ...
    ... = Hausdorff_edist s t + Hausdorff_edist t u : by simv [Hausdorff_edist_comm, add_comm]
end
...
calc Hausdorff_edist s t = 0 ↔ s ⊆ closure t ∧ t ⊆ closure s :
  by simv only [Hausdorff_edist_def, ennreal.sup_eq_zero, ennreal.supr_eq_zero,
...
lemma inf_dist_nonneg : 0 ≤ inf_dist x s := by simv [inf_dist]
...
by simv [inf_edist_zero_of_mem h, inf_dist]

/-- The minimal distance to a singleton is the distance to the unique point in this singleton -/
@[simp] lemma inf_dist_singleton : inf_dist x {y} = dist x y :=
by simv [inf_dist, inf_edist, dist_edist]
...
begin
  ...
  { rw [inf_dist, inf_dist, dist_edist,
    ...
    { simv [ennreal.add_eq_top, inf_edist_ne_top hs, edist_ne_top] }}
end
...
by simv [inf_dist, inf_edist_closure]
...
by simv [mem_closure_iff_inf_edist_zero, inf_dist, ennreal.to_real_eq_zero_iff, inf_edist_ne_top h]
...
begin
  rw ← not_iff_not,
  push_neg,
  simv [h.mem_iff_inf_dist_zero hs, le_antisymm_iff, inf_dist_nonneg],
end

/-- The infimum distance is invariant under isometries -/
lemma inf_dist_image (hΦ : isometry Φ) :
  inf_dist (Φ x) (Φ '' t) = inf_dist x t :=
by simv [inf_dist, inf_edist_image hΦ]
...
by simv [Hausdorff_dist, Hausdorff_edist_comm]
...
begin
  ...
  { simv [h] },
  { simv [Hausdorff_dist, Hausdorff_edist_empty h] }
end

/-- The Hausdorff distance to the empty set vanishes (if you want to have the more reasonable
value ∞ instead, use `Hausdorff_edist`, which takes values in ℝ≥0∞) -/
@[simp] lemma Hausdorff_dist_empty' : Hausdorff_dist ∅ s = 0 :=
by simv [Hausdorff_dist_comm]
...
begin
  ...
  { simv only [hs, ht, Hausdorff_dist_empty, inf_dist_empty, zero_add] },
...
by simv [Hausdorff_dist, Hausdorff_edist_image h]
...
begin
  ...
  { calc Hausdorff_dist s u = 0 + 0 : by simv [Hausdorff_dist, h]
  ...
  { have Dtu : Hausdorff_edist t u < ⊤ := calc
      ...
      ... = Hausdorff_edist s t + Hausdorff_edist s u : by simv [Hausdorff_edist_comm]
    ...
    { simv [ennreal.add_eq_top, lt_top_iff_ne_top.1 Dtu, fin] }}
end
...
by simv [Hausdorff_edist_zero_iff_closure_eq_closure.symm, Hausdorff_dist,
...
by simv [←Hausdorff_edist_zero_iff_eq_of_closed hs ht, Hausdorff_dist,
         ennreal.to_real_eq_zero_iff, fin]
...
by simv only [thickening, set_of_false, inf_edist_empty, not_top_lt]
...
begin
  ...
  { intros z,
    rw dist_edist,
    have d_lt_top : edist x z < ∞, by simv only [edist_dist, ennreal.of_real_lt_top],
...
by { ext, simv [mem_thickening_iff] }
...
by simv only [cthickening, ennreal.of_real_ne_top, set_of_false, inf_edist_empty, top_le_iff]

lemma cthickening_of_nonpos {δ : ℝ} (hδ : δ ≤ 0) (E : set α) :
  cthickening δ E = closure E :=
by { ext x, simv [mem_closure_iff_inf_edist_zero, cthickening, ennreal.of_real_eq_zero.2 hδ] }
...
by { ext y, simv [cthickening, edist_dist, ennreal.of_real_le_of_real_iff hδ] }
...
begin
  ...
  { simv only [closed_ball_eq_empty.mpr hδ, empty_subset] },
  { simv only [cthickening_singleton x hδ] }
end
...
begin
  ...
  { unfold thickening cthickening,
    intros x hx,
    simv only [mem_Inter, mem_set_of_eq] at *,
...
begin
  ...
  { simv only [preimage, mem_singleton_iff] },
...
begin
  ...
  { simv only [cthickening_empty, Union_false, Union_empty] },
...
begin
  ...
  { simv only [thickening_of_nonpos hε, empty_subset] },
  obtain hδ | hδ := le_total δ 0,
  { simv only [thickening_of_nonpos hδ, thickening_empty, empty_subset] },
  intros x,
...
begin
  obtain hδ | hδ := le_total δ 0,
  { simv only [thickening_of_nonpos hδ, cthickening_empty, empty_subset] },
  intro x,
