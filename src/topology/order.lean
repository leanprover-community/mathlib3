...
/-- Consider a set of sets that

- is closed under intersections: `s ∈ g → t ∈ g → s ∩ t ∈ g`;
- covers the whole space: `(⋃₀ g) = set.univ`.

Then a set is open in the topology generated by this set if and only if it is a union of a set of
generators. -/
...
begin
  ...
  { intro h, induction h,
    case basic : t ht
    { exact ⟨{t}, singleton_subset_iff.2 ht, (sUnion_singleton _).symm⟩ },
    case univ { exact ⟨g, subset.rfl, hU.symm⟩ },
    case inter : S T hS' hT' hS hT
    { rcases ⟨hS, hT⟩ with ⟨⟨S, hS, rfl⟩, T, hT, rfl⟩,
      refine ⟨image2 (∩) S T, image2_subset_iff.2 $ λ x hx y hy, hg _ (hS hx) _ (hT hy), _⟩,
      simp only [image2_eq_Union, sUnion_Union, sUnion_singleton, ← inter_Union₂,
        ← sUnion_eq_bUnion, ← Union₂_inter], },
    case sUnion : S hS' hS
    { choose! T hT using hS,
      refine ⟨⋃ t ∈ S, T t, Union₂_subset $ λ x hx, (hT x hx).1, _⟩,
      simp only [sUnion_Union, @sUnion_eq_bUnion _ S],
      exact Union₂_congr (λ x hx, (hT x hx).2) } },
  { rintro ⟨T, hT, rfl⟩,
    exact @is_open_sUnion _ (generate_from g) _ (λ t ht, is_open_generate_from_of_mem (hT ht)) }
end
