...
lemma preimage_neg : preimage (has_neg.neg : α → α) = image (has_neg.neg : α → α) :=
(image_eq_preimage_of_inverse neg_neg neg_neg).symm

lemma filter.map_neg_eq_comap_neg : map (has_neg.neg : α → α) = comap has_neg.neg :=
funext $ λ f, map_eq_comap_of_inverse (funext neg_neg) (funext neg_neg)
...
section preorder
variables [topological_space α] [preorder α] [t : order_closed_topology α] {a b : α}
include t

instance {p : α → Prop} : order_closed_topology (subtype p) :=
order_closed_topology.mk $ t.is_closed_le'.preimage $
  continuous_subtype_coe.prod_map $ continuous_subtype_coe
...
lemma is_closed_le' (a : α) : is_closed {b | b ≤ a} := is_closed_le continuous_id continuous_const
lemma is_closed_ge' (a : α) : is_closed {b | a ≤ b} := is_closed_le continuous_const continuous_id

lemma is_closed_Iic : is_closed (Iic a) := is_closed_le' a
lemma is_closed_Ici : is_closed (Ici a) := is_closed_ge' a
lemma is_closed_Icc : is_closed (Icc a b) := is_closed_Ici.inter is_closed_Iic

@[simp] lemma closure_Iic (a : α) : closure (Iic a) = Iic a := is_closed_Iic.closure_eq
@[simp] lemma closure_Ici (a : α) : closure (Ici a) = Ici a := is_closed_Ici.closure_eq
@[simp] lemma closure_Icc (a b : α) : closure (Icc a b) = Icc a b := is_closed_Icc.closure_eq
...
lemma le_of_tendsto {f : β → α} {x : filter β}
...
lemma ge_of_tendsto {f : β → α} {x : filter β} [ne_bot x]
...
lemma nhds_within_Ici_ne_bot (H₂ : a ≤ b) : ne_bot (𝓝[Ici a] b) :=nhds_within_ne_bot_of_mem H₂

@[instance] lemma nhds_within_Ici_self_ne_bot (a : α) : ne_bot (𝓝[≥] a) :=
nhds_within_Ici_ne_bot (le_refl a)

lemma nhds_within_Iic_ne_bot (H : a ≤ b) :
  ne_bot (𝓝[Iic b] a) :=
...
variables [topological_space α] [linear_order α] [order_closed_topology α] {a b : α}
...
lemma is_open_Iio : is_open (Iio a) := is_open_lt continuous_id continuous_const
lemma is_open_Ioi : is_open (Ioi a) := is_open_lt continuous_const continuous_id
lemma is_open_Ioo : is_open (Ioo a b) := is_open_Ioi.inter is_open_Iio

@[simp] lemma interior_Ioi : interior (Ioi a) = Ioi a := is_open_Ioi.interior_eq
@[simp] lemma interior_Iio : interior (Iio a) = Iio a := is_open_Iio.interior_eq
@[simp] lemma interior_Ioo : interior (Ioo a b) = Ioo a b := is_open_Ioo.interior_eq

lemma Ioo_subset_closure_interior : Ioo a b ⊆ closure (interior (Ioo a b)) :=
by simp only [interior_Ioo, subset_closure]

lemma Iio_mem_nhds (h : a < b) : Iio b ∈ 𝓝 a :=
is_open.mem_nhds is_open_Iio h

lemma Ioi_mem_nhds (h : a < b) : Ioi a ∈ 𝓝 b :=
...
@[simp]
lemma continuous_within_at_Ioo_iff_Ioi [topological_space β] {f : α → β} (h : a < b) :
...
@[simp] lemma nhds_within_Ico_eq_nhds_within_Iio (h : a < b) : 𝓝[Ico a b] b = 𝓝[<] b :=
by simpa only [dual_Ioc] using nhds_within_Ioc_eq_nhds_within_Ioi h.dual

@[simp] lemma nhds_within_Ioo_eq_nhds_within_Iio (h : a < b) : 𝓝[Ioo a b] b = 𝓝[<] b :=
by simpa only [dual_Ioo] using nhds_within_Ioo_eq_nhds_within_Ioi h.dual

@[simp] lemma continuous_within_at_Ico_iff_Iio {f : α → γ} (h : a < b) :
...
@[simp] lemma continuous_within_at_Ioo_iff_Iio {f : α → γ} (h : a < b) :
...
@[simp]
lemma continuous_within_at_Ico_iff_Ici [topological_space β] {f : α → β} (h : a < b) :
...
@[simp] lemma nhds_within_Icc_eq_nhds_within_Iic (h : a < b) :
  𝓝[Icc a b] b = 𝓝[≤] b :=
by simpa only [dual_Icc] using nhds_within_Icc_eq_nhds_within_Ici h.dual

@[simp] lemma nhds_within_Ioc_eq_nhds_within_Iic (h : a < b) :
  𝓝[Ioc a b] b = 𝓝[≤] b :=
by simpa only [dual_Ico] using nhds_within_Ico_eq_nhds_within_Ici h.dual

@[simp]
lemma continuous_within_at_Icc_iff_Iic [topological_space β] {f : α → β} (h : a < b) :
...
@[simp]
lemma continuous_within_at_Ioc_iff_Iic [topological_space β] {f : α → β} (h : a < b) :
...
section partial_order
variables [topological_space α] [partial_order α] [t : order_topology α] {a b : α}
include t
...
lemma lt_mem_nhds (h : a < b) : ∀ᶠ x in 𝓝 b, a < x := is_open.mem_nhds (is_open_lt' _) h

lemma le_mem_nhds (h : a < b) : ∀ᶠ x in 𝓝 b, a ≤ x :=
(𝓝 b).sets_of_superset (lt_mem_nhds h) $ assume b hb, le_of_lt hb

lemma gt_mem_nhds (h : a < b) : ∀ᶠ x in 𝓝 a, x < b := is_open.mem_nhds (is_open_gt' _) h

lemma ge_mem_nhds (h : a < b) : ∀ᶠ x in 𝓝 a, x ≤ b :=
...
lemma tendsto_order {f : β → α} {x : filter β} :
