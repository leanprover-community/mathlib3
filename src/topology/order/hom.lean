...
instance [preorder α] [preorder β] [topological_space α] [topological_space β]
  [continuous_order_hom_class F α β] : has_coe_t F (α →Co β) :=
...
/-- Copy of a `continuous_order_hom` with a new `to_fun` equal to the old one. Useful to fix
definitional
equalities. -/
protected def copy (f : α →Co β) (f' : α → β) (h : f' = f) : α →Co β :=
{ ..f.to_order_hom.copy f' h, ..f.to_continuous_map.copy f' h  }
