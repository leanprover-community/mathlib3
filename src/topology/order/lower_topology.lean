...
@[simp] lemma set_rw (Q : set α) : with_lower_topology.of_lower ⁻¹' (with_lower_topology.to_lower ⁻¹' Q) = Q := rfl
...
lemma test [preorder α] : (generate_from {s | ∃ (a : α), (Ici a)ᶜ = s}).is_open = (with_lower_topology.topological_space α).is_open :=
rfl
...
#check (with_lower_topology.topological_space α).is_open
...
lemma is_closed_upper_closure' (F : set (with_lower_topology α)) (h : F.finite) :
  is_closed (upper_closure F : set (with_lower_topology α)) := is_closed_upper_closure F h

lemma is_open_upper_closure_compl' (F : set (with_lower_topology α)) (h : F.finite) :
  is_open ((upper_closure F)ᶜ : set (with_lower_topology α)) :=
begin
  rw is_open_compl_iff,
  exact is_closed_upper_closure F h,
end
...
#check (generate_from {s : set (α × β) | ∃ (a : α) (b : β), (Ici (a, b))ᶜ = s}).

#check (with_lower_topology (α × β))

#check lower_topology.is_closed_upper_closure

variable (S : set α)

#check with_lower_topology.of_lower ⁻¹'  S

#check is_open_compl_iff

variables [order_bot α] [order_bot β] (F₂ : set β)

#check {(⊥ : α)} ×ˢ F₂

def test3 : preorder (α × β) :=
begin
  exact prod.preorder α β
end



def test4 : lower_topology (with_lower_topology (α × β)) :=
begin
  exact with_lower_topology.lower_topology (α × β)
end
...
#check with_lower_topology.of_lower ⁻¹'  P

lemma test2  (P : set (α × β)) [h: P.finite] : is_closed ( with_lower_topology.of_lower ⁻¹' (upper_closure P).carrier ) :=
begin
  apply lower_topology.is_closed_upper_closure,
  exact h,
end

#check with_lower_topology.of_lower ⁻¹' ((upper_closure P).carrier)ᶜ

lemma test5  (P : set (α × β)) [h: P.finite] : is_open ( with_lower_topology.of_lower ⁻¹' ((upper_closure P).carrier))ᶜ :=
begin
  rw is_open_compl_iff,
  apply lower_topology.is_closed_upper_closure,
  exact h,
...
#check (with_lower_topology.set_rw P)

lemma test5' (P : set (α × β)) [h: P.finite] : is_closed ( ((upper_closure P).carrier)) :=
begin
  rw ← (with_lower_topology.set_rw ( ((upper_closure P).carrier))),
  apply lower_topology.is_closed_upper_closure _ _,
  --convert lower_topology.is_closed_upper_closure _ _,
  exact h,
end
-/

lemma test6 (S : set (α × β)) :
  (generate_from {s : set (α × β) | ∃ (a : α × β), (Ici a)ᶜ = s}).is_open S =
  is_open (with_lower_topology.of_lower ⁻¹' S) := rfl



--((upper_closure (((univ : set α) ×ˢ F₂)).compl) : set (with_lower_topology (α × β)))

--#check (generate_from.is_open_compl_iff {s : set (α × β) | ∃ (p : α × β), (Ici p)ᶜ = s})
