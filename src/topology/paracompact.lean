...
begin
  ...
  set K : ℕ → set X := λ n, nat.cases_on n ∅ (λ n, nat.cases_on n ∅ (compact_exhaustion X)),
  -- Now we restate properties of `compact_covering X` for `K`
  have hKc : ∀ n, is_compact (K n),
  { rintro (_|_|n); simp [K, is_compact_compact_exhaustion] },
  have hKsub : ∀ n, K n ⊆ interior (K (n + 1)),
  { rintro (_|_|n); simp [K, compact_exhaustion_subset_interior_succ] },
  have hKsub' : ∀ n, K n ≤ K (n + 1),
    from λ n, subset.trans (hKsub n) interior_subset,
  have hKcov : ∀ x, ∃ n, x ∈ K (n + 2) \ K (n + 1),
  { intro x,
    have : ∃ n, x ∈ K (n + 2) := Union_eq_univ_iff.1 (Union_compact_exhaustion X) x,
