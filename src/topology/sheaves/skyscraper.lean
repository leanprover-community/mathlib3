...
variables [Π (U : opens X), decidable (p₀ ∈ U)] [has_colimits C]
...
noncomputable lemma skyscraper_presheaf_cocone_of_mem_closure₀_is_colimit {y : X} (h : p₀ ⤳ y) :
  is_colimit (skyscraper_presheaf_cocone_of_mem_closure₀ p₀ S h) :=
{ desc := λ c, (skyscraper_presheaf_of_mem_closure₀_from p₀ S h ≫ colimit.desc _ _ : S ⟶ c.X),
  fac' := λ c U,
  begin
    dsimp,
...
noncomputable def skyscraper_presheaf_stalk_of_mem_closure₀ {y : X} (h : p₀ ⤳ y) :
  (skyscraper_presheaf p₀ S).stalk y ≅ S :=
...
-/
@[simps] def skyscraper_presheaf_cocone_of_not_mem_closure₀ {y : X} (h : ¬p₀ ⤳ y) :
  cocone ((open_nhds.inclusion y).op ⋙ skyscraper_presheaf p₀ S) :=
{ X := terminal C,
  ι :=
...
noncomputable def skyscraper_presheaf_of_not_mem_closure₀_from
  {y : X} (h : ¬p₀ ⤳ y) :
  terminal C ⟶ (skyscraper_presheaf p₀ S).stalk y :=
...
noncomputable lemma skyscraper_presheaf_cocone_of_not_mem_closure₀_is_colimit
  {y : X} (h : ¬p₀ ⤳ y) :
  is_colimit (skyscraper_presheaf_cocone_of_not_mem_closure₀ p₀ S h) :=
{ desc := λ c, (eq_to_hom ((skyscraper_presheaf_obj_of_not_mem _
  ...
  begin
    ...
    {
      have eq1 : terminal.from (ite (p₀ ∈ (open_nhds.inclusion y).obj (unop U)) S (terminal C)) =
        eq_to_hom (if_neg h') ≫ 𝟙 (terminal C) := terminal_is_terminal.hom_ext _ _,
      have eq2 : terminal.from (ite (p₀ ∈ (open_nhds.inclusion y).obj ⊤) S (terminal C)) =
      ...
      have eq3 : terminal.from (ite (p₀ ∈ (open_nhds.inclusion y).obj
        ...
        eq_to_hom (if_neg (mem_nhds_of_not_mem_closure_singleton p₀ h).some_spec) ≫ 𝟙 (terminal C):=
      ...
      { symmetry,
        ...
        rw [show terminal.from (ite (p₀ ∈ (open_nhds.inclusion y).obj h₁.some) S (terminal C)) =
          eq_to_hom (if_neg temp) ≫ 𝟙 (terminal C), from terminal_is_terminal.hom_ext _ _,
          category.comp_id, category.comp_id, eq_to_hom_trans] at e,
      ...
      simp only [eq_coe],
      rw [←this, ←category.assoc, eq_to_hom_trans, eq_to_hom_refl, category.comp_id, comp_dite],
      clear this,
      generalize_proofs,
      have h₄ : p₀ ∉ (open_nhds.inclusion y).obj (h₁.some ⊓ unop U) := λ r, h' r.2,
      split_ifs,
      ...
      rw [dite_comp, category.comp_id] at this,
      split_ifs at this,
      rw [←this, eq1, category.comp_id, eq_to_hom_trans, ←category.assoc, eq_to_hom_trans], }
  end,
  uniq' := λ c f H,
  begin
