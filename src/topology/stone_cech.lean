...
/-- Up to isomorphism, there can only be one space `α` is densely embedded in, that satisfies
  `stone_cech_extension_property`. -/
def stone_cech_extension_property_unique
  {e₁ : α → δ₁} {e₂ : α → δ₂} (emb₁ : dense_embedding e₁) (emb₂ : dense_embedding e₂)
  (he₁ : stone_cech_extension_property e₁) (he₂ : stone_cech_extension_property e₂) : δ₁ ≃ₜ δ₂ :=
begin
  obtain ⟨c₁, d₁, c₂, d₂⟩ : continuous e₁ ∧ dense_range e₁ ∧ continuous e₂ ∧ dense_range e₂,
  { rcases emb₁ with ⟨⟨i₁, d₁⟩, _⟩, rcases emb₂ with ⟨⟨i₂, d₂⟩, _⟩,
    exact ⟨inducing.continuous i₁, d₁, inducing.continuous i₂, d₂⟩, },
  specialize he₁ e₂ c₂, choose ext₁ hext₁' using he₁, rcases hext₁' with ⟨⟨c_ext₁, hext₁⟩, _⟩,
  specialize he₂ e₁ c₁, choose ext₂ hext₂' using he₂, rcases hext₂' with ⟨⟨c_ext₂, hext₂⟩, _⟩,
  exact { continuous_to_fun := c_ext₁,
          ...
          { to_fun := ext₁,
            ...
            left_inv := begin
              ...
              { rw eq_on, intros x hx, simp at *, rcases hx with ⟨y, rfl⟩,
                rw [← function.comp_apply ext₁ e₁, ← hext₁, ← function.comp_apply ext₂ e₂],
                rw [ ← hext₂], },
