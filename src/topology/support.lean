...
lemma tsupport_mul_subset_left {α R : Type*} [topological_space α] [mul_zero_class R]
  {f g : α → R} : tsupport (λ x, f x * g x) ⊆ tsupport f :=
closure_mono (support_mul_subset_left _ _)

lemma tsupport_mul_subset_right {α R : Type*} [topological_space α] [mul_zero_class R]
  {f g : α → R} : tsupport (λ x, f x * g x) ⊆ tsupport g :=
