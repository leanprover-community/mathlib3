...
* `uniform_convergence.uniform_space`: uniform structure of uniform convergence. This is the
  ...
  We will denote this uniform space as `𝒰(α, β, uβ)`, both in the comments and as a local notation in
  the Lean code, where `uβ` is the uniform space structure on `β`.
...
* `uniform_convergence.t2_space`: the topology of uniform convergence on `α → β` is T2 if
  `β` is T2.
* `uniform_convergence.tendsto_iff_tendsto_uniformly`: `𝒰 α uβ` is
...
* `uniform_convergence_on.uniform_continuous_eval_of_mem` : evaluation at a point contained in a
  set of `𝔖` is uniformly continuous for `𝒱 𝔖 uβ`
...
* `uniform_convergence_on.tendsto_iff_tendsto_uniformly_on`:
  `𝒱 𝔖 uβ` is indeed the uniform structure of `𝔖`-convergence
