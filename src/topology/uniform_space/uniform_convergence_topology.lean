...
open set filter function topological_space
...
instance is_countably_generated_uniformity [(𝓤 β).is_countably_generated] :
  (𝓤 $ α →ᵤ β).is_countably_generated :=
begin
  rcases (𝓤 β).exists_antitone_basis with ⟨s, s_basis, -⟩,
  exact filter.has_countable_basis.is_countably_generated
    ⟨uniform_fun.has_basis_uniformity_of_basis α β s_basis, countable_univ⟩,
end
...
variables (α β) [uniform_space β] (𝔖 : set (set α)) {𝔗 : set (set α)}
...
/-!
One import fact about the `𝔖`-convergence uniform structure and topology, is that they
don't change if you replace `𝔖` by the set `𝔖'` of all `s : set α` which are contained in some
finite union of elements of `𝔖`. This `𝔖'` is the smallest ideal (in the sense of
`order.ideal`) containing `𝔖`, but in this context the litterature (e.g Bourbaki) often uses
"bornology" to mean `order.ideal (set α)`. Unfortunately, there is some disagreement on wether
...
form `{s | sᶜ ∈ F}` for `F : filter α`. We define an ad-hoc predicate `generate_same` to express
that two sets of subsets of `α` generate the same (noncovering) bornology, and we develop a minimal
...
/-- For `𝔖 𝔗 : set (set α)`, `generate_same 𝔖 𝔗` expresses the fact that `𝔖` and `𝔗` generate the
same *noncovering* bornology on `α` (i.e ideal of `set α`). We express this by saying that the
pointwise complements of `𝔖` and `𝔗` generate the same filter. See `generate_same_iff` for
a more concrete characterization. -/
def generate_same (𝔖 𝔗 : set (set α)) : Prop :=
generate (compl ⁻¹' 𝔖) = generate (compl ⁻¹' 𝔗)

/-- For every `𝔖`, there is a nonempty and directed `𝔗` which satisfies `generate_same 𝔗`. This
is essentially saying that any *noncovering* bornology admits a basis (in the dual sense of
`filter.has_basis`). We could even ask `𝔗` to be downwards-closed (i.e a full-blown *noncovering*
...
begin
  -- The proof is simple: the bornology generated by `𝔖` works.
...
/-- Key part of proving `uniform_on_fun_filter_generate`: given any uniform structure `u` on
    `α → β`, the set of all `S : set α` for which the restriction map from `(α → β, u)` → `S →ᵤ β`
    is uniformly continuous is a *noncovering* bornology on `α`.
...
/-- Replacing `𝔖` by the *noncovering* bornology generated by `𝔖` doesn't change the uniform
structure of `𝔖`-convergence. -/
...
begin
  ...
  ... ≤ 𝒱(α, β, {s | sᶜ ∈ filter.generate (compl ⁻¹' 𝔖)}, _) :
    infi_le_infi_of_subset (λ s hs, _),
...
/-- If `𝔖` and `𝔗` generate the same *noncovering* bornology (in the sense of
    `uniform_on_fun.generate_same`), then the uniform structures of `𝔖`-convergence and
    `𝔗`-convergence are equal. -/
lemma generate_same.uniform_space_eq {𝔖 𝔗 : set (set α)} (H : generate_same 𝔖 𝔗) :
  𝒱(α, β, 𝔖, _) = 𝒱(α, β, 𝔗, _) :=
by rw [← @uniform_on_fun_filter_generate _ _ _ 𝔖, ← @uniform_on_fun_filter_generate _ _ _ 𝔗,
        (show _ = _, from H)]
...
protected lemma has_basis_uniformity_of_basis (h𝔖𝔗 : generate_same 𝔖 𝔗) (h : 𝔗.nonempty)
  (h' : directed_on (⊆) 𝔗) {p : ι → Prop} {s : ι → set (β × β)} (hb : has_basis (𝓤 β) p s) :
  (𝓤 (α →ᵤ[𝔖] β)).has_basis
...
begin
  ...
  exact has_basis_binfi_of_directed h (λ S, (uniform_on_fun.gen _ S) ∘ s) _
    (λ S hS, uniform_on_fun.has_basis_uniformity_of_basis_aux₁ α β _ hb S)
    (uniform_on_fun.has_basis_uniformity_of_basis_aux₂ α β _ h' hb)
end
...
protected lemma has_basis_uniformity (h𝔖𝔗 : generate_same 𝔖 𝔗) (h : 𝔗.nonempty)
  (h' : directed_on (⊆) 𝔗) :
  (𝓤 (α →ᵤ[𝔖] β)).has_basis
    (λ SV : set α × set (β × β), SV.1 ∈ 𝔗 ∧ SV.2 ∈ 𝓤 β)
    (λ SV, uniform_on_fun.gen 𝔖 SV.1 SV.2) :=
uniform_on_fun.has_basis_uniformity_of_basis α β 𝔖 h𝔖𝔗 h h' (𝓤 β).basis_sets
...
`𝓑` of `𝓤 β`. -/
protected lemma has_basis_nhds_of_basis (f : α →ᵤ[𝔖] β) (h𝔖𝔗 : generate_same 𝔖 𝔗) (h : 𝔗.nonempty)
  (h' : directed_on (⊆) 𝔗) {p : ι → Prop} {s : ι → set (β × β)} (hb : has_basis (𝓤 β) p s) :
  (𝓝 f).has_basis
    (λ Si : set α × ι, Si.1 ∈ 𝔗 ∧ p Si.2)
...
begin
  ...
  exact nhds_basis_uniformity (uniform_on_fun.has_basis_uniformity_of_basis α β 𝔖 h𝔖𝔗 h h' hb)
end

/-- For `f : α →ᵤ[𝔖] β`, where `𝔖 : set (set α)` is nonempty and directed, `𝓝 f` admits the
family `{g | ∀ x ∈ S, (f x, g x) ∈ V}` for `S ∈ 𝔖` and `V ∈ 𝓤 β` as a filter basis. -/
protected lemma has_basis_nhds (f : α →ᵤ[𝔖] β) (h𝔖𝔗 : generate_same 𝔖 𝔗) (h : 𝔗.nonempty)
  (h' : directed_on (⊆) 𝔗) :
  (𝓝 f).has_basis
    (λ SV : set α × set (β × β), SV.1 ∈ 𝔗 ∧ SV.2 ∈ 𝓤 β)
...
begin
  ...
  letI : countable 𝔗 := hcount.to_subtype,
  refine @infi.is_countably_generated _ _ _ _ (λ T, _),
  change (comap _ (𝓤 $ (T : set α) →ᵤ β)).is_countably_generated,
  apply_instance
end
...
protected lemma le_with_closed_basis (h𝔖₁ : 𝔖.nonempty) (h𝔖₂ : directed_on (⊆) 𝔖) :
...
begin
  ...
  { rintros ⟨S, V⟩ ⟨hS, hV, V_closed⟩,
    letI t' := u'.to_topological_space,
    rw [uniform_on_fun.gen_eq_bInter],
    refine is_closed_bInter (λ x hx, V_closed.preimage (continuous.prod_map _ _));
...
begin
  ...
  rw h𝔖𝔗.uniform_space_eq,
  refine (uniform_on_fun.le_with_closed_basis β 𝔗 h𝔗₁ h𝔗₂).complete_space _,
  ...
  have : @@uniform_inducing u' _ ((⋃₀ 𝔗).restrict : (α →ᵤ[𝔗] β) → ⋃₀ 𝔗 → β),
  { split,
    simp_rw [Pi.uniformity, infi_uniformity', uniformity_comap rfl, comap_infi, comap_comap,
              infi_subtype, ← infi_sUnion],
    refl },
  rw [@@complete_space_iff_is_complete_range u' _ _ this, surjective.range_eq _],
  { exact complete_space_iff_is_complete_univ.mp (Pi.complete _) },
  { exact subtype.coe_injective.surjective_comp_right }
end
