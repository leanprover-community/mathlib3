...
@TODO: simplify terms generated by denominator elimination
-/
...
lemma mul_subst {α} [comm_ring α] {n1 n2 k e1 e2 t1 t2 : α} (h1 : n1 * e1 = t1) (h2 : n2 * e2 = t2) (h3 : n1*n2 = k): 
      k * (e1 * e2) = t1 * t2 := 
...
do tp ← infer_type h,
  ...
  if c = 0 then 
    do e' ← to_expr ``(zero_mul %%e), return (eq, e')
  else      
    do nm ← resolve_name (ineq_const_mul_nm iq), 
       e' ← to_expr ``(%%nm.mk_explicit _ _ _ %%(nat.to_pexpr c) %%h (by norm_num)) ff,
       return (iq, e')
...
open tree
meta def mk_prod_prf : ℕ → tree ℕ → expr → tactic pexpr
| v (node _ lhs rhs) `(%%e1 + %%e2) := 
  do v1 ← mk_prod_prf v lhs e1, v2 ← mk_prod_prf v rhs e2, return ``(add_subst %%v1 %%v2)
| v (node _ lhs rhs) `(%%e1 - %%e2) := 
  do v1 ← mk_prod_prf v lhs e1, v2 ← mk_prod_prf v rhs e2, return ``(sub_subst %%v1 %%v2)
| v (node n lhs@(node ln _ _) rhs) `(%%e1 * %%e2) := 
  do tp ← infer_type e1, v1 ← mk_prod_prf ln lhs e1, v2 ← mk_prod_prf (v/ln) rhs e2, npf ← to_expr ``(by norm_num : %%(ln.to_pexpr) * %%((v/ln).to_pexpr) = (%%(v.to_pexpr) : %%tp)), 
     return ``(mul_subst %%v1 %%v2 %%npf)
| v (node n lhs rhs@(node rn _ _)) `(%%e1 / %%e2) := 
  do tp ← infer_type e1, v1 ← mk_prod_prf (v/rn) lhs e1, npf ← to_expr ``(by norm_num : %%(rn.to_pexpr) / %%e2 = %%((1 : ℕ).to_pexpr)), npf2 ← to_expr ``(by norm_num : %%((v/rn).to_pexpr) * %%(n.to_pexpr) = (%%(v.to_pexpr) : %%tp)),
     return ``(div_subst %%v1 %%npf %%npf2)
| v t `(-%%e) := do v' ← mk_prod_prf v t e, return ``(neg_subst %%v')
| v _ e := do tp ← infer_type e, return ``(refl ((%%(v.to_pexpr) : %%tp) * %%e)) 
...
do pe ← mk_prod_prf n t e,  e' ← to_expr pe,  return (n, e')
...
do htp ← infer_type h,
   ...
   if expr_contains `has_div.div lhs then 
     ...
     (_, h'') ← mk_single_comp_zero_pf v h', 
