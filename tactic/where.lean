...
strip_pi_binders <$> (mk_const n >>= infer_type)
...
order_by (λ v u, binder_less_important v.2.1 u.2.1) $ inflate l

meta def format_variable : expr × binder_info × list name → tactic string
| (e, bi, ns) := do let (l, r) := binder_brackets bi,
                    e ← pp e,
                    let ns := string.intercalate " " $ ns.map to_string,
                    return sformat!"{l}{ns} : {e}{r}"

meta def compile_variable_list (l : list (name × binder_info × expr)) : tactic string :=
string.intercalate " " <$> (sort_variable_list l).mmap format_variable
...
do l ← get_includes_core f,
   let str := string.intercalate " " $ l.map $ λ n, to_string n.1,
