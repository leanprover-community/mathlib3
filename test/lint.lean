...
/-- testing a declaration that has sorry -/
def test_has_sorry (n m : ℕ) : ℕ := m + sorry
...
run_cmd do
  ...
  d ← get_decl `test_has_sorry,
  -- even though `test_has_sorry` is in the list, it is not flagged by the linter
  none ← linter.unused_arguments.test d,
  guard (l2.length = 5) <|> fail "wrong length",
  ...
  guard ((⟨`test_has_sorry, [1]⟩ : t) ∈ l2) <|> fail "sorry",
