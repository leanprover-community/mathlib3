...
show n - 1 - m = n - m - 1, by {remove_subs! none, }
...
by {remove_subs "h"; trivial }

example {a b c : ℕ} : a + b + c = a + b + c :=
begin
  success_if_fail_with_msg {remove_subs none} "no ℕ-subtraction found",
  refl
end
...
by remove_subs none; exact or.inl rfl

example {R} [ring R] {a b c d : ℕ} : ite (a - b = a - b) ((a : R) - b = a - b) (a - b = c - d) :=
by remove_subs none; rw if_pos rfl

example (a b : ℕ) : min a b + (a - b) = a :=
by remove_subs none; simp; assumption
