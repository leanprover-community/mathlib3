...
lemma top_eq_univ : (⊤ : set α) = univ := by timetac "top_eq_univ" refl
lemma bot_eq_empty : (⊥ : set α) = ∅ :=  by timetac "bot_eq_empty" refl
lemma sup_eq_union (s t : set α) : s ⊔ t = s ∪ t := by timetac "sup_eq_union" refl
lemma inf_eq_inter (s t : set α) : s ⊓ t = s ∩ t := by timetac "inf_eq_inter" refl
lemma le_eq_subset (s t : set α) : s ≤ t = (s ⊆ t) := by timetac "foo" refl
...
theorem set_of_set {s : set α} : set_of s = s := by timetac "set_of_set" refl

lemma sep_set_of {p q : α → Prop} : {a ∈ {a | p a } | q a} = {a | p a ∧ q a} :=
by timetac "sep_set_of" refl

lemma set_of_and {p q : α → Prop} : {a | p a ∧ q a} = {a | p a} ∩ {a | q a} :=
by timetac "set_of_and" refl

lemma set_of_or {p q : α → Prop} : {a | p a ∨ q a} = {a | p a} ∪ {a | q a} :=
by timetac "set_of_or" refl

theorem empty_def : (∅ : set α) = {x | false} := by timetac "empty_def" refl

theorem mem_empty_eq (x : α) : x ∈ (∅ : set α) = false := by timetac "mem_empty_eq" refl

theorem set_of_false : {a : α | false} = ∅ := by timetac "set_of_false" refl

theorem set_of_true : {x : α | true} = univ := by timetac "set_of_true" refl

theorem union_def {s₁ s₂ : set α} : s₁ ∪ s₂ = {a | a ∈ s₁ ∨ a ∈ s₂} :=
by timetac "union_def" refl

theorem union_self (a : set α) : a ∪ a = a := (ext $ λ x, or_self _) 

theorem union_empty (a : set α) : a ∪ ∅ = a := (ext $ λ x, or_false _)

theorem empty_union (a : set α) : ∅ ∪ a = a := ext $ λ x, false_or _
...
lemma union_univ {s : set α} : s ∪ univ = univ := sup_top_eq

lemma univ_union {s : set α} : univ ∪ s = univ := top_sup_eq

theorem inter_def {s₁ s₂ : set α} : s₁ ∩ s₂ = {a | a ∈ s₁ ∧ a ∈ s₂} := rfl

theorem inter_self (a : set α) : a ∩ a = a := ext $ λ x, and_self _

theorem inter_empty (a : set α) : a ∩ ∅ = ∅ := ext $ λ x, and_false _

theorem empty_inter (a : set α) : ∅ ∩ a = ∅ := ext $ λ x, false_and _
