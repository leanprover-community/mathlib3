...
theorem continuous_compose {f : α → β} {g : β → γ} (hf : continuous f) (hg : continuous g):
  continuous (g ∘ f) :=
assume s h, hf _ (hg s h)

theorem continuous_iff_towards {f : α → β} :
...
theorem continuous_iff_induced_le {t₁ : tspace α} {t₂ : tspace β} :
...
theorem continuous_inf_dom {t₁ t₂ : tspace α} {t₃ : tspace β}
  (h₁ : cont t₁ t₃ f) (h₂ : cont t₂ t₃ f) : cont (t₁ ⊓ t₂) t₃ f :=
assume s h, ⟨h₁ s h, h₂ s h⟩

theorem continuous_inf_rng_left {t₁ : tspace α} {t₃ t₂ : tspace β}
  (h : cont t₁ t₂ f) : cont t₁ (t₂ ⊓ t₃) f :=
assume s hs, h s hs.left

theorem continuous_inf_rng_right {t₁ : tspace α} {t₃ t₂ : tspace β}
  (h : cont t₁ t₃ f) : cont t₁ (t₂ ⊓ t₃) f :=
assume s hs, h s hs.right

theorem continuous_Inf_dom {t₁ : set (tspace α)} {t₂ : tspace β}
...
theorem continuous_infi_dom {t₁ : ι → tspace α} {t₂ : tspace β}
...
theorem continuous_top {t : tspace β} : cont ⊤ t f :=
assume s h, trivial

theorem continuous_bot {t : tspace α} : cont t ⊥ f :=
continuous_Inf_rng (mem_univ $ coinduced f t) continuous_coinduced_rng

theorem continuous_sup_rng {t₁ : tspace α} {t₂ t₃ : tspace β}
  (h₁ : cont t₁ t₂ f) (h₂ : cont t₁ t₃ f) : cont t₁ (t₂ ⊔ t₃) f :=
continuous_Inf_rng
  (show t₂ ≤ coinduced f t₁ ∧ t₃ ≤ coinduced f t₁, from ⟨h₁, h₂⟩)
  continuous_coinduced_rng

theorem continuous_sup_dom_left {t₁ t₂ : tspace α} {t₃ : tspace β}
  (h : cont t₁ t₃ f) : cont (t₁ ⊔ t₂) t₃ f :=
continuous_Inf_dom $ assume t ⟨h₁, h₂⟩ s hs, h₁ _ $ h s hs

theorem continuous_sup_dom_right {t₁ t₂ : tspace α} {t₃ : tspace β}
  (h : cont t₂ t₃ f) : cont (t₁ ⊔ t₂) t₃ f :=
continuous_Inf_dom $ assume t ⟨h₁, h₂⟩ s hs, h₂ _ $ h s hs
...
theorem map_nhds_induced_eq {a : α} (h : image f univ ∈ (nhds (f a)).sets) :
...
theorem continuous_prod_mk {f : γ → α} {g : γ → β}
...
theorem open_set_prod {s : set α} {t : set β} (hs : open' s) (ht: open' t) :
  open' (set.prod s t) :=
open_inter (continuous_fst s hs) (continuous_snd t ht)

theorem prod_eq_generate_from [tα : topological_space α] [tβ : topological_space β] :
...
theorem nhds_prod_eq {a : α} {b : β} : nhds (a, b) = filter.prod (nhds a) (nhds b) :=
...
theorem continuous_inl : continuous (@sum.inl α β) :=
continuous_inf_rng_left continuous_coinduced_rng

theorem continuous_inr : continuous (@sum.inr α β) :=
continuous_inf_rng_right continuous_coinduced_rng

theorem continuous_sum_rec {f : α → γ} {g : β → γ}
...
theorem continuous_subtype_mk {f : β → α}
  (hp : ∀x, p (f x)) (h : continuous f) : continuous (λx, (⟨f x, hp x⟩ : subtype p)) :=
continuous_induced_rng h

theorem map_nhds_subtype_val_eq {a : α} {ha : p a} (h : {a | p a} ∈ (nhds a).sets) :
  map (@subtype.val α p) (nhds ⟨a, ha⟩) = nhds a :=
map_nhds_induced_eq (by simp [subtype.val_image, h])

theorem continuous_subtype_nhds_cover {f : α → β} {c : ι → α → Prop}
...
theorem nhds_pi {ι : Type u} {α : ι → Type v} [t : ∀i, topological_space (α i)] {a : Πi, α i} :
...
/-- Tychonoff's theorem -/
theorem compact_pi_infinite {ι : Type v} {α : ι → Type u} [∀i:ι, topological_space (α i)]
