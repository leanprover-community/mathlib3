...
@[simp] theorem dist_eq_zero_iff {x y : α} : dist x y = 0 ↔ x = y :=
iff.intro eq_of_dist_eq_zero (assume : x = y, this ▸ dist_self _)

@[simp] theorem zero_eq_dist_iff {x y : α} : 0 = dist x y ↔ x = y :=
...
theorem open_ball_subset_open_ball_of_le (h : ε₁ ≤ ε₂) : open_ball x ε₁ ⊆ open_ball x ε₂ :=
